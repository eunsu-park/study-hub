...
### 배치 부분 실패 (HTTP 207 Multi-Status)

단일 요청에서 여러 항목을 처리할 때(대량 생성, 대량 업데이트), 일부 작업은 성공하고 일부는 실패할 수 있습니다. 단일 최상위 상태 코드를 반환하면 정보가 손실됩니다. 대신 항목별 결과와 함께 HTTP 207 Multi-Status를 사용하세요:
...
전체 요청이 작업으로는 "성공"했지만 개별 항목에 혼합된 결과가 있을 때 207을 사용하세요. 처리가 시작되기 전에 전체 요청이 구조적으로 유효하지 않다면 207이 아닌 400 또는 422를 반환하세요.
