...
불투명 커서는 변조에 강해야 합니다. 커서를 수정하는 클라이언트는 내부 데이터를 노출하거나 서버 오류를 유발할 수 있습니다:
...
수백만 행에서 `SELECT COUNT(*)`는 데이터베이스가 일치하는 모든 행을 스캔해야 하므로 비용이 많이 듭니다. 대안:

- **카탈로그의 근사 개수** (PostgreSQL): `SELECT reltuples FROM pg_class WHERE relname = 'users'` — 통계 추정값을 반환하며, autovacuum에 의해 업데이트됩니다. 빠르지만 부정확할 수 있습니다 (수천 개 오차 가능).
- **주기적 갱신을 통한 캐시된 개수**: Redis에 개수를 저장하고, 삽입/삭제 시 업데이트하며, 몇 분마다 예약 작업으로 갱신합니다. 신선도와 비용의 균형을 맞춥니다.
- **총 개수 생략**: 일부 API(GitHub API 포함)는 `has_next_page`와 커서만 반환하며, 총 개수는 선택적이거나 전혀 제공하지 않습니다. 매우 큰 데이터셋에 가장 확장성 높은 접근 방식입니다.
...
| 전략 | 최적 사용 사례 | 일관성 | 성능 | 페이지 이동 지원 | 커서 조작 위험 |
|------|--------------|--------|------|----------------|---------------|
| 오프셋 | 소규모 데이터셋, 관리자 UI | 낮음 (페이지 드리프트) | 깊이에 따라 저하 | 예 | 해당 없음 |
| 커서 | 피드, 타임라인 | 높음 | 일정 | 아니오 | 예 (서명 필요) |
| 키셋 | 대규모 정렬 데이터셋 | 높음 | 일정 | 아니오 | 낮음 |
