...
**이전**: [에러 처리(Error Handling)](./08_Error_Handling.md) | **다음**: [API 문서화](./10_API_Documentation.md)
...
Redis를 사용할 수 없을 때, rate limiter는 카운터를 확인할 수 없습니다. 두 가지 실패 모드 중 하나를 선택해야 합니다:
...
async def check_rate_limit_fail_open(key: str, redis_client: redis.Redis) -> bool:
    """
    Fail-open: Redis가 다운되면 모든 요청을 허용합니다.

    적합한 경우: 공개 API, 읽기 중심 엔드포인트, 엄격한 적용보다
    가용성이 더 중요한 시나리오.
    위험: Redis 장애 시 일시적으로 모든 rate limiting 보호가 제거됩니다.
    """
    try:
...
async def check_rate_limit_fail_closed(key: str, redis_client: redis.Redis) -> bool:
    """
    Fail-closed: Redis가 다운되면 모든 요청을 거부합니다.

    적합한 경우: 결제 API, 인증 엔드포인트, 가용성보다
    보안이 더 중요한 모든 흐름.
    위험: Redis 장애 시 rate-limited 경로에 대한 전체 API 중단이 발생합니다.
    """
    try:
        count = await redis_client.incr(key)
        if count == 1:
...
fail-open과 fail-closed 중 선택은 기술적 결정이 아닌 비즈니스 결정입니다: 다운타임 비용(fail-open 위험)과 남용 비용(fail-closed 위험)을 비교하여 결정하세요.
...
**이전**: [에러 처리(Error Handling)](./08_Error_Handling.md) | [개요](./00_Overview.md) | **다음**: [API 문서화](./10_API_Documentation.md)
