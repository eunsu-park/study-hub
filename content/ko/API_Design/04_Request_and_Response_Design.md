...
DELETE를 구현하는 두 가지 전략이 있습니다:

```python
# 소프트 삭제: 레코드를 삭제하지 않고 삭제된 것으로 표시합니다.
# 장점: 감사 추적 보존, 실행 취소 가능, 참조 무결성 충족.
# 단점: 데이터가 여전히 저장소를 차지하며, GDPR "삭제권"에는 추가 작업이 필요합니다.
@app.delete("/api/books/{book_id}", status_code=204)
async def soft_delete_book(book_id: int):
    # UPDATE books SET is_deleted=True, deleted_at=NOW() WHERE id=:id
    return Response(status_code=204)

# 하드 삭제: 데이터베이스에서 행을 제거합니다.
# 장점: 저장소 확보, GDPR 삭제 요청을 자연스럽게 충족.
# 단점: 복구 불가; 처리하지 않으면 외래 키 참조가 손상됩니다.
@app.delete("/api/books/{book_id}", status_code=204)
...
**삭제 후 올바른 상태 코드 선택:**
- `404 Not Found` — 클라이언트 관점에서 리소스가 더 이상 존재하지 않습니다. 기본값으로 사용하세요; "요청한 것이 여기에 없다"는 것과 일관성이 있습니다.
- `410 Gone` — 리소스가 존재했지만 의도적으로 제거되었음을 명시적으로 알립니다. 클라이언트가 URL을 캐시했고 재시도를 중단하길 원할 때 410을 선호하세요 (예: 영구적으로 삭제된 공개 기사).
