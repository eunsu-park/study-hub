...
### Shadow API와 Zombie API

OWASP API9(부적절한 인벤토리 관리)에서 식별된 두 가지 일반적인 API 인벤토리 문제:

**Shadow API**는 실행 중인 서비스에 존재하지만 OpenAPI 명세에는 없는 문서화되지 않은 엔드포인트입니다. 문서화된 경로에 적용된 보안 제어(인증, rate limiting, 입력 검증)를 우회하는 경우가 많습니다.
...
**Zombie API**는 마이그레이션 기한이 훨씬 지난 후에도 여전히 트래픽을 받는 더 이상 사용되지 않는 API 버전입니다. 패치가 현재 버전에는 적용되지만 이전 버전에는 적용되지 않기 때문에 보안 부채가 누적됩니다.

완화 전략:
1. **Sunset 헤더**: 클라이언트가 프로그래밍 방식으로 경고를 받을 수 있도록 더 이상 사용되지 않는 엔드포인트의 응답에 `Sunset: Sat, 01 Jan 2026 00:00:00 GMT`와 `Deprecation: true`를 포함하세요.
2. **트래픽 모니터링**: 옵저빌리티 스택에서 API 버전별 요청 수를 추적하고, 마이그레이션 기한 이후에도 더 이상 사용되지 않는 버전에 상당한 트래픽이 있을 때 알림을 받으세요.
3. **강제 마이그레이션 기한**: 하드 컷오프 날짜를 발표하고, API 게이트웨이에서 더 이상 사용되지 않는 버전을 차단하며(410 Gone 반환), 마이그레이션 가이드를 제공하세요.
