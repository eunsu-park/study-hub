...
URL 문자열만 검증하는 것으로는 충분하지 않습니다. 공격자는 DNS 리바인딩을 사용하여 처음에는 공개 IP로 해석되지만 나중에 내부 IP로 전환되는 공개 호스트명을 등록할 수 있습니다. 항상 등록 시점에 DNS를 해석하고 해석된 주소를 확인하세요:
...
이 검사를 등록 시점에 적용하고, 등록과 전달 사이의 간격이 길 경우 전달 시점에도 선택적으로 재검증하세요.
