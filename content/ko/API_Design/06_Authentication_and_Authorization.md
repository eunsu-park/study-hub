...
PKCE(RFC 7636)는 `client_secret`을 기밀로 유지할 수 없는 SPA와 모바일 앱에 필수입니다. PKCE 없이는 인증 코드를 가로챈 공격자가 이를 토큰과 교환할 수 있습니다.
...
# Step 1 (클라이언트 측): 리다이렉트하기 전에 code_verifier와 code_challenge를 생성합니다.
code_verifier = secrets.token_urlsafe(64)  # 랜덤 문자열, 43-128자

# S256 방식: challenge = BASE64URL(SHA256(verifier))
digest = hashlib.sha256(code_verifier.encode()).digest()
code_challenge = base64.urlsafe_b64encode(digest).rstrip(b"=").decode()

# Step 2: 인증 요청 URL에 code_challenge + code_challenge_method=S256을 포함합니다.
# 인증 서버는 challenge를 저장하고 인증 코드와 연결합니다.

# Step 3 (토큰 교환): 인증 코드와 함께 code_verifier를 전송합니다.
# 인증 서버는 verifier를 해시하고 저장된 challenge와 비교합니다.
# 일치하면 토큰이 발급됩니다. 가로챈 코드 단독으로는 사용할 수 없습니다.
...
token_exchange_params = {
    ...
    "redirect_uri": "https://yourapp.com/callback",
    "client_id": "your_client_id",
    "code_verifier": code_verifier,  # 요청자가 challenge를 생성했음을 증명
    # client_secret 불필요 — verifier가 신원 증명 역할
}
```

### 비대칭 JWT 사용 시점 (RS256 / ES256)

대칭 JWT(`HS256`)는 서명과 검증에 동일한 시크릿을 사용합니다. 토큰을 검증해야 하는 모든 서비스가 시크릿을 공유해야 하므로 보안 위험이 있습니다.

비대칭 JWT가 이를 해결합니다:
- **인증 서버**는 **개인 키**로 토큰에 서명합니다 (비밀 유지).
- 모든 **다운스트림 서비스**는 대응하는 **공개 키**로 토큰을 검증합니다 (배포 안전).
...
# 리소스 서버: RSA 공개 키로 검증 (개인 키 불필요)
with open("public_key.pem", "rb") as f:
    public_key = serialization.load_pem_public_key(f.read())

payload = jwt.decode(token, public_key, algorithms=["RS256"])
# RS256과 동등한 보안에서 더 작은 토큰을 위해 ES256(ECDSA)를 사용합니다.
```

여러 독립 서비스가 시크릿을 공유하지 않고 토큰을 검증해야 할 때 `RS256`/`ES256`을 사용하세요 — 마이크로서비스 아키텍처와 멀티 테넌트 플랫폼에서 일반적입니다.
...
| 알고리즘 | 유형 | 키 | 최적 용도 | 보안 수준 |
|---------|------|-----|----------|----------|
| HS256 | 대칭 | 공유 시크릿 | 단일 서비스, 내부 API | 중간 |
| RS256 | 비대칭 | RSA 키 쌍 | 마이크로서비스, 공개 API | 높음 |
| ES256 | 비대칭 | ECDSA 키 쌍 | 모바일, 성능 중요 환경 | 높음 |
| EdDSA | 비대칭 | Ed25519 키 쌍 | 최신 시스템 | 매우 높음 |
