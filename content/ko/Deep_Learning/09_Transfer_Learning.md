...
전이학습이 작동하는 이유는 깊은 네트워크가 한 작업에서 학습한 특징이 다른 작업에서 부분적으로 재사용 가능하기 때문입니다. 이 섹션은 "부분적으로"를 형식화합니다: 어떤 특징이 전이되는지, 초기 층이 후기 층보다 더 잘 전이되는 이유, 그리고 일부 파라미터를 동결하고 다른 것을 학습할 때 그래디언트 수학이 무엇을 말하는지.

이 섹션에서 다루는 내용:

- **A.** 사전학습 특징이 전이되는 이유: 계층적 일반성
- **B.** 특징 추출 vs 파인튜닝: 그래디언트 그림
- **C.** 도메인 시프트와 전이학습이 실패할 때
- **D.** 층별 학습률과 차별적 파인튜닝
...
| 이론 개념 | 본 레슨의 코드 구성 |
|-----------|---------------------|
| 계층적 특징 일반성 | 어떤 층을 동결할지 선택 |
| 특징 추출 (업데이트 없음) | `param.requires_grad = False` |
| 더 작은 LR로 파인튜닝 | 백본에 `1e-2` 대신 `lr = 1e-4` |
| 차별적 LR | `torch.optim.X(...)`의 다중 파라미터 그룹 |
