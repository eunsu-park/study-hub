...
`\hat{x}`는 실제와 생성 샘플 사이의 보간으로 샘플링: `\hat{x} = \alpha x + (1 - \alpha) G(z)`, `\alpha ~ U[0, 1]`. 페널티는 그래디언트 노름을 (1 미만이 아닌) 1 쪽으로 밀음 — Wasserstein 쌍대의 최적 Lipschitz 함수가 거의 모든 곳에서 정확히 그래디언트 노름 1을 가지기 때문.
