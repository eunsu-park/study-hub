...
CIFAR-10에서 "실용적 이미지 분류" 프로젝트는 모든 구성요소가 실제로 작동해야 하는, 만들 수 있는 가장 작은 종단간 ML 시스템입니다: 데이터 파이프라인, 증강, 학습 루프, 정규화, 평가. 이 섹션은 작은 데이터셋의 정확도를 주도하는 증강 선택의 이론, mixup/cutmix가 작동하는 이유, 그리고 테스트 타임 증강의 수학을 제공합니다.

이 섹션에서 다루는 내용:

- **A.** 증강이 작동하는 이유: 암묵적 정규화
- **B.** 표준 증강: flip, crop, color, RandAugment
- **C.** Mixup, CutMix, label smoothing
- **D.** 앙상블 평균으로서의 테스트 타임 증강

### A. 암묵적 정규화로서 증강

원시 CIFAR-10(50k 이미지)에서 학습된 분류기는 과적합 — 학습 정확도가 100%에 도달하지만 테스트 정확도가 평탄화. 증강이 학습 셋을 가상으로 확장: 각 에폭이 각 이미지의 다른 "인스턴스"를 봄. 수학적으로, `L(f(x), y)` 대신 `E_T[L(f(T(x)), y)]`에서 학습하며, `T`는 무작위 증강. 세 효과:

1. **유효 데이터셋 크기가 더 큼**: 각 예제가 많은 형태로 많이 보이지만, 모델이 같은 정확한 텐서를 두 번 보지 않음.
2. **모델이 증강에 불변하도록 강제됨**: 색, 정확한 위치, 정확한 크롭에 의존할 수 없음. 이 특징이 클래스에 영향을 주지 않아야 한다면 유용한 귀납 편향.
3. **암묵적 평활화**: 인접 증강이 인접 손실을 만들기 때문에 손실 지형이 더 부드러워짐.

### B. 표준 증강

CIFAR 스타일 자연 이미지의 경우:

- **패딩이 있는 무작위 크롭** (예: 36x36 패딩 이미지에서 32x32 크롭): 평행이동 불변성 학습.
- **무작위 수평 flip**: 거울 대칭 불변성 학습(대부분 자연 객체 클래스에 정확).
- **색 지터링** (밝기, 대비, 채도, 색상): 색 불변성 학습.
- **무작위 erasing** (Zhong et al. 2017): 무작위 사각형 0으로. 가림 강건성 학습.
- **RandAugment** (Cubuk et al. 2019): 고정 풀에서 N개 변환을 크기 M으로 무작위 적용; N과 M 둘 다 검색 가능 하이퍼파라미터. 손으로 증강 정책을 설계할 필요 제거.

작은 데이터셋에서 좋은 증강의 정확도 개선 자릿수: 3-10%. 이는 아키텍처 전환에 비교 가능.

### C. Mixup, CutMix, Label Smoothing

**Mixup** (Zhang et al. 2017): 두 개를 선형 보간하여 새 학습 예제 생성:

```
x = lambda * x_a + (1 - lambda) * x_b
y = lambda * y_a + (1 - lambda) * y_b               (원-핫 보간)
lambda ~ Beta(alpha, alpha)
```

모델이 이 볼록 결합에서 학습. 효과: 더 부드러운 결정 경계, 덜 과신, 더 나은 보정.

**CutMix** (Yun et al. 2019): 픽셀 블렌딩 대신, `x_b`에서 무작위 사각형을 `x_a`에 *붙여넣기*, 라벨이 대응하는 면적 가중 결합. 결합에서 학습하면서 원본 이미지의 픽셀별 날카로움 유지.

**Label smoothing** (Szegedy et al. 2016): 원-핫 타겟을 더 부드러운 분포로 대체: `y_smooth = (1 - \epsilon) * one_hot + \epsilon / K`. 모델이 과신하는 것을 막고 보정을 개선; 작지만 일관된 정확도 이득.

세 기법 모두 다른 메커니즘으로 유사한 효과(더 부드러운 결정 표면, 더 나은 보정) 달성. 종종 생산적으로 적층 — mixup + label smoothing이 표준 레시피.

### D. 테스트 타임 증강

추론 시, 같은 입력의 여러 증강 버전에 걸쳐 예측을 평균:

```
pred = mean over T_1, ..., T_K of softmax(model(T_i(x)))
```

이는 본질적으로 "모델 변형"이 증강 변형인 앙상블. 예측당 `K`번 순전파 비용; 일반적으로 0.5-2% 정확도 향상. 흔한 증강: 원본 + 수평 flip + 5 코너 크롭, 총 12 평가.

앙상블링과의 연결: TTA는 추가 학습이 필요 없는 저렴한 형태의 앙상블링. 베이지안 딥러닝은 이를 (데이터 증강 유도) 파라미터 이웃에 대한 사후 예측 적분 근사로 봄.

### 이론에서 아래 코드로

| 이론 개념 | 본 레슨의 코드 구성 |
|-----------|---------------------|
| 무작위 크롭 + flip | `transforms.RandomCrop(32, padding=4); transforms.RandomHorizontalFlip()` |
...
| TTA | 여러 `transform`에 걸쳐 `softmax(model(transform(x)))` 평균 |
