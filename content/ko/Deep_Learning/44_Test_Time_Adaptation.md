...
참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 도메인 시프트, 배치 정규화 적응, 엔트로피 최소화 기반 테스트 타임 적응(TENT).
...
테스트 타임 적응(TTA)은 표준 ML의 근본적 격차에 대응합니다: 학습 분포와 테스트 분포가 종종 다르고, 모델이 재학습할 기회가 없습니다. TTA 기법은 라벨도 추가 학습 데이터도 필요 없는 도메인 시프트 완화 전략으로, 비레이블 테스트 데이터만 사용해 *추론 중* 배포된 모델을 업데이트합니다. 이 섹션은 batch-norm 통계, 엔트로피, 그리고 다른 비지도 신호가 모델을 효과적으로 재조정할 수 있는 이유의 수학을 설명합니다.

이 섹션에서 다루는 내용:

- **A.** 도메인 시프트: 학습/테스트 분포 불일치의 수학
- **B.** 가장 단순한 TTA로서의 BatchNorm 적응
- **C.** TENT: 테스트 시 자기 지도 손실로서의 엔트로피 최소화
- **D.** TTT와 CoTTA: 더 정교한 적응 체제

### A. 도메인 시프트

표준 지도 학습은 학습과 테스트 데이터가 같은 분포 `P(x, y)`에서 추출됨을 가정. 실제 배포는 이를 깨뜨림 — 테스트 분포가 `P_T(x, y) \neq P_S(x, y)`. 세 종류:

- **공변량 시프트**: `P_T(x) \neq P_S(x)`이지만 `P(y | x)` 변하지 않음. 흔함: 조명, 날씨, 센서 보정 변화.
- **라벨 시프트**: `P_T(y) \neq P_S(y)` (클래스 비율). 흔함: 다른 질병 유병률을 가진 인구에 의료 모델 배포.
- **개념 시프트**: `P_T(y | x) \neq P_S(y | x)`. 같은 입력이 다른 의미. 심각하고 종종 전이에 치명적.

공변량 시프트의 경우, 모델의 예측이 여전히 의미 있음 — 단지 통계가 이동된 입력을 봄. TTA는 정확히 이 사례를 겨냥: 기저 분류를 변경하지 않고 새 입력 통계에 모델을 조정.

### B. BatchNorm 적응

BN은 학습 중 running 통계 `\mu_{run}, \sigma_{run}`을 계산하고 추론 시 사용. 테스트 분포가 다르면 그 통계가 잘못됨. 가장 단순한 TTA: **테스트 데이터에서 BN 통계 재계산**.

```
\mu_t = 테스트 샘플 배치에 대한 평균
\sigma_t = 테스트 샘플 배치에 대한 표준편차
y = (x - \mu_t) / \sigma_t * \gamma + \beta
```

예: 테스트 시 잡음이나 대비 변화로 인한 공변량 시프트에 충격적으로 효과적. 비용: 0 — 단지 테스트 입력을 배치로 처리하고 통계를 재계산하면 됨. Schneider et al. (2020)은 이것이 흔한 벤치마크에서 더 복잡한 TTA 방법을 일치시키거나 이긴다는 것을 보임.

함정: 합리적 크기 테스트 배치 필요. 온라인 설정(한 번에 한 입력)의 경우, 최근 테스트 입력에 대해 running average가 유지되어야 함.

### C. TENT: 테스트 타임 엔트로피 최소화

TENT (Wang et al. 2021)는 더 나아감: 예측 엔트로피를 최소화하여 BN의 어파인 파라미터 `\gamma, \beta`(테스트 시 학습되는 유일한 파라미터)도 업데이트:

```
L_TENT = -sum_i p_i log p_i        (softmax 출력의 배치별 엔트로피)
```

직관: (분포 외) 입력에 자신 있는 모델이 불확실한 것보다 정확할 가능성이 더 높음. 엔트로피 최소화가 예측을 날카롭게 하여, 사실상 모델에게 "답에 헌신하라"고 말함. 실험적으로 이는 시프트된 분포의 정확도를 개선.

중요한 세 설계 선택:

1. **모든 파라미터가 아닌 BN 어파인 파라미터만 업데이트**. 다른 파라미터는 즉시 테스트 데이터에 과적합.
2. **순전파가 (BatchNorm 적응처럼) 현재 배치의 BN 통계 사용**, running 통계 아님.
3. **라벨 관여 없음** — 엔트로피가 예측만으로 계산 가능한 자기 지도 신호.

### D. TTT와 CoTTA

**TTT** (Test-Time Training, Sun et al. 2020)는 주 작업과 함께 *보조 자기 지도 작업*을 학습. 테스트 시, 그 보조 작업은 테스트 입력에서 학습 가능(라벨 불필요)이며, 업데이트가 공유 층으로 다시 흐름. 이는 TENT보다 더 일반적이지만 보조 작업 선택과 학습이 필요.

**CoTTA** (Continual TTA, Wang et al. 2022)는 지속적으로 시프트하는 분포 처리: 적응된 모델을 소스 모델과 평균하여 catastrophic forgetting 방지, 나쁜 적응을 잊기 위한 확률적 재설정. 테스트 분포가 시간에 걸쳐 드리프트하는 긴 배포 기간을 위해 설계.

TTA 스펙트럼: BN 적응(무료, 단순, 종종 충분) → TENT(작은 오버헤드, 종종 더 정확) → TTT/CoTTA(정교, 어려운 사례에 필요).

### 이론에서 아래 코드로

| 이론 개념 | 본 레슨의 코드 구성 |
|-----------|---------------------|
| BN 전용 적응 | `model.eval()` 설정 후 `for m in model.modules(): if isinstance(m, BatchNorm2d): m.train()` |
| TENT 엔트로피 손실 | `loss = -(probs * probs.log()).sum(dim=-1).mean()` |
| BN 파라미터만 업데이트 | BN 파라미터만 옵티마이저에 전달 |
| 온라인 TTA 루프 | `for batch in test_loader: pred = model(batch); loss = entropy(pred); loss.backward(); opt.step()` |
