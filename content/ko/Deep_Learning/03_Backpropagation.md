...
역전파는 흔히 "체인 룰을 층마다 적용한다"고 소개되지만, 이는 효율성을 만드는 구조를 숨깁니다. 핵심 아이디어는 **벡터-야코비안 곱(Vector-Jacobian Product, VJP)**입니다: 모든 층에서 전체 야코비안 행렬을 만들지 않고, 항상 두 텐서의 곱이 아니라 한 텐서의 형태를 가지는 단일 코탄젠트 벡터를 전파합니다. 이것이 1억 파라미터 네트워크 학습이 애초에 가능한 이유입니다.

이 섹션에서 다루는 내용:

- **A.** 야코비안 형식의 체인 룰
- **B.** 벡터-야코비안 곱과 전체 야코비안을 만들지 않는 이유
- **C.** DAG 위 VJP 반복 적용으로서의 역전파
- **D.** 깊은 네트워크에서 forward-mode vs reverse-mode 트레이드오프

### A. 야코비안 형식의 체인 룰

`f = g \circ h`이고 `h: R^n -> R^m`, `g: R^m -> R^k`라고 합시다. 합성의 야코비안은 야코비안의 행렬 곱입니다:

```
J_f(x) = J_g(h(x)) * J_h(x)            [k x n]  =  [k x m] * [m x n]
```

신경망은 긴 합성 `f = f_L \circ f_{L-1} \circ ... \circ f_1`입니다. 귀납에 의해 그 야코비안은:

```
J_f = J_{f_L} * J_{f_{L-1}} * ... * J_{f_1}
```

이 곱을 오른쪽에서(forward mode) 또는 왼쪽에서(reverse mode) 단순하게 계산하면, 중간 행렬이 거대해집니다 — 완전 연결층의 `J_{f_l}`은 `m x n`이며 두 차원이 수천에서 수백만일 수 있습니다. 그런 행렬 하나만 저장해도 이미 기가바이트이고, 체인 전체를 저장하는 것은 불가능합니다.

### B. 벡터-야코비안 곱: 핵심 절약

스칼라 손실 `L`에 대해, 우리는 최종 야코비안의 *한 행*만 필요합니다 — `dL/dx`이며 형상은 `(1, n)` 또는 동등하게 `x`와 같은 형상입니다. Reverse-mode AD는 행 벡터 `v^T`를 왼쪽에서 오른쪽으로 전파하여 이를 활용합니다:

```
v_L^T  =  dL/dy_L                                     [형상: y_L과 같음]
v_{l-1}^T = v_l^T * J_{f_l}(x_{l-1})                  [형상: x_{l-1}과 같음]
```

곱 `v^T * J`가 **벡터-야코비안 곱(VJP)**입니다. 층의 구조가 벡터에 `J`를 직접 곱하는 방법을 알려주기 때문에, 각 VJP는 *J를 만들지 않고도* 계산할 수 있습니다. 예시:

- **선형층** `y = W x + b`: `J = W`이므로 VJP는 `v^T W`. 비용: O(m * n), 야코비안 저장 없음.
- **ReLU** `y_i = max(0, x_i)`: `J`는 대각이고 `J_{ii} = [x_i > 0]`. VJP는 원소별 마스크: `v_i * [x_i > 0]`. 비용: O(n).
- **Softmax + cross-entropy**: VJP는 `softmax(x) - one_hot(y)`로 단순화. 비용: O(n).

PyTorch의 모든 원시 연산은 손으로 작성한 VJP 규칙과 함께 제공됩니다 — 그것이 `grad_fn`이 실제로 저장하는 내용입니다.

### C. 역전파 = DAG 위 VJP 반복

완전한 역전파 알고리즘은 다음과 같습니다:

1. 순전파 실행; VJP 규칙이 필요로 하는 중간 활성화를 기억합니다.
2. 손실 노드에서 `v = 1`(시드 `dL/dL`)로 초기화합니다.
3. DAG를 역위상 순서로 순회합니다. 각 노드에서 VJP 규칙을 조회하여 들어오는 `v`에 지역 야코비안을 곱하고, 그 결과를 부모에게 전달합니다. 노드가 여러 소비자를 가지면 다시 흘러 들어오는 코탄젠트들을 합산합니다(체인 룰의 경로별 합이 fan-out 지점에서 문자 그대로 합이 됩니다).
4. `requires_grad=True`인 잎(leaf) 텐서의 `.grad`에 결과를 누적합니다.

각 VJP가 대응 순전파 연산과 거의 같은 비용이므로 총 비용은 `O(forward cost)`입니다.

### D. 깊은 네트워크에서 Forward vs Reverse Mode

`n`개 파라미터와 스칼라 손실 `L`을 가진 네트워크에 대해:

- **Reverse mode**: 한 번의 역전파 = `n`과 무관하게 O(forward). `loss.backward()`가 하는 일입니다.
- **Forward mode**: 한 번의 접선 전파 = O(forward)이지만, *입력 방향당 한 번의 패스*가 필요합니다. 모든 `n`개 파라미터에 대한 `dL/dx_i`를 얻으려면 `n`번의 순전파가 필요 — 완전히 비실용적입니다.

Forward mode는 `n << m`(파라미터 적고 출력 많음)일 때만 유리한데, 이는 딥러닝의 정반대입니다. PyTorch 2.x는 `torch.func.jacrev`와 `torch.func.jacfwd`로 둘 다 노출하며, 올바른 선택은 항상 형상 비율에 의존합니다.

### 이론에서 아래 코드로

| 이론 개념 | 본 레슨의 코드 구성 |
|-----------|---------------------|
| 체인 룰 (행렬 형태) | 손으로 쓴 `dL_dW = X^T @ dL_dy` 라인 |
| 선형층 VJP | NumPy의 `dL/dW`, `dL/dx` 유도 |
| ReLU VJP | 역전파에서 사용된 `(x > 0)` 마스크 |
| 역전파 = 역순 순회 | 순서 `dL_dy -> dL_dW2 -> dL_dh -> dL_dW1` |
