...
- **단어 수준**: 공백 + 구두점에서 분할. 단순하지만 어휘가 폭발(영어는 10만+ 흔한 단어; 이름/오타 같은 드문 단어가 out-of-vocabulary 문제 유발).
- **문자 수준**: 작은 어휘(~100 문자)와 OOV 없음, 하지만 시퀀스가 매우 길어지고 모델이 형태론을 처음부터 학습해야 함.
- **서브워드 (BPE, WordPiece, SentencePiece)**: ~30k-100k 서브워드 단위 어휘. 흔한 단어는 단일 토큰; 드문 단어는 조각으로 분해(`"unhappiness"` → `"un"`, `"happi"`, `"ness"`). OOV 없음, 관리 가능한 시퀀스 길이, 단위가 형태론적 신호를 가짐.
