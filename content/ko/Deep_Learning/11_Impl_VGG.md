...
VGG (Simonyan & Zisserman 2014)는 하나의 원리에 대한 아키텍처적 헌신입니다: 적층된 `3x3` 합성곱을 통한 깊이, 그리고 그 밖의 모든 것(커널 크기, 풀링 인자, 채널 두 배화)을 표준화. VGG를 처음부터 구현하는 것은 깊은 CNN의 파라미터 예산이 실제로 층 전반에 어떻게 분포하는지 내재화하고, 모든 후속 개선(BN, 잔차, depthwise conv)을 동기 부여한 고통(메모리, 계산)을 느끼는 가장 깔끔한 방법입니다.

이 섹션에서 다루는 내용:

- **A.** VGG 설계 원리: 작은 커널, 깊은 적층, 규칙적 두 배화
- **B.** VGG가 batch normalization 없이 작동하는 이유 (그리고 어떤 비용으로)
- **C.** 파라미터와 FLOP 분석: 비용이 어디 있는가
- **D.** 깊이 절제(ablation)로서의 VGG 패밀리(VGG-11/13/16/19)
...
| 이론 개념 | 본 레슨의 코드 구성 |
|-----------|---------------------|
| 3x3 conv + ReLU 블록 | `nn.Conv2d(c, c, 3, padding=1)` 다음 `nn.ReLU(inplace=True)` |
| MaxPool 다운샘플링 | 각 블록 후 `nn.MaxPool2d(2, 2)` |
| 채널 두 배화 | `64 -> 128 -> 256 -> 512` 채널 시퀀스 |
| FC 파라미터 지배 | ~1억 params인 4096 유닛 FC 층 |
