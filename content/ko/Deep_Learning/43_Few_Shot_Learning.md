...
참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 퓨샷 학습 문제, 메타 학습 프레임워크, 메트릭 기반 vs 최적화 기반 방법.
...
퓨샷 학습은 표준 "클래스당 많은 데이터" 가정을 "클래스당 한 줌의 예제"로 대체합니다. 이는 근본적으로 다른 메커니즘을 요구합니다: 메타 학습("학습하는 법을 학습")과 메트릭 학습(분류기가 아닌 임베딩 공간 학습). 에피소드 학습, 프로토타입 네트워크, MAML 뒤의 수학 각각이 하나의 관찰에서 나옵니다: 1-5개 예제만 있으면 평소처럼 경사 하강을 할 수 없습니다.

이 섹션에서 다루는 내용:

- **A.** 퓨샷 문제와 표준 지도 학습이 실패하는 이유
- **B.** 에피소드 학습: 학습 시 퓨샷 시뮬레이션
- **C.** Prototypical Networks: 임베딩 공간의 최근접 이웃
- **D.** MAML: 몇 스텝에 적응하는 초기화 메타 학습

### A. 퓨샷이 다른 이유

표준 지도 학습은 클래스당 수백에서 수천 예제를 가정. 1-5 예제로, 평범한 파인튜닝이 붕괴:

- **과적합**: 임의의 유연한 모델이 5 예제를 순간에 완벽히 암기.
- **초기화 민감도**: 어느 5 예제를 고르는지가 엄청나게 중요.
- **통계적 신호 없음**: 5 샘플의 경험적 평균이 모집단 평균의 빈약한 추정.

퓨샷 방법은 단일 작업이 아닌 *작업 분포*에서 학습하여 이를 회피. 모델은 하나의 특정 분류를 학습하기보다 "임의의 퓨샷 분류 작업에서 잘 되는 법"을 학습.

### B. 에피소드 학습

학습 셋이 테스트 조건을 모방하는 **에피소드**로 재구성. N-way K-shot 에피소드:

1. 학습 클래스 풀에서 N개 클래스 샘플.
2. **서포트 셋**을 위해 클래스당 K개 예제 샘플.
3. **쿼리 셋**을 위해 클래스당 추가 예제 샘플.
4. 서포트 셋을 사용해 쿼리 셋을 분류하도록 모델 학습.

모델은 학습 시 원시 클래스 라벨을 절대 보지 않음 — 단지 "이 쿼리가 어느 서포트 예제와 일치하는가?"의 상대적 구조만. 테스트 시, 새 클래스(학습 시 보지 못한)를 샘플링하고 같은 절차 적용. 모델의 일은 특정 클래스를 암기하는 것이 아니라 *메타 작업*(퓨샷 분류)을 일반화하는 것.

### C. Prototypical Networks

ProtoNets (Snell et al. 2017)은 가장 깔끔한 퓨샷 알고리즘:

1. **모든 예제를 공유 encoder `f_\theta`로 임베드**.
2. **프로토타입 계산**: 클래스당 K개 서포트 예제의 임베딩 평균:
   ```
   c_n = (1 / K) sum_{x in S_n} f_\theta(x)         (클래스 n의 프로토타입)
   ```
3. **최근접 프로토타입으로 쿼리 분류**:
   ```
   ...
   ```
   `d`는 제곱 유클리드 거리.
4. 올바른 쿼리 라벨의 로그 우도 최대화로 **학습**.

전체 알고리즘이 "같은 클래스 항목이 클러스터링되는 임베딩을 학습한 다음, 쿼리를 최근접 프로토타입화." 표준 벤치마크(Omniglot, miniImageNet)에서 훨씬 화려한 방법과 놀랍도록 경쟁력.

### D. MAML

Model-Agnostic Meta-Learning (Finn et al. 2017)은 다른 접근: 임의의 서포트 셋 `S`인 퓨샷 작업이 주어지면, `S`에서 몇 SGD 스텝이 쿼리 셋에서 좋은 성능을 만드는 *초기화* `\theta`를 메타 학습.

내부 루프(작업당):
```
\theta_i' = \theta - \alpha * \nabla_\theta L_S_i(\theta)         (한 번 또는 몇 SGD 스텝 후)
```
...
외부 루프 그래디언트는 *적응 후* 손실의 *적응 전* 파라미터에 대한 그래디언트. 이는 **2차** 그래디언트(그래디언트의 그래디언트), 비싸지만 autograd로 다룰 수 있음.

결과: "한 번에 많은 작업의 좋은 출발점"인 초기화, 거기서 몇 SGD 스텝이 빠르게 적응. 개념적으로 일반: SGD로 학습 가능한 임의의 모델과 작동.

### 이론에서 아래 코드로

| 이론 개념 | 본 레슨의 코드 구성 |
|-----------|---------------------|
| 에피소드 샘플링 | `support, query = sample_episode(dataset, n_way, k_shot, n_query)` |
| 임베딩 | 서포트와 쿼리 둘 다에 대한 `z = encoder(x)` |
| 프로토타입 계산 | `proto = z_support.view(N, K, d).mean(dim=1)` |
| 최근접 프로토타입 softmax | `-(z_query - proto).pow(2).sum(-1)` 그 다음 softmax |
| MAML 내부 루프 | `fast_weights = theta - alpha * autograd.grad(loss, theta)` |
