...
TensorBoard는 시각화 도구이며 딥러닝 개념이 아닙니다 — 하지만 학습을 *보는* 실천 자체가 그 뒤에 이론이 있는 분야입니다. 이 섹션은 표준 차트(손실 곡선, 그래디언트 노름, 활성화 히스토그램)가 모델 건강에 대해 실제로 무엇을 말하는지, 진단 패턴이 무엇을 의미하는지 다룹니다.

이 섹션에서 다루는 내용:

- **A.** 손실 곡선: train vs validation, 네 가지 진단 형태
- **B.** 그래디언트 노름 추적과 "그래디언트 폭발" 경보
- **C.** 활성화와 가중치 히스토그램: 죽은 유닛, 포화된 층
- **D.** 임베딩 시각화: t-SNE, UMAP, 그리고 그것들이 숨기는 것

### A. 손실 곡선 진단

스텝 대 train/validation 손실 그래프는 딥러닝에서 가장 정보가 많은 단일 차트입니다. 네 가지 흔한 형태:

1. **두 곡선이 부드럽게 감소, validation이 약간 더 높음**: 건강한 학습. 계속.
2. **Train이 감소, validation이 평탄하거나 상승**: 과적합. 정규화 추가, 더 많은 데이터, 또는 더 일찍 정지.
3. **둘 다 평탄하거나 추세 없이 잡음**: 학습 안 됨. 학습률(아마 너무 작음), 데이터 파이프라인(아마 잘못된 라벨), 또는 모델 용량(아마 너무 작음) 확인.
4. **손실 급증 / NaN**: 그래디언트 폭발이나 수치 문제. LR 낮추기, gradient clipping 추가, 0 나누기 확인.

훈련된 눈은 이 패턴을 초 단위로 읽습니다. TensorBoard의 평활화 슬라이더가 도움 됨 — 원시 손실은 잡음이 있지만 평활화된 곡선이 추세를 드러냄.

### B. 그래디언트 노름

스텝당(또는 층당) `||\nabla L||` 로깅은 여러 문제를 잡습니다:

- **폭발**: 노름이 무한 증가. `clip_grad_norm_` 적용. clipping에도 즉시 폭발하면 LR이 너무 높음.
- **소실**: 노름이 0으로 감쇠. 초기화 확인, tanh/sigmoid를 ReLU로 교체, 잔차 연결 추가, 또는 BN/LN 사용.
- **층별 패턴**: 초기 층이 후기 층보다 훨씬 작은 그래디언트를 가지면 깊이를 통한 고전적 소실 — 잔차나 BN이 필요.

모든 파라미터에 대한 단일 숫자 `||\nabla||_2`가 가장 단순한 진단; 층별 노름이 문제가 어디 사는지 알려줍니다.

### C. 활성화와 가중치 히스토그램

각 N 스텝마다 각 층의 *활성화*와 *가중치* 히스토그램을 플롯. 이들이 드러내는 세 가지 병리:

- **Dead ReLU**: 활성화의 큰 부분이 0에 갇힘. 종종 히스토그램의 0에서 스파이크로 보임. 수정: LR 감소, Leaky ReLU/GELU로 전환, 또는 음의 편향 초기화 확인.
- **포화된 sigmoid/tanh**: 히스토그램이 -1 / +1 (tanh) 또는 0 / 1 (sigmoid)에 스파이크. 거기서 그래디언트가 거의 0이라 학습을 죽임. 수정: ReLU/GELU 사용, 또는 입력을 더 적극적으로 정규화.
- **가중치 드리프트**: 가중치 히스토그램이 학습 동안 상당히 이동, 너무 높은 LR이나 부적절한 정규화 시사. 건강한 학습은 자라지만 제한된 분산으로 0 근처에 중심화된 가중치를 보임.

### D. 임베딩 시각화

학습된 표현의 경우, **t-SNE** 또는 **UMAP**로 2D로 투영하면 의미적으로 유사한 입력이 클러스터링되는지 볼 수 있습니다. 유용한 점검:

- 학습 후, 같은 클래스 예제가 함께 클러스터링되는가?
- 언어적으로 동의어 단어 / 시각적으로 유사한 이미지가 서로 가까운가?
- 두 점 사이의 보간이 의미 있는 중간 예제를 통과하는가(부드럽고 잘 형성된 매니폴드의 신호)?

주의: t-SNE와 UMAP 둘 다 **비선형**이고 **비등거리**. 2D 플롯의 거리는 원래 공간의 거리에 충실하지 않고, 명백한 클러스터 크기/분리는 부분적으로 알고리즘 하이퍼파라미터의 인공물입니다. 이 시각화를 정량적 주장이 아닌 정성적 확인에 사용하세요.

### 이론에서 아래 코드로

| 이론 개념 | 본 레슨의 코드 구성 |
|-----------|---------------------|
| 스칼라 로깅 | `writer.add_scalar('loss/train', loss, step)` |
| 히스토그램 로깅 | `writer.add_histogram('weights/conv1', model.conv1.weight, step)` |
| 그래디언트 노름 | `writer.add_scalar('grad_norm', total_norm, step)` |
| 임베딩 프로젝터 | `writer.add_embedding(embeddings, metadata=labels)` |
