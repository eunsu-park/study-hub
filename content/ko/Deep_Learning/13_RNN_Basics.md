# 13. RNN 기초 (Recurrent Neural Networks)

[이전: ResNet](./12_Impl_ResNet.md) | [다음: LSTM과 GRU](./14_LSTM_GRU.md)
...
순환 신경망은 시간 축에 대해 가중치 공유를 가지고 시간에 걸쳐 펼쳐진 피드포워드 네트워크입니다. 그 한 문장은 딥러닝의 가장 깊은 어려움 세 가지를 숨깁니다 — 장거리 신용 할당(credit assignment), 그래디언트 소실/폭발, 그리고 순차적 표현력과 병렬 계산 사이의 트레이드오프. 이 섹션은 vanilla RNN이 왜 어려움을 겪는지, 표준 해결책(gradient clipping, 신중한 초기화)이 실제로 무엇을 하는지의 수학을 제공합니다.

이 섹션에서 다루는 내용:

- **A.** RNN 순전파와 펼쳐진 계산 그래프
- **B.** Backpropagation Through Time (BPTT)와 그 O(T) 그래디언트 흐름
- **C.** 고유값 분석을 통한 그래디언트 소실/폭발
- **D.** 부분적 해결책으로서 gradient clipping과 직교 초기화

### A. RNN 순전파와 펼치기

Vanilla RNN은 은닉 상태 `h_t`를 유지하며 시퀀스 `x_1, x_2, ..., x_T`를 처리합니다:

```
h_t = tanh(W_x x_t + W_h h_{t-1} + b)
y_t = W_y h_t + c                          (선택적 출력)
```

같은 `(W_x, W_h, W_y, b, c)`가 모든 시간 스텝에서 사용됩니다 — 이것이 네트워크가 가변 길이 시퀀스를 다루게 하는 **시간적 가중치 공유**입니다. 개념적으로, RNN은 모든 층이 가중치를 공유하고 각 층의 입력에서 스킵 연결이 있는 `T`개 "층"의 피드포워드 네트워크입니다.

"펼쳐진" 관점은 점화식을 긴 DAG로 대체합니다: `h_0 -> h_1 -> ... -> h_T`. 순전파 시간은 `O(T)`이며 시간에 걸쳐 병렬화할 수 없습니다(각 `h_t`가 `h_{t-1}`에 의존). 이것이 Transformer를 동기 부여한 근본적 순차 보틀넥입니다.

### B. Backpropagation Through Time (BPTT)

손실 `L = sum_t L_t`의 `W_h`에 대한 그래디언트 계산은 모든 시간 스텝을 통한 역전파를 요구합니다:

```
dL/dW_h = sum_t  sum_{k <= t}  dL_t/dh_t * (prod_{j=k+1}^{t} dh_j/dh_{j-1}) * dh_k/dW_h
```

내부 곱은 `t - k`개 야코비안의 사슬에 대한 것입니다. 모든 은닉 야코비안이 `W_h`를 공유하기 때문에, 이는 다음으로 전개됩니다:
...
행렬 `W_h`가 자신과 `t - k`번 곱해집니다. 이것이 모든 RNN 그래디언트 병리의 원인입니다.

**Truncated BPTT (TBPTT)**는 역전파를 `K << T` 스텝의 윈도로 제한하여, 정확한 그래디언트를 다룰 수 있는 계산과 제한된 그래디언트 크기로 거래합니다.

### C. 고유값을 통한 그래디언트 소실/폭발

`\lambda_max`를 `W_h`의 가장 큰 절대 고유값(스펙트럼 반경)이라 합시다. 반복된 곱셈은 벡터를 약 `\lambda_max^{t-k}`만큼 스케일합니다:

- `\lambda_max < 1`이면: 그래디언트가 지수적으로 *소실*. 시간 `k`의 정보가 `t - k`가 클 때 시간 `t`의 손실에 영향을 줄 수 없습니다. 장거리 의존성이 경사 하강에 보이지 않게 됩니다.
- `\lambda_max > 1`이면: 그래디언트가 지수적으로 *폭발*. 업데이트가 거대해지고 학습이 발산(NaN 손실).
- `\lambda_max = 1`이면: 원리적으로 안정이지만 섭동에 극도로 민감.

`tanh'(z)` 인자가 이를 가중합니다 — `tanh'(z) <= 1`이고 `z = 0`에서만 1이므로, 실제 그래디언트는 `\lambda_max^{t-k}`보다 더 빨리 감쇠합니다. 이것이 스펙트럼 반경 1인 RNN조차 실전에서 보통 소실하는 이유입니다.

Bengio et al. (1994)은 vanilla RNN으로 장거리 의존성을 학습하는 것이 극한에서 *불가능*함을 형식적으로 증명했습니다: 임의의 학습 절차는 수축 매핑(안정 학습, 그래디언트 소실)과 확장 매핑(긴 기억, 그래디언트 폭발) 사이에서 거래해야 합니다.
...
그래디언트 노름을 `c`로 제한하여 폭발 사례를 방지. 그래디언트 소실에는 *아무것도 하지 않습니다*(이미 작음) — 단지 발산에서 구해줄 뿐. 임의의 RNN 학습에 사실상 필수입니다.

**직교 초기화**: `W_h`를 모든 고유값의 크기가 1인 임의의 직교 행렬로 선택. 곱 `W_h^{t-k}`가 벡터 노름을 정확히 보존하여, 소실과 폭발 둘 다를 미룹니다. 유용하지만 문제를 해결하지는 않습니다; `tanh'` 인자가 여전히 누적됩니다.

**아키텍처적 수정 (다음 레슨)**: LSTM과 GRU는 곱셈적 곱을 가산적 셀 상태(cell state)로 대체하여 `W_h^{t-k}` 사슬을 완전히 끊습니다. 이것이 유일한 진정한 해결책입니다.

### 이론에서 아래 코드로

| 이론 개념 | 본 레슨의 코드 구성 |
|-----------|---------------------|
| 순전파 점화식 | `nn.RNN`, `nn.RNNCell`, 또는 `tanh(W_x @ x + W_h @ h)`의 수동 루프 |
| BPTT | 펼쳐진 그래프에 대한 표준 `loss.backward()` |
| Truncated BPTT | 그래프를 끊기 위한 세그먼트 사이의 `h.detach()` |
