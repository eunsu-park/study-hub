...
ResNet (He et al. 2015)은 인용 수 기준으로 출판된 CNN 아키텍처 중 가장 영향력 있습니다. 그 중심 아이디어 — **잔차 연결(residual connection)** — 은 한 줄의 코드이지만, 그 한 줄이 왜 152층 네트워크를 학습 가능하게 하는지(56층 평이 네트워크가 *실패*하는데도) 그 뒤의 수학은 신중히 볼 가치가 있습니다. 그래디언트 그림을 한 번 보면 잔차 연결을 어디서나 알아볼 수 있습니다: U-Net, Transformer, BERT, GPT, Stable Diffusion 모두 사용합니다.

이 섹션에서 다루는 내용:

- **A.** Degradation 문제: 깊은 평이 네트워크가 실패하는 이유
- **B.** 항등 매핑 재매개변수화로서의 잔차 연결
- **C.** 잔차가 그래디언트 소실을 고치는 이유
- **D.** 보틀넥 블록과 ResNet-50/101/152로 가는 길
...
| 이론 개념 | 본 레슨의 코드 구성 |
|-----------|---------------------|
| 잔차 블록 `y = F(x) + x` | `out = self.conv2(self.conv1(x)) + x` |
| 형상 변경이 있는 shortcut | 스트라이드나 채널이 다를 때 `self.shortcut`의 `1x1` conv |
| 항등 매핑 초기화 이점 | `BasicBlock`이 모든-0 conv 초기화에서도 작동한다는 사실 |
| 보틀넥 인수분해 | `Conv2d(c, c//4, 1) -> Conv2d(c//4, c//4, 3) -> Conv2d(c//4, c, 1)` 패턴 |
