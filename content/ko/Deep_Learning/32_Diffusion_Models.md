...
가우시안의 재생성 성질(많은 작은 가우시안 합성곱을 합성하면 하나의 더 큰 가우시안 합성곱)에 의해, `x_0`이 주어진 임의의 시간 `t`의 분포가 닫힌 형태를 가짐:

```
q(x_t | x_0) = N(x_t ; sqrt(\bar\alpha_t) x_0, (1 - \bar\alpha_t) I)
여기서 \alpha_t = 1 - \beta_t,  \bar\alpha_t = prod_{s=1}^{t} \alpha_s
```

이는 임의의 `t`에 대해 반복 없이 `x_t`를 직접 샘플링할 수 있음을 의미:
