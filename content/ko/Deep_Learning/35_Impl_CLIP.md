...
이 레슨은 CLIP을 처음부터 구현하여, 이전 레슨의 contrastive 학습 이론을 구체적 텐서 연산에 고정합니다. 핵심은 대칭 손실을 올바르게 얻는 것, `N`개 배치가 실제로 무엇을 기여하는지 이해하는 것(예제당 `N - 1`개 음성), 그리고 zero-shot 추론을 수용 가능에서 우수로 바꾸는 프롬프트 엔지니어링입니다.

이 섹션에서 다루는 내용:

- **A.** 유사도 행렬에 대한 cross-entropy로서의 InfoNCE 손실
- **B.** 배치 크기가 contrastive 성능을 지배하는 이유
- **C.** 프롬프트 엔지니어링과 그것이 작동하는 이유
- **D.** 동결된 특징 추출기로서 CLIP 사용

### A. InfoNCE = 유사도 행렬의 Cross-Entropy
...
| 이론 개념 | 본 레슨의 코드 구성 |
|-----------|---------------------|
| 대칭 InfoNCE | `logits`와 `logits.T`에 대한 두 `F.cross_entropy` 호출 |
| 특징 L2 정규화 | 유사도 전 `F.normalize(features, dim=-1)` |
| 프롬프트 템플릿 | `prompts = [f"a photo of a {c}" for c in class_names]` |
| 동결된 CLIP linear probe | `clip_model.eval(); for p in clip_model.parameters(): p.requires_grad_(False)` |
