...
**다음**: [OSI 7계층 모델](./02_OSI_7_Layer_Model.md)
...
1. 네트워크가 무엇인지 설명하고, 핵심 구성 요소(노드(Node), 링크(Link), 프로토콜(Protocol))를 기술할 수 있다
2. PAN, LAN, MAN, WAN을 범위와 사용 사례에 따라 구분할 수 있다
3. 네트워크 토폴로지(버스, 스타, 링, 메시, 하이브리드)를 비교하고 각각의 장단점을 식별할 수 있다
4. 패킷 교환(Packet Switching)과 회선 교환(Circuit Switching)을 구분할 수 있다
5. 클라이언트-서버(Client-Server)와 P2P(Peer-to-Peer) 통신 모델을 비교할 수 있다
6. 일반적인 네트워크 장비와 각 장비가 동작하는 OSI 계층을 식별할 수 있다

---

웹 페이지를 불러오거나, 메시지를 보내거나, 영상을 스트리밍할 때마다 데이터는 네트워크를 가로질러 이동합니다. 수십 개의 장치와 수천 킬로미터를 단 몇 밀리초 만에 통과하기도 합니다. 네트워크의 구조, 장치들이 연결되는 방식, 그리고 데이터가 송신자에서 수신자까지 전달되는 방법을 이해하는 것은 네트워킹과 현대 IT 인프라의 모든 주제를 배우기 위한 기초입니다.
