...
**이전**: [라우팅 프로토콜](./09_Routing_Protocols.md) | **다음**: [UDP와 포트](./11_UDP_and_Ports.md)
...
1. TCP의 핵심 특성인 연결 지향(connection-oriented), 신뢰성(reliable), 순서 보장(ordered delivery)을 설명할 수 있다
2. 플래그, 시퀀스 번호, 윈도우 크기 등 TCP 헤더의 각 필드를 식별하고 설명할 수 있다
3. 연결 설정을 위한 3-Way 핸드셰이크(3-Way Handshake) 과정을 추적할 수 있다
4. 연결 종료를 위한 4-Way 핸드셰이크(4-Way Handshake) 과정을 추적할 수 있다
5. 시퀀스 번호와 확인 응답(ACK)이 신뢰성 있는 데이터 전달을 보장하는 방식을 설명할 수 있다
6. 슬라이딩 윈도우(sliding window) 메커니즘을 사용한 TCP 흐름 제어(Flow Control)를 설명할 수 있다
7. TCP 혼잡 제어(Congestion Control) 알고리즘인 Slow Start, Congestion Avoidance, Fast Retransmit, Fast Recovery를 비교할 수 있다

---

TCP는 인터넷을 신뢰할 수 있게 만드는 프로토콜입니다. 파일을 다운로드하거나, 웹 페이지를 열거나, 이메일을 전송할 때마다 TCP는 신뢰할 수 없는 라우터와 링크로 구성된 네트워크 위에서도 모든 바이트가 완전하고 순서대로, 그리고 중복 없이 도착하도록 조용히 보장합니다. TCP가 연결을 수립하고, 시퀀스 번호로 데이터를 추적하며, 네트워크 혼잡에 적응하는 방식을 이해하는 것은 애플리케이션 성능 문제를 디버깅하고 네트워크 시스템을 설계하는 데 필수적입니다.
...
> **비유 -- 등기 우편**: TCP는 등기 우편을 보내는 것과 같습니다. 배달 확인을 받을 수 있고(ACK), 우편 서비스가 모든 소포를 추적하며(시퀀스 번호), 분실된 것이 있으면 자동으로 재발송됩니다(재전송). 이 신뢰성에는 대가가 따릅니다 -- 그냥 우편함에 엽서를 넣는 것(UDP)보다 느립니다. 하지만 메시지가 완전하고 순서대로 도착했다는 확신이 필요할 때는, TCP의 오버헤드가 보장해주는 것에 충분히 값어치가 있습니다.
