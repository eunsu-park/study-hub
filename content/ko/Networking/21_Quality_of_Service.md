...
3. 트래픽 폴리싱(토큰 버킷)과 셰이핑 알고리즘을 구현한다
...
QoS 없이: 대용량 파일 전송이 음성 통화 대역폭을 고갈시킬 수 있습니다.
```
...
class TrafficShaper:
    ...
    폴리싱(드롭)과 달리, 셰이핑은 패킷을 버퍼에 저장했다가
    토큰이 가용해지면 전송합니다.
    """
