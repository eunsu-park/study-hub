...
**이전**: [네트워크 기초 개념](./01_Network_Fundamentals.md) | **다음**: [TCP/IP 모델](./03_TCP_IP_Model.md)
...
1. OSI 참조 모델(OSI Reference Model)의 목적과 계층화 아키텍처(Layered Architecture)가 중요한 이유를 설명할 수 있다
2. OSI 7개 계층 각각의 기능을 기술할 수 있다
3. 각 계층에서 PDU(Protocol Data Unit)의 명칭을 식별할 수 있다
4. 데이터가 계층 간 이동할 때 발생하는 캡슐화(Encapsulation)와 역캡슐화(Decapsulation) 과정을 추적할 수 있다
5. 일반적인 프로토콜과 네트워크 장비를 해당 OSI 계층에 매핑할 수 있다
6. OSI 모델을 체계적인 네트워크 트러블슈팅(Troubleshooting)의 프레임워크로 활용할 수 있다

---

OSI 모델은 네트워킹의 "로제타석(Rosetta Stone)"입니다. 엔지니어, 개발자, 학생이 데이터가 한 애플리케이션에서 다른 애플리케이션으로 네트워크를 거쳐 전달되는 방식을 공통된 어휘로 설명할 수 있게 해줍니다. 인터넷이 더 단순한 TCP/IP 스택으로 동작함에도 불구하고, OSI 모델은 모든 네트워크 자격증, 트러블슈팅 세션, 설계 논의에서 표준 사고 프레임워크로 계속 사용됩니다.

> **비유 - 우편 시스템(Postal System)**: OSI 모델은 7개 부서를 가진 우편 시스템과 같습니다. 편지를 보낼 때, 응용 계층(Application Layer)은 편지를 쓰는 당신 자신이고, 표현 계층(Presentation Layer)은 편지를 봉투에 넣는 것이며, 세션 계층(Session Layer)은 대화의 실(thread)이고, 전송 계층(Transport Layer)은 등기우편인지 일반우편인지 선택하는 것입니다. 네트워크 계층(Network Layer)은 어느 집하장을 거쳐야 하는지 결정하는 우편 경로 시스템이고, 데이터링크 계층(Data Link Layer)은 내 거리를 담당하는 집배원이며, 물리 계층(Physical Layer)은 우편 자루를 실어 나르는 실제 트럭입니다. 각 계층은 바로 위아래 계층하고만 소통합니다. 집배원이 편지 안에 무엇이 쓰여 있는지 알 필요가 없는 것처럼요.
...
> **왜 하위 계층부터(bottom-up)?** 하위 계층은 상위 계층 전체에 영향을 줍니다 — 케이블이 끊어지면(L1) DNS 디버깅(L7)은 무의미합니다. L1부터 위로 올라가면 하위 계층 장애가 원인인 증상을 헛되이 추적하지 않습니다. 각 계층에서 도구는 *해당 계층의 책임만 정확히* 테스트하도록 선택됩니다: `arp -a`는 L2 주소 해석(IP 라우팅 아님), `ping`은 L3 도달성(포트 연결 아님), `netstat -an`은 L4 포트 상태(애플리케이션 로직 아님)를 테스트합니다.
