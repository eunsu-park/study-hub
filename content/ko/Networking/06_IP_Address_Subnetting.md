...
**이전**: [데이터링크 계층](./05_Data_Link_Layer.md) | **다음**: [서브넷팅 실습](./07_Subnetting_Practice.md)
...
1. IP 주소의 목적을 설명하고, IP 주소와 MAC 주소의 차이점을 구분할 수 있다
2. IPv4 주소의 구조를 기술하고, 10진수와 2진수 표현 간 변환을 수행할 수 있다
3. IP 주소 클래스(A~E)와 각 클래스의 기본 서브넷 마스크(Subnet Mask)를 식별할 수 있다
4. 서브넷 마스크를 적용하여 주소의 네트워크 부분과 호스트 부분을 결정할 수 있다
5. CIDR 표기법(CIDR Notation)으로 주소를 표현하고, 사용 가능한 호스트 수를 계산할 수 있다
6. 공인 IP(Public IP)와 사설 IP(Private IP)를 구분하고, NAT를 설명할 수 있다
7. IPv6 주소 지정의 기초를 기술하고, IPv4에서 IPv6로의 전환이 필요한 이유를 설명할 수 있다

---

IP 주소 지정은 인터넷의 번호 체계입니다. 우편물 배달을 위해 모든 집에 고유한 주소가 필요하듯, 네트워크의 모든 장치에는 라우터가 패킷을 올바른 목적지로 전달할 수 있도록 IP 주소가 필요합니다. 서브네팅(Subnetting) — 네트워크를 더 작고 관리하기 쉬운 단위로 나누는 기술 — 을 숙달하는 것은 네트워킹에서 가장 실용적이고 자주 시험에 출제되는 역량 중 하나입니다.
