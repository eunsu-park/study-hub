...
**이전**: [보안 위협과 대응](./16_Security_Threats_Response.md) | **다음**: [IPv6](./18_IPv6.md)
...
1. `ping`을 사용하여 연결성을 테스트하고 RTT, TTL, 패킷 손실 통계를 해석한다
2. `traceroute`/`mtr`로 패킷 경로를 추적하고 문제 있는 홉을 식별한다
3. `ss`와 `netstat`을 사용하여 열린 포트와 활성 연결을 검사한다
4. `dig`와 `nslookup`으로 DNS 조회를 수행하고 출력 섹션을 해석한다
5. BPF 문법을 사용하여 `tcpdump`로 네트워크 패킷을 캡처하고 필터링한다
6. Wireshark 인터페이스를 탐색하고, 디스플레이 필터를 적용하며, TCP 스트림을 추적한다
7. `curl`을 사용하여 커스텀 메서드, 헤더, 인증으로 HTTP 엔드포인트를 테스트한다
8. 네트워크 문제를 진단하는 체계적인 계층별 방법론을 적용한다

---

**난이도**: ⭐⭐⭐

네트워크가 이론적으로 어떻게 동작하는지 아는 것은 절반에 불과합니다 -- 실제로 관찰하고, 진단하고, 문제를 해결할 수 있어야 합니다. 이 레슨은 구체적인 도구를 손에 쥐어줍니다: 무언가가 잘못되었을 때 네트워크 엔지니어와 시스템 관리자가 매일 찾는 바로 그 도구들입니다. 간단한 `ping`부터 Wireshark를 이용한 심층 패킷 검사까지, 이 유틸리티들은 추상적인 프로토콜 지식을 실행 가능한 트러블슈팅 기술로 변환시켜 줍니다.
