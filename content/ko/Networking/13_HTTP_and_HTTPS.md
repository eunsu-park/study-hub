...
**이전**: [DNS](./12_DNS.md) | **다음**: [기타 응용 계층 프로토콜](./14_Other_Application_Protocols.md)
...
1. HTTP 요청과 응답의 구조(헤더 및 바디 포함)를 설명할 수 있다
2. HTTP 메서드(GET, POST, PUT, PATCH, DELETE)와 그 속성(안전성(Safety), 멱등성(Idempotency), 캐시 가능성(Cacheability))을 구분할 수 있다
3. HTTP 상태 코드(Status Code)를 해석하고 클래스별(1xx--5xx)로 분류할 수 있다
4. HTTP/1.0, HTTP/1.1, HTTP/2, HTTP/3의 주요 개선 사항을 비교할 수 있다
5. HTTPS가 TLS/SSL을 사용하여 기밀성(Confidentiality), 무결성(Integrity), 인증(Authentication)을 제공하는 방법을 설명할 수 있다
6. TLS 1.2와 TLS 1.3의 핸드셰이크(Handshake) 과정을 개괄적으로 설명할 수 있다
7. 인증서 신뢰 체인(Chain of Trust)을 설명하고 DV, OV, EV 인증서를 구분할 수 있다

---

**난이도**: ⭐⭐⭐

HTTP는 웹의 언어입니다. 모든 클릭, 모든 API 호출, 모든 페이지 로드는 클라이언트와 서버 사이에서 이루어지는 HTTP 대화입니다. HTTP를 깊이 이해하는 것 -- 메서드, 상태 코드, 헤더, 버전별 변화 -- 은 웹 개발, API 설계, 네트워크 문제 해결의 기초입니다. 보안 변형인 HTTPS는 이제 모든 프로덕션 서비스에서 기본 요건으로 자리 잡았습니다.
...
## 7.5 심화: HTTP/2와 HTTP/3 (QUIC)

5절에서 버전 차이를 상위 수준으로 소개했지만, 이 섹션에서는 HTTP/2와 HTTP/3를 획기적으로 빠르게 만드는 내부 메커니즘과 도입 시점에 대해 탐구합니다.

### HTTP/1.1의 한계 상세 분석

HTTP/1.1은 수십 년간 웹을 지탱해 왔지만, 설계 구조가 근본적인 병목을 만듭니다:

```
┌──────────────────────────────────────────────────────────────────────┐
│              HTTP/1.1 HOL(Head-of-Line) 블로킹                        │
├──────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  연결 1:  [요청 A]──────────────────►[응답 A]                         │
│                          [요청 B 대기]──►[응답 B]                     │
│                                                                      │
│  문제: 응답 A가 느리면 (큰 파일, 느린 쿼리),                          │
│  같은 연결에서 요청 B를 시작할 수 없음.                                │
│                                                                      │
│  우회책: 브라우저가 6-8개 TCP 연결을 병렬로 개설                       │
│  → 6-8번의 TCP 핸드셰이크, 6-8번의 TLS 핸드셰이크, 6-8번의 slow start│
│  → 부하 시 서버 리소스 고갈                                           │
│  → 도메인 샤딩 핵 (assets1.example.com, assets2.example.com)         │
│                                                                      │
│  기타 HTTP/1.1 문제:                                                  │
│  - 텍스트 기반 헤더: 요청당 ~800바이트 (쿠키, user-agent)             │
│  - 헤더 압축 없음: 매 요청마다 같은 헤더 반복 전송                    │
│  - 서버 푸시 없음: 클라이언트가 각 리소스를 발견하고 요청해야 함      │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
```

### HTTP/2: 바이너리 프레이밍과 멀티플렉싱(Multiplexing)

HTTP/2는 HTTP 시맨틱스(메서드, 상태 코드, 헤더)는 유지하면서 전송 계층을 근본적으로 재설계합니다:
...
│              │ Flags (8 bits)                   │                    │
│              │ Stream Identifier (31 bits)      │  ← 멀티플렉싱     │
│              └──────────────────────────────────┘                    │
│                                                                      │
│  스트림: 각 요청-응답 쌍이 고유한 스트림 ID를 받음                    │
│  서로 다른 스트림의 프레임이 하나의 TCP 연결에서 인터리빙              │
│                                                                      │
│  단일 연결:                                                           │
│  ┌──────┬──────┬──────┬──────┬──────┬──────┬──────┐                │
│  │ S1:H │ S2:H │ S1:D │ S3:H │ S2:D │ S1:D │ S3:D │                │
│  └──────┴──────┴──────┴──────┴──────┴──────┴──────┘                │
│  S=스트림, H=HEADERS 프레임, D=DATA 프레임                           │
│  모든 스트림이 하나의 TCP 연결 공유 — HTTP 수준 HOL 블로킹 없음      │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
```

### HTTP/2 스트림 우선순위(Stream Prioritization)

클라이언트는 스트림에 우선순위를 할당하여 서버가 어떤 리소스가 가장 중요한지 알 수 있게 합니다:
...
우선순위 트리 (HTTP/2):
   ...
   analytics.js
   Weight: 32
   (Stream 1에 의존 — HTML 로드 후에 JS 로드)
```

**실무적 영향**: CSS와 HTML이 가장 높은 우선순위(렌더링 차단), 이미지는 낮은 우선순위, 분석 스크립트는 마지막에 로드됩니다.

### HPACK 헤더 압축

HTTP/2는 HPACK을 사용하여 헤더 오버헤드를 크게 줄입니다:

```
첫 번째 요청:
  헤더: Host: example.com, Accept: text/html, Cookie: session=abc123...
  전송: 전체 헤더 (~800 bytes)
  정적/동적 테이블에 인덱싱
...
두 번째 요청 (같은 연결):
  헤더: Host: example.com, Accept: text/html, Cookie: session=abc123...
  전송: 인덱스 참조만 (~20 bytes!)
  이유: HPACK이 이전에 본 헤더에 대해 테이블 인덱스만 전송

절약: 후속 요청에서 헤더 크기 ~95% 감소
```

### HTTP/2 서버 푸시(Server Push)

서버가 클라이언트가 요청하기 전에 사전적으로 리소스를 보낼 수 있습니다:

```bash
# 클라이언트가 index.html 요청
# 서버가 index.html로 응답하면서 style.css와 app.js도 푸시

# Nginx HTTP/2 서버 푸시 설정
# Why: 중요 리소스(CSS, JS)에 대해 1 RTT 절약
location = /index.html {
...
# curl로 확인:
# Why -v는 출력에서 푸시된 리소스를 보여줌
curl -v --http2 https://example.com/index.html
```

**참고**: HTTP/2 서버 푸시는 Chrome에서 폐기(2022)되었으며 `103 Early Hints`가 대안입니다. 많은 CDN에서 더 이상 지원하지 않습니다.

### HTTP/3와 QUIC: UDP 혁명

HTTP/3는 TCP를 QUIC로 대체합니다. QUIC는 전송과 암호화를 통합한 UDP 기반 프로토콜입니다:
...
│                                  │    IP        │                    │
│  연결: 2-3 RTT                   └──────────────┘                    │
│  (TCP 핸드셰이크 + TLS 핸드셰이크)  연결: 1 RTT                      │
│                                   (재연결 시 0-RTT!)                 │
│                                                                      │
│  TCP HOL 블로킹:                  QUIC 독립 스트림:                   │
│  스트림 1의 패킷 손실이           스트림 1의 패킷 손실이              │
│  모든 스트림을 차단               스트림 1만 차단                     │
│  (TCP는 하나의 바이트 스트림으로 봄) (QUIC는 스트림을 인식)           │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
```

### QUIC 연결 마이그레이션(Connection Migration)

QUIC의 가장 혁신적인 기능 중 하나: IP 주소가 변경되어도 연결이 유지됩니다.

```
기존 TCP:
  폰이 WiFi (IP: 192.168.1.10) ──TCP──► 서버
  폰이 4G로 전환 (IP: 10.0.0.5)
  → TCP 연결 끊김 (다른 IP = 다른 연결)
  → 완전한 재연결: TCP 핸드셰이크 + TLS 핸드셰이크 + 재인증

QUIC 연결 마이그레이션:
  폰이 WiFi (IP: 192.168.1.10) ──QUIC──► 서버
  폰이 4G로 전환 (IP: 10.0.0.5)
  → QUIC 연결 유지! (IP가 아닌 Connection ID로 식별)
  → 중단 없음, 재핸드셰이크 없음

이유: QUIC는 (출발 IP, 출발 포트, 목적 IP, 목적 포트) 튜플이 아닌
     64비트 Connection ID로 연결을 식별
```

### 0-RTT 연결 수립

이전에 방문한 서버에 재연결할 때, QUIC는 첫 번째 패킷에서 바로 데이터를 보낼 수 있습니다:

```
첫 연결:          1-RTT (키 교환 후 데이터 전송)
재연결:           0-RTT (캐시된 키로 즉시 데이터 전송)

  [클라이언트]                              [서버]
      │                                        │
      │──(1) ClientHello + 0-RTT 데이터 ────▶│   ← 첫 패킷에 데이터!
      │                                        │
      │◀─(2) ServerHello + 핸드셰이크 ───────│
      │                                        │
      │◀════════ 전체 통신 ═══════════════════▶│

주의: 0-RTT 데이터는 재전송 공격(replay)에 안전하지 않음.
멱등(Idempotent) 요청(GET)에만 사용. POST/PUT에는 절대 사용 금지.
```

### 종합 비교: HTTP/1.1 vs 2 vs 3

| 기능 | HTTP/1.1 | HTTP/2 | HTTP/3 |
|------|----------|--------|--------|
| **출시 연도** | 1997 | 2015 | 2022 |
| **전송 계층** | TCP | TCP | QUIC (UDP) |
| **형식** | 텍스트 | 바이너리 프레임 | 바이너리 프레임 |
| **멀티플렉싱** | 미지원 (한 번에 1개 요청/응답) | 지원 (스트림) | 지원 (스트림) |
| **HOL 블로킹** | HTTP 수준 + TCP 수준 | TCP 수준만 | 없음 |
| **헤더 압축** | 없음 | HPACK | QPACK |
| **암호화** | 선택적 (HTTPS) | 사실상 필수 | 항상 (TLS 1.3 내장) |
| **연결 수립** | 1 RTT (TCP) + 2 RTT (TLS 1.2) | 1 RTT (TCP) + 1 RTT (TLS 1.3) | 1 RTT (재연결 시 0-RTT) |
| **연결 마이그레이션** | 미지원 | 미지원 | 지원 (Connection ID) |
| **서버 푸시** | 미지원 | 지원 (실무에서 폐기) | 지원 (거의 미사용) |
| **채택률 (2025)** | ~20% 사이트 | ~35% 사이트 | ~30% 사이트 |

### HTTP/3 도입 시점

```
HTTP/3 도입이 좋은 경우:
✓ 사용자가 모바일 네트워크에 있음 (연결 마이그레이션이 도움)
✓ 사용자가 지리적으로 먼 거리에 있음 (0-RTT가 지연 시간 감소)
✓ 사이트가 많은 작은 리소스를 로드함 (HOL 없는 멀티플렉싱)
✓ CDN이 지원함 (Cloudflare, Fastly, Akamai 모두 지원)

HTTP/2를 유지하는 경우:
✓ 인프라가 UDP를 지원하지 않음 (일부 방화벽이 차단)
✓ 세밀한 트래픽 검사가 필요함 (QUIC는 항상 암호화됨)
✓ 통제된 환경 (내부 서비스, 낮은 지연 시간)
```

### HTTP/2 및 HTTP/3 테스트

```bash
# 서버가 HTTP/2를 지원하는지 확인
# Why: --http2가 HTTP/2를 강제; -I는 헤더만 가져옴
curl -I --http2 https://example.com

# 서버가 HTTP/3를 지원하는지 확인 (curl 7.66+ 필요)
# Why: --http3가 QUIC 연결 시도
curl -I --http3 https://example.com

# 협상된 프로토콜 확인
curl -w "Protocol: %{http_version}\n" -o /dev/null -s https://example.com

# openssl로 확인 (HTTP/2 ALPN 협상)
# Why: 서버가 ALPN에서 h2 (HTTP/2)를 광고하는지 확인
