...
**이전**: [물리 계층](./04_Physical_Layer.md) | **다음**: [IP 주소와 서브넷팅](./06_IP_Address_Subnetting.md)
...
1. 데이터링크 계층(Data Link Layer)의 역할을 설명하고, LLC와 MAC 서브계층을 구분할 수 있다
2. MAC 주소의 구조를 기술하고, OUI와 특수 주소(브로드캐스트, 멀티캐스트)를 해석할 수 있다
3. 이더넷 II 프레임(Ethernet II Frame) 구조를 분석하고, 각 필드의 목적을 식별할 수 있다
4. CSMA/CD의 동작 방식을 설명하고, 현대 전이중(Full-Duplex) 이더넷에서 CSMA/CD가 불필요한 이유를 말할 수 있다
5. 스위치의 학습, 전달, 플러딩(Flooding), 필터링 과정을 추적할 수 있다
6. ARP가 IP 주소를 MAC 주소로 어떻게 변환하는지 설명할 수 있다
7. 충돌 도메인(Collision Domain)과 브로드캐스트 도메인(Broadcast Domain)을 구분하고, 허브·스위치·라우터가 각 도메인에 미치는 영향을 설명할 수 있다

---

데이터링크 계층은 물리 신호와 의미 있는 네트워크 통신 사이의 간극을 메웁니다. 컴퓨터에 하드웨어 정체성(MAC 주소)을 부여하고, 비트를 프레임(Frame)으로 패킹하며, 전송 오류를 감지하고, 공유 매체에서 데이터가 올바른 인접 노드에 도달하도록 보장하는 계층입니다. 이 계층을 이해하는 것은 LAN 문제 해결, 스위치 설정, 그리고 지배적인 LAN 기술인 이더넷(Ethernet)의 실제 동작 원리를 파악하는 데 필수적입니다.
