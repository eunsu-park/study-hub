...
**이전**: [서브네팅 실습](./07_Subnetting_Practice.md) | **다음**: [라우팅 프로토콜](./09_Routing_Protocols.md)
...
1. 라우팅(Routing)이 무엇인지 설명하고, 네트워크 간 통신에서 라우터(router)의 역할을 기술할 수 있습니다
2. 라우팅 테이블(routing table)을 읽고 해석하며, 목적지(destination), 다음 홉(next hop), 메트릭(metric), 인터페이스(interface) 필드를 식별할 수 있습니다
3. 정적 경로(static route)를 설정하고 정적 라우팅이 적합한 상황을 설명할 수 있습니다
4. 동적 라우팅(dynamic routing)이 정적 라우팅과 어떻게 다른지 설명하고 그 장점을 나열할 수 있습니다
5. 기본 게이트웨이(default gateway)의 개념과 호스트 통신에서의 역할을 설명할 수 있습니다
6. 최장 접두어 매칭(longest prefix match) 규칙을 적용하여 라우터가 패킷의 최적 경로를 선택하는 방식을 결정할 수 있습니다

---

라우팅(Routing)은 인터넷이 작동하는 핵심 원리입니다. 지구 반대편 서버로 패킷을 전송할 때, 그 패킷은 수십 개의 라우터를 통과하며 각 라우터는 독립적으로 패킷을 어디로 전달할지 결정합니다. 라우팅 테이블, 정적 라우팅과 동적 라우팅의 차이, 접두어 매칭을 이해하는 것은 네트워크 설계, "목적지 도달 불가(destination unreachable)" 오류 진단, 가정용 라우터부터 기업 백본까지 모든 것을 구성하는 데 필수적입니다.
