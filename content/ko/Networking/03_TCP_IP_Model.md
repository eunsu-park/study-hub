...
**이전**: [OSI 7계층 모델](./02_OSI_7_Layer_Model.md) | **다음**: [물리 계층](./04_Physical_Layer.md)
...
1. TCP/IP 모델의 4계층과 각 계층의 역할을 설명할 수 있다
2. TCP/IP 모델과 OSI 7계층 모델을 비교하고 TCP/IP가 인터넷 표준이 된 이유를 설명할 수 있다
3. 웹 페이지 로딩 시 발생하는 전체 통신 흐름(DNS, TCP 핸드셰이크, HTTP, 캡슐화)을 추적할 수 있다
4. TCP와 UDP를 구별하고 각각의 적절한 사용 사례를 식별할 수 있다
5. 주요 보조 프로토콜(ICMP, ARP, DHCP)의 기능을 설명할 수 있다
6. 주요 응용 계층 프로토콜과 해당 포트 번호를 식별할 수 있다

---

OSI 모델이 이론적 어휘를 제공한다면, TCP/IP 모델은 실제로 인터넷을 구동하는 프로토콜 스택입니다. 웹 요청, 동영상 스트림, DNS 조회 등 당신이 보내는 모든 패킷은 이 4계층 아키텍처를 통해 처리됩니다. TCP/IP를 이해하는 것은 단순한 학문적 지식이 아니라, 네트워크 설정, 연결 문제 디버깅, 네트워크 애플리케이션 구축을 위한 실질적인 기반입니다.
...
> **왜 ICMP가 별도의 프로토콜로 존재하는가?** IP 자체에는 오류 보고 기능이 없습니다 — 패킷이 삭제되면(TTL 만료, 목적지 도달 불가) 발신자는 ICMP 없이는 알 수 없습니다. ICMP는 IP 패킷 안에 실려가지만 네트워크의 *피드백 채널* 역할을 합니다: `ping`은 Echo Request/Reply로 도달성을 테스트하고, `traceroute`는 각 홉에서 의도적으로 TTL 만료를 일으켜 경로를 매핑합니다. ICMP가 없으면 네트워크 문제 해결은 맹목적 추측이 됩니다.
