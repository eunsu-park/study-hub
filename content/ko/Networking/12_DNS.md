...
**이전**: [UDP와 포트](./11_UDP_and_Ports.md) | **다음**: [HTTP와 HTTPS](./13_HTTP_and_HTTPS.md)
...
1. DNS의 목적을 설명하고 인터넷 통신에서 왜 필수적인지 설명할 수 있습니다
2. 완전한 도메인 이름(FQDN, Fully Qualified Domain Name)을 계층적 구성 요소로 분해할 수 있습니다
3. 재귀적(Recursive) DNS 조회와 반복적(Iterative) DNS 조회를 구분할 수 있습니다
4. 루트(Root), TLD, 권한(Authoritative) 네임 서버의 역할을 식별할 수 있습니다
5. 주요 DNS 레코드 유형(A, AAAA, CNAME, MX, NS, TXT, PTR, SOA)과 사용 사례를 설명할 수 있습니다
6. DNS 캐싱 계층과 캐시 관리에서 TTL의 역할을 설명할 수 있습니다
7. DNS 진단 도구(`dig`, `nslookup`, `host`)를 사용하여 DNS 레코드를 조회하고 문제를 해결할 수 있습니다

---

**난이도**: ⭐⭐
**예상 학습 시간**: 2시간
**선수 지식**: [11_UDP_and_Ports.md](./11_UDP_and_Ports.md)

> **비유 -- 스마트폰 연락처 앱**: DNS는 인터넷의 연락처 앱입니다. 친구의 전화번호(192.168.1.100)를 외울 필요 없이 이름(google.com)을 누르면 앱이 번호를 찾아줍니다. DNS도 마찬가지입니다: 사람이 읽기 쉬운 도메인 이름을 기계가 이해하는 IP 주소로 변환합니다. 연락처가 기기 간에 동기화되고 누군가 번호를 바꾸면 업데이트되듯이, DNS도 변경 사항을 전 세계 서버 네트워크에 전파합니다.

URL을 입력하거나 이메일을 보내거나 동영상을 스트리밍할 때마다 DNS는 뒤에서 이름을 숫자로 변환하는 작업을 수행합니다. DNS가 없다면 인터넷은 기억하기 어려운 IP 주소의 바다가 될 것입니다. 이 레슨은 전 세계 이름 해석 시스템이 어떻게 구성되는지, 조회가 계층 구조를 통해 어떻게 흘러가는지, 이름 조회에 실패했을 때 어떻게 진단하는지를 다룹니다.
