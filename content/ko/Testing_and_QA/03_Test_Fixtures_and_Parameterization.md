...
다음 타임라인은 두 개의 테스트 모듈(각각 두 개의 테스트 함수 포함)을 가진 세션에서 각 scope가 fixture를 생성하고 해제하는 시점을 보여줍니다:
...
teardown 코드(`yield` 이후)에서 예외가 발생하면, pytest는 이를 FAILURE가 아닌 **ERROR**로 보고합니다. 이로 인해 원래 테스트 결과가 가려질 수 있습니다 — 통과하는 테스트가 ERROR로 나타나고, 실패하는 테스트의 어서션 메시지가 묻힐 수 있습니다. 항상 teardown을 `try`/`finally`로 보호하세요:
