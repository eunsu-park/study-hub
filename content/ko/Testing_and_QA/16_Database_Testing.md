...
**이전**: [비동기 코드 테스트](./15_Testing_Async_Code.md) | **다음**: [레거시 코드 테스트](./17_Testing_Legacy_Code.md)
...
**중요 주의 사항**: SQLite는 기본적으로 외래 키 제약 조건을 적용하지 **않습니다**. 명시적 활성화 없이는 외래 키 관계를 위반하는 `INSERT` 문이 아무런 오류 없이 성공하여, 참조 무결성 테스트가 무의미해집니다. SQLAlchemy 이벤트 리스너를 사용하여 모든 연결에서 외래 키를 활성화하세요:
...
이 리스너를 엔진 생성 직후에 배치하여 세션 fixture에서 생성된 연결을 포함한 모든 연결에 적용되도록 하세요.
