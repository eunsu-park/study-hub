...
> **주의 사항**: SQLite는 기본적으로 외래 키 제약 조건을 적용하지 **않습니다**. 명시적 활성화 없이는 외래 키 관계를 위반하는 `INSERT` 문이 아무런 오류 없이 성공합니다. 모든 연결에서 외래 키를 활성화해야 합니다:
>
> ```python
...
> ```
>
> 이 리스너를 `create_engine()` 직후에 배치하세요. 이것이 없으면 SQLite에 대한 참조 무결성 테스트는 의미가 없습니다.
