...
`spec`은 속성 *이름*을 검증하지만, 메서드 호출 *시그니처*는 검증하지 **않습니다**. `autospec=True` (또는 `create_autospec`)는 둘 다 수행합니다 — 실제 메서드 시그니처를 검사하고 잘못된 인자에 대해 `TypeError`를 발생시킵니다.
...
프로덕션 코드 mock에는 일반 `spec`보다 `autospec=True` (또는 `create_autospec`)를 선호하세요. `spec`만으로는 놓치는 — 잘못된 수의 인자로 메서드를 호출하는 것과 같은 — 유형의 버그를 잡아냅니다.
