...
> **주의 사항 — CI 러너 불안정성**: 공유 CI 러너(GitHub Actions, GitLab 공유 러너 등)는 이웃 프로세스(noisy neighbor)와 열 조절(thermal throttling)로 인해 CPU 성능이 일관되지 않습니다. 공유 인프라에서 `--benchmark-compare-fail`을 사용하면 허위 실패가 발생할 수 있습니다 — 단순히 러너의 경쟁 상태 때문에 벤치마크가 20% 느리게 나타날 수 있습니다. 완화 방법:
> - 벤치마크 작업에는 **전용/셀프 호스팅** 러너를 사용합니다
> - 넉넉한 임계값을 설정합니다 (예: `mean:10%` 대신 `mean:25%`)
> - **같은 러너 클래스**에서 기록된 베이스라인과 비교합니다
> - `--benchmark-warmup=on`을 사용하고 `--benchmark-min-rounds`를 늘립니다
> - 하드웨어를 제어하지 않는 한, CI 벤치마크 회귀를 하드 실패가 아닌 **경고**로 취급합니다
