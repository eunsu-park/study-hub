...
**이전**: [테스트 커버리지와 품질](./05_Test_Coverage_and_Quality.md) | **다음**: [통합 테스트](./07_Integration_Testing.md)
...
TDD 실천에는 두 가지 주요 학파가 있습니다:

**디트로이트 학파 (클래시스트 / Inside-Out)**: Kent Beck에서 시작되었습니다. 가장 안쪽의 도메인 객체에서 시작하여 바깥으로 구축합니다. 실용적인 한도 내에서 실제 협력자를 사용하며, 외부 경계(데이터베이스, 네트워크) 이외에는 mock을 피합니다. 테스트는 *상태* — 시스템이 무엇을 생산하는지를 검증합니다.

**런던 학파 (모키스트 / Outside-In)**: Steve Freeman과 Nat Pryce(*Growing Object-Oriented Software, Guided by Tests*)에 의해 대중화되었습니다. 가장 바깥의 인수 테스트에서 시작하여 안쪽으로 작업하며, 아직 개발되지 않은 협력자에 mock을 사용합니다. 테스트는 *상호작용* — 객체들이 어떻게 통신하는지를 검증합니다.

| 측면 | 디트로이트 (클래시스트) | 런던 (모키스트) |
|------|----------------------|----------------|
| 방향 | Inside-out | Outside-in |
| 테스트 대역 | 최소 (실제 객체) | 많음 (협력자를 mock) |
| 검증 대상 | 상태 / 출력 | 상호작용 / 메시지 |
| 리팩토링 비용 | 낮음 (내부 변경에 테스트가 생존) | 높음 (테스트가 구조에 결합) |
| 설계 가이드 | 도메인에서 자연스럽게 발현 | 역할과 프로토콜에 의해 주도 |
| 적합한 대상 | 도메인 로직, 알고리즘 | 계층형 아키텍처, API |

어느 학파도 보편적으로 우월하지 않습니다. 많은 실무자들이 양쪽을 혼합합니다: 상위 수준의 통합 설계에는 런던 스타일을, 도메인 로직에는 디트로이트 스타일을 적용합니다.
