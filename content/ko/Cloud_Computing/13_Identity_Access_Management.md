...
각 AWS 계정마다 IAM 사용자와 자격 증명을 별도로 관리하는 것은 수십~수백 개의 계정을 보유한 조직에서는 확장이 어렵습니다. **IAM Identity Center**(구 AWS Single Sign-On)는 AWS Organization의 모든 계정에 걸쳐 중앙화된 접근 관리를 제공합니다.

```bash
# IAM Identity Center 활성화 (콘솔 또는 Organizations를 통해 수행)
# 이후 계정별로 사용자/그룹에 권한 세트를 할당
...
### 9.2 SAML 2.0 페더레이션(SAML 2.0 Federation)

기존 ID 공급자(Okta, Azure AD, Google Workspace)를 보유한 조직은 SAML 2.0을 사용하여 회사 ID를 AWS에 페더레이션(연동)할 수 있습니다. 사용자는 회사 자격 증명으로 한 번만 로그인하면 임시 AWS 자격 증명을 받습니다 — 별도의 IAM 사용자가 필요하지 않습니다.

```bash
# IAM SAML ID 공급자 생성 (Okta/Azure AD의 IdP 메타데이터 사용)
aws iam create-saml-provider \
    --saml-metadata-document file://idp-metadata.xml \
    --name CorporateIdP

# 페더레이션 사용자가 가정할 수 있는 역할 생성
# 신뢰 정책이 SAML 공급자를 참조
...
### 9.3 서비스 제어 정책(SCP, Service Control Policies)

SCP는 AWS Organizations 수준에서 설정되는 조직 전체 가드레일입니다. 해당 계정의 IAM 정책이 무엇을 허용하든 상관없이, 조직 단위(OU) 내 모든 계정이 가질 수 있는 **최대 권한**을 정의합니다. SCP 자체는 권한을 부여하지 않습니다 — 오직 제한만 합니다.

```json
// SCP 예시: 모든 계정에서 CloudTrail 비활성화 방지
{
    "Version": "2012-10-17",
    "Statement": [{
...
### 9.4 권한 경계(Permission Boundary)

**권한 경계(Permission Boundary)**는 IAM 엔터티(사용자 또는 역할)가 가질 수 있는 최대 권한을 설정합니다. 엔터티에 연결된 정책이 `s3:*`를 허용하더라도, 엔터티는 권한 경계에서도 허용된 작업만 수행할 수 있습니다.

권한 경계는 주로 **역할 생성을 안전하게 위임**하는 데 사용됩니다: 개발자가 애플리케이션용 IAM 역할을 만들 수 있도록 허용하되, 자신의 권한 수준을 초과하는 역할을 만들지 못하게 합니다(권한 상승 방지).

```bash
# 권한 경계 정책 생성 (읽기 전용 S3 + CloudWatch로 제한)
aws iam create-policy \
