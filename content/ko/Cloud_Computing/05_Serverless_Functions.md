...
**Lambda 레이어(Layers)**를 사용하면 공유 라이브러리, 런타임, 데이터 파일을 함수 코드와 별도로 패키징하여 여러 함수에서 재사용할 수 있습니다. 주요 사용 사례로는 과학 라이브러리(`numpy`, `pandas`)와 내부 유틸리티가 있습니다.

```bash
# Python 라이브러리가 포함된 zip으로 레이어 생성
zip -r numpy-layer.zip python/   # /python/ 디렉토리 구조를 따라야 함
...
**데드 레터 큐(DLQ, Dead Letter Queue)**는 재시도 후에도 Lambda가 처리하지 못한 이벤트를 캡처합니다. 실패한 비동기 호출은 SQS 큐 또는 SNS 토픽으로 전송되어, 이벤트를 자동으로 손실하는 대신 검사, 재처리 또는 알림을 받을 수 있습니다.

```bash
# Lambda 함수에 SQS DLQ 설정
aws lambda update-function-configuration \
    --function-name my-function \
    --dead-letter-config TargetArn=arn:aws:sqs:ap-northeast-2:123456789012:my-dlq
```

**VPC 연결 Lambda**를 사용하면 인터넷에서 접근할 수 없는 RDS 데이터베이스나 ElastiCache 클러스터 같은 프라이빗 리소스에 함수가 접근할 수 있습니다. 단, Lambda가 시작 시 VPC에 탄력적 네트워크 인터페이스(ENI)를 프로비저닝해야 하므로 콜드 스타트 지연이 증가합니다(~500ms–1초).
