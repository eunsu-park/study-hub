...
## 7. AWS 트랜짓 게이트웨이(Transit Gateway)

VPC 피어링은 두 VPC를 연결하는 데 적합하지만, 대규모 조직에서는 확장이 어렵습니다. N개의 VPC가 있을 때 피어링은 N×(N-1)/2개의 개별 연결이 필요합니다 — 10개 VPC면 45개의 피어링 연결이 필요하고, 각각 고유한 라우팅 테이블 항목이 있습니다.

**트랜짓 게이트웨이(Transit Gateway)**는 단일 게이트웨이를 통해 여러 VPC와 온프레미스 네트워크를 연결하는 중앙 허브(허브-스포크 아키텍처, hub-and-spoke) 역할을 합니다.

```
VPC 피어링 (N² 연결):               트랜짓 게이트웨이 (N 연결):
VPC-A ──── VPC-B                        VPC-A ──┐
  │    ╲  ╱  │                          VPC-B ──┤
...
**VPC 피어링 대비 주요 장점:**
- **전이적 라우팅(Transitive routing)**: VPC-A는 직접 피어링 연결 없이도 트랜짓 게이트웨이를 통해 VPC-C에 접근할 수 있습니다.
- **중앙화된 라우팅 관리**: N²개의 개별 피어링 라우팅 항목 대신 하나의 라우팅 테이블로 관리합니다.
- **온프레미스 통합**: Site-to-Site VPN 또는 Direct Connect를 통해 회사 데이터 센터를 트랜짓 게이트웨이에 연결하면, 연결된 모든 VPC가 즉시 온프레미스 리소스에 접근할 수 있습니다.
...
# 2. VPC를 트랜짓 게이트웨이에 연결
aws ec2 create-transit-gateway-vpc-attachment \
    --transit-gateway-id tgw-12345678 \
    --vpc-id vpc-A \
    --subnet-ids subnet-A-private

# 3. VPC-A의 라우팅 테이블에 VPC-B의 CIDR을 TGW로 향하는 라우팅 추가
aws ec2 create-route \
...
**트랜짓 게이트웨이 vs VPC 피어링 선택 기준:**

| 기준 | VPC 피어링 | 트랜짓 게이트웨이 |
|------|------------|-----------------|
| VPC 수 | 2–5개 | 5개 이상 |
| 전이적 라우팅 필요 여부 | 불필요 | 필요 |
| 온프레미스 연결 | 직접 불가 | 가능 (VPN/Direct Connect) |
| 비용 | 무료 (데이터 전송 비용만) | $0.05/시간 + $0.02/GB |
