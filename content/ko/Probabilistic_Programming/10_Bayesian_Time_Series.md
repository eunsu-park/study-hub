# 10. 베이지안 시계열(Bayesian Time Series)

[이전: 가우시안 프로세스](./09_Gaussian_Processes.md) | [다음: 베이지안 최적화](./11_Bayesian_Optimization.md)

---

> **프레임워크 참고**: 이 레슨은 베이지안 시계열 모델링을 위해 PyMC 5.x와 Prophet을 사용합니다.
>
> 설치: `pip install pymc arviz numpy matplotlib pandas prophet`

## 학습 목표

- 베이지안 사전분포를 사용한 구조적 시계열 모델 구축
- 상태공간 모델(state-space model)과 칼만 필터(Kalman filter) 이해
- 분해 가능한 시계열을 위한 Facebook Prophet 사용
- 추세, 계절성, 변화점을 확률적으로 모델링
- 사후 예측 분포로 예측 불확실성 정량화

---

## 1. 시계열에 대한 베이지안 접근(Bayesian Approach to Time Series)

전통적인 시계열 방법(ARIMA)은 매개변수를 고정값으로 취급합니다. 베이지안 접근법은 추세, 계절성, 노이즈, 변화점 등 모든 구성요소의 불확실성을 정량화합니다.

### 1.1 구조적 시계열 분해(Structural Time Series Decomposition)

$$y_t = \mu_t + s_t + \epsilon_t$$

여기서 $\mu_t$는 추세, $s_t$는 계절성, $\epsilon_t$는 관측 노이즈입니다.
...
## 2. 국소 선형 추세 모델(Local Linear Trend Model)
...
## 3. 베이지안 계절성 모델(Bayesian Seasonal Model)
...
## 4. 변화점 감지(Changepoint Detection)
...
Prophet은 자동 변화점 감지를 갖춘 분해 가능한 시계열 모델을 구현합니다.
...
## 6. 상태공간 모델(State-Space Models)

### 6.1 칼만 필터(Kalman Filter)
...
### 6.2 PyMC에서의 베이지안 상태공간(Bayesian State-Space in PyMC)
...
## 7. 자기회귀 모델(Autoregressive Models)
...
## 8. 불확실성을 포함한 예측(Forecasting with Uncertainty)
...
| 모델 | 구성요소 | 추론 | 적합한 경우 |
|------|---------|------|-----------|
| 국소 선형 추세(Local Linear Trend) | 수준 + 기울기 | MCMC/칼만 | 단기, 비계절적 |
| 구조적 시계열(Structural TS) | 추세 + 계절성 | MCMC | 분해 가능한 시리즈 |
| Prophet | 추세 + 공휴일 + 계절성 | MAP/MCMC | 비즈니스 예측 |
| 상태공간(State-Space) | 잠재 상태 | 칼만/MCMC | 실시간 필터링 |
| 베이지안 AR(Bayesian AR) | 자기회귀 | MCMC | 정상 시리즈 |
