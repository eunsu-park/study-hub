...
아래 표는 노드 1이 루트, 2가 1의 중간 자식, 4가 2의 중간 자식, 3과 5가 경자식(light child)인 작은 트리에서 HLD를 보여줍니다:

| 트리 노드 | DFS 순서 (pos) | 체인 헤드 | 체인 인덱스 | 세그먼트 트리 인덱스 |
|----------|--------------|-----------|------------|------------------|
| 1 (루트) | 0 | 1 | 0 | 0 |
| 2 (1의 중간 자식) | 1 | 1 | 1 | 1 |
| 4 (2의 중간 자식) | 2 | 1 | 2 | 2 |
...
노드 1, 2, 4는 하나의 중간 체인을 형성합니다(헤드 = 1, 연속 위치 0–2). 노드 3과 5는 각각 독립 체인을 시작합니다. 노드 4에서 노드 3까지의 경로 쿼리는 하나의 경간선을 넘습니다: 노드 3의 체인에 대해 구간 [pos[3], pos[3]] = [3, 3]을 쿼리하고, 노드 4를 포함하는 체인에 대해 구간 [pos[1], pos[4]] = [0, 2]를 쿼리하여 O(log²N)에 결과를 누적합니다.
