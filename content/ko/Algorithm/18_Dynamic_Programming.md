...
#### LIS O(n log n): 인내심 정렬(Patience Sorting) 직관

O(n log n) 알고리즘은 **인내심 정렬(patience sorting)**이라는 카드 게임과 동일합니다. 각 숫자를 더미(pile) 위에 올린다고 상상하세요: 현재 숫자보다 위 카드 값이 크거나 같은 더미 중 가장 왼쪽에 올리거나, 그런 더미가 없으면 새 더미를 시작합니다. 마지막 더미의 수가 LIS 길이와 같습니다.

위 코드의 `tails` 배열은 지금까지 관찰된 각 길이의 증가 부분 수열의 최솟값 꼬리(tail)를 저장합니다. `tails[i]`는 길이 `i+1`의 증가 부분 수열을 끝낼 수 있는 가장 작은 수입니다. 이 배열은 항상 정렬된 상태를 유지하므로, `bisect_left`가 O(log n)에 올바른 더미를 찾을 수 있습니다. `tails[pos]`를 더 작은 수로 교체하면 LIS 길이는 변하지 않지만 꼬리를 최대한 작게 유지하여 미래의 수가 더 쉽게 확장할 수 있습니다.

**중요**: `tails` 배열은 실제 LIS가 **아닙니다** — 압축된 인코딩입니다. 실제 부분 수열을 복원하려면 이진 탐색 중에 각 원소의 선행자(predecessor)를 기록해야 합니다.

#### 롤링 배열(Rolling Array) 메모리 최적화

표준 O(n²) DP는 n개짜리 `dp` 배열을 사용하며, 이미 O(n)입니다. 그러나 2D DP 변형(예: 가중치가 있는 LIS, 2D 냅색)에서는 공간이 지배적 요소가 될 수 있습니다. **롤링 배열** 기법은 `dp[i][j]`가 이전 행 `dp[i-1][...]`에만 의존하는 경우 O(n²) 공간의 2D `dp[i][j]` 테이블을 두 개의 1D 행(O(n) 공간)으로 줄입니다:

```python
# 완전한 2D 테이블: O(n*W) 공간
dp = [[0] * (W + 1) for _ in range(n + 1)]

# 롤링 배열: O(W) 공간 — 이전 행을 제자리에서 덮어씀
dp = [0] * (W + 1)
for i in range(n):
...
역순 반복 방향은 0/1 냅색에서 매우 중요합니다: 정방향 반복은 같은 아이템을 여러 번 선택할 수 있게 하여 무제한 냅색(unbounded knapsack)이 됩니다. 이 패턴은 각 행이 바로 이전 행에만 의존하는 모든 DP에 적용됩니다.
