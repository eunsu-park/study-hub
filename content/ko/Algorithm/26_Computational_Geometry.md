...
#### 심화: 엡실론 선택과 볼록 껍질의 일직선 점 처리

적절한 엡실론을 선택하는 것은 간단하지 않습니다. 너무 작으면(예: 1e-15) 누적된 부동소수점 오차로 인해 일직선 점이 비일직선으로 잘못 분류됩니다. 너무 크면(예: 1e-3) 실제로 다른 점들이 잘못 일직선으로 처리됩니다. 좌표가 [−10⁶, 10⁶] 범위라면 `EPS = 1e-9`가 일반적인 휴리스틱입니다. 하지만 좌표가 훨씬 크거나 연산에 곱셈이 많이 포함된 경우, 그에 맞게 EPS를 조정하거나 **정수 연산**으로 전환하세요(모든 좌표에 10의 거듭제곱을 곱해 소수를 없앱니다).

**볼록 껍질의 일직선 점** 처리는 의도적인 선택이 필요합니다. 두 가지 주요 방침은 다음과 같습니다:

1. **일직선 점 제외** (엄격한 껍질): `ccw(...) <= 0`일 때 pop하여, 연속된 세 껍질 꼭짓점이 일직선이 되지 않도록 합니다. 최소 꼭짓점 집합을 얻습니다.
2. **일직선 점 포함** (모든 경계 점): `ccw(...) < 0`일 때만 pop합니다(실제 우회전만 제거). 껍질 변 위의 중간 점도 유지합니다.

```python
# 엄격한 껍질 — 일직선 경계 점 제외 (알고리즘 대회에서 가장 일반적)
while len(hull) >= 2 and ccw(points[hull[-2]], points[hull[-1]], points[i]) <= 0:
    hull.pop()

# 일직선 포함 껍질 — 경계 위의 모든 점 유지
while len(hull) >= 2 and ccw(points[hull[-2]], points[hull[-1]], points[i]) < 0:
    hull.pop()
```

문제에서 경계 격자점의 수나 둘레를 물을 때는 일직선 포함 변형을 사용합니다. 껍질을 정의하는 최소 꼭짓점 집합을 물을 때는 엄격한 변형을 사용합니다.
