...
## 8. Aho-Corasick: 다중 패턴 매칭

### 동기

KMP, Rabin-Karp, Z-알고리즘은 각각 **한 번에 하나의** 패턴을 검색합니다. 길이 n인 텍스트에서 **k개의** 패턴 전체를 동시에 찾아야 할 때, KMP를 k번 실행하면 O(n × k + 전체 패턴 길이)가 소요됩니다. **Aho-Corasick**은 이를 O(n + 전체 패턴 길이 + 매칭 수)로 줄입니다 — 패턴 수와 무관하게 텍스트 길이에 선형입니다.

### 핵심 아이디어

Aho-Corasick은 모든 패턴으로 **트라이(trie)**를 구축한 후, 부분 매칭이 실패했을 때 자동기계가 현재 상태의 가장 긴 고유 접미사이자 어떤 패턴의 접두사인 상태로 전이할 수 있도록 **실패 링크(failure link)**를 추가합니다(KMP 실패 함수와 유사). 이를 통해 텍스트를 왼쪽에서 오른쪽으로 한 번만 스캔합니다.

```
패턴: {"he", "she", "his", "hers"}

실패 링크가 있는 트라이 (단순화):
  root → h → e → r → s   (패턴: "hers")
             ↑ (패턴: "he")
  root → s → h → e       (패턴: "she")
             ↑ 실패 링크: s→h→e가 첫 번째 분기의 h→e를 재사용
  root → h → i → s       (패턴: "his")
```

실패 링크가 핵심 최적화입니다: 불일치 시 루트부터 재시작하는 대신, 자동기계가 실패 링크를 따라 더 짧은 매칭 접두사로 이동하여 계속 진행합니다.

### Aho-Corasick을 사용해야 하는 경우

| 시나리오 | 최적 알고리즘 |
|---------|--------------|
| 텍스트에서 단일 패턴 | KMP 또는 Z-알고리즘 |
| 짧은 패턴 소수, 쿼리 적음 | Rabin-Karp (구현이 더 간단) |
| 많은 패턴, 큰 텍스트, 모든 매칭 필요 | Aho-Corasick |
| 패턴이 자주 바뀜 | Rabin-Karp 또는 반복 KMP |

Aho-Corasick이 필요한 전형적인 알고리즘 대회 문제: 바이러스/키워드 필터링, DNA 서열 스크리닝, 사전 매칭, "이 텍스트에 k개의 단어 중 어떤 것이 등장하는가?"를 묻는 모든 문제.

### 복잡도

| 단계 | 시간 | 공간 |
|------|------|------|
| 트라이 + 실패 링크 구축 | O(전체 패턴 길이 × 알파벳 크기) | O(전체 패턴 길이 × 알파벳 크기) |
| 텍스트 검색 | O(n + 매칭 수) | O(1) 추가 |

알파벳 크기 Σ(영소문자의 경우 26)에 대해 각 트라이 노드는 Σ개의 자식 포인터를 저장합니다. 알파벳이 클 경우, 노드당 해시맵을 사용하면 약간의 상수 시간 비용으로 공간을 줄일 수 있습니다.
