...
아래 표는 노드 s, a, b, c, d, t와 간선 용량 s→a=6, s→c=4, a→b=4, a→d=3, b→t=5, c→d=3, d→t=5인 작은 네트워크에서 Ford-Fulkerson을 추적합니다:
...
각 단계에서 경로의 병목(최소 잔여 용량)이 보낼 수 있는 유량을 제한합니다. 3단계 후에는 잔여 용량이 양수인 s→t 경로가 없으므로 알고리즘이 종료됩니다.
...
### Ford-Fulkerson의 함정과 그 너머

**실수값 용량과 비종료**: DFS 기반 Ford-Fulkerson은 모든 용량이 정수(또는 유리수)일 때만 종료가 보장됩니다. 비합리적(irrational) 용량이 있을 경우, 알고리즘이 무한히 순환하면서 실제 최대 유량보다 작은 값으로 수렴하는 임의로 작은 개선만 반복할 수 있습니다. 고전적인 반례는 1/φ(황금비)를 포함하는 용량을 사용하며, DFS가 잘못된 값으로 수렴하는 증가 경로를 계속 선택합니다. **해결책**: 항상 Edmonds-Karp(BFS) 또는 Dinic 알고리즘을 사용하세요. 이들은 용량 유형에 관계없이 다항 시간 내에 종료됩니다.

**병리적 DFS 경로 선택**: 정수 용량이더라도, DFS 기반 Ford-Fulkerson은 O(E × max_flow)번의 증가 단계를 거칠 수 있습니다 — 예를 들어 용량 1짜리 중앙 병목 간선이 있을 때 두 경로를 번갈아 선택하는 경우입니다. Edmonds-Karp는 항상 최단 증가 경로를 선택함으로써 이를 방지합니다.

**최소 비용 최대 유량(Min-Cost Max-Flow, MCMF)**: 각 간선에 용량과 단위 유량당 비용이 모두 있을 때, 목표는 유량을 최대화하면서 총 비용을 최소화하는 것(또는 주어진 양의 유량을 최소 비용으로 보내는 것)이 됩니다. MCMF는 최소 비용 증가 경로를 찾기 위해 BFS 대신 SPFA(Bellman-Ford 큐) 또는 포텐셜을 사용한 Dijkstra를 사용합니다. 주요 응용 사례로는 비용이 있는 최적 작업-작업자 배정, 용량 제약 하의 최저 비용 라우팅, 수송 문제 등이 있습니다.
