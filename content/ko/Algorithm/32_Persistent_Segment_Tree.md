...
두 접두사 트리를 노드 단위로 빼면 범위 [l, r]에 대한 빈도 분포를 얻습니다:

```
K번째 최솟값 쿼리: count[version_r] - count[version_l-1]
...
"Difference" 트리의 각 노드는 `arr[l..r]`에서 해당 노드의 범위에 순위가 속하는 원소의 수를 담습니다. K번째 최솟값을 찾으려면 차이 트리를 내려갑니다: 왼쪽 자식의 카운트 ≥ K이면 왼쪽으로, 그렇지 않으면 K에서 왼쪽 카운트를 빼고 오른쪽으로 갑니다. 이 O(log M) 탐색이 O(M) 선형 스캔을 대체합니다.
