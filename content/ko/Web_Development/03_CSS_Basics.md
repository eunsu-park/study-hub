...
참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. CSS는 "선택자 + 캐스케이드(cascade) + 상속(inheritance) + 우선순위(specificity) → 요소별 속성별 최종 값"이라는 결정론적 알고리즘이며, 박스 모델은 그 값들이 시각적으로 무엇을 의미하는지를 결정합니다.

---

## 이론과 원리

CSS는 속성의 나열로 보입니다 — 그렇게 출하되기 때문입니다. 하지만 그 동작은 브라우저가 레이아웃 패스마다 요소마다 한 번씩 실행하는 *알고리즘* 입니다. 모든 "왜 이게 안 되지" 디버깅 세션은 같은 곳에서 끝납니다 — 알고리즘이 여러분이 기대한 것과 다른 값을 골랐고, 그 이유는 완벽히 추적 가능합니다. 이 절에서는 다음 절들의 규칙들이 더 이상 잡학으로 보이지 않도록, 움직이는 부품들의 이름을 짓겠습니다.

### A. 캐스케이드(Cascade): 속성이 값을 얻는 방법
...
### B. 상속과 계산 값(Computed Value) 파이프라인
...
- **CSS란 / 적용 방법 / 문법**(섹션 1–3)은 §A의 캐스케이드에 들어가는 선언들의 표면 형태를 줍니다.
- **선택자**(섹션 4)는 §D의 술어 언어이며, §A의 specificity 값을 가집니다.
- **박스 모델**(섹션 5)은 §C이며, 각 사각형을 채우는 속성 이름들이 추가됩니다.
- **우선순위 / 캐스케이드**(섹션 6–7)는 §A를 명시적으로 만들어 어떤 규칙이 이길지 예측할 수 있게 합니다.
- **상속**(섹션 8)은 §B 파이프라인의 중간이며, 명명된 키워드(`inherit`, `initial`, `unset`)가 같이 옵니다.
- **색상 / 타이포그래피 / 배경 / 효과**는 이 모든 기계를 통과해 평가되는 값들의 속성 카탈로그입니다.

레슨의 나머지를, 모든 속성이 캐스케이드가 결정하고, 여러분의 요소 컨텍스트에 대해 계산되고, 박스 안에 페인트된 값임을 알고 읽으세요.
