...
참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 검색 엔진은 *세 단계 파이프라인* (크롤 → 인덱스 → 랭크)을 실행하며, 모든 SEO 전술 — `robots.txt`, sitemap, canonical, meta 태그, structured data, Core Web Vitals — 은 그중 한 특정 단계에 대한 개입입니다.
...
오픈 웹의 SEO 조언은 의식(ritual)의 잡동사니 — "h1을 한 번 써라", "schema를 추가하라", "Core Web Vitals를 개선하라" — 처럼 보이는 경향이 있습니다. 그 아래의 시스템은 의식이 보이게 하는 것보다 단순합니다 — 모든 검색 엔진은 세 단계 파이프라인(**크롤 → 인덱스 → 랭크**)을 실행하고, 모든 SEO 행동은 그중 한 특정 단계에 대한 개입입니다. 단계에 이름을 붙이는 것이 의식들을 작고 예측 가능한 규칙서로 바꿔 줍니다.

### A. 크롤(Crawl): 페이지가 발견되는 방법

크롤러(Googlebot, Bingbot, GPTBot)는 링크를 따라가고 sitemap을 읽음으로써 웹을 순회합니다. 발견하는 각 도메인에 대해 **크롤 예산(crawl budget)** 이 있습니다 — 사이트의 응답성, 오류율, 인지된 중요도에서 도출되는 단위 시간당 요청의 부드러운 상한입니다. 느리거나 오류가 잦은 사이트는 예산을 재시도에 태우고, 빠르고 잘 구조화된 사이트는 그것을 새 콘텐츠에 씁니다.

세 가지 제어가 크롤 동작의 모양을 잡습니다.

- **`robots.txt`** — `https://example.com/robots.txt`에 위치, 크롤러에게 어느 경로를 건너뛸지 알립니다. *제안* 입니다(잘 행동하는 크롤러는 따르고, 악의적 크롤러는 따르지 않음). 그리고 *접근* 을 제어할 뿐 *인덱싱* 을 제어하지는 않습니다. `Disallow:` 페이지도 다른 페이지가 그것을 가리키면 결과에 나타날 수 있습니다 — URL이 설명 없이 보입니다.
- **`<meta name="robots" content="noindex">`** — 페이지의 `<head>`에. 크롤러에게 "가져갈 수는 있지만, 인덱스에 넣지 마라"라고 말합니다. 이것이 페이지를 결과에서 숨기는 올바른 방법입니다. 결정적으로, 크롤러가 이 지시문을 보려면 페이지를 *가져올* 수 있어야 합니다.
- **`sitemap.xml`** — `robots.txt`에서 참조됨. 크롤되기를 원하는 URL의 목록이며, 선택적인 마지막 수정 일자가 있습니다. sitemap은 랭킹을 올리지 않지만, 링크만으로 놓칠 수 있는 페이지(고아 페이지, 깊은 페이지, 페이지네이션된 아카이브)를 크롤러가 찾는 데 도움이 됩니다.

암묵적 네 번째 제어는 *빠른 것* 입니다. 5초 TTFB나 500 응답 행렬은 크롤러에게 더 적게 오라고 가르칩니다.

### B. 인덱스(Index): 무엇이, 어디에 저장되는가

크롤된 각 페이지에 대해 인덱서는 HTML을 파싱하고, JavaScript를 실행하고(Googlebot은 최신 Chromium을 사용), 텍스트를 추출하고, 접근성 트리를 짓고(예 — 접근성이 이제 인덱싱의 일부입니다), 역인덱스(inverted index)에 *무엇을* 저장할지 결정합니다.

저장 결정을 지배하는 여러 신호:

- **`<title>`과 `<meta name="description">`** — 검색 결과 스니펫이 보여주는 것. 타이틀은 그 안에 담긴 키워드에 대한 가장 강한 단일 온페이지 랭킹 신호이고, 디스크립션은 직접 랭크하지 않지만 클릭률을 결정합니다.
- **`<link rel="canonical" href="...">`** — 같은 콘텐츠가 여러 URL(`?utm_source=...`, 정렬 순서, 페이지네이션된 뷰)에 살 때를 위해. canonical은 인덱서에게 "진짜 URL은 이것이다 — 거기에 신호를 통합하라"라고 말합니다. 이것이 없으면 대안 URL에 대한 링크가 권위를 가르게 됩니다.
- **헤딩 계층.** 페이지당 단일 `<h1>`이 페이지를 기술하고, 중첩된 `<h2>`/`<h3>`/...가 그 구조를 개괄합니다. 인덱서는 헤딩 단어에 본문 단어보다 더 가중치를 줍니다.
- **JSON-LD를 통한 구조화 데이터** — `application/ld+json` 콘텐츠를 가진 script 태그로, [schema.org](https://schema.org) 어휘로 페이지를 기술. 이것이 **리치 결과(rich results)** 를 잠금 해제합니다 — 제품 페이지의 별점, 검색 결과의 FAQ 아코디언, 이벤트 일자, 레시피 이미지. JSON-LD가 권장 형식인 이유는 보이는 HTML과 분리되어 있기 때문입니다.
- **Open Graph와 Twitter Cards** — `<meta property="og:*">` 태그로, URL이 소셜 미디어에 공유될 때 어떻게 렌더되는지를 지배. 검색 랭킹 신호는 아니지만, 소셜에서의 클릭률의 큰 동인입니다.

인덱싱은 또한 **JavaScript로 렌더되는 콘텐츠** 가 빠질 수 있는 곳입니다. 페이지가 비어 있는 `<div id="root"></div>`이고 fetch 후 React로만 채워진다면, Googlebot은 콘텐츠를 보기 전에 그것을 렌더해야 합니다 — 이는 지연(latency)과 두 번째 패스 큐를 더합니다. 서버 사이드 렌더링, 정적 생성, 또는 점진적 렌더링(Next.js, Nuxt, Astro)이 이를 우회합니다.

### C. 랭크(Rank): 저장된 페이지가 정렬되는 방법

각 쿼리에 대해 엔진은 역인덱스에서 매칭 문서를 가져와 정렬합니다. 랭킹 신호는 Google 규모에서 수백 개 깊지만, 네 범주로 군집합니다.

1. **관련성(relevance)** — 페이지가 실제로 쿼리에 답하는가? 단어 빈도, 의미적 유사성(이제 대형 언어 모델을 통해), 그리고 엔티티/주제 매칭.
2. **권위(authority)** — 다른 평판 좋은 페이지가 여기로 링크하는가? PageRank 스타일의 링크 그래프 신호, 도메인 신뢰 신호로 보강.
3. **사용자 의도 적합성(user intent fit)** — 페이지가 사용자가 찾는 결과의 *종류* (정보적, 거래적, 내비게이셔널, 지역적)와 일치하는가? 신선도, 형식, 지역성.
4. **사용자 경험(user experience)** — Core Web Vitals(LCP, INP, CLS — 레슨 18 참조), HTTPS, 모바일 친화성, 침입적 인터스티셜, 접근성.

관련성과 권위는 직접 엔지니어링할 수 없습니다 — 다른 사람들이 링크하기로 선택하는 유용한 콘텐츠를 쓰는 것에 의존합니다. 4번 범주 — 기술적 기반 — 은 *직접* 엔지니어링할 수 있습니다.

### D. 왜 "블랙햇" 전술이 더 이상 작동하지 않는가

알고리즘의 각 세대는 한 부류의 지름길을 은퇴시켰습니다 — 키워드 채우기(1990년대), 링크 농장(2000년대), 정확 매치 도메인(2010년대), 얇은 AI 콘텐츠(2020년대). 모던 신호 스택은 충분히 정교하고 — 적발된 조작에 대한 페널티가 충분히 무거워서 — 지속 가능한 전략은 이 레슨이 가르치는 것입니다 — 크롤하기 쉽고, 인덱싱하기 쉽고, 독자에게 진정으로 유용한 페이지를 출하하는 것. 그 외의 모든 것은 다음 알고리즘 업데이트가 여러분을 잡지 못하리라는 도박입니다.

### 이론에서 아래 참조로

- **SEO 개요**(섹션 1)는 §A–§C의 파이프라인에 이름을 붙입니다.
- **메타 태그**(섹션 2)는 §B의 저장 제어입니다 — `<title>`, `<meta description>`, canonical, robots meta, Open Graph.
- **구조화 데이터**(섹션 3)는 §B의 리치 결과 잠금 해제입니다 — schema.org 어휘를 가진 JSON-LD.
- **기술적 SEO**(섹션 4)는 §A의 크롤 제어입니다 — `robots.txt`, sitemap, 리다이렉트, 모바일 친화성.
- **콘텐츠 SEO**(섹션 5)는 §C의 관련성 범주입니다 — 키워드 리서치, 헤딩, 내부 링킹.
- **측정과 도구**(섹션 6)는 Search Console(엔진의 사이트 뷰)과 Lighthouse(엔진 뷰에 대한 여러분의 뷰)를 다룹니다.

레슨의 나머지를, 파이프라인을 염두에 두고 읽으세요. 모든 지시문, 태그, 파일은 한 특정 단계를 제어합니다.
