...
프로젝트에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 세 가지 아키텍처 패턴이 프로젝트들을 관통합니다 — DOM이 그것으로부터 렌더되는 단일 진실 출처(single source of truth, Todo), 로딩/에러/빈/성공 UI 상태를 가진 비동기 데이터 페칭 파이프라인(Weather), 그리고 포커스 관리가 있는 옵저버 주도의 점진적 로딩(Gallery)입니다.
...
이전 레슨들은 각각 플랫폼의 한 조각 — DOM, 이벤트, 비동기, 스토리지 — 에 초점을 맞췄습니다. 실전 애플리케이션은 그 조각들을 작동하는 전체로 *통합* 하는 것이며, 그 통합은 적은 수의 반복되는 아키텍처 패턴에 의해 지배됩니다. 이 절에서는 그 패턴들에 이름을 붙여, 아래 프로젝트들이 임시변통 스크립트로 보이는 대신 다른 데이터에 적용된 같은 모양으로 보이도록 합니다.

### A. 단일 진실 출처(Single Source of Truth)로서의 상태

작은 앱조차 *상태(state)* 를 가집니다 — 할 일 목록, 현재 날씨 데이터, 열린 이미지의 인덱스. 단순한 접근은 DOM에서 상태를 읽고(`querySelectorAll('li')`로 todos를 얻기) 명령적으로 쓰는 것입니다. 30줄짜리 데모에는 작동하지만, 필터링·영속성·실행 취소(undo)를 추가할 때쯤이면 무너집니다 — DOM이 이제 *표시* 와 *진실* 둘 다이고, 둘이 어긋나기 때문입니다.

전문가의 패턴은 **상태 우선(state-first)** 입니다.

```
state (메모리) → render(state) → DOM
              ↑                    │
              └── 이벤트 핸들러 ────┘
```

1. 상태를 평범한 JavaScript 값으로 보관합니다(`let todos = []`, `let weather = null`).
2. 관련 DOM을 비우고 상태로부터 다시 짓는 단일 `render(state)` 함수를 작성합니다.
3. 이벤트 핸들러는 *상태* 를 갱신하고 `render()`를 호출합니다. DOM을 직접 만지지 않습니다.

모든 프레임워크(React, Svelte, Vue)가 자동화하는 것이 이것입니다. 손으로 한 번 해 보면 프레임워크가 무엇을 사 주는지 배우게 됩니다. 비용은 한 층의 추가, 보상은 화면이 항상 상태의 함수라는 점입니다 — 이는 버그가 "상태가 잘못되었다"로 환원된다는 뜻이고, DOM 곳곳이 아니라 한 변수에서 관찰 가능합니다.

### B. 영속성: 상태가 페이지보다 오래 살아야 할 때

탭 새로 고침은 JavaScript 메모리를 지웁니다. todos가 살아남기를 원한다면 **영속성 경계(persistence boundary)** — 상태를 직렬화해 내보내고 다시 읽어 들이는 단계 — 가 필요합니다. 플랫폼은 비용/능력 프로필이 다른 세 내장 저장소를 제공합니다.

- **`localStorage`** — 동기, 문자열만, ~5–10MB, 비울 때까지 지속. 작은 UI 상태와 작은 목록에 이상적. 동기는 메인 스레드를 막는다는 뜻이므로, 뜨거운 루프를 여기 두지 마세요.
- **`sessionStorage`** — 같은 API, 탭에 스코프되며 탭이 닫히면 비워집니다.
- **`IndexedDB`** — 비동기, 구조화(객체, 블롭), 큰 할당량. `localStorage`를 넘어설 때 사용.

살아남는 모양은 `JSON.stringify`를 깔끔히 통과하는 것이면 무엇이든입니다. 함수, `Map`/`Set`, `Date` 객체, 순환 참조는 그렇지 않습니다 — `JSON.parse(JSON.stringify(...))`가 그것들을 조용히 떨어뜨리거나 다시 씁니다. Todo 앱이 클래스 인스턴스가 아니라 ID와 문자열을 저장하는 이유입니다.

### C. 비동기 UI 라이프사이클: 로딩, 에러, 빈, 성공

모든 fetch 주도 뷰는 눈으로 구별해야 할 *최소 네 가지* 시각 상태를 가집니다.

1. **로딩(loading)** — 요청이 진행 중. 스켈레톤이나 스피너 표시.
2. **에러(error)** — 요청 실패(네트워크, HTTP, 파싱, abort). 재시도 옵션이 있는 메시지 표시.
3. **빈(empty)** — 요청은 성공했지만 데이터가 없음. 빈 화면 대신 명확한 "결과 없음" 메시지 표시.
4. **성공(success)** — 데이터를 렌더.

대부분의 "페이지가 망가졌다" 버그 보고는 빠진 케이스로 추적됩니다 — 결코 해소되지 않는 스피너(에러 UI 없음), 성공으로 조용히 렌더된 404, 또는 여전히 로딩 중인 요청과 구별되지 않는 빈 결과.

네 가지를 모두 존중하는 요청 핸들러의 모양:
...
async function load(query, signal) {
  ...
  try {
    ...
    if (err.name === 'AbortError') return; // 사용자가 떠남
    ui.setState('error', err.message);
  }
}
```

여기에 `AbortController`의 `signal`을 더해 빠르게 타이핑하는 사용자가 오래된 요청의 결과를 보지 않도록 합니다. 튜토리얼에서 가장 적게 쓰이고 프로덕션에서 가장 많이 필요한 단일 패턴입니다.

### D. 폴링 대신 옵저버: IntersectionObserver와 동료들

"이 요소가 보이는가" 또는 "사용자가 바닥 근처로 스크롤했는가"를 감시하던 오래된 코드는 `scroll` 리스너를 붙이고 모든 스크롤 이벤트마다 `getBoundingClientRect()`를 읽었습니다. 그것은 동시에 두 가지 성능 재앙입니다 — 고빈도 이벤트 핸들러, 그리고 호출당 강제 레이아웃(레슨 07 §A).

현대 웹 플랫폼 API는 폴링을 **옵저버(observer)** 로 대체합니다 — 브라우저가 컴포지터 스레드에서 일을 하고 무언가가 바뀌었을 때만 콜백합니다.

- **`IntersectionObserver`** — 요소가 뷰포트(또는 다른 루트)에 들어오거나 떠날 때 통보. 무한 스크롤(마지막 항목을 관찰; 나타나면 더 로드)과 지연 로딩(이미지를 관찰; 보이려 할 때 `src`를 설정)을 구동합니다.
- **`MutationObserver`** — 자손이 추가·제거·수정될 때 통보.
- **`ResizeObserver`** — 요소의 박스 크기가 바뀔 때 통보. 컨테이너 쿼리가 그 위에서 작동하는 것이 이것입니다.
- **`PerformanceObserver`** — 성능 항목(LCP, INP, long task)이 기록될 때 통보.

일반 규칙: "무언가 바뀌었는지 확인"하기 위해 `setInterval`이나 고빈도 이벤트 리스너를 추가하려고 한다면, 더 효율적이고 더 적은 코드로 통보해 주는 옵저버가 아마 있습니다.

### 이론에서 아래 참조로

- **프로젝트 1: Todo 앱** 은 §A와 §B의 가장 단순한 형태입니다 — 단일 인메모리 배열, 렌더 함수, 그리고 영속성 경계로서의 `localStorage`.
- **프로젝트 2: 날씨 앱** 은 프로덕션의 §C입니다 — `fetch` 주위의 로딩/에러/빈/성공 사이클, 그리고 레슨 02의 폼 입력 처리.
- **프로젝트 3: 이미지 갤러리** 는 §D입니다 — 무한 스크롤을 위한 `IntersectionObserver`, 그리고 포커스 관리(레슨 11의 접근성 패턴)가 있는 라이트박스.

이 패턴들에 이름이 붙은 채로 프로젝트들을 읽으세요. 아래 모든 섹션은 그중 하나가 다른 도메인에 적용된 것입니다.
