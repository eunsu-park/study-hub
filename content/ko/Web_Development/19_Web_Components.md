...
요소의 라이프사이클은 네 콜백을 노출합니다.

```js
class MyCard extends HTMLElement {
  static observedAttributes = ['title', 'expanded'];

  constructor() { super(); /* 일회성 설정, DOM 접근 없음 */ }
  connectedCallback()    { /* 이제 문서 안, 속성 읽기/렌더 안전 */ }
  disconnectedCallback() { /* 제거됨; 리스너, 옵저버, 타이머 정리 */ }
  attributeChangedCallback(name, oldVal, newVal) {
    /* observedAttributes의 어떤 속성에 대해서든 발사 */
  }
  adoptedCallback() { /* 문서 사이에서 이동됨 */ }
}
```
