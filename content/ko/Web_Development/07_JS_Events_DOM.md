...
참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. DOM은 *살아 있는* 변경 가능한 트리이며(변형은 리플로우(reflow)를 일으킬 수 있습니다), 이벤트는 그 트리를 통해 세 단계(캡처 → 타겟 → 버블)로 흐르는데 — 이것이 위임(delegation)이 작동하는 이유이며, `innerHTML` vs. `textContent`는 스타일 선택이 아니라 보안 경계입니다.
...
DOM API에는 *수백* 개의 메서드가 있지만, 그 뒤의 설계는 작습니다. DOM은 화면에 있는 것을 비추는 **살아 있는 변경 가능한 트리** 이고, 변형은 브라우저의 렌더 파이프라인을 발동시키며, 균일한 **이벤트 흐름(event flow)** 이 모든 노드에 일일이 구독하지 않고도 상호작용을 효율적으로 처리하게 해 줍니다. 이 셋이 명확해지면, 수십 개의 `appendChild`/`addEventListener`/`classList` 메서드가 어휘 목록이 아니라 자료 구조 위의 연산처럼 보이기 시작합니다.

### A. DOM은 렌더 트리의 살아 있는 거울이다
...
### D. 위임, `innerHTML` 안전성, 그리고 `textContent`가 존재하는 이유
...
- **DOM 기초**(섹션 1)는 §A의 트리를 소개합니다 — 요소 노드, 텍스트 노드, 문서 노드.
- **요소 선택**(섹션 2)은 §B입니다 — `querySelector`, `querySelectorAll`, `getElementById`, 그리고 라이브 vs. 정적 컬렉션의 구별.
- **내용 / 속성 / 클래스 / 스타일 조작**(섹션 3–6)은 §A의 렌더 파이프라인에 들어가는 변형이며, `textContent` vs. `innerHTML`은 §D의 보안 경계입니다.
- **요소 생성 / 삭제**(섹션 7)는 §B의 문서 밖 패턴을 사용하여 레이아웃 스래싱을 피합니다.
- **이벤트 기초 / 종류 / 위임**(섹션 8–10)은 §C와 §D입니다 — 흐름, 카탈로그, 그리고 흐름을 활용하는 패턴.
- **폼 처리**(섹션 11)는 02 레슨의 제약 검증 API를 §C의 이벤트 시스템에 꽂습니다.

레슨의 나머지를, 모든 메서드가 트리 변형이거나 트리 쿼리이거나 이벤트 흐름에 대한 후크라는 점을 알고 읽으세요.
