...
참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 세 코어 웹 바이탈(LCP, INP, CLS)은 *실제* 사용자 디바이스에서 인지되는 성능의 세 다른 측면(속도, 반응성, 안정성)을 측정하며, 각 메트릭은 렌더링 파이프라인의 특정 병목에 매핑됩니다.

---

## 이론과 원리

"페이지를 빠르게 하라"는 행동하기에 너무 모호합니다. Core Web Vitals는 그 모호한 목표를 세 정량적 메트릭으로 번역하며, 각각이 사용자 경험의 다른 *순간* 을 측정합니다 — 메인 콘텐츠를 볼 때까지 얼마나 걸리는지(**LCP**), 페이지가 입력에 반응하기까지 얼마나 걸리는지(**INP**), 사용자가 읽는 동안 레이아웃이 얼마나 점프하는지(**CLS**). 각 메트릭은 렌더링 파이프라인의 특정 단계에 대응하며, 이는 각 메트릭이 유한하고 예측 가능한 수정 목록을 가진다는 뜻입니다. 숫자는 임의가 아닙니다 — Google은 실제 사용자 백분위 데이터에 기반한 "양호 / 개선 필요 / 불량" 임계값을 게시합니다.

### A. 세 메트릭과 그 임계값

모든 측정은 실제 사용자 디바이스와 연결의 **75 백분위(75th percentile)** 에서 취해집니다 — 여러분의 개발자 기계에서가 아니고, Wi-Fi에서가 아니고, 노트북에서가 아닙니다. 이는 중요합니다 — M1 Mac에서 2초 LCP가 3년 된 폰의 4G에서는 6초 LCP일 수 있습니다.

| 메트릭 | 측정하는 것 | 양호 | 불량 |
|--------|------------|------|------|
| **LCP** (Largest Contentful Paint) | 가장 큰 이미지/텍스트 블록이 렌더될 때까지의 시간 | ≤ 2.5초 | > 4.0초 |
| **INP** (Interaction to Next Paint) | 어떤 사용자 입력에서 다음 페인트까지의 최악 지연 | ≤ 200ms | > 500ms |
| **CLS** (Cumulative Layout Shift) | 페이지 라이프타임 동안 예기치 않은 레이아웃 시프트의 합 | ≤ 0.1 | > 0.25 |

두 보완 메트릭이 컨텍스트를 채우지만 셋을 대체하지는 않습니다.

- **TTFB** (Time to First Byte) — 서버 응답 시간. 높은 TTFB는 LCP가 얼마나 좋을 수 있는지를 제한합니다.
- **FCP** (First Contentful Paint) — *어떤* 콘텐츠든 화면에 나오는 첫 시간. LCP가 괜찮을 때도 빈 화면 대기를 감지하는 데 유용.

INP는 2024년 3월에 FID(First Input Delay)를 대체했습니다 — FID는 *첫* 입력만 측정해서 이후의 모든 잼(jank)을 놓쳤기 때문입니다. INP는 페이지 라이프 전반의 *최악* INP이며, 세 번째 열릴 때 2초 걸리는 드롭다운을 표면화합니다.

### B. LCP: 첫 의미 있는 렌더의 속도

LCP는 내비게이션 시작에서 가장 큰 폴드 위(above-the-fold) 요소가 렌더를 마칠 때까지의 시간을 측정합니다. 후보는 이미지, 비디오 포스터 프레임, CSS로 로드된 배경 이미지, 그리고 블록 레벨 텍스트 노드입니다. 더 큰 요소가 도착하면 브라우저가 LCP 후보를 갱신합니다 — 첫 사용자 상호작용에서 타이머가 멈춥니다.

빈도순 흔한 병목들:

1. **느린 서버 응답(TTFB).** 1.5초 TTFB는 2.5초 LCP를 원한다면 다른 모든 것에 1.0초만 남깁니다. 수정: CDN, 엣지 캐싱, 서버 사이드 캐싱, 더 빠른 오리진.
2. **렌더 차단 리소스.** `defer`/`async` 없는 `<head>`의 `<script>`는 파서를 막습니다. `<link rel="stylesheet">`는 렌더를 막습니다. 수정: 스크립트에 `defer`, 임계 CSS 인라인, 키 에셋 preload.
3. **LCP 이미지가 늦게 도착.** 브라우저는 발견한 것만 다운로드할 수 있습니다. CSS `background-image`로서의 이미지는 CSS가 파싱된 후에 발견되고, URL이 JavaScript로 생성된 이미지는 더 늦습니다. 수정: `srcset`이 있는 `<img>` 사용, LCP 이미지에 `<link rel="preload" as="image">` 추가, `fetchpriority="high"` 설정.
4. **느린 이미지 디코드/렌더.** 5MB 히어로 이미지는 다운로드와 디코드에 시간이 걸립니다. 수정: AVIF/WebP, 올바른 치수(레슨 05의 `srcset`/`sizes`), 공간을 예약하기 위한 `width`/`height` 속성.

멘탈 모델: LCP는 네트워크(다운로드), 파서(리소스 찾기), 렌더러(페인트) 사이의 경주입니다. 각 수정은 그 다리 중 하나를 공격합니다.

### C. INP: 실사용 하의 반응성

INP는 페이지 라이프 동안 모든 상호작용(클릭, 탭, 키 입력)에 대해 입력에서 다음 시각적 갱신까지의 시간을 측정하고 — 최악(또는 최악 근처)을 보고합니다. 느린 INP는 한 원천에서 옵니다 — 사용자가 클릭했을 때 메인 스레드가 다른 일을 하느라 바빴습니다.

메인 스레드는 JavaScript를 실행하고, HTML/CSS를 파싱하고, 레이아웃/페인트를 실행하고, 이벤트를 디스패치합니다. 그것을 ~50ms 이상 독점하는 모든 것이 **long task** 이며, long task 동안 페이지는 반응할 수 없습니다. long task의 원천:

1. **임계 경로(critical path) 위의 무거운 JavaScript.** 큰 `useEffect` 후크, 한 번에 많은 컴포넌트 마운트, 이벤트 핸들러의 비싼 계산.
2. **서버 렌더링된 콘텐츠의 하이드레이션(hydration).** Next.js 같은 프레임워크는 초기 로드 시 모든 인터랙티브 컴포넌트를 파싱하고 다시 바인딩합니다 — 저사양 폰에서는 몇 초가 걸릴 수 있습니다.
3. **서드파티 스크립트.** 분석, A/B 테스트, 채팅 위젯이 종종 동기 초기화를 실행합니다.
4. **큰 레이아웃/페인트 연산.** 500개 DOM 노드 삽입은 그 아래의 모든 것에 대해 동기 레이아웃을 발동합니다.

완화책은 세 축을 따라 작동합니다.

- **작업을 쪼개세요.** `await new Promise(r => setTimeout(r, 0))` (또는 지원되는 곳에서 `scheduler.yield()`)는 이벤트 루프로 양보하여 보류 중인 입력이 실행되게 합니다. 새 `scheduler.postTask` API는 우선순위 힌트(`'background' | 'user-blocking'`)를 줍니다.
- **작업을 메인 스레드 밖으로 옮기세요.** 순수 계산에 Web Workers, 렌더링에 offscreen canvas, 캐시 로직에 service workers.
- **일을 덜 하세요.** 코드 스플리팅(레슨 13/15), 지연 로딩(레슨 09), 비용을 정당화하지 못하는 서드파티 스크립트 제거.

전형적인 앱에서 가장 효과적인 단일 INP 수정은 라우트별 클라이언트 사이드 JavaScript 번들 분할입니다 — 그래야 내비게이션이 목적지와 무관한 코드의 비용을 지불하지 않습니다.

### D. CLS: 시각적 안정성

CLS는 페이지가 로드되는 동안과 사용 중에 발생하는 모든 "예기치 않은" 레이아웃 시프트를 합산합니다. "시프트"는 *위 또는 옆* 의 무언가가 자라거나 줄어 새 위치로 이동하는, 이미 화면에 있던 콘텐츠입니다. 가장 고통스러운 버전은 사용자가 버튼을 탭하려는데 그 위로 광고가 로드되어 그들의 손가락 아래로 버튼을 밀어내는 것입니다.

이벤트당 시프트 점수는 `impact_fraction × distance_fraction`(뷰포트의 얼마가 움직였는지에 그것이 얼마나 멀리 움직였는지를 곱한 것)입니다. CLS는 페이지 라이프타임을 가로지른 이것들의 합이며, 사용자 상호작용 이후 500ms 이내의 시프트는 제외됩니다(그것들은 "예상된" 것입니다).

원인의 카탈로그는 짧고 수정은 기계적입니다.

1. **치수 없는 이미지.** `width`/`height` 없는 `<img>`는 로드될 때까지 0 공간을 예약하다가, 그 아래의 모든 것을 밀어 내립니다. 수정: 항상 둘 다 설정(모던 브라우저는 그것들로부터 종횡비를 계산); 반응형 이미지에 대해 *종횡비* 가 중요합니다.
2. **늦게 로드되는 폰트(FOUT/FOIT).** 커스텀 폰트는 폴백과 다른 메트릭을 가집니다 — 그것이 교환되면 모든 텍스트 노드가 다시 측정합니다. 수정: `font-display: optional`(시간 안에 폰트가 준비되지 않으면 폴백 사용), `size-adjust`와 `ascent-override`(폴백 메트릭을 커스텀 메트릭에 가깝게 가져와 교환이 시각적으로 보이지 않게).
3. **기존 콘텐츠 위의 동적 콘텐츠.** 페이지를 밀어 내리는 쿠키 배너, 지연 로드 광고, "관심 있을 만한" 삽입. 수정: 콘텐츠가 도착하기 전에 `min-height`로 슬롯 예약.
4. **레이아웃에 영향을 주는 CSS 애니메이션.** `top`/`left`/`width`/`height` 트랜지션은 매 프레임 레이아웃 시프트를 일으킵니다. 수정: `transform`과 `opacity` 애니메이트(레슨 14 §B).

CLS를 페이지 템플릿 수준에서 한 번 수정하면 수정된 채로 유지되는 경향이 있습니다 — 매 릴리스 회귀할 수 있는 LCP와 INP와 다릅니다.

### 이론에서 아래 참조로

- **코어 웹 바이탈 이해**(섹션 1)는 §A입니다 — 세 메트릭, 임계값, 75 백분위가 의미하는 것.
- **코어 웹 바이탈 측정**(섹션 2)은 필드 데이터(Chrome User Experience Report, web-vitals 라이브러리)와 랩 데이터(Lighthouse, WebPageTest)를 다룹니다.
- **LCP**(섹션 3)는 §B입니다 — 네 병목과 그 수정.
- **INP**(섹션 4)는 §C입니다 — long task, 작업 쪼개기, 메인 스레드 밖으로.
- **CLS**(섹션 5)는 §D입니다 — 네 원인과 그 기계적 수정.
- **Performance Observer API** 는 web-vitals 라이브러리가 사용하는 §A의 측정 프리미티브입니다.
- **리소스 힌트**(`preload`, `prefetch`, `preconnect`, `dns-prefetch`)와 **이미지 최적화** 는 세 메트릭 모두를 개선하는 가로지르는 기법입니다.

레슨의 나머지를, 모든 최적화가 파이프라인의 한 특정 단계를 겨냥하며 — 메트릭이 어느 단계를 겨냥할지 알려준다는 점을 알고 읽으세요.
