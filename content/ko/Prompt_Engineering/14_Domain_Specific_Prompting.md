# 14. 도메인 특화 프롬프팅(Domain-Specific Prompting)

**이전**: [13. 적대적 프롬프팅](./13_Adversarial_Prompting.md) | **다음**: [15. 프로덕션에서의 프롬프트 관리](./15_Prompt_Management_in_Production.md)

## 학습 목표

- 비정형 텍스트에서 구조화된 데이터를 안정적으로 추출하는 추출 프롬프트를 설계하기
- 제어 가능한 길이, 스타일, 충실도를 갖춘 요약 전략을 적용하기
- 법률, 의료, 금융, 교육 컨텍스트에 맞는 도메인 적응 프롬프트를 구축하기
- 용어 제어와 스타일 보존이 가능한 번역 프롬프트를 구현하기
- 학습자를 효과적으로 안내하는 소크라테스식 및 비계(Scaffolded) 교육 프롬프트를 만들기

---

범용 프롬프팅 기법은 80%까지 도달하게 해주지만, 도메인 특화 애플리케이션은 전문화된 패턴을 요구합니다. 의료 요약기는 창작 글쓰기 어시스턴트와 다른 안전장치가 필요합니다. 법률 문서 분석기는 일반 챗봇이 필요로 하지 않는 정밀도를 요구합니다. 이 레슨에서는 특정 전문 도메인에 맞춤화된 프롬프팅 패턴을 다룹니다 — 데이터 추출, 텍스트 분석, 요약, 번역, 교육, 창작 글쓰기. 각 도메인은 일반 프롬프트로는 해결할 수 없는 정확성, 톤, 용어, 안전성에 대한 고유한 과제를 도입합니다.

## 목차
1. [데이터 추출 프롬프팅](#1-데이터-추출-프롬프팅)
2. [텍스트 분석과 분류](#2-텍스트-분석과-분류)
3. [요약 전략](#3-요약-전략)
4. [번역 프롬프팅](#4-번역-프롬프팅)
5. [교육 프롬프팅](#5-교육-프롬프팅)
6. [법률 도메인 프롬프팅](#6-법률-도메인-프롬프팅)
7. [의료 및 금융 도메인 프롬프팅](#7-의료-및-금융-도메인-프롬프팅)
8. [창작 글쓰기 프롬프트](#8-창작-글쓰기-프롬프트)
9. [연구 지원 프롬프트](#9-연구-지원-프롬프트)

---

## 1. 데이터 추출 프롬프팅(Data Extraction Prompting)

데이터 추출(Data Extraction)은 비정형 텍스트를 구조화된 기계 판독 가능 형식으로 변환합니다. 핵심 과제는 신뢰성입니다 — 추출 프롬프트는 지저분하고 일관성 없는 실제 텍스트를 처리해야 합니다.
...
### 1.2 다중 엔티티 추출(Multi-Entity Extraction)

문서에서 여러 엔티티를 추출할 때는 스키마 기반 접근 방식을 사용합니다:
...
### 1.3 검증을 포함한 추출(Extraction with Validation)
...
## 2. 텍스트 분석과 분류(Text Analysis and Classification)

텍스트 분석 프롬프트는 정의된 차원을 따라 텍스트를 분류, 범주화 또는 점수를 매깁니다.

### 2.1 다중 레이블 분류(Multi-Label Classification)
...
### 2.2 측면 감성 분석(Sentiment Analysis with Aspect Extraction)
...
### 2.3 챗봇 의도 감지(Intent Detection for Chatbots)
...
## 3. 요약 전략(Summarization Strategies)

요약(Summarization)은 가장 일반적인 LLM 작업 중 하나이지만, 일관되고 고품질의 요약을 얻으려면 세심한 프롬프트 엔지니어링이 필요합니다.

### 3.1 추출적 vs. 추상적 요약 단서
...
### 3.2 길이 제어 전략(Length Control Strategies)
...
### 3.3 충실도 보존 요약(Faithfulness-Preserving Summarization)
...
## 4. 번역 프롬프팅(Translation Prompting)

LLM을 사용한 번역은 단어 대 단어 변환을 넘어섭니다. 효과적인 번역 프롬프트는 용어, 어조, 문화적 적응, 도메인별 어휘를 관리합니다.

### 4.1 용어 제어 번역(Translation with Terminology Control)
...
### 4.2 스타일 보존 번역(Style-Preserving Translation)
...
### 4.3 역번역을 통한 품질 보증(Back-Translation for Quality Assurance)
...
## 5. 교육 프롬프팅(Educational Prompting)

교육 프롬프트는 비계(Scaffolding), 소크라테스식 방법, 점진적 공개(Progressive Disclosure) 같은 교수법적 원칙을 사용하여 학습자를 교재를 통해 안내합니다.

### 5.1 소크라테스식 튜터링(Socratic Tutoring)
...
### 5.2 비계식 학습(Scaffolded Learning)
...
### 5.3 적응적 난이도(Adaptive Difficulty)
...
## 6. 법률 도메인 프롬프팅(Legal Domain Prompting)

법률 도메인 프롬프팅은 극도의 정밀도, 인용 인식, 신중한 면책 조항이 필요합니다.
...
## 7. 의료 및 금융 도메인 프롬프팅(Medical and Financial Domain Prompting)

의료 및 금융 도메인은 극도의 주의, 면책 조항, AI가 해야 할 것과 하지 말아야 할 것에 대한 명확한 경계가 필요하다는 점을 공유합니다.
...
## 8. 창작 글쓰기 프롬프트(Creative Writing Prompts)

창작 글쓰기 프롬프트는 구조와 창작의 자유 사이의 균형을 맞춥니다 — 너무 많은 제약은 창의성을 억압하고, 너무 적은 제약은 초점 없는 출력을 만듭니다.
...
## 9. 연구 지원 프롬프트(Research Assistance Prompts)

연구 지원 프롬프트는 문헌 검토, 가설 형성, 방법론 설계, 비판적 분석을 돕습니다.
...
### 9.2 가설 생성기(Hypothesis Generator)
...
### 9.3 비판적 분석 프롬프트(Critical Analysis Prompt)
...
### 연습문제 1: 다중 형식 데이터 추출기

여러 문서 유형(이메일, 청구서, 이력서)을 단일 구성 가능 프롬프트 패턴으로 처리할 수 있는 데이터 추출 시스템을 구축하세요.

**요구사항:**
- 최소 3가지 문서 유형에 대한 추출 스키마 정의
- 추출 전 문서 유형 자동 감지
- 스키마에 대한 추출 데이터 검증
- 부분적이거나 잘못된 형식의 문서를 우아하게 처리
...
여러 출력 형식(문단, 글머리 기호, 헤드라인, 트윗), 길이 목표, 대상 수준(전문가, 일반, 어린이)을 지원하는 요약 시스템을 구축하세요.

**요구사항:**
- 최소 4가지 출력 형식 지원
- 단어 수 목표를 포함한 길이 제어 구현
- 3가지 대상 수준에 맞게 어휘와 복잡도 조정
- 요약과 원문 비교를 포함한 충실도 검사 포함
...
### 연습문제 3: 오해 감지를 포함한 소크라테스식 튜터

학생 응답에서 흔한 오해를 감지하고 목표 질문을 통해 해결하는 소크라테스식 튜터링 시스템을 구축하세요.

**요구사항:**
- 직접적인 답을 절대 주지 않기 — 질문만 하기
- 학생이 이해하고 있는 것의 지식 모델 유지
- 오해를 감지하고 구체적으로 해결
- 대화를 통해 진행 상황 추적
- 구성 가능한 주제와 난이도 수준 지원
...
### 연습문제 4: 도메인 안전 의료 FAQ 생성기

안전 가드레일을 엄격히 유지하면서 환자 친화적인 의료 상태 설명을 만드는 의료 FAQ 생성기를 구축하세요.

**요구사항:**
- 구성 가능한 읽기 수준에서 설명 생성
- 항상 적절한 면책 조항 포함
- 진단 또는 치료 질문을 감지하고 거부
- 각 주제에 대해 "의사에게 물어볼 질문" 제공
- 반환 전 안전 준수를 위한 출력 검증
...
텍스트를 번역하고, 역번역을 통해 품질을 검증하고, 용어집 대비 용어 일관성을 확인하고, 품질 보고서를 생성하는 번역 파이프라인을 구축하세요.

**요구사항:**
- 용어집 제어 번역 지원
- 역번역 품질 검증 구현
- 용어 일관성 확인 (모든 용어집 용어가 지정된 번역을 사용해야 함)
- 품질 점수와 상세 보고서 생성
- 최소 2개의 언어 쌍 처리
