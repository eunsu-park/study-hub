# 13. 적대적 프롬프팅(Adversarial Prompting)

**이전**: [12. 평가와 지표](./12_Evaluation_and_Metrics.md) | **다음**: [14. 도메인 특화 프롬프팅](./14_Domain_Specific_Prompting.md)

## 학습 목표

- LLM 기반 애플리케이션에 대한 적대적 공격 벡터를 식별하고 분류하기
- 직접 프롬프트 인젝션(Direct Prompt Injection), 간접 프롬프트 인젝션(Indirect Prompt Injection), 탈옥(Jailbreaking)을 구분하기
- 일반적인 공격 기법에 저항하는 방어적 프롬프트 설계 패턴을 구현하기
- 프로덕션 시스템을 위한 입력 정제(Input Sanitization) 및 출력 검증(Output Validation) 파이프라인을 구축하기
- 자신의 프롬프트에서 취약점을 발견하기 위한 구조화된 레드팀(Red-Team) 연습을 수행하기

---

대규모 언어 모델은 강력하지만 본질적으로 신뢰적입니다 — 개발자 지시사항과 사용자 입력 사이의 본질적인 구분 없이 수신하는 모든 텍스트를 처리합니다. 이는 "코드"(프롬프트)와 "데이터"(사용자 입력)가 같은 채널을 공유하는 독특한 보안 환경을 만듭니다. 적대적 프롬프팅(Adversarial Prompting)은 이러한 혼합을 악용합니다. 이러한 공격을 이해하는 것은 프로덕션에서 LLM을 배포하는 모든 사람에게 선택이 아닙니다; 웹 개발자에게 SQL 인젝션을 이해하는 것이 필수적인 것처럼 필수적입니다. 이 레슨에서는 위협 모델, 공격 분류 체계, 실용적인 방어를 제공합니다.

## 목차
1. [LLM 애플리케이션의 위협 모델](#1-llm-애플리케이션의-위협-모델)
2. [프롬프트 인젝션: 직접 및 간접](#2-프롬프트-인젝션-직접-및-간접)
3. [탈옥 기법과 역사](#3-탈옥-기법과-역사)
4. [방어적 프롬프트 설계](#4-방어적-프롬프트-설계)
5. [입력 정제와 필터링](#5-입력-정제와-필터링)
6. [출력 검증](#6-출력-검증)
7. [샌드위치 방어](#7-샌드위치-방어)
8. [방어로서의 지시사항 계층 구조](#8-방어로서의-지시사항-계층-구조)
9. [자신의 프롬프트 레드팀하기](#9-자신의-프롬프트-레드팀하기)
...
위협 모델은 무엇을 보호하는지, 누가 공격할 수 있는지, 어떻게 공격하는지를 식별합니다. LLM 애플리케이션의 위협 모델은 전통적인 소프트웨어와 크게 다릅니다.

### 1.1 위협 대상 자산

LLM 애플리케이션에서 여러 자산이 손상될 수 있습니다:
...
다양한 공격자는 서로 다른 능력과 목표를 가집니다:

| 공격자 | 목표 | 기술 수준 | 접근 방식 |
|--------|------|----------|----------|
| 호기심 많은 사용자 | 시스템 프롬프트 추출 | 낮음 | 직접 채팅 |
| 악의적 사용자 | 안전 필터 우회 | 중간 | 직접 채팅 |
| 데이터 오염자 | 인젝션된 콘텐츠로 출력 조작 | 높음 | 간접 (문서, 웹 페이지) |
| 경쟁자 | 비즈니스 로직 리버스 엔지니어링 | 높음 | API 접근 |
| 레드팀 | 모든 취약점 발견 | 전문가 | 전체 접근 |
...
### 1.4 LLM에 적응된 STRIDE 모델

고전적인 STRIDE 위협 모델은 다음과 같이 적응할 수 있습니다:

| STRIDE 범주 | LLM 등가물 |
|------------|-----------|
| **S**poofing (위장) | 페르소나 하이재킹 (모델이 다른 사람을 사칭하게 만들기) |
| **T**ampering (변조) | 프롬프트 인젝션 (유효 지시사항 수정) |
| **R**epudiation (부인) | 추적할 수 없는 유해 콘텐츠를 모델이 생성 |
| **I**nformation Disclosure (정보 노출) | 시스템 프롬프트 유출, 훈련 데이터 추출 |
| **D**enial of Service (서비스 거부) | 컨텍스트 윈도우 고갈, 무한 루프 |
| **E**levation of Privilege (권한 상승) | 안전 필터 우회, 제한된 도구 접근 |

---

## 2. 프롬프트 인젝션: 직접 및 간접

프롬프트 인젝션(Prompt Injection)은 LLM 애플리케이션에 대한 가장 근본적인 공격입니다. LLM이 개발자 지시사항과 사용자 제공 텍스트를 본질적으로 구분할 수 없다는 사실을 악용합니다.

### 2.1 직접 프롬프트 인젝션(Direct Prompt Injection)

직접 프롬프트 인젝션은 사용자가 시스템 프롬프트를 무시하거나 전복시키는 입력을 만들 때 발생합니다. 사용자가 모델과 직접 상호작용하며 동작을 변경하려고 시도합니다.
...
간접 프롬프트 인젝션은 공격자가 모델과 직접 상호작용하지 않기 때문에 훨씬 더 위험합니다. 대신 모델이 처리할 콘텐츠 — 웹 페이지, 문서, 이메일, 데이터베이스 항목 — 에 악의적인 지시사항을 배치합니다.
...
## 3. 탈옥 기법과 역사(Jailbreaking Techniques and History)

탈옥(Jailbreaking)은 모델이 거부하도록 훈련받거나 지시받은 출력 — 일반적으로 유해하거나, 비윤리적이거나, 정책을 위반하는 콘텐츠 — 을 생성하게 만드는 기법을 말합니다. 프롬프트 인젝션(애플리케이션 수준의 지시사항을 대상으로 함)과 달리, 탈옥은 모델 수준의 안전 훈련을 대상으로 합니다.
...
### 3.3 매니샷 탈옥(Many-Shot Jailbreaking)

매니샷 탈옥은 2024년 Anthropic이 문서화한 것으로, 긴 컨텍스트 윈도우를 악용합니다. 모델이 유해한 질문에 답하는 것처럼 보이는 많은 예시를 제공함으로써, 인컨텍스트 학습 효과가 안전 훈련을 압도합니다:
...
## 4. 방어적 프롬프트 설계(Defensive Prompt Design)

방어적 프롬프트 설계는 외부 필터에만 의존하지 않고 보안을 프롬프트 자체에 직접 구축합니다.

### 4.1 최소 권한 원칙(Principle of Least Privilege)

모델에게 필요한 기능과 정보만 부여합니다:
...
### 4.2 명시적 경계 선언(Explicit Boundary Statements)

모델이 절대 해서는 안 되는 것을 명확하고 구체적인 언어로 정의합니다:
...
### 4.3 출력 형식 제약(Output Format Constraints)

출력 형식을 제한하면 공격자가 할 수 있는 피해를 제한할 수 있습니다:
...
### 4.4 카나리 토큰(Canary Tokens)

프롬프트에 숨겨진 마커를 삽입하여 출력에 절대 나타나지 않아야 합니다:
...
## 5. 입력 정제와 필터링(Input Sanitization and Filtering)

입력 정제는 사용자 입력이 모델에 도달하기 전에 잠재적으로 악의적인 콘텐츠를 제거하거나 무력화하여 변환합니다.

### 5.1 패턴 기반 필터링(Pattern-Based Filtering)
...
### 5.2 시맨틱 유사도 탐지(Semantic Similarity Detection)

패턴 매칭을 넘어, 임베딩을 사용하여 알려진 공격 프롬프트와 의미적으로 유사한 입력을 탐지합니다:
...
### 5.3 입력 길이 및 구조 검증(Input Length and Structure Validation)
...
좋은 입력 정제와 방어적 프롬프트가 있더라도, 출력이 사용자에게 도달하기 전에 반드시 검증해야 합니다.

### 6.1 출력 안전성 분류기(Output Safety Classifier)
...
### 6.2 LLM-심판 출력 검증(LLM-as-Judge Output Validation)

두 번째 LLM 호출을 사용하여 첫 번째 호출의 안전성과 적절성을 평가합니다:
...
## 7. 샌드위치 방어(The Sandwich Defense)

샌드위치 방어는 사용자 입력을 두 개의 시스템 지시사항 레이어 사이에 배치하는 프롬프트 구성 기법입니다. 아이디어: 사용자 입력이 지시사항을 무시하려고 해도, 모델은 이후에 강화 지시사항을 만나게 됩니다.
...
### 7.3 샌드위치 방어가 실패하는 경우

샌드위치 방어는 완벽하지 않습니다. 다음과 같은 경우 실패할 수 있습니다:
...
## 8. 방어로서의 지시사항 계층 구조(Instruction Hierarchy as Defense)

현대 LLM(Claude와 GPT-4 포함)은 시스템 수준 지시사항이 사용자 수준 메시지보다 우선하는 지시사항 계층 구조를 구현합니다. 이 계층 구조를 이해하고 활용하는 것이 핵심 방어 전략입니다.
...
## 9. 자신의 프롬프트 레드팀하기(Red-Teaming Your Own Prompts)

레드팀(Red-Teaming)은 실제 공격자가 하기 전에 자신의 시스템을 체계적으로 공격하여 취약점을 발견하는 관행입니다.
...
### 9.3 심각도 분류(Severity Classification)
...
### 9.4 CI/CD에서의 지속적 레드팀(Continuous Red-Teaming in CI/CD)
...
## 10. 책임 있는 공개(Responsible Disclosure)

LLM 시스템에서 취약점을 발견했을 때, 책임 있는 공개 관행은 모든 사람을 보호합니다.
...
### 10.3 LLM 애플리케이션의 버그 바운티 프로그램

주요 AI 연구소와 LLM을 사용하는 기업들은 점점 더 버그 바운티 프로그램을 제공하고 있습니다:

| 조직 | 프로그램 | 범위 |
|------|---------|------|
| Anthropic | Responsible Disclosure Policy | Claude 모델 동작, API 보안 |
| OpenAI | Bug Bounty (via Bugcrowd) | API 보안, 모델 안전 우회 |
| Google DeepMind | Vulnerability Reward Program | Gemini, 모델 안전 이슈 |
| Meta | Bug Bounty | Llama 모델 이슈, 플랫폼 통합 |
| HackerOne programs | 다양한 기업 | 애플리케이션 수준 LLM 취약점 |
...
### 연습문제 1: 프롬프트 인젝션 탐지기 구축

패턴 매칭과 휴리스틱 분석을 사용하여 직접 및 간접 프롬프트 인젝션 시도를 모두 탐지하는 함수를 구축하세요.

**요구사항:**
- 최소 5가지 다른 인젝션 패턴 탐지
- 위험 점수(0.0 ~ 1.0)와 탐지된 패턴 목록 반환
- 영어와 일반적인 인코딩 기반 공격 모두 처리
- 최소 10개의 양성 입력과 10개의 악성 입력으로 구성된 테스트 스위트 포함
...
XML 구분자를 사용하여 데이터와 지시사항을 명확히 분리하면서, 사전 지시사항과 사후 강화로 사용자 입력을 감싸는 강건한 샌드위치 방어 구현을 만드세요.

**요구사항:**
- 시스템 지시사항, 사용자 입력, 선택적 컨텍스트 문서를 수용
- 명확한 분리를 위해 XML 구분자 사용
- 모델에게 작업을 상기시키는 사후 강화 포함
- 최소 3가지 다른 인젝션 시도로 테스트
...
### 연습문제 3: 고객 지원 봇 레드팀하기

다음 고객 지원 봇의 시스템 프롬프트가 주어졌을 때, 최소 8가지 다른 공격 벡터를 테스트하는 레드팀 공격 스위트를 작성하세요. 각 공격에 대해 어떤 취약점을 대상으로 하는지와 성공적인 공격이 어떤 모습일지 설명하세요.
...
모델 응답이 사용자에게 도달하기 전에 검사하는 출력 방화벽을 만드세요. 방화벽은 PII 유출, 시스템 프롬프트 유출, 유해 콘텐츠, 형식 위반을 검사해야 합니다.

**요구사항:**
- 최소 5가지 카테고리의 안전하지 않은 출력 검사
- 애플리케이션별 구성 가능한 정책 지원
- 통과/경고/차단 상태를 포함한 구조화된 결과 반환
- 경고에 대한 수정(민감한 부분을 제거하여 콘텐츠 허용) 제공
...
### 연습문제 5: 심층 방어 아키텍처(Defense-in-Depth Architecture)

입력 검증, 방어적 프롬프트 설계, 샌드위치 방어, 출력 검증, 로깅을 포함하는 여러 보호 레이어를 결합한 완전한 심층 방어 시스템을 설계하고 구현하세요. 모든 컴포넌트를 단일 `SecureLLMPipeline` 클래스에 연결하세요.

**요구사항:**
- 최소 4개의 방어 레이어
- 애플리케이션별 구성 가능한 보안 정책
- 모든 보안 이벤트의 구조화된 로깅
- 전체 파이프라인을 실행하는 `process()` 메서드
- 현실적인 고객 지원 시나리오로 시연
