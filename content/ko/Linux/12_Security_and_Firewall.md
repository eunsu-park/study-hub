...
**이전**: [시스템 모니터링](./11_System_Monitoring.md) | **다음**: [Systemd 고급](./13_Systemd_Advanced.md)

## 학습 목표(Learning Objectives)

이 레슨을 완료하면 다음을 할 수 있습니다:

1. 최소 권한 원칙 적용 및 리눅스 보안 계층 설명
2. 루트 로그인 비활성화, 키 인증 사용, 접근 제한을 통한 SSH 설정 강화
3. UFW(Ubuntu)를 사용하여 네트워크 트래픽 허용, 차단, 속도 제한 설정
4. 존, 서비스, 리치 룰을 사용하여 firewalld(CentOS/RHEL) 설정
5. SELinux(CentOS)와 AppArmor(Ubuntu) 강제 접근 제어(MAC)의 기본 이해
6. fail2ban 설치 및 설정으로 무차별 대입 공격 자동 차단
7. Ubuntu와 CentOS 모두에서 무인 보안 업데이트 설정
8. 사용자, 서비스, 열린 포트, 파일 권한을 확인하는 보안 감사 수행

---

보안은 나중에 추가하는 기능이 아니라 처음부터 적용하는 사고방식입니다. 인터넷에 노출된 리눅스 서버는 온라인 상태가 된 지 몇 분 안에 자동화된 스캐너의 탐색 대상이 됩니다. 이 레슨은 모든 서버에 반드시 갖춰야 할 핵심 강화 단계를 다룹니다: SSH 잠금, 방화벽 설정, 침입 방지 활성화, 정기적인 보안 감사.
