...
**이전**: [고가용성 클러스터](./25_High_Availability_Cluster.md)

## 학습 목표

이 레슨을 마치면 다음을 할 수 있습니다:

1. Linux 시스템 문제를 진단하는 체계적인 5단계 방법론을 적용한다
2. 복구 모드(Recovery Mode)를 사용하여 GRUB, 파일시스템, systemd 부팅 장애로부터 복구한다
3. 인터페이스에서 DNS, 방화벽까지 단계별로 네트워크 연결 문제를 진단한다
4. 디스크 공간 고갈, 파일시스템 손상, I/O 병목을 식별하고 해결한다
5. OOM Killer 이벤트를 감지하고 실행 중인 프로세스의 메모리 누수(Memory Leak)를 진단한다
6. 좀비(Zombie) 프로세스, 높은 CPU 사용률, 파일 디스크립터(File Descriptor) 고갈을 포함한 프로세스 문제를 디버깅한다
7. 성능 기준선(Performance Baseline)을 구축하고 프로덕션 시스템의 병목 분석 체크리스트를 활용한다
...
프로덕션 서버가 새벽 3시에 다운되면 시행착오를 겪을 시간이 없습니다. 효과적인 트러블슈팅은 구조화된 방법론을 필요로 합니다: 증거를 수집하고, 가설을 세우고, 검증하고, 해결 과정을 문서화하는 것입니다. 이 레슨은 Linux 시리즈 전체의 진단 기법을 압박 상황에서도 부팅 장애, 네트워크 단절, 디스크 문제, 메모리 고갈, 성능 저하를 해결하는 데 활용할 수 있는 실용적인 참고 자료로 정리합니다.
...
[Linux 학습 가이드](./00_Overview.md)로 돌아가서 전체 학습 로드맵을 확인하세요.
