...
**이전**: [패키지 관리](./08_Package_Management.md) | **다음**: [네트워크 기초](./10_Network_Basics.md)

## 학습 목표(Learning Objectives)

이 레슨을 완료하면 다음을 할 수 있습니다:

1. 올바른 shebang 라인과 권한을 사용하여 셸 스크립트 작성 및 실행
2. 특수 변수($?, $@, $$) 및 환경 변수를 포함한 변수 선언과 사용
3. read로 사용자 입력을 받고 커맨드라인 인수 처리
4. if/elif/else와 case 문을 사용하여 조건 논리 구현
5. break와 continue 제어를 포함한 루프(for, while, until) 작성
6. 매개변수, 반환값, 지역 변수를 가진 함수 정의 및 호출
7. 백업, 상태 확인, 로그 분석을 위한 실용적인 자동화 스크립트 작성
8. set -x, set -e, set -u, set -o pipefail을 사용하여 셸 스크립트 디버그

---

셸 스크립팅은 리눅스의 자동화 슈퍼파워입니다. 매일 같은 명령어 시퀀스를 입력하는 대신, 한 번 스크립트에 작성하고 컴퓨터가 반복 작업을 처리하도록 합니다. 간단한 백업 스크립트부터 완전한 배포 파이프라인까지, 셸 스크립트는 리눅스 시스템 관리를 하나로 묶는 접착제입니다. Python과 Go의 시대에도 셸 스크립팅은 시스템 수준 작업을 자동화하는 가장 빠른 방법으로 남아 있습니다.
...
[네트워크 기초](./10_Network_Basics.md)에서 네트워크 관리를 배워봅시다!
