...
**이전**: [고급 타입](./05_Advanced_Types.md) | **다음**: [성능 프로파일링](./07_Performance_Profiling.md)

## 학습 목표

이 레슨을 완료하면 다음을 할 수 있다:

1. `reflect` 패키지를 사용하여 런타임에 타입과 값을 검사한다
2. 구조체 태그를 프로그래밍 방식으로 읽고 파싱한다
3. 리플렉션을 사용하여 동적 JSON/XML 마샬러를 구축한다
4. `go generate`와 템플릿으로 Go 코드를 생성한다
5. 리플렉션의 성능 영향을 이해한다

---

리플렉션은 런타임에 타입과 값을 검사하고 조작할 수 있게 해준다. 코드 생성은 빌드 시점에 타입 안전한 코드를 만들어 낸다. 둘 다 고급 도구이다 — 리플렉션은 최소한으로 사용하고, 가능하면 코드 생성을 선호한다.

## 목차
1. [reflect 패키지 기초](#1-reflect-패키지-기초)
2. [구조체 검사](#2-구조체-검사)
3. [구조체 태그](#3-구조체-태그)
4. [동적 값](#4-동적-값)
5. [코드 생성](#5-코드-생성)
...
1. **`reflect.TypeOf`과 `reflect.ValueOf`** — 리플렉션의 진입점이다.
2. **Kind vs Type** — Kind는 기반 타입 카테고리이고, Type은 특정 명명된 타입이다.
3. **구조체 태그는 메타데이터이다** — `field.Tag.Get("key")`로 접근할 수 있다.
4. **값 설정에는 포인터가 필요하다** — `reflect.ValueOf(&x).Elem().SetInt(42)`.
5. **리플렉션보다 코드 생성을 선호한다** — 타입 안전하고 빠른 코드를 위해 `go generate`를 사용한다.
6. **리플렉션은 느리다** — 10~100배의 오버헤드가 있다. 결과를 캐시하고 핫 패스를 피한다.
7. **`//go:generate` 지시어** — 빌드 프로세스의 일부로 코드 생성기를 실행한다.
...
### 연습 1: 구조체 비교
동일한 타입의 두 구조체를 비교하고 이전 값과 새 값이 포함된 변경된 필드 목록을 반환하는 함수를 작성한다.

### 연습 2: 설정 로더
구조체 태그(`env`, `default`, `required`)를 사용하여 환경 변수에서 설정 구조체를 채우는 설정 로더를 만든다.

### 연습 3: 코드 생성기
Go 인터페이스 정의를 읽고 모의 구현을 생성하는 코드 생성기를 만든다.

### 연습 4: ORM-Lite
테이블/컬럼 이름에 구조체 태그를 사용하여 구조체를 SQL 쿼리에 매핑하는 간단한 ORM을 만든다.
