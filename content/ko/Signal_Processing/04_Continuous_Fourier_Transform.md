...
**학습 목표**:
- 비주기 신호에 대한 푸리에 급수의 극한으로서 푸리에 변환 유도
- 주요 CTFT 성질 (선형성, 이동, 스케일링, 쌍대성, 합성곱) 증명
- 표준 신호의 순방향 및 역 푸리에 변환 계산
- 합성곱 정리를 적용하여 LTI 시스템 분석 단순화
- 주파수 영역 표현을 물리적으로 해석
- 이상적인 주파수 선택 필터 (저역통과, 고역통과, 대역통과) 설명
- 대역폭과 시간-대역폭 곱 (불확정성 원리) 이해
- Python (FFT)을 이용한 연속 신호 푸리에 분석
