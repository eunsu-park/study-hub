...
1. 임펄스 열(Impulse Train) 모델과 표본화된 신호의 스펙트럼을 사용하여 이상적인 표본화의 수학적 체계를 설명할 수 있습니다.
2. 나이퀴스트-섀넌(Nyquist-Shannon) 표본화 정리를 진술하고 증명하며, 나이퀴스트 레이트(Nyquist Rate)를 정의할 수 있습니다.
3. 에일리어싱(Aliasing)을 식별하고 주파수 영역에서의 원인을 설명하며, 안티에일리어싱 필터(Anti-aliasing Filter)를 구현하여 방지할 수 있습니다.
4. sinc 보간법(Sinc Interpolation)과 영차 홀드(Zero-Order Hold)/일차 홀드(First-Order Hold) 방법을 사용한 신호 복원을 구현할 수 있습니다.
5. 실용적인 ADC/DAC 시스템을 분석하고 오버샘플링(Oversampling)과 잡음 성형(Noise Shaping)의 이점을 평가할 수 있습니다.
...
**선수 과목:** [04. 연속 푸리에 변환](04_Continuous_Fourier_Transform.md)
