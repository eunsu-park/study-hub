...
프로덕션 챗봇은 이 코스의 거의 모든 개념의 종합입니다 — LLM이 언어 모델링(레슨 3-5), 토크나이저가 텍스트를 ID로 변환(레슨 1), 프롬프트 엔지니어링이 대화를 형성(레슨 9), RAG가 도메인 지식 주입(레슨 10-12), 도구 호출이 행동을 가능하게(레슨 14, 23), 대화 메모리가 다중 턴 일관성을 엮음(레슨 13), 서빙 인프라가 규모로 전달(레슨 18, 24). 이 레슨은 새 이론보다 **통합**에 관한 것 — 어떤 구성 요소가 잘 구성되는가, 규모에서 어떤 트레이드오프가 중요한가, 프로덕션에서만 나타나는 실패 모드.

이 섹션은 다음을 다룹니다:

- **(A) 상태 있는 함수로서의 챗봇** — 무엇이 대화를 단일 쿼리와 다르게 만드는가, 턴 기반 에이전트의 형식적 모델.
- **(B) 대화 메모리 아키텍처** — 버퍼, 윈도우, 요약, 벡터 — 각각 적합한 시점.
- **(C) RAG 통합** — 언제 검색하는가, 검색 결정 문제, 인용 처리.
- **(D) 스트리밍과 지연 예산** — 첫 토큰 지연 vs 처리량, 스트리밍이 모델 품질보다 UX를 더 바꾸는 이유.
- **(E) 안전성, 모더레이션, 거부** — 입력-LLM-출력 안전 파이프라인.
- **(F) 프로덕션 관심사** — 캐싱, 속도 제한, 폴백, 관측성, 프로덕션 평가.

### A. 상태 있는 함수로서의 챗봇

단일 LLM 호출은 `f(prompt) → response`. 챗봇은 *상태 있는* 함수 — 각 턴 `t`가 사용자 입력 `u_t`를 받아 어시스턴트 출력 `a_t`를 생성하며, 상태 `s_t`가 시스템이 지금까지 기억하기로 한 모든 것을 운반합니다:

```
(a_t, s_t) = chatbot(u_t, s_{t-1})
```

상태는 보통 다음을 포함 — 전체 메시지 히스토리(또는 압축된 형태), 검색된 문서 저장소, 사용자 선호/프로필, 세션 메타데이터(인증, 테넌트), 도구 호출 결과. 시스템의 일은 `(s_{t-1}, u_t)`로부터 턴 `t`의 프롬프트를 LLM이 유용한 `a_t`를 만들도록 구성하고 그 후 `s_t`를 갱신하는 것입니다.

이 상태 있는 함수 관점은 레슨의 모든 다른 것 — 메모리, RAG, 도구 — 이 *`s`에 무엇이 들어가는가*와 *`s`가 어떻게 프롬프트로 매핑되는가*를 형성하는 방법임을 명확히 합니다.

### B. 대화 메모리 아키텍처

순진한 접근 — 모든 프롬프트에 전체 메시지 히스토리 포함 — 은 짧은 대화에서 작동하지만 비용이 폭발하고 결국 컨텍스트 윈도우를 초과합니다. 네 전략(레슨 25에서 깊이 다룸):

**B.1 버퍼.** 모든 턴 포함. 가장 단순, 정확, 그러나 호출당 `O(n)` 토큰(`n`은 대화 길이).

**B.2 윈도우.** 마지막 `k` 턴만 유지. 호출당 `O(k)`이지만 초기 맥락 잃음.

**B.3 요약.** 주기적으로 오래된 턴을 LLM 생성 요약으로 대체. 장기적으로 `O(1)`이지만 요약 지연 추가, 정보 손실 위험.

**B.4 벡터 / 검색 증강.** 모든 과거 턴을 벡터 DB에 저장, 쿼리당 관련된 것 검색. 무한 스케일이지만 검색 지연 추가, 최근 무관한 맥락 놓칠 수 있음.

실무에서는 — 하이브리드 — 최근 K 턴 그대로(윈도우) + 요약된 오래된 턴(요약) + 명시적 사실 회상에 검색(벡터). 정확한 혼합은 제품별.

### C. RAG 통합

챗봇은 턴마다 RAG가 필요할 수도 안 할 수도 있습니다. 인사 "hi how are you"는 벡터 검색을 트리거하지 말아야 하고, 회사 정책에 대한 질문은 그래야 합니다. 두 접근:

**C.1 항상 검색.** 내용에 관계없이 모든 턴에 RAG 실행. 단순, 때로 낭비, 그러나 예측 가능한 비용.

**C.2 조건부 검색.** LLM(또는 저렴한 분류기)을 사용해 검색이 필요한지 결정. 비용 절약하지만 결정을 위한 턴당 LLM 호출 1개 추가. 변형 — function calling(모델에 `search` 도구를 주고 결정하게), 또는 명시적 이진 분류.

**C.3 Multi-hop 검색.** 복잡한 쿼리에 대해 레슨 12 패턴 따름 — 초기 검색 → LLM 추론 → 어쩌면 다시 검색.

인용 처리 — 프로덕션 챗봇은 검색된 사실 기반 모든 주장에 대해 출처를 인용합니다. 표준 패턴 — 프롬프트가 모델에게 `[1]`, `[2]` 마커를 방출하라고 지시하고, UI가 마커를 출처 링크로 매핑. 견고한 시스템은 인용된 마커가 검색된 청크와 일치하는지 검증.

### D. 스트리밍과 지연 예산

챗 UX는 응답이 토큰 단위로 스트리밍될 때 살아 있게 느껴집니다. 두 지연이 중요:

- **TTFB (Time-To-First-Byte / 첫 토큰)**: 어떤 응답이 나타날 때까지 얼마. 검색(RAG라면)과 LLM prefill(생성 전 프롬프트 처리 시간)에 의해 지배.
- **TPS (Tokens-Per-Second / 스트리밍 처리량)**: 첫 토큰 후 토큰이 얼마나 빠르게 나타나는가. 모델 크기와 디코딩에 의해 지배.

사용자는 TTFB가 빠르고 TPS가 안정적이면 더 높은 *총* 지연을 견딥니다. 0.5초에 시작해서 부드럽게 스트리밍하는 10초 응답이 5초 침묵 후 한꺼번에 던지는 5초 응답보다 낫게 느껴집니다.

구체적으로 — 
- TTFB 감소 — 프롬프트 캐싱(레슨 24), 짧은 시스템 프롬프트, 빠른 검색 인덱스, 작은 prefill 배치.
- TPS 증가 — 작은 모델, vLLM/TGI(레슨 18), speculative decoding, FP8/INT4 양자화.

### E. 안전성, 모더레이션, 거부

프로덕션 챗봇은 **입력**과 **출력** 가드레일이 필요:

```
사용자 입력
    ↓
[입력 모더레이션 (PII, 금지 콘텐츠, 프롬프트 인젝션)]
    ↓
[챗봇 코어: RAG + LLM]
    ↓
[출력 모더레이션 (PII 누출, 독성 콘텐츠, 환각된 인용)]
    ↓
응답
```

각 단계는 규칙 기반 검사(SSN 정규식, 욕설 목록), 고전 ML 분류기(독성 모델), 또는 미묘한 경우에 LLM-as-judge를 사용할 수 있습니다. 레슨 21이 안전성을 깊이 다룹니다.

**거부 정책**은 어딘가에 인코딩되어야 합니다 — 시스템 프롬프트에, 별도 모더레이터 LLM에, 또는 제품 수준 규칙에. 흔함 — 계층 방어 — 시스템 프롬프트가 특정 주제에 대한 거부를 요청, *추가로* 모델이 거부에 실패한 것을 잡는 출력 필터.

### F. 프로덕션 관심사

**F.1 캐싱.** 동일하거나 거의 동일한 쿼리는 응답을 재사용해야 합니다. 두 계층 — 정확 캐시(입력의 해시)와 시맨틱 캐시(쿼리 임베딩, 유사한 과거 응답 조회). 레슨 24가 다룹니다.

**F.2 속도 제한.** 사용자별과 전역 속도 제한이 남용과 비용 통제 불능을 방지. 토큰 기반, 요청 기반, 또는 둘 다.

**F.3 폴백.** 주 LLM이 실패할 때(타임아웃, 오류, 정책 위반) 다른 모델, 정적 응답, 또는 "문제가 있습니다" 메시지로 폴백. 레슨 24.

**F.4 관측성.** 모든 턴이 로그 — 입력, 검색된 청크, 보낸 프롬프트, 모델 응답, 지연, 토큰 수, 비용, 사용자 피드백. 디버깅과 오프라인 평가에 사용.

**F.5 온라인 평가.** A/B 테스트(레슨 24) — 사용자의 일부를 후보 모델/프롬프트로 라우팅, 참여 지표 비교. 변경이 실세계 UX를 실제로 향상하는지 알 수 있는 유일한 방법.

### 이론에서 아래 함수들로

- §1 (아키텍처) — §A 상태 있는 함수 관점을 틀.
- §2 (기본 챗봇) — 최소 LLM-with-history; §B.1 버퍼 패턴.
- §3 (RAG 챗봇) — 항상 검색으로 §C 구현.
- §4 (고급 대화 관리) — §B.2-§B.4 (윈도우, 요약, 벡터 메모리) 구현.
- §5 (스트리밍) — HuggingFace / OpenAI API로 §D 스트리밍.
- §6 (FastAPI 서버) — §F 관측성 후크와 함께 챗봇을 프로덕션 API로 감쌈.
- §7 (Gradio UI) — 테스트용 빠른 웹 UI.
- §8 (프로덕션 고려사항) — §E 안전 파이프라인과 §F 캐싱/폴백 패턴 구현.
