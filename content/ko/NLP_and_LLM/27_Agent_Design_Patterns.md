...
패턴별 참조 전에 [**이론과 원리**](#이론과-원리)를 먼저 읽어 보세요 — 모든 에이전트 패턴이 트레이드오프하는 작은 설계 힘 집합(제어 흐름, 상태, 안전성, 비용), 그리고 올바른 것을 고르기 위한 결정 흐름.
...
에이전트 설계 패턴은 에이전트 시스템의 반복되는 문제를 푸는 제어 흐름 + 상태 + 상호작용의 재사용 가능한 모양입니다. 각 패턴은 특정 관심사를 처리하기 위해 기본 에이전트 루프(레슨 14)를 배열하는 특정 방법입니다 — 복잡한 작업 분해(orchestrator-worker), 다양한 쿼리 라우팅(router), 우아한 실패(escalation), 인간 감독(HITL), 안전 경계(guardrails) 등. 올바른 패턴을 고르는 것이 중요한 이유는 잘못된 모양이 어떤 프롬프트 튜닝으로도 고칠 수 없는 버그를 도입하기 때문이고, 반대로 올바른 패턴은 엉킨 맞춤형 시스템을 몇 개의 표준 조각으로 바꿀 수 있기 때문입니다.

이 섹션은 다음을 다룹니다:

- **(A) 네 설계 힘** — 모든 패턴이 트레이드오프하는 제어 흐름, 상태, 안전성, 비용.
- **(B) 패턴 카탈로그** — 이 레슨의 11 패턴 개관과 각각이 다루는 것.
- **(C) 합성** — 패턴이 어떻게 중첩되고 결합하는가; "패턴의 패턴" 관점.
- **(D) 결정 흐름** — 패턴을 고를 때 물어볼 질문.
- **(E) 안티패턴** — 흔한 오용과 인식 방법.

### A. 네 설계 힘

모든 에이전트 패턴은 네 경쟁 힘의 특정 해법입니다:

**A.1 제어 흐름.** 누가 다음에 무엇이 일어날지 결정하는가 — LLM(자율적), 고정 파이프라인(결정론적), 또는 하이브리드(router + worker)? 더 많은 LLM 제어 = 더 많은 유연성, 더 적은 예측 가능성.

**A.2 상태.** 작업에 대한 정보가 어디 사는가 — 프롬프트(일시적, 추론하기 쉬움), 데이터베이스(지속적, 확장 가능), 공유 메시지 그래프(멀티 에이전트, 조정됨)? 더 많은 상태 외부화 = 더 많은 능력, 더 많은 복잡성.

**A.3 안전성.** 어떤 검사가 언제, 누구에 의해 일어나는가 — 사전 실행 검증(가드레일), 인간 승인(HITL), 사후 실행 감사(출력 필터)? 더 많은 안전 검사 = 더 낮은 위험, 더 높은 지연.

**A.4 비용.** 얼마나 많은 LLM 호출, 각각 얼마나 큰지, 얼마나 병렬 — orchestrator-worker는 작업당 더 많은 LLM 호출이지만 병렬 워커 허용; router는 쿼리당 추가 호출 1회지만 대부분을 저렴한 모델로 라우팅. 더 많은 자율성 = 종종 더 많은 비용.

카탈로그(§B)의 모든 패턴은 이 4D 트레이드오프 공간의 특정 점입니다. 트레이드오프를 아는 것이 패턴이 *왜* 그렇게 보이는지와 그 약점이 무엇인지 이해하는 데 도움이 됩니다.

### B. 패턴 카탈로그

이 레슨의 각 패턴이 무엇을 위해 최적화하는지의 빠른 지도:

| 패턴 | 해결되는 주 힘 | 사용 시점 |
|------|--------------|----------|
| Orchestrator-Worker | 제어 흐름 (분해) | 작업이 복잡, 독립적 부속 작업 |
| Router | 제어 흐름 (특화) | 많은 별개 쿼리 유형, 각각 다른 에이전트/모델로 최선 처리 |
| Escalation | 안전성 + 비용 (우아한 격하) | 대부분 쿼리가 쉽고, 일부만 비싼 처리 필요 |
| Human-in-the-Loop | 안전성 (감독) | 위험이 큰 행동이 승인 필요 |
| Guardrailed Agent | 안전성 (검증) | 입력/출력이 단단한 제약 만족 |
| Supervisor | 제어 흐름 (감독) | 멀티 에이전트 조정이 중앙 결정자 필요 |
| Parallel Execution | 비용 (지연) | 부속 작업이 독립적이고 동시 실행 가능 |
| Agent Handoff | 제어 흐름 + 상태 | 작업이 도메인 경계를 넘음 (영업 → 엔지니어링 등) |
| Error Recovery | 안전성 (회복력) | 도구 실패, 자동 재시도/폴백 필요 |
| Composition / Nesting | 재사용성 | 같은 에이전트가 다른 에이전트의 도구가 되어야 함 |

프로덕션 시스템은 보통 여러 가지를 결합 — 예: 특화된 워커로 라우팅하는 supervisor, 파괴적 행동에 HITL과 출력에 가드레일.

### C. 합성

패턴은 합성됩니다. 고전적 프로덕션 에이전트 스택:

```
[user] → Guardrail (입력 검사) → Router (어떤 에이전트가 처리?)
       → 특화 에이전트 (부속 작업에 Orchestrator-Worker)
                            ├→ Worker A (병렬)
                            └→ Worker B (병렬)
                            └→ 파괴적이면 HITL 승인
       → Guardrail (출력 검사) → [응답]
...
이 단일 흐름이 5 패턴(Guardrail × 2, Router, Orchestrator-Worker, Parallel Execution, HITL) 사용. 각 패턴이 한 관심사 처리; 전체가 시스템. 이것이 패턴 어휘를 가지는 것의 가치 — 동료가 인식하는 용어로 시스템을 설명할 수 있습니다.

### D. 결정 흐름

에이전트 시스템 설계 시 이 질문들을 순서대로 따라가세요:

1. **작업이 단순한가 복잡한가?**
   - 단순 → 단일 에이전트(레슨 14), 입력 다양성에 router일 수도.
   - 복잡 → orchestrator-worker(분해) 또는 계층적 패턴.

2. **부속 작업이 독립적인가 순차적인가?**
   - 독립적 → 병렬 실행.
   - 순차적 → 파이프라인 / 순차 plan-execute.

3. **에이전트가 파괴적 행동을 하는가?**
   - 예 → 그 행동에 HITL 게이트, 입력/출력에 가드레일.
   - 아니오 → 표준 에이전트 루프 괜찮음.

4. **비용-품질 타깃은?**
   - 낮은 비용 → router → 저렴한 모델 먼저, 필요할 때만 비싼 것으로 escalation.
   - 높은 품질 → 앙상블 / 토론, 전체에 걸쳐 큰 모델.

5. **이 에이전트가 다른 에이전트가 사용할 것인가?**
   - 예 → 도구로 노출(합성); 입력/출력/실패 모드를 신중히 문서화.
   - 아니오 → 독립형 괜찮음.

답이 §B에서 어떤 패턴들을 함께 쌓을지 결정합니다. 멋있어 보이는 것으로 패턴 고르지 말고 — 이 답들이 가리키는 것으로 고르세요.

### E. 안티패턴

흔한 오용:

**E.1 과도한 분해.** 단일 에이전트로 했을 일을 십여 에이전트로 분할. 증상 — 품질 이득 없이 지연 곱해짐; 비용 폭발; 디버깅 더 어려워짐. 수정 — 단순한 것이 더 낫다; 작업이 진정으로 구조를 가질 때만 분해.

**E.2 파괴적 행동에 가드레일 누락.** 에이전트가 자율적으로 데이터 삭제, 이메일 전송, 청구서 결제. 증상 — 단일 버그가 되돌릴 수 없는 손상 야기. 수정 — 파괴적 도구에 항상 HITL 또는 단단한 검증.

**E.3 스파게티 라우팅.** 라우팅 로직 자체가 유지하기 어려울 만큼 많은 분기를 가진 router. 증상 — router 프롬프트가 "사용자가 X를 언급하면..."의 2K 토큰. 수정 — 계층적 라우팅(주 router가 광범위한 범주로, 그 후 특화된 부속 router).

**E.4 검증 없는 신뢰.** LLM의 "작업을 완료했습니다" 주장 가정. 증상 — 에이전트가 미완성 작업에 성공 선언. 수정 — 에이전트의 자기 보고와 독립적인 프로그램적 결과 검증.

**E.5 타임아웃 / 단계 상한 없음.** 모호한 목표 추구에 영원히 실행될 수 있는 에이전트. 증상 — 비용 통제 불능, 시스템 정체. 수정 — 모든 차원(단계, 토큰, 벽시간, 달러)에 단단한 상한.

### 이론에서 아래 함수들로

- §1 (orchestrator-worker) — §B의 주 분해 패턴 구현.
- §2 (router) — §B 라우팅 구현.
- §3 (escalation) — §B 우아한 격하 구현.
- §4 (HITL) — §B 인간 감독 패턴 구현.
- §5 (guardrailed agents) — §B 안전 검증 패턴 구현(레슨 21 가드레일 사용).
- §6 (supervisor) — §B 중앙집중 멀티 에이전트 제어 구현.
- §7 (병렬 실행) — §B 병렬 실행 구현.
- §8 (agent handoff) — §B 특화된 에이전트 사이 핸드오프 구현.
- §9 (오류 복구) — §B 회복력 패턴 구현(레슨 24 재시도/폴백 사용).
- §10 (합성 / 중첩) — §C 도구로서의 에이전트 합성 구현.
- §11 (적절한 패턴 선택) — §D 결정 흐름을 실용 가이드로 구현.
