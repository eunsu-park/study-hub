# 20. LLM 평가 지표 (Evaluation Metrics)
...
LLM 평가는 분류기 평가보다 어렵습니다. 분류기는 입력당 단일 정답 라벨을 가지지만, LLM은 많은 다른 출력이 똑같이 정확할 수 있는 개방형 텍스트를 만듭니다. 이 분야는 계층 평가 전략을 진화시켰습니다 — 구체적 참조가 있는 작업에 **n-gram 중첩**, 의미적 동치성에 **임베딩 유사도**, 능력에 **객관식 벤치마크**, 코드에 **실행 기반 점수화**, 미묘한 품질 판단에 **LLM-as-judge**, 황금 표준으로 **인간 평가**. 각 계층은 알려진 편향과 실패 모드를 가집니다 — 잘못된 지표를 고르면 실제 회귀를 가리거나 가짜 향상을 만들어낼 수 있습니다.

이 섹션은 다음을 다룹니다:

- **(A) 근본적 도전 과제** — 왜 "이 출력이 정확한가?"가 단일 답을 가진 질문이 아닌가.
- **(B) N-gram 지표 — BLEU, ROUGE, METEOR** — 정의, 무엇을 포착하고 어디서 실패하는가.
- **(C) 임베딩 기반 지표 — BERTScore, BLEURT** — 어휘 중첩 대신 의미적 유사도.
- **(D) Perplexity** — 언어 모델링 내재 지표, 사용과 한계.
- **(E) 객관식 벤치마크 — MMLU, HellaSwag, ARC** — 가능도를 통한 능력 측정.
- **(F) 실행 기반 — HumanEval, MBPP** — 정답이 검증 가능한 도메인으로서의 코드.
- **(G) LLM-as-judge** — 강력한 LLM을 사용해 출력 채점, 편향 풍경.
- **(H) 인간 평가** — 언제, 어떻게, 제대로 하는 비용.

### A. 근본적 도전 과제

번역 "The cat sat on the mat" → French에 대해 "Le chat s'est assis sur le tapis"와 "Le chat était assis sur le tapis" 모두 유효합니다. 단일 정답이 없습니다. 다중 참조? 도움이 되지만 (무한한) 유효 번역의 공간을 열거하지는 못합니다. 더 나쁜 것은 — 패러프레이즈가 한 단어도 공통이 아닌데도 유효한 번역일 수 있습니다.

이것이 핵심 문제 — **수용 가능한 출력의 공간이 열려 있다**. 고정 참조와 비교하는 어떤 지표든 근사입니다. 각 지표는 "근사"의 다른 측면을 트레이드오프합니다.

### B. N-gram 지표

**B.1 BLEU** (Papineni 등, 2002). 기계 번역용. n = 1에서 4에 대한 *수정된 n-gram 정밀도*(클립됨 — 후보의 각 n-gram이 어떤 참조에서 만큼만 카운트) 계산, 그 후 기하 평균, 그 후 brevity penalty:

```
BLEU = BP · exp( Σ_{n=1..4} (1/4) · log p_n )
BP = 1 if c > r else exp(1 − r/c)    (c = 후보 길이, r = 참조 길이)
```

Brevity penalty는 모델이 가장 가능한 1-2 단어만 방출하여 정밀도를 게임하는 것을 막습니다. BLEU는 코퍼스 수준에서 번역 품질과 잘 상관하지만 문장 수준에서는 빈약합니다.

**B.2 ROUGE** (Lin, 2004). 요약용. ROUGE-N은 n-gram recall, ROUGE-L은 가장 긴 공통 부분 시퀀스. 요약이 참조를 덮는지 포착, BLEU는 후보가 참조에 포함되는지 포착.

**B.3 METEOR.** 어간 추출과 동의어 매칭(WordNet) 추가. 문장 수준 평가에서 BLEU보다 견고, 덜 표준.

**실패 모드.** 모든 n-gram 지표:
- 글자 그대로 복사를 보상, 유효 패러프레이즈를 페널티.
- 결정적 단어 교체를 양식적 재표현과 구별 못함.
- 표준 참조가 없는 작업(개방형 생성, 대화)에서 실패.

### C. 임베딩 기반 지표

**C.1 BERTScore** (Zhang 등, 2020). 후보의 각 토큰에 대해 (BERT 임베딩 공간에서) 참조에서 가장 유사한 토큰을 찾고, cosine 유사도 평균. precision/recall에 대해 대칭, F1이 결합.

```
P_BERT = (1/|x|) · Σ_{xᵢ ∈ x}  max_{yⱼ ∈ y}  cos(BERT(xᵢ), BERT(yⱼ))
```

의미적 동치성을 포착 — n-gram 지표는 다르게 보지만 "began"과 "started"가 높게 점수. BERT forward pass 비용이지만 본질적으로 zero-shot — 작업별 튜닝 없음.

**C.2 BLEURT.** 인간 품질 평가에 파인튜닝된 BERT 모델. 출력은 BERTScore보다 인간 판단과 더 잘 상관하는 학습된 스칼라 점수. 더 정확하지만 모델 특화.

### D. Perplexity

Perplexity는 **내재** LM 지표:

```
PPL(model, text) = exp( − (1/N) · Σᵢ log p_model(tokenᵢ | tokens<ᵢ) )
```

낮을수록 좋음. 코퍼스에 대한 `1 / p(token)`의 기하 평균과 같음 — "모델이 평균적으로 얼마나 놀라는가?" PPL = 100은 모델이 평균적으로 토큰당 100개의 똑같이 가능한 후보 중에서 선택함을 의미.

용도:
- 같은 평가 코퍼스에서 두 LM 비교(낮은 PPL = 그 코퍼스 예측에 더 나음).
- 학습 진척 추적.

한계:
- 토크나이저 의존 — 다른 토크나이저를 가진 모델 사이 비교 불가(다른 N, 다른 단위).
- 품질 측정 아님 — Wikipedia에서 낮은 PPL을 가진 모델이 채팅에 반드시 도움이 되는 것은 아님.
- 생성 품질을 평가할 수 없고, 보존 텍스트에 대한 확률 할당만.

### E. 객관식 벤치마크

**E.1 MMLU** (Massive Multitask Language Understanding). 57과목(수학, 역사, 법학, ...)의 객관식 질문. 점수 — 모델이 정답 옵션에 가장 높은 확률을 할당하는 질문의 비율.

**E.2 HellaSwag.** 인간에게 쉽지만 초기 LM에 어렵도록 설계(적대적으로 필터링)된 문장 완성 질문. 상식 추론 포착.

**E.3 ARC, TruthfulQA, GSM8K, BIG-Bench.** 다양한 작업 모음. ARC는 초등 과학; TruthfulQA는 일반 오해에 대한 저항 테스트; GSM8K는 초등 수학 단어 문제.

**채점.** 이 모두가 옵션별 가능도를 비교하여 개방형 언어 모델링을 닫힌 작업으로 변환. 평가 저렴(생성 없음, 점수만). 위치 편향(LLM이 때로 옵션 (A)를 선호)과 암기(테스트 셋이 학습 데이터에 있음)에 취약.

### F. 실행 기반 평가

코드 생성에 대해 정답은 *실행*입니다. **HumanEval**(Chen 등, 2021)과 **MBPP**(Mostly Basic Python Problems) — 함수 시그니처와 docstring이 주어졌을 때, 함수 본문 생성; 숨은 테스트 슈트 실행; 점수 = 통과한 비율.
...
문제당 `n`개 해를 표본 추출, `c`개가 테스트 통과; 공식이 `k`개 독립 표본 중 *어떤 것이라도* 통과할 확률의 unbiased 추정치를 줍니다.

이는 진정으로 **객관적** — 통과는 통과. 주의 — 테스트가 불완전할 수 있음(모델이 테스트로 실행되지 않은 버그를 가진 채 통과 가능), 문제가 사전학습 데이터에 있을 수 있음.

### G. LLM-as-Judge

테스트 중인 모델의 출력을 채점하기 위해 강력한 LLM(GPT-4, Claude)을 사용. 흔한 패턴:
- **직접 채점** — "이 응답을 1-5로 평가하라."
- **쌍별 비교** — "A와 B 중 어느 것이 더 나은가?"
- **Faithfulness 검사** — "응답이 맥락에서 따라 나오는가?"

**편향:**
- **위치 편향** — 판사들이 종종 내용이 동일해도 옵션 A를 B보다 선호.
- **길이 편향** — 더 긴 응답이 더 높은 점수.
- **자기 선호** — GPT-4가 판단할 때 GPT-4 출력을 선호하는 경향.
- **아첨(sycophancy)** — 사용자가 드러낸 선호로 조종될 수 있음.

완화책 — 위치 무작위화, swap-and-average, 여러 판사 사용, Chain-of-Thought 사용("점수를 매기기 전에 추론 설명").

LLM-as-judge는 대부분의 작업에서 인간 판단과 ~0.6-0.8 상관 — n-gram 지표보다 낫고, 인간보다 훨씬 저렴, 판사 모델이 향상되면서 향상.

### H. 인간 평가

주관적 품질의 황금 표준이 여전합니다. 표준 절차 — 루브릭 정의, 평가자 고용, 평가자 간 동의 검증을 위한 파일럿, 계층화된 표본에 실행, 다수결/평균 점수 계산.

**비용**이 결정적 제약 — 작업 복잡도에 따라 라벨된 비교당 $0.50-$10. 드물게 사용 — 최종 모델 평가, 정렬 데이터, 또는 더 저렴한 지표(LLM-as-judge, 자동)의 보정 타깃으로.

견고한 평가 프로토콜은 이를 계층화 — 모든 커밋에 자동 지표, 주요 변경에 LLM-as-judge, 릴리스에 인간 평가.

### 이론에서 아래 함수들로

- §1 (중요성) — §A 근본적 도전 과제를 틀.
- §2 (텍스트 유사도) — §B (BLEU, ROUGE, METEOR)와 §C (BERTScore) 구현.
- §3 (LM 지표) — §D perplexity 구현.
- §4 (코드 생성) — HumanEval / MBPP에 §F pass@k 구현.
- §5 (LLM 벤치마크) — §E 벤치마크(MMLU, HellaSwag) 실행.
- §6 (LLM-as-judge) — 편향 완화 패턴과 함께 §G 구현.
- §7 (인간 평가) — 평가자 워크플로우 설정을 위한 §H 프로토콜.
- §8 (통합 프레임워크) — §B-§H를 다층 평가 슈트로 결합.
