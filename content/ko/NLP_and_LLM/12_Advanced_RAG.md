...
기본 RAG(레슨 10)는 한 번의 검색, 그 후 한 번의 생성을 합니다. 프로덕션의 거의 모든 RAG 품질 문제는 이 단일 샷 아키텍처로 거슬러 올라갑니다 — 사용자의 질문이 좋은 검색 쿼리가 아닐 수 있고(쿼리-문서 불일치), 검색기가 너무 많거나 너무 적은 후보를 반환할 수 있고(precision-recall 트레이드오프), 답이 단일 검색으로는 함께 찾을 수 없는 여러 문서의 정보를 필요로 할 수 있고(multi-hop), 검색이 실패할 때 모델이 회복할 방법이 없습니다. **고급 RAG는 단일 샷 제약을 깨는 기법들의 카탈로그**입니다 — 쿼리를 변환하거나, 검색을 루프하거나, 코퍼스를 계층적으로 조직하거나, LLM이 무엇을 언제 검색할지 동적으로 결정하게 함으로써.

이 섹션은 다음을 다룹니다:

- **(A) 왜 단일 샷 RAG가 실패하는가** — 모든 고급 기법을 동기화하는 네 가지 실패 모드.
- **(B) 쿼리 변환** — HyDE, 쿼리 확장, 부속 질문 분해.
- **(C) Multi-hop과 반복 검색** — 한 번의 검색으로 부족할 때.
- **(D) 계층적 검색 (RAPTOR)** — 단일 청크를 초과하는 문서를 위한 재귀 요약.
- **(E) Late-interaction (ColBERT)** — 토큰 수준 검색, 어려운 쿼리에서 단일 벡터 dense 검색을 이기는 이유.
- **(F) Self-reflective와 Agentic RAG** — LLM이 검색 루프를 제어, 언제 검색하고 언제 멈출지 결정.
- **(G) 재순위 매기기(reranking)** — 최종 정밀 필터로서의 cross-encoder reranker.

### A. 왜 단일 샷 RAG가 실패하는가

네 가지 별개의 실패 모드:

**A.1 쿼리-문서 불일치.** 사용자가 "what's the deal with the new chip?"이라 묻지만 코퍼스는 "Foundry-Series-7nm announcement"로 표현. 사용자 질문의 단일 임베딩이 관련 청크에서 멀리 떨어집니다.

**A.2 Multi-hop 질문.** "Who founded the company that makes the iPhone?"은 두 사실에 걸쳐 Apple → Steve Jobs를 검색해야 합니다. 단일 검색은 둘 중 하나는 찾을 수 있지만, 결합 추론 사슬은 거의 못 찾습니다.

**A.3 컨텍스트 윈도우 포화.** 긴 문서가 50개 청크로 분할될 수 있습니다. 검색은 올바른 청크를 찾지만, 답은 5개 청크 앞의 맥락 이해를 요구합니다.

**A.4 검색 실패.** 관련 사실이 코퍼스에 없거나 컷오프 아래에 묻혔습니다. 단일 샷 RAG는 조용히 자신감 있는 잘못된 답(맥락 내 방해 요소로부터의 환각)을 생성합니다.

각 고급 기법이 이 중 하나 이상을 다룹니다.

### B. 쿼리 변환

**B.1 HyDE (Hypothetical Document Embeddings)** (Gao 등, 2022). 사용자의 *질문*을 임베딩하는 대신, LLM에게 *답을 가설*하라고 한 다음 가설을 임베딩합니다. 가설은 실제 문서와 비슷한 어휘를 사용(LLM이 비슷한 산문으로 학습되었으므로)하여 쿼리-문서 격차를 줄입니다. 놀랍게도 가설이 *사실적으로 틀려도* 괜찮습니다 — 어휘적/의미적 스타일만 실제 문서와 일치하면 됩니다.

```
user: "what's the deal with the new chip?"
LLM hypothesis: "The new chip is a 7nm processor with improved power efficiency..."
embed(hypothesis) → 실제 발표 청크 검색
```

**B.2 쿼리 확장.** 쿼리의 여러 패러프레이즈 생성, 각각에 대해 검색, 결과 합집합. 같은 의도의 다른 표현을 포착. ~5 패러프레이즈를 넘으면 한계 효용 감소.

**B.3 부속 질문 분해.** 복잡한 쿼리에 대해 LLM에게 질문을 원자 부속 질문으로 분해, 각각에 대해 별도 검색, 그 후 종합. §C multi-hop 기법의 토대.

### C. Multi-Hop과 반복 검색

**C.1 패턴.** 초기 검색 → LLM이 학습한 것과 아직 알아야 할 것 추출 → 격차를 반영한 다음 검색 → 질문이 답할 수 있을 때까지 반복.

**C.2 IRCoT (Interleaved Retrieval CoT)** (Trivedi 등, 2022). LLM이 한 CoT 추론 단계 생성, 시스템이 그 단계로 검색, LLM이 검색된 증거에 조건부로 추론 계속, 다시 검색 등. 각 검색이 모델이 이미 추론한 것에 조건부 — 탐색이 코퍼스를 통한 안내된 걸음이 됩니다.

**C.3 Self-Ask** (Press 등, 2022). LLM이 자신에게 명시적으로 부속 질문을 묻고, 답을 검색하고, 그것들로 최종 추론. IRCoT보다 추론과 검색의 더 깔끔한 분리, 디버그가 더 쉬움.

**C.4 작동 원리.** Multi-hop 질문은 single-hop 질문의 시퀀스로 분해됩니다. 각 hop이 미지를 줄임 — "Apple makes the iPhone"을 알면 "who founded Apple?"은 single-hop 검색입니다. LLM이 분해를 하고, 검색기가 각 조각을 처리.

### D. 계층적 검색: RAPTOR

RAPTOR(Sarthi 등, 2024) — 청크를 검색하는 대신 **여러 단위(granularity) 수준의 요약**을 검색.

**D.1 구축.** 모든 청크 군집화(임베딩에 k-means). LLM으로 각 군집 요약. 요약을 군집화. 요약의 군집을 요약. 단일 루트 요약을 가질 때까지 계속. 결과는 잎이 문서 청크이고 내부 노드가 점진적으로 더 높은 수준의 요약인 트리.

**D.2 검색.** 쿼리 임베딩. 트리의 *모든 노드*(모든 수준의 청크 + 요약)와 유사도 계산. 상위 k 반환.

**D.3 작동 원리.** 고수준 주제에 대한 쿼리("이 논문이 무엇을 결론짓는가?")는 루트나 루트 근처 요약과 매칭, 간결한 초록 반환. 특정 세부 사항에 대한 쿼리("3일째에 무엇을 측정했는가?")는 잎 청크와 매칭. 트리가 자연스러운 규모-적응 검색 제공 — 단일 벡터 검색은 이를 할 수 없습니다, 어떤 단일 청크도 두 쿼리 유형 모두에 올바른 단위가 아니기 때문.

### E. Late-Interaction: ColBERT

단일 벡터 dense 검색(레슨 10)은 전체 문서를 하나의 `d` 차원 벡터로 압축합니다. 유용한 토큰이 무용한 토큰과 평균화 — 희귀하지만 결정적인 용어가 묻힐 수 있습니다.

**ColBERT**(Khattab & Zaharia, 2020)는 쿼리와 문서 모두에 **토큰당 벡터**를 유지합니다. 유사도는 **MaxSim** 연산자:

```
score(Q, D) = Σ_{q ∈ Q}  max_{d ∈ D}  sim(q, d)
```

각 쿼리 토큰에 대해 가장 유사한 문서 토큰을 찾습니다. 모든 쿼리 토큰에 걸쳐 합산. 토큰 수준 정보를 보존하고 각 쿼리 용어가 가장 잘 매칭되는 문서 용어를 찾게 합니다.

비용 — 문서당 많은 벡터를 저장하면 인덱스가 ~100배 부풀고 지연이 비례 상승. ColBERT는 일반적으로 더 저렴한 단일 벡터 검색에서 나온 더 작은 후보 집합 위의 *재순위* 매기기로 등장하며, 주 검색기로는 잘 안 씁니다.

### F. Self-Reflective와 Agentic RAG

**F.1 Self-RAG** (Asai 등, 2023). LLM이 생성 중 특수 토큰 방출 — 검색을 트리거하는 `[Retrieval=Yes]`, 검색된 증거를 등급 매기는 `[Relevant]` / `[Irrelevant]`, 자체 답변을 등급 매기는 `[Supported]` / `[Partially]` / `[No]`. 학습이 LLM을 이 토큰들을 정확히 사용하도록 파인튜닝. 결과 — LLM이 토큰별로 검색이 필요한지 동적으로 결정.

**F.2 Agentic RAG.** 검색을 에이전트(레슨 14)가 다른 도구(계산기, 코드 실행기, 웹 검색)와 함께 호출할 수 있는 하나의 도구로 취급. 에이전트가 ReAct 루프에서 추론, 증거가 필요하다고 판단할 때 검색기 호출. 가장 유연한 아키텍처, 가장 높은 비용.

원리 — **고정 파이프라인이 아닌 LLM이 검색 정책을 결정합니다.** 단일 샷 RAG가 가능한 가장 단순한 정책("항상 한 번 검색")이고, Agentic RAG가 가장 일반적입니다.

### G. 재순위 매기기

검색(dense, sparse, 또는 하이브리드)은 recall 지향 단계 — 많은 후보를 저렴하게 반환. **재순위 매기기**는 정밀 지향 단계 — 더 비싼 모델로 각 후보를 신중히 점수, 상위 몇 개 유지.

**Cross-encoder 재순위 매기기**는 (쿼리, 후보)를 단일 연결된 입력으로 받아 관련성 점수를 만듭니다. 예 — `bge-reranker-large`, `Cohere Rerank`, MS MARCO cross-encoder.

Cross-encoder는 수백만 문서에 대해 실행하기에 너무 느림(미리 계산된 인덱스 불가능 — 각 쿼리-문서 쌍을 함께 인코딩해야 함). 그러나 저렴한 검색의 상위 50-100에 대해 재순위 매기면 NDCG가 5-20 점 일상적으로 향상. 대부분의 프로덕션 RAG — dense 검색 → 상위 100 → cross-encode 재순위 → 상위 5 → LLM.

### 이론에서 아래 함수들로

- §1 (RAG 한계) — §A 실패 모드를 틀.
- §2 (쿼리 변환) — §B HyDE와 쿼리 확장 구현.
- §3 (Agentic RAG) — §F 도구 사용 LLM 제어 검색 코딩.
- §4 (multi-hop) — §C IRCoT와 Self-Ask 구현.
- §5 (RAPTOR) — §D 재귀 요약 트리 구축과 쿼리.
- §6 (ColBERT) — §E late-interaction MaxSim 검색 구현.
- §7 (Self-RAG) — §F self-reflective 토큰 패턴 구현.
