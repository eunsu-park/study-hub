...
단어 임베딩이 답하려는 질문은 단 하나입니다. *어떻게 하면 이산적 어휘를 연속적 벡터 공간으로 옮겨, 그 공간의 기하 관계가 언어적 관계를 반영하도록 만들 수 있는가?* Word2Vec, GloVe, FastText 같은 모든 정적(static) 임베딩 방법은 결국 각 단어에 저차원 `d`-벡터를 학습시켜, 코퍼스의 **공기(co-occurrence) 패턴**이 벡터 사이의 **내적(dot product)**으로 나타나게 하는 서로 다른 방식들입니다.

이 섹션은 다음을 다룹니다:

- **(A) 분포 가설(distributional hypothesis)과 룩업 테이블** — "단어는 그 주변 단어로 알 수 있다"가 왜 단어당 밀집 벡터(dense vector) 학습을 정당화하는가.
- **(B) Word2Vec Skip-gram + Negative Sampling** — 손실 유도, 왜 log-sigmoid + 음성 표본이 softmax를 저렴하게 근사하는가.
- **(C) CBOW** — 쌍대(dual) 문제이며, 언제 유리한가.
- **(D) GloVe** — 전역 공기 행렬의 로그-선형(log-bilinear) 분해와 가중 최소제곱 손실 유도.
- **(E) 벡터 연산이 작동하는 이유** (`king − man + woman ≈ queen`) — 선형 부분구조 성질과 PMI와의 연결.

### A. 분포 가설과 룩업 테이블

분포 가설(Harris, 1954): *비슷한 맥락에 등장하는 단어들은 비슷한 의미를 가진다.* 실용적으로, `cat`과 `dog`이 모두 `pet`, `feed`, `bowl`, `vet` 근처에 등장한다면, 단어로부터 맥락을(또는 맥락으로부터 단어를) 예측하는 어떤 모델이든 둘에게 비슷한 내부 표현을 부여하지 않을 수 없습니다.

가장 단순한 학습 가능한 표현은 **룩업 테이블** `E ∈ ℝ^{V × d}`입니다. `i`번째 행이 단어 `i`의 `d`-차원 벡터입니다. 정확히 PyTorch의 임베딩 레이어(`nn.Embedding`)입니다. 파라미터 수는 `V × d`이며, `V = 30K`, `d = 300`이면 9M로 현대 기준에서는 작은 편입니다.

핵심은 임베딩 자체가 원-핫 곱셈의 단축 표기일 뿐이라는 점입니다: `E[i] = E^T · onehot(i)`. "마법"은 전적으로 `E`를 학습시키는 손실 함수에 있습니다.

### B. Word2Vec: Skip-gram with Negative Sampling (SGNS)

**B.1 Skip-gram 목적함수.** 중심 단어 `wₜ`가 주어졌을 때, `j ∈ [-c, c] \ {0}`에 대한 각 맥락 단어 `wₜ₊ⱼ`를 예측합니다. 다음을 최대화합니다:

```
J = (1/T) · Σₜ Σⱼ  log p(wₜ₊ⱼ | wₜ)
```

순진한 파라미터화는 전체 어휘에 대한 softmax를 사용합니다:

```
p(o | c) = exp(uₒ · vc) / Σ_{w ∈ V} exp(uw · vc)
```

여기서 `vc`는 중심 단어의 *입력* 임베딩, `uₒ`는 맥락 단어의 *출력* 임베딩입니다. 두 개의 테이블을 학습합니다. 분모는 학습 예시당 `O(V)`가 들고, `V = 10⁵+`에서는 사실상 불가능합니다.

**B.2 Negative sampling — 작동시킨 트릭.** 다중 클래스 softmax를 이진 로지스틱 회귀로 대체합니다. *진짜* (중심, 맥락) 쌍과 `k`개의 *가짜* 쌍 `(중심, 표본 노이즈 단어)`를 구별합니다. 양성 쌍 1개와 음성 `k`개의 손실:

```
L = -log σ(uₒ · vc) − Σᵢ₌₁..k  𝔼_{wᵢ ~ Pₙ(w)}  log σ(−uᵢ · vc)
```

`σ`는 시그모이드, `Pₙ(w) ∝ U(w)^{3/4}`는 unigram 분포의 0.75 거듭제곱(매우 흔한 단어를 약간 다운가중). `k = 5-20`이면 각 갱신은 출력 테이블의 `k+1` 행만 건드립니다 — `O(V)` 대신 `O(1)`.

**B.3 작동 원리.** 이 손실은 잡음 대조 추정(NCE, noise-contrastive estimation) 목적함수로 유도할 수 있고, 같은 softmax 분포를 점근적으로 추정하지만 분배 함수(partition function)를 우회합니다. Mikolov 등(2013)은 SGNS가 *암묵적으로* 시프트된 PMI 행렬을 분해하고 있음을 보였습니다:

```
uₒ · vc ≈ PMI(o, c) − log k
```

여기서 `PMI(o, c) = log[ p(o, c) / (p(o) · p(c)) ]`는 점별 상호정보량(pointwise mutual information)입니다. 이것이 GloVe로 가는 다리입니다.

### C. CBOW: 맥락에서 중심 단어 예측

CBOW(Continuous Bag of Words)는 예측 방향을 뒤집습니다. 맥락 단어 `{wₜ₋c, ..., wₜ₊c}`가 주어졌을 때 중심 단어 `wₜ`를 예측합니다. 맥락 벡터들은 평균되어 단일 표현이 되고, softmax(실무에서는 hierarchical softmax 또는 negative sampling)를 통과해 `wₜ`를 예측합니다.

| 속성 | Skip-gram | CBOW |
|------|-----------|------|
| 예측 방향 | 중심 → 맥락 | 맥락 → 중심 |
| 단어당 유효 예시 수 | `2c` (맥락 위치당 1개) | 1 (중심당 1개) |
| 희귀 단어 표본 효율 | 우수 (각 출현이 2c번 학습) | 열위 |
| 학습 속도 | 느림 (문장당 갱신 횟수 많음) | 빠름 |
| 작은 데이터셋 품질 | 우수 | 열위 |

학습 시간이 결정적 제약이 아니라면 대부분의 실무 환경에서 Skip-gram이 기본 선택입니다.

### D. GloVe: Global Vectors for Word Representation

Word2Vec은 **국소(local)** 방법입니다 — 코퍼스 위에 윈도우를 슬라이딩하며 한 번에 한 예시씩 갱신합니다. GloVe(Pennington 등, 2014)는 **전역(global)** 방법입니다 — 먼저 전체 단어-맥락 공기 행렬 `X`(`Xᵢⱼ` = 단어 `i`의 맥락에 단어 `j`가 등장한 횟수)를 만들고, 이를 분해합니다.

**D.1 Log-bilinear 모델.** 모델은 다음을 가정합니다:

```
wᵢ · w̃ⱼ + bᵢ + b̃ⱼ ≈ log Xᵢⱼ
```

`wᵢ`는 단어 벡터, `w̃ⱼ`는 별도의 맥락 벡터, `bᵢ`, `b̃ⱼ`는 편향 항입니다. 동기는 *공기 확률의 비율*이 의미를 부호화한다는 관찰입니다. 예: `P(solid | ice) / P(solid | steam) >> 1`이며, 내적 구조가 이런 비율을 선형화합니다.
...
```
f(x) = (x / xₘₐₓ)^α   x < xₘₐₓ 일 때,   그 외에는 1     (보통 xₘₐₓ = 100, α = 0.75)
```

가중치가 필요한 이유: 가중치가 없으면 매우 빈번한 쌍(예: 모든 것과 공기하는 `the`)이 손실을 지배합니다. 상한 `xₘₐₓ`이 이를 막고, `xₘₐₓ` 이하에서의 다항 감쇠는 대부분 잡음에 가까운 매우 희귀한 쌍을 다운가중합니다. 학습 후 최종 단어 벡터는 `wᵢ + w̃ᵢ`(두 역할을 평균하면 안정성이 향상됩니다).

**D.3 GloVe vs Word2Vec.** GloVe의 손실은 공기 행렬을 명시적으로 분해하고, Word2Vec의 SGNS는 암묵적으로 같은 일을 합니다(B.3에서 본 시프트 PMI 분해). 둘 다 내재적(intrinsic) 과제에서 비슷한 수준의 임베딩을 만듭니다. GloVe는 메모리 사용이 많지만(행렬 `X`가 거대할 수 있음), 고정 코퍼스에서 학습 속도는 더 빠릅니다 — 개별 단어 출현이 아닌 `X`를 순회하기 때문입니다.

### E. 벡터 연산이 작동하는 이유

유명한 결과 `king − man + woman ≈ queen`은 우연이 아니라 log-bilinear 구조에서 따라옵니다. 임베딩이 대략 다음을 만족한다고 가정합니다:

```
w_x · w_y ≈ log P(x | y) + const
```

(man, woman), (king, queen) 같은 관계 쌍에 대해:

```
w_king − w_man ≈ log P(king | y) − log P(man | y) for any y
```

`royalty` 관계가 두 성별 변형 모두에 같은 방식으로 분포를 시프트한다면 `w_king − w_man ≈ w_queen − w_woman`이 되고, 정리하면 `w_king − w_man + w_woman ≈ w_queen`입니다. 유추는 기저 관계가 *선형으로 부호화*될 때 작동합니다 — 성별, 복수, 국가-수도, 비교급-최상급 등이 해당됩니다. 선형으로 부호화되지 않는 관계(예: 동의어의 미묘한 등급)는 이 검사를 통과하지 못합니다.

### 이론에서 아래 함수들로

- §1 (개요) — §A의 분포 가설을 장난감 벡터로 구체화.
- §2 (Word2Vec)는 §B (Skip-gram + negative sampling)와 §C (CBOW)를 gensim의 `Word2Vec` 클래스로 구현.
- §3 (GloVe)는 §D의 전역 공기 행렬 구축을 따라갑니다.
- §4 (사전학습 임베딩)는 Google News word2vec과 Stanford GloVe 체크포인트 로드 — 모두 §B와 §D 알고리즘의 산출물.
- §5 (임베딩 연산)은 §E의 선형 성질(유추, 유사도, 이질 항목)을 시연합니다.
