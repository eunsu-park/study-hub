...
LLM 추론은 원시 FLOP가 아닌 **메모리 대역폭과 KV-cache 관리**에 의해 병목됩니다. 현대 GPU는 초당 조 단위 FLOP를 할 수 있지만, 모든 생성 토큰은 모델 가중치와 KV-cache를 메모리에서 읽어야 합니다 — 그리고 메모리 대역폭이 계산보다 훨씬 먼저 포화됩니다. 추론 최적화는 본질적으로 *불필요한 바이트를 옮기지 않고*, 옮겨야 하는 바이트를 많은 동시 요청에 걸쳐 분할 상환하기 위한 *배칭*의 공학입니다.

이 섹션은 다음을 다룹니다:

- **(A) 두 단계 — prefill과 decode** — 왜 완전히 다른 성능 프로필을 가지는가.
- **(B) 추론이 메모리 바운드인 이유** — 산술 강도(arithmetic intensity) 논거.
- **(C) KV-cache — 구조, 메모리, 단편화 문제** — 서빙의 지배적 메모리 소비자.
- **(D) Continuous batching** — 가변 길이 생성에 정적 배칭이 잘못된 기본 단위인 이유.
- **(E) PagedAttention (vLLM)** — KV-cache를 위해 운영 체제로부터 가상 메모리 차용.
- **(F) Speculative decoding** — 작은 "드래프트" 모델로 큰 모델 가속.
- **(G) Tensor parallelism vs pipeline parallelism** — 한 GPU가 부족할 때.

### A. 두 단계 — Prefill과 Decode
...
### G. Tensor와 Pipeline Parallelism
...
- §1 (병목) — §A 두 단계와 §B 메모리 바운드 논거를 틀.
- §2 (vLLM) — §C-§E (KV-cache + PagedAttention + continuous batching) 구현.
- §3 (TGI) — Text Generation Inference, 비슷한 최적화의 대안 서빙 스택.
- §4 (speculative decoding) — §F 드래프트+타깃 패턴 구현.
- §5 (양자화) — 레슨 17 안내.
- §6 (배치 처리) — §D continuous batching 전략.
- §7 (성능 벤치마킹) — §B 산술 강도를 실무에서 측정.
