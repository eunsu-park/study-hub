...
GPT(Generative Pre-trained Transformer)는 단일 목표 — **다음 토큰 예측** — 으로 학습된 Transformer **디코더** 스택이며, 자기회귀(autoregressive) 생성으로 모든 일을 합니다. BERT가 양방향 맥락으로 이해를 최적화한다면, GPT는 그것을 *생성* 능력과 *프롬프트*로 조종 가능한 능력과 맞바꿉니다. 현대 LLM에서 우리가 연관 짓는 모든 행동 — 채팅, 인컨텍스트 학습, 코드 자동 완성 — 은 충분한 규모의 다음 토큰 예측에서 발현됩니다.

이 섹션은 다음을 다룹니다:

- **(A) 자기회귀 언어 모델링** — 목적함수, 왜 이것으로 충분한가, 사슬 법칙 분해.
- **(B) 인과적 마스킹(causal masking)** — 구현, 기하학적 의미, 왜 병렬 학습을 가능하게 하는가.
- **(C) 디코딩 전략** — greedy, beam, sampling, top-k, top-p (nucleus), temperature; 각각의 수학.
- **(D) KV-cache** — 자기회귀 추론은 순진하게는 `O(L²)`; 캐시가 `O(L)`로 만든다.
- **(E) 인컨텍스트 학습** — 충분히 큰 LM이 그래디언트 갱신 없이 few-shot 예시를 패턴 매칭할 수 있는 이유.
- **(F) 스케일링 법칙** — Kaplan / Chinchilla 연산-데이터-파라미터 트레이드오프.
...
- §1 (개요) — 이전 레슨의 Transformer 변형 가운데 GPT를 위치시킵니다.
- §2 (자기회귀 LM) — §A의 사슬 법칙 손실을 시프트된 로짓에 대한 `cross_entropy`로 구현.
- §3 (아키텍처) — §B 인과적 마스크와 함께 디코더 스택 조립.
- §4 (텍스트 생성) — §C의 디코딩 전략(greedy, beam, top-k, top-p, temperature) 코딩.
- §5 (GPT 시리즈) — §F의 스케일링 법칙 여정을 GPT-1부터 GPT-4까지 개관.
- §6 (HuggingFace GPT-2) — §A-§D를 `GPT2LMHeadModel.generate()`로 연결.
- §7 (인컨텍스트 학습) — §E의 zero-shot / few-shot 프롬프팅 시연.
- §8 (KV 캐시) — §D 캐시 구현과 속도 향상 벤치마크.
