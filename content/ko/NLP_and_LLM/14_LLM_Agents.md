...
```
state ← 초기 컨텍스트 (사용자 쿼리, 사용 가능 도구, 히스토리)
while not done:
    action ← LLM(state)             # 무엇을 할지 결정
    observation ← execute(action)    # 그것을 수행
...
1. **LLM**(두뇌) — 현재 상태가 주어지면 구조화된 행동 선택을 출력.
2. **도구 집합**(수행자) — 에이전트가 호출할 수 있는 함수(검색, 계산기, 코드 실행, 파일 읽기/쓰기, API 호출).
...
> 1. **LLM (Brain)** -- 두뇌 (의사결정)
> 2. **Planning** -- 계획 수립
> 3. **Tools** (검색, 계산, 코드실행) + **Memory** (대화 이력, 지식 베이스)
...
>
> 1. **사용자 메시지**
> 2. **LLM (사고)** --tool_use--> **도구 실행기** --tool_result--> **LLM**
> 3. LLM이 최종 텍스트 응답을 생성할 때까지 반복
> 4. **최종 응답**
...
[멀티 에이전트 시스템](./15_Multi_Agent_Systems.md)에서 멀티 에이전트 아키텍처와 오케스트레이션 패턴을 학습합니다.
