# 08. PEFT (Parameter-Efficient Fine-Tuning) 통합
...
PEFT(Parameter-Efficient Fine-Tuning)는 단일 경험적 관찰 위에 세워진 방법군입니다 — **큰 사전학습 모델을 다운스트림 작업에 적응시키는 데 필요한 갱신은 전체 파라미터 공간의 아주 작은 부분공간에 살고 있다.** 그 부분공간이 "저랭크"(LoRA)이든, "추가된 MLP 모듈"(adapter)이든, "앞에 붙인 가상 토큰"(prefix/prompt tuning)이든, "스케일링 벡터"(IA³)이든 목표는 같습니다 — 기본 모델을 완전히 동결하고, ~0.1-1%의 파라미터만 학습하며, 풀 파인튜닝 품질의 ~99%를 회복.

이 섹션은 다음을 다룹니다:

- **(A) 왜 저차원 갱신으로 충분한가** — 내재 차원성(intrinsic dimensionality) 논거와 그것이 파인튜닝에 의미하는 것.
- **(B) LoRA** — 저랭크 분해 `ΔW = BA`, 갱신 규칙 유도, 랭크 vs 품질 트레이드오프.
- **(C) Adapter 모듈** — 각 Transformer 레이어에 삽입되는 병목 MLP, additive PEFT 군.
- **(D) Prefix tuning, prompt tuning, P-tuning** — 키/값 앞에 붙이는 부드러운 가상 토큰, "입력 측" PEFT 군.
- **(E) QLoRA** — 4-bit 베이스 모델 양자화(NF4 + double quantization)와 LoRA를 결합, 65B 파라미터 파인튜닝을 단일 48 GB GPU에서 가능하게.
- **(F) 방법 비교** — additive vs reparameterization vs selective; 언제 어느 것이 이기는가.

### A. 왜 저차원 갱신으로 충분한가

Aghajanyan 등(2021)은 파인튜닝의 **내재 차원성**을 측정했습니다 — 무작위 방향의 부분공간에서 *오직* 그 부분공간에서만 파인튜닝하여 풀 파인튜닝 성능을 회복하는 데 필요한 자유 파라미터의 최소 수. 결과는 인상적이었습니다 — GLUE 작업의 BERT-base에서, 총 ~110M 중 단지 수백~수천 자유 파라미터로 충분했습니다.

이는 풀 파인튜닝의 `ΔW`(파인튜닝 가중치와 사전학습 가중치의 차이)가 **중복(redundant)**임을 말합니다. 110M 차원 갱신의 거의 모든 방향이 사용되지 않습니다. 갱신을 저차원 공간에 명시적으로 제한하는 방법은 파라미터의 작은 일부만으로 거의 동등한 품질에 도달해야 합니다.

PEFT는 이 통찰의 공학적 실현입니다. 각 방법은 특정 부분공간 선택과 그것을 파라미터화하는 특정 방식에 해당합니다.

### B. LoRA: Low-Rank Adaptation

**B.1 분해.** 동결된 가중치 행렬 `W₀ ∈ ℝ^{d × k}`(예: Transformer의 어텐션 사영)에 대해, LoRA는 표준 파인튜닝 갱신 `W = W₀ + ΔW`를 다음으로 대체합니다:

```
W = W₀ + BA       B ∈ ℝ^{d × r}, A ∈ ℝ^{r × k}, 랭크 r << min(d, k)
```

`A`와 `B`만 학습; `W₀`는 동결. 새 학습 가능 파라미터 수는 `d·k` 대신 `r·(d + k)`. `d = k = 4096`, `r = 8`에서 65K vs 16M, 250배 감소.

**B.2 초기화와 스케일링.** `A`는 가우시안 잡음으로 초기화; `B`는 0으로. 그래서 학습 시작 시 `BA = 0`이고 모델은 사전학습 체크포인트와 동일합니다. 학습의 갱신이 `B`를 0에서 멀어지게 하고 갱신 `ΔW = BA`가 거기서부터 자라납니다.
...
`α`와 `r`을 독립적으로 조정하면 학습률 재조정 없이 랭크를 변경할 수 있습니다(재스케일링 없는 더 높은 랭크는 갱신을 증폭시킬 것).

**B.3 저랭크가 작동하는 이유.** A의 내재 차원성과 — 어텐션의 QKV 사영이 사전학습 상태에서 종종 거의 저랭크라는 경험적 사실 — 을 결합. 파인튜닝 작업은 작업 관련 특징과 정렬된 몇 개의 랭크-1 보정만 추가하면 됩니다. 랭크 `r = 4`에서 `r = 32`가 표준 스위트 스폿.

**B.4 추론 비용.** 추론 시 `W' = W₀ + BA`를 한 번 계산하고 `W'`를 직접 사용 — 풀 파인튜닝 대비 추가 지연 없음. 또는 분리된 채로 유지하여 요청별로 LoRA 어댑터를 교체, matmul당 작은 `O(r·d)` 추가.

**B.5 어디에 적용.** 원래는 Q와 V 사영에만(Hu 등, 2021); 현대의 모범 사례는 Q, K, V와 FFN 출력 사영에. 모든 곳(모든 선형 레이어)에 적용하면 더 많은 파라미터를 대가로 약간의 추가 향상.

### C. Adapter 모듈

원조 PEFT 방법(Houlsby 등, 2019). 각 어텐션과 FFN 부속 레이어 후에 작은 병목 MLP를 삽입:

```
adapter(x) = x + Up(GeLU(Down(x)))     Down: d → m, Up: m → d, m << d
```

`Down`과 `Up` 행렬만 학습. 병목 차원 `m = 16`에서 `m = 64`이 전형적이며 레이어당 ~1% 추가 파라미터.

Adapter는 네트워크의 *순차적* 추가입니다 — 모든 forward pass가 통과합니다. LoRA의 핵심 장점은 학습 후 갱신이 `W₀`에 병합되어 추론 오버헤드가 0이라는 점입니다. Adapter는 별도의 비선형 계산이라 병합이 불가능합니다.

### D. Prefix, Prompt, P-Tuning

**D.1 Prefix tuning** (Li & Liang, 2021). 학습 가능한 가상 토큰 `P ∈ ℝ^{p × d}` `p`개를 모든 Transformer 레이어의 키와 값 앞에 붙입니다. 모델의 동결된 어텐션이 이 prefix 토큰들이 "말하는" 것에 의해 조종됩니다. 학습 가능 파라미터 수: `p · d · n_layers · 2` (K와 V에 대해 인자 2). 전형적 `p = 5-20`.

**D.2 Prompt tuning** (Lester 등, 2021). 더 얕은 변형 — 학습 가능한 토큰을 *입력* 레이어에만 붙입니다. 훨씬 적은 파라미터(`p · d` 총)이지만 매우 큰 모델 규모(10B+)에서만 잘 작동.

**D.3 P-tuning v2** (Liu 등, 2021). prompt를 입력만이 아닌 모든 레이어에 삽입한 prefix tuning, 모든 모델 크기에서 경쟁력이 있게 합니다.

이들은 LoRA/adapter와 개념적으로 다릅니다 — 모델의 가중치를 수정하는 대신 모델이 조건부로 삼는 *입력 분포*를 수정합니다. 기본 모델은 적응이 아니라 조종됩니다.

### E. QLoRA

QLoRA(Dettmers 등, 2023)는 단일 48 GB GPU에서 65B 파라미터 모델을 파인튜닝하기 위해 세 가지 혁신을 결합합니다:

**E.1 4-bit NormalFloat (NF4) 양자화** — 동결된 베이스 가중치에 적용. 사전학습 가중치는 경험적으로 0 중심 거의 정규 분포를 따릅니다. NF4는 16개 양자화 레벨을 `N(0, 1)`의 분위수(quantile)로 선택 — 정규 분포 가중치에 특화된 양자화 오류 최소화. 가중치당 16비트 대신 4비트, 4배 메모리 감소.

**E.2 Double quantization.** 양자화 스케일 자체(64 가중치 블록당 하나)도 양자화하여 가중치당 ~0.4비트 추가 절약.

**E.3 Paged optimizer.** GPU 메모리가 정점에 달할 때(예: 긴 시퀀스의 그래디언트 스파이크) NVIDIA 통합 메모리를 사용해 Adam 옵티마이저 상태를 CPU로 페이징, OOM 충돌 방지.

**E.4 위에 LoRA.** 양자화된 모델 위에 LoRA 어댑터 추가. LoRA 행렬 `A, B`만 fp16으로 저장; 그래디언트는 역양자화된 베이스 가중치를 통해 흐르지만 `A, B`만 갱신합니다. 베이스 가중치는 forward pass 중 즉시 역양자화되고 저장을 위해 재양자화됩니다.

이 결합은 ~1/16 메모리에서 풀 fp16 파인튜닝과 거의 같은 품질을 달성합니다.

### F. PEFT 군 비교

서로 다른 메커니즘의 세 군:

| 군 | 메커니즘 | 예시 | 병합 가능? | 추론 비용 |
|----|----------|------|----------|----------|
| Reparameterization | `ΔW` 분해 | LoRA, IA³, DoRA | 가능 (LoRA) | 병합 후 없음 |
| Additive | 새 모듈 삽입 | Adapter, Houlsby | 불가 | 작은 추가 |
| Prompt 기반 | 학습 가능 토큰 앞에 붙임 | Prefix, P-tuning | 불가 | 작음 (시퀀스 더 김) |
| Selective | 가중치 부분집합 갱신 | BitFit (편향만) | 가능 (이미 그 자리에) | 없음 |

LoRA가 실무에서 지배적입니다 — 이유: (1) 추론 비용 0으로 병합, (2) 모든 모델 규모에서 작동, (3) 랭크가 해석 가능하고 조정 가능. QLoRA가 LoRA의 도달 범위를 GPU가 적은 환경으로 확장. Adapter와 prefix tuning은 주로 연구에 남아 있습니다.

### 이론에서 아래 함수들로

- §1 (PEFT 개요) — §A 내재 차원성 논거를 틀.
- §2 (LoRA) — §B.1의 `W₀ + BA` 분해를 §B.2 초기화와 §B.5 배치 선택으로 구현.
- §3 (adapter 방법) — §C의 병목 MLP와 Houlsby vs Pfeiffer 배치 코딩.
- §4 (prompt 기반 방법) — §D의 prefix와 prompt tuning 구현.
- §5 (HuggingFace PEFT) — §B-§D를 통합 `peft` 라이브러리 API로 연결.
- §6 (방법 비교) — §F 표를 벤치마크 작업에 경험적으로 적용.
