...
파인튜닝은 이미 *일반적인* 언어를 아는 모델을 가져와 *특정* 작업이나 도메인으로 구부리는 일입니다. 사전학습이 비싼 일을 모두 해 두었습니다 — 통사, 기본 의미, 세상 지식을 테라바이트의 라벨 없는 텍스트로 학습한 것입니다. 파인튜닝은 그 표현을 작은 라벨된 데이터셋(수십억이 아닌 수천 개)으로 활용합니다. 핵심은 **모델을 처음부터 유용하게 만든 일반 지식을 파괴하지 않으면서 작업을 학습할 만큼만 가중치를 갱신하는 것**입니다.

이 섹션은 다음을 다룹니다:

- **(A) 전이학습** — 처음부터 학습보다 사전학습 후 파인튜닝이 지배적인 이유.
- **(B) 풀 파인튜닝 vs 특징 추출 vs 헤드만** — 갱신하는 파라미터의 양 스펙트럼.
- **(C) 파국적 망각(catastrophic forgetting)** — 모든 "부드럽게" 레시피를 만든 실패 모드.
- **(D) 학습률 스케줄과 워밍업** — 왜 파인튜닝이 `1e-3` 코사인이 아닌 `2e-5` 선형을 쓰는가.
- **(E) 작업별 손실 함수** — 분류·NER(토큰별)·QA(시작/끝 span)·seq2seq에 대한 cross-entropy 변형.
- **(F) 정규화 트릭** — early stopping, dropout, weight decay, label smoothing, layer-wise learning rate decay.
...
- §1 (개요) — §A의 전이학습 명제를 틀.
- §2 (텍스트 분류 FT) — §E.1을 `BertForSequenceClassification`로 구현, §D의 학습률 레시피 사용.
- §3 (NER FT) — §E.2를 서브워드 토큰 정렬과 함께 구현.
- §4 (QA FT) — §E.3의 시작/끝 span 손실을 SQuAD에 구현.
- §5 (PEFT) — §B의 우측 열을 가리키며, 전체 레슨은 08.
- §6 (대화형 FT) — §E.5의 instruction tuning 구현.
- §7 (학습 최적화) — §D 스케줄, 혼합 정밀도, 그래디언트 누적 적용.
- §8 (전체 예시) — §C-§F를 실행 가능한 엔드 투 엔드 파이프라인으로 묶습니다.
