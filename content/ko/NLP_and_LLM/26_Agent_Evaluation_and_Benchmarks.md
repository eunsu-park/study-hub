...
벤치마크와 방법론 참조 전에 [**이론과 원리**](#이론과-원리)를 먼저 읽어 보세요 — 에이전트 평가가 LLM 평가와 다른 점, 궤적-vs-결과 구분, 모든 에이전트 벤치마크가 항해하는 비용-품질 프런티어.
...
에이전트 평가는 LLM 평가보다 어렵습니다. LLM은 한 입력(프롬프트)과 한 출력(응답)을 가지며 평가가 출력을 참조와 비교합니다. 에이전트는 *궤적(trajectory)* — (생각, 행동, 관측) 튜플의 시퀀스 — 을 가지며 목표에 도달하기 위해 많은 LLM 호출과 도구 호출이 걸릴 수 있습니다. 두 에이전트가 같은 작업에서 모두 성공해도 매우 다른 비용, 지연, 안전성, 궤적 품질을 가질 수 있습니다. 에이전트 평가는 목표 달성 여부만이 아니라 어떻게, 어떤 비용으로 달성했는지 측정해야 합니다.

이 섹션은 다음을 다룹니다:

- **(A) 궤적 vs 결과 구분** — 왜 "성공했는가?"가 충분하지 않은가.
- **(B) 작업 성공과 부분 점수** — 작업을 설계하는 것이 측정 프로토콜을 설계하는 것.
- **(C) 주요 벤치마크** — AgentBench, SWE-bench, WebArena, GAIA, 각각이 실제로 테스트하는 것.
- **(D) 실패 모드 분류** — 에이전트가 실패하는 흔한 방식(루프, 환각, 도구 오용, 조기 중단).
- **(E) 궤적 분석** — 관측성, 추적, 구조화된 로그의 가치.
- **(F) 비용-품질 프런티어** — 파레토 트레이드오프와 그것을 어떻게 전달하는가.
- **(G) 커스텀 평가 구축** — 작업 설계, 정답, 자동 채점.
...
- §1 (도전 과제) — §A 궤적-vs-결과와 §B 성공 정의를 틀.
- §2-§5 (AgentBench, SWE-bench, WebArena, GAIA) — §C 주요 벤치마크 구현.
- §6 (평가 방법론) — §B 등급 채점과 §F 다중 목적 지표.
- §7 (실패 모드 분석) — 감지기와 함께 §D 분류 구현.
- §8 (관측성과 추적) — LangSmith/Phoenix와 함께 §E 구조화된 로깅.
- §9 (비용-품질 분석) — §F 파레토 프런티어 시각화.
- §10 (커스텀 평가) — §G 작업 정의 + 채점 + 추적 파이프라인.
