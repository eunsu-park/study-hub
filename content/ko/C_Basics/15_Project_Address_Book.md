...
**이전**: [프로젝트: 숫자 맞추기 게임](./14_Project_Number_Guessing.md) | **다음**: [C 고급](../C_Advanced/00_Overview.md)
...
1. `typedef struct`를 설계하여 관련 필드(이름, 전화번호, 이메일)를 단일 레코드 타입으로 그룹화하기
2. 고정 크기 구조체 배열을 구축하고 활성 항목 수를 추적하는 카운트 변수 관리하기
3. 구조체 포인터와 배열 이동을 사용하여 완전한 CRUD 작업(생성, 읽기, 수정, 삭제) 구현하기
4. 파일 입출력 함수(`fopen`, `fread`, `fwrite`, `fclose`)를 적용하여 세션 간 바이너리 데이터 유지하기
5. 텍스트 모드(`fprintf`/`fscanf`)와 바이너리 모드(`fread`/`fwrite`) 파일 작업의 차이 구별하기
6. `fgets`와 `strcspn`을 사용하여 안전하게 문자열 입력을 읽고 후행 줄바꿈 제거하기
7. `strstr`을 사용하여 여러 필드에 걸친 부분 문자열 매칭으로 레코드 검색하기

---

주소록은 구조체와 파일 입출력을 배우기 위한 고전적인 연습입니다. 실제 데이터베이스 애플리케이션을 반영하기 때문입니다: 레코드를 생성하고, 목록을 보고, 키워드로 검색하고, 필드를 편집하고, 항목을 삭제합니다 -- 완전한 CRUD 사이클. 연락처를 바이너리 파일에 저장함으로써 휘발성 인메모리 데이터와 내구성 있는 디스크 저장소의 차이도 경험하게 됩니다. 이 구분은 모든 실세계 프로그램에서 중요합니다.
...
3. **그룹 기능**: 연락처에 그룹 속성 추가 (가족, 친구, 직장)

4. **중복 감지**: 연락처 추가 전에 이름이나 전화번호가 이미 존재하는지 확인하고 사용자에게 경고

5. **삭제 취소**: 연락처를 즉시 제거하는 대신 삭제된 것으로 표시하고, 마지막으로 삭제된 연락처를 복원하는 "취소" 옵션 제공

---

## 다음 단계

C 기초를 완료한 것을 축하합니다! [C 고급](../C_Advanced/00_Overview.md)으로 계속하여 시스템 프로그래밍, 고급 데이터 구조 등을 배워보세요.
