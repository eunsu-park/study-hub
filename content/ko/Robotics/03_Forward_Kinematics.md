...
1. 운동 체인(kinematic chain)을 설명하고, 회전 관절(revolute joint)과 직선 관절(prismatic joint)을 구분한다
2. 데나비트-하텐베르크(Denavit-Hartenberg, DH) 규약을 적용하여 각 관절에 좌표 프레임을 체계적으로 할당하고, 4개의 DH 매개변수를 추출한다
3. DH 매개변수로부터 개별 변환 행렬을 구성하고, 이를 곱하여 순운동학을 계산한다
4. 2-링크 평면 로봇, 3-자유도 공간 로봇, 6-자유도 매니퓰레이터 등 주요 로봇 구성에 대한 순운동학을 계산한다
5. 관절 구성을 탐색하여 매니퓰레이터의 작업 공간(workspace)을 분석한다
6. 파이썬으로 범용 FK 솔버를 구현한다

---

## 왜 중요한가

순운동학(Forward Kinematics)은 로보틱스에서 가장 근본적인 질문에 답한다: **"관절을 이 각도로 설정하면 말단 장치(end-effector)는 어디에 위치하는가?"** 로봇이 움직일 때마다 컨트롤러는 실제 도구 위치를 추적하기 위해 FK를 초당 수천 번 계산한다. FK가 없으면 모터들이 맹목적으로 회전할 뿐이지만, FK가 있으면 로봇은 자신의 기하학적 형태를 파악할 수 있다.
...
- **개방 체인(open chain, serial)**: 각 링크가 최대 두 개의 다른 링크에 연결된다. 베이스는 고정되고 말단 장치는 자유롭다. (예: 산업용 로봇 팔)
...
$n$-자유도 직렬 매니퓰레이터에서:
- **링크**: $0$ (베이스/지면)에서 $n$ (말단 장치 링크)까지 번호를 매긴다
...
다음 다이어그램은 3-관절(RRR) 팔에서의 DH 프레임 할당을 보여준다. 각 프레임 $\{i\}$는 $z_i$가 관절 $i+1$의 축과 정렬되고, $x_i$가 $z_{i-1}$에서 $z_i$까지의 공통 법선을 따르며, 원점이 이들의 교점에 위치하도록 배치된다.

```
        z0 (joint 1 axis)          z1 (joint 2 axis)
        ^                          ^
        |                          |
        |  x1 (common normal)      |   x2 (common normal)
        | ------>                   | ------->
   {0}  O============= {1}  O================ {2}  O----> x3
        |    link 1 (a1)     |      link 2 (a2)     |
        |                    |                       |
   ...
   Legend:
   O          Frame origin (at intersection of x_i and z_i)
   ======     Link (rigid body)
   z_{i-1}    Axis of joint i (rotation axis for revolute)
   x_i        Common normal from z_{i-1} to z_i
...
> **규약 참고: 고전 DH vs 수정 DH.** 이 레슨에서는 **고전(표준) DH 규약**(Spong, Siciliano, Paul)을 사용하며, 변환 순서는 $Rot_z(\theta_i) \cdot Trans_z(d_i) \cdot Trans_x(a_i) \cdot Rot_x(\alpha_i)$이고 프레임 $\{i\}$는 링크 $i$의 원위(distal) 끝에 부착된다. 대안적인 **수정 DH 규약**(Craig)은 매개변수 할당을 역전시킨다: $a_{i-1}$, $\alpha_{i-1}$, $d_i$, $\theta_i$이며, 프레임 $\{i\}$는 링크 $i$의 근위(proximal) 끝에 배치된다. 변환 순서는 $Rot_x(\alpha_{i-1}) \cdot Trans_x(a_{i-1}) \cdot Rot_z(\theta_i) \cdot Trans_z(d_i)$가 된다. 두 규약 모두 일관되게 적용하면 동일한 말단 장치 FK를 산출하지만, 한 규약의 DH 매개변수 표를 다른 규약의 변환 공식에 사용할 수는 없다. 참고 문헌의 DH 표를 재사용하기 전에 항상 어떤 규약을 사용하는지 확인해야 한다.
...
$$\phi = \theta_1 + \theta_2 \quad \text{(말단 장치 방위각)}$$
...
로봇의 **작업 공간(workspace)**은 말단 장치가 도달할 수 있는 모든 위치의 집합이다. 관절 구성을 탐색하여 이를 시각화할 수 있다.

### 작업 공간의 종류

- **도달 가능 작업 공간(Reachable workspace)**: 말단 장치 끝이 *하나 이상의* 방위로 도달할 수 있는 모든 점
...
- **작업 공간(workspace)**은 링크 길이, 관절 유형, 관절 한계에 의해 결정되는 말단 장치의 모든 도달 가능 위치의 집합이다
...
2. DH 매개변수 표를 작성한다
3. FK를 기호적으로 계산한다 (말단 장치 $x$, $y$, $\phi$)
...
1. 이 로봇의 FK를 구현한다
2. $\theta_1 = 45°$, $\theta_2 = -30°$, $d_3 = 0.15$ m, $\theta_4 = 60°$에 대한 말단 장치 포즈를 계산한다
