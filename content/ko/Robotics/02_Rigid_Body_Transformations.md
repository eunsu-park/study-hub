...
모든 로봇 시스템은 겉보기에 단순해 보이는 질문에 답해야 한다: "이것은 어디에 있고, 어느 방향을 향하고 있는가?" 관절이든, 말단 장치든, 카메라든, 장애물이든 간에, 우리는 그것의 **위치(position)**와 **방향(orientation)** — 통칭해서 **자세(pose)**라 함 — 을 기술하는 정밀한 수학 언어가 필요하다.
...
로봇공학에서 우리는 **좌표계(coordinate frames)** (오른손 직교 좌표계)를 물체에 부착한다: 세계, 로봇 베이스, 각 링크, 말단 장치, 센서, 장애물.
...
카메라가 로봇 말단 장치에 장착되어 있다. 다음이 주어졌을 때:
- ${}^{0}_{ee}T$: 베이스 좌표계에서 말단 장치 (z축 기준 45도 회전, 이동 $(0.5, 0.3, 0.8)$)
- ${}^{ee}_{cam}T$: 말단 장치 좌표계에서 카메라 (x축 기준 180도 회전, 이동 $(0, 0, 0.1)$)
