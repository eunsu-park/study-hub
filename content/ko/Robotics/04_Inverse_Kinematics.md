...
1. 역운동학(IK) 문제를 형식화하고, 이것이 순운동학보다 근본적으로 어려운 이유를 설명한다
2. 기하학적 방법과 대수적 방법을 사용하여 단순한 로봇(2-링크 평면, 3-자유도)의 IK를 해석적으로 푼다
3. 수치적 IK 방법을 적용한다: 뉴턴-랩슨(Newton-Raphson), 야코비안 유사역행렬(Jacobian pseudo-inverse), 감쇠 최소 제곱법(Levenberg-Marquardt)
4. 다중 해, 작업 공간 경계, 도달 불가 목표 지점을 파악하고 처리한다
5. 특이점(singularity)과 IK 해에 미치는 영향을 설명하고, 경계 특이점과 내부 특이점을 구별한다
6. 6 자유도 이상의 로봇을 위한 여유 자유도 해결(redundancy resolution) 전략을 소개한다

---

## 왜 중요한가

순운동학이 "관절 각도가 주어졌을 때 말단 장치는 어디에 있는가?"를 묻는다면, 역운동학은 그 *반대*를 묻는다: "원하는 말단 장치 포즈가 주어졌을 때, 어떤 관절 각도가 이를 구현하는가?" 이것은 모든 실제 로봇 작업이 궁극적으로 귀결되는 질문이다. 외과의사가 "메스를 왼쪽으로 2mm 이동시켜"라고 명령하거나 픽-앤-플레이스(pick-and-place) 시스템이 선반 위의 상자에 도달해야 할 때, 컨트롤러는 실시간으로 IK를 풀어야 한다.
...
원하는 말단 장치 포즈 $T_d \in SE(3)$ (4x4 동차 변환 행렬)가 주어졌을 때, 다음을 만족하는 관절 구성 $\mathbf{q} = (q_1, q_2, \ldots, q_n)^T$를 찾는다:
...
```
  ...
  Left figure:  theta2 > 0 (elbow up)
  Right figure: theta2 < 0 (elbow down)
...
여기서 $\mathbf{p}_d$는 원하는 말단 장치 위치, ${}^{0}R_d$는 원하는 방위각, $d_6$는 손목에서 도구까지의 거리다.
...
모든 수치적 IK 방법은 **야코비안(Jacobian)** (Lesson 5에서 자세히 다룸)을 기반으로 한다. 야코비안 $J(\mathbf{q})$는 관절 각도의 미소 변화를 말단 장치 포즈의 미소 변화와 연관시킨다:
...
여기서 $\mathbf{f}(\mathbf{q}_k)$는 FK로부터 구한 현재 말단 장치 포즈다.
...
$m > n$ (과결정(over-determined) — 관절보다 작업 차원이 더 많은 경우):
...
특이점에서:
- 일부 말단 장치 속도가 **불가능**해진다 (로봇이 하나 이상의 운동 자유도를 잃음)
...
### IK 해 선택(IK Solution Selection)

다중 IK 해가 존재할 때(예: 구형 손목을 가진 6-자유도 로봇에서 최대 16개), 컨트롤러는 최선의 해를 선택해야 한다. 실용적인 선택 알고리즘은 다음과 같다:

1. **모든 실행 가능한 해를 계산한다**: 해석적 IK를 사용하거나 (수치적 IK의 경우 여러 초기 추정값을 샘플링)
2. **관절 한계 필터링**: $q_i \notin [q_{i,min}, q_{i,max}]$인 해를 폐기한다
3. **자기 충돌 필터링**: 링크 간 빠른 기하학적 충돌 검사를 수행한다
4. **나머지 후보에 가중 기준으로 점수를 부여한다**:
   - **현재 구성으로부터의 거리**: $w_1 \|\mathbf{q}_{candidate} - \mathbf{q}_{current}\|$ — 부드러운 전환을 위해 작은 관절 운동을 선호
   - **관절 한계 근접도**: $w_2 \sum_i \frac{1}{(q_i - q_{i,min})(q_{i,max} - q_i)}$ — 관절 한계 근처의 구성에 불이익
   - **조작 가능도**: $w_3 / \sqrt{\det(J J^T)}$ — 특이점 근처의 구성에 불이익
5. **총점이 가장 낮은 후보를 선택한다**

실시간 응용에서는 현재 구성으로부터의 거리 기준이 지배적(가중치 $w_1$이 가장 큼)이어서, 로봇이 구성 간 최단 경로를 취하고 IK 분기 간 급격한 점프를 피하도록 보장한다.
...
6 자유도(6-DOF 작업 기준) 이상의 관절을 가진 로봇은 **운동학적 여유 자유도(kinematic redundancy)**를 가진다 — 무한히 많은 관절 구성이 동일한 말단 장치 포즈를 실현한다.

### 영공간(Null Space)

유사역행렬 해 $\delta \mathbf{q} = J^+ \delta \mathbf{x}$는 최소 노름 해다. 말단 장치에 영향을 주지 않으면서 $J$의 **영공간(null space)**에 있는 어떤 벡터도 추가할 수 있다:

$$\delta \mathbf{q} = J^+ \delta \mathbf{x} + (I - J^+ J) \mathbf{z}$$

여기서 $(I - J^+ J)$는 **영공간 투영 행렬(null-space projector)**이고 $\mathbf{z}$는 임의의 벡터다.

영공간 성분 $(I - J^+ J)\mathbf{z}$는 말단 장치를 움직이지 않고 관절을 움직인다 — 이를 **자기 운동(self-motion)**이라 한다.
...
- **역운동학(Inverse kinematics)**은 원하는 말단 장치 포즈에 대한 관절 각도를 구한다 — 로봇 제어의 근본적인 문제
...
3. 특이 구성 $q = (45°, 0°)$에서, 어떤 말단 장치 속도가 불가능한가?
...
3. 원하는 말단 장치 포즈 $T_d$ (직접 선택)에 대해 손목 중심을 계산하고 $\theta_1$을 기하학적으로 구한다
