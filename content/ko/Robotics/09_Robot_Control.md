...
### 1.3 안티 와인드업(Anti-Windup) 전략

액추에이터가 포화(명령 토크가 모터 용량을 초과)되면 적분 항이 계속 오차를 누적하는 현상이 발생하는데, 이를 **적분기 와인드업(integrator windup)**이라 한다. 포화가 끝난 후 부풀어진 적분값은 큰 오버슈트와 진동을 유발한다. 두 가지 일반적인 안티 와인드업 전략이 있다:

**역계산(Back-calculation)**: 액추에이터가 포화될 때, 명령 토크와 실제(포화된) 토크의 차이를 계산하여 적분 누적기를 줄이도록 피드백한다:

$$\dot{e}_{I,i} = e_i + \frac{1}{T_{t,i}}(\tau_{sat,i} - \tau_{cmd,i})$$

여기서 $T_{t,i}$는 추적 시정수(보통 $T_{t,i} \approx \sqrt{T_i / T_d}$, $T_i = K_p/K_i$, $T_d = K_d/K_p$)이다.

**클램핑(Clamping)**: 출력이 포화되고 오차가 적분값과 같은 부호일 때 단순히 적분 누적을 멈춘다:

$$\text{If } |\tau_{cmd}| > \tau_{max} \text{ and } \text{sgn}(e_i) = \text{sgn}(e_{I,i}): \quad \dot{e}_{I,i} = 0$$

실제로 클램핑은 구현이 더 간단하고 대부분의 로봇 응용에 충분하다. 역계산은 더 부드러운 과도 응답을 제공하며 고성능 시스템에서 선호된다.

> **참고**: 제약 조건 하에서의 더 진보된 궤적 추종을 위해, **모델 예측 제어(MPC, Model Predictive Control)**는 액추에이터 한계, 관절 제약, 장애물 회피를 고려한 유한 지평 비용 함수를 최적화하여 포화를 사후 처리가 아닌 본질적으로 다룬다. 계산 능력의 향상에 따라 MPC는 로봇 매니퓰레이터에서 점차 실용적이 되고 있다.
...
다음 블록 다이어그램은 계산 토크 피드백 루프를 보여준다:
