...
1. SLAM 문제를 로봇 자세(pose)와 지도의 결합 추정(joint estimation)으로 공식화한다
2. 상태 확장(state augmentation) 및 랜드마크 관리를 활용한 EKF-SLAM을 구현한다
3. 그래프 기반 SLAM(graph-based SLAM)과 그 최적화 공식을 이해한다
4. 파티클 필터 SLAM인 FastSLAM과 그 장점을 설명한다
5. 시각적 SLAM(visual SLAM) 접근법을 비교한다: 특징 기반(feature-based, ORB-SLAM) vs. 직접 방식(direct methods)
6. 루프 폐쇄 감지(loop closure detection)와 지도 일관성에서의 핵심 역할을 설명한다

---

## 왜 중요한가

이전 레슨에서는 알려진 센서 모델과 랜드마크 지도가 주어진 상황에서 로봇의 상태를 추정하는 방법을 배웠다. 그런데 로봇이 지도를 갖고 있지 않다면 어떨까? 이것이 새로운 환경을 탐색하는 대부분의 자율 시스템이 마주하는 현실이다. 낯선 건물 안의 배달 로봇, 미지의 지형을 탐사하는 화성 탐사차, 재난 현장을 촬영하는 드론 등이 이에 해당한다. 로봇은 환경의 지도를 구축하는 동시에 그 지도 안에서 자신의 위치를 파악해야 한다.

이것이 바로 SLAM을 그토록 도전적이고 매력적으로 만드는 **닭이 먼저냐 달걀이 먼저냐의 문제**다. 정확한 위치 추정에는 좋은 지도가 필요하고, 좋은 지도를 만들려면 정확한 위치 추정이 필요하다. SLAM은 "모바일 로봇공학의 성배(holy grail)"라고 불리며, 이 분야에서 가장 활발히 연구되는 주제 중 하나다. 지난 30년에 걸쳐 실용적인 SLAM 솔루션은 이론적 호기심에서 벗어나 자율주행 자동차, 로봇 청소기, 증강현실 헤드셋을 구동하는 생산 준비 완료 시스템으로 발전했다.

> **비유**: SLAM은 지도 없이 낯선 도시에 던져지는 것과 같다. 지도를 그리는 동시에 그 지도 위에서 자신의 위치를 파악해야 한다. 처음에는 모든 것이 불확실하다. 하지만 걸어 다니다 보면 랜드마크("저 분수 또 나왔네!")를 알아보게 되고, 매번 인식할 때마다 자신의 위치와 지도 모두가 정교해진다. 큰 루프를 돌아 출발점을 알아보는 순간, 그동안 쌓인 작은 오차들을 한꺼번에 수정할 수 있다. 이것이 루프 폐쇄(loop closure)이며, 전역적으로 일관된 지도를 만드는 핵심이다.
...
1. **높은 차원성(High dimensionality)**: 상태에는 로봇 자세(3~6 자유도)와 모든 랜드마크 위치($2N$ 또는 $3N$ 차원)가 포함된다. 수천 개의 랜드마크가 있을 경우 상태 공간이 매우 거대해진다.
2. **데이터 연관(Data association)**: 로봇은 어느 관측이 어느 랜드마크에 해당하는지 파악해야 한다. 잘못된 연관은 지도를 치명적으로 손상시킨다.
3. **루프 폐쇄(Loop closure)**: 작은 오도메트리(odometry) 오차가 큰 드리프트로 누적된다. 로봇이 이전에 방문한 위치를 재방문할 때, 그 장소를 인식하고 전체 궤적에 걸쳐 누적된 오차를 수정해야 한다.
...
EKF-SLAM은 로봇 자세와 모든 랜드마크 위치를 모두 포함하는 결합 상태 벡터를 유지한다:
...
비대각 블록 $P_{m_i m_j}$는 **랜드마크 간 상관관계(correlations between landmarks)**를 포착한다. 이 상관관계가 SLAM의 핵심 통찰이다. 로봇이 하나의 랜드마크를 관측하면, 갱신이 상관관계를 통해 전파되어 다른 랜드마크의 추정값도 개선된다.
...
랜드마크는 움직이지 않으므로, 예측 단계에서 랜드마크의 상태와 공분산 행/열은 변하지 않는다. 로봇의 행과 로봇-랜드마크 간 교차 공분산(cross-covariance)만 갱신된다.
...
- **이차 복잡도(Quadratic complexity)**: 공분산 행렬의 크기가 $(3+2N) \times (3+2N)$이므로, 각 갱신에 $O(N^2)$의 비용이 든다. 수천 개의 랜드마크가 있으면 이는 감당하기 어려워진다.
- **선형화 오차(Linearization errors)**: 모든 EKF와 마찬가지로 1차 근사가 비일관성(inconsistency)을 초래할 수 있으며, 특히 긴 궤적 이후에 두드러진다.
- **데이터 연관(Data association)**: 관측을 랜드마크에 올바르게 매칭하는 것이 중요하고 어렵다. 특히 반복적인 특징이 많은 환경에서 더욱 그렇다. 매칭에는 유클리드 거리(Euclidean distance)보다 마할라노비스 거리(Mahalanobis distance, $d_M = \sqrt{(\mathbf{z} - \hat{\mathbf{z}})^T S^{-1} (\mathbf{z} - \hat{\mathbf{z}})}$)가 선호되는데, 이는 혁신(innovation)의 공분산 구조를 고려하기 때문이다. 예를 들어, 거리 센서 노이즈가 0.1 m일 때의 1 m 오차는 10 m 노이즈일 때보다 훨씬 중요하다. 유클리드 거리는 모든 차원을 동등하게 취급하고 불확실성을 무시하므로, 센서 노이즈가 비등방적(anisotropic)일 때 잘못된 연관을 초래한다.

---

## 그래프 기반 SLAM(Graph-Based SLAM)

### 3.1 그래프 최적화로서의 SLAM

그래프 기반 SLAM은 문제를 **인수 그래프(factor graph)** 최적화로 재구성한다:
...
각 엣지는 불확실성과 함께 측정값을 인코딩한다. 최적화는 모든 제약을 동시에 가장 잘 만족하는 노드 위치를 찾는다.

### 3.2 자세 그래프 최적화(Pose Graph Optimization)

**자세 그래프 SLAM(pose graph SLAM)**에서는 랜드마크를 제거하고 로봇 자세만 노드로 사용한다. 엣지는 다음으로부터 생성된다:
...
여기서 $\mathbf{z}_{ij}$는 측정된 상대 변환, $h(\mathbf{x}_i, \mathbf{x}_j)$는 예상 상대 변환, $\Omega_{ij}$는 정보 행렬(information matrix, 공분산의 역행렬)이다.

### 3.3 가우스-뉴턴 최적화(Gauss-Newton Optimization)

자세 그래프 SLAM의 표준 솔버는 **가우스-뉴턴(Gauss-Newton)** 또는 **레벤버그-마르콰르트(Levenberg-Marquardt)**이다:

$$\Delta\mathbf{x}^* = -(J^T \Omega J)^{-1} J^T \Omega \mathbf{e}$$

여기서 $J$는 오차 함수의 야코비안이고, $\mathbf{e}$는 오차 벡터이다.

행렬 $H = J^T \Omega J$ (헤시안 근사)는 각 엣지가 두 개의 노드만 관련되므로 **희소(sparse)**하다. 이 희소성 덕분에 희소 선형 대수(희소 촐레스키 분해)를 사용한 효율적인 풀이가 가능하다.
...
## 파티클 필터 SLAM — FastSLAM

### 4.1 핵심 통찰: 조건부 독립성(Conditional Independence)

FastSLAM은 **로봇 궤적이 주어지면 랜드마크 추정이 독립적**이라는 사실을 활용한다. 이를 통해 다음과 같이 인수분해할 수 있다:
...
실제로 FastSLAM은 랜드마크를 EKF를 통해 분석적으로 추정하므로, 결합 상태에 대한 순수 파티클 필터보다 훨씬 적은 파티클이 필요하다.
...
**특징 기반 방법(feature-based methods)**은 프레임 간에 시각적 특징(모서리, 점)을 추출하고 추적한다:
...
**ORB-SLAM**(및 후속작 ORB-SLAM3)은 가장 잘 알려진 특징 기반 시각적 SLAM 시스템이다:
...
**직접 방식(direct methods)** (예: LSD-SLAM, DSO)은 특징을 추출하는 대신 픽셀 밝기값을 직접 사용한다:

$$E_{photo} = \sum_{\mathbf{p} \in \mathcal{P}} \| I_{ref}(\mathbf{p}) - I_{cur}(\pi(T \cdot \pi^{-1}(\mathbf{p}, d_\mathbf{p}))) \|^2$$

여기서 광도 오차(photometric error)는 추정된 카메라 움직임 $T$와 깊이 지도(depth map) $d$ 아래에서 참조 영상 $I_{ref}$와 현재 영상 $I_{cur}$가 얼마나 잘 일치하는지 측정한다.
...
루프 폐쇄가 없으면 오도메트리 드리프트(drift)가 무한정 누적된다. 100미터를 이동한 후 로봇의 위치 오차가 1~5미터에 달할 수 있다. 로봇이 이전에 방문한 위치로 돌아올 때, 이 **재방문**을 감지하면 전체 궤적을 수정할 수 있다.

### 6.2 장소 인식(Place Recognition)

루프 폐쇄 감지는 근본적으로 **장소 인식(place recognition)** 문제다. 현재 센서 관측을 기반으로, 이 장소를 전에 본 적이 있는가?

**시각적 장소 인식(visual place recognition)** 접근법:

1. **시각 단어 사전(Bag of Visual Words, BoVW)**: 특징 기술자를 시각 어휘(visual vocabulary)로 클러스터링한다. 각 이미지를 시각 단어의 히스토그램으로 표현한다. TF-IDF 유사도를 사용하여 히스토그램을 비교한다.

2. **신경망 임베딩(Neural network embeddings)**: CNN을 사용하여 각 이미지의 전역 기술자(global descriptor)를 계산한다. 코사인 유사도로 기술자를 비교한다. 현대 방법(NetVLAD, SuperGlue)은 인상적인 성능을 달성한다.

3. **점군 기술자(Point cloud descriptors)**: LiDAR의 경우, 점군의 전역 기술자를 계산한다 (Scan Context, PointNetVLAD).

### 6.3 루프 폐쇄 검증(Loop Closure Verification)

잘못된 루프 폐쇄(서로 다른 두 장소를 잘못 매칭)는 치명적이다. 지도를 돌이킬 수 없이 손상시킨다. 따라서 엄격한 검증이 필수적이다:

1. **기하학적 검증(Geometric verification)**: 후보 매칭을 찾은 후, 특징 간 기하학적 관계가 일관성이 있는지 검증한다 (기본 행렬(fundamental matrix) 계산, 인라이어(inlier) 비율 확인).
2. **시간적 일관성(Temporal consistency)**: 수락하기 전에 여러 개의 연속적인 루프 폐쇄 후보가 필요하다.
...
## SLAM 프론트엔드와 백엔드(SLAM Frontends and Backends)
...
1. **EKF-SLAM 시뮬레이션**: 위의 EKF-SLAM 클래스를 구현한다. 20x20 m 환경에 10개의 랜드마크를 배치한다. 로봇이 정사각형 경로로 주행하면서 8 m 범위 내의 랜드마크를 관측하도록 시뮬레이션한다. 시간에 따른 추정 및 실제 랜드마크 위치를 그래프로 표시한다. 로봇이 랜드마크를 재방문할수록 각 랜드마크의 불확실성 타원이 어떻게 변하는지 확인한다.

2. **자세 그래프 최적화**: 루프(예: 정사각형 궤적)를 도는 로봇의 자세 그래프를 만든다. 작은 가우시안 노이즈가 있는 오도메트리 엣지를 추가한다. 그런 다음 마지막 자세와 첫 번째 자세를 연결하는 루프 폐쇄 엣지를 추가한다. 최적화기를 실행하고 최적화 전후의 궤적을 비교한다. 루프 폐쇄 엣지가 누적된 드리프트를 어떻게 재분배하는지 확인한다.

3. **데이터 연관 도전**: EKF-SLAM을 수정하여 최근접 이웃 데이터 연관(각 관측을 가장 가까운 기존 랜드마크에 매칭)을 사용한다. 두 랜드마크가 서로 가까운 시나리오를 만든다. 잘못된 데이터 연관이 어떻게 지도를 손상시킬 수 있는지 보여준다. 그런 다음 간단한 마할라노비스 거리(Mahalanobis distance) 게이팅 전략을 구현하고 강건성이 향상됨을 확인한다.

4. **시각적 특징 매칭**: Python(OpenCV)을 사용하여 약간 다른 시점에서 찍은 두 이미지에서 ORB 특징을 감지한다. Lowe의 비율 테스트를 적용한 브루트-포스 매칭으로 특징을 매칭한다. 필수 행렬(essential matrix)을 추정하고 상대 카메라 자세를 복원한다. 매칭된 점을 삼각측량하여 희소 3D 재구성을 만든다.

5. **루프 폐쇄 효과**: 로봇이 긴 복도를 이동하다(50개 자세) 시작점으로 돌아오는 자세 그래프 SLAM 시나리오를 만든다. 먼저 루프 폐쇄 없이 최적화를 실행한다. 그런 다음 루프 폐쇄 엣지를 추가하고 재최적화한다. (a) 최종 위치 오차와 (b) 전체 궤적 오차 측면에서 개선 정도를 정량화한다. 루프 폐쇄 측정의 품질이 결과에 어떻게 영향을 미치는지 확인한다.
