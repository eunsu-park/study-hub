...
동역학은 기하학과 제어 사이의 다리이다. 로봇 팔이 관절 1을 빠르게 흔들면 원심력 효과가 관절 2를 가속시킨다(결합 효과). 팔이 수평으로 펼쳐지면 중력이 모터가 끊임없이 저항해야 하는 토크를 만들어낸다. 팔이 빠르게 움직이면 속도에 의존하는 힘(코리올리 효과)이 예기치 않은 힘을 만들어 말단 장치(end-effector)를 경로에서 벗어나게 한다. 이러한 효과들을 이해하고 정확하게 계산하는 것이 고성능 모션 제어의 핵심이다.
...
기구학이 제공하는 것:
- **순기구학(FK)**: 관절 각도 $\to$ 말단 장치 포즈
- **역기구학(IK)**: 원하는 포즈 $\to$ 관절 각도
- **야코비안**: 관절 속도 $\to$ 말단 장치 속도

하지만 다음에는 답할 수 없다: **원하는 관절 가속도를 달성하려면 모터에 어떤 토크를 가해야 하는가?**
...
| 코리올리/원심력 | $C(\mathbf{q}, \dot{\mathbf{q}})$ | $n \times n$ | 속도 의존 힘 (관절 간 결합) |
| 중력 | $\mathbf{g}(\mathbf{q})$ | $n \times 1$ | 각 관절의 중력 토크 |
| 관절 토크 | $\boldsymbol{\tau}$ | $n \times 1$ | 가해진 모터 토크 (제어 입력) |
...
이것은 최소 작용의 원리(principle of least action)로부터 유도된다. 로봇의 궤적은 라그랑지안의 시간 적분을 최소화한다. 이 프레임워크는 구속력(constraint force)을 계산할 필요 없이 어떤 좌표계(관절 각도, 작업 공간 좌표 등)에서도 작동하기 때문에 매우 강력하다.
...
야코비안을 사용하면 $\mathbf{v}_{c_i}$와 $\boldsymbol{\omega}_i$를 관절 속도로 표현할 수 있다.
...
- 링크 1: 길이 $l_1$, 질량 $m_1$, 관절 1에서 거리 $l_{c_1}$에 질량 중심, 관성 $I_1$
- 링크 2: 길이 $l_2$, 질량 $m_2$, 관절 2에서 거리 $l_{c_2}$에 질량 중심, 관성 $I_2$
- 중력 $g$는 $-y$ 방향으로 작용한다
...
첫 번째 행에는 코리올리 항($-2h\dot{\theta}_1\dot{\theta}_2$)과 원심력 항($-h\dot{\theta}_2^2$)이 포함된다. 두 번째 행은 관절 1이 관절 2에 미치는 원심력 항이다.
...
$M$은 로봇이 움직임에 따라 변한다. 일부 형태 구성에서는 특정 관절가 다른 형태 구성보다 더 많은 관성을 경험한다.

### 4. 유계성(Bounded)

$\lambda_{min}(M) \leq \frac{\dot{\mathbf{q}}^T M \dot{\mathbf{q}}}{\|\dot{\mathbf{q}}\|^2} \leq \lambda_{max}(M)$

$M$의 고유값(eigenvalue)은 유계인 관절 형태 구성에 대해 위아래로 유계이다. 이것은 제어 설계에 중요하다.
...
**원심력 효과**: 관절 $i$가 회전할 때 링크 $i+1$ (및 그 이후)에 대한 원운동으로 인한 "바깥쪽" 유사 힘. $\dot{q}_i^2$에 비례한다.

**코리올리 효과**: 두 관절 속도 사이의 교차 결합. 관절 $i$와 $j$가 모두 움직이면 다른 관절에 $\dot{q}_i \dot{q}_j$에 비례하는 힘이 발생한다.
...
## 관절 마찰 모델링(Joint Friction Modeling)

### 쿨롱 마찰과 점성 마찰

표준 매니퓰레이터 동역학 방정식 $M(\mathbf{q})\ddot{\mathbf{q}} + C(\mathbf{q}, \dot{\mathbf{q}})\dot{\mathbf{q}} + \mathbf{g}(\mathbf{q}) = \boldsymbol{\tau}$은 무마찰 관절을 가정한다. 실제로 관절 마찰은 상당하며(종종 정격 토크의 5-15%) 정밀한 제어를 위해 반드시 모델링해야 한다.

가장 일반적인 마찰 모델은 **쿨롱 마찰(Coulomb friction)**(일정한 크기, 운동에 반대)과 **점성 마찰(viscous friction)**(속도에 비례)을 결합한다:

$$\tau_{friction,i} = b_i \dot{q}_i + \mu_{c,i} \, \text{sgn}(\dot{q}_i)$$

여기서:
- $b_i$는 **점성 마찰 계수** [N$\cdot$m$\cdot$s/rad] — 고속에서 지배적
- $\mu_{c,i}$는 **쿨롱 마찰 계수** [N$\cdot$m] — 운동에 반대하는 상수 토크
- $\text{sgn}(\dot{q}_i)$는 부호 함수

마찰을 포함한 전체 동역학은 다음과 같다:

$$M(\mathbf{q})\ddot{\mathbf{q}} + C(\mathbf{q}, \dot{\mathbf{q}})\dot{\mathbf{q}} + \mathbf{g}(\mathbf{q}) + \boldsymbol{\tau}_{friction} = \boldsymbol{\tau}$$

**제어에 대한 영향**: 모델링되지 않은 마찰은 추종 성능을 저하시키며, 특히 저속에서 쿨롱 마찰이 **점착-미끄럼 거동(stick-slip behavior)**(관절이 점착과 미끄러짐을 번갈아 반복)을 유발한다. 계산 토크 제어에서 마찰 순방향 보상 항 $\hat{\boldsymbol{\tau}}_{friction}$을 제어 법칙에 추가하면 저속 추종 정확도가 크게 향상된다. 고정밀 응용에서는 정지 마찰 동역학과 사전 미끄럼 변위를 포착하는 **LuGre 모델** 같은 더 정교한 모델이 사용된다.
...
2. **역방향 패스(tip to base)**: 뉴턴과 오일러의 방정식을 사용하여 각 관절의 힘과 토크를 계산
...
def newton_euler_2link(arm, q, q_dot, q_ddot):
    ...
    # 평면 케이스 단순화: tau2는 관절 2에 대한 모멘트를 포함
...
- **로봇 동역학**은 관절 토크와 운동을 연결한다: $M(\mathbf{q})\ddot{\mathbf{q}} + C(\mathbf{q}, \dot{\mathbf{q}})\dot{\mathbf{q}} + \mathbf{g}(\mathbf{q}) = \boldsymbol{\tau}$
...
- **뉴턴-오일러** 재귀적 알고리즘은 $O(n)$ 시간에 역동역학을 계산하여 많은 관절를 가진 실시간 응용에 필수적이다
...
2링크 팔을 이용하여:
1. $q = (45°, 60°)$이고 $\dot{q} = (2, 0)$ (관절 1만 움직임)일 때 관절 2에 대한 코리올리 토크를 계산하라
2. 관절 1이 움직이면 관절 2에 토크가 생기는 이유를 물리적으로 설명하라
...
2. PD + 중력 보상
3. 계산 토크 제어
각 경우에 추종 오차와 관절 토크를 플로팅하라. 정상 상태 오차와 과도 응답을 파악하라.
