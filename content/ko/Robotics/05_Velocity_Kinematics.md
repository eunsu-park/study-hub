...
[← 이전: 역운동학](04_Inverse_Kinematics.md) | [다음: 로봇 동역학 →](06_Robot_Dynamics.md)

## 학습 목표

1. 강체의 각속도와 선속도를 정의하고 서로 다른 기준 좌표계에서 표현한다
2. 매니퓰레이터 야코비안(Jacobian)을 유도하고, 이것이 관절 공간과 작업 공간 사이의 기본 속도 매핑임을 이해한다
...
5. 힘/토크 매핑(정역학 쌍대성(statics duality))을 적용하여 말단 장치(end-effector)의 힘과 관절 토크를 연결한다
...
야코비안(Jacobian)은 로보틱스에서 가장 중요한 수학적 대상이라 해도 과언이 아니다. 야코비안은 어디에나 등장한다. 속도 제어(관절 속도를 말단 장치 속도로 변환), 힘 제어(말단 장치 힘을 관절 토크로 변환), 특이점 분석(로봇이 민첩성을 잃는 위치 판단), 궤적 추종(관절 가속도 계산), 그리고 동역학(운동 방정식 구성)에 이르기까지 야코비안이 핵심 역할을 한다.
...
> **비유**: 야코비안은 기어비(gear ratio)와 같다. 기어비가 엔진 RPM을 바퀴 속도로 변환하듯(그리고 그 비율이 기어 선택에 따라 달라지듯), 야코비안은 관절 속도를 말단 장치 속도로 변환한다(그리고 그 "비율"은 로봇의 형태 구성에 따라 달라진다).
...
**매니퓰레이터 야코비안** $J(\mathbf{q})$는 관절 속도를 말단 장치 속도로 매핑하는 행렬이다.

$$\begin{bmatrix} \mathbf{v} \\ \boldsymbol{\omega} \end{bmatrix} = J(\mathbf{q}) \, \dot{\mathbf{q}}$$

여기서:
- $\mathbf{v} \in \mathbb{R}^3$은 말단 장치의 선속도
- $\boldsymbol{\omega} \in \mathbb{R}^3$은 말단 장치의 각속도
- $\dot{\mathbf{q}} \in \mathbb{R}^n$은 관절 속도 벡터
- $J(\mathbf{q}) \in \mathbb{R}^{6 \times n}$ (6자유도 전체 작업 공간의 경우)

### 야코비안 열: 각 관절의 기여

$J$의 $i$번째 열은 $i$번째 관절에서 단위 속도가 발생할 때(나머지 관절는 정지) 말단 장치에 생기는 속도를 나타낸다. $n$자유도 직렬 매니퓰레이터의 경우:

$$J = \begin{bmatrix} J_{v_1} & J_{v_2} & \cdots & J_{v_n} \\ J_{\omega_1} & J_{\omega_2} & \cdots & J_{\omega_n} \end{bmatrix}$$

**회전(revolute)** 관절 $i$의 경우:
...
**직동(prismatic)** 관절 $i$의 경우:
$$J_i = \begin{bmatrix} J_{v_i} \\ J_{\omega_i} \end{bmatrix} = \begin{bmatrix} \hat{z}_{i-1} \\ \mathbf{0} \end{bmatrix}$$

여기서:
- $\hat{z}_{i-1}$은 기저 좌표계(base frame)로 표현된 관절 축(좌표계 $\{i-1\}$의 z축)
- $\mathbf{p}_n$은 기저 좌표계에서 말단 장치의 위치
...
# 검증: 관절 1이 1 rad/s일 때 선속도와 각속도 모두 발생해야 함
...
관절 속도를 **공간 트위스트(spatial twist)**(선속도 + 각속도)로 매핑한다.
...
관절 속도를 **포즈 매개변수화(pose parameterization)의 시간 미분**(예: 위치 + 오일러 각도)으로 매핑한다.
...
- $V \in \mathbb{R}^{n \times n}$: 열 벡터는 **관절 공간의 특이 방향**

**물리적 해석**:
- $\sigma_i$: $i$번째 특이 방향에서의 "이득(gain)" (관절 움직임이 작업 공간 움직임을 얼마나 만들어내는가)
- 특이점은 어떤 $i$에 대해 $\sigma_i = 0$일 때 발생한다
- 특이점 근방: $\sigma_i \approx 0$ (작은 작업 공간 움직임을 위해 큰 관절 움직임이 필요)
...
로보틱스에서 가장 우아한 결과 중 하나: 야코비안 전치(transpose)가 말단 장치 힘을 관절 토크로 매핑한다.

말단 장치가 환경에 렌치(wrench, 힘 + 토크) $\mathbf{F} = (\mathbf{f}, \boldsymbol{\tau}_{ee})^T$를 가하면, 이에 해당하는 관절 토크는:
...
힘 타원체는 제한된 관절 토크($\|\boldsymbol{\tau}\| \leq 1$)로 달성 가능한 말단 장치 힘을 보여준다.
...
**조작 가능도 타원체(manipulability ellipsoid)**는 단위 관절 속도($\|\dot{\mathbf{q}}\| \leq 1$)로 달성 가능한 말단 장치 속도를 시각화한다.

```
  Well-conditioned (theta2 ~ 90 deg)     Near-singular (theta2 ~ 5 deg)

       vy                                      vy
        ^                                       ^
        |    ___                                |
        |   /   \                               |        /
        |  |  o  |   <-- nearly circular        |       / <-- very elongated
        |   \___/        (sigma1 ~ sigma2)      |      /      (sigma1 >> sigma2)
        |                                       |  ___/
   -----+-----------> vx                   -----+/-----------> vx
        |                                       |\___
        |  kappa ~ 1.5                          |     \
  ...
  Left:  Arm in a regular pose. The robot can move roughly
         equally well in all directions. Good dexterity.
  Right: Arm nearly fully extended. Motion along the arm axis
         is easy (long axis), but radial motion is nearly
         impossible (short axis). Near singularity.
```
...
- **회전 관절**의 경우 각 야코비안 열은 $\hat{z} \times (\mathbf{p}_{ee} - \mathbf{p}_{joint})$ (선속도)와 $\hat{z}$ (각속도)를 포함한다
- **직동 관절**의 경우 열은 $\hat{z}$ (선속도)와 $\mathbf{0}$ (각속도)이다
- **기하 야코비안**은 공간 트위스트를 제공하고, **해석적 야코비안**은 오일러 각도 변화율을 제공한다
- **특이점** ($\det(J) = 0$)은 자유도 손실을 나타낸다. 로봇이 특정 방향으로 이동할 수 없는 상태이다
- **정역학 쌍대성**: $\boldsymbol{\tau} = J^T \mathbf{F}$는 말단 장치 힘을 관절 토크로 매핑한다
...
3. 유사역행렬(pseudo-inverse)을 사용하여 손실된 방향으로 말단 장치를 이동시키려 할 때 관절 속도에 어떤 일이 발생하는가?
4. DLS(Damped Least Squares, 감쇠 최소 제곱)를 다양한 $\lambda$ 값으로 반복하고 비교하라

### 연습 3: 힘 매핑

$q = (0°, 90°)$의 2링크 평면 팔 ($l_1 = l_2 = 0.5$ m)이 말단 장치에서 오른쪽(+x 방향)으로 20 N의 힘을 발휘해야 한다.
1. $\tau = J^T F$를 사용하여 필요한 관절 토크를 계산하라
2. 관절 1의 최대 토크가 15 N*m라면, 이 힘을 달성할 수 있는가?
