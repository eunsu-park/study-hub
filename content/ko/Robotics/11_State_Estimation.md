# 상태 추정과 필터링(State Estimation and Filtering)
...
### 6.4 필터 건강 모니터링(Filter Health Monitoring)

실행 중인 필터는 조용히 발산할 수 있다 — 확신에 찬 그러나 잘못된 추정값을 생성하면서. 안전이 중요한 응용에서 필터 건강 모니터링은 필수적이다.

**정규화된 혁신 제곱(NIS, Normalized Innovation Squared)**: 각 측정 갱신에서 혁신(측정 잔차) $\boldsymbol{\nu}_k = \mathbf{z}_k - h(\hat{\mathbf{x}}_k^-)$은 예측된 공분산 $S_k = H P_k^- H^T + R$과 일관성이 있어야 한다. NIS 통계량:

$$\epsilon_k = \boldsymbol{\nu}_k^T S_k^{-1} \boldsymbol{\nu}_k$$

은 필터가 일관적일 때 자유도 $m$(측정 차원)의 $\chi^2$ 분포를 따른다. $\epsilon_k$가 $\chi^2$ 임계값(예: 95번째 백분위수)을 지속적으로 초과하면, 필터가 발산할 가능성이 높다 — 프로세스 노이즈 $Q$가 너무 작거나, 측정 모델이 잘못되었거나, 모델링되지 않은 바이어스가 있을 수 있다.

**실용적인 발산 감지**: 최근 $N$ 스텝에 대한 NIS의 윈도우 평균을 계산한다. $\bar{\epsilon} > \chi^2_{m, 0.95}$가 $N$회 연속 발생하면 발산을 경고한다. 일반적인 복구 조치에는 $P$를 더 큰 값으로 재설정하거나, $Q$를 일시적으로 증가시키거나, 재초기화를 트리거하는 것이 포함된다.

**초기 공분산 $P_0$ 지침**: $P_0$을 너무 작게 설정하면 필터가 잘못된 초기 상태를 과신하여 유효한 측정값을 거부한다. 너무 크게 설정하면 초기 수렴 시간을 낭비한다. 실용적인 규칙: $P_0$의 대각 원소를 각 상태 성분의 최대 예상 초기 오차의 제곱으로 설정한다. 예를 들어, 초기 위치 불확실성이 $\pm 1$ m이면 $P_{0,xx} = 1.0$ m$^2$로 설정한다.

---

## 다중 주기 센서 융합(Multi-Rate Sensor Fusion)
