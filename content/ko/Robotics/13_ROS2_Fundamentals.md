...
로봇에는 많은 좌표 프레임이 있다: 베이스(base), 각 관절(joint), 각 센서, 지도(map), 오도메트리(odometry) 프레임. **TF2**는 모든 프레임 간의 변환 트리를 유지하여, 어떤 노드든 언제든지 임의의 두 프레임 간의 변환을 조회할 수 있도록 한다.
...
**동적 변환(Dynamic transforms)**: 시간에 따라 변하는 관계(예: 관절 각도, 오도메트리). 높은 빈도로 지속적으로 발행한다.
