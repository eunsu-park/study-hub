...
Device plugin은 다음을 하는 Pod(보통 DaemonSet)입니다:

1. `/var/lib/kubelet/device-plugins/`의 Unix 소켓을 통해 kubelet에 **등록**.
2. 로컬 노드의 디바이스를 **발견**(예: `nvidia-smi`를 통한 NVIDIA GPU).
3. 스케줄러가 매치할 수 있는 extended resource(`nvidia.com/gpu: 4`)로 **광고**.
4. 스케줄링 시점에 특정 디바이스 ID를 파드에 **할당**하고, 런타임에 디바이스 파일을 컨테이너에 마운트하라고 지시.
...
스케줄러는 이를 CPU/메모리와 정확히 동일하게 취급 — 용량 있는 노드 찾기, 파드 스케줄, kubelet이 device plugin에게 특정 GPU 할당 요청, 런타임이 `/dev/nvidia0`(또는 어떤 것이든)을 컨테이너에 마운트.
