...
컴포넌트 투어에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 쿠버네티스가 컨트롤 플레인과 데이터 플레인을 분리한 이유, 모든 kubectl 호출이 결국 etcd 쓰기로 귀결되는 과정, 그리고 독립적인 컨트롤러들이 서로 직접 통신하지 않으면서도 협력할 수 있게 만드는 watch/list/reconcile 루프의 원리를 다룹니다.
...
쿠버네티스 클러스터는 "컨테이너를 실행하는 여러 머신의 집합"이 아니라 **단일 분산 데이터베이스(etcd)에 REST 프론트엔드(API 서버)가 붙고, 데이터베이스가 말하는 모습대로 세상을 지속적으로 조정하는 독립적인 에이전트 무리**로 이해하는 것이 가장 정확합니다. 이 관점이 자리 잡으면, 아키텍처의 모든 조각은 임의의 데몬이 아니라 동일한 저장소의 특화된 클라이언트로 보이기 시작합니다. 이 섹션은 전체 시스템을 묶어주는 네 가지 핵심 아이디어를 설명합니다: 워치(watch) 기반 이벤트 모델, etcd의 Raft 로그, 2단계 옵티마이저로서의 스케줄러, 그리고 레벨 트리거(level-triggered) 조정 루프로서의 컨트롤러.

### A. 폴링이 아닌 워치(Watch) — 컨트롤러들이 직접 통신하지 않고 협력하는 방법
...
뒤따르는 워크스루는 이 추상을 실제 산출물로 추적합니다:

- **상위 수준 아키텍처** 다이어그램은 §A·§B의 컴포넌트들을 박스로 보여줍니다.
- **컨트롤 플레인 컴포넌트**(§2)와 **노드 컴포넌트**(§3)는 §A·§D의 워처(watcher)와 리컨실러(reconciler)입니다.
- **API 요청 라이프사이클**(§4)은 kubectl에서 출발해 인증, 어드미션, 검증을 거쳐 §B에서 설명한 선형성 보장 쓰기를 통해 etcd에 도달하는 경로입니다.
- **etcd 데이터 모델**(§5)은 protobuf 인코딩 객체를 담는 계층적 키 레이아웃을 보여줍니다.
- **인증과 인가 흐름**(§7)은 etcd 쓰기 시도 전에 요청이 통과하는 게이트들을 분해합니다.
- **스케줄러 알고리즘**(§8)은 §C의 predicates+priorities 2단계 파이프라인입니다.
- **컨트롤 루프와 조정**(§9)은 §D의 레벨 트리거 알고리즘을 구체적 예제로 마무리합니다.

이론을 한 번 읽고 나면, 레슨의 나머지는 단절된 사실의 나열이 아니라 이 패턴들의 인스턴스로 읽히게 됩니다.
