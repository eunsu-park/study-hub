...
1. **모든 Pod는 고유 IP를 받는다.** raw Docker처럼 포트 매핑 곡예가 없습니다. Pod 내부에서 보는 IP가 다른 모두가 보는 IP입니다.
2. **Pod-Pod 통신은 NAT 없이 동작한다.** 노드 A의 Pod가 노드 B의 Pod와 통신할 때, source = Pod-A-IP, destination = Pod-B-IP인 패킷을 보내면 Pod B는 정확히 그 source를 봅니다. 노드 간 경로에서 source IP 재작성 없음.
3. **Node-Pod 통신은 NAT 없이 동작한다.** 노드 에이전트(kubelet, kube-proxy, 모니터링)는 자신의 노드 위 어떤 Pod든 Pod IP로 도달할 수 있습니다.
