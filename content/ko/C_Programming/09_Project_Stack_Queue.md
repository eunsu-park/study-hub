...
**이전**: [프로젝트 6: 파일 암호화 도구](./08_Project_File_Encryption.md) | **다음**: [프로젝트 8: 해시 테이블](./10_Project_Hash_Table.md)
...
1. LIFO(스택)와 FIFO(큐)의 순서 방식을 구분하고 각각의 실세계 예시를 식별할 수 있다
2. push, pop, peek, isEmpty, isFull 연산을 갖춘 고정 크기 배열 기반 스택을 구현할 수 있다
3. 헤드 삽입(head insertion)을 사용하여 O(1) push와 pop이 가능한 연결 리스트 기반 스택을 구현할 수 있다
4. 모듈러 연산(modular arithmetic)을 통해 배열 슬롯을 재사용하여 공간 낭비를 없애는 원형 큐(circular queue)를 만들 수 있다
5. front와 rear 포인터를 사용하여 O(1) enqueue와 dequeue가 가능한 연결 리스트 기반 큐를 구현할 수 있다
6. 여는 괄호와 닫는 괄호를 짝짓는 괄호 검사 문제를 스택으로 해결할 수 있다
7. 스택을 사용하여 산술 표현식을 평가하는 후위 표기법(postfix notation) 계산기를 만들 수 있다

---

스택과 큐는 가장 단순한 두 가지 추상 자료형(Abstract Data Type)이지만, 어디에서나 등장합니다. 프로그램을 실행하는 함수 호출 스택, 편집기의 실행 취소 버퍼, 운영체제의 인쇄 큐, 그래프 알고리즘의 BFS 탐색 경계까지. C에서 이를 구현하다 보면 자료구조 자체뿐 아니라, 일관된 오류 처리를 갖춘 깔끔하고 재사용 가능한 ADT 인터페이스를 작성하는 훈련도 함께 할 수 있습니다.
...
[프로젝트 8: 해시 테이블](./10_Project_Hash_Table.md) → 해시 테이블을 배워봅시다!
