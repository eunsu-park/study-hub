...
**양자 카운팅 알고리즘(quantum counting algorithm)**(Brassard, Hoyer, Tapp 1998)은 그로버 연산자와 양자 위상 추정([레슨 9](09_Quantum_Fourier_Transform.md) 참조)을 결합하여, 해의 수 $M$을 사전에 알지 못해도 추정할 수 있게 합니다.

2차원 부분 공간에서 그로버 연산자 $G$의 고유값은 $e^{\pm 2i\theta}$이며, 여기서 $\sin^2\theta = M/N$입니다. $G$에 양자 위상 추정을 적용하면 $\theta$의 추정값을 얻고, 이로부터 $M$을 구할 수 있습니다:

$$M = N \sin^2(\theta)$$

이 알고리즘은 상대 정확도 $\epsilon$으로 $M$을 추정하는 데 $O(\sqrt{N/M})$번의 오라클 호출을 사용하며, 이는 그로버 탐색 자체의 질의 복잡도와 동일합니다. 이는 해의 수를 알 수 없고 그 수가 최적 반복 횟수에 직접 영향을 미치는 응용에서 유용합니다.
