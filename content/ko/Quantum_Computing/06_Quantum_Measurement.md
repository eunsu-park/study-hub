...
1. 사영 연산자(projection operator)를 사용하여 사영 측정(폰 노이만 측정(von Neumann measurement))을 형식화하고 측정 공준을 설명할 수 있다
2. 임의의 기저에서 측정을 수행하고 측정 후 상태를 계산할 수 있다
3. 다중 큐비트 시스템의 부분 측정을 분석하고 측정되지 않은 큐비트의 결과 상태를 유도할 수 있다
4. $\{M_m\}$ 연산자를 사용한 일반 측정 형식론을 설명할 수 있다
5. POVM(양의 연산자값 측도(positive operator-valued measure))과 그것이 유용한 경우를 설명할 수 있다
6. 양자 제논 효과(quantum Zeno effect)와 그 물리적 함의를 설명할 수 있다
7. Python에서 측정과 부분 추적(partial traces)을 시뮬레이션할 수 있다
...
상태 벡터 형식론은 **순수 상태(pure state)** — 완전한 양자 정보를 가진 상태 — 를 기술합니다. 그러나 부분 측정과 부분 시스템의 추적 제거는 자연스럽게 **혼합 상태(mixed state)**를 만들어 내며, 이를 기술하려면 **밀도 행렬(density matrix)** 형식론이 필요합니다.

**순수 상태 밀도 행렬**: 상태 $|\psi\rangle$에 대해 밀도 행렬은 다음과 같습니다:

$$\rho = |\psi\rangle\langle\psi|$$

이것은 랭크 1의 양의 준정부호(positive semidefinite) 행렬로, $\text{Tr}[\rho] = 1$이고 $\text{Tr}[\rho^2] = 1$입니다.

**혼합 상태 밀도 행렬**: 시스템이 고전적 확률 $p_i$로 상태 $|\psi_i\rangle$에 있을 때, 밀도 행렬은 다음과 같습니다:

$$\rho = \sum_i p_i |\psi_i\rangle\langle\psi_i|$$

혼합 상태의 경우 $\text{Tr}[\rho^2] < 1$입니다. **순도(purity)** $\text{Tr}[\rho^2]$의 범위는 $1/d$(최대 혼합, $d$ = 차원)에서 $1$(순수)까지입니다.

**부분 추적**: 이분(bipartite) 시스템 $AB$가 상태 $\rho_{AB}$에 있을 때, 부분 시스템 $A$의 축소 상태는 $B$를 추적 제거하여 얻습니다:

$$\rho_A = \text{Tr}_B[\rho_{AB}] = \sum_j (I_A \otimes \langle j|_B) \, \rho_{AB} \, (I_A \otimes |j\rangle_B)$$

얽힌 순수 상태 $|\psi\rangle_{AB}$의 경우, 축소 밀도 행렬 $\rho_A$는 혼합 상태입니다. 폰 노이만 엔트로피(von Neumann entropy) $S(\rho_A) = -\text{Tr}[\rho_A \log_2 \rho_A]$는 $A$와 $B$ 사이의 얽힘을 정량화합니다.

**밀도 행렬에 대한 측정 규칙**: 결과 $m$의 확률과 측정 후 상태는 다음과 같습니다:

$$P(m) = \text{Tr}[P_m \rho], \quad \rho_m' = \frac{P_m \rho P_m}{\text{Tr}[P_m \rho]}$$

이것은 보른 규칙을 혼합 상태로 일반화한 것이며, 잡음, 결어긋남(decoherence), 개방 양자 시스템의 분석에 필수적입니다([레슨 20](20_Noise_and_Quantum_Channels.md) 참조).
