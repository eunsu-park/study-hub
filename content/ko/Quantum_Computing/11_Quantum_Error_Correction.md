...
모든 계산은 오류를 만듭니다. 고전 컴퓨터는 이를 중복성(Redundancy)으로 처리합니다 — 각 비트를 세 번 저장하고 다수결로 복원합니다. 양자 컴퓨터는 훨씬 더 어려운 도전에 직면합니다: 복제 불가 정리는 양자 상태를 복사하는 것을 금지하고, 연속적인 오류(작은 각도의 회전)는 정정하기 위해 무한한 정밀도가 필요해 보이며, 측정은 보호하려는 정보 자체를 파괴합니다. 수십 년간 많은 물리학자들은 양자 오류 정정이 불가능하다고 믿었습니다.

양자 오류 정정이 *가능하다*는 발견 — 쇼어(1995)와 스테인(1996)에 의해 — 은 양자 정보 이론에서 가장 놀라운 결과 중 하나였습니다. 이 모든 장애물에도 불구하고, 영리한 인코딩이 정보가 무엇인지 알지 못하면서도 양자 정보를 노이즈로부터 보호할 수 있음을 보였습니다. 이 돌파구는 오류 허용 양자 컴퓨팅의 전체 사업을 뒷받침합니다: 이것 없이는 어떤 양자 알고리즘도 유용할 만큼 충분히 오래 실행될 수 없습니다.
...
```
  Surface Code Lattice (distance d = 3)
  ...
  ○ = vertices (Z-stabilizer locations)
  d = data qubits (on edges)
  [X_f] = faces (X-stabilizer locations)

  Z-stabilizer (vertex v1): Z_d01 · Z_d12 · Z_d14
  X-stabilizer (face f0):   X_d01 · X_d03 · X_d14 · X_d34

  Data qubits: 2L²-2L+1 = 7 for L=2
  Logical X: chain of X operators across lattice (horizontal)
  Logical Z: chain of Z operators across lattice (vertical)
```
...
표면 코드의 임계값 $\sim 1\%$가 가장 실용적으로 중요합니다. 아래는 물리적 오류율 $p$에 대한 대표적인 오버헤드 추정입니다:

| 물리적 오류율 $p$ | 코드 거리 $d$ | 논리적 큐비트당 물리적 큐비트 ($\sim 2d^2$) | 논리적 오류율 (대략) |
|---|---|---|---|
| $10^{-2}$ (임계값) | 임계값 | — | 개선 불가 |
| $10^{-3}$ | $d = 17$ | $\sim 578$ | $\sim 10^{-10}$ |
| $10^{-3}$ | $d = 27$ | $\sim 1{,}458$ | $\sim 10^{-17}$ |
| $10^{-4}$ | $d = 7$ | $\sim 98$ | $\sim 10^{-10}$ |

RSA-2048에 대해 쇼어의 알고리즘을 실행하려면 약 4,000개의 논리적 큐비트가 필요합니다. $p = 10^{-3}$이고 $d = 17$인 경우 약 $4{,}000 \times 578 \approx 230$만 개의 물리적 큐비트가 필요합니다. 물리적 오류율을 $10^{-4}$로 낮추면 $\sim 400{,}000$개의 물리적 큐비트로 줄어듭니다 — 게이트 충실도를 개선하는 것이 왜 엄청난 실용적 영향을 미치는지를 보여줍니다.

### 9.5 시사점

임계값 정리는 오류 허용 양자 컴퓨팅의 *존재 증명(Existence Proof)*입니다. 원칙적으로 양자 컴퓨터를 신뢰할 수 있게 만들 수 있다고 말합니다. 실질적인 질문은 오버헤드(논리적 큐비트당 수천 개의 물리적 큐비트)가 기술적으로 실현 가능할 만큼 충분히 줄어들 수 있는지 여부입니다. 이것이 이 분야의 중심 과제로 남아 있습니다.
