...
1. 다양한 인코딩 전략을 사용하여 고전 데이터를 양자 상태로 인코딩하는 방법을 설명할 수 있다
2. 양자 특성 맵(quantum feature map)과 고차원 특성 공간 생성에서의 역할을 설명할 수 있다
3. 변분 양자 분류기(variational quantum classifier)를 구성하고 훈련할 수 있다
4. 양자 커널 방법(quantum kernel method)과 고전 커널 SVM과의 연관성을 설명할 수 있다
5. 불모 고원(barren plateau) 문제와 양자 ML 훈련 가능성에 미치는 영향을 설명할 수 있다
6. 머신러닝에서의 양자 우위(quantum advantage) 주장을 비판적으로 평가할 수 있다
7. Python으로 양자 특성 맵, 분류기, 커널 계산을 구현할 수 있다
...
### 7.4 탈양자화(Dequantization) 결과

양자 ML에 대한 열정을 누그러뜨린 주요 발전은 Ewin Tang(2018)이 시작한 **탈양자화(dequantization)** 프로그램이었습니다. Tang은 이전에 지수적 양자 속도 향상을 제공한다고 여겨졌던 Kerenidis-Prakash의 양자 추천 시스템 알고리즘이, 고전 알고리즘이 데이터에 대한 "표본 및 질의(sample and query)" 접근(양자 진폭 접근과 유사)을 가지는 경우 다항식 오버헤드만으로 고전 알고리즘에 의해 대응될 수 있음을 보였습니다.

Chia, Gilyen, Li, Lin, Tang, Wang의 후속 연구는 탈양자화를 다른 양자 선형대수 알고리즘으로 확장했습니다:

| 양자 알고리즘 | 원래 주장된 속도 향상 | 탈양자화됨? |
|---|---|---|
| 양자 추천 (Kerenidis-Prakash) | 지수적 | **예** (Tang 2018) |
| 양자 PCA | 지수적 | **예** (Tang 2018) |
| 양자 지도 클러스터링 | 지수적 | **예** (Chia et al. 2020) |
| 양자 SVM (최소제곱) | 지수적 | **예** (Chia et al. 2020) |
| HHL 선형 시스템 해법 | 지수적 (조건부) | **부분적** (출력 요구사항에 따라 다름) |

핵심 교훈: 많은 양자 ML 속도 향상은 고전 데이터를 양자 상태로 로드하는 것이 "무료"라는 가정에 의존했습니다. 데이터 로딩 비용을 적절히 고려하면, 양자 우위는 종종 줄어들거나 사라집니다. 진정한 양자 우위는 *양자 데이터*(양자 과정에 의해 생성된 데이터) 또는 특성 맵의 양자 구조가 필수적이고 고전적으로 모방 불가능한 문제에서 가장 가능성이 높습니다.

### 7.5 양자 ML이 도움이 될 수 있는 곳
