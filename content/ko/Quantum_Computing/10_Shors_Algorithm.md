...
### 4.5 연분수 알고리즘(Continued Fraction Algorithm)
...
### 6.5 N = 15에 대한 상세 연분수 추출

측정 결과 $m = 384$, $2^t = 512$에 대해 연분수 알고리즘을 추적해 봅시다.

$384/512 = 3/4$의 연분수 전개를 계산합니다:
...
수렴값 $3/4$의 분모 $k = 4 \leq N = 15$이므로, 후보 주기는 $r = 4$입니다. 검증: $7^4 = 2401 \equiv 1 \pmod{15}$. 확인됨.

$m = 128$의 경우: $128/512 = 1/4$. 연분수는 $[0; 4]$이고, 수렴값 $1/4$을 주므로 $r = 4$를 직접 얻습니다.

$m = 256$의 경우: $256/512 = 1/2$. 연분수는 $r = 2$를 줍니다. 검증: $7^2 = 49 \equiv 4 \not\equiv 1 \pmod{15}$이므로 $r = 2$는 틀립니다. 두 배를 시도할 수 있습니다: $2r = 4$, 그리고 $7^4 \equiv 1$임을 확인합니다. 이는 때때로 연분수가 참 주기의 *약수*를 줄 수 있으며, 작은 배수를 시도하여 처리할 수 있음을 보여줍니다.
...
2024년 8월, NIST는 최초의 세 가지 포스트-양자 암호 표준(FIPS 203, 204, 205)을 최종 확정하여, 양자 위협에 대비하는 주요 이정표를 세웠습니다:

- **ML-KEM (FIPS 203)** — 구 CRYSTALS-Kyber: 키 교환을 위한 격자 기반 키 캡슐화(Lattice-based key encapsulation)
- **ML-DSA (FIPS 204)** — 구 CRYSTALS-Dilithium: 격자 기반 디지털 서명(Lattice-based digital signatures)
- **SLH-DSA (FIPS 205)** — 구 SPHINCS+: 해시 기반 디지털 서명(Hash-based digital signatures, stateless)

네 번째 표준인 **FN-DSA**(구 Falcon, NTRU 격자 기반 서명)가 뒤따를 예정입니다. 이 알고리즘들은 고전적 공격과 양자 공격 모두에 저항하도록 설계되었으며, "지금 수확하고, 나중에 복호화(harvest now, decrypt later)" 위협 때문에 조직들은 즉시 마이그레이션을 시작할 것을 권고받고 있습니다.
