...
블로흐 구는 *순수* 상태(pure state) — $|\psi\rangle = \cos(\theta/2)|0\rangle + e^{i\phi}\sin(\theta/2)|1\rangle$로 쓸 수 있는 상태 — 를 나타냅니다. 그러나 큐비트는 상태 벡터가 아닌 **밀도 행렬(density matrix)** $\rho$로 기술되는 *혼합* 상태(mixed state)에도 놓일 수 있습니다. 혼합 상태는 시스템이 어떤 양자 상태에 있는지에 대해 고전적 불확실성이 있을 때 발생합니다(예: 얽힌 시스템의 일부를 트레이스 아웃한 후).

임의의 단일 큐비트 밀도 행렬은 $\rho = \frac{1}{2}(I + \vec{r} \cdot \vec{\sigma})$로 쓸 수 있으며, 여기서 $\vec{r} = (r_x, r_y, r_z)$는 **블로흐 벡터(Bloch vector)**, $\vec{\sigma} = (X, Y, Z)$는 파울리 행렬입니다. 순수 상태의 경우 $|\vec{r}| = 1$(블로흐 구의 표면 위)이고, 혼합 상태의 경우 $|\vec{r}| < 1$(구의 내부)입니다. 최대 혼합 상태(maximally mixed state) $\rho = I/2$는 $\vec{r} = \vec{0}$으로 원점에 위치합니다. 유효한 상태의 전체 집합은 구의 표면만이 아니라 **블로흐 볼(Bloch ball)**(속이 찬 구)을 채웁니다. 밀도 행렬은 [레슨 6](06_Quantum_Measurement.md)에서 공식화하고, 잡음 채널이 블로흐 볼을 어떻게 변환하는지는 [레슨 20](20_Noise_and_Quantum_Channels.md)에서 탐구합니다.
