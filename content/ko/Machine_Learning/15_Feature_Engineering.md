...
**이전**: [실전 프로젝트](./14_Practical_Projects.md) | **다음**: [모델 설명 가능성](./16_Model_Explainability.md)
...
1. 피처 엔지니어링이 모델 선택보다 더 큰 영향을 미치는 이유를 설명할 수 있습니다
2. 모델 가정에 맞게 수치형 변환(로그, 거듭제곱, 구간화(binning), 다항식)을 적용할 수 있습니다
3. 범주형 인코딩 전략(원-핫(one-hot), 타겟(target), 빈도(frequency), 해시(hash))을 구현하고 카디널리티(cardinality)에 따라 적절한 방법을 선택할 수 있습니다
4. 순환 인코딩(cyclical encoding), 래그값(lag values), 롤링 통계(rolling statistics)를 사용하여 시계열 피처를 엔지니어링할 수 있습니다
5. TF-IDF, n-그램(n-gram), 기본 텍스트 통계를 사용하여 텍스트 피처를 추출할 수 있습니다
6. sklearn Pipeline을 사용하여 피처 엔지니어링 중 데이터 누수(data leakage)를 식별하고 방지할 수 있습니다
7. 원시 피처 기준선(baseline) 대비 모델 성능을 향상시키는 완전한 피처 엔지니어링 파이프라인을 구축할 수 있습니다

---

평범한 모델과 경연 우승 모델의 차이는 알고리즘이 아닌 피처에 있는 경우가 대부분입니다. 피처 엔지니어링(feature engineering)은 원시 데이터를 모델이 학습해야 할 내재된 패턴을 드러내는 표현으로 변환하는 예술이자 과학입니다. 날짜는 "요일(day of week)"과 "공휴일 여부(is_holiday)"가 되고, 주소는 도심과의 거리가 되며, 원시 텍스트는 TF-IDF 벡터가 됩니다. 이 레슨은 어떤 모델에서든 최대한의 성능을 이끌어낼 수 있도록 포괄적인 변환 도구 모음을 제공합니다.

> **레시피 전 조리 준비.** 요리사는 재료를 냄비에 그냥 던져 넣지 않습니다 — 씻고, 껍질을 벗기고, 자르고, 재우고, 계량하는 과정을 먼저 거칩니다. 피처 엔지니어링은 머신러닝의 "미장플라스(mise en place)"입니다: 원시 데이터를 모델이 실제로 활용할 수 있는 재료로 변환하는 것입니다. 날짜는 "요일"과 "공휴일 여부"가 되고, 주소는 도심과의 거리가 되며, 원시 텍스트는 TF-IDF 벡터가 됩니다. 종종 영리한 피처 엔지니어링이 더 정교한 알고리즘을 선택하는 것보다 더 중요합니다 — 아무리 좋은 모델도 피처에 표현되지 않은 패턴은 학습할 수 없습니다.

---

## 이론과 원리

피처 엔지니어링은 도메인 지식과 모델의 귀납 편향 사이의 다리입니다. 변환은 임의의 트릭이 아닙니다 — 각각이 분포 가정에 뿌리를 둔 수학적 정당화를 가지며, 각각이 Lesson 13의 누수 규칙과 상호작용합니다. 이 정당화를 이해하면 피처 엔지니어링이 추측에서 의도적인 실천으로 바뀝니다.

### A. 변환이 도움이 되는 이유: 귀납 편향 매칭

대부분의 ML 알고리즘은 특성이 타깃과 어떻게 관계되는지에 대한 암묵적 가정을 가집니다:
- 선형/로지스틱 회귀는 타깃이 특성에 대해 대략 선형이라 가정.
- k-NN은 타깃이 작은 이웃에서 대략 일정하다 가정.
- 거리 기반 방법(k-NN, K-Means, RBF SVM)은 특성이 비슷한 스케일을 가진다 가정.
- Naive Bayes는 특성의 조건부 독립을 가정.

이 가정들을 데이터가 더 잘 만족하게 만드는 변환은 추가 용량 없이 *공짜로* 모델의 편향을 줄입니다. 그것이 피처 엔지니어링의 모든 수학적 정당화 — 알고리즘의 귀납 편향에 맞도록 데이터를 미리 형성하는 것.

### B. 로그 변환과 거듭제곱 변환

특성이 여러 자릿수에 걸쳐 있으면(소득, 인구, 파일 크기), 모델은 그 전체 범위에 걸쳐 매끄럽게 반응하는 함수를 학습하도록 강요됩니다. 로그 변환은 스케일을 압축하고 곱셈 관계를 덧셈 관계로 바꿉니다:

```
log(a · b) = log(a) + log(b)
log1p(x)   = log(1 + x)         ← x ≥ 0(0 포함)에 안전
```

정당화: 많은 실세계 양은 로그 정규(log-normal)이며, 로그 정규 `y = exp(βᵀx + ε)`는 정확히 `log(y)`를 `x`에 회귀할 때 OLS가 적합하는 모델. 잔차가 비대칭이고 이분산이면 타깃을 로그 변환하는 것이 종종 둘 다 한 번에 고칩니다.

**Box-Cox 변환**은 자유 매개변수 `λ`로 이를 일반화:
...
`λ`는 변환된 데이터를 가능한 한 가우시안에 가깝게 만들도록 최대가능도로 적합. `λ = 1` ⟹ 변환 없음, `λ = 0` ⟹ 로그, `λ = -1` ⟹ 역수. Box-Cox는 양수 값을 요구; **Yeo-Johnson**은 0과 음수 값도 처리하는 버전. 둘 다 가우시안 잔차를 가정하는 알고리즘(선형 모델, GMM, LDA)에 변환된 특성이 더 잘 맞도록 합니다.

### C. 범주형 인코딩: 트레이드오프의 위계

모델은 문자열을 소비할 수 없습니다 — 범주형 특성은 숫자가 되어야 합니다. 인코딩 선택은 알고리즘 유형과 카디널리티 모두와 상호작용:

- **레이블 인코딩(Label encoding)**: 각 수준에 정수 할당. 빠르지만 가짜 순서를 주입 — `apple = 0, banana = 1, cherry = 2`는 "banana"를 "apple"과 "cherry" 사이에 놓고, 트리와 선형 모델이 그것을 문자 그대로 해석. 순서형 특성(low/medium/high)에만 사용.
- **원-핫 인코딩(One-hot encoding, OHE)**: 수준당 하나의 이진 열. 알고리즘 무관, 정확, 단 고카디널리티 특성에 대해 특성 수가 폭발(10K 수준의 우편번호가 10K 열이 됨).
- **타깃 인코딩(Target encoding)**: 각 수준을 그 수준의 평균 타깃 값으로 대체. 압축적(K개 대신 1열), 정보적 — 단 거대한 함정: **특성을 통해 타깃을 누설**합니다.

### C.1 타깃 인코딩이 폴드 외 계산을 필요로 하는 이유

순진한 타깃 인코딩(전체 학습셋에 대한 `y_mean.groupby(level).mean()`)은 교과서적인 누수 함정. 행 `i`에 대한 인코딩 값이 `y_i` 자체를 포함하므로, 인코딩된 특성에 학습된 모델은 학습 타깃을 완벽히 외울 수 있고 — 끔찍하게 일반화합니다.

해법은 **폴드 외 타깃 인코딩(out-of-fold target encoding)**:

```
각 폴드 f에 대해:
    train_other = 폴드 f에 없는 학습 행
    means = train_other.groupby(level).mean(target)
    encoded[fold f] = level → means    (폴드 f의 행은 자신 없이 계산된 means 사용)
```

각 행의 인코딩된 값은 행 자신의 타깃을 *포함하지 않는* 데이터에서 계산. CV 분할 안에서 인코더를 실행하는 것과 동등. 항상 이렇게, 절대 순진한 버전 안 함.

본 적 없는 테스트 수준에 대해서는 전역 평균으로 폴백. 희귀 수준의 인코딩을 안정화하려면 **평활(smoothing)** 사용:

```
encoded(level) = (count(level) · mean(level) + α · global_mean)  /  (count(level) + α)
```

`α`는 하이퍼파라미터; 희귀 수준이 전역 평균으로 끌립니다.

### D. 수치 비닝과 이산화

연속 특성이 타깃에 대해 비단조적 효과를 가질 때가 있습니다 — 예: 나이(아동과 노인이 중년과 다르게 행동). 선형 모델은 다항식 항 없이 이를 포착할 수 없음. 특성을 이산 버킷으로 비닝하고 버킷을 원-핫 인코딩하면 모델이 각 버킷에 자신의 계수를 할당.

세 가지 비닝 전략:
- **등너비(Equal-width)**: 범위 `[min, max]`를 `K`개 동일 구간으로 분할. 이상치에 민감.
- **등빈도(Equal-frequency, 분위수)**: 각 버킷이 같은 수의 학습 예제를 가짐. 이상치 강건, 흔한 기본.
- **결정 트리 기반**: `(x_j, y)`에 얕은 트리를 적합하고 그 리프를 빈으로 사용. 지도 — 버킷이 타깃과의 관계를 존중.

비닝은 해상도와 비선형성을 맞바꿉니다. 트리 기반 모델은 이익을 보지 않음(내부적으로 비닝); 선형 모델은 종종 이익.

### E. 상호작용 특성

많은 실제 관계는 두 특성을 함께 포함합니다: BMI = 체중 / 키², 침실당 가격 = 가격 / 침실 수. 선형 모델은 `(체중, 키)`만으로 이를 학습할 수 없음 — 곱이나 비율을 명시적 특성으로 공급해야 함.

다항 특성 `(x_1, x_2)` → `(x_1, x_2, x_1², x_1·x_2, x_2²)`는 차수 `d`의 모든 상호작용을 자동으로 열거. 트리와 그래디언트 부스팅 모델은 중첩 분할을 통해 상호작용을 암묵적으로 발견하지만, 그것들조차 비율이 진정 의미 있는 양일 때 손코딩된 몇 개의 비율로부터 종종 이익을 봅니다.

비용은 특성 폭발: 차수 `d`에 대해 `O(p^d)` 다항 특성. 정칙화(Lasso)와 결합하여 잡음 있는 것을 자동으로 가지치기.

### F. 특성 선택: 세 가족

특성이 너무 많을 때, 무관한 것을 제거하면 과적합 위험을 줄이고 학습을 빠르게 합니다. 세 가족의 선택 방법:

1. **필터 방법(Filter)**: 단변량 통계(분산, 타깃과의 상호 정보, 카이제곱)로 특성 순위 매기고 상위 `k` 유지. 빠르고, 모델 무관, 특성 상호작용 무시.
2. **래퍼 방법(Wrapper)**: 전진/후진 선택 — 한 번에 한 특성 추가/제거하고 재학습. 느리지만 상호작용 고려.
3. **임베디드 방법(Embedded)**: 학습에 내장된 정칙화(Lasso L1) 또는 트리 기반 중요도. 모델 적합의 부수 효과로 무료; 실전에서 가장 흔한 선택.

올바른 선택은 차원과 컴퓨팅 예산에 의존: `p`가 거대하면 필터, 정칙화 모델 하나를 적합할 수 있다면 임베디드, 래퍼는 드뭅니다.

### G. 피처 엔지니어링에 대한 누수 세금

모든 변환은 Lesson 13의 누수 규칙을 존중해야 합니다. 파이프라인 래핑 규율은 협상 불가:

| 변환 | 파이프라인 안에? | 이유 |
|------|------------------|------|
| `log(x)` | 선택(학습 매개변수 없음) | 상수 변환 |
| `StandardScaler` | 예 | 평균과 표준편차가 학습됨 |
| 중앙값 대치 | 예 | 중앙값이 학습됨 |
| 타깃 인코딩 | 예(폴드 외와 함께) | 타깃 사용 |
| 고정 스키마의 원-핫 인코딩 | 선택 | `handle_unknown='ignore'`이면 |
| PCA | 예 | 고유벡터가 학습됨 |
| 다항 특성 | 선택(학습 매개변수 없음) | 결정적 확장 |

뭔가가 파이프라인에 속하는지의 *시험*: "이것이 다른 행의 값이나 타깃을 읽는가?" 그렇다면 파이프라인에 속함. 예외 없음.

### From Theory to the Code Below

- 섹션 2의 `np.log1p`, `PowerTransformer(method='box-cox' | 'yeo-johnson')`는 (B)의 가족.
- 섹션 3의 `OneHotEncoder`, `LabelEncoder`, 타깃 인코딩 예제는 (C)에 매핑; 섹션 3.4의 폴드 외 타깃 인코딩 루프는 정확히 (C.1).
- 섹션 4의 `KBinsDiscretizer(strategy='uniform' | 'quantile' | 'kmeans')`는 (D)의 비닝 위계.
- 섹션 5의 `PolynomialFeatures`는 (E)의 상호작용 열거.
- 섹션 6의 `SelectKBest`, `RFE`, `SelectFromModel`은 (F)의 세 가족: 필터, 래퍼, 임베디드.
- 전체에 걸쳐 나타나는 `Pipeline` 래핑은 (G)의 누수 규율.
