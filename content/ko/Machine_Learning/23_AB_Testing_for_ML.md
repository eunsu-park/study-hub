...
ML을 위한 A/B 테스트는 응용 통계 가설 검정입니다 — 모든 단계(표본 크기, p-값, 다중 비교, 순차 검정)가 정확한 수학적 정당화를 가지며, 가장 흔한 A/B 테스트 실수는 그 단계의 잘못된 적용입니다. 이 섹션은 실천을 통계에 근거를 둠으로써 레슨 나머지의 코드가 미스터리한 레시피이기를 멈추고 의도적인 의사결정이 되도록 합니다.
...
- 섹션 1의 "오프라인 ≠ 온라인" 논의는 (A)의 전체 프레임워크의 동기.
- 섹션 2의 `statsmodels.stats.power` 표본 크기 계산은 (B)의 공식을 구현; MDE 개념이 테스트가 결정적일지 결정.
- 섹션 3의 z-검정 / t-검정 구현(`scipy.stats.ttest_ind`, `proportions_ztest`)은 (C)의 알고리즘.
- 섹션 4의 "p-값 함정" 콜아웃은 (D)의 경고를 반영.
- 섹션 5의 Bonferroni / FDR 보정 코드는 (E)의 다중 비교 처리.
- 섹션 6의 세그먼트 분석(지역별, 기기별)은 (F)의 이질성 점검.
- 섹션 7의 베이지안 A/B 예제(PyMC 또는 베타-이항 사용)는 (G)의 대안 패러다임.
