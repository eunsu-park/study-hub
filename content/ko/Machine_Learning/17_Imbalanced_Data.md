...
불균형 처리 기법의 세 가족 — 재표본추출(SMOTE 등), 클래스 가중, 임계값 튜닝 — 은 같은 문제를 세 가지 다른 수학적 각도에서 공격합니다. 어떤 것을 먼저 잡을지 알려면 각각이 손실 표면과 결정 경계에 실제로 무엇을 하는지 이해해야 합니다.

### A. 불균형이 해를 주는 이유: 손실 표면 관점
...
`y_i`의 99%가 음성 클래스라면, 그래디언트 신호 `∂L/∂θ`의 99%가 다수 클래스에서 옵니다. 소수 클래스는 매개변수를 거의 움직이지 못함. 더 나쁜 것은, *자명한* 해 `f(x) = 0`(항상 다수 예측)이 진단 가치가 0임에도 거의 최적인 손실을 달성합니다.

편향-분산 렌즈(Lesson 1)가 그림을 날카롭게 합니다: 불균형 데이터는 진짜 위험(비용 가중)이 두 클래스를 동등하게 중요하게 다루더라도 경험적 위험을 다수 클래스 쪽으로 편향시킵니다. 이 레슨의 모든 완화는 경험적 위험을 실제 신경 쓰는 것과 재정렬하는 방법입니다.

### B. SMOTE와 그 보간 알고리즘

**SMOTE**(Synthetic Minority Over-sampling Technique, Chawla et al., 2002)는 선형 보간으로 합성 소수 클래스 예제를 생성:

```
각 소수 예제 x_i에 대해:
    x_i의 k개 최근접 소수 이웃 선택        (보통 k = 5)
    원하는 각 합성 표본에 대해:
        무작위로 한 이웃 x_n 선택
        λ ~ Uniform(0, 1) 선택
        synthetic = x_i + λ · (x_n - x_i)            ← 보간된 점
```

각 합성 점은 소수 예제와 그 소수 이웃 중 하나 사이의 선분 위에 있음. 반복하면, 소수 인구를 원하는 비율로 곱합니다. 모델은 이제 (A의 손실 표면 의미에서) 균형 잡힌 학습 분포를 보며, 데이터를 버리지 않습니다.

가정은 두 소수 점의 볼록 결합이 *역시* 그럴듯한 소수 점 — 즉, 소수 클래스가 특성 공간에서 볼록(또는 적어도 국소적으로)이라는 것. 이것이 실패하면 SMOTE가 다수 영역에 떨어지는 점을 만들고 경계를 *악화*시킵니다. 두 가지 중요한 변형이 이를 다룹니다:

- **Borderline SMOTE**: 적어도 하나의 다수 이웃을 가진 소수 점("경계" 사례)에서만 보간. 합성 생성을 경계가 있는 곳에 집중.
- **ADASYN**: 각 소수 점이 분류하기 *얼마나 어려운지*에 따라 합성 생성을 가중(다수 이웃 근처에 더 많은 합성 표본). 국소 어려움에 적응.

### B.1 결정적: SMOTE는 파이프라인 안에 있어야 함

CV 전에 전체 학습셋에 적용된 SMOTE는 불균형 처리에서 가장 미묘하고 해로운 누수입니다. 한 행을 사용해 생성된 합성 점이 그 행의 이웃에도 의존 — 그리고 CV 폴드에서 그 이웃이 검증 슬라이스에 있을 수 있음. 모델은 사실상 합성 점을 통해 검증 레이블을 봅니다.

해법: `sklearn.pipeline.Pipeline`(모든 것에 호출) 대신 `imblearn.pipeline.Pipeline`(각 분할의 학습 쪽에서만 SMOTE 호출)을 사용. 또는 각 CV 폴드 안에서 수동으로 래핑. 분할 전 SMOTE 절대 안 함.

### C. 언더샘플링과 정보 손실 트레이드오프

반대 접근: 클래스가 균형 잡힐 때까지 다수 예제를 버림. **무작위 언더샘플링(Random undersampling)**이 가장 단순한 버전. 빠르고, 학습 비용이 저렴하며, SMOTE의 "합성 점이 비현실적" 문제를 피하지만 — 정보를 버림.

언더샘플링이 의미 있는 경우:
- 다수 클래스가 거대하고 대부분 중복(예: 수십억 합법 클릭, 수백 사기).
- 컴퓨팅이 구속 제약이고 전체 다수 집합에서 학습할 수 없음.
- 앙상블과 결합(BalancedRandomForest, EasyEnsemble): 각 기저 추정기가 다른 언더샘플 분할을 보므로, 정보가 영구적으로 손실되지 않음 — 다른 다수 부분집합이 앙상블 전반에 걸쳐 전체 데이터셋을 커버.

**Tomek 링크**(반대 클래스 최근접 이웃 쌍 제거)와 **NearMiss**(소수 경계에 가까운 다수 점만 유지) 같은 더 똑똑한 변형은 "쉬운" 다수 점을 제거하고 정보적인 것을 유지하려 합니다.

### D. 클래스 가중: 손실을 직접 재가중

데이터를 바꾸는 대신 손실을 바꿉니다. **클래스 가중 손실(Class-weighted loss)**은 각 예제의 기여를 클래스 의존 가중치로 스케일:

```
L_weighted(θ) = (1/N) · Σ_i  w_{y_i} · ℓ(y_i, f(x_i; θ))
```

scikit-learn의 표준 `class_weight='balanced'` 레시피:

```
w_k = N / (K · n_k)
```

여기서 `K`는 클래스 수, `n_k`는 클래스 `k`의 카운트. 이는 각 *클래스*가 예제 수에 무관하게 손실에 같은 총합을 기여하게 만듭니다.

클래스 가중은 (상수까지) 소수를 `w_minority / w_majority`로 오버샘플링하는 것과 수학적으로 동등하지만 더 저렴함 — 생성할 합성 점 없음, 처리할 추가 행 없음. 모델은 여전히 원래 분포를 보고; 그래디언트가 재가중됩니다.

클래스 가중이 선호되는 경우:
- 기저 학습기가 지원할 때(대부분의 scikit-learn 분류기, XGBoost는 `scale_pos_weight`, LightGBM은 `is_unbalance` / `scale_pos_weight`).
- 평가를 원래(불균형) 분포에서 유지하고 싶을 때.
- SMOTE의 볼록성 가정을 신뢰하지 않을 때.

### E. 임계값 튜닝: 가장 저렴하고 가장 덜 사용되는 도구

모든 확률적 분류기는 `[0, 1]`의 점수를 출력. 단단한 예측으로 변환하려면 임계값을 골라야 함. 기본값 0.5는 불균형 하에서 거의 최적이 아닙니다.

Lesson 4의 베이즈 최적 비용 규칙 회상:

```
양성 예측  ⟺  P(y=1 | x) > c_FP / (c_FP + c_FN)
```

불균형 하에서, `c_FN`(양성 놓침의 비용)이 보통 `c_FP`보다 훨씬 큼, 따라서 최적 임계값이 0.5보다 *훨씬* 낮음. 구체적으로:
- 임계값을 0에서 1로 휩쓸음.
- 각 임계값에 대해 실제 신경 쓰는 지표(F1, F-beta, 총 비용) 계산.
- argmax 선택.

임계값 튜닝은 불균형 처리에서 가장 덜 사용되는 단일 도구. 비용이 없음 — 추가 학습 없음, 아키텍처 변경 없음 — 그리고 종종 SMOTE나 가중이 제공할 이득의 대부분을 회복. 프로덕션에서는 임계값을 세그먼트별로도 튜닝 가능(지역/고객 등급별 다른 비용 구조).

### F. 불균형 데이터 평가: 지표가 중요

섹션 1이 이미 보여주듯이 정확도는 여기서 잘못된 지표. 올바른 지표:

- **Precision-Recall 곡선과 AUC-PR**: 양성 클래스에만 집중. 풍부한 음성에 의해 ROC의 FPR이 지배되므로 심한 불균형 하에서 ROC-AUC보다 나음.
- **F-beta 점수**: 정밀도(`β < 1`) 또는 재현율(`β > 1`)로 가중된 조화 평균.
- **균형 정확도(Balanced accuracy)**: 클래스별 재현율의 평균. 크기와 무관하게 두 클래스를 대칭적으로 다룸.
- **Cohen의 카파**: 우연으로 보정된 일치도, 클래스 편향에 강건.
- **비용 가중 오차**: 실제 비용을 안다면, 올바른 지표는 정확히 달러 액수.

CV 전략도 불균형을 고려해야 함: **층화 K-폴드**가 모든 폴드에서 클래스 비율을 보존. 층화 없이는 운 나쁜 폴드가 검증 슬라이스에 양성 예제 0개를 가져 무의미한 점수를 만들 수 있음.

### G. 도구 결합: 실용 레시피

단일 최선 방법은 없음 — 올바른 결합은 데이터 크기, 기저 모델, 비용 구조에 의존. 합리적인 기본 순서:

1. **항상**: 층화 CV, 주요 지표로 AUC-PR(정확도 아님), 정밀도-재현율 곡선 그리기.
2. **먼저 시도**: `class_weight='balanced'`(또는 XGBoost는 `scale_pos_weight = n_neg / n_pos`). 가장 저렴, 종종 충분.
3. **여전히 재현율 부족**: 그 위에 임계값 튜닝 추가. 가중 후 가장 저렴한 이득.
4. **여전히 재현율 부족**: 파이프라인 안에서 SMOTE / Borderline SMOTE 시도.
5. **다수가 거대하고 중복적**: 언더샘플링 또는 BalancedRandomForest 시도.
6. **비용이 명시적**: (E)의 베이즈 최적 임계값을 직접 사용.

단계 1–3이 실제 사례의 다수를 커버. 단계 4–6은 어려운 문제에 대한 에스컬레이션.

### From Theory to the Code Below

- 섹션 1의 "항상 다수 예측" 시연은 (A)의 손실 표면 논거 — 손실이 다수 클래스 예제에 의해 지배되므로 정확도가 높음.
- 섹션 2의 `imbalanced-learn`의 `SMOTE`, `BorderlineSMOTE`, `ADASYN`은 (B)의 알고리즘; `imblearn.pipeline.Pipeline` 사용은 (B.1)의 누수 규율.
- 섹션 3의 `RandomUnderSampler`, `TomekLinks`, `NearMiss`는 (C)의 언더샘플링 변형; `BalancedRandomForestClassifier`와 `EasyEnsembleClassifier`는 앙상블 기반 정보 보존 전략.
- 섹션 4의 `class_weight='balanced'`와 `scale_pos_weight`는 (D)의 가중.
- 섹션 5의 임계값 튜닝 루프는 정확히 (E); 비용 행렬 변형이 베이즈 최적 규칙을 구현.
- 섹션 6의 `precision_recall_curve`, `f1_score`, `balanced_accuracy_score`는 (F)의 불균형 친화 지표.
