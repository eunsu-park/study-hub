...
모든 분류 지표는 같은 2×2 혼동 행렬의 서로 다른 요약입니다. 정확도, 정밀도, 재현율, F1, ROC-AUC, PR-AUC 사이의 선택은 스타일이 아닙니다 — 각각이 다른 운영 질문에 답하며, 잘못된 것을 쓰면 사실상 쓸모없는 모델을 숨길 수 있습니다.
...
### D. 임계값 선택과 비용 민감 학습(Cost-Sensitive Learning)
...
- 섹션 1.1의 `confusion_matrix`는 (A)의 2×2 표입니다; 나머지 섹션의 모든 분류 지표는 거기 나열된 비율 중 하나입니다.
- 섹션 1.2의 `precision_score`, `recall_score`, `f1_score`는 (A)와 (B)의 공식을 구현합니다.
- 섹션 1.3의 `roc_auc_score`는 (C)의 확률입니다; ROC 곡선 플롯이 그것을 기하학적으로 보여줍니다.
- 섹션 2의 임계값 튜닝 코드는 (D)의 메커니즘입니다 — 임계값을 휩쓸어 선택한 목적을 최대화하는 운영점을 고릅니다.
- 섹션 3의 회귀 지표는 (E)에 직접 매핑됩니다.
