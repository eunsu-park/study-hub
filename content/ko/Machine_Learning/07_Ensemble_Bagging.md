...
"군중의 지혜" 직관은 은유 이상입니다 — 독립적 추정량들의 평균화에 관한 정확한 통계 정리에서 따라옵니다. 배깅은 부트스트랩 표본추출로 그 정리를 운영화하고, 랜덤 포레스트는 한 가지 추가 무작위화 단계로 그것을 확장합니다. 둘 다 이해하는 것이 왜 이 방법들이 표 형식 데이터의 *기본*인지 보는 가장 쉬운 방법입니다.
...
### C. 랜덤 포레스트: 두 번째 비상관화(decorrelation) 원천
...
- 섹션 1의 배깅 vs 부스팅 vs 스태킹 구분은 (A)와 (E)의 분산 vs 편향 구분입니다.
- 섹션 2의 부트스트랩 표본 루프는 (B)의 행 재표본추출 단계입니다; `RandomForestClassifier(oob_score=True)`가 보고하는 OOB 오차는 (B)의 무료 추정량입니다.
- 섹션 3의 `RandomForestClassifier(max_features='sqrt')`는 (C)의 `m_try` 비상관화 단계입니다.
- 섹션 3의 `n_estimators` 매개변수는 (A) 분산 공식의 `M`입니다 — `M`이 `(1-ρ)σ²/M`이 `ρσ²`에 비해 무시할 만한 지점을 넘으면 수익 체감.
- 섹션 4의 `voting='hard'` vs `voting='soft'` 토글은 (D)의 선택입니다.
