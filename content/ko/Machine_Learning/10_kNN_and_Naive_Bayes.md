...
k-NN과 나이브 베이즈가 같은 페이지에 있는 이유는 편향-분산 스펙트럼의 정반대 끝을 보여주기 때문입니다: k-NN은 편향 0과 무한 분산, 나이브 베이즈는 높은 편향과 매우 낮은 분산. 각각이 작동하는(그리고 어디서 실패하는) 이유의 수학은 잘 사용하기 위해 필수적입니다.

### A. k-NN: 학습 단계가 없는 비모수 추정량
...
### D. 나이브 베이즈: 강한 조건부 독립 가정과 함께한 베이즈 정리
...
- 섹션 1.1의 "게으른 학습" 표현은 (A)의 학습 단계 없음 성질입니다.
- 섹션 1.3의 거리 측정 예제는 (B)의 민코프스키 가족; 적합 전 표준화 단계는 (B)의 차원-저주 완화입니다.
- 섹션 1.4의 `KNeighborsClassifier(algorithm='kd_tree' | 'ball_tree' | 'brute')`는 (C)의 자료구조를 선택합니다.
- 섹션 2의 `GaussianNB`, `MultinomialNB`, `BernoulliNB`는 (E)의 세 변형입니다.
- Multinomial/Bernoulli NB의 `alpha` 매개변수는 (F)의 라플라스 평활 매개변수입니다.
- 끝의 "k-NN은 비선형 경계에 좋고, NB는 텍스트에 좋다" 비교는 (G)의 편향-분산 위치 요약입니다.
