...
이 레슨의 차원 축소 세 가족 — PCA, t-SNE, UMAP — 은 고차원 데이터에 대한 세 가지 다른 질문에 답합니다. PCA는 전역 선형 구조(분산)를 보존합니다. t-SNE는 전역 기하학을 희생하고 국소 이웃을 보존합니다. UMAP은 퍼지 위상(fuzzy topology) 프레임워크로 둘을 균형 잡습니다. 각각이 어떤 질문에 답하는지 아는 것이 어떤 것을 사용할지 알려줍니다.

### A. PCA: 공분산 행렬의 고유값 분해
...
### D. PCA vs t-SNE vs UMAP: 언제 어떤 것을 사용할까
...
- 섹션 2의 `PCA(n_components=r)`는 (A)와 (A.1)의 SVD 기반 PCA를 실행; `pca.explained_variance_ratio_`는 (A.2)의 공식.
- 섹션 3의 `TSNE(perplexity=...)`는 (B)의 알고리즘; `perplexity`는 (B)의 대역폭 통제 매개변수.
- 섹션 4의 `UMAP(n_neighbors=..., min_dist=...)`는 (C)의 퍼지 단순 복합체 알고리즘.
- 어떤 전처리 단계에서든 PCA 전 표준화(`StandardScaler`)는 (A.2)의 스케일 균등화 경고.
