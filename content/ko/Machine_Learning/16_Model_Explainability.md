...
**이전**: [피처 엔지니어링](./15_Feature_Engineering.md) | **다음**: [불균형 데이터 처리](./17_Imbalanced_Data.md)
...
1. 내재적 해석 가능성(intrinsic interpretability)과 사후 설명 가능성(post-hoc explainability) 방법을 구별할 수 있습니다
2. 불순도 기반(impurity-based) 피처 중요도와 순열 기반(permutation-based) 피처 중요도를 비교하고 각각의 한계를 파악할 수 있습니다
3. 샤플리값(Shapley values) 이론을 설명하고 SHAP을 적용하여 전역(global) 및 지역(local) 설명을 생성할 수 있습니다
4. LIME을 구현하여 개별 예측을 설명하고 SHAP 결과와 비교할 수 있습니다
5. 부분 의존도 플롯(Partial Dependence Plots), ICE 플롯, ALE 플롯을 해석하여 피처 효과를 시각화할 수 있습니다
6. 설명 가능성 기법을 적용하여 ML 모델의 공정성(fairness) 및 편향(bias) 문제를 탐지할 수 있습니다
7. 예측과 함께 사람이 읽을 수 있는 근거를 반환하는 프로덕션 모델용 설명 가능성 래퍼(wrapper)를 설계할 수 있습니다

---

정확한 예측을 하지만 설명할 수 없는 모델은 고위험 분야에서 큰 부담이 됩니다. 규제 기관은 설명을 요구하고(GDPR 제22조, EU AI 법), 임상의는 블랙박스를 신뢰하지 않으며, 데이터 과학자는 조용히 실패하는 모델을 디버깅해야 합니다. 이 레슨은 SHAP, LIME, PDP, ALE 등의 도구를 통해 블랙박스 모델을 열어 모델이 실제로 무엇을 학습했는지 이해하고, 불투명한 예측을 실행 가능한 인사이트로 전환할 수 있게 해줍니다.

---

## 이론과 원리

이 레슨의 네 가지 설명 가능성 방법 — SHAP, LIME, PDP, ICE — 은 진정 다른 네 가지 질문에 답하며, 매우 다른 네 가지 수학적 토대를 가집니다. SHAP은 협력 게임 이론에서 오며 네 가지 공리로 유일하게 특성화됩니다. LIME은 블랙박스를 국소적으로 선형 모델로 근사합니다. PDP/ICE는 한계 효과를 직접 계산합니다. 어떤 방법이 어떤 질문에 답하는지 아는 것이 가장 흔한 설명 가능성 실수를 막습니다: 전역 설명을 국소인 것처럼 읽거나 그 반대.

### A. SHAP과 Shapley 값 공리

Shapley 값은 협력 게임 이론(Shapley, 1953)에서 옵니다. 설정: `p`명의 플레이어가 협력하여 보상 `v(S)`(플레이어 부분집합 `S`의 "가치 함수")를 생산. 총 보상 `v(N)`을 `p`명의 플레이어에게 어떻게 공정하게 분배할까요?

Shapley는 네 가지 공리를 만족하는 *유일한* 할당 `φ_i`가 있음을 증명했습니다:

1. **효율성(Efficiency)**: 할당의 합이 총합. `Σ φ_i = v(N) - v(∅)`.
2. **대칭성(Symmetry)**: 두 플레이어가 교환 가능하면(모든 `S`에 대해 `v(S ∪ {i}) = v(S ∪ {j})`), 같은 할당을 받음.
3. **더미(Dummy, 또는 영 플레이어)**: 어떤 연합에도 아무것도 추가하지 않는 플레이어의 할당은 0.
4. **가산성(Additivity)**: 두 게임 `v`와 `w`에 대해, `φ_i(v + w) = φ_i(v) + φ_i(w)`.
...
피합산항은 연합 `S`에 대한 플레이어 `i`의 *한계 기여*이며, `i`가 도착할 때 이미 존재하는 집합으로 `S`를 만드는 순서의 수로 가중.

**SHAP**(SHapley Additive exPlanations, Lundberg & Lee, 2017)은 이를 ML에 해석합니다: 플레이어는 특성, "보상"은 모델 예측, "연합"은 값이 관찰된 특성의 부분집합(나머지는 데이터 분포에 대해 주변화). 특성 `i`와 특정 입력 `x`에 대한 Shapley 값 `φ_i`는 특성이 설명에 추가될 수 있는 모든 가능한 순서에 걸쳐 공정하게 분배된, 그 예측에 대한 특성 `x_i`의 평균 기여입니다.

네 가지 공리가 SHAP에 이론적 보장을 줍니다:
- 효율성 ⟹ 설명이 완전(합이 예측 빼기 베이스라인과 같음).
- 대칭성 ⟹ 동일한 특성이 동일한 귀속을 받음.
- 더미 ⟹ 진정 무관한 특성은 0 귀속을 받음.
- 가산성 ⟹ 앙상블 설명은 기저 모델 설명의 합.

비용: `φ_i`를 정확히 계산하려면 `2^p` 부분집합에 걸쳐 합산 — 특성 수에 지수적. SHAP의 실용적 기여는 *효율적 근사* 가족: TreeSHAP(트리 앙상블에 대해 `O(TLD²)`로 정확), KernelSHAP(모델 무관, 표본 추출과 가중 최소제곱 사용), DeepSHAP(딥넷에서 귀속을 전파하기 위해 역전파 사용).

### B. LIME: 국소 선형 대리

LIME(Local Interpretable Model-agnostic Explanations, Ribeiro et al., 2016)는 완전히 다른 접근. 공리적 귀속 대신, LIME은 블랙박스 모델을 해석 가능한 대리(surrogate)로 국소적으로 *근사*합니다.

설명할 단일 인스턴스 `x`에 대해:

```
1. x 주변의 교란된 표본 z_1, ..., z_K 생성         (특성의 무작위 마스킹)
2. 각각 예측: f(z_k)
3. x와의 근접도로 각각 가중: w_k = π_x(z_k)        (예: RBF 커널)
4. 가중치 w_k로 (z_k, f(z_k))에 희소 선형 모델 g (LASSO) 적합
5. g의 계수가 국소 특성 귀속
```

LIME의 주장: `x`의 충분히 작은 이웃에서, 복잡한 모델조차 대략 선형으로 행동. 적합된 대리 `g`는 국소 선형 근사. 계수가 직접 해석 가능.

강점:
- 진정 모델 무관(`f(·)`만 필요, 그래디언트 불필요).
- 표 형식, 이미지(슈퍼픽셀 마스크), 텍스트(토큰 존재) 입력에 균일하게 작동.
- 인스턴스당 저렴.

약점:
- 교란 분포와 근접도 커널에 민감 — 다른 선택이 다른 설명을 줌.
- 국소성이 휴리스틱; "충분히 작은 이웃"이 형식화되지 않음.
- 무작위 교란이 다르므로 가까운 두 인스턴스가 매우 다른 LIME 설명을 받을 수 있음.

LIME과 SHAP은 같은 인스턴스에 대해 종종 동의하지 않습니다. 일관성과 이론적 보장이 중요할 때 SHAP이 선호; 속도와 구현 용이성이 지배할 때 LIME이 선호.

### C. PDP: 특성의 한계 효과

특성 `j`에 대한 **부분 의존도 그래프(partial dependence plot, PDP)**는 다른 모든 특성을 주변화한 채 `x_j`의 함수로 평균 예측을 보여줍니다:

```
PDP_j(v) = (1/N) · Σ_{i=1..N}  f(x_i의 x_{i,j}를 v로 대체)
```

수학적으로: `PDP_j(v) = E_{X_{-j}}[f(v, X_{-j})]`. 특성 `j`의 값 `v`를 고르고, 모든 학습 예제의 `j`번째 특성을 `v`로 설정하고, 모델의 예측을 평균. 곡선을 얻기 위해 특성의 범위에 걸쳐 `v`를 휩씁니다.

PDP는 특성의 **평균 전역 효과**를 보여줍니다. 유용함: "소득이 증가하면 예측 이탈률이 올라가는가 내려가는가?"

두 가지 잘 알려진 약점:

1. **이질성을 숨김.** 모델이 인구의 절반에 강하게 양의 효과를, 다른 절반에 강하게 음의 효과를 할당해도 평균이 평평한 선이 될 수 있음. 평균이 상쇄.
2. **상관 특성 하에서 깨짐.** `x_j`가 `x_k`와 상관되면, `x_k`를 관찰된 값에 두고 `x_j = v`로 설정하면 데이터 분포에 존재하지 않을 수 있는 행이 생성. PDP가 모델이 학습된 적 없는 영역에서 모델을 질의 — 외삽 잡음.

### D. ICE: 평균 없는 PDP

**개별 조건부 기댓값(Individual Conditional Expectation, ICE)** 그래프는 평균화 단계 없는 PDP:

```
각 개체 i에 대해, v가 휩씀에 따라 f(x_i의 x_{i,j}를 v로 대체)를 그림.
```

개체당 한 곡선을 봅니다. PDP는 이 곡선들의 평균. ICE는 PDP가 숨기는 이질성을 노출:
- 모든 곡선이 평행하고 비슷한 모양 ⟹ 특성이 균일한 효과를 가짐, PDP가 충실.
- 곡선이 교차하거나 발산 ⟹ 효과가 인구 전반에 걸쳐 변동, PDP가 오해의 신호를 평균.

상관 특성 왜곡에 대한, **ALE(Accumulated Local Effects)**가 원칙적 해법: 모든 곳에서 `x_j = v`로 대체하는 대신, 경험적 밀도로 가중하여 `x_j`의 범위에 걸쳐 국소 조건부 그래디언트 `∂f/∂x_j`를 적분. ALE 그래프는 비현실적 데이터 문제에 영향받지 않음.

### E. 국소 vs 전역, 그리고 "설명"의 의미

네 가지 방법이 두 축을 따라 분리됩니다:

|  | 인스턴스별(국소) | 데이터셋 전반(전역) |
|---|------------------|---------------------|
| 귀속 | SHAP(인스턴스별 값), LIME | SHAP(평균 절댓값 특성별) |
| 함수 모양 | ICE(인스턴스별 곡선) | PDP, ALE |

"특성 `j`가 가장 중요하다" 같은 전역 설명은 모델이 특성 `j`를 *어떻게* 사용하는지(단조? 비단조? `k`와 상호작용?)를 알려주지 않음. "*이* 고객에 대해 나이가 이탈 예측에 +12% 기여" 같은 국소 설명은 나이가 전체 인구에 걸쳐 중요한지 알려주지 않음. 둘 다 사용.

흔한 실수: 특성-중요도 막대 차트(전역)를 개별 결정을 설명하는 것처럼(국소) 읽는 것. 거부된 대출에 대한 규제 기관용 보고서에는 인스턴스별 SHAP 설명이 필요하지 전역 특성-중요도 순위가 아닙니다.

### F. 설명 가능성의 비용

각 방법이 추론 비용을 추가:
- TreeSHAP: 인스턴스당 `O(TLD²)`, 트리 모델에 대해 종종 가장 저렴한 정확 옵션.
- KernelSHAP: 인스턴스당 `O(K · M)`, `K`는 표본 수, `M`은 정방향 패스 비용. 큰 `M`에 느림.
- LIME: 인스턴스당 `O(K · M)`, KernelSHAP과 비슷.
- PDP / ICE: `N`개 학습 예제와 `G`개 그리드 점에 대해 `O(N · G)`. 특성당 일회성; 저렴.

프로덕션 시스템은 대표 표본에 대해 SHAP 값을 오프라인으로 사전 계산하고, 질의 시점에 가장 가까운 사전 계산된 설명을 제공하는 경우가 많습니다.

### From Theory to the Code Below

- 섹션 2의 `shap.TreeExplainer(model)`는 (A)의 TreeSHAP 사용; `shap.KernelExplainer`는 KernelSHAP 사용. 폭포 그래프는 (A)의 특성별 `φ_i`를 시각화.
- 섹션 3의 `lime.lime_tabular.LimeTabularExplainer`는 (B)의 교란-적합 절차를 실행.
- 섹션 4의 `partial_dependence`와 `PartialDependenceDisplay`는 (C)의 PDP를 계산하고 그림.
- 같은 `PartialDependenceDisplay(kind='individual')`이 (D)의 ICE 곡선을 만듦.
- 끝의 "특성 중요도" 막대 차트는 (E)의 전역 집계; SHAP의 `mean(|φ_i|)`이 그것의 원칙적 버전.
...
- **[Interpretable_AI](../Interpretable_AI/00_Overview.md)**: 고급 해석 가능성 — 그래디언트 어트리뷰션, GradCAM, 어텐션 분석, TCAV, 반사실적 설명, 인과적 공정성, 기계적 해석 가능성 및 AI 거버넌스
