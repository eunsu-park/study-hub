...
시계열 ML은 다른 종류의 모델이 아닙니다 — 다른 종류의 *데이터 준비*입니다. 데이터에 시간적 순서가 있어 다른 모든 레슨의 i.i.d. 가정을 깨고, 그 순서가 특성 공학, 검증, 그리고 어떤 모델이 적절한지에 특정한 제약을 부과합니다. 이 섹션은 그 제약을 처음부터 못박습니다.

### A. 정상성: 통계적 토대

시계열 `{y_t}`가 시간 이동 하에서 결합 분포가 불변일 때 **엄격 정상(strictly stationary)**:

```
P(y_{t_1}, y_{t_2}, ..., y_{t_n})  =  P(y_{t_1 + h}, y_{t_2 + h}, ..., y_{t_n + h})  모든 h에 대해
```

엄격 정상성은 실전에 너무 강함. **약(공분산) 정상성**이 작업 정의:

1. `E[y_t] = μ` (일정한 평균)
2. `Var[y_t] = σ²` (일정한 분산)
3. `Cov(y_t, y_{t+h}) = γ(h)` (자기공분산이 절대 시간이 아니라 시차 `h`에만 의존)

대부분의 고전 ML은 데이터가 정상적이거나 그렇게 만들 수 있다고 가정. 두 가지 흔한 변환:

- **차분(Differencing)** `Δy_t = y_t - y_{t-1}`이 추세를 제거. ARIMA(p, d, q)의 "I"가 `d`차 차분.
- **로그-후-차분**이 곱셈 계절성을 가진 시계열의 분산을 안정화.

정상성 검정(Augmented Dickey-Fuller, KPSS)이 차분할지 알려줌. 이를 건너뛰고 비정상 시계열을 ML에 공급하면, 모델이 추세에 매달려 무의미한 외삽을 만들 수 있음.

### B. 자기상관 함수: ACF와 PACF

시차 `h`에서의 **자기상관 함수(ACF)**:

```
ACF(h) = γ(h) / γ(0) = Cov(y_t, y_{t+h}) / Var(y_t)
```

ACF는 `y_t`가 `y_{t-h}`와 얼마나 강하게 상관되는지 측정 — `y_{t-1}, ..., y_{t-h+1}`을 통한 모든 간접 상관 포함.

시차 `h`에서의 **편자기상관 함수(PACF)**는 `y_{t-1}, ..., y_{t-h+1}`의 선형 효과를 *제거한 후* `y_t`와 `y_{t-h}` 사이의 상관. 중간 시차에 의해 매개되지 않는 직접 상관.

ACF/PACF 그래프 읽기가 어떤 시차 특성이 중요한지 알려줌:
- ACF가 시차 `q`에서 끊김 ⟹ 순수 MA(q) 과정.
- PACF가 시차 `p`에서 끊김 ⟹ 순수 AR(p) 과정.
- 둘 다 천천히 감쇠 ⟹ ARMA(p, q).

ML 특성 공학에 PACF가 더 나은 가이드: 더 짧은 시차가 이미 제공하는 것 *너머의* 정보를 어떤 시차 특성이 추가하는지 알려줌.

### C. 한 식의 ARIMA

시계열 `y_t`에 대한 ARIMA(p, d, q):

```
1. d번 차분:   z_t = Δ^d y_t
2. z_t에 ARMA(p, q) 적합:
       z_t = c + φ_1 z_{t-1} + ... + φ_p z_{t-p}    ← AR(p): 자기회귀
                + ε_t + θ_1 ε_{t-1} + ... + θ_q ε_{t-q}  ← MA(q): 이동 평균
```

시계열의 `p`개 시차 값(AR), `d`차 차분(I), `q`개 시차 잔차(MA). 매개변수는 최대가능도로 추정. SARIMA는 주기적 패턴을 위해 `s`-주기 시차의 계절 `(P, D, Q, s)` 블록 추가(연간 계절성을 가진 월별은 s=12, 주간을 가진 일별은 s=7).

ARIMA는 선형, 단변량, 가우시안 잔차를 가정. 해석성과 이론적 토대는 비교 불가지만, 그 경직성 때문에 데이터가 풍부할 때 ML 접근이 지배.

### D. Prophet: 분해 가능한 가산 모델

Prophet(Facebook, 2017)은 가산 분해:

```
y(t) = g(t) + s(t) + h(t) + ε_t
```

- `g(t)`: 추세(자동 변경점 탐지가 있는 조각별 선형 또는 조각별 로지스틱).
- `s(t)`: 계절성(주간, 연간, 사용자 정의 주기에 대한 푸리에 급수).
- `h(t)`: 휴일(효과를 가진 사용자 제공 특별 날짜 목록).
- `ε_t`: 잡음.

각 성분이 베이지안 추론(내부적으로 Stan)으로 적합. 결과는 해석 가능, 결측 데이터와 이상치에 강건, 비즈니스 시계열에 즉시 작동. 트레이드오프: 관계가 많은 특성의 복잡한 비선형 상호작용일 때 그래디언트 부스팅 ML보다 훨씬 경직.

### E. 무작위 K-폴드를 사용할 수 없는 이유

표준 K-폴드는 데이터를 무작위로 폴드에 섞음. 시계열에서 이는 *재앙적* 누수: 학습 폴드가 시간 `t_1, t_3, t_5, ...`의 예제를 포함하고 검증 폴드가 `t_2, t_4, t_6, ...`을 포함. 모델이 검증셋을 예측할 때 *미래* 데이터에 접근 — 배포될 방식의 정확히 반대.

시계열의 무작위 K-폴드의 CV 오차는 위쪽으로 편향(현실보다 더 좋아 보임), 모델이 이미 본 학습 예제 사이를 본질적으로 보간하기 때문. 프로덕션 성능은 훨씬 더 나쁠 것.

### F. Walk-Forward 검증: 올바른 CV

시계열 CV는 배포를 모방해야 함: 과거에 학습, 미래에 테스트. 두 가지 동등한 방식:

- **확장 윈도우(Expanding window)**: 학습셋이 시간에 걸쳐 자람. 폴드 `k`가 `[1..t_k]`에서 학습하고 `[t_k+1..t_k+h]`에서 테스트.
- **롤링 윈도우(Rolling window)**: 학습셋이 앞으로 미끄러지는 고정 길이 윈도우. 폴드 `k`가 `[t_k-W..t_k]`에서 학습하고 `[t_k+1..t_k+h]`에서 테스트.

확장 윈도우는 폴드당 더 많은 데이터 사용; 롤링 윈도우는 모델이 일정 크기의 최근 슬라이스에서 학습하도록 강제(분포가 표류할 때 더 나음).

미묘한 함정: 학습 종료와 검증 시작 사이의 **간격(gap)**. 예측 지평선이 `h`라면, 학습과 검증 사이에 크기 `h`의 간격 둠 — 그렇지 않으면 모델이 배포에서 아직 사용 가능하지 않을 정보에 접근.

scikit-learn의 `TimeSeriesSplit`이 확장 윈도우 CV를 구현; `gap`과 `max_train_size` 매개변수가 변형 통제.

### G. 시차, 롤링, 달력 특성

시간적 CV 규율을 받아들이면, ML이 시계열을 세 특성 가족을 통해 표 형식 문제로 환원:

- **시차 특성(Lag features)**: `y_{t-1}, y_{t-2}, ..., y_{t-L}`. PACF가 어떤 시차가 중요한지 알려줌.
- **롤링 윈도우 특성**: `mean(y_{t-W..t-1}), std(y_{t-W..t-1}), max(...), min(...)`. 최근 이력을 작은 요약 집합으로 집계.
- **달력 특성**: `day_of_week, month, day_of_month, is_holiday, days_since_last_event`. 원시 타임스탬프가 유용한 범주형 예측자로 변환.

이들 각각은 과거 데이터만 사용하여 행별로 독립적으로 계산할 수 있어 누수 문제 없음 — 시간적 분할 후 파이프라인 안에서 계산하는 *한*.

더 미묘한 함정: **시간 블록별 타깃 인코딩**. 인코딩되는 행 이후의 행을 포함한 모든 학습 행을 사용하여 `month`를 타깃 인코딩하면, 미래 정보를 누설. 인코딩 시 과거 행만 사용.

### H. 다단계 예측 전략

`h > 1` 단계 앞을 예측하는 세 가지 접근:

1. **재귀(Recursive, 일단계-앞 반복)**: 이력에서 `t+1`을 예측하는 한 모델 학습, 그다음 그 예측을 입력으로 다시 공급하여 `t+2` 예측 등. 오류가 누적.
2. **직접(Direct, 지평선당 한 모델)**: 고정된 `k`에 대해 `y_{t+k}`를 예측하는 `h`개 별도 모델 학습. 오류 누적 없음, 단 유지할 모델 `h`개.
3. **다출력(Multi-output, 단일 모델, h 출력)**: 출력이 `(y_{t+1}, ..., y_{t+h})`인 한 모델 학습. 지평선 간 상관 포착; 일부 프레임워크에서 사용 가능(Prophet, 신경망).

직접이 적당한 `h`에 가장 안전한 기본; 재귀는 지평선과 함께 더 나쁘게 스케일; 다출력은 지원할 수 있을 때 최선.

### From Theory to the Code Below

- 섹션 1의 "ML vs 고전" 표현은 (A)–(D)의 이분법.
- 섹션 2의 `statsmodels`의 `adfuller` 검정은 (A)의 정상성 점검; 차분 변환도 거기.
- 섹션 3의 시차/롤링/달력 특성 공학은 정확히 (G); 섹션 3의 ACF/PACF 그래프는 (B)의 진단.
- 섹션 4의 `TimeSeriesSplit(n_splits=K, gap=...)`는 (F)의 walk-forward CV.
- 섹션 5의 재귀 예측 루프는 (H.1)의 전략; 다중 모델 접근은 (H.2).
- 섹션 6의 Prophet 예제는 (D)의 가산 분해 모델; XGBoost/LightGBM과의 비교는 (D)의 경직성-vs-유연성 트레이드오프.
