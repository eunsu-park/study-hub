...
**이전**: [파이프라인과 실무](./13_Pipelines_and_Practice.md) | **다음**: [피처 엔지니어링](./15_Feature_Engineering.md)
...
1. 문제 정의부터 배포까지 머신러닝 프로젝트의 전체 워크플로우(end-to-end workflow)를 설명할 수 있습니다
2. EDA, 특성 엔지니어링(feature engineering), 모델 비교, 하이퍼파라미터 튜닝을 포함한 완전한 분류 프로젝트(타이타닉 생존 예측)를 구현할 수 있습니다
3. 잔차 분석(residual analysis)과 특성 중요도(feature importance)를 포함한 완전한 회귀 프로젝트(주택 가격 예측)를 구현할 수 있습니다
4. K-겹 교차 검증(K-Fold cross-validation)과 OOF(out-of-fold) 예측 생성을 포함한 교차 검증 전략을 적용할 수 있습니다
5. 블렌딩(blending)과 스태킹(stacking) 기법을 사용하여 앙상블 예측(ensemble prediction)을 구축할 수 있습니다
6. 분류에 적합한 지표(F1, AUC)와 회귀에 적합한 지표(RMSE, R2, MAE)로 모델을 평가할 수 있습니다
7. 예측 오류를 식별하고 분석하여 반복적인 모델 개선을 유도할 수 있습니다

---

알고리즘과 이론은 실제 문제에 처음부터 끝까지 적용할 수 있을 때만 의미가 있습니다. 이 레슨은 EDA, 전처리, 모델 선택, 튜닝, 평가, 앙상블을 두 개의 완전한 프로젝트를 통해 모두 통합합니다 -- Kaggle 대회나 실제 비즈니스 환경에서 따를 워크플로우를 그대로 반영합니다. 여기서 연습하는 패턴들은 모든 응용 ML 프로젝트의 토대가 됩니다.

---

## 이론과 원리

실용 ML 프로젝트는 단순한 API 호출 시퀀스가 아닙니다 — 일련의 아키텍처적 트레이드오프이며, 각각이 편향-분산 렌즈, no-free-lunch 현실, 이전 레슨의 누수 규칙에 의해 제약됩니다. 실용 프로젝트의 "이론"은 기본값이 조용히 그것들을 고르도록 두는 대신 그 트레이드오프를 명시적으로 만드는 규율입니다.

### A. ML 프로젝트의 비용 구조

모든 프로젝트는 같은 대략적 비용 모양을 가집니다:

```
단계별 노력 예산 (Andrew Ng, 대략적 업계 규칙):
- 문제 정의:                    ~5%
- 데이터 수집/정제:             ~50-70%
- EDA:                          ~10%
- 모델링(학습/튜닝):           ~10-20%
...
잔혹한 비대칭은 60–80%의 작업이 "모델링"이 전혀 아니라는 것입니다. 데이터 품질이 병목이라면, XGBoost를 더 화려한 모델로 바꿔도 얻는 것이 없습니다. 첫 아키텍처적 질문은 항상: *한 시간의 한계 노력이 어디서 가장 멀리 가는가?*

### B. 베이스라인을 먼저 만드는 이유

모든 프로젝트의 첫 번째 모델은 작동하는 가장 멍청한 모델이어야 합니다:
- 분류 베이스라인: 다수 클래스 예측(또는 층화 무작위).
- 회귀 베이스라인: 평균 예측(또는 중앙값).
- 약간 더 나은 베이스라인: 비공학 특성에 단일 선형/로지스틱 모델.

이 베이스라인은 진지한 경쟁자가 아닙니다 — **모든 진지한 모델이 이겨야 할 바닥**입니다. 두 가지 이유:
1. **파이프라인의 정신 점검.** 화려한 모델이 어떻게든 다수 클래스 예측보다 *더 나쁘게* 하고 있다면, 버그가 있습니다 — 누수 버그, 타깃 열 뒤집기, 또는 학습/테스트 불일치 — 어떤 튜닝이 의미가 있기 전에 고쳐야 합니다.
2. **ROI의 기준점.** 베이스라인 위 1% 정확도 개선은 이미 강한 모델 위 1% 개선과 같지 않습니다. 다음 반복이 노력할 가치가 있는지 알기 위해 베이스라인이 필요합니다.

Lesson 1의 편향-분산 분해가 베이스라인이 트레이드오프의 어느 쪽에서 실패하는지 알려줍니다. 과소적합(높은 편향) ⟹ 더 유연한 모델 시도. 과적합(높은 분산, 학습 >> 테스트) ⟹ 정칙화, 더 많은 데이터, 또는 단순화.

### C. 누수 감사

모든 변환은 "학습에서만 적합" vs "한 번에 전역적으로 계산"으로 분류되어야 합니다. 이를 잘못하면 CV 점수는 허구입니다.

**파이프라인 안에 있어야 하는** 것들(학습 폴드에서만 적합):
- 모든 스케일링과 정규화
- 모든 결측치 대치(평균, 중앙값, KNN, 모델 기반)
- 범주형 특성의 타깃 인코딩
- 특성 선택(분산 임계값, 상호 정보, 모델 기반)
- 차원 축소(PCA 등)
- 클래스 균형(SMOTE, 언더샘플링 — 학습/테스트 분할 후여야 함)

**전역적으로 계산해도 안전한** 것들:
- 인코더가 고정 스키마를 사용한다면(`handle_unknown='ignore'`) 원-핫 인코딩
- 상수 변환(log, sqrt, log1p)
- 행 자체만 사용하는 손코딩 특성 공학

규율은: 변환이 다른 어떤 행의 *값*(평균, 중앙값, 백분위수, 이웃 거리)을 읽는다면, 파이프라인 안에 있어야 합니다. 예외 없이.

### D. 편향-분산 탐색으로서의 모델 선택

Lesson 1의 "no free lunch" 정리는 어떤 단일 모델도 모든 문제에 최선이 아니라고 합니다. 실용 전략은 트레이드오프 공간을 매핑하는 것:

| 편향-분산 위치 | 먼저 시도 |
|----------------|-----------|
| 작은 `N`, 단순 구조 | 정칙화된 선형 / 로지스틱 회귀 |
| 표 형식 데이터, 혼합 타입 | Gradient Boosting (XGBoost / LightGBM) |
| 고차원, 희소, 텍스트 | 선형 모델, Naive Bayes |
| 이미지/오디오/텍스트 원시 신호 | 사전학습 딥 모델 + 파인튜닝 |
| 이해관계자 위한 해석성 필요 | 로지스틱 회귀, 단일 결정 트리, EBM |
| 최대 정확도 필요, 해석은 부차적 | Boosting + Stacking + 신중한 CV |

순서가 중요: 저렴하게 시작, 베이스라인까지의 격차 측정, 저렴한 것이 의미 있는 격차를 남길 때만 더 유연한(그리고 튜닝 비싼) 모델로 에스컬레이션.

### E. CV 전략이 곧 아키텍처

Lesson 5가 CV 변형을 도입했고; 여기는 프로젝트 맥락에서 어떤 것을 언제 사용할지:

- **Stratified K-fold (5 또는 10)**: 균형 잡힌 분류의 기본.
- **Stratified K-fold + `class_weight='balanced'`**: 적당한 불균형(10:1에서 100:1).
- **Stratified K-fold + 파이프라인 내 SMOTE**: 심각한 불균형(>100:1) — CV 바깥에서 절대 SMOTE 안 함.
- **Group K-fold**: 같은 출처(사용자, 환자, 문서)의 행이 반복적으로 나타날 때.
- **시계열 분할(확장/롤링 윈도우)**: 데이터가 시간적이고 배포가 시간 앞으로 갈 때.
- **중첩 CV(외부 5, 내부 3 또는 5)**: 컴퓨팅이 있고 모델 간 비교가 통계적으로 깨끗해야 할 때만.

잘못된 CV 전략을 고르는 것이 "노트북에서 훌륭해 보이는" 모델이 프로덕션에서 나쁘게 수행하는 가장 흔한 이유입니다. CV 분할은 배포를 모방해야 합니다.

### F. 스태킹과 수익 체감 곡선

여러 합리적인 모델이 있으면 스태킹(Lesson 21)이 1–3% 더 정확도를 짜낼 수 있습니다. 하지만 한계 비용이 빠르게 자랍니다 — 유지할 모델 더 많아짐, 더 많은 추론 지연시간, 버그가 있을 곳 더 많아짐. 올바른 질문은 "스태킹할 수 있는가?"가 아니라 "이득이 복잡성의 가치가 있는가?"

간단한 결정 규칙:
- 단일 최선 모델: 정확도 목표를 충족하면 사용.
- 다른 귀납 편향을 가진 상위 3개의 평균(예: LightGBM + Random Forest + 공학 특성에 로지스틱 회귀): 저렴한 다양화, 거의 항상 도움.
- 메타 학습기를 사용한 완전 스태킹: 이전 단계가 여전히 의미 있는 격차를 남길 때만.

각 단계가 운영 비용을 추가; 정확도 예산이 요구할 때만 사용.

### From Theory to the Code Below

- 섹션 1의 워크플로우 다이어그램은 (A)의 비용 구조; 시간 할당 제안은 거기 경험칙을 반영.
- 섹션 2의 "베이스라인 먼저 수립" 단계는 (B)의 규율.
- 섹션 3의 `Pipeline` + `ColumnTransformer` 설정은 (C)의 누수 감사 체크리스트가 코드로 구현된 것.
- 섹션 4의 모델 비교 루프(Logistic, RF, XGBoost, LightGBM, ...)는 (D)의 편향-분산 탐색.
- 섹션 5의 CV 설정은 (E)의 규칙을 따름; `StratifiedKFold`와 `GroupKFold` 사이의 선택은 의도적이어야지 기본값이 아니어야 함.
- 섹션 6의 투표/스태킹 실험은 (F)의 수익 체감 보정.
