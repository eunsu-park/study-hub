...
결정 트리는 if-else 문의 더미처럼 보이지만, 그것을 만드는 알고리즘은 진지한 수학을 합니다: 모든 노드에서 최적화 문제(어떤 분할이 불순도를 가장 많이 줄이는가)를 풀고, 그 국소 해를 결합하는 재귀 절차가 **CART** 알고리즘입니다. 트리를 잘 사용하려면 — 그리고 특히 트리 앙상블이 표 형식 데이터를 지배하는 이유를 이해하려면 — 형식적 그림이 필요합니다.

### A. 분할 기준: 정보 이득과 지니

노드에서의 분할은 데이터를 자식 집합 `L`과 `R`로 분할합니다. "각 자식의 데이터가 부모보다 더 순수한가?"를 말해주는 스칼라가 필요합니다. 두 가지 표준 불순도 측정이 지배적이며, 둘 다 노드의 클래스 분포에서 만들어집니다.

`p_k`를 노드의 클래스 `k`의 비율이라 합시다. 그러면:

```
엔트로피(Entropy):    H = - Σ_k p_k · log₂(p_k)            ← 비트(bits) 단위
지니 불순도(Gini):    G = 1 - Σ_k p_k² = Σ_{k ≠ k'} p_k · p_{k'}
```

둘 다 한 클래스가 지배할 때(`p_k = 1` for some `k`) 0이고, 클래스가 균일할 때 최대입니다. 분할의 **정보 이득(information gain)**은 기대 불순도 감소입니다:

```
IG(parent → L, R) = H(parent) - [ |L|/|parent| · H(L) + |R|/|parent| · H(R) ]
```

(`H` 대신 `G`를 써서 지니 이득 사용.) 탐욕적(greedy) CART 알고리즘은 각 노드에서 후보 분할을 열거합니다 — 각 수치 특성에 대해 인접 정렬 값 사이의 모든 중간점; 각 범주형 특성에 대해 모든 이진 분할 — 그리고 최대 이득을 가진 것을 고릅니다.

### A.1 지니 vs 엔트로피: 실용적 차이

둘은 실전에서 거의 동등합니다. 둘 다 `p`의 오목 함수, 둘 다 균일 분포에서 정점, 둘 다 순수 노드에서 0. 둘이 고른 수치 최적값은 대부분 일치합니다. 차이는:

- **계산 비용**: 지니는 `log`를 피하고 약간 더 빠릅니다. scikit-learn은 그 이유로 지니가 기본값.
- **클래스 확률 변화에 대한 민감도**: 엔트로피는 `p = 0` 또는 `p = 1` 근처의 변화에 더 민감합니다(`log p → -∞`이므로), 따라서 불순도를 0으로 강하게 몰고 가는 분할을 선호합니다. 지니는 더 선형적으로 자랍니다. 강하게 불균형한 데이터에서 선택이 다른 트리를 만들 수 있지만, 명확한 승자가 있는 경우는 드뭅니다.

회귀 트리는 이 기준을 노드 내 분산 `Var(y)`로 대체하고, 이득은 가중 분산 감소입니다. 같은 알고리즘, 다른 불순도.

### B. CART 알고리즘

CART(Classification And Regression Trees)는 재귀적 탐욕 절차입니다:

```
function build_tree(data):
    if stopping_criterion(data):                 # 아래 참조
...
탐욕성은 본질적이면서 한계입니다. 모든 노드에서 CART는 *국소적으로* 최선인 분할을 고르고, 결코 재고하지 않습니다. 이는 계산적으로 다루기 쉽지만 전역 최적성 보장이 없습니다 — 결과 트리는 데이터와 일관된 가장 작은 트리인 경우가 드물고, 학습셋의 작은 변화가 매우 다른 트리를 만들 수 있습니다(높은 분산).

**중지 기준(stopping criteria)**은 빌드 시점에서 과적합을 통제하는 방법입니다:
- `max_depth`: 트리 깊이의 단단한 상한
- `min_samples_split`: 이보다 적은 표본의 노드는 분할 안 함
- `min_samples_leaf`: 모든 리프는 최소 이만큼의 표본을 가져야 함
- `min_impurity_decrease`: 이득이 임계값 미만인 분할은 건너뜀

이 중 어떤 것도 없으면 CART는 모든 리프가 순수해질 때까지 분할을 계속하여 학습셋을 완벽히 적합하는 트리를 만들고 — 거의 확실히 과적합합니다.

### C. 가지치기: 비용-복잡도 이론

사전 가지치기(위 중지 기준)는 보수적입니다 — 잘못된 초기 결정에서 회복할 수 없습니다. **사후 가지치기(post-pruning)**는 트리를 완전 깊이로 키운 다음 잘라냅니다. 원칙적인 버전은 **비용-복잡도 가지치기**(Breiman et al., 1984)입니다:
...
`α ≥ 0`은 복잡도 매개변수. 각 `α`에 대해 `R_α`를 최소화하는 유일한 가장 작은 부분트리 `T_α`가 있습니다. `α`가 0에서 ∞로 증가하면 최적 부분트리는 단조롭게 축소되어, 연관된 `α` 값을 갖는 *유한한* 중첩 후보 트리 시퀀스 `T_0 ⊃ T_1 ⊃ ... ⊃ T_root`를 만듭니다. CV가 이 시퀀스에서 최선의 `α`를 고릅니다.

scikit-learn은 이를 `ccp_alpha`로 노출합니다: 가지치기를 위해 양수 값으로 설정. `α`가 클수록 결과 트리가 더 작고(편향이 더 큽니다).

### D. 트리가 유용하면서 동시에 한계가 있는 이유

트리의 강점은 알고리즘이 작동하는 방식에서 따라옵니다:

- **특성 스케일링 불필요.** 분할은 원시 특성 값의 임계값을 사용합니다; 특성을 1000배 해도 아무것도 변하지 않습니다.
- **혼합 특성 유형.** 수치와 범주형 특성이 같은 분할 기계로 처리됩니다(적절한 열거와 함께).
- **해석 가능.** 트리는 끝에서 끝까지 읽을 수 있는 흐름도; 특성 중요도가 학습의 부산물로 떨어집니다.
- **상호작용 포착.** 루트에서 리프까지의 각 경로는 조건의 연언(conjunction) — 특성 간 상호작용이 일급 객체.

약점도 마찬가지로 구조적입니다:

- **높은 분산.** 탐욕적 분할 선택이 트리를 부서지기 쉽게 만듭니다: 데이터를 부트스트랩 재표본하면 완전히 다른 트리가 나올 수 있습니다. 이것이 배깅(Lesson 7)이 발명된 *바로 그* 약점입니다.
- **축에 정렬된 분할.** 단일 결정 경계가 특성 축에 평행합니다. 매끄러운 대각선 경계는 많은 작은 단계를 요구합니다 — 선형 모델이 한 번에 적합하는 것을 근사하기 위해 깊은 트리가 필요.
- **수준이 많은 특성에 대한 편향.** 1000개 고유값을 갖는 범주형 특성은 고를 후보 분할이 더 많아 우연히 "이길" 수 있습니다. 랜덤 포레스트가 각 분할에서 특성을 부분표본화하여 부분적으로 해결합니다.

### From Theory to the Code Below

- 섹션 1.1의 트리 구조 예제는 (B)의 CART 의사코드의 재귀적 `Node`/`Leaf` 패턴입니다.
- 섹션 2의 `DecisionTreeClassifier(criterion='gini' | 'entropy')`는 (A)의 두 불순도 측정 사이의 선택입니다.
- 섹션 2의 `max_depth`, `min_samples_split`, `min_samples_leaf`는 (B)의 사전 가지치기 중지 기준입니다.
- `ccp_alpha` 매개변수(사용 시)는 (C)의 비용-복잡도 가지치기 매개변수입니다.
- 특성 중요도 점수(`model.feature_importances_`)는 각 특성이 사용된 모든 노드에서 합한 총 불순도 감소입니다 — (A)의 분할 기준의 직접적 따름정리.
