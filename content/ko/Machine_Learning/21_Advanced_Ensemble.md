...
스태킹과 블렌딩은 단순한 "다른 모델을 결합하는 모델 사용" 트릭처럼 보이지만, 그 아래의 수학이 그것이 실제로 도움이 되는지 단지 복잡성을 추가하는지를 결정합니다. 두 가지 핵심 질문: 정보를 누설하지 않고 메타 학습기를 어떻게 학습할 것인가, 그리고 어떤 종류의 기저 모델 다양성이 실제로 잘 결합하는가.
...
```
Layer 0 (기저):     M개 기저 모델  f_1, ..., f_M    각각 x → ŷ_m
Layer 1 (메타):     메타 학습기 g  (ŷ_1, ..., ŷ_M) → ŷ
```

메타 학습기는 원래 특성이 아니라 기저 모델의 *예측*에서 학습. 전체 파이프라인은 `g(f_1(x), ..., f_M(x))`. 메타 학습기는 선형(로지스틱 회귀가 가장 흔한 선택 — 해석 가능한 가중치) 또는 비선형(작은 그래디언트 부스팅 모델이 기저 예측 사이의 상호작용을 잡을 수 있음)일 수 있음.

### B. 결정적인 누수 문제

학습 데이터에서 기저 모델 `f_m`을 학습하고, 그 예측 `f_m(x_train)`을 얻은 다음, 메타 학습기를 `(f_m(x_train), y_train)`에서 학습하면, 누수 재앙을 만든 것. 기저 모델이 학습 레이블을 *외웠으므로*, 그 학습셋 예측이 비현실적으로 좋음. 메타 학습기가 그것들을 완벽히 신뢰하도록 학습. 실제 테스트 데이터에 배포하면, 기저 모델이 덜 잘 예측 — 그리고 메타 학습기가 이제 잘못 보정됨.

해법은 **폴드 외(out-of-fold, OOF) 예측**:

```
학습 데이터를 K개 폴드로 분할.
각 폴드 k에 대해:
    각 기저 모델 f_m을 다른 K-1 폴드에서 학습.
    폴드 k에 대한 예측 생성.
모든 폴드 후: 모든 학습 행이 그 행에서 *학습되지 않은*
              각 기저 모델로부터의 예측을 가짐.
이 OOF 예측 vs 진짜 레이블에 메타 학습기 학습.
```

테스트 예측의 경우: 각 기저 모델을 *전체* 학습셋에서 학습, 테스트에 예측, 그 예측을 메타 학습기에 공급. 이는 대칭적 — 배포 시, 기저 모델이 보지 못한 데이터에 예측.

scikit-learn의 `StackingClassifier`와 `StackingRegressor`가 OOF 학습을 자동으로 구현. 사용자 정의 스태킹 구현은 거의 항상 누수 버그를 가짐; 특정 이유가 없는 한 라이브러리 버전 사용.

### C. 블렌딩: 더 저렴한 사촌

**블렌딩**은 K-폴드 OOF 예측 대신 단일 학습/홀드아웃 분할을 사용하는 스태킹의 단순화된 버전:

```
학습 데이터를 A(예: 80%)와 B(예: 20%)로 분할.
각 기저 모델 f_m을 A에서 학습.
B에 대한 예측 생성: ŷ_m^{(B)} = f_m(x_B).
(ŷ_m^{(B)}, y_B)에 메타 학습기 학습.
테스트의 경우: A에서 학습된 각 f_m으로 예측, 메타 학습기에 공급.
```

트레이드오프:
- **더 저렴**: 각 기저 모델이 K번이 아니라 한 번 학습.
- **메타 학습기에 더 적은 데이터**: `|train|`이 아니라 `|B|` 예제만.
- **더 높은 분산**: 메타 학습기가 단일 잡음 분할을 봄.

블렌딩은 기저 모델 학습이 매우 비싸고(예: 신경망) 검증셋이 안정한 메타 학습기를 적합할 만큼 충분히 클 때 합리적 선택.

### D. 기저 모델이 잘 결합하게 하는 것

스태킹은 기저 모델이 충분히 *다양*하지 않으면 최선의 기저 모델 위로 개선되지 않음. 항상 동의하는 두 기저 모델은 추가 정보를 기여하지 않음.

대략적 기준: 오류가 기저 모델 전반에 걸쳐 비상관이어야 함. 유용한 다양성의 원천:
- **다른 모델 가족**: 로지스틱 회귀 + 랜덤 포레스트 + LightGBM은 매우 다른 세 가지 귀납 편향(선형, 배깅을 가진 축 정렬 분할, 부스팅을 가진 축 정렬 분할)을 줌.
- **다른 특성 관점**: 다른 특성 부분집합에 대한 같은 모델 클래스(예: 텍스트 전용 모델 + 수치 전용 모델 + 결합).
- **다른 하이퍼파라미터**: 같은 알고리즘의 더 깊은 vs 더 얕은 트리.
- **다른 학습 데이터 슬라이스**: 시간대별, 지역별, 사용자 세그먼트별.

수익 체감이 빨리 시작: 두 번째 다양한 모델을 추가해서 얻는 이득이 크고; 다섯 번째에서 한계; 열 번째에서 종종 음수, 과적합과 복잡성이 지배.

### E. 메타 학습기 선택

메타 학습기는 보통:

- **선형(Logistic / Linear Regression)**: 가장 단순, 가장 해석 가능. 계수가 각 기저 모델의 상대 가중치를 알려줌. 과적합하기 어려움. 표준 기본.
- **정칙화된 선형(Lasso, Ridge)**: 많은 기저 모델이 상관될 때, 정칙화가 가중치를 안정 유지. Lasso가 무용한 모델을 자동으로 0으로 만들 수 있음.
- **그래디언트 부스팅(작은)**: 기저 예측 사이의 비선형 상호작용 포착. 더 표현력 있지만 메타 학습기를 과적합하기 더 쉬움. 선형 메타 학습기가 명확한 이득을 보이고 비선형 상호작용을 의심할 때만 사용.
- **평균 / 가중 평균**: 정말 "학습된" 메타 학습기는 아니지만 유용한 베이스라인. 선형 메타 학습기가 단순 평균을 거의 못 이긴다면, 다양한 기저 모델이 많이 추가하지 않는 것; 스태킹이 운영 비용 가치가 있는지 고려.

특정 위험: 비선형 메타 학습기가 OOF 규율이 제거한 과적합을 다시 도입할 수 있음. 항상 보류된 집합에서 단순 평균에 대해 스태커를 평가한 후에 우승자로 선언.

### F. 스태킹, 블렌딩, 투표를 한 프레임에

| 방법 | 결합기 | 비용 | 언제 |
|------|--------|------|------|
| 하드 투표 | 다수 클래스 | 무료 | 빠르게, 다중 클래스, 확률 불필요 |
| 소프트 투표 | 평균 확률 | 무료 | 기저 모델이 잘 보정된 확률을 출력할 때 |
| 가중 투표 | 사전 고정 가중치 | 무료(가중치 선택 후) | 어떤 기저 모델이 더 나은지 알 때 |
| 블렌딩 | 홀드아웃에 메타 학습기 | 저렴 | 비싼 기저 모델, 큰 검증셋 |
| 스태킹 | OOF에 메타 학습기 | K× 기저 비용 | 진지한 앙상블의 기본 |

진행은 "고정 규칙 사용"(투표)에서 "작은 데이터셋에서 규칙 학습"(블렌딩)을 거쳐 "교차검증된 예측에서 규칙 학습"(스태킹)으로. 각 단계가 컴퓨팅을 정확도와 맞바꿈, 스태킹 너머 수익 체감.

### G. 스태킹하지 말아야 할 때

Lesson 14의 수익 체감 곡선이 여기에 날카롭게 적용. 스태킹이 추가:
- 2×에서 5× 학습 비용(K개 기저 모델 × K 폴드 + 최종 재학습 + 메타 학습기).
- 배포 시 2×에서 5× 추론 지연시간(모든 기저 모델이 평가되어야 함).
- 상당한 운영 복잡성(각 기저 모델이 버전 관리, 모니터링, 재학습할 별도 아티팩트).

올바른 질문은 "이득이 운영 비용 가치가 있는가?" 종종 잘 튜닝된 단일 LightGBM이 1/10 추론 지연시간과 1/5 운영 두통으로 스태킹 앙상블의 정확도와 일치. 정확도 예산이 요구할 때 스택; 그렇지 않으면 더 단순한 모델 출시.

### From Theory to the Code Below

- 섹션 1의 편향-분산 표는 (D)의 다양한 기저 모델 근거.
- 섹션 2의 scikit-learn에서 `StackingClassifier(estimators=[...], final_estimator=LogisticRegression())`는 (B)의 OOF 학습으로 (A)를 구현.
- 섹션 3의 단일 학습/검증 분할을 가진 블렌딩 예제는 (C)의 알고리즘.
- 섹션 4의 메타 학습기 비교(Linear vs LightGBM)는 (E)의 선택에 매핑.
- 섹션 5의 스태킹 모델 vs 최선 기저 모델에 대한 `cross_val_score`는 (G)의 경험적 점검 — 스택이 실제로 최선의 성분을 이기는가?
