...
**이전**: [프로젝트: 동적 배열](./04_Project_Dynamic_Array.md) | **다음**: [프로젝트: 스택과 큐](./06_Project_Stack_Queue.md)
...
1. 각 노드가 데이터와 다음 노드에 대한 포인터를 포함하는 자기 참조 구조체를 구현할 수 있다
2. push-front, push-back, insert-at-index, remove-at-index 연산을 갖춘 단일 연결 리스트를 구축할 수 있다
3. 배열(O(1) 접근, O(n) 삽입)과 연결 리스트(O(n) 접근, O(1) 삽입)의 시간 복잡도 트레이드오프를 비교할 수 있다
4. 삽입, 삭제, 제자리 역전 시 노드를 재연결하기 위한 포인터 조작을 적용할 수 있다
5. O(1) pop-back을 위해 `prev`와 `next` 포인터를 모두 가진 이중 연결 리스트를 구현할 수 있다
6. 일반적인 연결 리스트 오류(NULL 포인터 역참조, 잃어버린 노드, 잊어버린 `free` 호출)를 디버그할 수 있다
7. 단일, 이중, 원형 연결 리스트를 구분하고 각각의 적절한 사용 사례를 식별할 수 있다

---

동적 배열이 번호가 매겨진 우편함의 행이라면, 연결 리스트는 보물찾기입니다: 각 단서가 다음 단서로 이어집니다. 즉각적인 랜덤 접근을 포기하는 대신 어디서든 상수 시간에 요소를 삽입하거나 제거할 수 있습니다 -- 시프팅이 필요 없습니다. 이 프로젝트는 포인터 조작을 깊이 있게 다루며, C를 사용하는 사람과 진정으로 이해하는 사람을 구분하는 기술입니다.
...
> **비유 -- 보물찾기**: 연결 리스트는 각 단서(노드)에 상품(데이터)과 다음 단서로의 안내(포인터)가 있는 보물찾기와 같습니다. 5번째 상품을 찾으려면 처음부터 모든 단서를 따라가야 합니다 -- 건너뛸 수 없습니다. 하지만 중간에 새 단서를 삽입하는 것은 간단합니다: 안내 한 세트만 다시 쓰면 됩니다. 이것을 배열과 비교하면, 배열은 선반 위에 상품이 줄지어 있는 것과 같습니다 -- 5번을 찾기는 빠르지만 (그냥 세면 됨) 중간에 삽입하려면 모든 것을 밀어야 합니다.
...
void list_reverse(LinkedList *list) {
    ...
    list->tail = list->head;  // 이전 head가 새 tail이 됨
...
단계 1: prev=NULL, current=1
NULL <- 1    2 -> 3 -> NULL

단계 2: prev=1, current=2
NULL <- 1 <- 2    3 -> NULL

단계 3: prev=2, current=3
NULL <- 1 <- 2 <- 3
...
## 단계 4: 이중 연결 리스트(Doubly Linked List)

앞뒤 양방향으로 순회할 수 있는 연결 리스트:
...
| 포인터 조작 | 삽입/삭제 시 링크 변경 |
| 동적 메모리 | 각 노드마다 malloc/free |

### 연결 리스트의 종류

| 종류 | 특징 |
|------|------|
| 단일 연결 리스트(Singly Linked List) | next만 보유 |
| 이중 연결 리스트(Doubly Linked List) | prev와 next 모두 보유 |
...
1. **중복 제거**: 리스트에서 중복 값을 제거하세요

2. **두 리스트 병합**: 두 정렬된 리스트를 하나의 정렬된 리스트로 병합하세요

3. **사이클 감지**: 리스트에 사이클이 있는지 확인하세요 (Floyd 알고리즘)

4. **스택/큐 구현**: 연결 리스트를 사용하여 스택과 큐를 구현하세요

5. **LRU 캐시**: 이중 연결 리스트와 해시 테이블을 사용하여 간단한 LRU (Least Recently Used) 캐시를 구현하세요

---

## 다음 단계

[프로젝트: 스택과 큐](./06_Project_Stack_Queue.md) -> LIFO와 FIFO 자료 구조를 만들어봅시다!
