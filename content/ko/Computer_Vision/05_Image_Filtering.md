...
이미지 필터링은 그 수학적 핵심에서 **2D 신호에 적용되는 컨볼루션**입니다. 이 레슨의 모든 선형 필터 — 평균, 가우시안, Sobel, Laplacian, 샤프닝, 엠보스 — 는 같은 연산에 서로 다른 커널을 꽂은 것뿐입니다. 두 필터(§4 중앙값, §5 양방향)는 컨볼루션 틀 *바깥에* 있으며, 그것이 바로 그들에게 특별한 경계 보존 동작을 부여합니다.
...
`cv2.filter2D`로 만든 모든 필터(가우시안, Sobel, Laplacian, 샤프닝, 엠보스, 커스텀)는 LSI. **중앙값 필터**(§4)와 **양방향 필터**(§5)는 선형이 *아닙니다* — 이 비선형성이 선형 필터로는 불가능한 일을 가능하게 합니다.
...
**블러링이 잡음을 제거하는 이유 — 그리고 에지도 흐리는 이유**: 센서 잡음과 소금-후추 잡음은 픽셀 간 급격한 변화로 나타남, 거의 정의상 고주파 성분. 실제 장면 특징은 더 천천히 변하고 저주파에 집중. 저역 통과 커널이 잡음이 사는 고주파 대역을 억제하면서 신호가 사는 저주파 대역을 보존. 걸림돌: **날카로운 에지도 고주파 성분**이라, 선형 저역 통과 필터는 "잡음 때문의 고주파"와 "에지 때문의 고주파"를 구별할 수 없어 — 둘 다 약화시킴. 그래서 모든 선형 블러는 에지를 흐리고, 진짜 경계 보존 평활화에는 비선형이 필요(§5).
