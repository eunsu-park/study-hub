...
| 파일명 | 난이도 | 주요 내용 |
|--------|--------|----------|
| [시맨틱 세그멘테이션 (Semantic Segmentation)](./25_Semantic_Segmentation.md) | ⭐⭐⭐⭐ | FCN, U-Net, DeepLab, 픽셀 단위 분류 |
| [인스턴스 세그멘테이션 (Instance Segmentation)](./26_Instance_Segmentation.md) | ⭐⭐⭐⭐ | Mask R-CNN, YOLACT, 인스턴스 수준 마스크 |
| [파놉틱 세그멘테이션 (Panoptic Segmentation)](./27_Panoptic_Segmentation.md) | ⭐⭐⭐⭐ | Panoptic FPN, stuff + things, 통합 세그멘테이션 |

### 신경 3D 및 새로운 시점 합성 (28-29)

| 파일명 | 난이도 | 주요 내용 |
|--------|--------|----------|
| [Neural Radiance Fields (NeRF)](./28_Neural_Radiance_Fields.md) | ⭐⭐⭐⭐⭐ | NeRF, 볼류메트릭 렌더링, 위치 인코딩 |
| [3D Gaussian Splatting](./29_3D_Gaussian_Splatting.md) | ⭐⭐⭐⭐⭐ | 3DGS, 미분 가능한 래스터라이제이션, 실시간 렌더링 |

### 비디오 및 데이터 (30-32)

| 파일명 | 난이도 | 주요 내용 |
|--------|--------|----------|
| [비디오 이해 (Video Understanding)](./30_Video_Understanding.md) | ⭐⭐⭐⭐ | C3D, I3D, SlowFast, Video Transformers, 행동 인식 |
| [Optical Flow](./31_Optical_Flow.md) | ⭐⭐⭐⭐ | Lucas-Kanade, Farneback, RAFT, FlowNet |
| [합성 데이터 생성 (Synthetic Data Generation)](./32_Synthetic_Data_Generation.md) | ⭐⭐⭐⭐ | 도메인 랜덤화, sim-to-real, 데이터 증강 |
...
### 세그멘테이션 및 최신 주제 (3~4주)
```
3D 비전 심화 + 25 → 26 → 27 → 28 → 29 → 30 → 31 → 32
```
