...
#### 역방향 매핑 — OpenCV가 실제로 하는 방법

반대로, 각 *출력* 픽셀 `(x', y')`마다 역변환 `T⁻¹`을 적용해 대응되는 소스 좌표 `(x, y)`를 구하고, 앞서 본 보간 커널 중 하나로 그 (일반적으로 분수) 좌표에서 소스를 보간합니다. 두 가지 장점이 있습니다:
