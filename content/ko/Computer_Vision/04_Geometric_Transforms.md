...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 좌표 변환, 보간법, 순방향 vs 역방향 매핑에 대한 수학적 기초를 다룹니다.
...
OpenCV 함수를 보기 전에, 기하 변환(Geometric Transformation)이 실제로 무엇인지를 독립적인 세 조각으로 분리하면 이해가 쉬워집니다:

- **(A) 좌표 매핑(Coordinate mapping)** — `(x, y)`를 `(x', y')`로 보내는 규칙.
- **(B) 보간(Interpolation)** — 픽셀 값은 정수 격자 위에만 존재하므로, 부동소수 좌표에서 값을 얻는 방법.
- **(C) 매핑 방향(Mapping direction)** — 계산을 순방향(소스 → 목적지)으로 할지, 역방향(목적지 → 소스)으로 할지.

이 레슨의 뒤에 나오는 모든 OpenCV 함수는 이 틀의 구체적인 한 조각입니다.
...
이 레슨의 아래 각 섹션은 위 틀의 구체적 구현입니다:

- `cv2.resize(..., interpolation=...)` — 순수 스케일 유사 변환(§A)에 §B의 보간 커널을 파라미터로 받음.
- `cv2.getRotationMatrix2D(center, angle, scale)` — §A.2의 2×3 행렬을 빌드.
- `cv2.getAffineTransform(src_pts, dst_pts)` — §A.3의 6 DoF 시스템을 3점 쌍으로 푼다.
- `cv2.getPerspectiveTransform(src_pts, dst_pts)` — §A.4의 8 DoF 시스템을 4점 쌍으로 푼다.
- `cv2.warpAffine` / `cv2.warpPerspective` — §C.2의 역방향 매핑 루프를 돌리며 각 단계에서 §B의 보간을 호출.
