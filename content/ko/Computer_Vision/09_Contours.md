...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 이진 이미지에서 경계 추적으로 윤곽선을 복원하는 방법, 검색 모드가 의미하는 트리 구조, 그리고 Douglas-Peucker 알고리즘이 윤곽선 단순화의 표준인 이유를 다룹니다.
...
윤곽선(contour)은 이진 이미지에서 연결된 전경 영역을 둘러싸는 **경계 픽셀의 순서 리스트**입니다. 이는 컴팩트하고 구조화된 표현입니다. 영역 전체를 그 경계로부터 재구성할 수 있고, 많은 형태 속성(면적, 둘레, 모멘트)을 경계 위에서 직접 계산할 수 있으며, 매칭이나 분류를 경계 기술만으로 수행할 수 있습니다.

이 섹션은 다음을 다룹니다:

- **(A) 윤곽선이 실제로 무엇인가** — 수학적 정의와 그것을 찾는 경계 추적 알고리즘.
- **(B) 트리 구조로서의 검색 모드** — `RETR_EXTERNAL`, `RETR_LIST`, `RETR_CCOMP`, `RETR_TREE`가 어떤 계층 관계를 보존하는가.
- **(C) 체인 코드(Chain code)** — 컴팩트한 내부 표현과 `CHAIN_APPROX_SIMPLE`이 실제로 버리는 것.
- **(D) Douglas-Peucker 단순화** — `approxPolyDP`가 재귀적 점 선택으로 잡음 있는 윤곽선을 소수 정점 다각형으로 축소하는 방법.
- **(E) 모멘트(Moments)** — 윤곽선을 형상 지문으로 바꾸는 적분 통계량.

### A. 윤곽선이 무엇인가, 형식적으로

이진 이미지에서 **연결 성분**은 4- 또는 8-연결로 이어진 전경 픽셀의 극대 집합입니다. 그런 성분은 모두 닫힌 곡선 — 그 **경계** — 로 둘러싸여 있으며, 연속 픽셀이 8-이웃인 픽셀 위치의 순서 수열 `(x₀, y₀), (x₁, y₁), ..., (x_{n-1}, y_{n-1})`로 표현할 수 있습니다.

이런 경계를 추출하는 고전 알고리즘은 Suzuki-Abe(1985)이며, `cv2.findContours`가 정확히 이를 구현합니다:

1. 이미지를 왼쪽에서 오른쪽, 위에서 아래로 스캔.
2. 배경에서 전경으로의 전이마다 경계 시작을 표시하고 성분 주위를 추적, 각 경계 픽셀을 순서대로 기록.
3. **외부 경계**(전경이 배경에 둘러싸임)와 **구멍 경계**(배경이 전경에 둘러싸임)를 구분.
4. 어느 윤곽선이 어느 안에 중첩되어 있는지 기록하는 트리를 구축.

출력은 윤곽선당 하나의 순서 점 리스트와 중첩을 기술하는 **계층(hierarchy)**입니다.

### B. 트리 구조로서의 검색 모드

네 검색 모드는 계층의 어느 부분을 보존하는지에서 다릅니다:

```
 RETR_EXTERNAL    가장 바깥 윤곽선만. 모든 구멍과 중첩 객체 무시.
 RETR_LIST        모든 윤곽선, 계층 없음. 평평한 리스트.
 RETR_CCOMP       2단계 계층: 외부 윤곽선 + 직접 자식 구멍.
                  (구멍 내부의 객체는 중첩이 아니라 최상위로 나열됨.)
 RETR_TREE        전체 N단계 중첩. 각 윤곽선이 부모, 자식, 형제를 앎.
```

계층은 `(N, 4)` 정수 배열로 반환되며 각 행은 `[next, prev, first_child, parent]` 인덱스(없으면 `-1`). 복잡한 문서나 프랙탈 같은 이미지에는 `RETR_TREE`, 가장 바깥 객체만 원하고 중첩 추적 오버헤드를 건너뛰고 싶으면 `RETR_EXTERNAL`을 쓰세요.

### C. 체인 코드와 CHAIN_APPROX_SIMPLE

내부적으로 `findContours`는 8-이웃 방향을 따라 경계를 추적합니다. 이 방향 코드(0-7) 수열이 **Freeman 체인 코드** — 원래 저장용이었지만 지금은 대부분 내부 용도입니다.

**근사화 모드**가 반환되는 윤곽선이 어떻게 압축되는지를 제어합니다:

- **`CHAIN_APPROX_NONE`** — 모든 경계 픽셀 저장. 완벽한 수평 100픽셀 에지는 100점 저장.
- **`CHAIN_APPROX_SIMPLE`** — 연속 공선 점 합병. 같은 100픽셀 수평 에지는 2점 저장(끝점). 직선에 대해 무손실; 결과 다각형은 같은 에지를 가지지만 정점은 훨씬 적음.
- **`CHAIN_APPROX_TC89_L1`**, **`CHAIN_APPROX_TC89_KCOS`** — 지배 점(코너)만 유지하는 Teh-Chin 알고리즘, SIMPLE보다 더 공격적.

대부분의 응용에 `CHAIN_APPROX_SIMPLE`이 올바른 기본값: 축 정렬 내용에 대해 무손실이고 전형 형상에 대해 메모리를 10-100× 줄입니다.

### D. Douglas-Peucker 단순화

`cv2.approxPolyDP(contour, epsilon, closed)`가 윤곽선을 더 적은 정점의 다각형으로 단순화합니다. 픽셀 단위 허용 오차 `epsilon`으로 제어. 알고리즘은 재귀적:

1. 윤곽선의 두 끝점을 현재 근사로 시작.
2. 그들을 연결하는 직선에서 가장 먼 윤곽선 점을 찾음.
3. 최대 거리가 `epsilon`을 초과하면 그 점을 근사에 추가하고 두 부분 분절에 재귀. 그렇지 않으면 중지 — 분절이 "충분히 직선적".

출력은 원 윤곽선의 엄격한 부분집합을 정점으로 가진 다각형이며, 버려진 모든 점은 다각형에서 `epsilon` 이내에 놓입니다. 전형 사용: `epsilon = 0.02 · arcLength(contour, True)` (둘레의 2%)로 설정하고 결과가 정확히 4개 정점을 가지는지 확인해 사변형 문서 검출(04 레슨 §5 참고).

Douglas-Peucker는 표준 곡선 단순화 알고리즘 — GIS의 지도 일반화와 GPS 궤적 단순화에 쓰이는 것과 같은 것.

### E. 모멘트: 형상 지문

윤곽선의 기하 모멘트는 매칭에 적합한 소수의 숫자로 형상을 압축합니다. 윤곽선으로 둘러싸인 영역의 `(p, q)` 차 **원시 모멘트**:

```
M_{pq} = ΣΣ  x^p · y^q     윤곽선 내부의 모든 픽셀 (x, y)에 대해
```

주요 파생 값:

- **면적** = `M_{00}` — 내부 픽셀 수.
- **질량 중심(Centroid)** = `(M_{10}/M_{00}, M_{01}/M_{00})` — 영역의 질량 중심.
- **중심 모멘트(Central moments)** `μ_{pq}` = 질량 중심 기준으로 계산된 원시 모멘트. 평행이동 불변.
- **정규화된 중심 모멘트** `η_{pq}` = 면적의 거듭제곱으로 스케일된 `μ_{pq}`. 평행이동 및 스케일 불변.
- **Hu 모멘트** (7개 스칼라 `h₁...h₇`) = 추가로 **회전 불변**이고 (작은 수정으로) 반사 불변인 `η_{pq}`의 특정 조합.

Hu 모멘트는 위치, 크기, 방향과 무관하게 형상을 비교하는 고전적 방법 — 현대 응용은 대개 학습된 디스크립터를 사용하지만. 10 레슨이 모멘트를 이용한 형상 분석을 더 깊이 다룹니다.

윤곽선 관련 기타 파생 값:

- **둘레**: `cv2.arcLength(contour, closed)` — 연속 정점 간 거리의 합.
- **경계 상자**(축 정렬): `cv2.boundingRect(contour)`, 윤곽선을 포함하는 축 정렬 최소 사각형.
- **최소 면적 회전 사각형**: `cv2.minAreaRect(contour)`, 회전 허용.
- **최소 외접 원**: `cv2.minEnclosingCircle(contour)`.
- **Convex hull(볼록 껍질)**: `cv2.convexHull(contour)`, 윤곽선을 포함하는 가장 작은 볼록 다각형. 볼록성 결함(`cv2.convexityDefects`)을 확인할 때 유용 — 손 제스처에서 손가락 사이의 간극을 식별할 수 있음.

### 이론에서 아래 함수들로

- `cv2.findContours(binary, mode, method)` — 경계 추출(§A), `(contours, hierarchy)` 반환. `mode`가 검색(§B) 선택; `method`가 체인 코드 압축(§C) 선택.
- `cv2.drawContours(img, contours, idx, color, thickness)` — 시각화.
- `cv2.approxPolyDP(contour, epsilon, closed)` — Douglas-Peucker 단순화(§D).
- `cv2.moments(contour)` — §E의 원시, 중심, 정규화, Hu 모멘트를 한 딕셔너리로.
- `cv2.contourArea`, `cv2.arcLength` — 모멘트 파생 값의 효율적 계산.
- `cv2.boundingRect`, `cv2.minAreaRect`, `cv2.minEnclosingCircle`, `cv2.fitEllipse` — 경계 기하.
- `cv2.convexHull`, `cv2.convexityDefects` — 볼록성 분석.
