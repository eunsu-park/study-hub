...
출력은 원 윤곽선의 엄격한 부분집합을 정점으로 가진 다각형이며, 버려진 모든 점은 다각형에서 `epsilon` 이내에 놓입니다. 전형 사용: `epsilon = 0.02 · arcLength(contour, True)` (둘레의 2%)로 설정하고 결과가 정확히 4개 정점을 가지는지 확인해 사변형 문서 검출(`04_Geometric_Transforms.md` §5 참고).
