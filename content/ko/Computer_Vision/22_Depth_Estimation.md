...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 학습된 사전분포 없이 단안 깊이가 불량하게 설정되는 이유, 상대 vs 절대 깊이 구분, 그리고 MiDaS, DPT, 다중 뷰 SfM이 각각 모호성을 깨는 방법을 다룹니다.
...
**단안 깊이 추정**은 단일 RGB 이미지에서 픽셀별 깊이 `Z(u, v)`를 추론하는 것 — 21 레슨이 근본적으로 모호함을 보여준 문제입니다. 모델은 어떻게 이를 풀 수 있을까요? 수학만으로는 안 됩니다. 방대한 RGBD 훈련 데이터에서 **실세계 장면의 깊이가 일반적으로 어떻게 생겼는지에 대한 사전분포(prior)**를 학습함으로써입니다.

이 섹션은 다음을 다룹니다:

- **(A) 단안 깊이의 불량 설정성** — 기하학이 해결할 수 없는 작은 스케일 모호성.
- **(B) 인간이 쓰는 단서** — 텍스처 기울기, 가림, 음영, 원근 — 그리고 CNN/Transformer 깊이 모델이 이들을 학습하는 방법.
- **(C) 상대 vs 절대 깊이** — 대부분의 단안 모델이 상대 깊이를 출력하는 이유와 "스케일과 시프트까지"의 의미.
- **(D) MiDaS와 교차 데이터셋 훈련** — 이질적 깊이 데이터 전반에 걸친 훈련을 가능하게 하는 scale-and-shift-invariant 손실.
- **(E) DPT와 transformer 아키텍처** — Attention이 밀집 예측에 도움이 되는 이유.
- **(F) "진짜" 해결로서의 Structure-from-Motion** — 여러 단안 이미지를 사용해 기하학적으로 절대 깊이를 복원하는 방법.
...
### D. MiDaS: Scale-and-Shift-Invariant 훈련
...
### F. Structure-from-Motion: 여러 단안 이미지에서 절대 깊이 복원
...
- `cv2.dnn.readNetFromONNX('midas.onnx')` — DNN 모듈(§19)을 통해 MiDaS 또는 DPT(§D / §E) 로드.
- 전처리: MiDaS는 384×384 입력(또는 더 큰), ImageNet 평균/표준편차로 `[0, 1]` 정규화 기대.
- `net.forward()` 출력: 임의 단위의 단채널 역 깊이 맵, bilinear 보간으로 원 이미지 크기로 리사이즈.
- SfM의 경우: `cv2.findEssentialMat`, `cv2.recoverPose`, `cv2.triangulatePoints`(§21). 대규모 SfM은 별도 도구로 COLMAP 또는 OpenSfM 사용.
- 상대 → 절대 캘리브레이션: 중앙값이 알려진 참조와 일치하도록 선택된 스칼라를 예측 역 깊이에 곱하기.
