...
참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 도메인 갭 문제, 도메인 랜덤화가 작동하는 이유, sim-to-real 전이 트레이드오프, 그리고 생성 모델이 합성 데이터 풍경을 바꾸는 방법을 다룹니다.
...
합성 데이터 생성은 — 3D 렌더링, 절차적 합성, 증강, 또는 생성 모델을 통해 — 학습 이미지를 프로그래밍 방식으로 생성해 실세계 주석 데이터를 **보충하거나 대체**합니다. 실제 데이터는 비싸고(수동 주석), 제약이 있고(드문 사건이 부족하게 표현됨), 또는 사용 불가능(의료 영상 프라이버시, 자율 주행 엣지 케이스). 합성 데이터가 이런 한계를 우회하지만 **도메인 갭** 문제를 도입: 합성 이미지로 훈련된 모델이 실제 이미지에 일반화하지 않을 수 있음.

이 섹션은 다음을 다룹니다:

- **(A) 데이터 문제와 합성이 도움이 되는 이유** — 실용적 동기.
- **(B) 도메인 갭** — 합성과 실제 사이에 무엇이 다른가, 왜 중요한가.
- **(C) 도메인 랜덤화** — 훈련의 *더 많은 변화*가 실제로의 *더 나은* 전이를 가져온다는 놀라운 통찰.
- **(D) 절차적 생성** — 통제된 변화로 원시에서 장면 구성.
- **(E) 데이터를 위한 생성 모델** — diffusion 기반과 GAN 기반 증강.
- **(F) 도메인 적응** — 합성만으로 충분하지 않을 때 갭 좁히기.
...
- **Blender + Python API**: 무료, 완전히 스크립팅 가능, path-traced. 작은/중간 규모의 photorealistic 합성 데이터셋에 최선.
- **NVIDIA Omniverse / Isaac Sim**: 로보틱스를 위한 물리 + 렌더링; 내장 도메인 랜덤화.
- **Unity Perception / Unreal Engine**: 게임 엔진 기반 파이프라인, 종종 자율 주행에 사용(CARLA simulator).
- **Albumentations / torchvision.transforms**: 2D 증강 라이브러리(자르기, color jitter 등) — 가장 단순한 형태의 합성 증강, 기존 실제 이미지에 적용.
- **Diffusion 기반**: 조건부 생성을 위한 Stable Diffusion + ControlNet, 변화를 위한 prompt engineering.

전형적 합성 데이터 프로젝트 순서: 자산 구축 → 장면 구성 스크립팅 → 대규모 랜덤화 및 렌더링 → 베이스라인 훈련 → 도메인 갭 측정 → 작은 실제 집합에 파인튜닝 → 반복.
