...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 에지를 이미지 미분으로 보는 관점, Sobel이 잡음에 강한 기울기 근사인 이유, Canny의 5단계 파이프라인이 특정 최적성 기준에 대응하는 방식, 그리고 Laplacian 제로 크로싱과 Difference-of-Gaussians의 관계를 다룹니다.
...
이미지에서 "에지(edge)"는 밝기가 급격히 변하는 위치입니다. 그 직관 뒤의 수학적 대상은 **이미지의 미분**입니다. 이 레슨의 모든 에지 검출기는 미분의 이산 근사와, 어떤 크기가 에지로 간주되는지·응답을 어떻게 경계당 한 픽셀로 얇게 만드는지에 대한 결정의 조합입니다.

이 섹션은 다음을 다룹니다:

- **(A) 미분으로서의 에지** — 고전적 에지 검출이 근사하는 연속 미적분.
- **(B) 1차 연산자** — Sobel, Scharr, Prewitt: 잡음 특성이 서로 다른 이산 기울기 근사.
- **(C) 2차 연산자** — Laplacian과 Laplacian-of-Gaussian(LoG), 그리고 제로 크로싱.
- **(D) Difference-of-Gaussians** — LoG를 근사하는 이유와 분리 가능성.
- **(E) Canny 알고리즘** — Canny 기준(좋은 검출, 좋은 국소화, 단일 응답)에서 유도되는 5단계.
...
- `cv2.Sobel(img, ddepth, dx, dy, ksize)` — §B.2. 음수 기울기 클리핑을 피하려면 `ddepth`는 보통 `CV_64F`. `dx, dy`는 편미분 방향 선택.
- `cv2.Scharr(img, ddepth, dx, dy)` — §B.3. 3×3 전용, Sobel보다 더 등방적.
- `cv2.Laplacian(img, ddepth, ksize)` — §C. 보통 먼저 `GaussianBlur`로 LoG 동작을 만듭니다.
- `cv2.Canny(img, threshold1, threshold2, apertureSize, L2gradient)` — §D. `threshold1, threshold2`가 `T_low, T_high`. `L2gradient=True`면 `|G_x| + |G_y|` 대신 제대로 된 `sqrt` 크기.
- `cv2.magnitude`, `cv2.phase` — §B.4, 기울기 크기/방향 맵을 직접 원할 때(HOG, 에지 히스토그램 등).
