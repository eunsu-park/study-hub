...
속도를 위해 임베디드 비전이나 모바일 SoC처럼 평방근이 비싼 환경에서는 `|∇I| ≈ |G_x| + |G_y|` (L1 근사)가 쓰입니다 — 회전 대칭이 아니지만 빠릅니다. 크기가 후단(예: Canny, HOG)에서 쓰일 때는 제대로 된 `sqrt` 형식이 필수입니다.
...
약한 픽셀이 강한 픽셀에 (8-이웃, 전이적으로도) 연결되어 있을 때만 강함으로 바꿉니다. 최종 에지 맵은 원래 강한 픽셀 + 그로부터 도달 가능한 모든 약한 픽셀. §07 임계처리에서 본 히스테리시스 임계처리와 같은 아이디어 — 중간 경우의 결정은 이웃 기반.

결과: 얇은(비최대 억제 덕분) 에지가 실제 에지 위치 가까이(최적화된 필터 덕분) 있고, 잡음 영역을 가로질러 연결됨(히스테리시스 추적 덕분).
