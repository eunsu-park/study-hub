...
참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 시간 모델링 문제, two-stream에서 3D conv를 거쳐 video transformer로의 아키텍처 진행, 그리고 각각이 만드는 공간-시간 트레이드오프를 다룹니다.
...
비디오 이해는 이미지 수준 비전 작업(분류, 검출, 세그멘테이션)을 **프레임 시퀀스**로 확장합니다. 핵심 도전: **시간 차원**을 어떻게 통합해 모델이 단지 프레임이 아니라 행동을 인식하도록 할 것인가?

"다이빙" 비디오의 단일 프레임은 임의의 포즈처럼 보임; 한 프레임에서 다이빙인지 점프인지 스트레칭인지 알 수 없음. 시간 패턴 — 시간에 걸친 포즈 시퀀스 — 이 행동을 정의하는 것. 비디오 이해 아키텍처는 이 시간 구조를 모델링하는 방법에서 다름.

이 섹션은 다음을 다룹니다:

- **(A) 프레임별 분류로 부족한 이유** — 잃어버리는 시간 정보.
- **(B) Two-stream 네트워크** — 동작을 외관에서 분리.
- **(C) 3D 컨볼루션** — 공간 conv를 시공간으로 확장.
- **(D) SlowFast** — 다중 속도 시간 모델링.
- **(E) Video transformer** — 공간과 시간에 걸친 attention.
- **(F) 시간 국소화** — 행동이 언제 시작하고 끝나는지 찾기.
...
- **MMAction2**(PyTorch, MMLab): 포괄적 비디오 이해 툴킷. SlowFast, I3D, TSN, TimeSformer, Video Swin 등 구현.
- **PyTorchVideo**(Meta): 비디오 특정 모델 zoo와 dataloader.
- **HuggingFace transformers**: 사전 훈련된 TimeSformer, VideoMAE, ViViT 제공.
- **Decord**(배치 비디오 읽기에 OpenCV보다 빠름): 수백만 클립으로 훈련할 때 OpenCV의 프레임별 디코딩 오버헤드 회피.

OpenCV 추론의 경우: 많은 비디오 모델이 ONNX로 내보내지고 `cv2.dnn`을 통해 로드 가능, 하지만 다중 프레임 입력 모양은 신중한 전처리 필요(종종 배치 구성을 위해 `blobFromImages` 사용).
