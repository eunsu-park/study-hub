...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. Epipolar 기하, disparity-to-depth 방정식, 두 뷰에서의 삼각측량, 그리고 3D 표면의 이산 샘플링으로서의 포인트 클라우드를 다룹니다.
...
단일 2D 이미지는 깊이를 잃습니다 — 이미지 평면의 한 점은 카메라 광심에서 나오는 광선을 따라 있는 어떤 3D 점에도 대응할 수 있습니다. 이미지에서 3D를 복원하는 것은 추가 정보로 **깊이를 제약**하는 것을 의미합니다: 두 번째 시점(스테레오), 움직임(SFM), 알려진 기하(템플릿), 또는 깊이 센서(Kinect, LiDAR). 이 레슨은 이 모든 것에 공유되는 기하학적 기초를 다룹니다.

이 섹션은 다음을 다룹니다:

- **(A) 2D → 3D 역전 문제** — 한 이미지로 부족한 이유, 두 개면 충분한 이유.
- **(B) Epipolar 기하** — 두 뷰를 관계시키는 essential과 fundamental matrix.
- **(C) Disparity와 depth** — 스테레오가 작동하게 하는 `Z = f·B/d` 관계.
- **(D) 삼각측량(Triangulation)** — 두 이미지 관찰에서 3D 점 복원.
- **(E) 포인트 클라우드와 메시** — 3D 표면의 이산 표현.
...
- `cv2.stereoCalibrate`, `cv2.stereoRectify` — §B.3 스테레오 리그 캘리브레이션.
- `cv2.StereoBM_create`, `cv2.StereoSGBM_create` — block-matching과 semi-global-matching 스테레오 disparity(§C).
- `cv2.reprojectImageTo3D(disparity, Q)` — rectification 행렬 `Q`를 사용해 disparity 맵을 포인트 클라우드로 변환(§E.3).
- `cv2.findFundamentalMat`, `cv2.findEssentialMat` — RANSAC으로 `F` / `E` 복원(§B).
- `cv2.recoverPose(E, ...)` — essential matrix에서 `R`, `t` 추출(§B.2).
- `cv2.triangulatePoints(P1, P2, pts1, pts2)` — DLT 삼각측량(§D).
- Open3D(별도 라이브러리): 포인트 클라우드 I/O, 법선, Poisson reconstruction, ICP, 시각화.
