...
참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 명시적 원시 표현, 비등방 3D 가우시안, 미분 가능한 래스터라이제이션을 통한 splatting, 그리고 적응적 밀도 제어를 다룹니다.
...
3D Gaussian Splatting(Kerbl 등, 2023)은 NeRF의 암묵적 신경 표현을 **3D 가우시안의 명시적 집합**으로 대체합니다. 결과: **실시간 렌더링**(1080p에서 100+ FPS)과 **빠른 훈련**(시간 대신 분)으로 비슷한 시각 품질. 핵심 아이디어는 장면을 비등방 3D 가우시안 "blob" 컬렉션으로 표현하고 미분 가능한 splat 연산으로 이미지로 rasterize하는 것.

이 섹션은 다음을 다룹니다:

- **(A) 렌더링에 명시적이 암묵적을 이기는 이유** — 조회 vs 평가 트레이드오프.
- **(B) 3D 가우시안 원시** — 비등방 모양, 색상, 불투명도.
- **(C) Splatting: 미분 가능한 래스터라이제이션** — 가우시안이 이미지 픽셀로 투영되는 방법.
- **(D) 적응적 밀도 제어** — 최적화가 가우시안을 키우고 가지치기하는 방법.
- **(E) 시점 의존 색상을 위한 spherical harmonics** — 시점 의존 MLP 없이 specularity 포착.
- **(F) NeRF와의 비교** — 구체적 트레이드오프.
...
### E. 시점 의존 색상을 위한 Spherical Harmonics
...
- **원본 3DGS 구현**(Inria/Graphdeco): 참조 C++/CUDA + Python.
- **Nerfstudio**: 이제 3DGS를 따르는 Splatfacto trainer 포함.
- **Gsplat**(Nerfstudio 팀): 확장이 있는 깨끗한 PyTorch 재구현.
- **Spz / .ply 포맷**: 훈련된 Gaussian Splatting 장면 저장의 표준 포맷.

NeRF처럼 3DGS는 OpenCV의 일부가 아님 — 하지만 렌더링 파이프라인(카메라 투영 + 알파 블렌딩)은 §18과 §21의 기하를 직접 사용.
