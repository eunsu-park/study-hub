...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. "색상 공간"이 실제로 무엇인지, HSV가 RGB와 달리 색상과 밝기를 분리하는 방법, 그리고 CIE LAB 모델이 무엇을 해결하는지를 다룹니다.
...
"색"은 지각 현상입니다 — 빛 자체의 성질이 아니라 인간 시각의 성질입니다. **색상 공간(color space)**은 지각된 색을 숫자 튜플로 변환하는 수학적 매개변수화입니다. 애플리케이션마다 다른 매개변수화가 필요합니다. RGB는 디스플레이 하드웨어에 자연스럽고, HSV는 예술가와 분할 알고리즘이 신경 쓰는 속성을 분리하며, CIE LAB은 튜플 간 수치 거리가 지각 색 차이와 일치하도록 설계되었습니다.

이 섹션은 다음을 다룹니다:

- **(A) 왜 여러 색상 공간이 존재하는가** — 인간 시각계가 서로 다른 파장의 빛에 세 종류의 원뿔세포를 통해 반응한다는 핵심 사실.
- **(B) RGB** — 가산 모델과 그 한계(지각적으로 균일하지 않고, 채널들이 독립적이지 않음).
- **(C) HSV** — 색상을 밝기와 분리하는 원통형 재매개변수화와 변환 공식.
- **(D) CIE LAB** — 색 차이를 거리로 만드는 지각적으로 균일한 공간.
- **(E) 그레이스케일 변환** — 휘도 가중치와 그 유래.
- **(F) 감마 인코딩** — 대부분의 색 수학에 영향을 주는, 모든 8비트 sRGB 이미지에 숨어 있는 비선형성.
...
1. **지각적으로 균일하지 않음**. 고정된 유클리드 거리 `Δ = ‖(R₁,G₁,B₁) - (R₂,G₂,B₂)‖`는 고정된 지각 색 차이에 대응하지 않습니다. 초록 영역에서 눈은 지나치게 민감하고, 파랑에서는 둔합니다. §D가 LAB이 이를 해결하는 방법을 보여줍니다.
...
mask = cv2.inRange(hsv, (100, 150, 50), (130, 255, 255))   # 파란 계열, 어떠한 밝기에서도
```

#### C.3 OpenCV의 `uint8` 스케일링
...
- `cv2.cvtColor(img, flag)` — 이 레슨의 모든 공간 간 변환을 위한 통합 API. flag가 원본과 목적 공간을 선택합니다(예: `COLOR_BGR2HSV`, `COLOR_BGR2LAB`, `COLOR_BGR2GRAY`).
- `cv2.split(img)` / `cv2.merge(channels)` — 다채널 이미지를 채널별 배열로 분해하고 다시 조립. 단일 채널에 연산할 때 필수(§D: `L*`는 건드리지 않고 `a, b`에만 연산).
- `cv2.inRange(hsv, lower, upper)` — §C.2를 활용한 HSV 색상 분할.
- **BGR vs RGB** — 02 레슨 §D.2의 역사적 사고, 이 레슨 상단에도 영향.
