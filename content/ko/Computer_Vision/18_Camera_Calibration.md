...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 핀홀 투영 모델, intrinsic/extrinsic 분해, 방사형 및 접선형 왜곡 다항식, 그리고 Zhang의 평면 패턴 캘리브레이션 아이디어를 다룹니다.
...
카메라 캘리브레이션은 실제 카메라가 3D 세계의 점을 2D 이미지 픽셀로 어떻게 매핑하는지를 기술하는 수학적 모델을 추정하는 과정입니다. 이 모델을 알면:

- 렌즈 왜곡을 보정해 기하학적으로 깨끗한 이미지를 얻을 수 있음.
- 여러 뷰에서 3D 점을 삼각측량할 수 있음.
- 이미지 측정값으로부터 실세계 거리를 계산할 수 있음.
- 증강현실, Structure-from-Motion, SLAM, 로봇 비전 시스템을 구축할 수 있음.

이론은 세 층으로 구성됩니다: **이상적 핀홀 투영**(순수 기하), **렌즈 왜곡 모델**(실제 광학에 대한 보정), 그리고 **캘리브레이션 알고리즘**(알려진 타겟 이미지들로부터 모든 파라미터 추정). 이 섹션은 다음을 다룹니다:

- **(A) 핀홀 카메라 모델** — 이상적 투영 방정식과 intrinsic/extrinsic 분해.
- **(B) 카메라 행렬 K** — 초점 거리, 주점, 비틀림이 픽셀에서 의미하는 것.
- **(C) 렌즈 왜곡** — 방사형과 접선형 모델, 그리고 저차 다항식으로 충분한 이유.
- **(D) Zhang의 방법** — 모든 OpenCV 캘리브레이션 뒤의 1999년 알고리즘. 문제를 평면 패턴의 여러 뷰로 축소.
- **(E) 재투영 오차** — 캘리브레이션이 실제로 잘 됐는지 알려주는 메트릭.
...
- `cv2.findChessboardCorners(img, patternSize)` — §D.3의 1단계. 체스보드 코너를 서브픽셀 정확도로 검출.
- `cv2.calibrateCamera(objectPoints, imagePoints, imageSize)` — 전체 Zhang 파이프라인(§D). `(retval, K, dist, rvecs, tvecs)` 반환.
- `cv2.undistort(img, K, dist)` — §C의 역 왜곡 다항식을 적용해 이미지를 교정.
- `cv2.initUndistortRectifyMap` / `cv2.remap` — §C 왜곡 해제를 미리 계산된 픽셀별 맵으로 분해. 비디오 레이트로 많은 프레임을 왜곡 해제할 때 유용.
- `cv2.projectPoints(objectPoints, rvec, tvec, K, dist)` — §A의 순방향 투영, 재투영 오차 계산에 사용(§E).
- `cv2.solvePnP(objectPoints, imagePoints, K, dist)` — 알려진 intrinsic 캘리브레이션과 2D-3D 대응이 주어지면 `[R | t]` 풀기. extrinsic 부분의 역.
