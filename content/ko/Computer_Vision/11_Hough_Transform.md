...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 이미지 공간과 파라미터 공간의 쌍대성, `(ρ, θ)` 법선 형식이 `y = mx + b`의 수직선 문제를 해결하는 방식, 그리고 같은 투표 아이디어가 직선에서 원을 거쳐 임의 형태로 어떻게 확장되는지를 다룹니다.
...
허프 변환은 특정 문제를 풉니다: **에지 점들의 집합이 주어지면, 그것들을 가장 잘 설명하는 기하학적 형태(직선, 원, ...)를 찾아라**. RANSAC 같은 순진한 접근은 무작위 샘플링으로 형태를 맞추지만, 허프는 *파라미터 공간에서의 투표*라는 다른 접근을 취합니다. 이는 데이터 누락에 강하고, 여러 형태를 동시에 다루며, 깨끗한 해석적 의미를 가집니다.

이 섹션은 다음을 다룹니다:

- **(A) 이미지 공간 / 파라미터 공간 쌍대성** — 이미지 공간의 직선이 파라미터 공간의 점에 대응하고, 이미지 공간의 점이 파라미터 공간의 곡선에 대응한다는 핵심 아이디어.
- **(B) 왜 법선 `(ρ, θ)` 매개변수화를 쓰는가** — `y = mx + b`의 수직선 병리와 극형식이 그것을 해결하는 방법.
- **(C) 투표 절차** — 이산 알고리즘: 에지 픽셀이 투표하고, 어큐뮬레이터의 피크가 검출된 형태.
- **(D) 원으로의 확장** — 같은 아이디어, 더 고차원의 파라미터 공간, 이를 다루기 위한 기울기 트릭.
- **(E) 확률적 허프와 일반 허프 변환** — 런타임 최적화와 임의 형태로의 확장.

### A. 이미지 공간 / 파라미터 공간 쌍대성
...
- `cv2.HoughLines(edges, rho, theta, threshold)` — 표준 허프 직선 검출(§B, §C). `rho, theta`는 어큐뮬레이터 빈 너비. `threshold`는 피크가 직선으로 선언되기 위한 최소 투표 수. `(ρ, θ)` 쌍 반환.
- `cv2.HoughLinesP(edges, rho, theta, threshold, minLineLength, maxLineGap)` — 확률적 허프(§E.1). 끝점을 가진 **선분** 반환, 차선 검출이나 그리기에 적합.
- `cv2.HoughCircles(img, method, dp, minDist, param1, param2, minRadius, maxRadius)` — 기울기 최적화를 적용한 원 허프(§D). `dp`는 어큐뮬레이터 해상도 비율; `param1`은 내부 에지 검출용 상위 Canny 임계값; `param2`는 어큐뮬레이터 임계값.
