...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. "좋은" 특징점의 조건, Harris 코너 검출 뒤의 structure tensor, SIFT 뒤의 scale-space와 DoG 피라미드, 그리고 ORB를 낳는 이진 디스크립터의 트레이드오프를 다룹니다.
...
특징점(*keypoint* 또는 *interest point*로도 불림)은 이미지가 평행이동, 회전, 스케일, 블러된 후에도 **다시 찾을 수 있을 만큼 충분히 독특한** 위치입니다. 이 분야는 Harris, FAST, SIFT, SURF, ORB, BRISK, AKAZE 등 여러 검출기를 만들어왔고, 이들은 무엇을 찾는가와 어떤 변환에 불변인가에서 차이가 납니다.

이 섹션은 다음을 다룹니다:

- **(A) 좋은 키포인트의 조건** — 모든 검출기가 만족하려 하는 세 가지 기준.
- **(B) 구조 텐서(Structure tensor)** — Harris, Shi-Tomasi, 대부분의 코너 검출기 뒤의 수학적 대상.
- **(C) Harris 코너 응답** — 구조 텐서로부터 유도되어 픽셀을 코너로 순위 매기는 특정 스칼라.
- **(D) 스케일-공간과 DoG** — SIFT가 블러된 이미지의 피라미드를 탐색하는 이유와 DoG 피라미드가 Laplacian-of-Gaussian을 근사하는 방식.
- **(E) 디스크립터** — 매칭을 위해 각 키포인트에 동반되는 벡터. 기울기 히스토그램(SIFT) vs 이진(ORB/BRIEF) 트레이드오프.

### A. 좋은 키포인트의 조건

유용한 키포인트는 세 가지 속성을 만족해야 합니다 — 셋은 서로 긴장 관계이며, 모든 검출기가 다르게 타협합니다.

#### A.1 반복성 (Repeatability)
...
LoG는 직접 계산이 비쌉니다. §08.C.3에서 유도한 대로, **Difference-of-Gaussians**가 스케일된 LoG를 근사합니다:
...
- `cv2.cornerHarris(img, blockSize, ksize, k)` — Harris 응답 맵 직접 반환(§C). 임계처리 및 국소 최대 찾기로 코너 획득.
- `cv2.goodFeaturesToTrack(img, maxCorners, qualityLevel, minDistance)` — Shi-Tomasi 변형(§C). 중복 제거된 코너 리스트 직접 반환.
- `cv2.FastFeatureDetector_create(threshold, nonmaxSuppression, type)` — FAST 코너(§A 속도 사례). 검출만, 디스크립터 없음.
- `cv2.SIFT_create(nfeatures, nOctaveLayers, contrastThreshold, edgeThreshold, sigma)` — 전체 SIFT 파이프라인(§D + §E.1). 파라미터가 피라미드와 응답 임계값 제어.
- `cv2.ORB_create(nfeatures, scaleFactor, nlevels, ...)` — 전체 ORB 파이프라인(FAST 검출 + 방향 BRIEF 디스크립터, §E.2).
- `detector.detectAndCompute(img, mask)` — 검출 + 디스크립터 추출 결합. 키포인트 리스트와 `(N, D)` 디스크립터 행렬 반환.
