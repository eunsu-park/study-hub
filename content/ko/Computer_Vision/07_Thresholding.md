...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 임계처리를 1D 분류 문제로 보는 관점, Otsu 방법이 클래스 내 분산을 최소화해 임계값을 자동 선택하는 원리, 그리고 조명이 불균일할 때 적응형 방법이 필요한 이유를 다룹니다.
...
임계처리는 가장 단순한 분류기입니다. 각 픽셀은 밝기가 임계값보다 크냐 작냐에 따라 "전경" 또는 "배경"으로 배정됩니다. 단순함에도 불구하고 임계처리는 수많은 컴퓨터 비전 파이프라인의 첫 단계 — OCR, 산업 검사, 의료 영상 — 입니다. 당신의 이미지에 작동한다면, 더 화려한 분할 방법은 필요 없기 때문입니다.

이 섹션이 답하는 질문:

- **(A) 분류 문제로서의 임계처리** — 임계값이 실제로 무엇인지, 그리고 언제 단일 전역 임계값이 정당한지.
- **(B) Otsu의 방법** — 클래스 내 분산을 최소화하는 원리로부터 유도되는 자동 임계값 선택 알고리즘.
- **(C) 전역 임계처리가 실패하는 이유** — 불균일 조명 문제.
- **(D) 적응형 임계처리** — 픽셀마다 국소 임계값을 계산하는 방식과 두 가지 창 통계(평균, 가우시안)의 차이.
- **(E) 다중 임계값과 색상 임계처리** — 그레이스케일의 단순 이진 분류를 넘어서는 확장.
...
- `cv2.threshold(img, T, max, type)` — 전역 임계처리(§A). `type` 플래그는 출력 인코딩을 선택(binary, binary inverse, truncate, to-zero, to-zero inverse).
- `cv2.threshold(img, 0, 255, THRESH_BINARY + THRESH_OTSU)` — Otsu 방법(§B). 명시적 `0`은 무시됨. Otsu가 내부적으로 `T*`를 계산해 첫 번째 반환값으로 돌려줍니다.
- `cv2.adaptiveThreshold(img, max, method, type, blockSize, C)` — 적응형 임계처리(§D). `method`가 평균(§D.1) 또는 가우시안(§D.2)을 고름. `blockSize`가 창 크기, `C`가 편향.
- `cv2.inRange(img, lower, upper)` — 색상 임계처리(§E.2). 일반적으로 `cv2.cvtColor(img, COLOR_BGR2HSV)` 후 적용.
