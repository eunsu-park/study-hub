...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 도형의 적분 통계로서의 모멘트, 이동/스케일/회전 불변성의 유도, Hu의 7개 불변 모멘트, 그리고 그 위에 만들어진 형상 매칭 메트릭을 다룹니다.
...
형상 분석은 구체적인 질문에 답합니다: **이미지 어디에, 얼마나 크게, 어떤 방향으로 나타나든 같은 모양이 항상 같은 벡터를 내도록, 2D 모양을 어떻게 작은 숫자 벡터로 기술할 것인가?** 고전적 답은 M.-K. Hu(1962)에 의한 것 — 이동, 스케일링, 회전에 불변함이 증명된 이미지 모멘트의 스칼라 함수 집합 **모멘트 불변량**.

이 섹션은 다음을 다룹니다:

- **(A) 모멘트란 무엇인가** — 형상의 적분 통계에 대한 일반적 아이디어.
- **(B) 원시, 중심, 정규화 모멘트** — 점진적으로 더 불변적인 통계의 세 단계.
- **(C) Hu의 7개 모멘트** — 정규화 중심 모멘트의 회전 불변 조합.
- **(D) 모멘트가 구별할 수 있는 것과 없는 것** — 반사 모호성과 7개 스칼라의 한계.
- **(E) 형상 매칭** — 모멘트 벡터의 거리 메트릭과 `cv2.matchShapes`가 실제로 계산하는 것.
- **(F) Convex hull 기하** — 볼록성, 볼록성 결함, 그것들이 형상에 대해 밝혀주는 것.

### A. 모멘트: 형상의 적분 통계

이진 형상 `f(x, y) ∈ {0, 1}`(`1`이 형상 내부)에 대해, `(p + q)`차 원시 기하 모멘트:

```
M_{pq} = ΣΣ  x^p · y^q · f(x, y)
```

이것은 **모든 형상 픽셀에 대한 가중 합**이며, 가중치는 `x^p · y^q`. 서로 다른 `(p, q)` 선택이 서로 다른 정보를 추출:

- `M_{00}` = `Σ f(x, y)` = 전경 픽셀 수 = **면적**.
- `M_{10}` = `Σ x · f(x, y)` = 형상 픽셀의 x-좌표 합.
- `M_{01}` = `Σ y · f(x, y)` = y-좌표 합.
- `M_{11}` = 혼합 xy-좌표 합.
- `M_{20}` = x-좌표 제곱 합.
- `M_{02}` = y-좌표 제곱 합.

고차 모멘트로 가족이 이어집니다. 모두 직관적인 통계적 해석을 가집니다: 형상을 균일 확률 밀도로 보면 `M_{pq}`는 `E[X^p · Y^q]`에 비례.

### B. 원시 → 중심 → 정규화: 불변성 쌓아가기

#### B.1 원시 모멘트는 이동 불변이 아님

형상을 `(Δx, Δy)`만큼 이동하면 `p + q > 0`인 모든 원시 모멘트가 변합니다. 예: `M_{10}` → `M_{10} + Δx · M_{00}`. 형상이 항상 같은 위치에 있지 않는 한 매칭에는 쓸모없음.
...
```
(x̄, ȳ) = (M_{10} / M_{00},  M_{01} / M_{00})
```

그 다음 질량 중심 기준으로 모멘트를 계산:

```
μ_{pq} = ΣΣ  (x - x̄)^p · (y - ȳ)^q · f(x, y)
```

이것들은 **이동 불변**: 형상을 움직이면 질량 중심이 같은 양만큼 이동하고, 뺄셈으로 상쇄됩니다. 특히 `μ_{10} = μ_{01} = 0`이 항상(질량 중심의 정의상).

2차 중심 모멘트는 물리적 해석도 가집니다:

- `μ_{20}` = x-좌표의 분산 = "형상이 수평으로 얼마나 퍼져 있는가".
- `μ_{02}` = y-좌표의 분산 = 수직 퍼짐.
- `μ_{11}` = xy-공분산 = 회전 기울기.

2×2 행렬 `[[μ_{20}, μ_{11}], [μ_{11}, μ_{02}]]`이 **형상의 관성 텐서** — 그 고유값이 장축과 단축의 길이를, 고유벡터가 방향을 줍니다. 이것이 정확히 `cv2.fitEllipse`와 `minAreaRect`가 내부적으로 쓰는 계산입니다.

#### B.3 정규화 중심 모멘트 (스케일 불변)

형상을 `α`배 스케일하면 `μ_{pq}`는 `α^{p + q + 2}`만큼 스케일됩니다(`+2`는 면적 변화에서). 면적 스케일링을 나눠버림:

```
η_{pq} = μ_{pq} / M_{00}^((p + q)/2 + 1)
```

이제 `η_{pq}`는 **이동 및 스케일 불변**. 어떤 크기로 스케일해도 같은 `η` 값.

### C. Hu의 7개 모멘트: 회전 불변성 추가

회전은 같은 전체 차수의 중심 모멘트들을 서로 섞습니다: 같은 모양의 수평 타원과 수직 타원은 `μ_{20}`와 `μ_{02}`가 바뀝니다. 단일 모멘트는 회전 불변이 아니지만, **특정 조합은 불변입니다**. Hu(1962)는 3차까지 그런 조합 7개를 유도했습니다:
...
7개 모두 동시에 **이동, 스케일, 회전 불변**. 같은 형상의 어떤 포즈에서도 같은 7차원 형상 서명을 형성합니다.

### D. 모멘트가 구별할 수 있는 것과 없는 것

Hu 모멘트는 손실 압축입니다: 7개 숫자는 모든 형상 쌍을 완벽하게 구별할 만큼의 정보를 담을 수 없습니다. 알려진 한계:

- **반사 모호성**: `h₇`는 반사 아래 부호를 뒤집음 — 형상과 그 거울상을 구별하려면 부호 있는 `h₇`를 쓰거나 추가 테스트가 필요.
- **전역 통계만**: 모멘트는 형상의 벌크 분포를 요약하지만 세밀한 기하는 아님. 정사각형과 원은 다른 Hu 모멘트를 가지지만, 두 서로 다른 거의 원형 블롭은 종종 그렇지 않습니다.
- **수치 민감도**: `h₅, h₆, h₇`는 고차 모멘트를 포함하며 작은 형상에서 수치적으로 불안정. 많은 구현이 넓은 동적 범위를 압축하기 위해 비교 시 `|log(|h_i|)|` 또는 부호 보존 로그를 사용.

매칭이 신뢰성 있게 필요할 때, 현대 시스템은 **윤곽선 기반 디스크립터**(shape context, Fourier descriptor) 또는 CNN의 **학습된 특징 벡터**를 사용합니다. Hu 모멘트는 여전히 기본 베이스라인: 빠르고, 분석적으로 이해되어 있고, 단순 형상 분류에는 충분.

### E. 형상 매칭 메트릭

`cv2.matchShapes(contour1, contour2, method, parameter)`가 두 윤곽선을 비교 — 각각의 Hu 모멘트를 계산하고 세 거리 메트릭 중 하나를 적용. `m_i = sign(h_i) · log|h_i|`로 동적 범위를 압축하면:

- **`CV_CONTOURS_MATCH_I1`**: `Σᵢ |1/m_i^A - 1/m_i^B|` — 상대 절대 차이.
- **`CV_CONTOURS_MATCH_I2`**: `Σᵢ |m_i^A - m_i^B|` — 절대 차이.
- **`CV_CONTOURS_MATCH_I3`**: `Σᵢ |m_i^A - m_i^B| / |m_i^A|` — 비대칭 상대 차이(작은 `m_i^A`에 민감).

셋 모두 동일 형상에 0을 반환, 비유사도에 따라 증가. I2(선형)가 가장 흔한 기본값. I1과 I3는 큰/작은 불변량 중 어느 것을 더 중시하느냐에 따라 다르게 가중.

### F. Convex Hull 기하

형상은 내부 두 점을 잇는 선분이 항상 내부에 놓이면 **볼록**. 그렇지 않으면 오목이며 **볼록성 결함(convexity defects)** — 윤곽선이 convex hull에서 안쪽으로 휘는 영역 — 이 있습니다.

점 집합의 **convex hull**은 모든 점을 포함하는 가장 작은 볼록 다각형. 알고리즘: Graham scan `O(n log n)`, Quickhull 유사. OpenCV는 Sklansky 알고리즘을 쓰는데, 이미 정렬된 윤곽선 경계에 대해 `O(n)`.

볼록성 결함이 유용한 이유는 형상의 비볼록 부분을 특성화하기 때문:

- **손**은 네 개의 결함(손가락 쌍 사이)을 가짐. 결함 세기는 올린 손가락을 세는 고전적(신경망 이전) 기법.
- **별 모양**은 각 오목 각에 결함.
- **맞닿은 객체**는 접촉점에서 결함을 생성, 그것으로 분리 가능.

각 결함은 `(start_index, end_index, far_index, depth)`로 기술, `depth`는 그 돌출부에서 hull 에지에서 윤곽선까지의 최대 수직 거리. 깊이로 필터링하면 얕은(잡음 유발) 결함을 제거.

### 이론에서 아래 함수들로

- `cv2.moments(contour)` — `m_{pq}`, `μ_{pq}`, `η_{pq}`와 질량 중심 반환(§A–§B).
- `cv2.HuMoments(moments_dict)` — moments 딕셔너리에서 `h₁...h₇` 계산(§C).
- `cv2.matchShapes(c1, c2, method, parameter)` — Hu 모멘트 형상 거리(§E).
- `cv2.fitEllipse(contour)` — 관성 텐서의 고유값과 고유벡터를 타원으로 복원(§B.2).
- `cv2.minAreaRect(contour)`, `cv2.boundingRect(contour)`, `cv2.minEnclosingCircle(contour)`, `cv2.minEnclosingTriangle(contour)` — 경계 기하.
- `cv2.convexHull(contour)`, `cv2.isContourConvex(contour)`, `cv2.convexityDefects(contour, hull)` — §F convex hull 분석.
