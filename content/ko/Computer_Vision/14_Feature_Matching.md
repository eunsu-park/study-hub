...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 디스크립터 공간을 거리 공간으로 보는 관점, 디스크립터 유형별 L2 vs Hamming 거리 선택, 통계적 필터로서의 Lowe's ratio test, 그리고 기하학적 대응을 강건하게 추정하기 위한 RANSAC을 다룹니다.
...
특징점 매칭은 두 독립된 키포인트 집합을 **대응 집합** — "A 이미지의 이 점은 B 이미지의 저 점과 같은 물리적 점이다"를 말하는 쌍 `(kp_i^A, kp_{j(i)}^B)` — 으로 바꾸는 것입니다. 이것이 모든 기하학적 비전 작업의 전제조건: 호모그래피 추정, fundamental matrix 추정, 3D 삼각측량, 이미지 스티칭, Structure-from-Motion, Visual Odometry, SLAM.

매칭 문제는 두 어려운 부분을 가집니다. 첫째는 **어떤 디스크립터가 가장 가까운가** — 잠재적으로 고차원 공간에서의 최근접 이웃 탐색으로, 순진하게는 `O(n_A · n_B)` 비용. 둘째는 **어느 최근접 이웃이 실제로 맞는가** — A의 모든 키포인트가 B에 진짜 매치를 가지지 않으며(가림, 다른 장면 내용), 디스크립터가 잡음을 포함하므로 베스트 매치조차 틀릴 수 있습니다.

이 섹션은 다음을 다룹니다:

- **(A) 거리 공간으로서의 디스크립터 공간** — SIFT 대 ORB에서 "거리"가 의미하는 것, 그리고 메트릭 선택이 디스크립터 유형에 의해 결정되는 이유.
- **(B) 최근접 이웃 탐색** — brute-force 대 근사(FLANN), 속도/품질 트레이드오프.
- **(C) 매칭 필터링** — cross-check, Lowe's ratio test, 대칭성. 완벽한 디스크립터가 있어도 이것이 필요한 이유.
- **(D) 기하학적 검증** — 필터링 후에도 많은 매치가 틀림. RANSAC이 다수의 올바르지 않은 데이터에서 참 기하 모델을 복원.
- **(E) 호모그래피와 fundamental matrix** — 검증에 가장 흔히 쓰이는 두 기하 모델과 각각이 요구하는 가정.
...
흔히 쓰임: squared-L2 (속도를 위해 sqrt 생략; 같은 순서), L1(Manhattan 거리, 이상치 차원에 덜 민감).
...
```
d(a, b) = popcount(a XOR b)   = 다른 비트 수
```

비트별 XOR 후 설정 비트 수 세기(`__builtin_popcount` / `POPCNT` 명령어)로 계산. 256비트 디스크립터에서 Hamming 거리는 0(동일)에서 256(최대 다름) 범위; 무작위 쌍은 기대 거리 128.

이진 디스크립터의 Hamming 거리는 float 디스크립터의 L2보다 **대략 100배 빠름** — SIFT가 너무 느린 실시간 시스템에서 ORB가 쓰이는 이유.

규칙: **거리 메트릭을 디스크립터 유형과 맞추세요**. 이진 디스크립터에 L2를 쓰는 것은 잘못(거리 공간의 벡터가 아님); float 디스크립터에 Hamming은 아예 의미가 없음.

### B. 최근접 이웃 탐색: Brute-Force 대 FLANN
...
- **Float 디스크립터**의 경우, 무작위화된 **KD-트리** 숲을 사용. 각 트리가 디스크립터 공간을 초직사각형으로 분할; 몇 트리를 병렬로 탐색해 선형 스캔보다 훨씬 빠르게 근사-최근접 이웃을 생성.
- **이진 디스크립터**의 경우, **Locality-Sensitive Hashing (LSH)**을 사용. 무작위 초평면 투영이 유사 디스크립터가 충돌할 가능성 있는 해시 버킷을 생성. 최근접 이웃 찾기는 같은 버킷으로 해싱되는 후보만 확인하는 것.

FLANN의 속도 향상은 데이터셋 크기와 함께 커짐 — 큰 디스크립터 집합에 대해 보통 brute-force보다 10-100배 빠름, 참 최근접 이웃의 작은 비율을 놓치는 대가. 매칭이 더 큰 파이프라인의 한 단계일 때(예: 스티칭), 이것은 거의 항상 승리. 모든 다운스트림 단계가 FLANN의 근사 오차를 무색하게 하는 자체 잡음을 가지기 때문.

### C. 필터링: 어느 매치를 신뢰할지 결정
...
- `j(i) = argmin_j  d(desc_A[i], desc_B[j])` (B의 j가 A의 i의 베스트 매치), **그리고**
- `i = argmin_i  d(desc_A[i], desc_B[j(i)])` (A의 i가 B의 j(i)의 베스트 매치).
...
아이디어: 베스트 매치가 두 번째보다 훨씬 가까우면 올바를 가능성이 높음; 둘 다 비슷하게 가까우면 디스크립터가 모호(반복 패턴, 또는 진짜 매치가 없음)하므로 거부. 이것은 **통계적 테스트** — Lowe는 0.8 비율이 거짓 매치의 ~90%를 제거하면서 올바른 것의 ~5%만 잃는다고 경험적으로 보였습니다.

비율 테스트는 `knnMatch(k=2)` 후 비율 확인으로 구현:

```python
good_matches = [m for m, n in knn_matches if m.distance < 0.75 * n.distance]
```

이것이 SIFT/ORB 매칭 파이프라인의 표준 필터이며, 고전 컴퓨터 비전에서 실용적으로 가장 중요한 결과 중 하나.

### D. 기하학적 검증: RANSAC

디스크립터 기반 필터링 후에도 보통 20-50%의 이상치가 남음 — 디스크립터 필터를 통과하지만 다른 물리적 점에 대응하는 매치. 이상치는 반복 패턴, 조명 변화, 다른 객체의 거의 동일한 디스크립터, 또는 단순히 A의 점이 B에 진짜 매치를 갖지 않아서 발생.
...
1. 모델을 맞추는 데 필요한 최소 수의 점을 **무작위 샘플링**(호모그래피는 4개, fundamental matrix는 7 또는 8개).
2. **그 점들에만 모델을 맞춤** — 최소 해법기, 최소제곱 없음.
3. **Inlier 세기**: 이 모델 아래 오차가 임계값 이하인 전체 집합의 점들.
4. 많은 무작위 샘플에 대해 **반복**.
5. **가장 많은 inlier를 가진 모델 유지**. 선택적으로 모든 inlier에 최소제곱으로 다시 맞춰 최종 다듬기.
...
### E. 호모그래피와 Fundamental Matrix

OpenCV의 `findHomography(pts1, pts2, method=cv2.RANSAC, threshold=5.0)`가 호모그래피를 모델로(샘플당 4점) RANSAC 알고리즘을 실행. 3×3 호모그래피 행렬과 어느 매치가 inlier로 쓰였는지 나타내는 마스크를 반환.

호모그래피가 올바른 모델일 때에 대한 두 중요한 점:

- **평면 장면**. 모든 매칭된 3D 점이 단일 평면 위에 있음. 평면의 어떠한 두 이미지도(어떤 시점에서도) 호모그래피로 관계됨. 평평한 것(포스터, 바닥)의 이미지 스티칭과 교과서 예제는 이것을 씀.
- **순수 회전**. 카메라가 회전만 하면(이동 없이), 임의 3D 장면이 두 뷰 간에 호모그래피로 관계됨. 삼각대 장착 카메라로의 파노라마 스티칭 기반.

장면이 **카메라 이동과 함께 3D 구조**를 가지면, 호모그래피가 잘못된 모델 — `findFundamentalMat`를 대신 사용. Fundamental matrix는 rank 2인 3×3, epipolar 기하로 뷰를 관계시키며, RANSAC이 같은 방식으로 찾음(다른 최소 해법기, 다른 inlier 테스트).

### 이론에서 아래 함수들로

- `cv2.BFMatcher(normType, crossCheck)` — brute-force 최근접 이웃(§B.1). SIFT/SURF는 `normType = cv2.NORM_L2`(§A.1); ORB/BRIEF는 `cv2.NORM_HAMMING`(§A.2).
- `cv2.FlannBasedMatcher(indexParams, searchParams)` — 근사 최근접 이웃(§B.2). Float 디스크립터에 `KDTREE` 인덱스, 이진에 `LSH` 인덱스.
- `matcher.match(desc1, desc2)` — 쿼리당 단일 베스트 매치.
- `matcher.knnMatch(desc1, desc2, k)` — 쿼리당 top-k 매치, Lowe's ratio test(§C.2)에 필요.
- `cv2.findHomography(src_pts, dst_pts, method, ransacReprojThreshold)` — RANSAC 호모그래피 추정(§D + §E).
- `cv2.findFundamentalMat(pts1, pts2, method, ransacReprojThreshold)` — 이동을 가진 비평면 장면을 위한 RANSAC fundamental matrix.
- `cv2.drawMatches(img1, kp1, img2, kp2, matches, ...)` — 대응의 시각화.
...
- [객체 탐지 기초 (Object Detection Basics)](./15_Object_Detection_Basics.md) - Template Matching, Haar Cascade, HOG+SVM
