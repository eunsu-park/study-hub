...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 연속적인 장면이 어떻게 정수의 이산 배열이 되는지, OpenCV가 쓰는 픽셀 좌표계, 그리고 왜 RGB가 아니라 BGR이 기본인지를 다룹니다.
...
OpenCV에서 "디지털 이미지"는 부호 없는 8비트 정수로 구성된 3D NumPy 배열입니다. 실제 장면이 어떻게 이 배열로 변환되는지 — 그리고 세 개의 인덱스, 정수 값, 채널 순서가 실제로 무엇을 의미하는지 — 를 이해하는 것은 이 트랙의 모든 다른 연산의 전제 조건입니다. 이 섹션은 다음을 다룹니다:

- **(A) 연속 장면에서 이산 배열로** — 샘플링, 양자화, 그리고 이미지 처리 아티팩트가 근본적으로 어디서 오는가.
- **(B) 픽셀 좌표계** — 왜 `img[y, x]`에서 `y`가 먼저 오는지, 그리고 축의 방향은 무엇인지.
- **(C) 비트 깊이와 동적 범위** — 왜 8비트가 기본이며, 언제 그것이 문제가 되고, `float32` / 16비트 이미지는 무엇을 위한 것인지.
- **(D) 컬러 채널과 OpenCV가 BGR을 쓰는 이유** — 사용자를 계속 괴롭히는 역사적 사고.
...
### D. 컬러 채널 — 그리고 OpenCV가 BGR을 쓰는 이유
...
- `cv2.imread(path, flag)` — 디스크에서 읽어 §D에서 설명한 `uint8` BGR 배열을 생성.
- `cv2.imshow(window, img)` — BGR을 직접 표시(이는 `imread`와 일관). 그래서 matplotlib는 swap이 필요하고 `imshow`는 필요 없습니다.
- `cv2.imwrite(path, img)` — 같은 BGR 관례로 쓰고, 포맷이 요구할 때 §A.3의 양자화 단계를 적용(JPEG 압축은 손실; PNG는 무손실).
- `img.shape`, `img.dtype`, `img.size` — §C의 차원과 비트 깊이.
- `img[y, x]`, `img[y1:y2, x1:x2]` — §B의 `(행, 열)` 관례를 쓴 NumPy 인덱싱.
- `img[:, :, c]` — §D.1의 채널 추출.
