...
함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 밝기 불변성 제약(brightness constancy), 한 픽셀의 밝기 변화가 움직임을 결정하기에 부족한 이유(aperture problem), 그리고 이를 해결하는 두 고전적 가족: Lucas-Kanade(국소 최소제곱)와 Horn-Schunck(전역 매끄러움 정칙화)을 다룹니다.
...
Optical flow는 두 연속 비디오 프레임을 관계 짓는 **픽셀별 속도장(velocity field)**입니다. 시간 `t`의 각 픽셀 `(x, y)`에 대해 optical flow는 "이 픽셀 내용이 시간 `t + 1`에는 `(x + u, y + v)`로 이동했다"는 의미의 2D 벡터 `(u, v)`를 할당합니다. 이것이 동작 추정, 비디오 추적, 프레임 보간, 비디오 압축, 행동 인식의 기초입니다.

문제는 단순해 보이지만 깔끔히 해결되는 데 수십 년이 걸린 미묘함을 담고 있습니다. **한 픽셀의 밝기 변화는 그 움직임을 결정하기에 정보가 부족합니다**(*aperture problem*). 움직임을 특정하기 위한 서로 다른 가정이 두 고전적 가족 — Lucas-Kanade와 Horn-Schunck — 을 낳았고, 같은 근본 문제가 현대 딥러닝 방법도 여전히 지배합니다.

이 섹션은 다음을 다룹니다:

- **(A) 밝기 불변성 제약** — optical flow를 정의하는 단 하나의 방정식과 그것이 미결정된 이유.
- **(B) Aperture 문제** — 단 하나의 방정식이 무한히 많은 해를 갖는 기하학적 이유.
- **(C) Lucas-Kanade** — 작은 창에서 흐름이 일정하다고 가정, 여러 밝기 방정식을 쌓아 최소제곱으로 풀기.
- **(D) Horn-Schunck** — 전역 매끄러움 사전분포로 흐름장 전체를 한 번에 풀기.
- **(E) Coarse-to-fine 피라미드** — 이 방법들을 큰 움직임까지 확장하는 표준 트릭.
- **(F) 현대 학습 기반 방법이 바꾼 것** — 딥 네트워크가 벤치마크에서 고전 방법을 능가하지만 근본 제약은 여전한 이유.
...
### E. Coarse-to-Fine 피라미드: 큰 움직임 다루기
...
- `cv2.calcOpticalFlowPyrLK(prev, next, prevPts, ...)` — 피라미드 Lucas-Kanade(§C + §E). 희소 — 추적할 점을 주입(보통 `goodFeaturesToTrack`에서).
- `cv2.calcOpticalFlowFarneback(prev, next, ...)` — Farnebäck의 밀집 방법(Horn-Schunck 가족의 다항식 전개 변형, §D). LK보다 느리지만 모든 픽셀에 흐름 제공.
- `cv2.DISOpticalFlow_create(...).calc(prev, next, ...)` — Dense Inverse Search, 유사 품질에 Farnebäck보다 보통 ~5배 빠른 새로운 밀집 방법.
- `cv2.optflow.calcOpticalFlowDenseRLOF` (`opencv_contrib`에) — Robust Local Optical Flow, 현대 변분 방법.
- RAFT / 학습 모델의 경우: PyTorch 참조 구현 또는 내보낸 ONNX 모델로 `cv2.dnn` 사용.
