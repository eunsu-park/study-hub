...
참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 통합 장면 이해로서의 파놉틱 세그멘테이션, things-vs-stuff 온톨로지, Panoptic FPN의 2-헤드 융합, 그리고 recognition과 segmentation quality로 분해되는 PQ 메트릭을 다룹니다.
...
파놉틱 세그멘테이션(Kirillov 등, 2019)은 그 이전의 두 세그멘테이션 작업을 통합: 시맨틱 세그멘테이션(§25)은 모든 픽셀에 클래스를 할당하지만 객체 인스턴스를 분리할 수 없음; 인스턴스 세그멘테이션(§26)은 인스턴스를 분리하지만 "things"(셀 수 있는 객체)만 세그멘트하고 "stuff"(하늘, 도로, 잔디 같은 비정형 영역)를 무시. 파놉틱 세그멘테이션은 **모든 픽셀에 대한 클래스 레이블과 인스턴스 ID**를 요구, things와 stuff를 균일하게 다룸.

이 섹션은 다음을 다룹니다:

- **(A) 통합 출력 포맷** — 파놉틱 세그멘테이션이 반환하는 것과 그것이 유용한 이유.
- **(B) Things vs stuff** — 작업 정의에 내장된 존재론적 구분.
- **(C) Panoptic FPN** — 시맨틱과 인스턴스 예측을 결합하는 2-헤드 접근.
- **(D) 통합 프레임워크로서의 Mask2Former** — Transformer가 작업 유형 간 구분을 제거하는 방법.
- **(E) Panoptic Quality (PQ)** — 메트릭과 Recognition Quality / Segmentation Quality로의 분해.
...
### D. Mask2Former: 모든 것을 위한 하나의 아키텍처
...
- **Panoptic FPN**: Detectron2의 파놉틱 구현; 입력 RGB, 출력 파놉틱 마스크 + 시맨틱 + 인스턴스 맵.
- **Mask2Former**: HuggingFace transformers가 사전 훈련된 파놉틱 모델 제공. 단일 모델 추론이 파놉틱 마스크를 직접 반환.
- **Panoptic DeepLab**: bottom-up 접근(시맨틱 헤드 + 중심으로의 오프셋 예측).
- **MaskFormer / kMaX-DeepLab**: transformer 접근의 변형.
