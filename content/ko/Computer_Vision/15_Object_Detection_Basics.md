...
OpenCV 함수 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. Sliding-window 패러다임, 템플릿 매칭 뒤의 유사도 메트릭, Haar 특징에 AdaBoost를 적용한 cascade 이론, HOG+SVM 파이프라인, 그리고 오늘날 모든 검출기가 여전히 사용하는 IoU/NMS 후처리를 다룹니다.
...
객체 탐지는 분류보다 어려운 질문을 합니다: **"이 이미지의 어느 영역에 객체가 있고, 각각이 무엇인가?"** 분류기는 "이 이미지는 고양이"라고 말합니다. 검출기는 "여기에 고양이 하나, 저기에 개 하나, 나머지에는 아무것도 없음"이라고 말합니다. 모든 고전 검출기는 같은 전체 아키텍처를 공유합니다: **모든 후보 영역을 점수 매기고, 좋은 것을 유지하고, 중복 제거**. 현대 신경 검출기(YOLO, Faster R-CNN)는 주로 점수가 매겨지는 방식에서 다릅니다 — 나머지 두 단계는 거의 그대로입니다.

이 섹션은 다음을 다룹니다:

- **(A) Sliding-window 패러다임** — 모든 고전 검출기가 구현하는 기반 접근.
- **(B) 템플릿 매칭** — 가장 단순한 점수 함수(픽셀별 유사도)와 세 표준 메트릭.
- **(C) Haar 특징과 cascade 분류기** — Viola-Jones가 2001년에 integral image와 AdaBoost로 실시간 얼굴 검출을 달성한 방법.
- **(D) HOG + SVM** — 기울기 히스토그램 특징을 검출에 도입한 Dalal-Triggs(2005) 보행자 검출 파이프라인.
- **(E) IoU와 Non-Maximum Suppression** — 여러 겹친 검출을 깨끗한 리스트로 바꾸는 후처리.

### A. Sliding-Window 패러다임
...
### C. Haar 특징과 Viola-Jones Cascade
...
### E. IoU와 Non-Maximum Suppression (NMS)
...
- `cv2.matchTemplate(img, template, method)` — 템플릿 매칭(§B). `method` = `TM_CCOEFF_NORMED`가 안전한 기본값.
- `cv2.minMaxLoc(result)` — 템플릿 매치 결과 맵에서 최고 점수 위치 찾기.
- `cv2.CascadeClassifier(xml_path)` — 사전 훈련된 Haar cascade 로드; `.detectMultiScale(img, ...)`가 스케일 전반에 걸쳐 cascade 실행(§C).
- `cv2.HOGDescriptor()`, `cv2.HOGDescriptor_getDefaultPeopleDetector()` — Dalal-Triggs HOG+SVM (§D).
- `cv2.dnn.NMSBoxes(boxes, scores, score_threshold, nms_threshold)` — greedy NMS (§E.2), 현대 DNN 검출기에서 재사용.
