# Lesson 4: 어텐션 해석(Attention Interpretation)

[이전: 클래스 활성화 매핑](./03_Class_Activation_Mapping.md) | [다음: 프로빙 및 표현 분석](./05_Probing_and_Representation_Analysis.md)

---

## 학습 목표

- HuggingFace Transformer 모델에서 헤드(Head), 레이어(Layer), 모델(Model) 수준의 어텐션 행렬(Attention Matrix)을 추출하고 시각화
- 어텐션 롤아웃(Attention Rollout, Abnar & Zuidema 2020)을 구현하여 모든 레이어에 걸친 토큰 수준 중요도 계산
- "어텐션은 설명이 아니다(Attention is not Explanation)" 주장(Jain & Wallace 2019)을 코드 실험으로 비판적으로 평가
- Wiegreffe & Pinter(2019)의 반론과 어텐션이 유의미한 조건 이해
- 잔차 스트림(Residual Stream)을 반영한 유효 어텐션(Effective Attention)을 계산하고 감성 분석에 적용

---

트랜스포머(Transformer)는 NLP, 비전(Vision), 멀티모달(Multimodal) AI 전반에서 지배적인
아키텍처가 되었습니다. 자연스러운 질문은 매 순전파(Forward Pass)마다 계산되는
어텐션 가중치(Attention Weight)가 모델 예측의 설명으로 사용될 수 있는지 여부입니다.
결론적으로 대답은 복잡합니다. 어텐션 가중치는 어떤 입력이 중요한지에 대해
*때때로* 유용한 신호를 제공하지만, 설명으로서 충분하지도 필수적이지도 않습니다.

이 레슨은 어텐션 해석의 전체 흐름을 다룹니다: 어텐션 추출 및 시각화, 레이어 간
집계(롤아웃, 플로우), Jain & Wallace(2019)의 회의적 증거 직면,
Wiegreffe & Pinter(2019)의 응답과의 조화, 그리고 균형 잡힌 실용적 이해에
도달합니다. HuggingFace Transformers를 사용하여 모든 것을 구현하고 실제
감성 분류(Sentiment Classification) 작업에 적용합니다.

---

## 1. 어텐션 메커니즘 복습(Attention Mechanics Recap)

### 1.1 트랜스포머의 셀프 어텐션(Self-Attention in Transformers)
...
### 1.2 멀티헤드(Multi-Head) 및 다층(Multi-Layer) 복잡성
...
## 2. HuggingFace 모델에서 어텐션 추출(Extracting Attention from HuggingFace Models)

### 2.1 기본 추출(Basic Extraction)
...
### 2.2 세 가지 수준의 시각화(Visualization at Three Levels)
...
### 2.3 BertViz를 활용한 인터랙티브 시각화(Using BertViz for Interactive Visualization)
...
## 3. 어텐션 롤아웃(Attention Rollout, Abnar & Zuidema 2020)

### 3.1 단일 레이어 어텐션의 문제점(The Problem with Single-Layer Attention)
...
## 4. 어텐션 플로우(Attention Flow, 정보 이론적 접근)
...
## 5. "어텐션은 설명이 아니다(Attention is not Explanation)" (Jain & Wallace 2019)

이 논문은 어텐션 가중치가 설명으로서 신뢰할 수 없다는 강력한 증거를 제시하여
NLP 해석가능성(Interpretability) 커뮤니티에 큰 충격을 주었습니다.

### 5.1 핵심 주장(The Key Arguments)
...
### 5.2 대안적 분포 실험 재현(Reproducing the Alternative Distributions Experiment)
...
### 5.3 그래디언트 중요도와의 상관관계(Correlation with Gradient Importance)
...
## 6. "어텐션은 설명이 아닌 것이 아니다(Attention is not not Explanation)" (Wiegreffe & Pinter 2019)

### 6.1 반론(The Counter-Arguments)
...
### 6.2 적대적 테스트 구현(Implementing the Adversarial Test)
...
## 7. 조화: 어텐션이 유의미한 경우(The Reconciliation: When Attention IS Informative)
...
## 8. 유효 어텐션: 잔차 스트림 통합(Effective Attention: Incorporating the Residual Stream)

### 8.1 원시 어텐션이 오해를 유발하는 이유(Why Raw Attention Misleads)
...
## 9. 실용 적용: 감성 분석 어텐션(Practical Application: Sentiment Analysis Attention)

### 9.1 전체 파이프라인(Complete Pipeline)
...
## 10. 모범 사례 및 권장 사항(Best Practices and Recommendations)
...
- **어텐션 추출(Attention Extraction)**: HuggingFace 모델에서 `output_attentions=True`로 간단히 추출할 수 있습니다. 각 레이어는 헤드당 하나의 어텐션 행렬을 생성하며, BERT-base의 경우 총 144개의 행렬이 됩니다. 헤드, 레이어, 모델 수준에서 시각화하면 서로 다른 패턴(위치적, 구문적, 의미적)이 드러납니다.

- **어텐션 롤아웃(Attention Rollout)** (Abnar & Zuidema 2020)은 어텐션 행렬을 레이어 간에 곱하여(잔차 항등 행렬 추가) 집계합니다. 이를 통해 다층 정보 흐름을 포착하는 단일 토큰 중요도 점수를 생성합니다. 권장되는 기본 집계 방법입니다.

- **"어텐션은 설명이 아니다(Attention is not Explanation)"** (Jain & Wallace 2019)는 대안적 어텐션 분포가 동일한 예측을 생성할 수 있으며, 어텐션이 그래디언트 기반 중요도와 약하게 상관관계를 보인다는 것을 보여줍니다. 이는 어텐션을 특성 중요도로 해석하는 순진한 접근 방식에 도전합니다.

- **"어텐션은 설명이 아닌 것이 아니다(Attention is not not Explanation)"** (Wiegreffe & Pinter 2019)는 대안의 존재가 어텐션을 무효화하지 않으며, 적대적 훈련이 어텐션이 부분적으로 제약됨을 보여주고, 완전한 설명은 아니더라도 유용한 부분적 신호를 제공한다고 주장합니다.

- **조화(Reconciliation)**: 어텐션은 명시적 정렬이 필요한 작업(번역, QA), 얕은 모델, 레이어 간 집계 시 더 유의미합니다. 분류 작업의 심층 모델에서는 덜 유의미합니다. 항상 그래디언트 방법으로 검증해야 합니다.

- **유효 어텐션(Effective Attention)** (Brunner et al. 2020)은 잔차 스트림을 고려하여, 잔차가 종종 지배적임을 보여줍니다. 이는 원시 어텐션이 출력 표현에 대한 어텐션 메커니즘의 영향을 과대 평가함을 의미합니다.

- **프로덕션 사용 시**: 권장 파이프라인은 탐색적 분석을 위한 어텐션 롤아웃(Attention Rollout), 엄밀한 어트리뷰션을 위한 그래디언트 기반 방법(Integrated Gradients), 그리고 신뢰도 보정을 위한 두 방법 간 비교입니다.

---

## 연습문제(Exercises)

### 연습문제 1: 어텐션 패턴 발견(Attention Pattern Discovery, 탐색적)

1. BERT-base를 로드하고 다음 문장을 입력하세요: "The cat chased the mouse because it was hungry."
2. 레이어 0과 레이어 11의 12개 헤드를 모두 시각화하세요.
3. 다음을 식별하세요: (a) 위치적 헤드(인접 토큰에 주목), (b) [SEP]에 주목하는 헤드, (c) 다양한 어텐션 패턴을 보이는 헤드.
4. 레이어 11에서 "it"은 무엇에 주목하나요? 공참조(Coreference)를 해결하는 헤드(it -> cat)가 있나요?

### 연습문제 2: 롤아웃 vs. 원시 어텐션(Rollout vs. Raw Attention, 코딩 + 분석)

1. 10개의 감성 분석 예제에 대해 BERT의 어텐션 롤아웃을 구현하세요.
2. 각 예제에 대해 다음을 계산하세요: (a) [CLS]에 대한 원시 마지막 레이어 어텐션, (b) [CLS]에 대한 어텐션 롤아웃, (c) 그래디언트 기반 중요도(임베딩에 대한 Integrated Gradients).
3. 각 어텐션 방법과 그래디언트 중요도 간의 스피어만 상관계수(Spearman Correlation)를 계산하세요.
4. 어떤 방법이 그래디언트와 더 잘 일치하나요? 이것이 예제 전반에 걸쳐 일관성이 있나요?

### 연습문제 3: Jain & Wallace 재현(The Jain & Wallace Replication, 연구)

1. BERT를 SST-2에 파인튜닝하거나 HuggingFace의 사전 훈련된 SST-2 모델을 사용하세요.
2. 50개 테스트 예제에 대해 어텐션-그래디언트 스피어만 상관계수를 계산하세요.
3. 상관계수 분포를 그래프로 나타내세요. 0.5를 초과하는 비율은 얼마인가요?
4. 어텐션과 그래디언트가 강하게 불일치하는 예제를 찾으세요. 그 이유를 분석하세요.

### 연습문제 4: 유효 어텐션 분석(Effective Attention Analysis, 코딩)

1. Section 8.2의 `compute_effective_attention` 함수를 구현하세요.
2. BERT의 레이어 0, 6, 11에 대해 원시 어텐션과 유효 어텐션을 비교하세요.
3. 어떤 레이어에서 잔차 스트림이 가장 지배적인가요?
4. 감성 분류 예제에 대해, 유효 어텐션이 원시 어텐션보다 그래디언트 중요도와 더 잘 상관관계를 보이나요?

### 연습문제 5: 교차 아키텍처 비교(Cross-Architecture Comparison, 고급)

1. 세 가지 다른 트랜스포머 아키텍처에서 어텐션을 추출하세요: BERT(양방향), GPT-2(인과적/자기회귀), DistilBERT(증류).
2. 동일한 입력 텍스트에 대해 다음을 비교하세요: (a) 어텐션 패턴, (b) 롤아웃 분포, (c) 그래디언트 중요도와의 상관관계.
3. "어텐션은 설명이 아니다" 발견이 세 아키텍처 모두에 동일하게 적용되나요? 어떤 아키텍처의 어텐션이 가장 유의미한가요?

---

[이전: 클래스 활성화 매핑](./03_Class_Activation_Mapping.md) | [개요](./00_Overview.md) | [다음: 프로빙 및 표현 분석](./05_Probing_and_Representation_Analysis.md)
