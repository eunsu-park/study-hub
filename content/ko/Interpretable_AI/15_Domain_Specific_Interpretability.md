# 레슨 15: 도메인 특화 해석 가능성(Domain-Specific Interpretability)

[이전: 프로덕션 해석 가능성](./14_Production_Interpretability.md) | [다음: 기계적 해석 가능성](./16_Mechanistic_Interpretability.md)

---

## 학습 목표

- 임상 의사결정 지원에 대한 FDA 가이던스를 적용하여 임상의, 환자, 규제기관에 적절한 설명을 제공하는 해석 가능한 의료 AI 시스템을 설계한다
- ECOA 준수 불리한 조치 통지 및 SR 11-7 모델 리스크 관리를 금융 AI에 구현한다 (신용 평가를 위한 특성 중요도 및 스트레스 테스트 포함)
- 토큰 수준 SHAP, 어텐션 기반 하이라이팅, 근거 추출(ERASER 벤치마크)을 사용하여 충실한 텍스트 설명으로 NLP 모델 예측을 설명한다
- 픽셀 귀속 방법과 개념 기반 설명을 의료 영상 및 자율 주행 등의 컴퓨터 비전 작업에 적용한다
- 이해관계자 요구사항, 규제 요건, 기술적 제약을 고려하는 구조화된 의사결정 매트릭스를 사용하여 주어진 도메인에 적합한 해석 가능성 방법을 선택한다

---

## 1. 의료 AI 해석 가능성(Healthcare AI Interpretability)

### 1.1 규제 맥락: FDA와 임상 의사결정 지원(Clinical Decision Support)

의료 AI는 가장 엄격한 규제 요건 하에서 운영된다. FDA의 의료 기기 소프트웨어(Software as a Medical Device, SaMD) 프레임워크는 시스템의 의도된 용도와 위험도에 따라 어떤 수준의 감독이 적용되는지 결정한다.
...
### 1.2 의료 영상을 위한 GradCAM

GradCAM은 의료 영상 AI에서 가장 널리 사용되는 해석 가능성 방법이다. 임상의가 자신의 도메인 전문 지식과 대조하여 평가할 수 있는 시각적 설명을 생성한다.
...
### 1.3 진단을 위한 개념 기반 설명(Concept-Based Explanations)
...
## 2. 금융 AI 해석 가능성(Financial AI Interpretability)

### 2.1 ECOA 불리한 조치 통지(Adverse Action Notices)

균등신용기회법(Equal Credit Opportunity Act, ECOA)은 대출기관이 신용을 거부할 때 구체적인 이유를 제공하도록 요구한다. 이는 해석 가능성에 대한 법적 의무를 생성한다.
...
### 2.2 SR 11-7 모델 리스크 관리(Model Risk Management)
...
## 3. NLP 해석 가능성(NLP Interpretability)

### 3.1 텍스트 분류를 위한 토큰 수준 SHAP(Token-Level SHAP)
...
### 3.2 근거 추출(Rationale Extraction, ERASER 벤치마크)
...
## 4. 컴퓨터 비전 해석 가능성(Computer Vision Interpretability)

### 4.1 픽셀 귀속 방법 비교(Pixel Attribution Comparison)
...
## 5. 방법 선택 의사결정 매트릭스(Method Selection Decision Matrix)

### 5.1 도메인에 적합한 방법 선택하기
...
## 6. 실습: 다중 이해관계자 설명이 포함된 임상 예측 모델
...
- **의료 AI**는 FDA CDS 가이던스 하에서 운영되며, 해석 가능성이 규제 분류에 직접 영향을 미친다 -- 투명한 모델은 기기 규제에서 면제될 수 있지만 블랙박스 버전은 전체 FDA 승인이 필요하다
- **GradCAM**은 의료 영상의 지배적 방법이지만 모델이 어디를 보는지만 보여주고 왜 그런 결정을 내리는지는 보여주지 않는다 -- 임상 온톨로지(ABCD 기준, BIRADS)를 사용한 개념 기반 설명(Concept-Based Explanations)이 픽셀 귀속과 임상 추론 사이의 격차를 해소한다
- **금융 AI**는 ECOA 불리한 조치 통지 요건(신용 거부에 대한 구체적 이유)과 SR 11-7 모델 리스크 관리(개념적 건전성, 결과 분석)를 준수해야 한다 -- SHAP 값은 불리한 조치 사유 코드에 직접 매핑된다
- **NLP 해석 가능성**은 토큰 수준 귀속(SHAP, 차폐)과 근거 추출(Lei et al., ERASER 벤치마크)을 사용하여 예측을 주도하는 단어를 식별하며, 충실성(faithfulness)과 충분성(sufficiency)이 핵심 품질 지표이다
- **컴퓨터 비전**은 여러 귀속 방법(바닐라 그래디언트, 적분 그래디언트(Integrated Gradients), GradCAM, SmoothGrad)을 비교하는 것이 유익하다 -- 방법들이 일치하면 설명에 대한 확신이 높고, 불일치하면 조사가 필요하다
- **의사결정 매트릭스(Decision Matrix)**는 (도메인, 데이터 유형, 이해관계자)를 지연 시간, 충실성, 규제 요건을 고려한 권장 해석 가능성 방법에 매핑한다
- **동일한 모델도 다른 이해관계자에게는 다른 설명이 필요하다**: 임상의는 임상 용어와 비정상 값 표시가 필요하고, 환자는 쉬운 언어와 실행 가능한 조언이 필요하며, 감사자는 세분화된 지표와 문서가 필요하다
...
다음 각 AI 시스템이 CDS 면제 기준을 충족하는지 판단하라:

1. 심전도(ECG) 신호를 분석하여 심방세동을 감지하고 의사에게 알리는 AI
2. 규칙 데이터베이스를 사용하여 환자의 체중, 신장 기능, 약물 상호작용을 기반으로 약물 용량을 권장하는 의사결정 지원 도구
3. 흉부 X선에서 5년 사망 위험을 예측하는 딥 러닝 모델로, 웰니스 앱에서 환자에게 표시됨
4. 임상 노트에서 청구 코딩을 위한 구조화된 데이터를 추출하는 자연어 처리 시스템

각각에 대해 어떤 CDS 기준(1-4)을 충족하지 못하는지(해당되는 경우) 식별하고, 면제를 달성하기 위해 어떤 변경이 필요한지 설명하라.

### 연습문제 2: ECOA 불리한 조치 파이프라인

완전한 불리한 조치 통지 파이프라인을 구축하라:

1. 10개의 특성을 가진 합성 데이터로 신용 평가 모델을 훈련한다
2. 각 거부된 신청에 대해 SHAP 값을 계산한다
3. 상위 4개 SHAP 특성을 소비자가 읽을 수 있는 사유 코드에 매핑한다
4. 5개의 샘플 거부에 대해 형식화된 불리한 조치 통지를 생성한다
5. 사유가 SHAP 값을 정확하게 반영하는지 검증한다(오해의 소지가 있는 사유 없음)
6. 인구통계 그룹 간 사유 분포가 금지된 차별을 드러내지 않는지 확인한다

### 연습문제 3: NLP 충실성 평가

ERASER 벤치마크 방법론을 사용하여 텍스트 분류 작업에서 세 가지 토큰 귀속 방법의 충실성을 평가하라:

1. 차폐 기반, 그래디언트 기반, 어텐션 기반 귀속을 구현한다
2. 각 방법에 대해 20%, 30%, 40% 압축률로 근거를 추출한다
3. 충분성(sufficiency)을 계산한다: P(y|rationale) vs P(y|full text)
4. 포괄성(comprehensiveness)을 계산한다: P(y|full text) - P(y|complement)
5. 충실성 기준으로 방법의 순위를 매기고 어떤 것이 가장 신뢰할 수 있는지 설명한다

### 연습문제 4: 다중 이해관계자 설명 시스템

네 명의 이해관계자를 위한 완전한 다중 이해관계자 설명 시스템을 대출 승인 모델에 대해 설계하고 구현하라:

1. **신청자**: 반사실적(counterfactual) 설명이 포함된 쉬운 언어 설명("무엇이 바뀌면 결정이 달라지는가")
2. **대출 담당자**: 위험 요소가 강조된 특성 중요도 대시보드
3. **컴플라이언스 담당자**: 편향 분석 및 불리한 조치 통지 생성
4. **모델 검증자**: 안정성 분석, 특성 중요도 드리프트, 분포 밖(out-of-distribution) 감지

각 이해관계자에 대해 설명 생성을 구현하고, 출력을 적절하게 형식화하며, 샘플 입력으로 시연하라.

### 연습문제 5: 도메인 전환 도전

신용 평가(정형 데이터)에 대해 잘 테스트된 SHAP 설명 파이프라인이 있다. 이를 세 가지 새로운 도메인에 적용하라:

1. **의료 진단** (정형 환자 데이터): 특성 이름 지정, 정규화, 표현에서 어떤 변경이 필요한가?
2. **문서 분류** (텍스트): 가변 길이 입력과 토큰 상호작용을 어떻게 처리하는가?
3. **품질 검사** (이미지): 픽셀 수준에서 영역 수준 설명으로 어떻게 전환하는가?

각 도메인에 대해 다음을 식별하라: (a) 신용 평가 파이프라인에서 재사용할 수 있는 것, (b) 재구축해야 하는 것, (c) 적용되는 새로운 도메인 특화 요건.

---

[이전: 프로덕션 해석 가능성](./14_Production_Interpretability.md) | [개요](./00_Overview.md) | [다음: 기계적 해석 가능성](./16_Mechanistic_Interpretability.md)
