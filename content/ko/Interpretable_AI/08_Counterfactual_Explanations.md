# 레슨 8: 반사실적 설명(Counterfactual Explanations)

[이전: 개념 기반 설명](./07_Concept_Based_Explanations.md) | [다음: 해석 가능성을 위한 인과 추론](./09_Causal_Inference_for_Interpretability.md)

---

## 학습 목표

- 반사실적 설명(Counterfactual Explanations)을 최적화 문제로 형식화하고 Wachter et al. (2017) 공식을 처음부터 구현한다
- 집합 다양성을 위한 행렬식 점 과정(Determinantal Point Processes)을 사용하여 DiCE로 다양한 반사실적 설명을 생성한다
- 근접성(Proximity), 희소성(Sparsity), 타당성(Plausibility), 실행 가능성(Actionability) 메트릭을 사용하여 반사실적 설명의 품질을 평가한다
- 반사실적 설명이 특성 간 실제 의존성을 존중하도록 인과적 제약 조건을 통합한다
- 다중 클래스 환경에서 "왜 B가 아닌 A인가?"에 답하는 대조적 설명(Contrastive Explanations)을 구축한다

---

대출 신청이 거부되었을 때, 신청자는 "특성 7이 점수에 -0.3 기여했습니다"라는 것을 알고 싶어하지 않는다. 그들이 알고 싶은 것은: **"승인을 받으려면 무엇을 변경해야 하는가?"**이다. 이것이 반사실적 설명의 핵심 약속이다: 결정에 대한 책임을 귀속시키는 대신, 결정을 변경할 수 있는 입력에 대한 최소한의 변경을 기술한다.

반사실적 설명은 본질적으로 실행 가능하다. 자연스럽게 추천으로 전환되며("소득을 $5,000 늘리고 부채를 $2,000 줄이세요") 알고리즘적 구제(Algorithmic Recourse) — 불리한 자동화된 결정을 변경하기 위해 무엇을 할 수 있는지 알 권리 — 에 대한 법적 요구 사항을 충족한다.

---

## 1. 반사실적 설명의 기초(Counterfactual Foundations)
...
## 2. DiCE: 다양한 반사실적 설명(DiCE: Diverse Counterfactual Explanations)
...
## 3. 반사실적 설명의 품질 메트릭(Quality Metrics for Counterfactual Explanations)
...
## 4. 인과적 제약 조건(Causal Constraints)

### 4.1 인과적 제약 조건이 중요한 이유
...
## 5. 대조적 설명(Contrastive Explanations)
...
## 6. 실습: 대출 승인 반사실적 설명(Practical: Loan Approval Counterfactuals)
...
- **반사실적 설명(Counterfactual Explanations)**은 모델의 예측을 변경하는 입력에 대한 최소한의 변경을 기술한다. 본질적으로 실행 가능하며 알고리즘적 구제(Algorithmic Recourse) 요구 사항을 충족한다.
- **Wachter et al. (2017)**은 반사실적 설명 생성을 예측 손실(목표 클래스 도달)과 거리 손실(변경 최소화)을 균형 잡는 최적화 문제로 공식화한다. L1 거리는 희소한 변경을 장려하고, L2는 부드러운 변경을 장려한다.
- **DiCE**(Mothilal et al. 2020)는 행렬식 점 과정(Determinantal Point Processes)을 사용하여 다양한 반사실적 설명 집합을 생성하여, 사용자에게 결과를 변경하는 여러 경로를 제공한다. 다양성은 단일 설명에 대한 과도한 의존을 방지한다.
- **품질 메트릭(Quality Metrics)**에는 근접성(얼마나 가까운가), 희소성(얼마나 적은 특성이 변경되었는가), 타당성(LOF/KDE를 통한 현실성), 유효성(목표를 달성하는가), 실행 가능성(변경이 실현 가능한가)이 포함된다.
- **인과적 제약 조건(Causal Constraints)**은 반사실적 설명이 실세계 의존성을 존중하도록 보장한다: 소득이 증가하면 저축도 증가해야 한다. 이는 인과 그래프를 통해 개입을 전파하기 위한 구조적 인과 모델이 필요하다.
- **대조적 설명(Contrastive Explanations)**은 두 클래스를 구분하는 최소한의 특성을 식별하여 "왜 B가 아닌 A인가?"에 답하며, 인간이 결정에 대해 자연스럽게 추론하는 방식과 일치한다.

---

## 연습 문제(Exercises)

### 연습 1: 기본 반사실적 설명 생성 (초급)

UCI Adult Income 데이터셋에서 로지스틱 회귀 모델을 훈련하라(소득 >$50k 예측). $50k 미만을 버는 10명의 개인에 대해 Wachter 방법을 사용하여 반사실적 설명을 생성하라. L1과 L2 거리 메트릭을 비교하라: 어떤 것이 더 희소한 설명을 생성하는가? 어떤 것이 더 작은 총 변경을 생성하는가? 결정 경계와 함께 특성 공간에서 반사실적 설명을 시각화하라.

### 연습 2: DiCE를 이용한 다양한 반사실적 설명 (중급)

신용 부도 데이터셋에서 dice-ml 라이브러리를 사용하여 거부된 20명의 신청자 각각에 대해 5개의 다양한 반사실적 설명을 생성하라. 반사실적 설명 집합의 쌍별 다양성(평균 쌍별 L1 거리)을 측정하라. DPP 다양성 방법과 무작위 초기화를 비교하라. 모든 신청자에 걸쳐 몇 개의 구별되는 "전략"(유사한 반사실적 설명의 클러스터)이 나타나는가?

### 연습 3: 반사실적 설명 품질 감사 (중급)

세 가지 방법으로 생성된 반사실적 설명에 대해 전체 CounterfactualEvaluator를 구현하라: Wachter, DiCE, 그리고 무작위 기준선(우연히 결정 경계를 넘는 무작위 섭동). 근접성, 희소성, 타당성(LOF), 실행 가능성에 대해 세 가지를 모두 비교하라. 각 방법의 품질 트레이드오프를 보여주는 레이더 차트를 생성하라.

### 연습 4: 인과적 반사실적 설명 (고급)

독일 신용 데이터셋에 대해 최소 5개의 인과 간선(예: 직업 유형이 소득의 원인, 나이가 고용 기간의 원인)을 가진 인과 그래프를 정의하라. CausalCounterfactual 클래스를 구현하고 거부된 10명의 신청자에 대한 설명을 생성하라. 인과적 반사실적 설명을 표준(비인과적) 반사실적 설명과 비교하라: 표준 반사실적 설명이 인과적 제약 조건을 얼마나 자주 위반하는가? 인과성을 통합함으로써 타당성 향상을 측정하라.

### 연습 5: 대조적 설명 시스템 (고급)

다중 클래스 질병 진단 모델(최소 5개 질병)을 위한 대조적 설명 시스템을 구축하라. 각 예측에 대해 상위 2개 대안 진단에 대한 대조적 설명을 생성하라: "왜 B가 아닌 질병 A인가?" 및 "왜 C가 아닌 질병 A인가?" 의학 교과서나 전문가와 상의하여 대조적 특성이 의학적으로 의미 있는지 평가하라. 의사가 대화형으로 "왜 [특정 질병]이 아닌가?"라고 질문할 수 있는 사용자 인터페이스를 구현하라.

---

[이전: 개념 기반 설명](./07_Concept_Based_Explanations.md) | [개요](./00_Overview.md) | [다음: 해석 가능성을 위한 인과 추론](./09_Causal_Inference_for_Interpretability.md)
