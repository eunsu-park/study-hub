# 레슨 14: 프로덕션 해석 가능성(Production Interpretability)

[이전: AI 규제와 거버넌스](./13_AI_Regulation_and_Governance.md) | [다음: 도메인별 해석 가능성](./15_Domain_Specific_Interpretability.md)

---

## 학습 목표

- 프로덕션 SLA 요구사항을 충족하면서 의미 있는 해석 가능성을 제공하는 설명 서빙 아키텍처(동기식, 비동기식, 하이브리드)를 설계한다
- 입력 해싱(Input Hashing)과 모델 버전 인식 캐시 무효화(Model-Version-Aware Cache Invalidation)를 사용한 설명 캐싱 전략을 구현하여 계산 비용을 절감한다
- 시간 경과에 따른 특성 중요도(Feature Importance) 분포 변화를 감지하고 알림을 트리거하는 설명 드리프트 모니터링(Explanation Drift Monitoring) 시스템을 구축한다
- MLflow, CI/CD 검증 게이트, 자동화된 설명 품질 검사를 사용하여 해석 가능성 아티팩트를 MLOps 파이프라인에 통합한다
- Redis 캐싱, 백그라운드 사전 계산, 실시간 모니터링 대시보드를 포함한 완전한 FastAPI 설명 서비스를 구성한다

---

## 1. 설명 서빙을 위한 아키텍처 패턴

### 1.1 설명 서빙 문제(The Explanation Serving Problem)

프로덕션에서 설명을 생성하는 것은 종종 예측을 생성하는 것보다 더 비용이 크다. 단일 예측에 대한 SHAP 설명은 예측 자체보다 100배에서 1000배 더 오래 걸릴 수 있다. 이것은 엔지니어링 과제를 만든다: 지연 시간 SLA를 위반하지 않으면서 어떻게 설명을 제공할 것인가?
...
### 1.2 동기식 설명 서빙(Synchronous Explanation Serving)

가장 단순한 패턴으로, 모든 예측과 함께 설명을 계산한다. 빠른 설명 방법에만 적합하다.
...
### 1.3 비동기식 설명 서빙(Asynchronous Explanation Serving)

설명이 실시간 서빙에 비해 너무 비용이 클 경우, 메시지 큐(Message Queue)나 백그라운드 워커(Background Worker)를 사용하여 예측 경로에서 분리한다.
...
### 2.1 입력 해시 기반 설명 캐싱(Input-Hash-Based Explanation Caching)

설명 계산은 주어진 (입력, 모델 버전) 쌍에 대해 결정론적(Deterministic)이다. 캐싱은 이 특성을 활용하여 중복 계산을 방지한다.
...
### 2.2 빈번한 입력에 대한 사전 계산(Pre-computation for Common Inputs)

자주 조회되는 입력이나 대표적인 예시에 대해 오프라인으로 설명을 사전 계산하여 즉시 제공한다.
...
## 3. 설명 드리프트 모니터링(Explanation Drift Monitoring)

### 3.1 특성 중요도 변화 감지(Detecting Changes in Feature Importance)

시간 경과에 따라 특성 중요도(Feature Importance) 분포가 변할 때, 이는 개념 드리프트(Concept Drift), 데이터 드리프트(Data Drift), 또는 모델 성능 저하를 나타낼 수 있다. 설명 드리프트 모니터링은 조기 경보 시스템을 제공한다.
...
### 4.1 MLflow의 설명 아티팩트(Explanation Artifacts in MLflow)

설명을 MLOps의 일급 아티팩트(First-Class Artifact)로 취급하면 추적 가능성(Traceability), 버전 관리(Versioning), 재현 가능성(Reproducibility)이 보장된다.
...
### 4.2 CI/CD 검증 게이트(CI/CD Validation Gates)
...
### 5.1 설명 효과 측정(Measuring Explanation Effectiveness)
...
### 6.1 완전한 서비스 구현(Complete Service Implementation)

이 섹션에서는 지금까지 논의한 모든 패턴을 결합한 프로덕션 수준의 설명 서비스를 구축한다: 동기식 서빙, 캐싱, 사전 계산, 모니터링.
...
- **설명 서빙(Explanation Serving)**은 설명이 종종 예측보다 100~1000배 더 비용이 크기 때문에 신중한 아키텍처가 필요하다 -- 세 가지 패턴(동기식, 비동기식, 하이브리드)이 서로 다른 지연 시간 요구사항을 해결한다
- **캐싱(Caching)**은 프로덕션 효율성에 필수적이다: (모델 버전, 입력 특성) 쌍을 해싱하여 캐시 키로 사용하고, 모델이 업데이트될 때 전체 캐시를 무효화하여 오래된 설명이 제공되는 것을 방지한다
- **사전 계산(Pre-computation)**은 클러스터 중심점과 빈번한 입력에 대해 설명을 미리 계산하여, 비용이 큰 방법(SHAP, LIME, 반사실적 설명)을 실시간 지연 시간 없이 가능하게 한다
- **설명 드리프트 모니터링(Explanation Drift Monitoring)**은 통계적 검정(KS 검정)을 사용하여 시간 경과에 따른 특성 중요도 분포 변화를 감지하며, 데이터 드리프트, 개념 드리프트 또는 모델 성능 저하에 대한 조기 경보를 제공한다
- **MLOps 통합**은 설명을 일급 아티팩트로 취급한다: 버전 관리되고, 저장되며, CI/CD 파이프라인에서 검증되고, MLflow와 같은 레지스트리에서 모델 메트릭과 함께 추적된다
- **A/B 테스트 설명**은 UX 실험을 적용하여 어떤 설명 형식(막대 차트 vs. 서술형, 기술적 vs. 평이한 언어)이 사용자의 신뢰, 이해, 행동 요구를 가장 잘 충족하는지 결정한다
- **알림 시스템**은 예측 드리프트와 설명 드리프트 모두에서 트리거되어야 하며, 설명 변화가 정확도 저하에 선행할 수 있기 때문이다
- **완전한 설명 서비스**(FastAPI + Redis로 시연)는 이 모든 패턴을 헬스 체크, 메트릭, 관리 엔드포인트를 갖춘 프로덕션 수준 시스템으로 결합한다

---

## 연습문제

### 연습문제 1: 지연 시간 예산 분석(Latency Budget Analysis)

서로 다른 SLA를 가진 세 가지 ML 애플리케이션에 대해 최적의 설명 아키텍처를 설계하라:

1. **실시간 사기 탐지**(50ms SLA, 초당 10,000건 요청): 어떤 설명 방법과 패턴을 사용할 것인가? 어떤 캐싱 전략이 적합한가?
2. **대출 승인 시스템**(5초 SLA, 분당 100건 요청, 설명에 대한 규제 요구사항): 모든 결정에 규정을 준수하는 설명이 있도록 어떻게 보장할 것인가?
3. **의료 영상 분석**(30초 SLA, 일일 10건 요청, 고위험): 어떤 수준의 설명 깊이가 적절한가? 임상의의 상세한 설명 요구를 어떻게 처리할 것인가?

각각에 대해 아키텍처 패턴, 설명 방법, 캐싱 전략, 예상 적중률을 명시하라.

### 연습문제 2: 캐시 무효화 시스템 구축(Build a Cache Invalidation System)

다음을 처리하는 보다 정교한 캐시 무효화 시스템을 구현하라:

1. **모델 버전 변경**: 이전 버전의 모든 항목을 무효화
2. **특성 스키마 변경**: 특성 이름이나 순서가 변경되었을 때 감지
3. **부분 무효화**: 특성 값 범위에 맞는 항목을 무효화 (예: `income > 100000`인 모든 항목)
4. **지터가 있는 TTL**: TTL에 랜덤 지터를 추가하여 만료 시 썬더링 허드(Thundering Herd)를 방지
5. **캐시 워밍**: 무효화 후 가장 자주 액세스되는 입력에 대한 설명으로 캐시를 사전 채움

### 연습문제 3: 드리프트 감지 파이프라인(Drift Detection Pipeline)

다음을 수행하는 완전한 드리프트 감지 파이프라인을 구축하라:

1. 설명 서비스로부터 특성 중요도 벡터를 수집
2. 1시간 윈도우에 대한 롤링 통계(평균, 표준편차, 백분위수)를 계산
3. 여러 통계적 검정(KS, 카이제곱, PSI)을 적용하여 드리프트를 감지
4. 심각도 수준(정보, 경고, 위험)이 있는 알림을 생성
5. 시각화(matplotlib)가 포함된 주간 드리프트 보고서를 생성
6. 드리프트가 임계값을 초과할 때 자동 모델 재학습 트리거를 구현

### 연습문제 4: 설명 A/B 테스트 설계(Explanation A/B Test Design)

신용 점수 시스템에 대한 세 가지 설명 형식을 비교하는 완전한 A/B 테스트를 설계하고 구현하라:

1. **변형 A**: 상위 3개 특성 중요도 막대 차트
2. **변형 B**: 결정을 설명하는 자연어 단락
3. **변형 C**: 반사실적 설명("X가 Y였다면 결정이 바뀌었을 것이다")

메트릭을 정의하고, 통계적 검정력에 필요한 표본 크기를 계산하며, 무작위 할당을 구현하고, 분석 파이프라인을 구축하라. 시뮬레이션된 사용자 데이터에서 테스트를 실행하고 결과를 보고하라.

### 연습문제 5: 프로덕션 서비스 확장(Production Service Extension)

FastAPI 설명 서비스를 다음을 포함하도록 확장하라:

1. **속도 제한(Rate Limiting)**: 사용자당 분당 최대 100건의 설명 요청
2. **설명 버전 관리**: 타임스탬프별로 과거 설명을 저장하고 검색
3. **웹훅 콜백(Webhook Callbacks)**: 비동기 설명이 완료되면 클라이언트에 알림
4. **배치 엔드포인트**: 여러 예측 요청을 수락하고 설명을 배치로 반환
5. **Prometheus 메트릭**: 요청 수, 지연 시간 히스토그램, 캐시 적중률, 오류율을 Prometheus 호환 메트릭으로 내보내기

---

[이전: AI 규제와 거버넌스](./13_AI_Regulation_and_Governance.md) | [개요](./00_Overview.md) | [다음: 도메인별 해석 가능성](./15_Domain_Specific_Interpretability.md)
