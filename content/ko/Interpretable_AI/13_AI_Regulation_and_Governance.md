# 레슨 13: AI 규제와 거버넌스(AI Regulation and Governance)

[이전: 공정성 완화](./12_Fairness_Mitigation.md) | [다음: 프로덕션 해석 가능성](./14_Production_Interpretability.md)

---

## 학습 목표

- EU AI Act의 위험 기반 분류 체계를 이해하고, AI 애플리케이션을 적절한 위험 범주에 매핑하여 해당 의무사항을 파악한다
- 자동화된 의사결정(Automated Decision-Making)에 대한 GDPR 제22조 요구사항을 적용하여, 의미 있는 설명(Meaningful Explanations)과 이의제기 권리(Right to Contest)를 포함한다
- NIST AI 위험 관리 프레임워크(NIST AI Risk Management Framework)의 4가지 핵심 기능(Govern, Map, Measure, Manage)을 조직의 AI 거버넌스에 활용한다
- 모델 카드(Model Cards)와 데이터셋 데이터시트(Datasheets for Datasets)를 확립된 템플릿에 따라 작성하여 모델 행동과 데이터 출처를 문서화한다
- 구조화된 체크리스트와 영향 평가 템플릿을 사용하여 AI 시스템에 대한 실질적인 규제 준수 평가를 수행한다
...
고위험 분야에 AI 시스템이 배포되면서 규제 프레임워크에 대한 긴급한 필요성이 생겼다. 전통적인 소프트웨어와 달리 AI 시스템은 내재적 편향(Emergent Biases), 불투명한 의사결정(Opaque Decision-Making), 그리고 기존 규제가 대응하도록 설계되지 않은 예측 불가능한 실패 모드(Failure Modes)를 보일 수 있다.
...
각 관할권은 AI 규제에 근본적으로 다른 접근 방식을 취하고 있다. 이러한 차이를 이해하는 것은 글로벌로 운영되는 조직에 필수적이다.
...
EU AI Act는 4단계 위험 등급을 수립한다. 각 등급은 완전한 금지부터 최소한의 투명성 요구사항까지 서로 다른 의무를 수반한다.
...
### 2.2 고위험 시스템의 적합성 평가(Conformity Assessment)

고위험 AI 시스템은 EU 시장에 출시하기 전에 적합성 평가를 거쳐야 한다. 이것은 가장 까다로운 준수 요구사항이다.
...
### 2.3 기술 문서 요구사항(Annex IV)

Annex IV는 기술 문서에 포함되어야 할 내용을 명시한다. 이것은 고위험 시스템에 대한 가장 상세한 단일 준수 요구사항이다.
...
## 3. GDPR 제22조: 자동화된 의사결정

### 3.1 설명에 대한 권리(Right to Explanation)

GDPR 제22조는 유럽에서 AI 설명 가능성(Explainability)에 대한 가장 널리 인용되는 법적 근거이다. 이 조항은 자동화된 의사결정의 대상이 되는 개인에게 특정 권리를 부여한다.
...
NIST AI RMF는 AI 위험을 관리하기 위한 자발적 프레임워크를 제공한다. EU AI Act와 달리 법적 구속력은 없지만, 미국에서 AI 거버넌스의 사실상(de facto) 표준이 되었으며 국제적으로도 점점 더 많이 참조되고 있다.
...
## 5. NYC 지방법 144: 자동화된 고용 결정 도구

### 5.1 개요 및 요구사항

NYC 지방법 144(NYC Local Law 144, 2023년 7월 5일 시행)는 고용 분야의 자동화된 의사결정 도구를 구체적으로 대상으로 하는 최초의 미국 지방법이다.
...
## 6. 모델 카드(Model Cards)와 데이터시트(Datasheets)

### 6.1 모델 카드(Mitchell et al., 2019)

모델 카드(Model Cards)는 학습된 머신러닝 모델에 대한 표준화된 문서이다. Google의 Margaret Mitchell 등이 다양한 조건에서의 모델 성능에 대한 투명한 보고를 촉진하기 위해 제안했다.
...
### 6.2 데이터셋 데이터시트(Gebru et al., 2021)
...
AI 영향 평가(AIA)는 배포 전에 AI 시스템의 잠재적 영향을 평가하기 위한 구조화된 프로세스이다. 이것은 데이터 보호 영향 평가(DPIA, GDPR 제35조 요구), NIST AI RMF, 그리고 새롭게 부상하는 모범 사례의 요소를 결합한다.
...
## 9. 실습: 모델 카드 작성과 준수 점검

### 9.1 종단 간(End-to-End) 준수 워크플로우

이 실습 연습은 실제 AI 시스템에 대한 규제 문서를 작성하는 방법을 시연하며, 모델 카드(Model Cards), 편향 감사(Bias Audits), 영향 평가(Impact Assessments)를 결합한다.
...
- **EU AI Act**는 4가지 범주(허용 불가, 고위험, 제한적, 최소)의 위험 기반 프레임워크를 수립하며, 고위험 시스템은 적합성 평가(Conformity Assessment), 기술 문서(Technical Documentation), 인간 감독(Human Oversight)을 포함한 광범위한 의무를 지닌다
- **GDPR 제22조**는 개인에게 중대한 영향을 미치는 완전 자동화된 결정의 대상이 되지 않을 권리를 부여하며, "관련 로직에 대한 의미 있는 정보(Meaningful Information About the Logic Involved)"를 요구한다 — 이것이 EU에서 AI 설명의 법적 근거이다
- **NIST AI RMF**는 4가지 기능(Govern, Map, Measure, Manage)의 자발적 프레임워크를 제공하며, 관할권에 관계없이 AI 위험을 체계적으로 관리하기 위해 조직이 채택할 수 있다
- **NYC 지방법 144(NYC Local Law 144)**는 자동화된 고용 결정 도구에 대한 구체적인 편향 감사(Bias Audit) 요구사항을 선구적으로 도입했으며, 인구통계학적 그룹 간 선발률(Selection Rates)과 영향 비율(Impact Ratios)의 계산을 요구한다
- **모델 카드(Model Cards)**(Mitchell et al., 2019)와 **데이터셋 데이터시트(Datasheets for Datasets)**(Gebru et al., 2021)는 모델 행동과 데이터 특성을 문서화하기 위한 표준화된 템플릿을 제공한다
- **AI 영향 평가(AI Impact Assessments)**는 위험 식별, 이해관계자 분석, 완화 계획을 배포 전 구조화된 검토 프로세스로 결합한다
- **산업 표준**인 ISO/IEC 42001은 규제 요구사항을 보완하는 인증 가능한 AI 관리 시스템 프레임워크를 제공한다
- 준수(Compliance)는 일회성 이벤트가 아니다 — 지속적인 모니터링, 문서 업데이트, 그리고 조직 거버넌스 구조를 필요로 한다
...
다음 AI 시스템에 대해 EU AI Act에 따라 각각을 분류하라:

1. 키 입력과 화면 시간을 추적하여 직원 생산성을 모니터링하는 AI 시스템
2. 음악 스트리밍 서비스의 추천 시스템
3. 피고의 위험 프로필을 기반으로 판사가 보석금을 설정하는 데 도움을 주는 AI 도구
4. 고객이 올바른 신발 사이즈를 선택하도록 도와주는 챗봇
5. 자율 주행 차량 비상 제동에 사용되는 AI 시스템

각각에 대해 다음을 결정하라: (a) 위험 수준, (b) 해당되는 Annex III 범주(고위험인 경우), (c) 핵심 의무사항.

### 연습 2: GDPR 제22조 분석

건강보험 회사가 고객 제공 건강 설문지 데이터, 웨어러블 디바이스 데이터, 공개적으로 이용 가능한 라이프스타일 정보를 기반으로 보험료를 자동으로 설정하는 AI 시스템을 사용한다. 개별 가격 결정에 대한 인간 검토는 없다.

1. GDPR 제22조가 적용되는가? 각 조건을 검토하라
2. 보험사가 의존할 수 있는 예외 사항은 무엇인가?
3. 보험료가 인상된 고객을 위한 레벨 2 설명을 작성하라
4. 제22조(3)항 요구사항을 충족하는 이의제기 메커니즘을 설계하라

### 연습 3: 완전한 모델 카드 작성

공개 데이터셋(예: UCI Adult, COMPAS, German Credit)에서 선택한 모델을 학습시키고, Mitchell 등의 템플릿에 따라 완전한 모델 카드를 작성하라. 다음을 포함한다:

1. 최소 3개 인구통계학적 그룹에 대한 세분화된 성능 지표
2. 근거가 있는 최소 3개의 범위 외 사용 사례
3. 구체적인 "결과를 바꿀 수 있는 것" 반사실적(Counterfactual) 설명
4. 모델의 한계와 윤리적 우려에 대한 정직한 평가

### 연습 4: 편향 감사 구현

채용 모델에 대한 완전한 NYC LL144 준수 편향 감사를 구현하라:

1. 성별 및 인종/민족 속성이 있는 합성 채용 데이터를 생성하라
2. "채용" 결정을 예측하는 분류기를 학습시키라
3. 모든 인구통계학적 그룹에 대한 선발률과 영향 비율을 계산하라
4. 4/5 규칙에 실패하는 그룹을 식별하라
5. 실패하는 각 그룹에 대한 구체적인 완화 전략을 제안하라
6. 하나의 완화 전략을 적용한 후 감사를 다시 실행하라

### 연습 5: 규제 매핑

독일 은행이 EU 고객을 위해 사용할 신용 평가 AI 시스템을 배포한다. 다음에 걸쳐 요구사항을 매핑하는 준수 매트릭스를 작성하라:

1. EU AI Act (고위험 의무)
2. GDPR (제13, 14, 22, 35조)
3. 독일 연방 데이터 보호법(BDSG 제37조)
4. NIST AI RMF (4가지 기능 전체)
5. ISO/IEC 42001 (관련 조항)

각 요구사항에 대해 다음을 식별하라: (a) 필요한 것, (b) 책임자, (c) 준수를 입증하는 증거, (d) 모니터링 빈도.

---

[이전: 공정성 완화](./12_Fairness_Mitigation.md) | [개요](./00_Overview.md) | [다음: 프로덕션 해석 가능성](./14_Production_Interpretability.md)
