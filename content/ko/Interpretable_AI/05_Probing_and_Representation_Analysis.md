# 레슨 5: 프로빙과 표현 분석(Probing and Representation Analysis)

[이전: 어텐션 해석](./04_Attention_Interpretation.md) | [다음: 고급 SHAP](./06_Advanced_SHAP.md)

---

## 학습 목표

- 학습된 표현(Learned Representations)을 분석하기 위한 프로빙 분류기(Probing Classifier) 방법론과 이론적 기초를 이해한다
- 비전 모델(Vision Model)의 개별 뉴런에 인코딩된 의미 개념을 식별하기 위해 네트워크 해부(Network Dissection)를 적용한다
- 표현이 레이어와 모델에 걸쳐 어떻게 진화하는지 이해하기 위해 표현 유사도 방법(CKA, SVCCA, Procrustes)을 비교한다
- 트랜스포머(Transformer) 언어 모델의 중간 계산을 검사하기 위해 로짓 렌즈(Logit Lens)와 튜닝된 렌즈(Tuned Lens) 기법을 구현한다
- 어떤 표현이 특정 모델 행동을 주도하는지 식별하기 위한 인과적 개입(Causal Intervention) 도구로서 활성화 패칭(Activation Patching)을 사용한다

---

프로빙과 표현 분석은 다음과 같은 질문을 던진다: **신경망은 실제로 무엇을 학습했는가?** SHAP이나 LIME처럼 개별 예측을 설명하는 것이 아니라, 이러한 방법들은 모델의 내부 표현 속으로 들어가 어떤 정보가 인코딩되어 있는지, 어디에 인코딩되어 있는지, 그리고 그 정보가 레이어를 거치며 어떻게 변환되는지를 이해한다. 이것은 사후 설명(Post-hoc Explanation, 레슨 1-4)과 새롭게 부상하는 기계적 해석 가능성(Mechanistic Interpretability, 레슨 16) 분야 사이의 다리 역할을 한다.

핵심 통찰은 기만적으로 단순하다: BERT의 은닉 상태(Hidden States)에서 구문 분석 트리(Syntactic Parse Trees)를 추출하는 간단한 분류기를 훈련할 수 있다면, BERT는 사전 훈련(Pre-training) 동안 구문에 대해 무언가를 학습했을 것이다 — 비록 아무도 명시적으로 구문을 가르치지 않았더라도. 그러나 앞으로 보겠지만, 이 추론에는 미묘한 함정이 있으며 이 분야는 이를 이해하는 데 수년을 보냈다.

---

## 1. 프로빙 분류기: 기초(Probing Classifiers: Foundations)

### 1.1 핵심 아이디어

**프로빙 분류기**(진단 분류기(Diagnostic Classifier)라고도 함)는 신경망의 고정된 표현으로부터 언어적 또는 의미적 속성을 예측하도록 훈련된 단순한 모델이다. 핵심 제약 조건은 프로브가 단순해야 한다는 것이다 — 복잡한 프로브를 사용하면 표현에서 속성을 추출하는 것이 아니라 속성 자체를 학습할 수 있다.
...
## 2. 프로빙의 함정과 통제 실험(Probing Pitfalls and Controls)

### 2.1 기억화 문제(The Memorization Problem)

핵심적인 우려 사항: 프로빙 분류기가 성공하는 이유가 정보가 표현에 있어서가 아니라, 프로브 자체가 입력에서 레이블로의 매핑을 기억할 만큼 강력하기 때문일 수 있다. 이는 MLP 프로브에서 특히 위험하다.
...
### 2.2 선택성: Hewitt & Liang (2019) 통제 과제(Control Task)
...
## 3. 네트워크 해부(Network Dissection)

### 3.1 개요

네트워크 해부(Bau et al., 2017)는 프로빙과는 근본적으로 다른 접근 방식을 취한다: 정보를 추출하기 위해 분류기를 훈련하는 대신, 개별 뉴런이 사람이 해석 가능한 의미 개념에 반응하는지를 직접 측정한다.
...
## 4. 표현 유사도 분석(Representation Similarity Analysis)

### 4.1 표현을 비교하는 이유

표현 유사도 방법은 다음과 같은 질문에 답한다: 서로 다른 모델이 같은 것을 학습하는가? 표현은 레이어에 걸쳐 어떻게 변하는가? 파인튜닝된 표현은 사전 훈련된 표현과 유사한가?
...
## 5. 로짓 렌즈와 튜닝된 렌즈(Logit Lens and Tuned Lens)
...
- **프로빙 분류기(Probing Classifiers)**는 고정된 표현에 대해 단순한 모델을 훈련하여 신경망이 어떤 정보를 학습했는지 테스트한다. 선형 프로브(Linear Probes)는 보수적이며, MLP 프로브는 상한을 제공한다.
- **프로빙의 함정**에는 기억화(Memorization, 프로브가 과제 자체를 학습하는 것)와 낮은 선택성(Low Selectivity)이 포함된다. Hewitt & Liang (2019) 통제 과제와 MDL 프로빙(Voita & Titov 2020)은 필수적인 통제 실험이다.
- **네트워크 해부(Network Dissection)**(Bau et al.)는 활성화 맵과 분할 레이블 간의 IoU를 사용하여 개별 뉴런을 의미 개념에 매핑하며, CNN 내부의 해석 가능한 단위를 드러낸다.
- **표현 유사도(Representation Similarity)** 방법(CKA, SVCCA, Procrustes)은 레이어와 모델에 걸쳐 표현을 비교한다. CKA는 불변성 속성과 사용 편의성 덕분에 현재의 표준이다.
- **로짓 렌즈(Logit Lens)**는 최종 언임베딩 행렬(Unembedding Matrix)을 통해 중간 트랜스포머 표현을 디코딩하여 예측이 레이어에 걸쳐 어떻게 진화하는지 보여준다. **튜닝된 렌즈(Tuned Lens)**는 레이어별 아핀 프로브(Affine Probes)로 이를 개선한다.
- **활성화 패칭(Activation Patching)**은 중간 표현에 대해 인과적 개입을 수행하여 특정 활성화가 특정 행동에 필요한지를 테스트한다 — 이는 기계적 해석 가능성(Mechanistic Interpretability)의 기초이다.

---

## 연습문제

### 연습문제 1: 레이어별 POS 프로빙 (초급)

사전 훈련된 BERT 모델과 Universal Dependencies 영어 트리뱅크(Treebank)를 다운로드하라. 각 레이어(0-12)에서 POS 태그를 예측하는 선형 프로브를 훈련하라. 정확도 곡선을 그리고 구문에 가장 적합한 레이어를 식별하라. 결과를 Belinkov & Glass (2019)와 비교하라.

### 연습문제 2: 선택성 분석 (중급)

연습문제 1을 Hewitt & Liang 선택성 통제로 확장하라. 각 레이어에 대해 선택성 점수(실제 정확도에서 통제 정확도를 뺀 값)를 계산하라. 선택성을 고려할 때 "어떤 레이어가 POS를 인코딩하는가"에 대한 결론이 바뀌는가? MLP 프로브와 선형 프로브에서는 어떤 일이 발생하는가?

### 연습문제 3: CKA 비교 (중급)

BERT-base와 BERT-large 사이의 CKA 유사도를 비교하라. 대응하는 레이어(예: base의 레이어 6과 large의 레이어 12)가 높은 CKA를 보이는가? 전체 CKA 맵을 구축하고 블록 구조를 식별하라. 더 넓은 네트워크에 대한 Kornblith et al.의 발견을 재현하라.

### 연습문제 4: 로짓 렌즈 조사 (고급)

다음 프롬프트에 대해 GPT-2에 로짓 렌즈를 적용하고 정답이 처음 나타나는 시점을 분석하라:
- "The capital of Japan is"
- "2 + 2 ="
- "She picked up the phone and called her"
- "The quick brown fox jumps over the lazy"

각 프롬프트에 대해 올바른 예측이 처음으로 50% 확률을 달성하는 레이어를 식별하라. 프롬프트 간의 차이가 사실적 회상(Factual Recall)과 패턴 완성(Pattern Completion)에 대해 무엇을 말해주는가?

### 연습문제 5: 활성화 패칭 회로 발견 (고급)

ActivationPatcher 클래스를 사용하여 "간접 목적어 식별(Indirect Object Identification)" 과제를 조사하라:
- 깨끗한 입력: "John gave the book to Mary because she" ("she" → "liked" 기대)
- 손상된 입력: "John gave the book to Mary because he" ("he" → "wanted" 기대)

모든 (레이어, 위치) 조합에서 패칭하고 복구 히트맵을 구축하라. 대명사 해소에 중요한 위치와 레이어를 식별하라. 발견한 결과를 Wang et al. (2022)에서 설명된 IOI 회로와 비교하라.

---

[이전: 어텐션 해석](./04_Attention_Interpretation.md) | [개요](./00_Overview.md) | [다음: 고급 SHAP](./06_Advanced_SHAP.md)
