# Lesson 3: 클래스 활성화 매핑(Class Activation Mapping)

[이전: 그래디언트 어트리뷰션](./02_Gradient_Attribution.md) | [다음: 어텐션 해석](./04_Attention_Interpretation.md)

---

## 학습 목표

- 원본 CAM 방법이 전역 평균 풀링(Global Average Pooling) 레이어를 활용하여 클래스 판별적 위치 맵을 생성하는 방법 설명
- PyTorch 훅(Hook)을 사용하여 처음부터 GradCAM을 구현하고, 이것이 모든 CNN 아키텍처로 CAM을 일반화하는 이유 이해
- GradCAM++, Score-CAM, Eigen-CAM의 이론적 동기와 실용적 트레이드오프 비교
- 의료 영상 사용 사례에 클래스 활성화 매핑을 적용하고 결과를 비판적으로 해석
- 해상도, 클래스 판별성, 아키텍처 가정을 포함한 CAM 계열 방법의 근본적 한계 식별

---

클래스 활성화 매핑(Class Activation Mapping, CAM) 방법은 그래디언트 어트리뷰션만으로는 답할 수 없는 공간적 질문에 답합니다: *"모델이 결정을 내리기 위해 이미지의 어디를 봤는가?"* 그래디언트 현저성 맵(Lesson 02)이 개별 픽셀을 강조하는 반면, CAM 방법은 이미지 *영역*을 강조하는 대략적인 위치 맵을 생성하여 인간의 해석에 훨씬 더 직관적입니다.

이 레슨은 원본 CAM(특정 아키텍처 필요)에서 GradCAM(모든 CNN에서 작동)을 거쳐 Score-CAM과 Eigen-CAM 같은 현대 변형까지의 발전을 추적합니다. 각 방법을 구현하고 실제 사용 사례에 적용하며 한계를 비판적으로 검토합니다.

---

## 1. 원본 CAM (Zhou et al. 2016)
...
- **원본 CAM** (Zhou et al. 2016)은 분류 레이어의 가중치를 사용하여 마지막 합성곱 레이어의 특성 맵을 선형 결합함으로써 클래스 판별적 위치 맵을 생성합니다. GAP + 단일 FC 아키텍처가 필요합니다.

- **GradCAM** (Selvaraju et al. 2017)은 FC 가중치를 그래디언트 기반 중요도 가중치(클래스 점수의 그래디언트에 대한 전역 평균 풀링)로 대체하여 CAM을 모든 CNN으로 일반화합니다. 가장 널리 사용되는 CAM 변형입니다.

- **GradCAM++** (Chattopadhay et al. 2018)은 2차 그래디언트 정보를 사용하여 공간 위치에 대한 가중된(비균일) 그래디언트 풀링으로 다중 객체 위치 추정을 개선합니다.

- **Score-CAM** (Wang et al. 2020)은 그래디언트를 완전히 제거하고, 각 특성 맵을 마스크로 사용하여 결과적인 클래스 점수 변화를 측정합니다. 더 원칙적이지만 상당히 느립니다(특성 맵당 하나의 순전파).

- **Eigen-CAM** (Muhammad & Yeasin 2020)은 SVD를 사용하여 활성화 맵의 주성분을 찾습니다. 빠르고 그래디언트가 필요 없지만 클래스 판별적이지 **않습니다**.

- CAM 방법은 특히 **의료 영상**에서 모델이 임상적으로 관련 있는 영역에 집중하는지 확인하는 데 유용하지만, 잠재적 아티팩트 기반 예측을 감지하기 위해 테두리 활성화 검사를 사용해야 합니다.

- **근본적 한계**로는 거친 해상도(특성 맵 격자 크기에 의해 제한), 아키텍처 가정(합성곱 레이어 필요), 전역 설명 또는 세밀한 픽셀 어트리뷰션을 제공할 수 없다는 점이 포함됩니다.

---

## 연습문제

### 연습문제 1: GradCAM 처음부터 구현 (코딩)

1. 사전 학습된 VGG-16 모델(GAP 레이어가 없는)을 로드하세요.
2. 마지막 합성곱 레이어(`features[-1]`)를 대상으로 GradCAM을 구현하세요.
3. ImageNet 이미지에서 합리적인 히트맵을 생성하는지 확인하세요.
4. `features[10]`(더 초기 레이어)을 대상으로 시도하세요. CAM이 어떻게 변하는가?

### 연습문제 2: 방법 비교 (코딩 + 분석)

1. 사전 학습된 ResNet-50 사용:
   a. 동일한 이미지에 대해 GradCAM, GradCAM++, Score-CAM, Eigen-CAM을 생성하세요.
   b. 모든 방법 간의 쌍별 피어슨 상관관계를 계산하세요.
   c. GradCAM과 Eigen-CAM이 크게 불일치하는 이미지를 찾으세요.
   d. 어떤 방법을 더 신뢰하는가, 그리고 왜?

### 연습문제 3: 다중 클래스 판별 (코딩)

1. 두 가지 다른 클래스를 포함하는 ImageNet 이미지를 찾으세요 (예: 개와 고양이).
2. 각 클래스에 대해 별도로 GradCAM을 생성하세요.
3. 히트맵이 서로 다른 영역에 집중하는지 확인하세요.
4. 두 클래스 모두에 대해 Eigen-CAM을 생성하면 어떻게 되는가?

### 연습문제 4: 의료 영상 파이프라인 (통합)

1. 흉부 X선 데이터셋(예: CheXpert 또는 NIH Chest X-ray)을 다운로드하세요.
2. 사전 학습된 DenseNet-121(의료 영상에서 흔히 사용)을 사용하세요.
3. Section 7의 `medical_imaging_gradcam` 파이프라인을 구현하세요.
4. 20개 이미지에 대해 테두리 활성화 검사를 실행하세요. 몇 개가 경고를 발생시키는가?
5. 경고된 이미지를 수동으로 검사하고 경고가 정당한지 판단하세요.

### 연습문제 5: CAM 방법의 정상성 검사 (연구)

1. Adebayo et al.의 모델 무작위화 테스트(Lesson 02, Section 5)를 GradCAM에 적용하세요.
2. ResNet-50의 레이어를 위에서 아래로 점진적으로 무작위화하세요.
3. 학습된 CAM과 각 무작위화된 CAM 간의 피어슨 상관관계를 플롯하세요.
4. GradCAM이 정상성 검사를 통과하는가? 어떤 레이어에서 CAM이 저하되기 시작하는가?
5. Score-CAM과 비교하세요. 그래디언트 프리 방법이 다른 정상성 검사 동작을 보이는가?

---

[이전: 그래디언트 어트리뷰션](./02_Gradient_Attribution.md) | [개요](./00_Overview.md) | [다음: 어텐션 해석](./04_Attention_Interpretation.md)
