...
현대 GPU는 단순한 렌더링 기계가 아니다 -- 초당 수조 회 연산이 가능한 대규모 병렬 프로세서이다. 고성능 GPU는 수천 개의 코어가 일제히 동작하며, 적합한 워크로드에서 CPU 대비 10~100배의 처리량을 제공한다. 이 성능은 실시간 그래픽스뿐 아니라 딥러닝(Deep Learning), 과학 시뮬레이션, 암호화폐 채굴, 데이터 분석을 구동한다.

GPU가 어떻게 연산하는지 이해하는 것은 그래픽스나 성능이 중요한 컴퓨팅 분야에서 일하는 모든 사람에게 필수이다. CUDA나 Vulkan 컴퓨트 셰이더를 직접 작성하지 않더라도, GPU가 잘하는 것(그리고 못하는 것)을 아는 것은 알고리즘과 데이터 구조를 설계하는 방식에 근본적인 영향을 미친다.
...
CPU는 **지연 시간(Latency)** 최적화를 위해 설계되었다 -- 큰 캐시, 분기 예측(Branch Prediction), 비순서 실행(Out-of-order Execution)으로 단일 스레드를 최대한 빠르게 실행한다. GPU는 **처리량(Throughput)** 최적화를 위해 설계되었다 -- 수천 개의 스레드를 동시에 실행하며 대규모 병렬성으로 메모리 지연을 숨긴다.
...
GPU는 **SIMT**(Single Instruction, Multiple Threads, 단일 명령 다중 스레드) 모델로 명령을 실행한다. 스레드 그룹(NVIDIA에서는 **워프(Warp)**, AMD에서는 **웨이브프런트(Wavefront)**라고 함)이 동일한 명령을 동시에 실행한다:

- **NVIDIA**: 워프(Warp) = 32개 스레드
- **AMD**: 웨이브프런트(Wavefront) = 32 또는 64개 스레드

워프 내 모든 스레드는 동기적으로(Lockstep) 실행된다. 스레드가 다른 분기를 취하면(분기 발산, Divergence), 두 경로 모두 순차적으로 실행되며 활성 경로에 없는 스레드는 마스킹된다. 이로 인해 분기(Branching)가 비용이 크다.
...
**워크 그룹(Work Group)** 스레드는 다음을 할 수 있다:
- **배리어(Barrier)**를 통한 동기화 (모든 스레드가 배리어에 도달할 때까지 대기)
- **공유 메모리(Shared Memory)**(빠른 온칩 SRAM)를 통한 데이터 공유
- 그룹 내 통신

서로 다른 워크 그룹의 스레드는 단일 디스패치 중에 **직접 통신하거나 동기화할 수 없다**.

### 1.4 점유율 (Occupancy)

**점유율(Occupancy)**은 활성 워프 수 대 SM이 지원할 수 있는 최대 워프 수의 비율이다. 점유율이 높을수록 지연 숨기기(Latency Hiding)가 더 효과적이다:
...
**경험 법칙**: 50% 이상의 점유율 목표. 때로는 더 나은 레지스터 활용으로 낮은 점유율이 더 빠를 수 있다(명령 수준 병렬성이 보완).
...
**컴퓨트 셰이더(Compute Shader)**는 그래픽스 파이프라인 외부에서 실행되는 GPU 프로그램이다. 임의의 버퍼와 텍스처를 읽고 쓰며 범용 연산을 수행한다.
...
컴퓨트 셰이더는 3D 워크 그룹 크기(로컬 크기)를 정의하며 워크 그룹의 3D 그리드로 디스패치된다:
...
**공유 메모리(Shared Memory)**(HLSL에서 "groupshared", GLSL에서 "shared")는 워크 그룹의 모든 스레드가 접근할 수 있는 빠른 온칩 메모리이다:
...
공유 메모리는 SM당 일반적으로 16~96 KB이며, 전역 메모리보다 약 100배 빠른 접근 지연 시간을 가진다.
...
**문제**: $N$개 요소의 합(또는 최대, 최소 등)을 계산한다.
...
각 단계에서 활성 스레드가 절반으로 줄어든다. $\log_2 N$ 단계 후 결과는 요소 0에 있다.
...
**문제**: 배열 $[a_0, a_1, ..., a_{n-1}]$이 주어졌을 때, 누적 합 $[a_0, a_0+a_1, a_0+a_1+a_2, ...]$ (포함 스캔, Inclusive Scan) 또는 $[0, a_0, a_0+a_1, ...]$ (배타 스캔, Exclusive Scan)을 계산한다.

프리픽스 합은 다음에서 사용되는 핵심 빌딩 블록이다:
...
**바이토닉 정렬(Bitonic Sort)**은 비교 패턴이 고정되어 있어(데이터 독립적) 효율적인 병렬 구현이 가능하여 GPU 실행에 적합한 비교 기반 정렬 알고리즘이다.

**바이토닉 수열(Bitonic Sequence)**은 단조 증가 후 단조 감소하는(또는 반대) 수열이다. 바이토닉 병합(Bitonic Merge)은 $O(\log n)$ 병렬 단계로 바이토닉 수열을 정렬한다.

**전체 바이토닉 정렬**: $O(\log^2 n)$ 병렬 단계, 각 단계에서 요소 쌍을 비교하고 교환한다.
...
**문제**: 배열과 조건자(Predicate)가 주어졌을 때, 조건을 만족하는 요소를 추출한다(예: "죽은" 파티클 제거).
...
이미지 컨볼루션(Image Convolution)은 각 픽셀에 커널(필터)을 적용한다:

$$(I * K)(x, y) = \sum_{i=-r}^{r}\sum_{j=-r}^{r} K(i, j) \cdot I(x+i, y+j)$$

이는 자연스럽게 병렬화된다: 각 픽셀의 출력은 자신의 이웃에만 의존한다.

**GPU 구현**: 각 스레드가 하나의 출력 픽셀을 처리한다. 공유 메모리는 이미지의 로컬 타일을 저장해 중복된 전역 메모리 읽기를 방지한다.

### 4.2 분리 가능한 필터 (Separable Filters)

많은 유용한 커널(가우시안(Gaussian), 소벨(Sobel), 박스 필터(Box Filter))은 **분리 가능(Separable)**한다: 2D 커널을 두 번의 1D 패스로 분해할 수 있다:
...
**분리 컨볼루션(Separable Convolution)**은 작업량을 $O(n^2 k^2)$에서 $O(n^2 \cdot 2k)$로 줄인다, 여기서 $k$는 커널 반경이다. 5x5 커널의 경우 2.5배, 11x11의 경우 5.5배 빠르다.
...
**CUDA**(Compute Unified Device Architecture)는 NVIDIA의 독점 GPU 컴퓨팅 플랫폼(2007년)이다. GPU 커널 작성을 위한 C/C++ 확장을 제공한다.
...
**OpenCL**(Open Computing Language)은 CPU, GPU, FPGA, DSP를 위한 크로스 플랫폼 병렬 컴퓨팅 표준이다.
...
**OpenGL 컴퓨트 셰이더(Compute Shaders)**(4.3+), **Vulkan Compute**, **Metal Compute**, **DirectX 컴퓨트 셰이더**는 그래픽스 API 내에서 GPU 컴퓨팅을 제공한다:
...
**WebGPU**는 WebGL을 대체하는 차세대 웹 GPU API이다:
...
**병합 접근(Coalesced Access)**: 연속된 스레드가 연속된 메모리 주소에 접근하면 GPU는 읽기를 단일 와이드 트랜잭션(128 바이트)으로 결합한다. 이는 성능에 매우 중요하다:
...
공유 메모리는 **뱅크(Bank)**(일반적으로 32개 뱅크, 각 4바이트)로 나뉜다. 두 스레드가 동시에 같은 뱅크에 접근하면 접근이 직렬화된다(뱅크 충돌).
...
**완화 방법**: 같은 뱅크를 반복 접근하는 스트라이드(Stride) 패턴을 피하기 위해 공유 메모리 배열을 패딩(Padding)한다.
...
PCIe를 통한 CPU-GPU 데이터 전송이 병목이 되는 경우가 많다:
...
**원칙**: CPU-GPU 전송을 최소화한다. 데이터를 최대한 오래 GPU에 유지한다. GPU에서 연산하고 최종 결과만 전송한다.

### 7.2 동기화 (Synchronization)

**워크 그룹 내**: 배리어(GLSL의 `barrier()`, CUDA의 `__syncthreads()`) 사용.

**워크 그룹 간**: 메모리 배리어를 사용하고 다른 디스패치를 재실행한다. 컴퓨트 셰이더는 단일 디스패치 내에서 워크 그룹 간 동기화를 할 수 없다.

**CPU-GPU**: 펜스(Fence)를 사용해 GPU 완료를 대기한다. 타이트한 루프에서 왕복(Round-trip) 동기화를 피한다(파이프라인 지연 발생).
...
그래픽스 엔진의 GPU는 전통적인 렌더링을 넘어 다양한 컴퓨트 역할을 수행한다:
...
**비동기 컴퓨트(Async Compute)**: 현대 API(Vulkan, DX12, Metal)는 서로 다른 GPU 큐에서 컴퓨트와 그래픽스 작업을 동시에 실행할 수 있도록 지원하여 활용률을 높이다.
