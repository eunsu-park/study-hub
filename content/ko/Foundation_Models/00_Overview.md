...
- **NLP_and_LLM 폴더**: BERT, GPT, HuggingFace, Fine-tuning, RAG
...
### Section 6: 적응 & 평가 (19-22)
| 파일 | 주제 | 핵심 내용 | 난이도 |
|------|------|----------|--------|
| [19_Instruction_Tuning.md](19_Instruction_Tuning.md) | Instruction Tuning | FLAN, Self-Instruct, Evol-Instruct | ⭐⭐⭐ |
| [20_Continued_Pretraining.md](20_Continued_Pretraining.md) | Continued Pre-training | 도메인 적응, Catastrophic Forgetting 방지 | ⭐⭐⭐⭐ |
| [21_API_and_Evaluation.md](21_API_and_Evaluation.md) | API & 평가 | OpenAI/Anthropic/Google API, 벤치마크 | ⭐⭐⭐ |
| [22_Research_Frontiers.md](22_Research_Frontiers.md) | 연구 최전선 | World Models, o1 Reasoning, Synthetic Data | ⭐⭐⭐⭐ |

> **참고**: PEFT (LoRA/QLoRA), 추론 최적화 (vLLM), Advanced RAG는 더 실용적인 학습을 위해 [NLP_and_LLM](../NLP_and_LLM/00_Overview.md)으로 이동되었습니다.
...
│                              ▼                                          │
│  ┌──────────────────────────────────────────────┐                       │
│  │      Phase 4: 적응 & 프론티어 (Week 5)         │                       │
│  │      19 → 20 → 21 → 22                        │                       │
│  │      (Instruction → Continued → API → 프론티어)│                       │
│  └──────────────────────────────────────────────┘                       │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
### NLP_and_LLM 폴더와의 연계
| NLP_and_LLM 레슨 | Foundation_Models 확장 |
|-----------------|----------------------|
| 04-05_BERT_GPT | 08-09_LLaMA_Mistral (최신 오픈소스) |
| 08_PEFT_and_QLoRA | 실용 PEFT (FM에서 이동) |
| 12_Advanced_RAG | 고급 RAG 기법 (FM에서 이동) |
| 18_Inference_Optimization | 추론 최적화 (FM에서 이동) |
...
```
01 → 02 → 08 → 09 → 10 → 11
(패러다임 → Scaling → LLaMA → Mistral → Long-Context → Small LMs)
```
...
(기초 → Vision → VLM → GPT-4V → Audio/Video)
```
