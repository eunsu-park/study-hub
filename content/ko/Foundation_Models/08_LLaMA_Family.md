...
- [../NLP_and_LLM/08_PEFT_and_QLoRA.md](../NLP_and_LLM/08_PEFT_and_QLoRA.md): LLaMA Fine-tuning (LoRA)
...
### 연습 문제 1: RoPE vs 절대 위치 인코딩

회전 위치 인코딩(RoPE, Rotary Position Encoding)과 학습된 절대 위치 인코딩을 다음 차원에서 비교하세요:

| 차원 | 절대 PE | RoPE |
|------|--------|------|
| 학습 길이 이상 외삽(Extrapolation) | ? | ? |
| 위치 인코딩 방식 | ? | ? |
| 상대 거리 표현 | ? | ? |
| 메모리 비용 | ? | ? |
...
| 차원 | 절대 PE | RoPE |
|------|--------|------|
| **학습 길이 이상 외삽** | 나쁨 — 학습된 최대값 이상의 위치는 학습된 임베딩이 없어 성능이 저하됨 | 더 나음 — RoPE의 수학적 구조(복소 평면에서의 회전)는 더 긴 시퀀스에 더 자연스럽게 일반화됨; YaRN 또는 LongRoPE 스케일링으로 추가 확장 가능 |
| **위치 인코딩 방식** | 각 토큰 임베딩에 학습된 또는 고정된 벡터를 더함 (위치 정보가 임베딩 공간에 있음) | 위치를 기반으로 한 회전 행렬로 쿼리와 키 벡터를 곱함 (위치 정보가 어텐션 내적 관계에 인코딩됨) |
| **상대 거리 표현** | 간접적 — 상대 거리가 절대 위치에서 계산되어 모델이 이 관계를 학습해야 함 | 직접적 — RoPE는 상대 거리를 명시적으로 인코딩함: RoPE 회전된 Q와 K 사이의 내적은 절대 위치가 아닌 상대 위치에만 의존함 |
| **메모리 비용** | (max_len × dim) 크기의 학습된 임베딩 행렬 저장 필요 | 추가 파라미터 없음 — 회전 행렬을 통해 즉시 계산 |

RoPE의 핵심 실용적 장점: `Q_m · K_n ∝ f(m-n)` — 상대 거리가 어텐션 계산에 "내장"되어 있어 모델이 위치 독립적인 패턴을 더 쉽게 학습합니다.

</details>

---

### 연습 문제 2: GQA 메모리 절감 계산

LLaMA 2 70B는 64개의 쿼리 헤드와 8개의 KV 헤드로 그룹화 쿼리 어텐션(GQA, Grouped Query Attention)을 사용합니다.

1. 이 모델에서 레이어당 토큰당 KV 캐시에 얼마나 많은 메모리가 필요한가요 (fp16, head_dim=128 가정)?
2. 대신 다중 헤드 어텐션(MHA, Multi-Head Attention)을 사용했다면 (64 KV 헤드), KV 캐시에 얼마나 더 많은 메모리가 필요한가요?
3. 배치 크기 32, 시퀀스 길이 4096, 80개의 트랜스포머 레이어에서 GQA vs MHA의 총 KV 캐시 메모리 절감은 얼마인가요?

<details>
<summary>정답 보기</summary>

**1. GQA를 사용한 KV 캐시 (8개 KV 헤드):**
```
레이어당 토큰당 = 2 (K+V) × 8 KV헤드 × 128 헤드차원 × 2 바이트 (fp16)
= 2 × 8 × 128 × 2 = 4,096 바이트 = 4 KB
```

**2. MHA를 사용한 KV 캐시 (64 KV 헤드):**
```
레이어당 토큰당 = 2 × 64 × 128 × 2 = 32,768 바이트 = 32 KB
```
MHA는 GQA보다 8배 더 많은 KV 캐시를 필요로 합니다.

**3. 배치=32, 시퀀스 길이=4096, 80 레이어에서의 총 절감:**

GQA 합계: 4 KB × 32 × 4096 × 80 ≈ **40.96 GB**
MHA 합계: 32 KB × 32 × 4096 × 80 ≈ **327.68 GB**

**절감: ~286.72 GB** — GQA를 통해 이 배치/시퀀스 크기에서 MHA보다 훨씬 적은 GPU로 70B 모델을 서빙할 수 있습니다.

</details>

---

### 연습 문제 3: SwiGLU vs GELU 분석

LLaMA는 활성화 함수로 GELU 대신 SwiGLU를 사용합니다. SwiGLU는 다음을 계산합니다:

```python
SwiGLU(x, W, V, b, c) = Swish(xW + b) ⊙ (xV + c)
```

1. 요소별 곱셈 `⊙` (게이팅)이 표준 활성화 함수로 불가능한 무엇을 가능하게 하나요?
2. "키-값 메모리"로 작동하는 언어 모델의 피드포워드 레이어에서 게이팅 메커니즘이 왜 유익할까요?
3. SwiGLU는 하나 대신 두 개의 가중치 행렬(W와 V)이 필요합니다. 원래 FFN의 hidden_dim이 4×d_model이었다면, LLaMA는 파라미터 수를 유지하기 위해 히든 차원을 어떻게 조정하나요?

<details>
<summary>정답 보기</summary>

**1. 게이팅이 가능하게 하는 것:**
요소별 곱셈은 **조건부 계산(conditional computation)** 메커니즘을 생성합니다: 게이트 `Swish(xW)`는 값 투영 `(xV)`의 특정 차원을 "끄거나" 줄일 수 있는 학습 가능한 필터 역할을 합니다. 이를 통해 네트워크가 어떤 정보가 통과할지 선택적으로 결정할 수 있습니다 — 본질적으로 입력 의존적 특징 선택을 구현합니다. 표준 활성화 함수는 컨텍스트에 관계없이 동일한 비선형성을 적용합니다.

**2. 게이팅이 FFN "키-값 메모리"에 유익한 이유:**
키-값 메모리 해석에서, 각 FFN 히든 유닛은 "키" 패턴과 쓸 "값"에 해당합니다. 게이트를 통해 네트워크가 현재 입력을 기반으로 관련 없는 메모리를 동적으로 억제할 수 있습니다. 코드, 추론, 사실적 회상 등 다양한 컨텍스트를 처리하는 언어 모델에게, 관련 지식 저장소를 선택적으로 활성화하는 것은 항상 전체 히든 레이어를 균일하게 적용하는 것보다 더 표현력이 풍부합니다.

**3. 차원 조정:**
표준 FFN: (d_model × 4·d_model) 모양의 행렬 2개 → 총 파라미터 = 2 × d × 4d = 8d²

SwiGLU FFN: (d_model × h) 모양의 행렬 3개 (W, V, 출력 투영) → 총 = 3 × d × h

일치시키기 위해: 3dh ≈ 8d² → h ≈ 8d/3 ≈ 2.67×d_model

LLaMA는 FFN 히든 차원을 ~(8/3)×d_model로 설정합니다 (하드웨어 효율성을 위해 256의 배수로 반올림). 4× 확장 대신 ~2.67× 확장을 3개 행렬로 사용하여 총 파라미터 수를 거의 동일하게 유지합니다.
...
연구자가 다음 사용 사례들을 가지고 있습니다. 각각에 대해 가장 적합한 LLaMA 버전과 크기를 추천하고 이유를 설명하세요.

1. 스마트 홈 어시스턴트를 위한 8GB RAM의 Raspberry Pi 5에서 추론 실행.
2. 병원 챗봇을 위한 단일 A100 80GB GPU에서 의료 QA 데이터셋 파인튜닝.
3. 4× H100 80GB GPU 서버에 범용 채팅 어시스턴트 배포.
4. 계약 분석을 위한 50K 토큰 법률 문서 처리.

<details>
<summary>정답 보기</summary>

**1. Raspberry Pi / 엣지 디바이스 (8GB RAM):**
- **LLaMA 3.2 1B** — 1B 모델은 모바일/엣지 하드웨어의 온디바이스 추론을 위해 특별히 설계되었습니다 (Qualcomm, MediaTek 칩 최적화). 4비트 양자화로 ~0.5GB 메모리 사용. LLaMA 3.2는 엣지 배포를 위해 명시적으로 설계되었습니다.

**2. 단일 A100 80GB, 의료 파인튜닝:**
- **LLaMA 3 8B** with QLoRA — 8B 모델은 fp16에서 ~16GB로 편안하게 맞고 파인튜닝 메모리 오버헤드를 위한 충분한 공간을 남깁니다. QLoRA (4비트 기반 + LoRA 어댑터)는 메모리를 ~6GB로 줄여 더 큰 배치 크기를 가능하게 합니다. LLaMA 3 8B는 도메인 적응에서 LLaMA 2보다 더 나은 기반 성능을 가집니다.

**3. 4× H100 서버, 범용 채팅:**
- **LLaMA 3.1 70B** — bf16의 70B 모델은 ~140GB를 필요로 하며, 텐서 병렬화로 4×H100에 맞습니다. 이 크기는 범용 채팅에서 GPT-4 수준의 성능을 제공합니다. LLaMA 3.1은 특히 프로덕션 어시스턴트에 적합한 강력한 지시 따르기 및 도구 사용 기능을 추가했습니다.

**4. 50K 토큰 문서 처리:**
- **LLaMA 3.1 8B 또는 70B** — LLaMA 3.1은 128K 컨텍스트 창을 기본 지원하며, 50K 토큰 문서에 필수적입니다. LLaMA 3 (.1 없음)은 기본 8K 컨텍스트만 지원하고 이런 긴 문서에는 비용이 많이 드는 컨텍스트 확장 기법이 필요합니다. 비용을 위해서는 8B, 정확도를 위해서는 70B를 선택하세요.
