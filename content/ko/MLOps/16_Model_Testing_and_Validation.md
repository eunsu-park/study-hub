[← 이전: 15. LLMOps](15_LLMOps.md) | [다음: 17. 클라우드 MLOps 플랫폼 →](17_Cloud_MLOps_Platforms.md)

# 모델 테스트와 검증(Model Testing and Validation)

## 학습 목표

1. ML 모델 테스트가 전통적인 소프트웨어 테스트와 근본적으로 다른 이유를 이해한다
2. ML 테스트의 4가지 범주인 데이터 검증(Data Validation), 모델 품질(Model Quality), 행동 테스트(Behavioral Testing), 인프라(Infrastructure) 테스트를 구현한다
3. 자동화된 ML 검증을 위해 테스트 프레임워크(pytest, Great Expectations, Deepchecks)를 활용한다
4. 쓰레기 입력-쓰레기 출력(garbage-in-garbage-out) 오류를 방지하는 훈련 전 데이터 품질 게이트를 설계한다
5. 성능 벤치마크와 공정성(Fairness) 지표를 갖춘 훈련 후 검증 파이프라인을 구축한다
6. 섀도우 배포(Shadow Deployment)와 카나리아 릴리스(Canary Release) 전략을 적용해 모델을 안전하게 출시한다
7. 프로덕션 성능 저하를 감지하고 재훈련을 트리거하는 지속적인 검증 루프를 설정한다

---

## 개요

전통적인 소프트웨어 공학에서는 함수가 올바른 결과를 반환하거나 그렇지 않거나 둘 중 하나다 — 테스트는 결정론적이다. 머신러닝은 이 전제를 무너뜨린다. 모든 단위 테스트를 통과한 모델이라도 데이터가 변화하거나, 특정 하위 집단이 과소 표현되거나, 미묘한 피처 상호작용이 변경되면 프로덕션에서 치명적으로 실패할 수 있다. 모델 테스트와 검증은 ML 라이프사이클 전반에 걸쳐 — 원시 데이터가 파이프라인에 들어오는 순간부터 모델이 백만 번째 예측을 제공하는 날까지 — 체계적인 안전장치를 구축하는 분야다.

이 레슨은 ML 테스트의 전 영역을 다룬다. 무엇을 테스트할 것인지, 언제 테스트할 것인지, 어떻게 자동화할 것인지를 다룬다. 데이터 검증에서 시작해 모델 품질 검사, 행동 테스트, 공정성 감사, 섀도우 배포, 지속적인 프로덕션 모니터링까지 차례로 쌓아 올린다. 이 레슨을 마치면 사용자에게 도달하기 전에 실패를 포착하는 테스트 전략을 설계할 수 있게 된다.

> **비유**: ML 모델을 테스트하는 것은 공장의 품질 관리와 같다. 생산 라인에 들어가기 전에 원자재를 검사하고(데이터 검증), 조립 과정 자체를 테스트하며(훈련 검증), 완성품을 사양에 맞춰 검사하고(모델 평가), 고객 보고를 통해 현장 성능을 모니터링한다(프로덕션 모니터링). 어느 단계든 생략하면 불량품이 고객에게 전달된다 — ML에서 "불량"은 편향되거나, 부정확하거나, 위험할 정도로 잘못된 예측을 의미한다.

---

## 1. ML 테스트가 소프트웨어 테스트와 다른 이유
...
## 2. 데이터 검증(Data Validation) 테스트

데이터는 토대다. 데이터가 잘못되어 있으면 아무리 모델을 튜닝해도 소용없다.

### 2.1 스키마 검증(Schema Validation)
...
### 2.2 분포 검증(Distribution Validation)
...
### 2.3 Great Expectations를 활용한 데이터 품질
...
## 3. 모델 품질 테스트(Model Quality Tests)

데이터가 검증되면 모델 자체를 테스트한다.

### 3.1 성능 임계값(Performance Threshold) 테스트
...
### 3.2 슬라이스 기반 테스트(Slice-Based Testing)
...
## 5. 인프라 테스트(Infrastructure Tests)

### 5.1 서빙 성능 테스트(Serving Performance Tests)
...
## 6. 공정성 테스트(Fairness Testing)

### 6.1 공정성 지표(Fairness Metrics)
...
## 7. 훈련 전·후 검증(Pre-Training and Post-Training Validation)

### 7.1 훈련 전 데이터 품질 게이트(Pre-Training Data Quality Gates)
...
### 7.2 훈련 후 검증(Post-Training Validation)
...
## 8. 섀도우 배포(Shadow Deployment)와 카나리아 릴리스(Canary Release)

### 8.1 섀도우 배포(Shadow Deployment)
...
## 9. 프로덕션에서의 지속적 검증(Continuous Validation in Production)

### 9.1 프로덕션 모니터링 루프(Production Monitoring Loop)
...
class ContinuousValidator:
    ...
    def should_retrain(self) -> dict:
        ...
        # 다중 알림 유형 → 복합 신호(compound signal)는 개별 알림보다 강력한 신호
        ...
        # 단일 유형이지만 지속적인 알림은 노이즈가 아닌 시스템적 문제를 시사
        if n_alerts >= 3:
            return {
                "decision": "RETRAIN",
...
## 10. CI/CD 파이프라인에서의 테스트 자동화

### 10.1 GitHub Actions에서의 ML 테스트
...
jobs:
  ...
  model-quality:
    ...
    steps:
      ...
      # if: always()로 테스트 실패 시에도 보고서 업로드 보장 — 디버깅에 필요
...
### 10.2 ML 테스트를 위한 Pytest 픽스처(Pytest Fixtures)
...
class TestModelQuality:
    ...
    def test_prediction_distribution(self, trained_model, test_data):
        ...
        상수 예측기(constant predictor)는 불균형 데이터에서도 정확도 임계값을 통과할 수 있다.
        """
        X, _ = test_data
        y_pred = trained_model.predict(X)
...
## 11. 모델 카드(Model Card) 생성

### 11.1 자동화된 모델 카드(Automated Model Cards)
...
### 12.1 엔드-투-엔드 테스트 파이프라인(End-to-End Testing Pipeline)
...
def run_complete_ml_testing_pipeline(
    ...
    # 빠른 실패(fail-fast): 잘못된 데이터는 이후 모든 테스트를 무의미하게 만듦
...
모델 테스트와 검증은 취미 ML 프로젝트와 프로덕션 수준의 ML 시스템을 구분하는 분야다. 핵심 정리:

| 레이어 | 테스트 대상 | 실행 시점 | 주요 도구 |
|--------|-------------|-----------|-----------|
| 데이터 검증(Data Validation) | 스키마, 분포, 누수 | 훈련 전 | Great Expectations, Pandera |
| 모델 품질(Model Quality) | 정확도, F1, AUC 임계값 | 훈련 후 | pytest, 커스텀 지표 |
| 행동 테스트(Behavioral Tests) | 불변성, 방향성, MFT | 훈련 후 | CheckList, 커스텀 스위트 |
| 공정성(Fairness) | 인구통계학적 동등성, 균등 기회 | 훈련 후 | Fairlearn, Aequitas |
| 인프라(Infrastructure) | 지연시간, 메모리, 처리량 | 배포 전 | Locust, 커스텀 벤치마크 |
| 섀도우 테스트(Shadow Testing) | 실제 트래픽 비교 | 배포 중 | 서비스 메시, 커스텀 라우팅 |
| 지속적 모니터링(Continuous Monitoring) | 드리프트, 신뢰도, 오류 | 프로덕션 중 | Evidently, Prometheus |

ML 테스트 철학: **일찍, 자주, 실제 데이터로, 예상치 못한 것에 대비해 테스트하라.**
...
### 연습문제 1: 데이터 검증 스위트
원하는 데이터셋을 위한 데이터 검증 스위트를 구축하라. 다음을 포함할 것:
- 최소 5개의 컬럼 규칙을 갖는 스키마 검증
- PSI를 사용한 분포 검증
- 설정 가능한 임계값을 갖는 결측값 분석
- 의도적으로 손상된 데이터로 스위트를 테스트하여 문제를 감지하는지 확인

### 연습문제 2: 행동 테스트 스위트
텍스트 분류 모델(감성 분석, 스팸 등)을 위해:
- 최소 기능 테스트 케이스 10개 작성
- 동의어 치환을 활용한 불변성 테스트 5개 구현
- 방향성 기대 테스트 3개 구현
- 각 테스트 유형별 실패율 보고

### 연습문제 3: 공정성 감사 보고서
보호 속성이 있는 데이터셋으로 훈련된 이진 분류 모델을 활용해:
- 인구통계학적 동등성(Demographic Parity), 균등 기회(Equalized Odds), 예측 동등성(Predictive Parity)을 계산하라
- 5분의 4 규칙을 적용하고 위반 사항을 문서화하라
- 감지된 편향에 대한 완화 전략을 제안하라
- 결과를 문서화한 모델 카드를 생성하라

### 연습문제 4: CI/CD 통합
다음을 수행하는 GitHub Actions 워크플로우를 작성하라:
- 모든 풀 리퀘스트에서 데이터 검증 실행
- 모델 코드가 변경될 때만 모델 품질 테스트 실행
- 차단(BLOCKING) 테스트가 실패하면 병합 차단
- 아티팩트로 테스트 보고서를 생성하고 업로드

### 연습문제 5: 섀도우 배포 시뮬레이터
다음 기능을 갖는 섀도우 배포 시뮬레이터를 구현하라:
- 프로덕션 모델과 섀도우 모델로 요청을 라우팅
- 일치율, 지연시간 비교, 정확도 차이 추적
- 카나리아 승격 로직 구현
- 승격(promote), 유지(hold), 롤백(rollback) 중 하나를 권장하는 보고서 생성
