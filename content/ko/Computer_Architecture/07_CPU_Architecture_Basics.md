...
**이전**: [순차 논리 회로](./06_Sequential_Logic.md) | **다음**: [제어장치](./08_Control_Unit.md)
...
1. CPU의 주요 구성요소(ALU, 레지스터, 제어 장치, 버스)를 식별한다
2. 인출-해독-실행 사이클(fetch-decode-execute cycle)을 단계별로 추적한다
3. 프로그램 카운터(PC), 명령어 레지스터(IR), MAR/MBR의 역할을 설명한다
4. 데이터패스(datapath)가 CPU 구성요소를 어떻게 연결하는지 설명한다
5. 레지스터-레지스터(register-register) 방식과 레지스터-메모리(register-memory) 방식을 구분한다
6. 단일 사이클(single-cycle)과 다중 사이클(multi-cycle) CPU 구현을 비교한다
7. 서로 다른 CPU 설계에서 실행 시간과 CPI를 계산한다

---

CPU는 모든 컴퓨터의 두뇌이지만, 인간의 두뇌와 달리 몇 가지 구성요소를 통해 신호를 추적하면 동작 원리를 완전히 이해할 수 있습니다. CPU 구조를 이해하면 특정 프로그램이 왜 더 빠르게 실행되는지, 명령어 집합(instruction set)이 왜 중요한지, 그리고 하드웨어와 소프트웨어가 어떻게 함께 진화하는지를 알 수 있습니다. 이 레슨에서는 CPU의 내부 구성요소, 레지스터의 종류, 데이터패스 구조, 그리고 명령어 실행 사이클에 대해 상세히 살펴봅니다.
