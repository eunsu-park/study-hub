...
**이전**: [명령어 집합 아키텍처](./09_Instruction_Set_Architecture.md) | **다음**: [파이프라이닝](./11_Pipelining.md)
...
1. 어셈블리 언어와 기계어의 관계 설명하기
2. 일반적인 레지스터와 그 관례 파악하기 (범용 레지스터, 스택 포인터, 프레임 포인터)
3. 데이터 전송, 산술, 분기 명령어를 사용하는 간단한 어셈블리 프로그램 작성하기
4. 어셈블리 프로그램 실행을 직접 손으로 추적하기
5. 고수준 언어 구조(if/else, 반복문, 함수 호출)가 어셈블리로 변환되는 방식 설명하기

---

어셈블리 언어는 원시 기계어 바로 위에 있는 가장 얇은 추상화 계층입니다. 어셈블리를 배운다는 것이 매일 어셈블리로 프로그래밍한다는 의미는 아닙니다 -- 하지만 C, Python, Java 프로그램이 하드웨어에서 실제로 무엇을 하는지 꿰뚫어 볼 수 있는 투시력을 줍니다. 성능 최적화, 크래시 덤프 디버깅, 또는 보안 익스플로잇(exploit)을 이해해야 할 때 어셈블리 지식은 필수입니다.
