...
**이전**: [제어장치](./08_Control_Unit.md) | **다음**: [어셈블리 언어 기초](./10_Assembly_Language_Basics.md)
...
1. ISA를 하드웨어와 소프트웨어 간의 계약으로 정의하기
2. 명령어 유형 분류하기 (데이터 전송, 산술, 제어 흐름)
3. 구체적인 예시를 통해 RISC와 CISC 설계 철학 비교하기
4. 주요 주소 지정 방식 설명하기 (즉시, 레지스터, 직접, 간접, 인덱스)
5. ISA 선택이 컴파일러 설계 및 프로그램 성능에 미치는 영향 분석하기
6. RISC 예시로서 MIPS/ARM 명령어 형식 설명하기

---

명령어 집합 아키텍처(Instruction Set Architecture)는 컴퓨팅에서 가장 중요한 인터페이스로, 소프트웨어와 하드웨어 간의 계약입니다. 어떤 언어로 작성하든 모든 프로그램은 결국 ISA 명령어로 컴파일됩니다. RISC 대 CISC 논쟁은 스마트폰의 ARM 칩부터 노트북의 x86에 이르기까지 현대 프로세서 전체 지형을 결정지었습니다.
...
> ISA를 하드웨어와 소프트웨어 간의 계약 또는 API로 생각하세요. 하드웨어는 특정 연산(명령어) 집합을 구현하겠다고 약속하고, 소프트웨어(컴파일러, OS)는 그 연산만 사용하겠다고 약속합니다. 어느 쪽도 상대방의 내부 동작 방식을 알 필요가 없습니다 -- 이 추상화 덕분에 서로 다른 CPU 구현에서도 동일한 프로그램을 실행할 수 있습니다.
...
│    │    - 데스크톱/서버 시장 진출 (Apple M4, AWS Graviton4)            │ │
│    └─────────────────────────────────────────────────────────────────┘ │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│    │  현재: 스마트폰, 태블릿, IoT, 서버 (Apple M1→M4, AWS Graviton4)│ │
│    │  특징: 저전력, 라이선스 기반 비즈니스                            │ │
│    └─────────────────────────────────────────────────────────────────┘ │
│                                                                         │
│    레지스터 (AArch64):                                                  │
