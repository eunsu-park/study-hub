...
**이전**: [CPU 구조 기초](./07_CPU_Architecture_Basics.md) | **다음**: [명령어 집합 아키텍처](./09_Instruction_Set_Architecture.md)
...
1. 명령어 실행 사이클에서 제어장치(control unit)의 역할을 설명한다
2. 하드와이어드 제어(hardwired control)와 마이크로프로그램 제어(microprogrammed control)를 구분한다
3. 명령어의 opcode가 특정 제어 신호에 매핑되는 과정을 추적한다
4. 마이크로명령어(microinstruction)의 구조(마이크로 연산, 시퀀싱, 조건)를 설명한다
5. 하드와이어드(속도)와 마이크로프로그램(유연성) 제어 방식 간의 트레이드오프를 비교한다
6. 제어 신호가 데이터패스(datapath) 구성요소를 어떻게 활성화하는지 설명한다

---

ALU가 계산기이고 레지스터가 연습장이라면, 제어장치는 무엇을 언제 계산할지를 결정하는 두뇌입니다. 명령어의 추상적인 의미를 하드웨어가 실제로 동작하게 만드는 구체적인 전기 신호로 변환합니다. 제어장치 설계를 이해하면 RISC 프로세서가 CISC 설계보다 왜 더 단순하고 빠를 수 있는지를 알 수 있습니다.
...
> 제어장치는 오케스트라의 지휘자와 같습니다. 지휘자는 직접 악기를 연주하지 않지만, 그의 신호 — 템포, 시작 큐, 여리게 하는 몸짓 — 가 CPU의 다른 모든 부분이 무엇을 언제 하는지를 결정합니다. 지휘자가 악보(명령어)를 해석하고 바이올린(ALU), 타악기(메모리), 금관악기(레지스터)를 조율하여 교향곡을 만들어내듯, 제어장치는 각 명령어를 해독하고 데이터패스(datapath)를 조율하는 정밀한 제어 신호를 생성합니다.
