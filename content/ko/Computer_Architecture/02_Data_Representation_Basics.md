...
**이전**: [컴퓨터 시스템 개요](./01_Computer_System_Overview.md) | **다음**: [정수와 부동소수점 표현](./03_Integer_Float_Representation.md)
...
1. 이진수, 8진수, 10진수, 16진수 사이에서 수를 변환할 수 있습니다
2. 컴퓨터가 이진 표현을 사용하는 이유를 설명할 수 있습니다
3. 올림(carry)/빌림(borrow)을 포함한 이진 덧셈과 뺄셈을 수행할 수 있습니다
4. 부호-크기(sign-magnitude), 1의 보수(1's complement), 2의 보수(2's complement)를 이용해 음수를 표현할 수 있습니다
5. 이진 산술에서 오버플로우(overflow) 검출을 설명할 수 있습니다
6. 데이터 단위(비트(bit), 니블(nibble), 바이트(byte), 워드(word))와 각각의 역할을 구분할 수 있습니다

---

컴퓨터는 10진수, 문자, 이미지를 이해하지 못합니다 — 오직 0과 1의 연속만을 이해합니다. 픽셀의 색상부터 주식 가격에 이르기까지 컴퓨팅의 모든 것은 이진수로 인코딩되어야 합니다. 데이터 표현을 이해하면 정수 오버플로우 버그가 발생하는 이유, 부동소수점 산술에서 예상치 못한 결과가 나오는 이유, 그리고 데이터가 하드웨어 수준에서 어떻게 저장되는지를 알 수 있습니다.
