...
**이전**: [캐시 메모리](./15_Cache_Memory.md) | **다음**: [입출력 시스템](./17_IO_Systems.md)
...
1. 하드웨어 관점(MMU, 페이지 테이블, TLB)에서 가상 메모리(Virtual Memory)를 설명할 수 있다
2. TLB(Translation Lookaside Buffer)가 주소 변환을 어떻게 가속하는지 설명할 수 있다
3. 다단계 페이지 테이블을 통한 가상-물리 주소 변환 과정을 추적할 수 있다
4. 가상 메모리가 프로세스 간 메모리 보호를 어떻게 제공하는지 설명할 수 있다
5. 하드웨어 관리 TLB와 소프트웨어 관리 TLB 설계를 비교할 수 있다
6. 캐시와 가상 메모리의 상호작용(VIVT, PIPT, VIPT)을 분석할 수 있다

---

가상 메모리는 운영체제와 하드웨어가 만나는 지점입니다. OS가 페이지 테이블을 정의하지만, 실제 주소 변환은 하드웨어(MMU와 TLB)가 모든 메모리 접근마다 — 초당 수십억 번 — 수행합니다. 가상 메모리의 하드웨어 측면을 이해하면 TLB 미스가 왜 비용이 큰지, 왜 거대 페이지(Huge Page)가 성능을 향상시키는지, 그리고 메모리 보호가 회로 수준에서 어떻게 동작하는지를 이해할 수 있습니다.
