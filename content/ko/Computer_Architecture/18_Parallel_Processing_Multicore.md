...
**이전**: [입출력 시스템](./17_IO_Systems.md) | **다음**: [RISC-V 아키텍처](./19_RISC_V_Architecture.md)
...
1. 클럭 주파수 스케일링이 전력 장벽(power wall)에 부딪힌 이유를 설명할 수 있다
2. 플린 분류(Flynn's taxonomy)(SISD, SIMD, MISD, MIMD)를 설명할 수 있다
3. 암달의 법칙(Amdahl's law)을 설명하고 이론적 속도 향상 한계를 계산할 수 있다
4. 공유 메모리(shared-memory)와 분산 메모리(distributed-memory) 멀티프로세서 아키텍처를 비교할 수 있다
5. 캐시 일관성 프로토콜(MESI)과 그 필요성을 설명할 수 있다
6. 현대 컴퓨팅에서 GPU와 SIMT 실행의 역할을 설명할 수 있다

---

수십 년간 CPU는 클럭 주파수를 높여 매년 더 빨라졌다. 하지만 2005년경 전력 소비가 한계에 달하면서 그 시대는 끝났다. 업계의 해답은 멀티코어, 즉 단일 칩에 여러 프로세서를 탑재하는 것이었다. 병렬 처리를 이해하면 16코어 프로세서가 단일 코어보다 16배 빠르지 않은 이유, 캐시 일관성이 어려운 이유, GPU가 머신러닝 워크로드를 지배하는 이유를 알 수 있다.
...
│                                                             │
│  Test-and-Set (TAS):                                        │
│  // 원자성(atomicity)이 중요한 이유: 읽기와 쓰기가 별도     │
│  // 명령어라면, 두 스레드가 모두 0("잠금 해제")을 읽은 뒤   │
│  // 둘 다 1을 쓸 수 있다 — 둘 다 락을 획득했다고 착각.     │
│  // x86의 LOCK 접두사는 읽기-수정-쓰기(read-modify-write)를 │
│  // 하나의 불가분(indivisible) 연산으로 완료하도록 보장한다. │
│  ┌───────────────────────────────────────────────────┐     │
│  │  int TAS(int *lock) {                             │     │
│  │      int old = *lock;   // 읽기                   │     │
│  │      *lock = 1;         // 쓰기      원자적으로    │     │
...
│  // TTAS가 단순 TAS보다 훨씬 나은 이유: 매 TAS는 LOCK 버스  │
│  // 트랜잭션을 발행하여 다른 모든 캐시의 복사본을 무효화     │
│  // (invalidate)한다 — 락이 잡혀 있어 실패할 것이 뻔해도.   │
│  // TTAS에서는 내부 while 루프가 로컬 캐시(Shared 상태)에서 │
│  // 읽으므로 버스 트래픽이 0이다. 값이 변할 때(락 해제)만    │
│  // 비싼 TAS를 시도한다. 이로써 스핀 반복당 버스 경합이      │
│  // O(N)에서 해제 시 O(1)로 감소한다(N = 대기 코어 수).     │
│  ┌───────────────────────────────────────────────────┐     │
│  │  void lock(int *lock) {                           │     │
│  │      while (1) {                                  │     │
│  │          while (*lock == 1) {                     │     │
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  // CAS 재시도 루프가 동작하는 이유: load와 CAS 사이에 다른  │
│  // 스레드가 counter를 수정하지 않았다면 교환이 성공하여 완료.│
│  // 다른 스레드가 증가시켰다면 CAS가 실패하고, 'old'가 현재  │
│  // 값으로 갱신된 뒤 새 값으로 재시도한다. 라운드당 최소     │
│  // 1개 스레드가 성공하므로 진행(progress)이 보장된다.       │
...
```cuda
// 벡터 덧셈 CUDA 커널
// __global__인 이유: 이 한정자(qualifier)는 NVCC 컴파일러에게 vectorAdd가
// GPU(device)에서 실행되지만 CPU(host)에서 호출 가능함을 알린다. 백만 개의
// 스레드가 각각 다른 idx 값으로 이 동일한 함수 본문을 실행한다 —
// 이것이 SIMT(Single Instruction, Multiple Threads) 실행 모델이다.

__global__ void vectorAdd(float *A, float *B, float *C, int N) {
    // 전역 스레드 인덱스 계산
    // 이 공식의 의미: blockIdx.x는 이 스레드가 속한 블록 번호, blockDim.x는
    // 블록당 스레드 수(256), threadIdx.x는 블록 내 위치. 이 셋을 조합하면
    // 그리드 전체에서 0부터 N-1까지의 고유 인덱스가 만들어진다.
    int idx = blockIdx.x * blockDim.x + threadIdx.x;

    // 범위 검사가 필요한 이유: 그리드가 N개보다 더 많은 스레드를 실행할 수 있다
    // (blocksPerGrid가 올림 계산이므로). 이 가드(guard) 없이는 범위를 벗어난
    // 스레드가 배열 끝을 넘어 읽기/쓰기를 하여 메모리 손상(corruption)을 일으킨다.
    if (idx < N) {
        C[idx] = A[idx] + B[idx];
    }
}
...
int main() {
    ...
    // 커널 실행 설정
    // 블록당 256 스레드인 이유: 일반적인 최적점(sweet spot). GPU SM은 레지스터와
    // 공유 메모리가 제한적이므로, 256 스레드는 점유율(occupancy, 메모리 지연을
    // 숨길 충분한 워프)과 자원 압박 사이의 균형을 맞춘다.
    // 너무 적으면 SM이 유휴; 너무 많으면 레지스터가 느린 로컬 메모리로 스필(spill).
    int threadsPerBlock = 256;
    // 올림 계산 이유: N=1000000이고 threadsPerBlock=256이면,
    // ceil(1000000/256) = 3907 블록이 필요. (N + 255) / 256 공식은
    // 정수 올림(integer ceiling) 트릭이다.
...
다음 레슨 [RISC-V 아키텍처](./19_RISC_V_Architecture.md)에서는 오픈소스 RISC-V 명령어 집합 아키텍처를 탐구합니다. 학계와 산업계 모두에서 빠르게 채택되고 있는 현대적이고 모듈화된 ISA입니다.
