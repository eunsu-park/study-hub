...
**이전**: [데이터 표현 기초](./02_Data_Representation_Basics.md) | **다음**: [논리 게이트](./04_Logic_Gates.md)
...
1. 2의 보수(two's complement)가 다른 음수 표현 방식보다 선호되는 이유를 설명할 수 있다
2. 2의 보수 산술 연산(덧셈, 뺄셈, 부호 반전)을 수행할 수 있다
3. 단정밀도와 배정밀도에 대한 IEEE 754 부동소수점 형식(부호, 지수, 가수)을 설명할 수 있다
4. IEEE 754 특수값(무한대, NaN, 비정규화 수)을 식별할 수 있다
5. 부동소수점 산술에서 0.1 + 0.2가 0.3과 같지 않은 이유를 설명할 수 있다
6. 고정소수점(fixed-point)과 부동소수점(floating-point) 표현 방식의 트레이드오프를 비교할 수 있다

---

"대부분의 프로그래밍 언어에서 0.1 + 0.2가 왜 0.3이 아닌가?"라는 악명 높은 질문의 답은 IEEE 754 부동소수점 표현에 있습니다. 컴퓨터가 정수와 실수를 표현하는 방법을 이해하면, 현실 세계에서 실제 재난을 일으킨 버그들의 원인을 알 수 있습니다 -- 아리안 5 로켓 폭발(정수 오버플로우)부터 패트리어트 미사일 실패(부동소수점 오차 누적)까지.
