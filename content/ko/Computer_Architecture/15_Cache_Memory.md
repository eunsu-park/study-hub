...
**이전**: [메모리 계층 구조](./14_Memory_Hierarchy.md) | **다음**: [가상 메모리](./16_Virtual_Memory.md)
...
1. 캐시가 지역성 원리(Principle of Locality)를 어떻게 활용하는지 설명할 수 있다
2. 직접 사상(Direct-Mapped), 집합 연관(Set-Associative), 완전 연관(Fully-Associative) 캐시 사상 방식을 비교할 수 있다
3. LRU와 FIFO 캐시 교체 정책을 구현할 수 있다
4. 쓰기 즉시(Write-Through)와 나중 쓰기(Write-Back) 정책을 구분할 수 있다
5. 캐시 미스를 강제(Compulsory), 용량(Capacity), 충돌(Conflict) 미스(3C)로 분류할 수 있다
6. 다중 레벨 캐시 구조(L1/L2/L3)와 포함 정책(Inclusion Policy)을 설명할 수 있다
7. 캐시 성능 지표(히트율, AMAT)를 계산할 수 있다

---

캐시 메모리는 소프트웨어 성능에 있어 가장 중요한 단일 하드웨어 최적화입니다. 캐시 히트(Cache Hit)는 약 1나노초가 걸리지만, 메인 메모리까지 가는 캐시 미스(Cache Miss)는 약 100나노초가 걸립니다 — 100배의 성능 차이입니다. 캐시의 동작 원리를 이해하면 행렬 순회 순서가 왜 중요한지, 배열의 구조체(Array-of-Structs)보다 구조체의 배열(Struct-of-Arrays)이 왜 유리한지, 그리고 아무리 정교하게 최적화된 알고리즘이라도 캐시 친화적이지 않으면 왜 느릴 수 있는지를 이해할 수 있습니다.
...
> 캐시를 책상, 메인 메모리를 방 건너편의 서류 캐비닛, 디스크를 도시 반대편의 창고라고 생각해 보세요. 보고서를 작성할 때 필요한 문서들은 바로 책상 위(L1 캐시)에 놓아두어 즉시 접근할 수 있게 합니다. 덜 자주 필요한 파일들은 서류 캐비닛(메인 메모리)에 — 걸어서 잠깐 가면 되는 거리에 — 보관합니다. 거의 쓰지 않는 문서는 창고(디스크)에 — 꺼내오는 데 훨씬 더 오랜 시간이 걸립니다. 캐시 설계란 다음에 어떤 파일이 필요할지 예측하여 요청하기 전에 미리 책상 위에 올려두는 기술입니다.
...
┌─────────────────────────────────────────────────────────────┐
│  // 소프트웨어 프리페치(Software Prefetch) 예시              │
│  for (int i = 0; i < N; i++) {                              │
│      // __builtin_prefetch(addr, rw, locality):             │
│      //   addr = 프리페치할 주소                              │
│      //   rw: 0 = 읽기용, 1 = 쓰기용                        │
│      //   locality: 0-3 시간적 지역성 힌트                    │
│      //     3 = 모든 캐시 레벨에 유지 (재사용 높음)           │
│      //     0 = 사용 후 즉시 축출 가능 (스트리밍 접근)        │
│      // 16개 앞 ≈ 캐시 라인 1개 분량의 선행 거리:             │
│      //   64B 라인 / 4B int = 라인당 16개 int이므로,          │
│      //   다음 캐시 라인의 로드를 딱 맞는 시점에 시작.         │
...
// C는 2차원 배열을 행 우선(Row-Major)으로 저장: a[i][0], a[i][1], ... a[i][N-1]이
// 메모리에 연속 배치. 64바이트 캐시 라인은 int 16개를 담으므로, 행을 따라 순차
// 접근하면 1번 미스당 15번 히트 (공간적 지역성(Spatial Locality)).

// Before: 열 우선 (나쁜 지역성)
// 내부 루프에서 i를 반복하면 접근마다 N*4바이트 점프 (스트라이드(Stride)-N).
// N > 16 (캐시라인 / 요소 크기)이면 매 접근마다 미스 — 공간적 지역성 없음.
for (j = 0; j < N; j++)
    for (i = 0; i < N; i++)
        a[i][j] = b[i][j] + c[i][j];

// After: 행 우선 (좋은 지역성)
// 내부 루프에서 j를 반복하면 연속 메모리 주소에 접근.
// 16개 요소당 1번만 미스 — 공간적 지역성만으로 93.75% 히트율.
for (i = 0; i < N; i++)
    for (j = 0; j < N; j++)
        a[i][j] = b[i][j] + c[i][j];
...
루프 융합 (Loop Fusion):
// Before: 두 개의 루프 — a[i]를 루프 1에서 쓰고 루프 2에서 읽음.
// 배열이 캐시보다 크면 루프 2가 읽기 전에 a[i]가 캐시에서 축출됨.
for (i = 0; i < N; i++) a[i] = b[i] + 1;
for (i = 0; i < N; i++) c[i] = a[i] * 2;

// After: 하나의 루프 — a[i]를 쓰자마자 L1 캐시에 있는 동안 바로 읽음.
// a[]에 대한 용량 미스(Capacity Miss)를 제거하고 총 메모리 트래픽을 약 절반으로 줄임.
for (i = 0; i < N; i++) {
    a[i] = b[i] + 1;
    c[i] = a[i] * 2;
}
...
루프 타일링 (Loop Tiling):
// Before — 단순 행렬 곱셈.
// B[k][j]는 열 방향(스트라이드(Stride)-N)으로 접근: N이 크면 캐시 성능 매우 나쁨.
// 작업 집합(Working Set) = B 행렬 전체로, N이 크면 캐시를 초과함.
for (i = 0; i < N; i++)
    for (j = 0; j < N; j++)
        for (k = 0; k < N; k++)
            C[i][j] += A[i][k] * B[k][j];
...
// BLOCK은 A, B, C의 BLOCK×BLOCK 부분 행렬이 모두 캐시에 동시에 들어가도록 선택
// (3 × BLOCK² × sizeof(double) ≤ L1 캐시 크기).
// 각 타일 내에서는 작업 집합이 작아 B도 좋은 지역성으로 접근됨.
...
7. 다음 코드의 캐시 성능을 분석하시오 (64바이트 캐시 라인, 4바이트 int):
   ...
   // 순차 접근(Sequential Access) 패턴: 각 배열을 선형으로 순회.
   // 64B 라인 / 4B int = 라인당 16개 int → 16번 접근당 1번 강제 미스(Cold Miss).
   // 힌트: 총 접근 수, 강제 미스 수를 세고 히트율을 도출하시오.
