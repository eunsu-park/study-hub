...
**이전**: [파이프라이닝](./11_Pipelining.md) | **다음**: [슈퍼스칼라와 비순차 실행](./13_Superscalar_Out_of_Order.md)
...
1. 분기가 파이프라이닝을 방해하는 이유를 설명할 수 있습니다
2. 정적 분기 예측(static branch prediction)과 동적 분기 예측(dynamic branch prediction)을 구별할 수 있습니다
3. 1비트 및 2비트 분기 예측기(branch predictor)를 구현할 수 있습니다
4. 분기 타겟 버퍼(Branch Target Buffer, BTB)와 그 역할을 설명할 수 있습니다
5. 분기 예측 실패(branch misprediction)가 성능에 미치는 영향을 계산할 수 있습니다
6. 투기적 실행(speculative execution)과 예측의 관계를 설명할 수 있습니다

---

현대 CPU는 초당 수십억 개의 명령어를 실행할 수 있지만, 잘못 예측된 분기 하나가 수십 사이클의 작업을 낭비시킬 수 있습니다. 분기 예측, 즉 분기가 확정되기 전에 CPU가 결과를 추측하는 기법은 파이프라인을 가득 채워 유지하는 데 매우 중요합니다. 예측 원리를 이해하면 루프 성능이 분기가 많은 코드와 다른 이유, 그리고 Spectre 계열 취약점이 존재하는 이유를 알 수 있습니다.
