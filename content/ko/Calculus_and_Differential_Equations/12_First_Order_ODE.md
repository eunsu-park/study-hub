...
## 5. 베르누이 방정식(Bernoulli Equations)

### 5.1 형태와 치환

**베르누이 방정식(Bernoulli equation)**은 다음 형태를 가진다:

$$y' + P(x)\,y = Q(x)\,y^n, \quad n \neq 0, 1$$

$n = 0$이면 4절의 1계 선형 ODE와 같다. $n = 1$이면 분리 가능하다. 그 외의 모든 $n$ 값에 대해 방정식은 비선형이지만, 단 하나의 치환으로 선형화할 수 있다.

**치환:** $v = y^{1-n}$으로 놓는다. 그러면 $v' = (1-n)y^{-n}y'$이고, 따라서 $y' = \dfrac{y^n}{1-n}v'$이다.

원래 방정식을 $y^n$으로 나누고 치환하면:

$$\frac{v'}{1-n} + P(x)\,v = Q(x)$$

$$\boxed{v' + (1-n)P(x)\,v = (1-n)Q(x)}$$

이것은 $v$에 대한 **1계 선형 ODE**이며, 적분인자 방법으로 풀 수 있다.

### 5.2 예제

$y' - y = xy^2$ ($n = 2$)를 풀어라.

**1단계:** $v = y^{1-2} = y^{-1}$로 놓으면, $v' = -y^{-2}y'$이다.

**2단계:** $y^2$으로 나누면: $y^{-2}y' - y^{-1} = x$. $v$와 $v'$를 대입하면:

$$-v' - v = x \quad \Longrightarrow \quad v' + v = -x$$

**3단계:** 적분인자 $\mu = e^x$로 풀기:
...
## 7. 존재성과 유일성(Existence and Uniqueness)

### 7.1 피카르-린델뢰프 정리(The Picard-Lindelof Theorem)
...
### 8.1 뉴턴의 냉각 법칙(Newton's Law of Cooling)
