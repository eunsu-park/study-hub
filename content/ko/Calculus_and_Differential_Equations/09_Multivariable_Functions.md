...
## 7. 라그랑주 승수법(Lagrange Multipliers, 제약 최적화)

2계 도함수 판정법은 변수가 **자유로울 때** $f(x, y)$의 극값을 찾는다. 많은 실제 문제는 **제약 조건** $g(x, y) = 0$ 하에서 $f$를 최적화해야 한다 -- 예를 들어 고정된 둘레로 넓이를 최대화하거나, 생산 목표를 만족시키면서 비용을 최소화하는 경우.

### 방법

제약된 극값에서 $f$의 기울기는 $g$의 기울기에 평행해야 한다:

$$\nabla f = \lambda \, \nabla g$$

여기서 $\lambda$를 **라그랑주 승수(Lagrange multiplier)**라고 한다. 제약 조건과 결합하면 다음 연립 방정식을 얻는다:

$$f_x = \lambda \, g_x, \qquad f_y = \lambda \, g_y, \qquad g(x, y) = 0$$

**기하학적 직관:** 최적점에서 $f$의 등고선과 제약 곡선 $g = 0$은 접해야 한다 -- 서로 교차한다면, 제약 조건을 따라 이동하여 $f$를 더 증가시킬 수 있기 때문이다.

### 풀이 예제

**$x + y = 10$을 만족하면서 $f(x, y) = xy$를 최대화**하라.

제약 조건을 $g(x, y) = x + y - 10 = 0$으로 쓴다.

연립 방정식을 세우면:

$$f_x = y = \lambda \cdot 1, \qquad f_y = x = \lambda \cdot 1, \qquad x + y = 10$$

처음 두 방정식으로부터: $y = \lambda$이고 $x = \lambda$이므로 $x = y$이다. 제약 조건에 대입하면: $2x = 10$, 따라서 $x = y = 5$.

**최댓값:** $f(5, 5) = 25$.

**검증:** AM-GM 부등식에 의해 $xy \leq \left(\frac{x+y}{2}\right)^2 = 25$이며, 등호(최댓값)는 $x = y$일 때 성립한다.
