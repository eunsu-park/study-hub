...
## 초기값 정리와 최종값 정리(Initial and Final Value Theorems)

이 두 정리는 역 라플라스 변환을 계산하지 않고도 $F(s)$로부터 $f(t)$에 관한 핵심 정보를 직접 추출할 수 있게 해준다 -- 공학 분석에서 매우 유용한 단축법이다.

### 초기값 정리(Initial Value Theorem)

$$\lim_{t \to 0^+} f(t) = \lim_{s \to \infty} s F(s)$$

이것은 두 극한이 모두 존재하고 $f(t)$가 $t = 0$에서 충격을 가지지 않을 때 성립한다.

**해석:** $f(t)$의 $t = 0$ 근방 거동은 $s \to \infty$일 때 $sF(s)$의 거동에 대응한다. 큰 $s$ 값은 단기 거동을 "탐색"한다.

### 최종값 정리(Final Value Theorem)

$$\lim_{t \to \infty} f(t) = \lim_{s \to 0^+} s F(s)$$

**중요 조건:** 이 정리는 $sF(s)$의 모든 극점(pole)이 엄격히 음의 실수부를 가질 때에만 유효하다 (즉, 시스템이 안정적이고 $f(t)$가 실제로 유한한 정상 상태 값을 가질 때). $sF(s)$의 극점이 허수축 위에 있거나 우반평면에 있으면 이 정리는 적용되지 않는다.

**예제:** 위에서 풀었던 IVP에서 $Y(s) = \dfrac{1}{s+1} + \dfrac{1}{s+2} - \dfrac{1}{s+3}$.

- **초기값:** $\lim_{s \to \infty} sY(s) = \lim_{s \to \infty} \left(\dfrac{s}{s+1} + \dfrac{s}{s+2} - \dfrac{s}{s+3}\right) = 1 + 1 - 1 = 1$. 이것은 $y(0) = 1$과 일치한다.
- **최종값:** $\lim_{s \to 0^+} sY(s) = 0 + 0 - 0 = 0$. 해는 $t \to \infty$에서 0으로 감쇠한다 (모든 극점이 음수: $-1, -2, -3$), 시스템이 안정적임을 확인한다.

**이것이 중요한 이유:** 제어 시스템에서 최종값 정리는 완전한 역변환을 풀지 않고도 폐루프 시스템의 정상 상태 출력을 알려준다. 제어기를 설계하고 출력이 원하는 설정값에 정착하는지 확인할 필요가 있다면, 단순한 극한 계산 하나로 충분하다.
