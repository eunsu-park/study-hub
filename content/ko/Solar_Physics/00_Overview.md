...
| 파일 | 설명 |
|------|------|
| `01_solar_structure.py` | 표준 태양 모델: 다방정식(polytropic) 적분을 통한 온도·밀도·압력의 반경 방향 분포 |
| `02_pp_chain.py` | 가모프 피크 계산, pp 연쇄 에너지 생성률, 중성미자 플럭스 |
| `03_helioseismology.py` | 구면 조화 분해, l-ν 다이어그램, 음파 광선 추적 |
| `04_limb_darkening.py` | Eddington-Barbier 해, 주연 감광 곡선, 분광선 프로파일 |
| `05_chromosphere_dem.py` | 다파장 EUV를 이용한 태양 대기의 차분 방출 측도(DEM) 분석 |
| `06_pfss_model.py` | PFSS(Potential Field Source Surface) 외삽, 코로나 자기 위상 구조 |
| `07_sunspot_cycle.py` | 태양 주기 분석: 흑점수 시계열, FFT 주기성, Waldmeier 효과 |
| `08_flare_energetics.py` | 태양 플레어 에너지 분배 모델: 자기 에너지 추정 및 다파장 방출 |
| `09_cme_propagation.py` | 앙상블 예측을 포함한 항력 기반 CME 전파 모델, 도달 시간 예측 |
| `10_parker_spiral.py` | 파커 나선: 행성 간 자기장 기하, 동결-입 자속, 태양풍 속도 프로파일 |
| `11_solar_spectrum.py` | 합성 태양 스펙트럼: 플랑크 함수, 흡수선, 제이만 분열 |
| `12_sep_transport.py` | 집속 수송 방정식: 파커 나선을 따른 1D SEP 전파 |
