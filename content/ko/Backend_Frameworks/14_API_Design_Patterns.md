...
패턴 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. HTTP 멱등성의 수학적 정의, RFC 7807 Problem Details 명세, 그리고 같은 축 위에서 비교한 REST/GraphQL/gRPC를 다룹니다.
...
API 설계 패턴은 관습 목록처럼 보이지만, 사실 세 가지 더 깊은 아이디어의 하류입니다. 각각을 내재화하면 나머지는 기계적으로 따라옵니다.

- **(A) 수학적 계약으로서의 HTTP 메서드 의미** — safe, idempotent, cacheable은 분위기가 아니라 정확한 성질입니다.
- **(B) RFC 7807 Problem Details와 기계 판독 가능한 오류** — "구조화된 오류 응답"의 명세.
- **(C) REST vs RPC vs GraphQL vs gRPC** — 같은 축 위에서 비교된 네 가지 아키텍처 스타일.

### A. 멱등성의 수학적 정의

HTTP/1.1 명세(RFC 9110)는 미들박스(프록시, 재시도, 브라우저)가 메서드를 어떻게 다루는지를 제약하는 두 가지 성질을 정의합니다. 둘 다 와이어 포맷이 아니라 *서버 동작*의 수학적 성질입니다.

#### A.1 Safe 메서드

메서드가 **safe**하다는 것은 그 의미가 본질적으로 읽기 전용 — 호출이 서버 상태를 수정하지 않는다는 뜻입니다. 형식적으로:

```
∀ 요청 r: state(server, r 이후) = state(server, r 이전)
```

`GET`, `HEAD`, `OPTIONS`는 safe입니다. `POST`, `PUT`, `PATCH`, `DELETE`는 아닙니다. 계약이 중요한 이유는:

- 브라우저와 크롤러는 safe 메서드를 자유롭게 prefetch합니다.
- 캐시는 묻지 않고 safe 메서드 응답을 저장합니다.
- 전송 오류 시의 재시도는 safe 메서드에 무조건 OK입니다.

몰래 쓰기를 로깅하는 "safe" 메서드는 명세를 위반하며 — 중간자를 조용히 깨뜨립니다. 규율: `GET` 핸들러는 상태를 수정해서는 안 됩니다. 끝.

#### A.2 멱등 메서드

메서드가 **멱등(idempotent)**이라는 것은 같은 요청을 여러 번 만드는 것이 한 번 만드는 것과 서버에 같은 효과를 가진다는 뜻입니다. 형식적으로:

```
state(server, [r, r, r, ...] 이후) = state(server, [r] 이후)
```

멱등성은 "매번 같은 응답을 반환한다"는 뜻이 아닙니다 — *서버 상태*가 같은 값으로 수렴한다는 뜻입니다. `DELETE /users/42`는 멱등입니다: 한 번 호출 후 사용자가 사라지고, 백 번 호출 후에도 사용자는 여전히 사라져 있습니다(후속 호출은 `404`를 반환하지만 상태는 같습니다).

HTTP 메서드 분류:

| 메서드 | Safe | 멱등 |
|--------|------|-----------|
| GET, HEAD, OPTIONS | 예 | 예 |
| PUT, DELETE | 아니오 | 예 |
| POST, PATCH (때때로) | 아니오 | 아니오 |

이것이 중요한 이유: 멱등 메서드는 *재시도*해도 안전합니다. 타임아웃 난 `PUT`은 다시 보낼 수 있습니다 — 최악의 경우 같은 쓰기를 두 번 합니다. 비멱등 `POST`는 그럴 수 없습니다 — 재시도가 두 번째 주문을 만들거나 두 번 청구할 수 있습니다.

#### A.3 멱등성 키: 애플리케이션 계층에서 POST를 멱등으로

실세계 API는 멱등 결제·주문·메시지 엔드포인트를 필요로 하지만, 그 연산들은 POST 모양(생성)입니다. 표준 해결책은 `Idempotency-Key` 헤더(RFC 초안, Stripe 관습)입니다.

```
POST /payments
Idempotency-Key: 7e3b1c8d-4f9a-4...

# 첫 호출: 서버가 처리하고 키로 결과 저장
# 반복 호출(같은 키): 서버가 저장된 결과 반환; 새 청구 없음
```

서버는 적어도 요청 재시도 창(보통 24시간) 동안 키를 기억해야 합니다. 이는 명세가 도울 수 없는 멱등성 성질을 애플리케이션 계층으로 밀어 넣습니다.

### B. RFC 7807: HTTP API를 위한 Problem Details

RFC 7807 이전에는 모든 API가 자체 오류 응답 형식을 발명했습니다: `{"error": "..."}`, `{"errors": [...]}`, `{"code": 42, "message": "..."}`. 명세는 잘 정의된 의미를 가진 작은 필드 집합을 표준화합니다.
...
```json
{
  ...
  "title": "신용이 부족합니다.",
  "status": 403,
  "detail": "잔액은 30이지만, 요청된 금액은 50입니다.",
  "instance": "/account/12345/transactions/67890"
}
```

- `type` — 문제 클래스를 식별하는 URI. 같은 `type`의 두 응답은 같은 종류의 문제를 의미합니다. URI(불투명한 코드가 아니라)를 쓰는 이유는 dereference해서 문서화로 이어질 수 있기 때문입니다.
- `title` — 짧은 사람이 읽을 수 있는 요약, 이상적으로 주어진 `type`에 대해 일정.
- `status` — HTTP 상태 코드, 본문만 읽는 클라이언트를 위해 중복 표기.
- `detail` — 인스턴스별 사람이 읽을 수 있는 설명.
- `instance` — *이 특정 발생*을 식별하는 URI.

Content-Type은 `application/problem+json`(또는 `+xml`)입니다. 어떤 확장 필드든 허용됩니다 — 예를 들어 위 예시에서 `balance: 30, requested: 50`.

#### B.2 `type`이 코드가 아니라 URI인 이유

정수 코드(`E001`, `E002`)를 발명하고 싶은 유혹이 있습니다. RFC 7807이 URI를 선택한 이유는:

- URI는 조정 없이 전역적으로 고유합니다.
- URI는 dereference할 수 있습니다 — 브라우저로 가리키면 문서가 반환됩니다.
- 코드는 버전을 넘나들며 흘러가지만, URI는 독립적으로 버전 관리됩니다.

실제로 대부분의 API는 자체 도메인 아래의 경로(`https://example.com/probs/out-of-credit`)를 사용하고, 그 URL에서 문서를 제공합니다.

#### B.3 검증 오류: 구조화된 배열 확장

RFC 7807 자체는 *여러* 검증 오류를 어떻게 표현할지 명시하지 않습니다. 사실상의 관습은 `errors` 배열을 추가하는 것입니다.
...
```json
{
  ...
  "status": 422,
  "errors": [
    {"field": "email", "code": "invalid", "message": "유효한 이메일 주소가 아닙니다"},
    {"field": "age", "code": "too_low", "message": "최소 18세 이상이어야 합니다"}
  ]
}
```

이 모양은 클라이언트가 자유 텍스트를 파싱하지 않고 폼에서 필드별 오류 메시지를 렌더링할 수 있게 해줍니다.

### C. 비교된 네 가지 아키텍처 스타일

REST는 백엔드 API의 네 가지 널리 쓰이는 스타일 중 하나입니다. 각각이 설계 공간의 다른 지점입니다. 지능적으로 선택한다는 것은 축을 안다는 뜻입니다.

#### C.1 비교 매트릭스

| 성질 | REST | RPC (JSON-RPC) | GraphQL | gRPC |
|----------|------|---------------|---------|------|
| 전송 | HTTP | HTTP | HTTP | HTTP/2 + Protobuf |
| 발견 | URL 관습 | 메서드 디렉터리 | 스키마 | Protobuf .proto |
| Over-fetch | 가능 | 모양당 메서드 | 클라이언트가 선택 | 모양당 메서드 |
| 버전 관리 | URL/헤더 | 메서드 이름 | 스키마 deprecation | 패키지 버전 |
| 스트리밍 | SSE / chunked | 별도 구현 | Subscription | 일급 |
| 도구 | 브라우저, cURL | OpenRPC | GraphiQL | grpcurl |
| 브라우저 지원 | 네이티브 | 네이티브 | 네이티브 | grpc-web (제한적) |

#### C.2 REST의 강점과 약점

**강점:** 브라우저 네이티브(추가 라이브러리 불필요), 모든 계층에서 캐시 가능(URL이 캐시 키), 잘 이해된 의미(모든 개발자가 `GET`/`POST`를 압니다).

**약점:** Over-fetch와 under-fetch(`GET /users/42`가 전체 사용자를 반환하지만 보통 이름만 필요). 중첩 데이터에 대한 N+1 라운드트립. 연산이 CRUD에 맞지 않을 때의 동사 불일치(`POST /orders/42/cancel/`은 우회).

#### C.3 GraphQL의 제안

클라이언트가 *정확히* 필요한 필드를 지정하는 쿼리를 보냅니다.
...
라운드트립 한 번, over-fetch 없음, under-fetch 없음. 비용: 서버는 모든 수준의 모든 필드에 대한 resolver를 구현해야 합니다(resolver가 행별 쿼리를 하면 N+1 위험 — DataLoader가 표준 해결책). 캐싱이 더 어려운데, 캐시 키가 URL이 아니라 쿼리이기 때문입니다.

GraphQL은 클라이언트가 다양할 때(모바일 + 웹 + 대시보드)나 데이터 모양이 뷰별로 극적으로 달라질 때 이깁니다.

#### C.4 gRPC의 제안

gRPC는 HTTP/2 + Protobuf를 사용합니다. 스키마는 `.proto` 파일에 살고, 코드 생성이 여러 언어로 타입이 있는 클라이언트를 만듭니다. 스트리밍(서버 측, 클라이언트 측, 양방향)이 일급입니다.

지연시간, 언어 간 타입 안전성, 바이너리 효율성이 중요한 곳에서 이깁니다 — 마이크로서비스 아키텍처 내부의 서비스 간 통신에 전형적입니다. 브라우저 클라이언트에서는 패배(gRPC-Web이 부분적 우회)하고, 사람이 디버깅 가능한 API에도 패배(Protobuf는 바이너리, curl 친화적이지 않음)합니다.

#### C.5 선택 원칙

*주된* 소비자에 대한 마찰을 최소화하는 스타일을 고르세요.

- **외부, 공개 API** → REST(브라우저 친화적, 잘 문서화됨, 잘 캐시됨).
- **풍부한 공유 백엔드를 가진 모바일 + 웹 앱** → GraphQL(스키마 하나, 모양 여럿).
- **내부 마이크로서비스** → gRPC(타입 안전, 빠름, 스트리밍).
- **CRUD가 어색한 액션 중심 도메인** → 액션 엔드포인트가 있는 JSON-RPC 또는 REST.

### 이론에서 아래 패턴으로

뒤에 나오는 각 절은 이 틀의 한 조각을 구체화합니다.

- §1 (RESTful 리소스 설계)는 §C.2 "기본값으로서의 REST" 기준선의 동사 위 명사 원칙입니다.
- §2 (HTTP 메서드와 멱등성)은 §A.1, §A.2, §A.3을 구체적인 코드로.
- §3 (상태 코드 관례)는 레슨 01 §A.2의 5개 상태 클래스의 정확한 사용입니다.
- §4 (페이지네이션 패턴)은 리스트 엔드포인트 수준에서의 over-fetch 방어입니다 — offset, cursor, keyset.
- §5 (필터링, 정렬, 필드 선택)은 REST 모양 쿼리 안에서의 GraphQL 약속(§C.3)의 부분 버전입니다.
- §6 (API 버전 관리)는 §C.5 계약을 클라이언트를 깨지 않고 진화시키는 방법입니다(URL 접두, Accept 헤더, 파라미터).
- §7 (HATEOAS)는 응답 안의 link relation으로 표현된 원래의 REST §균일 인터페이스 제약입니다.
- §8 (오류 응답 형식)은 §B의 RFC 7807을 구체적인 JSON으로.
- §9 (속도 제한 헤더)는 클라이언트 측 백오프를 위한 `X-RateLimit-Remaining`, `Retry-After` 관습입니다.
