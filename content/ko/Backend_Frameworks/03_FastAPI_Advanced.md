...
프레임워크 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. 의존성 주입이 어떻게 그래프가 되어 FastAPI가 요청 시점에 위상 정렬로 풀어 가는지, `async`/`await`가 ASGI 이벤트 루프와 어떻게 결합되는지, 미들웨어가 어떻게 동심원 양파 계층으로 합성되는지를 다룹니다.
...
FastAPI의 고급 기능 — Depends, async, 미들웨어, WebSocket, lifespan — 은 데코레이터의 잡동사니처럼 보이지만, 모두 세 가지 기저 메커니즘의 사례입니다. 이 셋을 하나의 멘탈 모델로 보게 되면, 표면 API는 외워야 하는 것이 아니라 도출 가능한 것이 됩니다.

- **(A) 그래프로서의 의존성 주입** — `Depends()`는 FastAPI가 요청당 한 번 위상 정렬·해결하는 DAG를 만든다.
- **(B) 이벤트 루프 위의 async/await** — `async def` 핸들러가 ASGI 워커에서 실제로 무엇을 하는지, `def`(동기) 핸들러는 어디서 실행되는지.
- **(C) 양파로서의 미들웨어** — 요청과 응답이 중첩 계층을 거꾸로 통과한다.

### A. 의존성 주입: 함수 호출이 아니라 그래프
...
- §1 (Depends)는 §A의 DAG 해결과 yield 의미입니다.
- §2 (OAuth2 + JWT)는 현재 사용자를 반환하는 Depends 체인 — `get_token_from_header → decode_jwt → load_user` — 입니다. 순수한 §A의 적용입니다.
- §3 (파일 업로드)은 Depends를 사용해 `UploadFile`을 받는데, 이는 레슨 02 §A.3의 요청 본문 위에 얹힌 스트리밍 래퍼입니다.
- §4 (백그라운드 태스크)는 응답이 송신된 후 루프에 작업을 스케줄합니다 — §B의 이벤트 루프 위에 만들어진 것입니다.
- §5 (WebSocket)은 레슨 02 §A.3의 HTTP 요청/응답을 장기 양방향 스트림으로 대체하지만, 여전히 ASGI 위에서 동작합니다.
- §6 (커스텀 미들웨어)은 §C의 양파 계층을 손으로 작성한 것입니다.
- §7 (APIRouter)은 합성입니다: 자체 Depends와 미들웨어를 가진 여러 서브 앱을 부모 아래에 mount합니다.
- §8 (Lifespan)은 레슨 02 §A.1의 ASGI `lifespan` 이벤트를 시작/종료 자원 관리에 활용한 것입니다.
