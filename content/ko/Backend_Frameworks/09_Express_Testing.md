...
프레임워크 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. Supertest가 임시 포트에 Express 앱을 마운트하는 방법(그리고 왜 진짜 서버보다 그것이 나은지), Jest와 Mocha가 철학적으로 어떻게 다른지, 그리고 모든 모킹 결정을 설명하는 테스트 더블 분류(mock vs stub vs spy vs fake)를 다룹니다.
...
Express 애플리케이션의 테스트 스위트는 세 개의 독립된 조각으로 만들어지며, 각각은 API가 시사하는 것보다 더 깊은 메커니즘을 가집니다.

- **(A) HTTP 테스트 트랜스포트** — Supertest가 소켓 수준에서 무엇을 하는지, 그리고 네트워크를 통째로 건너뛰는 대안.
- **(B) Jest vs Mocha — 다른 철학** — Jest의 내장 배터리 vs Mocha의 à la carte 합성, 그리고 그 선택이 다른 모든 테스트 결정에 어떻게 색을 입히는지.
- **(C) 테스트 더블 분류** — mock, stub, spy, fake는 동의어가 아닙니다; 각각이 특정 설계 질문을 다룹니다.
...
- §1 (Jest와 Supertest)는 §B.1 배터리 포함 러너와 §A.1 임시 포트 HTTP 트랜스포트를 도입합니다.
- §2 (설정)은 Jest의 워커 격리(§B.1)와 레슨 08의 데이터베이스를 테스트 환경에 맞게 구성합니다.
- §3 (라우트 테스트)는 Supertest를 통한 통합 테스트입니다 — §A.1을 구체적인 코드로.
- §4 (미들웨어 테스트)는 단위 격리를 위해 모킹된 `req`/`res`로 §A.3 직접 호출 패턴을 자주 사용합니다.
- §5 (데이터베이스 호출 모킹)은 Prisma 메서드에 대한 대부분 §C.1 stub(때때로 §C.2 mock)입니다.
- §6 (테스트 데이터베이스를 사용한 통합 테스트)는 §C.3 fake 패턴입니다: 레슨 05 §C.1의 트랜잭션 롤백 격리를 가진 진짜 데이터베이스.
- §7 (인증 테스트)는 Supertest로 "로그인 → 토큰 사용"을 체이닝합니다 — 순수 §A 흐름.
- §8 (커버리지)는 §A–§C 스위트가 실제로 시험하는 코드를 측정하는 §B.1 내장 커버리지 리포트입니다.
