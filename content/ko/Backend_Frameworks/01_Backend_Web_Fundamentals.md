...
프레임워크 참조에 들어가기 전에, [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽어보세요. HTTP 와이어 프로토콜, REST의 6가지 아키텍처 제약, 그리고 프레임워크들이 특화시키는 동기 vs 비동기 서버 모델을 다룹니다.
...
백엔드 웹 프레임워크는 와이어 레벨의 **HTTP 메시지**를 우리가 쓰는 언어의 함수 호출로 번역하고, 함수의 반환값을 다시 HTTP 메시지로 번역하는 얇은 층입니다. 프레임워크를 의도적으로(복붙이 아니라) 사용하려면, 프레임워크가 사용성을 위해 한데 묶어 둔 세 가지를 분리해서 이해해야 합니다.

- **(A) 전송 프로토콜(Transport protocol)** — 소켓을 흐르는 바이트가 무엇을 의미하는가.
- **(B) 아키텍처 스타일(Architectural style)** — REST, RPC, GraphQL 중 어떤 방식으로 그 바이트들을 API 계약으로 조직하는가.
- **(C) 서버 실행 모델(Server execution model)** — 단일 프로세스가 어떻게 수천 개의 동시 연결을 처리하는가.

이 레슨 뒤에서 마주칠 모든 구체적 결정 — `200` vs `204`, 경로 버전 관리 vs 헤더 버전 관리, WSGI vs ASGI — 은 결국 이 세 축 중 하나로 환원됩니다.
...
- §1 (HTTP 사이클)은 §A의 와이어 포맷을 예제로 구체화합니다.
- §2 (REST 원칙)은 §B의 Fielding 6 제약을 API 설계 예제로 풀어 갑니다.
- §3 (WSGI vs ASGI)는 §C의 동기 모델과 이벤트 루프 모델 사이를 선택하는 Python 인터페이스입니다.
- §4 (JSON)은 §A의 모든 요청·응답에서 "본문" 영역을 채우는 본문 포맷입니다.
- §5 (API 버전 관리)는 §B.1의 `균일 인터페이스`를 기존 클라이언트를 깨지 않고 진화시키는 방법입니다.
- §6 (요청 생명주기)는 단일 HTTP 요청을 라우팅·미들웨어·직렬화를 거쳐 추적합니다 — 위의 모든 것에 대한 프레임워크의 내부 구현입니다.
