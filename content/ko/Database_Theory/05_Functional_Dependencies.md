...
1. 함수 종속성(Functional Dependency, FD)을 정의하고, 관계형 스키마 설계와 정규화의 형식적 기반으로서의 역할을 설명합니다.
2. Armstrong의 공리(반사율, 증가율, 이행률)와 파생 규칙을 적용하여 주어진 FD 집합으로부터 새로운 함수 종속성을 도출합니다.
3. 주어진 FD 집합 하에서 속성 집합의 속성 폐쇄(Attribute Closure)를 계산하여 키 포함 여부를 결정합니다.
4. 함수 종속성을 체계적으로 분석하여 릴레이션의 모든 후보 키(Candidate Key)를 식별합니다.
5. 중복을 제거하여 함수 종속성 집합의 최소 커버(Minimal Cover, 정준 커버)를 계산합니다.
6. 잘못된 스키마 설계에서 이상 현상(삽입, 삭제, 갱신 이상)이 발생하는 원인을 설명하고, FD가 이를 피하기 위한 기준을 어떻게 형식화하는지 설명합니다.
