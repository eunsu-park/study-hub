...
1. Codd의 관계형 모델(Relational Model)이 등장한 역사적 맥락을 설명하고 이전 데이터 모델의 문제를 어떻게 해결했는지 서술합니다.
2. 도메인(Domain), 튜플(Tuple), 속성(Attribute), 스키마(Schema) 등 관계의 형식적 구성 요소를 수학적 표기법으로 정의합니다.
3. 슈퍼키(Superkey), 후보키(Candidate Key), 기본키(Primary Key), 외래키(Foreign Key)를 구별하고 각각이 무결성을 어떻게 강제하는지 설명합니다.
4. 개체 무결성(Entity Integrity)과 참조 무결성(Referential Integrity) 제약조건을 적용하여 관계형 스키마를 검증합니다.
5. NULL 값의 의미를 설명하고 3치 논리(Three-Valued Logic: TRUE, FALSE, UNKNOWN)를 사용하여 표현식을 평가합니다.
6. 비형식적 데이터 요구사항을 적절한 제약조건과 표기법을 갖춘 형식적 관계형 스키마로 변환합니다.
