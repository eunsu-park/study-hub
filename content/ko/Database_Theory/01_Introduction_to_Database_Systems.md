...
1. 데이터베이스 시스템(Database System)을 정의하고, 파일 기반 데이터 관리의 한계를 설명합니다.
2. 데이터베이스 관리 시스템(DBMS)의 역할과 책임을 서술하고 핵심 구성 요소를 식별합니다.
3. 3단계 스키마(ANSI/SPARC) 아키텍처를 설명하고 물리적(Physical), 개념적(Conceptual), 외부(External) 스키마를 구분합니다.
4. 물리적 데이터 독립성(Physical Data Independence)과 논리적 데이터 독립성(Logical Data Independence)을 구별하고 그 중요성을 설명합니다.
5. 주요 데이터 모델(관계형, 계층형, 네트워크형, 객체지향형)을 식별하고 특성을 비교합니다.
6. 데이터베이스 사용자 유형을 인식하고 각각의 역할과 DBMS와의 상호작용 방식을 설명합니다.
...
> **비유 -- 건물로 생각하기**:
> - **외부 스키마(External schema)** = 세입자에게 제공되는 평면도. 각 세입자는 자신과 관련된 공간만 봅니다(레스토랑은 주방 배치도를, 사무실은 회의실을 봅니다). 서로 다른 세입자가 같은 건물의 서로 다른 뷰를 받습니다.
> - **개념 스키마(Conceptual schema)** = 건축가의 설계도(blueprint). 모든 방, 벽, 문, 복도를 하나의 통일된 설계로 기술합니다 -- 건물의 완전한 논리적 구조입니다.
> - **내부 스키마(Internal schema)** = 벽 안에 숨겨진 전기 배선, 배관, 구조용 강철. 세입자는 이러한 세부사항을 전혀 보지 않으며, 전기 기사가 케이블을 재배선해도 건축가의 설계도는 변경되지 않습니다.
>
> 재배선이 건축가에게 설계도를 다시 그리도록 강제하지 않고(물리적 독립성), 세입자를 위해 새 방을 추가해도 배선도가 변경되지 않는 것처럼(논리적 독립성), 3단계 스키마 아키텍처는 각 수준을 다른 수준의 변경으로부터 격리합니다.
...
```sql
-- 등록 담당자를 위한 뷰: 학업 컬럼(이름, 전공, GPA, 성적)만 노출.
-- 재정 컬럼(장학금, 대출)은 의도적으로 제외 —
-- 기본 테이블을 수정하지 않고 컬럼 수준 접근 제어(column-level access control)를 구현.
...
-- 재정 지원을 위한 뷰: 재정 컬럼만 노출.
-- 학업 세부정보(GPA, 성적)는 숨겨짐 — 재정 지원 부서는 이를 볼
-- 업무상 이유가 없으며, 최소 권한 원칙(principle of least privilege)을 따름.
CREATE VIEW financial_aid_view AS
SELECT s.student_id, s.name, s.financial_status,
       s.scholarship_amount, s.loan_balance
FROM students s;

-- 학생 포털을 위한 뷰: 행 수준 + 컬럼 수준 제한.
-- WHERE 절이 각 학생을 자신의 행으로 제한(행 수준 보안, row-level security),
-- SELECT 목록은 student_id와 재정 데이터 같은 민감한 필드를 생략.
...
class DBA:
    ...
    def schema_management(self):
        ...
        왜: DBA는 개념 스키마(conceptual schema)의 소유자 — 어떤 데이터가
        존재하는지에 대한 유일한 진실의 원천(single source of truth)입니다.
        중앙집중식 스키마 제어 없이는 팀들이 서로 충돌하는 테이블 정의를
        만들어 냅니다(파일에서 DBMS로 이동한 원래 동기인 중복 문제와 동일).
        """
        # CREATE TABLE, ALTER TABLE, CREATE INDEX
        # 다양한 사용자 그룹을 위한 뷰 정의 — 최소 권한 접근 강제
        # 스키마 마이그레이션 관리 — 스키마 변경을 앱 릴리즈와 조율

    def security_management(self):
        """데이터베이스에 대한 접근 제어.

        왜: DBMS는 서로 다른 신뢰 수준을 가진 많은 사용자의 데이터를 저장합니다.
        DBA는 조직의 접근 정책을 GRANT/REVOKE 문으로 변환하여, 각 역할이
        필요한 것만 볼 수 있도록(최소 권한 원칙, principle of least privilege)
        하고 모든 접근이 감사 가능하도록 합니다.
        """
        # GRANT/REVOKE 권한 — 업무 역할을 DB 권한에 매핑
        # 역할 생성 및 사용자 할당 — 그룹 기반 접근이 감사하기 쉬움
        # 접근 로그 감사 — 비인가 또는 비정상 쿼리 탐지

    def performance_tuning(self):
        """데이터베이스 성능 최적화.

        왜: 1,000행에서 잘 실행되던 쿼리가 1,000만 행에서는 허용할 수 없이
        느려질 수 있습니다. DBA는 논리적 스키마(데이터의 모습)와 물리적
        스키마(데이터의 저장 방식) 사이의 간극을 메우며, 실제 워크로드에
        맞게 인덱스를 추가하고 저장소를 조정합니다.
        """
        # 쿼리 실행 계획 분석 — 사용자가 알아채기 전에 병목 발견
        # 워크로드 기반 인덱스 생성/삭제 — 읽기 속도와 쓰기 비용 균형
        # 버퍼 풀, 캐시 크기 구성 — 자주 사용되는 데이터를 메모리에 유지
        # 대형 테이블 파티셔닝 — I/O 분산 및 빠른 스캔 가능

    def backup_and_recovery(self):
        """데이터 지속성 보장.

        왜: 하드웨어는 고장나고, 사람은 실수하고, 재해는 발생합니다.
        검증된 백업/복구 절차 없이는 디스크 장애 하나 또는 실수로 인한
        DELETE 하나가 영구적인 데이터 손실을 야기할 수 있습니다.
        """
        # 정기 백업 예약 (전체, 증분) — 데이터 손실 구간 최소화
        # 복구 절차 테스트 — 테스트되지 않은 백업은 백업이 아님
        # 트랜잭션 로그 관리 — 특정 시점 복구(point-in-time recovery) 가능
        # 재해 복구 처리 — 치명적 장애 후 서비스 복원

    def capacity_planning(self):
        """성장 계획.

        왜: 데이터베이스는 시간이 지나면서 커집니다. 선제적 계획은
        압박 속의 긴급 마이그레이션을 피합니다(예: 새벽 3시에 디스크
        공간 부족 상황).
        """
        # 디스크 사용 트렌드 모니터링 — 현재 저장소가 언제 부족해질지 예측
        # 미래 저장 요구 예측 — 비즈니스 성장 전망과 맞춤
        # 하드웨어 업그레이드 계획 — 한계에 도달하기 전에 예산 확보 및 일정 수립
```
