...
1. 관계 대수(Relational Algebra)가 SQL의 형식적 기반으로서 하는 역할을 설명하고, 쿼리 최적화기(Query Optimizer)가 대수적 표현식을 어떻게 활용하는지 서술합니다.
2. 단항 연산인 선택(Selection, σ)과 투영(Projection, π)을 적용하여 관계를 필터링하고 재구성합니다.
3. 합집합(Union), 교집합(Intersection), 차집합(Difference), 카티션 곱(Cartesian Product), 조인(Join) 변형을 사용하여 관계를 결합하고 관련 데이터를 검색합니다.
4. 다중 테이블 질의를 위한 관계 대수 표현식을 구성하고 SQL과 대수적 동치 표현 사이를 변환합니다.
5. 쿼리 트리(Query Tree)를 작성하고 단순화하며, 대수적 동치 규칙을 적용하여 쿼리 계획을 최적화합니다.
6. 관계 대수(절차적)와 관계 해석(Relational Calculus, 선언적)의 차이를 구분하고 표현력의 동치성을 설명합니다.
