...
**이전**: [파일 시스템 구현](./17_File_System_Implementation.md) | **다음**: [고급 가상 메모리](./20_Advanced_Virtual_Memory.md)
...
1. 포트, 버스, 장치 컨트롤러를 포함한 I/O 하드웨어 구성을 설명할 수 있습니다
2. 프로그램 I/O(폴링), 인터럽트 방식 I/O, DMA 전송 방식을 비교할 수 있습니다
3. 디바이스 드라이버의 역할과 계층형 I/O 서브시스템 구조를 설명할 수 있습니다
4. 파이프, 메시지 큐, 공유 메모리, 소켓을 포함한 IPC 메커니즘을 비교할 수 있습니다
5. 성능과 복잡도 관점에서 공유 메모리와 메시지 패싱(Message Passing)의 적합한 사용 시나리오를 설명할 수 있습니다
6. 익명 파이프, 명명 파이프(FIFO), POSIX 공유 메모리를 사용한 기본 IPC를 구현할 수 있습니다
7. TCP 소켓을 사용한 간단한 클라이언트-서버 통신을 설계할 수 있습니다

---

컴퓨터는 I/O 없이는 쓸모가 없습니다 -- 키보드, 디스플레이, 디스크, 네트워크는 컴퓨터가 세상과 상호작용하는 방법입니다. 프로세스 간 통신(IPC)은 이 개념을 프로세스들끼리 서로 통신하는 것으로 확장합니다. I/O와 IPC는 함께 운영체제의 신경계를 이루며, 격리된 프로세스를 하드웨어와 서로에게 연결합니다. 이 레슨은 프로세스, 메모리, 파일 시스템에 대해 배운 모든 것을 하나로 엮어, OS가 실행 중인 시스템을 어떻게 관리하는지의 완전한 그림을 완성합니다.
...
int main() {
    ...
    if (pid == 0) {
        // 자식 프로세스: 파이프에서 읽기
        // Why: 읽기 프로세스에서 사용하지 않는 쓰기 끝(write end)을 닫습니다.
        // 파이프는 쓰기 끝(write end)이 하나라도 열려 있는 한 열린 상태를 유지합니다.
        // 자식이 fd[1]을 열어 두면, 부모가 쓰기 끝을 닫은 후에도 read()가 EOF를
        // 영원히 기다리며 블로킹(blocking)됩니다 -- 파이프에 아직 열린 쓰기 끝
        // (자식 자신의 것)이 남아있기 때문입니다.
        ...
        // 부모 프로세스: 파이프에 쓰기
        // Why: 쓰기 프로세스에서 사용하지 않는 읽기 끝(read end)을 닫습니다.
        // 부모가 fd[0]을 열어 두고 자식이 종료하면, 읽는 프로세스가 없는 파이프에
        // 쓰기를 할 때 일반적으로 SIGPIPE가 발생합니다. 하지만 부모가 읽기 끝을
        // 보유하고 있으면 커널은 아직 읽는 프로세스가 있다고 판단하여, 오류 신호
        // 대신 write()가 블로킹됩니다. 사용하지 않는 끝은 항상 닫아야 합니다.
...
> **왜 fork+exec 패턴을 파이프와 함께 사용하는가?** `fork()`는 부모의 모든 열린 파일 디스크립터(file descriptor)를 상속받는 자식 프로세스를 생성합니다 -- 파이프의 양쪽 끝도 포함됩니다. `exec()`는 자식의 프로그램 이미지를 교체하지만 열린 파일 디스크립터는 보존합니다. 이 두 단계 패턴이 바로 셸 파이핑(`cmd1 | cmd2`)이 작동하는 방식입니다: 셸이 파이프를 생성하고, 두 번 fork한 뒤, stdout/stdin을 파이프 끝으로 리다이렉트하고, 각 명령을 exec합니다. 새 프로그램들은 파이프로 연결된 것을 전혀 모릅니다 -- 그냥 평소처럼 stdin에서 읽고 stdout에 쓸 뿐입니다.
...
> **왜 동기화가 필수적인가**: 공유 메모리(Shared Memory)는 데이터를 복사하지 않기 때문에 가장 빠른 IPC 메커니즘입니다 -- 두 프로세스가 커널 개입 없이 동일한 물리 메모리 영역을 직접 읽고 씁니다. 그러나 이 속도에는 대가가 따릅니다: 두 프로세스가 같은 메모리 위치에 동시에 쓸 수 있으므로, 명시적 동기화(explicit synchronization) 없이는 데이터 경합(data race)이 발생합니다. 파이프(Pipe)나 메시지 큐(Message Queue)처럼 커널이 접근을 직렬화해주는 것과 달리, 공유 메모리에서는 뮤텍스(mutex), 세마포어(semaphore) 등의 동기화 원시타입(synchronization primitive)을 사용하는 것이 전적으로 프로그래머의 책임입니다.
