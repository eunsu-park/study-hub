...
**이전**: [세그멘테이션](./13_Segmentation.md) | **다음**: [페이지 교체](./15_Page_Replacement.md)
...
1. 가상 메모리(virtual memory)의 개념과 다중 프로그래밍(multiprogramming) 및 메모리 격리(memory isolation)에 대한 이점을 설명할 수 있습니다
2. 요구 페이징(demand paging)과 이것이 필요할 때만 페이지를 로드하여 메모리 사용량을 줄이는 방식을 설명할 수 있습니다
3. 트랩 발생부터 명령어 재실행까지 페이지 폴트(page fault) 처리 순서를 단계별로 추적할 수 있습니다
4. 페이지 폴트 발생률과 처리 시간을 이용하여 유효 메모리 접근 시간(effective memory access time)을 계산할 수 있습니다
5. 페이지 테이블 항목에서 유효/무효 비트(valid/invalid bit)와 더티 비트(dirty bit)가 작동하는 방식을 설명할 수 있습니다
6. 순수 요구 페이징(pure demand paging)과 프리페이징(prepaging)을 구분하고 각각의 적합한 상황을 설명할 수 있습니다
7. Copy-on-Write(COW) 의미론을 구현하고 이것이 fork()를 어떻게 가속화하는지 설명할 수 있습니다
8. 메모리 매핑 파일(memory-mapped files, mmap)과 기존 파일 I/O 대비 장점을 설명할 수 있습니다

---

가상 메모리(Virtual Memory)는 운영체제 설계에서 가장 혁신적인 아이디어 중 하나입니다. 이는 프로세스가 물리적으로 존재하는 것보다 더 많은 메모리를 사용할 수 있게 하고, 프로세스 간 메모리 격리를 가능하게 하며, 공유를 효율적으로 만듭니다. 모든 현대 운영체제가 이에 의존하며, 이를 이해하면 컴퓨터가 RAM이 부족하지 않고도 수십 개의 애플리케이션을 동시에 실행할 수 있는 이유를 알 수 있습니다.
...
> 가상 메모리는 호텔의 객실 예약 시스템과 같습니다. 호텔(물리 메모리)에는 100개의 객실이 있지만 200개의 예약(프로세스)을 받았습니다. 이것이 가능한 이유는 모든 투숙객이 동시에 객실에 있는 것이 아니기 때문입니다. 일부는 수영장에, 일부는 관광을 나가 있습니다. 호텔은 실제로 투숙 중인 투숙객을 위한 객실만 있으면 됩니다. 마찬가지로 가상 메모리는 운영체제가 물리적으로 존재하는 것보다 더 많은 메모리를 약속할 수 있게 하며, 실제로 필요할 때만 디스크에서 페이지를 로드합니다.
...
[페이지 교체](./15_Page_Replacement.md)에서 페이지 교체 알고리즘을 배워봅시다!
