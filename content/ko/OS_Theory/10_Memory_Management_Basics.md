...
**이전**: [데드락](./09_Deadlock.md) | **다음**: [연속 메모리 할당](./11_Contiguous_Memory_Allocation.md)
...
1. 논리 주소(logical address)와 물리 주소(physical address)를 구별할 수 있습니다
2. 컴파일 시간, 로드 시간, 실행 시간의 주소 바인딩(address binding)을 설명할 수 있습니다
3. 주소 변환에서 MMU(Memory Management Unit)의 역할을 설명할 수 있습니다
4. 스와핑(swapping)과 그 성능적 함의를 설명할 수 있습니다
5. 서로 다른 메모리 할당 전략을 비교할 수 있습니다

---

프로그램의 모든 변수, 모든 함수, 모든 자료구조는 메모리 어딘가에 존재합니다. 운영체제가 그 위치를 결정하며 — 이 결정은 성능, 보안, 그리고 프로그램이 실행될 수 있는지 여부에 영향을 미칩니다. 메모리 관리는 프로그램의 추상적인 메모리 관점과 메인보드의 물리적 RAM 칩 사이를 연결하는 다리입니다.
...
[연속 메모리 할당](./11_Contiguous_Memory_Allocation.md)에서 메모리 분할과 할당 전략을 배워봅시다!
