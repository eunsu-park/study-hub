...
**이전**: [동기화 도구](./08_Synchronization_Tools.md) | **다음**: [메모리 관리 기초](./10_Memory_Management_Basics.md)
...
1. 데드락의 네 가지 코프만 조건(Coffman conditions)을 나열하고 설명할 수 있습니다
2. 데드락 예방(prevention)과 회피(avoidance), 탐지(detection)의 차이를 구별할 수 있습니다
3. 은행원 알고리즘(Banker's algorithm)을 적용하여 안전 상태와 불안전 상태를 판별할 수 있습니다
4. 자원 할당 그래프(resource allocation graph)를 구성하고 분석할 수 있습니다
5. 다양한 데드락 처리 전략의 트레이드오프(tradeoff)를 평가할 수 있습니다

---

데드락(Deadlock)은 조용한 시스템 킬러입니다 — 시스템이 멈추고, 오류 메시지도 없으며, 어떤 단일 프로세스도 "잘못"되지 않습니다. 프로세스들이 원형 사슬 구조로 자원을 보유한 채 서로를 기다릴 때 발생합니다. 데드락을 이해하는 것은 조용히 멈춰버리지 않는 시스템을 설계하는 데 필수적입니다.
...
> 네 대의 차가 동시에 사거리에 도착하여, 각 차가 오른쪽 차가 먼저 가기를 기다리는 상황을 상상해 보세요. 아무도 영원히 움직이지 않습니다. 이것이 데드락입니다: 각 프로세스는 다른 프로세스가 필요로 하는 자원을 보유하고, 모두가 스스로는 절대 풀릴 수 없는 순환 대기 상태에 놓입니다.
...
`examples/OS_Theory/09_deadlock_detection.py`를 실행하고 확장하세요.

**과제:**
1. `WaitForGraph` 클래스를 수정하여 처음 발견된 사이클뿐만 아니라 그래프의 모든 사이클(Cycle)을 반환하도록 하세요
2. 어떤 사이클에든 참여하는 프로세스의 집합을 반환하는 `involved_processes()` 메서드를 추가하세요
3. 두 개의 독립적인 사이클이 포함된 그래프로 테스트하세요: {P1→P2→P1}과 {P3→P4→P5→P3}

### 실습 2: 은행원 알고리즘 확장

`examples/OS_Theory/09_deadlock_detection.py`의 `BankersAlgorithm` 클래스를 사용하세요:

**과제:**
1. 프로세스가 완료될 때 자원을 해제하는 `release(pid, resources)` 메서드를 추가하세요
2. 전체 실행 시퀀스를 시뮬레이션하세요: 프로세스가 자원을 요청하고, 작업을 수행하고, 자원을 해제합니다. 각 단계에서 시스템 상태를 추적하세요
3. 초기 상태에서 시스템을 안전하게 유지하면서 단일 프로세스가 요청할 수 있는 최대 자원 수를 찾으세요

### 실습 3: 교착 상태 없는 자원 관리자

자원 순서화(Resource Ordering)를 사용하여 교착 상태(Deadlock)를 방지하는 완전한 자원 관리자를 구축하세요:

**과제:**
1. 모든 자원에 전역 순서를 할당하는 `ResourceManager` 클래스를 만드세요
2. 자원이 오름차순으로 요청되는지 확인하는 `request(process, resource_list)`를 구현하세요 (아닌 경우 거부)
3. 5개 프로세스와 5개 자원으로 테스트하세요. 1000번의 무작위 할당/해제 사이클을 실행하고 교착 상태가 발생하지 않는지 확인하세요
4. 은행원 알고리즘을 사용한 회피 버전과 성능(처리량)을 비교하세요

---

## 연습 문제

### 연습 1: 코프만 조건(Coffman Conditions) 분석

각 시스템 설명에서 네 가지 코프만 조건(상호 배제(mutual exclusion), 점유 대기(hold-and-wait), 비선점(no-preemption), 순환 대기(circular wait))이 존재하는지 확인하고, 교착 상태(deadlock)가 가능한지 판단하세요.

**시스템 A**: 트랜잭션이 행을 독점적으로 잠그고(공유 읽기 없음), 커밋할 때까지 모든 잠금을 유지해야 하며, 잠금을 강제로 빼앗길 수 없는 데이터베이스. 두 트랜잭션이 각각 상대방이 잠근 행을 기다리고 있습니다.

**시스템 B**: 모든 작업이 선점될 수 있고 인쇄 작업이 취소되어 나중에 처음부터 다시 시작할 수 있는 프린트 스풀러(print spooler).

**시스템 C**: 각 철학자가 포크를 동시에 둘 다 집거나 기다리는 (하나를 들고 다른 것을 기다리지 않는) 철학자 문제 변형.

각 시스템에 대해:
1. 각 코프만 조건을 확인하세요 (존재 / 없음 / 부분적)
2. 교착 상태가 가능한지 결론 짓고 이유를 설명하세요

### 연습 2: 자원 할당 그래프(Resource Allocation Graph) 분석

다음 자원 할당 그래프로 질문에 답하세요. 표기법: P→R은 프로세스 P가 자원 R을 요청함을 의미; R→P는 자원 R이 프로세스 P에 할당됨을 의미.
...
각 자원은 정확히 하나의 인스턴스를 가집니다.

1. 자원 할당 그래프를 그리세요
2. 그래프에서 사이클(cycle)을 식별하세요
3. 교착 상태가 있나요? 어떤 프로세스가 교착 상태인가요?
4. P5가 도착하여 R3를 요청합니다. R3가 P4에 의해 해제될 경우 P5에게 부여된다면 교착 상태가 해결되나요?

### 연습 3: 은행원 알고리즘(Banker's Algorithm)

시스템에 3가지 자원 유형이 있습니다: A (10개), B (5개), C (7개).

**현재 상태:**

| 프로세스 | 할당 (A,B,C) | 최대 필요 (A,B,C) |
|----------|-------------|------------------|
| P0 | 0, 1, 0 | 7, 5, 3 |
| P1 | 2, 0, 0 | 3, 2, 2 |
| P2 | 3, 0, 2 | 9, 0, 2 |
...
1. 가용(Available) 벡터를 계산하세요
2. 각 프로세스의 필요(Need) 행렬을 계산하세요
3. 은행원 알고리즘을 사용하여 안전 순서(safe sequence)를 찾으세요. 각 단계에서 가용 벡터를 보여주세요
4. P1이 (1, 0, 2)를 요청합니다. 시스템이 이 요청을 허락할 수 있나요? 은행원 알고리즘을 실행하고 결정을 설명하세요

### 연습 4: 예방 전략 트레이드오프

네 가지 교착 상태 예방(prevention) 전략 각각에 대해 실용적인 실제 시스템이나 상황 하나와 비실용적인 상황 하나(이유 포함)를 설명하세요.

| 전략 | 실용적인 실제 상황 | 비실용적인 실제 상황 |
|------|-------------------|---------------------|
| 상호 배제 거부 | | |
| 점유 대기 거부 | | |
| 선점 허용 | | |
...
### 연습 5: 탐지(Detection)와 복구(Recovery)

시스템은 교착 상태를 예방하거나 회피하지 않고, 대신 5분마다 교착 상태 탐지기를 실행합니다.

**t=60분의 탐지 결과**: 프로세스 P2, P5, P7, P8이 교착 상태이며, 각자 공유 자원 R1, R2, R3, R4 중 하나를 보유하고 있습니다 (모두 단일 인스턴스).

1. 교착 상태를 해제하기 위해 최소 몇 개의 프로세스를 종료해야 하나요? 어떤 것들인가요? (모두 동등한 우선순위라고 가정)
2. 종료 대신 OS가 자원을 선점(preempt)한다면, 낭비를 최소화하기 위해 어떤 순서로 선점해야 하나요? 어떤 기준을 사용하겠나요?
3. 복구 후 P2와 P5가 재시작하고 즉시 다시 교착 상태에 빠집니다. 이것이 근본적인 설계에 대해 무엇을 시사하며, 어떻게 해야 하나요?
4. 시스템이 T분마다 탐지기를 실행합니다. 작은 T와 큰 T를 선택하는 트레이드오프는 무엇인가요?
...
- [메모리 관리 기초](./10_Memory_Management_Basics.md) - 메모리 관리 (예정)
