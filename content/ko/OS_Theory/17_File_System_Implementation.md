...
**이전**: [파일 시스템 기초](./16_File_System_Basics.md) | **다음**: [I/O와 IPC](./18_IO_and_IPC.md)
...
1. 연속, 연결, 인덱스 파일 할당 방식의 장단점을 비교할 수 있습니다
2. FAT(File Allocation Table) 구조와 대용량 디스크에서의 한계를 설명할 수 있습니다
3. Unix/Linux에서 사용하는 inode 기반 할당 방식을 설명하고 최대 파일 크기를 계산할 수 있습니다
4. 저널링(Journaling)이 크래시 후 파일 시스템 손상을 방지하는 방법을 설명할 수 있습니다
5. ext4의 쓰기 선행 로깅 모드(journal, ordered, writeback)를 구분할 수 있습니다
6. RAID 레벨(0, 1, 5, 6, 10)을 용량, 성능, 내결함성 관점에서 비교할 수 있습니다
7. 디렉토리 엔트리 제거부터 블록 해제까지 파일 삭제의 전체 과정을 추적할 수 있습니다

---

파일을 저장할 때 OS는 디스크의 어디에 파일을 배치할지, 블록을 어떻게 추적할지, 쓰기 도중 전원이 나가면 어떻게 복구할지를 결정해야 합니다. 파일 시스템 구현은 이 질문들에 답합니다 -- 그리고 이 선택들은 속도, 신뢰성, 최대 파일 크기에 직접적인 영향을 미칩니다. USB 드라이브에서 사용하는 단순한 FAT부터 Linux 서버를 구동하는 저널링 ext4까지, 이 레슨에서 다루는 기술들은 여러분이 사용하는 모든 저장 장치에서 동작하고 있습니다.
...
[I/O와 IPC](./18_IO_and_IPC.md)에서 I/O 시스템과 프로세스 간 통신을 배워봅시다!
