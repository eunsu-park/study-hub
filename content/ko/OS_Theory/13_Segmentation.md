...
**이전**: [페이징](./12_Paging.md) | **다음**: [가상 메모리](./14_Virtual_Memory.md)
...
1. 세그멘테이션(Segmentation)을 프로그래머가 프로그램을 바라보는 방식을 반영한 논리적 메모리 관점으로 설명할 수 있습니다
2. 기준(base), 한계(limit), 보호(protection) 필드를 포함하는 세그먼트 테이블 항목의 구조를 설명할 수 있습니다
3. 세그먼트 번호와 오프셋을 이용하여 세그먼트 기반 주소 변환을 직접 수행할 수 있습니다
4. 단편화(fragmentation), 보호(protection), 공유(sharing) 등 여러 차원에서 세그멘테이션과 페이징을 비교할 수 있습니다
5. 세그먼트 페이징(segmented paging)을 설명하고, 현대 시스템이 두 기법을 결합하는 이유를 설명할 수 있습니다
6. Intel x86 아키텍처가 보호 모드(protected mode)에서 세그멘테이션을 구현하는 방식을 분석할 수 있습니다
7. 세그멘테이션의 외부 단편화(external fragmentation)와 페이징의 내부 단편화(internal fragmentation)를 구분할 수 있습니다

---

페이징(Paging)이 운영체제의 편의를 위해 메모리를 고정 크기 블록으로 나누는 반면, 세그멘테이션은 프로그램의 논리적 구조인 코드, 데이터, 스택, 힙에 따라 메모리를 나눕니다. 세그멘테이션은 프로그래머가 실제로 메모리를 생각하는 방식을 반영하며, 이는 페이징의 자연스러운 보완책이 됩니다. 현대 x86 같은 시스템이 두 기법을 모두 결합하고 있고, 세그먼트 수준의 보호(segment-level protection) 개념이 오늘날 모든 운영체제의 메모리 안전성의 근간을 이루므로, 세그멘테이션을 이해하는 것은 필수적입니다.

> **비유**: 세그멘테이션(Segmentation)은 프로그래머의 정신 모델을 그대로 반영합니다: 코드, 데이터, 스택, 힙은 각각 고유한 기준 주소(base address)와 크기 제한(size limit)을 가진 별도의 논리 단위입니다. 이것은 서류 캐비닛의 서랍에 비유할 수 있습니다 -- 각 서랍(세그먼트)은 계약서, 청구서, 서신 등 다른 종류의 문서를 보관합니다. 서랍마다 크기가 다를 수 있고, 독립적으로 잠글 수 있으며, 특정 사람에게만 공유할 수 있습니다. 반면 페이징(Paging)은 내용과 무관하게 모든 문서를 같은 크기의 페이지로 자르는 것과 같습니다.
...
int translate_segment_address(uint16_t segment, uint32_t offset,
    ...
    // 3. 한계 검사
    // Why: 하드웨어는 오프셋(offset)을 기준 주소(base)에 더하기 전에 한계(limit)를
    // 먼저 검사합니다. 이렇게 하면 버그가 있는 프로그램이 세그먼트 경계를 넘어 접근하려
    // 할 때, 인접 세그먼트의 메모리를 조용히 손상시키는 대신 하드웨어가 즉시 트랩을
    // 발생시킵니다. 즉, 버퍼 오버플로(buffer overflow)가 인접 세그먼트로 침범하는
    // 것을 방지합니다.
    ...
    // 5. 물리 주소 계산
    // Why: 세그먼트 테이블 항목의 기준+한계(base+limit) 쌍은 하나의 메커니즘으로
    // 재배치(relocation)와 보호(protection)를 동시에 제공합니다. 기준 주소(base)는
    // 세그먼트를 물리 메모리 어디든 재배치할 수 있게 해주고(연속된 논리-물리 매핑이
    // 필요 없음), 한계(limit)는 모든 접근이 범위 내에 머무르도록 보장합니다.
    // 이것이 세그멘테이션이 세그먼트별 보호를 "무료로" 제공하는 이유입니다 --
    // 주소를 변환하는 동일한 하드웨어가 경계도 함께 강제합니다.
...
[가상 메모리](./14_Virtual_Memory.md)에서 요구 페이징과 가상 메모리 시스템을 배워봅시다!
