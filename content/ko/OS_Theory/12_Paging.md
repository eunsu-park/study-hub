...
**이전**: [연속 메모리 할당](./11_Contiguous_Memory_Allocation.md) | **다음**: [세그멘테이션](./13_Segmentation.md)
...
1. 페이징(Paging)이 외부 단편화(External Fragmentation)를 제거하는 방법을 설명할 수 있다
2. 페이지에서 프레임으로의 주소 변환(Address Translation)을 수동으로 수행할 수 있다
3. 페이지 테이블 엔트리(Page Table Entry)의 구조와 목적을 설명할 수 있다
4. TLB(Translation Lookaside Buffer)가 페이징 성능에 왜 중요한지 설명할 수 있다
5. 단일 수준, 다단계, 해시, 역 페이지 테이블을 비교할 수 있다
6. 페이지 테이블의 메모리 오버헤드를 계산할 수 있다

---

페이징(Paging)은 운영체제에서 가장 중요한 개념 중 하나입니다. 페이징을 통해 OS는 각 프로세스에게 크고 연속된 주소 공간이 있다는 환상을 제공하면서, 실제로는 프로세스를 물리 메모리 전체에 분산시킵니다. 논리 주소와 물리 주소의 이러한 분리는 메모리 보호, 공유, 그리고 가상 메모리를 가능하게 합니다.
...
> 번호가 붙은 칸(프레임)이 있는 책장(물리 메모리)과 번호가 붙은 페이지가 있는 교과서(프로세스)를 생각해보세요. 페이지는 연속된 칸에 놓일 필요가 없습니다. 1페이지는 5번 칸에, 2페이지는 12번 칸에 들어갈 수 있습니다. 페이지 테이블(Page Table)은 각 페이지 번호를 해당 칸 위치에 매핑하는 색인 카드입니다. 이 단순한 아이디어, 즉 조회 테이블(Lookup Table)을 이용한 비연속 할당(Non-contiguous Allocation)은 외부 단편화(External Fragmentation)를 완전히 제거합니다.
...
[세그멘테이션](./13_Segmentation.md)에서 세그멘트 기반 메모리 관리를 배워봅시다!
