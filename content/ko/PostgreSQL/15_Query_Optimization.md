...
EXPLAIN으로 들어가기 전, [**이론과 원리**](#이론과-원리) 절을 먼저 읽으세요 — planner의 cost model(`seq_page_cost`, `random_page_cost`, `cpu_*`), `pg_statistic`의 selectivity 추정이 어떻게 들어가는지, join-order 탐색 문제와 dynamic programming, 그리고 fallback인 Genetic Query Optimizer(GEQO)를 다룹니다.
...
PostgreSQL의 옵티마이저는 *cost-based* planner입니다. 모든 쿼리에 대해 여러 plan(다른 scan 방법, 다른 join 순서, 다른 join 알고리즘)을 고려하고, 작은 tunable 상수 집합과 테이블 통계를 바탕으로 각각에 수치 **cost**를 할당하고, 가장 낮은 cost의 plan을 선택합니다. `EXPLAIN`을 읽는 것은 그 cost가 무엇을 뜻하고 어디서 오는지 이해할 때만 의미가 있습니다. 이 절은 모든 이후 최적화 팁이 발 딛고 서는 기초 — 이것 없이는 "인덱스를 추가했더니 느려졌다"가 신비롭고, 이것이 있으면 cost 산수에서 답이 떨어집니다.

이 절에서 다루는 내용:

- **(A)** Cost model — 상수(`seq_page_cost`, `random_page_cost`, `cpu_tuple_cost` 등)와 공식.
- **(B)** `pg_statistic`의 통계 — histogram, MCV(most common value), correlation, selectivity 추정.
- **(C)** Plan tree와 `EXPLAIN` 출력 — 노드, cost, 행 수, actual vs estimated.
- **(D)** Join-order 탐색 — dynamic programming, 탐색 공간 폭발, Genetic Query Optimizer(GEQO).
...
- **`EXPLAIN`, `EXPLAIN ANALYZE`, `EXPLAIN (BUFFERS)`** — plan tree 읽기 (§C).
- **`SET random_page_cost = 1.1`** — SSD 스토리지를 위한 튜닝 (§A.1).
- **`ANALYZE table_name`** — `pg_statistic` 새로 고침 (§B.1).
- **`CREATE STATISTICS s ON col1, col2 FROM t`** — correlated 컬럼을 위한 extended statistics (§B.3).
- **인덱스 전략** — 9번 레슨의 컬럼 선택, partial, expression, covering 인덱스 (§A.3).
- **`SET enable_seqscan = off`** — planner를 어떤 plan에서 강제로 떼어내는 디버깅 도구, 운영에서는 절대 사용 안 함.
- **`SET geqo_threshold = 20`** — 다중-way join이 있고 DP planning 시간이 허용 가능하면 GEQO cutoff 올림 (§D.3).
