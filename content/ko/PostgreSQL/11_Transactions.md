...
디스크의 모든 tuple은 header에 두 개의 트랜잭션 ID를 들고 다님(03번 레슨 §B.1):

- **`xmin`** — 이 row version을 *생성*한 트랜잭션. `INSERT` 또는 `UPDATE`의 `INSERT 절반`이 설정.
- **`xmax`** — 이 row version을 *삭제 또는 대체*한 트랜잭션. `DELETE` 또는 `UPDATE`의 `delete 절반`이 설정. 살아 있으면 0.

트랜잭션이 commit되면, PostgreSQL은 **clog**(`pg_xact`)에 그 `xid`를 committed로 표시하는 1비트를 씁니다. "xid 12345가 committed인가?"를 묻는 다른 모든 트랜잭션은 그 1비트를 읽습니다.

#### A.2 Snapshot

트랜잭션이 시작될 때(또는 isolation level에 따라 첫 SQL statement가 실행될 때), PostgreSQL은 **snapshot**을 캡처 — `(xmin_horizon, xmax_horizon, in_progress[])` 삼중자.

- `xmin_horizon` — *현재 실행 중인* 트랜잭션의 가장 작은 `xid`. 이보다 오래된 것은 확정적으로 committed(또는 확정적으로 abort).
- `xmax_horizon` — 다음에 할당될 `xid`. 이 이상은 미래이고 invisible.
- `in_progress[]` — `xmin_horizon`과 `xmax_horizon` 사이에서 실행 중이지만 아직 commit되지 않은 `xid`의 명시적 목록.

#### A.3 Visibility 규칙

row version은 다음 두 조건이 모두 성립할 때만 snapshot에 visible합니다.

1. **`xmin`이 committed이고 이 snapshot에 visible** —
   - `xmin < xmax_horizon` AND
   - `xmin`이 `in_progress[]`에 없음 AND
   - clog에서 `xmin`이 committed로 보임 (또는 snapshot 자신의 `xid`)
2. **`xmax`가 0이거나, in_progress이거나, abort되었거나, 미래** —
   - `xmax = 0` OR
   - clog에서 `xmax`가 abort로 보임 OR
   - `xmax`가 `in_progress[]`에 있음 (그리고 snapshot 자신의 `xid`가 아님) OR
   - `xmax >= xmax_horizon`
