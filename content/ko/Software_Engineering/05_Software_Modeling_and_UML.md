...
1. 소프트웨어 모델이 왜 가치 있으며 어느 수준의 세부 사항으로 그려야 하는지 설명한다
2. UML 2.x에서 구조 다이어그램과 행동 다이어그램을 구별한다
3. 액터, 유스케이스, 관계를 포함한 유스케이스 다이어그램을 그리고 해석한다
4. 클래스, 속성, 오퍼레이션, 다섯 가지 관계 유형을 보여주는 클래스 다이어그램을 그린다
5. 생명선, 메시지, 활성화 박스, 복합 단편을 포함한 시퀀스 다이어그램을 그린다
6. 결정 노드, 포크/조인, 수영 레인을 포함한 액티비티 다이어그램을 그린다
7. 상태, 전이, 가드, 진입/진출 액션을 포함한 상태 머신 다이어그램을 그린다
8. 컴포넌트 및 배포 다이어그램의 목적을 설명한다
9. 모델링 모범 사례를 적용하고 일반적인 실수를 피한다
...
### 연습 1: 유스케이스 다이어그램 읽기 및 검토

ATM 시스템의 부분적인 유스케이스 다이어그램 설명을 살펴보세요:

- 액터(Actor): 고객(Customer), 은행 네트워크(Bank Network)
- 유스케이스(Use Case): 카드 삽입, PIN 입력, 현금 인출, 잔액 조회, 현금 입금, PIN 변경
- 관계(Relationship): 고객은 모든 유스케이스와 연관되어 있으며; 현금 인출, 잔액 조회, 현금 입금, PIN 변경에서 PIN 입력을 `<<include>>` 함

(a) ASCII 아트 또는 정확한 설명으로 이 다이어그램을 그리세요.
(b) 설계에서 최소 세 가지 실수 또는 개선 사항을 식별하세요 (3.5절 일반적인 실수 참조).
(c) 전제 조건, 주요 성공 시나리오, 두 가지 대안 흐름을 포함하는 "현금 인출" 유스케이스 산문 설명을 작성하세요.

### 연습 2: 클래스 다이어그램 설계

최소한 다음 개념을 포함하는 **병원 환자 관리 시스템**을 모델링하세요: 환자(Patient), 의사(Doctor), 간호사(Nurse), 예약(Appointment), 의무기록(MedicalRecord), 부서(Department), 처방전(Prescription). 다음을 포함하세요:

- 4.2절의 5가지 관계 유형 전부 (최소 세 가지 사용)
- 모든 연관 관계에 올바른 다중성(Multiplicity) 표시
- 최소 하나의 추상 클래스 또는 인터페이스
- 클래스당 최소 다섯 개의 속성과 두 개의 연산(Operation)

조합(Composition) 대 집합(Aggregation) 선택에 대해 각각 한 문장으로 근거를 제시하세요.

### 연습 3: 시퀀스 다이어그램 작성

연습 2의 병원 시스템을 사용하여 "환자가 의사와 예약을 잡는" 시나리오에 대한 시퀀스 다이어그램을 그리세요. 다음을 포함하세요:

- 최소 다섯 개의 생명선(Lifeline) (예: 환자, 웹사이트, 예약서비스, 의사일정, 알림서비스)
- 하나의 `alt` 복합 단편(Combined Fragment) (의사가 가능한 경우 vs. 예약이 꽉 찬 경우)
- 하나의 `opt` 복합 단편 (환자에게 이메일이 등록된 경우 확인 이메일 전송)
- 모든 동기 호출에 대한 반환 메시지

### 연습 4: 활동 다이어그램으로 병행 워크플로 모델링

병원에서 환자를 다음 절차로 퇴원시킵니다: 의사가 퇴원을 승인하고, 간호사는 퇴원 요약을 준비하고 마지막 활력 징후를 측정하며 (이 두 가지는 병행 가능), 약국이 복용 의약품을 준비하고, 청구 부서가 청구서를 발행하며 (약국과 병행), 마지막으로 환자가 퇴원 서류에 서명하고 떠납니다.

다음을 포함하는 활동 다이어그램을 그리세요:
- 의사(Doctor), 간호사(Nurse), 약국(Pharmacy), 청구(Billing)에 대한 수영 레인(Swimlane)
- 병행 활동을 위한 분기(Fork)/합류(Join) 쌍
- 결정 노드 (환자가 후속 예약이 필요한 경우 vs. 그렇지 않은 경우)

### 연습 5: 실제 객체에 대한 상태 기계 다이어그램

다음 중 하나를 선택하세요: (a) 신호등 컨트롤러, (b) 문서 워크플로 (초안 → 검토 → 승인 → 게시), (c) TCP 연결 생명주기.

다음을 포함하는 완전한 상태 기계 다이어그램을 그리세요:
- 의미 있는 이름을 가진 모든 상태
- 적절한 경우 진입/종료 액션(Entry/Exit Action)
- 필요한 경우 전이(Transition)에 가드(Guard) 조건
- 초기 및 최종 의사 상태(Pseudostate)
- 해당하는 경우 최소 하나의 복합 상태(Composite State, 계층적 상태)

가드가 생략될 경우 비결정론적(Non-deterministic) 동작을 일으킬 수 있는 전이 하나를 설명하세요.
