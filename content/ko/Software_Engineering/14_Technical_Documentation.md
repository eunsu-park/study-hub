...
1. 기술 문서의 각 유형과 해당 대상 독자를 식별한다
2. 소스 코드와 함께 문서를 관리하는 코드로서의 문서(docs-as-code) 워크플로를 적용한다
3. 표준 형식으로 아키텍처 결정 기록(Architecture Decision Record, ADR)을 작성한다
4. OpenAPI 명세와 이것이 API 문서화를 어떻게 가능하게 하는지 설명한다
5. 소프트웨어 프로젝트를 위한 README 파일을 효과적으로 구성한다
6. C4 모델을 적용하여 계층적 아키텍처 다이어그램을 작성한다
7. 문서 부패(documentation rot)를 진단하고 해결한다
8. 런북(runbook)과 플레이북(playbook)의 차이를 구분하고 각각의 사용 시점을 파악한다
