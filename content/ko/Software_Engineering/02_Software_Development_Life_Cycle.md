...
1. 소프트웨어 개발 생명주기의 표준 단계를 설명한다
2. 폭포수(Waterfall), V-모델(V-Model), 점진적(Incremental), 나선형(Spiral), RAD, 프로토타이핑(Prototyping) 모델을 설명한다
3. 각 모델의 강점, 약점, 적합한 사용 사례를 식별한다
4. 주어진 프로젝트에 적합한 모델을 선택하기 위한 의사결정 프레임워크를 적용한다
5. 프로세스 모델이 리스크, 요구사항 안정성, 팀 규모와 어떻게 관련되는지 이해한다
