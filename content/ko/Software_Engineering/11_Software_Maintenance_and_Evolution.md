...
1. 네 가지 유형의 소프트웨어 유지보수를 구분하고 각각의 적용 시기 파악
2. 레만의 법칙(Lehman's Laws)과 장기 시스템에 대한 시사점 설명
3. 레거시 시스템 현대화 전략 평가
4. 동작을 변경하지 않고 코드를 개선하는 리팩토링 기법 적용
5. 역공학, 소프트웨어 노후화, 마이그레이션 전략 이해
6. API 및 서비스에 적합한 폐기(Deprecation) 정책 선택
7. 핵심 지표를 사용한 유지보수 건전성 측정
