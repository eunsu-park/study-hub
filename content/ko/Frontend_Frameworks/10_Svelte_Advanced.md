...
이게 전부입니다. 베이스 클래스도, 상속도, 라이브러리 import도 필요 없습니다. `subscribe`를 구현한 어떤 것이든 *Svelte 스토어*입니다. 프레임워크는 그것을 반응형 값 원천으로 취급합니다.

계약에는 세 가지 암묵적 보장이 있습니다.

1. **`subscribe`는 즉시, 동기적으로 현재 값으로 `listener`를 한 번 호출합니다.** 소비자가 미래의 갱신만이 아니라 시작값을 항상 가지도록 하기 위함.
2. **`subscribe`는 `unsubscribe` 함수를 반환합니다** — 호출되면 연결을 끊습니다. 스토어는 `unsubscribe` 반환 후에는 `listener`를 호출해서는 안 됩니다.
3. **리스너는 변경마다 한 번씩 새 값으로 호출됩니다.** 계약 수준의 배칭 보장은 없습니다(`writable`과 `derived`는 `derived` 정합성을 위해 내부적으로 배칭합니다).
...
// writable: 외부 코드가 set/update로 갱신 가능
const count = writable(0);
count.set(5);
count.update(n => n + 1);

// readable: 생성자만 보는 setter로 만들어짐
// (스스로 갱신하는 시계 같은 것에 적합)
const time = readable(new Date(), (set) => {
  const id = setInterval(() => set(new Date()), 1000);
  return () => clearInterval(id);  // 마지막 구독자가 떠나면 정리
});

// derived: 하나 이상의 기존 스토어로부터 새 스토어 생성
