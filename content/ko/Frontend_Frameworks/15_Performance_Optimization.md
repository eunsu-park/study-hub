...
최적화 레시피에 들어가기 전에 [**이론과 원리**](#이론과-원리)를 먼저 읽어보세요. React Fiber와 time slicing, 코드 분할이 순회하는 의존성 그래프, 그리고 메모이제이션이 실제로 무엇을 절약해 주는지(그리고 어디서 절약보다 더 많은 비용이 드는지)를 다룹니다.
...
성능 최적화는 민속학으로 가득 차 있습니다 — "useMemo로 다 감싸라", "리스트는 항상 가상화하라", "어디서나 코드 분할하라". 맥락 없이 보면 대부분 틀렸습니다. 옳은 수는 어느 예산을 쓰고 있느냐에 달려 있습니다 — 바이트(네트워크), 밀리초(CPU), 메모리. 이 절은 기반 메커니즘 — Fiber의 time slicing, 코드 번들러가 하는 의존성 그래프 분할, 메모이제이션의 재계산 회피 로직 — 을 명명해, 모방하지 말고 실제 병목에 최적화를 매칭할 수 있게 합니다.

### A. 세 가지 성능 예산

모든 프론트엔드 최적화는 세 예산 중 하나에서 지출합니다.

| 예산 | 측정 단위 | 무엇이 지출 | 무엇으로 보충 |
|------|-----------|-------------|----------------|
| **네트워크 바이트** | 전송된 KB | 코드, 이미지, 폰트 추가 | 코드 분할, 트리 셰이킹, 이미지 최적화, 캐싱 |
| **CPU 시간** | 메인 스레드 블록 ms | 렌더링, JS 실행, 레이아웃 | 메모이제이션, 가상화, time slicing, web worker |
| **메모리** | 할당된 MB | 긴 리스트, 보유된 클로저 | 가상화, weak reference, 이펙트 cleanup |

역효과를 내는 "최적화" 대부분은 한 예산을 다른 예산을 위해 쓰면서 *그리고* 거래가 잘못되어 있습니다. 모든 값을 `useMemo`로 감싸면, 이미 싼 재계산을 피하기 위해 메모리와 렌더당 비교 비용을 지출합니다. 20개짜리 리스트를 가상화하면 거의 아무것도 절약하지 못하고 복잡도를 지출합니다. 너무 공격적으로 코드 분할하면 이미 작은 바이트를 절약하기 위해 라운드트립을 지출합니다.

모든 최적화의 첫 번째 질문: **어느 예산이 바닥났고, 제안된 변경이 다른 둘에서 얼마나 비용을 들이는가?**

### B. 핵심 웹 지표: 브라우저가 실제로 측정하는 것

Google의 핵심 웹 지표는 세 가지 사용자 경험을 세 숫자로 환원합니다.

- **LCP (Largest Contentful Paint)**: 가장 큰 가시 요소가 렌더될 때까지의 시간. 지출처: 네트워크(자산 다운로드), 서버(TTFB), CPU(렌더링). 최적화: 히어로 이미지 preload, 렌더 블로킹 JS 감소, 캐싱.
- **INP (Interaction to Next Paint)**: 사용자 입력에서 다음 시각 갱신까지의 중앙값. 지출처: CPU(JS 실행, 레이아웃). 최적화: 긴 작업 쪼개기, 비핵심 작업 미루기, `useTransition`으로 갱신을 저우선순위로 표시.
- **CLS (Cumulative Layout Shift)**: 시각 안정성 — 페이지가 로드되며 요소가 얼마나 점프하는가. 지출처: 차원 없이 늦게 도착하는 이미지로 인한 레이아웃 시프트, 폰트 swap, 비동기로 삽입되는 광고. 최적화: `width`/`height` 설정, 공간 예약, `font-display: optional`.

이 셋이 일급인 이유는 "페이지가 느린가?", "페이지가 끊기는가?", "페이지가 점프하는가?"라는 사용자가 느끼는 증상에 대응하기 때문입니다.

### C. React Fiber와 Time Slicing

React 16 이전에는 렌더가 컴포넌트 트리 전체를 동기로 순회했습니다. 30 ms 렌더는 메인 스레드를 30 ms 동안 블록 — 프레임을 떨어뜨리기에 충분한 시간이고 클릭 핸들러를 지연시키기에 더 충분한 시간. **Fiber**(React 16에서 도입, React 18에서 확장)는 렌더러를 **인터럽트 가능**하게 재구조화했습니다.

```
옛 reconciler (동기):
  render() → 트리 전체 순회 → commit
  ↑─────────────── 30ms 블록 ─────────────────↑

Fiber (인터럽트 가능):
  render() → N개 작업 단위 처리 → 검사: 양보해야 하나?
                                          yes → 상태 저장, 이벤트 루프로 반환
                                          no  → 계속
  → 더 많은 단위 처리 → ... → commit
  ↑─5ms─↑─5ms─↑─5ms─↑─5ms─↑─5ms─↑─5ms─↑
   양보  양보   양보   양보   양보   양보
   (그 사이 브라우저가 입력 처리, 스크롤, 페인트)
```

각 컴포넌트의 렌더가 **fiber**(작업 단위)가 됩니다. 스케줄러가 fiber를 처리하고, 몇 ms마다 브라우저가 메인 스레드를 필요로 하는지(입력, 애니메이션, 페인트) 검사하고, 필요하면 양보합니다. 렌더는 일시정지되거나, 재개되거나 — 저우선순위 갱신의 경우 — 폐기되고 더 새로운 상태로 재시작됩니다.

두 사용자 대면 API가 이를 노출합니다.

- **`useTransition`**: 상태 갱신을 저우선순위로 표시. 뒤따르는 렌더는 더 높은 우선순위 렌더(예: 입력 타이핑)에 의해 인터럽트될 수 있음.
- **`useDeferredValue`**: 값을 받아 저우선순위로 갱신되는 "지연된" 버전을 만듦.

둘 다 타이핑된 값이 비싼 리스트 리렌더를 구동할 때도 타이핑이 쾌적함을 유지하게 해 줍니다. 리스트는 *결국* 갱신되지만, 더 급한 것이 진행 중이지 않을 때만.

Vue의 스케줄러와 Svelte의 update 큐는 비슷한 목적(배칭, 마이크로태스크 flush)을 수행하지만, Fiber가 노출하는 완전한 "인터럽트 가능 렌더" 모델은 어느 쪽에도 없습니다.

### D. 코드 분할: 모듈 의존성 그래프 순회

번들러는 그래프를 빌드합니다 — 노드는 모듈, 엣지는 `import` 문. 기본적으로 모두 하나의 큰 파일로 연결됩니다. **코드 분할**은 "첫 로드에 이게 다 필요하지 않다"에 대한 번들러의 응답입니다.
...
라우트 분할 번들:
  app.js     = main + Header + Sidebar + Home                         (200 KB 초기)
  products.js = Products + ProductCard                                 (/products에서 lazy 로드)
  cart.js    = Cart + CartItem                                         (/cart에서 lazy 로드)
  checkout.js = Checkout + PaymentForm + AddressForm                   (/checkout에서 lazy 로드)
  settings.js = Settings + ProfileEdit + Preferences                   (/settings에서 lazy 로드)
```

메커니즘은 `import()` — 모듈로 해소되는 Promise를 반환하는 함수. 번들러는 `import('./Foo')`를 보고, Foo의 추이적 의존성을 별도 청크로 만들고, 런타임이 필요할 때 가져오도록 배치합니다.

```js
// React.lazy는 import() 위의 얇은 래퍼
const Products = React.lazy(() => import('./Products'));

// Vue의 defineAsyncComponent
const Products = defineAsyncComponent(() => import('./Products.vue'));

// Svelte: 라우트 loader의 동적 import
load: async () => ({ Products: (await import('./Products.svelte')).default });
```

트레이드오프:

- **각 청크는 네트워크 라운드트립.** 너무 공격적으로 작은 청크 여러 개로 분할하면 단일 200 KB 다운로드가 10×20 KB 다운로드가 됩니다. HTTP/2 multiplexing이 도와주지만 지연을 없애지는 못합니다.
- **공유 의존성은 중복 제거되어야.** 두 라우트 청크가 모두 `lodash`를 import하면 `lodash`를 두 번 번들하지 않아야. 번들러는 "vendor"나 "common" 청크로 이를 처리.
- **입자도.** 라우트 레벨이 보통 적당. 컴포넌트 레벨은 컴포넌트가 무겁고 거의 보이지 않을 때만 가치가 있음(예: 설정 페이지에서 보이는 차트 라이브러리).

### E. 메모이제이션: 재계산을 피하기 위해 메모리를 지출

메모이제이션은 함수의 출력을 입력에 키잉해 캐시합니다. 같은 입력으로 다음 호출은 캐시를 반환합니다. 베팅:

```
캐시 조회 비용 + 동등성 검사 비용  <  재계산 비용
```

0.1 ms에 실행되는 함수의 경우, 캐시 조회 + 동등성 검사가 0.05 ms일 수 있습니다 — 간신히 이득, 메모리를 고려하면 종종 손해. 50 ms에 실행되는 함수(큰 리스트의 정렬, 필터)의 경우 캐시가 결정적으로 이깁니다.

React의 `useMemo`와 `useCallback`에서:

```jsx
const sorted = useMemo(() => bigList.sort(compareFn), [bigList]);
//   ↑ bigList 참조가 바뀔 때만 재계산
```

의존성 배열은 원소별로 `Object.is`로 비교됩니다. 같으면 캐시 값 반환, 아니면 재계산. 절약이 진짜인 때:

1. 계산이 비쌈(큰 데이터에 대한 sort, filter, reduce).
2. 의존성 참조가 실제 데이터를 바꾸지 않는 렌더 간에 안정적.
3. 캐시 값이 메모된 자식의 prop으로 또는 다른 훅의 의존성으로 사용됨.

절약이 *음수*인 때:

- 계산이 쌈(예: `useMemo(() => a + b, [a, b])` — 덧셈이 비교 비용보다 빠름).
- 의존성이 매 렌더마다 바뀜(캐시가 절대 hit하지 않음. 조회 비용만 지불).
- 캐시 값이 참조 안정성을 신경 쓰지 않는 것에 의해 소비됨.

`React.memo(Component)`는 컴포넌트 레벨의 등가물 — props가 `Object.is`로 같으면 자식의 리렌더 건너뜀. `useMemo`와 같은 거래 — 안정적 props를 가진 비싼 자식에는 가치 있고, 싼 자식이나 불안정 props에는 해로움.

Vue의 `computed`와 Svelte의 `$derived`는 *자동으로 메모이제이션됨* — 정의상 반응형 source가 바뀔 때만 재계산. 메모이제이션을 선택하지 않습니다 — 반응성이 대신해 줍니다. 이것이 Vue와 Svelte가 성능에 대해 "덜 수동적으로" 느껴지는 구조적 이유 중 하나입니다.

### F. 가상화: 보이는 것만 렌더

DOM의 10,000행 리스트는 레이아웃, 페인트, 갱신할 10,000개 요소입니다. 사용자가 30행만 봐도 브라우저가 느려집니다.

**가상화**("windowing"이라고도 함)는 보이는 행과 작은 버퍼만 렌더합니다. 사용자가 스크롤할 때 렌더된 집합이 이동합니다.
...
5행 아래로 스크롤:
   105, 106, ..., 134 렌더
   (100-104 언마운트, 130-134 마운트)
```

`react-window`, `react-virtual`, `@tanstack/virtual`, Vue의 `vue-virtual-scroller` 같은 라이브러리들이 이를 구현합니다. 항목 위치를 계산하고, 스크롤 이벤트를 듣고, 뷰의 슬라이스만 다시 렌더합니다.

비용:

- **스크롤 시 일정한 CPU**(몇 행마다 마운트와 언마운트).
- **구현 복잡도.** 혼합 높이 항목, sticky 헤더, 가상 경계를 넘는 포커스 보존이 모두 까다로움.
- **접근성.** 스크린 리더는 전체 리스트가 DOM에 있기를 기대 — 가상화는 명시적 ARIA 작업 없이는 그것을 깸.

가치 있을 때: 리스트 ≥ 1000개. 가치 없을 때: 50개짜리 리스트, 페이지 크기가 이미 작은 페이지네이션 리스트.

### G. 이미지와 폰트 성능

이미지는 페이지 무게를 지배합니다. 모던 기법 셋이 각자 특정한 낭비를 다룹니다.

- **반응형 이미지**(`srcset`, `<picture>`): 브라우저가 viewport에 맞는 가장 작은 허용 이미지를 고름. 레티나 iPhone은 750px 이미지를, 데스크톱 모니터는 1500px 이미지를 받음. 서버는 둘 다 미리 생성.
- **Lazy 로딩**(`loading="lazy"`): viewport에 가까워질 때까지 이미지 패치 미룸. 2020년 이후 네이티브 브라우저 지원.
- **모던 포맷**(WebP, AVIF): 동등 품질에서 JPEG보다 25-50% 더 작음. 모든 모던 브라우저가 지원.

폰트는 일어나기를 기다리는 레이아웃 시프트입니다.

- **`font-display: swap`**: 웹 폰트가 로드될 때까지 fallback 폰트로 텍스트 표시. "FOIT/FOUT"(invisible/unstyled text의 깜박임)을 일으킴 — 짜증나지만 CLS 손상 없음.
- **`font-display: optional`**: 충분히 빨리 로드되었을 때만 웹 폰트 사용. 아니면 fallback에 머묾. 레이아웃 시프트를 완전히 제거.
- **핵심 폰트 preload**: `<link rel="preload" as="font" ...>` — 브라우저가 CSS가 필요함을 드러낼 때까지 기다리는 대신 HTML 파싱과 병렬로 폰트 패치.

### 이론에서 아래 절들로

- §1 *핵심 웹 지표* — (B). 다른 모든 절이 그것에 대해 최적화하는 메트릭.
- §2 *코드 분할* — (D). `import()`와 라우트 레벨 청크.
- §3 *트리 쉐이킹* — 번들러의 또 다른 의존성 그래프 트릭. ES 모듈 import 기반 dead-code 제거.
- §4 *이미지 최적화* — (G)의 이미지 절반.
- §5 *폰트 최적화* — (G)의 폰트 절반.
- §6 *가상 스크롤링* — (F). 언제 그리고 왜 가상화하는가.
- §7 *메모이제이션* — (E). 재계산 건너뛰기를 위한 React/Vue/Svelte API.
- §8 *성능 측정* — Lighthouse, DevTools, real-user 메트릭. "무엇을 최적화해야 하나?"의 과학적 방법 측면.
