...
문법 투어에 들어가기 전에 [**이론과 원리**](#이론과-원리)를 먼저 읽어보세요. JSX가 `React.createElement` 호출의 설탕(syntactic sugar)이라는 점, 가상 DOM(virtual DOM)이 비교 가능한 값이라는 점, 그리고 `key`가 재조정(reconciliation) 알고리즘을 제어하는 레버라는 점을 다룹니다.
...
React의 사용자 표면은 JavaScript 안에 HTML을 끼워 넣고 그 사이를 중괄호로 양념한 모습입니다. 그 아래에서는 세 가지 독립적인 발상이 모든 일을 합니다. JSX는 단지 평범한 함수 호출의 설탕이고, 그 호출은 가상 DOM(virtual DOM)이라 불리는 평범한 객체의 트리를 만들며, 컴포넌트 타입과 `key` prop이 주도하는 휴리스틱 diff 알고리즘이 실제 DOM에 무엇을 할지를 결정합니다. 이 절은 그 셋을 명시적으로 분리합니다 — React에서 마주치는 모든 "이상한" 동작(입력 포커스 손실, 컴포넌트 상태 손실, 불필요한 렌더링)이 결국 이 셋 중 하나로 환원되기 때문입니다.

### A. JSX는 곧 `React.createElement`

JSX 자체는 런타임 의미가 없습니다. 빌드 도구(Babel, SWC, esbuild, TypeScript 컴파일러)가 모든 JSX 표현식을 함수 호출로 다시 작성한 뒤에야 브라우저가 코드를 봅니다. 클래식 변환(classic transform)에서는 다음과 같습니다.
...
React 17부터 기본인 자동 변환(automatic transform)에서는 임포트되는 함수가 `react/jsx-runtime`의 `jsx`이지만, 결과의 모양은 같습니다. 두 호출 모두의 결과는 평범한 JavaScript 객체이며 — 흔히 "React element"라 불립니다 — 대략 다음과 같은 형태입니다.

```js
{ type: 'button', props: { className: 'primary', onClick: handleClick, children: 'Save' }, key: null, ... }
```

여기서 곧바로 세 가지 결론이 따라옵니다.

1. **대소문자 규칙은 임의가 아닙니다.** `button` 같은 소문자 태그명은 *문자열* `'button'`으로 emit되고 HTML 요소로 취급됩니다. `Button` 같은 대문자 시작 이름은 *변수* `Button`으로 emit되고 컴포넌트 참조로 취급됩니다. JSX는 첫 글자 외에 둘을 구별할 방법이 없습니다.
2. **`class` 대신 `className`, `for` 대신 `htmlFor`.** props 객체는 JavaScript 객체의 속성 할당이 되는데, `class`와 `for`는 예약어입니다.
3. **JSX는 단일 루트(single root)를 가져야 합니다.** 함수 호출은 단일 값을 반환합니다. 인접한 두 요소를 감싸지 않고 반환할 수 없습니다. `<>...</>` (Fragment 단축형)는 `React.createElement(React.Fragment, null, ...)`로 emit됩니다 — 호출 하나, 반환값 하나, 추가 DOM 노드 없음.

"JSX는 그냥 JavaScript다"라는 표현은 정확합니다. JavaScript 표현식이 허용되는 곳이면 어디든 JSX가 허용되고, 문(statement)만 허용되는 곳(`if` 블록, `for` 루프 본문)에서는 JSX가 *허용되지 않습니다*. React 관용구가 `&&`, 삼항 연산자, `.map(...)`을 즐겨 쓰는 이유가 그것입니다 — 모두 표현식입니다.

### B. 가상 DOM은 비교 가능한 값

렌더링은 컴포넌트 함수를 호출한 결과입니다. 반환값은 `{ type, props, children }` 객체의 트리이고, 그 안의 자식들도 같은 모양으로 끝까지 내려갑니다. React는 이전 트리를 들고 있다가 새 트리와 비교합니다. 실제 DOM은 그 차이만큼만 변형됩니다.

```
상태 변경
   │
   ▼
컴포넌트 함수 실행 → 새 VDOM 트리 (평범한 객체)
   │
   ▼
이전 VDOM 트리와 diff
   │
...
가상 DOM 트리의 세 가지 성질이 이를 가능하게 합니다.

1. **할당 비용이 싸다.** 그저 객체 리터럴입니다 — DOM 할당도, 레이아웃도, 페인트도 없습니다. 새 트리를 만드는 비용은 대략 렌더 함수를 실행하는 비용 정도이며, 이는 거의 항상 실제 DOM 변형보다 쌉니다.
2. **위치별로 비교 가능하다.** 두 트리는 같은 루트를 가지며, 알고리즘은 둘을 평행하게 순회합니다. 모든 위치에서 묻습니다: "타입이 같은가?" 같으면 그 자리에서 갱신, 다르면 서브트리를 버리고 다시 만든다.
3. **렌더러와 분리되어 있다.** 같은 VDOM 트리를 `react-dom`(브라우저 DOM), `react-native`(네이티브 뷰), `react-three-fiber`(WebGL 장면 그래프)가 모두 소비할 수 있습니다. diff 알고리즘은 자신이 패치하는 것이 어떤 종류의 "DOM"인지 모릅니다.

일반 트리 diff는 O(n³)입니다. React는 부분 트리 간 최적 이동을 포기하고 단 하나의 규칙에 집중함으로써 이를 O(n)으로 떨어뜨립니다: **연속한 두 트리에서 같은 위치의 타입이 바뀌었다면, 옛 서브트리를 통째로 버린다.** `<div>`가 `<span>`이 되면 그 div와 모든 자손이 폐기됩니다 — 그 안의 컴포넌트가 들고 있던 상태도 함께. `<UserCard>`가 `<AdminCard>`가 되어도 마찬가지입니다. 렌더링된 DOM이 거의 동일해 보이더라도 그렇습니다. "내 입력 포커스가 사라졌다", "왜 폼이 자기 자신을 비웠지" 같은 문제의 가장 흔한 원인이 이것입니다 — 트리 위쪽 어딘가에서 같은 위치의 컴포넌트 타입이 바뀐 것입니다.

### C. `key`: 재조정이 리스트 항목을 식별하는 방법

리스트는 "위치별 비교" 규칙이 무너지는 유일한 곳입니다. "같은 위치"가 무엇을 뜻하는지가 자명하지 않기 때문입니다. `[A, B, C]`를 렌더링하다가 `[X, A, B, C]`를 렌더링한다고 합시다. 위치별로 보면
...
사용자는 그저 앞에 항목 하나를 끼워 넣었을 뿐인데, 모든 원소가 "변경"된 것처럼 보입니다. `A`, `B`, `C` 안의 상태(입력값, 스크롤 위치, 포커스)가 사라집니다. `key`는 정확히 이 버그를 해결하기 위해 존재합니다.

자식들이 키를 가지면, React는 더 이상 인덱스로 매칭하지 않습니다. 이전과 새 리스트로 두 개의 `key → element` 맵을 만들어 키로 매칭합니다.

```
old:  [A(key=a), B(key=b), C(key=c)]
new:  [X(key=x), A(key=a), B(key=b), C(key=c)]

key=a: 양쪽에 다른 위치로 존재 → 이동, 재마운트 안 함
key=b: 양쪽에 다른 위치로 존재 → 이동, 재마운트 안 함
key=c: 양쪽에 다른 위치로 존재 → 이동, 재마운트 안 함
key=x: 새 트리에만 존재 → 마운트
```

세 가지 규칙이 따라옵니다.

1. **키는 렌더 간 안정(stable)해야 한다.** `Math.random()`이나 `Date.now()`를 쓰면 매 렌더마다 다시 생성되어 매칭이 무너집니다 — 매번 모든 항목이 새로워 보입니다.
2. **키는 형제(sibling)들 사이에서 유일해야 한다.** 중복은 `key → element` 맵을 모호하게 만듭니다. 트리 전체에서 유일할 *필요는 없습니다* — 형제 사이에서만 유일하면 됩니다.
3. **인덱스를 키로 쓰는 것은 최후의 수단.** 리스트가 절대 재정렬되지 않고, 중간에 항목이 삽입되거나 제거되지 않으며, 항목들이 상태를 갖지 않을 때만 옳습니다. 이 조건들 중 하나라도 깨지는 순간, 원래의 버그가 다시 살아납니다.

일반 원칙: 키는 React에게 "이 항목은 렌더 간에 *논리적으로 같은* 항목이며 위치만 옮겼다"라고 말해 줍니다. 키가 없으면 정체성은 위치이고, 키가 있으면 정체성은 여러분이 정한 그것입니다.

### D. Render → Commit 파이프라인

렌더링은 즉각적인 DOM 갱신이 아닙니다. React는 작업을 두 단계로 나눕니다.

```
RENDER 단계 (일시정지/재시작/폐기 가능)
   - 컴포넌트 함수 호출
   - 새 VDOM 트리 빌드
   - 이전 트리와 diff
   - 필요한 DOM 연산 목록("effect")을 만든다
   - 아직 DOM 변형 없음, 아직 사이드 이펙트 없음

COMMIT 단계 (동기, 인터럽트 불가)
   - DOM 연산을 순서대로 적용
   - 이전 이펙트의 cleanup 실행
   - 새 이펙트(useEffect, useLayoutEffect) 실행
   - ref 갱신
```

이 분리 때문에 컴포넌트 함수는 **렌더링에 관해서 순수해야 합니다(pure)**. 여러 번, 어떤 순서로든 호출될 수 있고, 폐기되는 호출은 관찰 가능한 어떤 일도 해서는 안 됩니다. 모든 사이드 이펙트 — 패칭, 구독, 로깅, DOM 변형 — 는 commit 단계에 속하며, `useEffect` 등을 통해 접근합니다. render 단계는 기술을 계산하는 곳, commit 단계는 그 기술을 적용하는 곳입니다.

### 이론에서 아래 절들로

- §1 *JSX 기초*는 (A)로 desugar되는 표면 문법.
- §2 *함수형 컴포넌트*와 §3 *Props와 Children*은 반환값이 (B)인 함수를 작성하는 방법.
- §6 *목록 렌더링*은 (C)의 실전 — 모든 `key` 경고는 diff 알고리즘이 여러분을 보호하려는 신호입니다.
- §7 *이벤트 처리*는 commit 단계에 후킹됩니다. 핸들러는 commit 시점에 부착되고, 이후 발화되며, 다음 render → commit 사이클을 트리거합니다.
- §8 *컴포넌트 조합*은 (B)의 위치+타입 규칙이 제공하는 구조적 정체성에 의존합니다 — 그것이 컴포넌트를 중첩하고, 재정렬하고, 재사용하면서도 어느 것이 어느 것인지 잃지 않게 해 줍니다.
