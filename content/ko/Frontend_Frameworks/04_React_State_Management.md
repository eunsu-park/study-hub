...
API 투어에 들어가기 전에 [**이론과 원리**](#이론과-원리)를 먼저 읽어보세요. prop-drilling 문제, Context의 참조 동등성이 어떻게 리렌더를 강제하는지, flux 패턴, 그리고 Zustand 같은 subscribe 기반 스토어가 둘 모두를 어떻게 우회하는지 다룹니다.
...
상태 관리는 사실 "데이터를 어디에 둘 것인가"의 문제가 아닙니다. **누가 데이터를 바꿀 수 있고, 누가 읽을 수 있고, 데이터가 바뀔 때 무엇이 리렌더를 트리거하는가**의 문제입니다. 라이브러리들은 이 세 가지에 서로 다른 답을 내놓고, 그 차이는 스타일이 아니라 메커니즘에 있습니다. 이 절은 네 가지 주류 답 — 로컬 상태, Context, flux/Redux, 외부 subscribe 기반 스토어 — 을 차례로 다루며 각각이 등장하게 된 실패 모드를 보여 줍니다.
...
### D. Subscribe 기반 스토어: 셀렉터로 구동되는 리렌더
...
- §1 *useReducer*는 컴포넌트 스코프의 (C). 액션, reducer, 단일 상태 객체.
- §2 *Context API*는 (B). prop-drilling을 해결하는 대가로 (B)의 리렌더 규칙을 도입.
- §3 *Context의 한계: 리렌더링 문제*는 (B)의 버그를 정확히 명명하고 그 해법을 보여줌.
- §4 *Zustand*는 (D) — 가장 작은 API로 셀렉터 기반 subscribe.
- §5 *무엇을 언제 사용할까*는 (E)와 (F)의 매트릭스를 실전화한 결정표. 로컬은 로컬에, 저빈도 횡단 상태는 Context에, 고빈도 또는 광범위 상태는 Zustand에, 서버에서 오는 것은 무엇이든 TanStack Query에.
