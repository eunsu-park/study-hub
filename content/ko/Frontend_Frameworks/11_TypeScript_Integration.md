...
TypeScript는 **structural**입니다 — 두 타입이 *모양*이 일치하면 호환됩니다.
...
const v: Vec     = p;  // OK — 같은 모양
```

세 가지 프론트엔드 결과:

1. **Prop 인터페이스가 duck-typed.** `{ title: string; description: string }`로 선언된 `Card` 컴포넌트는 그 필드들을 적어도 가진 어떤 객체든 받습니다. 호출자는 prop 타입을 import하거나 extend할 필요가 없습니다 — 그저 올바른 모양을 전달합니다.
2. **Excess property check가 리터럴에서 작동.** `<Card title="x" description="y" extra={1} />`는 TypeScript가 일반적으로 추가 필드를 싫어해서가 아니라, *리터럴* 객체가 excess-check되기 때문에 에러입니다. 같은 값을 변수를 거쳐 할당하면 검사를 우회합니다. JSX의 오타를 잡으면서도 합성이 작동하게 하려는 의도된 ergonomic 트레이드입니다.
3. **너비 vs 깊이.** 컴포넌트에 필수 prop을 추가하면 모든 호출자가 깨집니다(*너비* 변경). 선택적 prop이나 비필수 필드를 추가해도 깨지지 않습니다. "내 타입의 모양에 대한 breaking change인가?"의 프레임워크 버전입니다.
...
- 모든 타입 정보는 런타임 전에 제거됩니다. 런타임의 `typeof x` 리플렉션은 없습니다. `instanceof`가 작동하는 이유는 클래스가 값이기도 하기 때문.
- 일부 구성요소는 양쪽 걸침: `class Foo`는 값(생성자)과 타입(인스턴스) 둘 다 도입. `interface`는 타입 전용. `const`는 값 전용.

프레임워크 코드에서 이는 타입을 값 레벨에서 사용하고 싶을 때 중요합니다. `typeof` 연산자(*타입 레벨* 것)는 값을 타입 공간으로 투사합니다.
