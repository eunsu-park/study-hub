...
store.subscribers.forEach(notify)
       │
       ▼
useSelector(s => s.user.name) — 셀렉터 재실행
       │
       ▼
셀렉터 결과가 참조 기준으로 바뀌었으면 컴포넌트 리렌더
```

| 속성 | 값 |
|------|-----|
| 상태 위치 | 단일 글로벌 스토어 |
| 변경 API | `dispatch(action)` |
| 갱신 메커니즘 | 순수 reducer가 새 상태 반환 |
| 리렌더 트리거 | 셀렉터 반환값의 참조 동등성 |
| dispatch당 시간 비용 | O(n), n = 구독자 수 |
| 공간 비용 | 구독당 스토어 + reducer + 셀렉터 캐시 |

**강점**: 모든 변경에 이름과 페이로드가 있으므로 devtools가 재생, 시간여행, 감사를 할 수 있습니다. reducer가 순수해 테스트가 자명합니다. 액션 기반 분리로 미들웨어가 모든 쓰기에 끼어들 수 있습니다(로깅, 낙관적 갱신).
...
// 컴포넌트 A는 countAtom을 읽음 — 구독.
// 컴포넌트 B는 doubledAtom을 읽음 — doubled에 구독(count에 의존).
// countAtom 설정은 둘 모두를 트리거. 무관한 atom만 읽는 가상의
// 컴포넌트 C는 영향 없음.
```

| 속성 | 값 |
|------|-----|
| 상태 위치 | atom 키별로 글로벌 등록되는 작은 atom 다수 |
| 변경 API | atom별 `setAtom(value)` |
| 갱신 메커니즘 | setter가 atom의 저장값을 변경, 의존 atom은 재계산 |
| 리렌더 트리거 | atom 값이 참조 기준으로 변경 |
| 쓰기당 시간 비용 | O(k), k = 추이적으로 의존하는 atom 수 |
| 공간 비용 | 의존성 그래프의 atom당 노드 하나 |
