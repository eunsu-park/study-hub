...
─────────┼───────────────────────┼─────────────────────┼─────────────────
CSR      | JS 실행 후(클라이언트)  | 페이지 내비게이션마다 | 빈 셸 먼저. SEO 적대적
SSR      | 요청마다(서버)         | 서버 후 클라이언트   | 개인화. 요청당 서버 비용
SSG      | 빌드 타임에            | 하이드레이션 후      | 가장 쌈. 모두에게 정적
ISR      | 빌드와 그 후 on-demand | 하이드레이션 후      | 페이지별 캐시. 필요 시 재검증
         | (서버)                 |                     |
```

작업이 언제 일어나는지의 시각적 배치:

```
              빌드 타임           요청 타임              클라이언트 타임
              ─────────────────────────────────────────────────────────
CSR           JS 번들                                    다운로드 → 실행 → 렌더
SSG           모든 페이지 렌더                            다운로드 → 하이드레이트
SSR                                요청 시 렌더            다운로드 → 하이드레이트
ISR           일부 페이지 렌더    stale 페이지 재렌더     다운로드 → 하이드레이트
```

**트레이드오프**:

- **CSR**: 가장 싼 서버(정적 파일만). JS를 기다리니 첫 페인트가 느림. 크롤러가 JS를 안정적으로 실행하지 않으면 SEO에 나쁨(모던 Googlebot은 함. 많은 소셜미디어 봇은 안 함).
- **SSR**: 빠른 첫 페인트(HTML이 준비되어 도착). 개인화 작동(쿠키, 사용자, 요청 시점 결정). 그러나 모든 요청이 서버에 닿음(비용)이고 TTFB는 렌더 시간에 의존.
- **SSG**: HTML이 CDN에서 서빙되는 정적 파일. 가능한 가장 빠른 TTFB. 개인화 없음(모두에게 같은 HTML). 페이지 수가 늘면 빌드가 느려짐.
- **ISR**: 선택적 재생성을 가진 SSG. 페이지가 첫 요청에서 생성되고 캐시됨. `revalidate` 간격 후(또는 on-demand 무효화 시) 다음 요청이 stale 버전을 서빙하면서 백그라운드에서 재생성을 트리거. SSG의 빠른 서빙과 SSR의 신선함을 결합.
...
[시간 경과. 느린 컴포넌트가 서버에서 해소됨]

HTML <script>...replacePlaceholder(slowContent)...</script>
```

브라우저는 셸을 즉시 보여주고, 각 `<Suspense>` 경계의 데이터가 해소되는 대로 서버가 해소된 HTML과 그것을 끼워 넣는 작은 스크립트를 푸시합니다. **Selective hydration**은 각 청크가 자기 JS가 준비되는 즉시 인터랙티브해지게 해, 전체 번들을 기다리지 않게 합니다.

메커니즘이 요구하는 것:

- 스트리밍 가능한 서버(Node.js `Readable` 스트림, edge 런타임, React의 `renderToPipeableStream`).
