...
고 카디널리티(high-cardinality) 라벨은 Prometheus 배포에서 가장 흔한 함정 중 하나입니다. 각 고유한 라벨 조합이 새로운 시계열(time series)을 생성하며, 제어되지 않는 증가는 메모리를 고갈시키고 서버를 크래시시킬 수 있습니다.
...
│  │  http_requests_total{                                      │ │
│  │    method="GET",                                           │ │
│  │    path="/api/users/12345",    ← 사용자 ID별 고유!         │ │
│  │    user_id="u-67890",          ← 무제한                    │ │
│  │    session_id="sess-abc123"    ← 무제한                    │ │
│  │  }                                                         │ │
│  │                                                            │ │
│  │  사용자 1M × 메서드 3개 × 경로 100개 = 3억 시계열!        │ │
│  │  → Prometheus OOM 크래시                                   │ │
│  │                                                            │ │
│  └────────────────────────────────────────────────────────────┘ │
│                                                                 │
...
│  │  http_requests_total{                                      │ │
│  │    method="GET",                                           │ │
│  │    path="/api/users/:id",      ← 매개변수화된 템플릿      │ │
│  │    status="200"                ← 작은 유한 집합            │ │
│  │  }                                                         │ │
│  │                                                            │ │
│  │  메서드 3개 × 경로 20개 × 상태 5개 = 300 시계열            │ │
│  │  → 완벽하게 관리 가능                                      │ │
│  │                                                            │ │
│  └────────────────────────────────────────────────────────────┘ │
│                                                                 │
│  경험 법칙:                                                      │
│  • 사용자 ID, 세션 ID, 트레이스 ID를 라벨로 사용하지 말 것      │
│  • URL 경로를 매개변수화 (/users/:id, /users/12345 아님)        │
│  • 메트릭당 총 시계열을 10,000 이하로 유지                       │
│  • 레코딩 룰로 고 카디널리티 데이터를 사전 집계                  │
│  • 시계열 수 모니터링: prometheus_tsdb_head_series               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
