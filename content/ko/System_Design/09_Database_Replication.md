...
**이전**: [데이터베이스 확장](./08_Database_Scaling.md) | **다음**: [데이터 일관성 패턴](./10_Data_Consistency_Patterns.md)
...
1. 데이터베이스 복제(Replication)의 목적과 제공하는 이점(가용성, 내결함성, 읽기 확장)을 설명할 수 있다
2. 단일 리더(Single-Leader), 다중 리더(Multi-Leader), 리더 없는(Leaderless) 복제 토폴로지와 각각의 트레이드오프를 비교할 수 있다
3. 동기(Synchronous) 복제와 비동기(Asynchronous) 복제를 구분하고, 반동기(Semi-Synchronous)를 실용적인 절충안으로 설명할 수 있다
4. 복제 지연(Replication Lag) 문제를 파악하고, 자신의 쓰기 읽기(Read-Your-Writes)와 단조 읽기(Monotonic Reads) 같은 일관성 보장을 설명할 수 있다
5. 리더 선출(Leader Election)을 위한 장애 조치(Failover) 과정을 설명하고, 스플릿 브레인(Split-Brain) 시나리오 방지 방법을 설명할 수 있다
6. 쿼럼(Quorum) 공식(R + W > N)을 적용하여 분산 데이터베이스의 일관성 수준을 설정할 수 있다

**난이도**: ⭐⭐⭐ (중급)

---

모든 운영 데이터베이스는 여러 머신에 데이터 복사본을 유지해야 합니다. 데이터베이스 서버가 단 하나뿐인데 그 서버가 충돌하면 모든 데이터를 잃게 됩니다. 복제(Replication)는 이러한 복사본들을 동기화된 상태로 유지하는 메커니즘이지만, 세부 사항에 악마가 숨어 있습니다. 복제본들은 얼마나 빨리 수렴하는지, 네트워크 파티션(Network Partition) 중에는 어떤 일이 일어나는지, 충돌하는 쓰기는 누가 해결하는지 — 이 질문들이 분산 시스템 설계의 핵심이며, 사실상 모든 시스템 설계 면접에서 등장합니다.
...
│     └────────────────────────────────────────────────────┘     │
│                                                                 │
│  3. 스플릿 브레인 방지: 펜싱 토큰 패턴                          │
│     ┌────────────────────────────────────────────────────┐     │
│     │                                                    │     │
│     │  구체적 예시:                                      │     │
│     │  1. Old Leader가 토큰 T=33으로 리스(lease) 보유    │     │
│     │  2. 네트워크 파티션 발생                            │     │
│     │  3. New Leader가 토큰 T=34로 선출됨                │     │
│     │  4. Old Leader가 T=33으로 쓰기 시도                │     │
│     │  5. 스토리지가 쓰기 거부: T=33 < 최종 확인 T=34   │     │
│     │                                                    │     │
...
│     │                              "T=34 = 34, ACCEPT"   │     │
│     │                                                    │     │
│     │  펜싱 토큰은 잠금/리스 서비스가 발급하는 단조      │     │
│     │  증가(monotonically increasing) 숫자입니다.         │     │
│     │  스토리지 노드는 확인한 최대 토큰을 추적하고,      │     │
│     │  더 낮은 토큰의 쓰기를 거부합니다.                  │     │
│     │                                                    │     │
│     └────────────────────────────────────────────────────┘     │
│                                                                 │
