...
### 4.4 캐시 스탬피드 방지 (Cache Stampede Prevention)

캐시 스탬피드(썬더링 허드(Thundering Herd)라고도 함)는 인기 있는 캐시 항목이 만료되었을 때 다수의 동시 요청이 동시에 해당 항목을 재생성하려고 시도하여 백엔드에 과부하를 주는 현상입니다.
...
│  └────────────────────────────────────────────────────────────┘ │
│                                                                  │
│  방법 2: 확률적 조기 만료 (XFetch)                                │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │                                                            │ │
│  │  값과 재계산 시간(delta)을 함께 저장:                      │ │
│  │                                                            │ │
...
│  │      return data                                           │ │
│  │                                                            │ │
│  │  만료 시점에 가까워질수록 재계산 확률이 높아지므로,        │ │
│  │  하나의 요청이 자연스럽게 먼저 갱신하게 됨.                │ │
│  │                                                            │ │
│  └────────────────────────────────────────────────────────────┘ │
│                                                                  │
│  비교:                                                           │
...
│  │              │ 뮤텍스 잠금          │ XFetch (확률적)    │    │
│  ├──────────────┼─────────────────────┼────────────────────┤    │
│  │ 메커니즘     │ 잠금, 하나만 재구축  │ 조기 갱신          │    │
│  │ 블로킹       │ 나머지 대기          │ 블로킹 없음        │    │
│  │ 복잡도       │ 낮음                 │ 중간               │    │
│  │ 적합 환경    │ 중간 트래픽          │ 매우 높은 트래픽   │    │
│  └──────────────┴─────────────────────┴────────────────────┘    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
