...
1. 모놀리식(Monolithic)과 마이크로서비스(Microservices) 아키텍처를 비교하고 각 방식의 트레이드오프를 설명할 수 있다.
2. 잘 설계된 마이크로서비스의 핵심 특성(단일 책임, 자율적 배포, 분산 데이터)을 열거할 수 있다.
3. 도메인 주도 설계(Domain-Driven Design) 개념(경계 컨텍스트(Bounded Context), 애그리거트(Aggregate))을 적용하여 서비스 경계를 정의할 수 있다.
4. 서비스별 데이터베이스(Database-per-Service) 패턴과 서비스 간 데이터 일관성 관리 전략을 설명할 수 있다.
5. 동기(REST, gRPC)와 비동기(이벤트 기반) 서비스 간 통신 방식의 차이를 구별할 수 있다.
6. 스트랭글러 피그(Strangler Fig) 패턴을 사용하여 모놀리스에서 마이크로서비스로 실용적인 마이그레이션 전략을 수립할 수 있다.
