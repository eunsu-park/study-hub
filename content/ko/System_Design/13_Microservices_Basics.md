...
**이전**: [메시지 시스템 비교](./12_Message_System_Comparison.md) | **다음**: [마이크로서비스 패턴](./14_Microservices_Patterns.md)
...
1. 모놀리식(Monolithic)과 마이크로서비스(Microservices) 아키텍처를 비교하고 각 방식의 트레이드오프를 설명할 수 있습니다.
2. 잘 설계된 마이크로서비스의 핵심 특성(단일 책임, 자율적 배포, 분산 데이터)을 열거할 수 있습니다.
3. 도메인 주도 설계(Domain-Driven Design) 개념(경계 컨텍스트(Bounded Context), 애그리거트(Aggregate))을 적용하여 서비스 경계를 정의할 수 있습니다.
4. 서비스별 데이터베이스(Database-per-Service) 패턴과 서비스 간 데이터 일관성 관리 전략을 설명할 수 있습니다.
5. 동기(REST, gRPC)와 비동기(이벤트 기반) 서비스 간 통신 방식의 차이를 구별할 수 있습니다.
6. 스트랭글러 피그(Strangler Fig) 패턴을 사용하여 모놀리스에서 마이크로서비스로 실용적인 마이그레이션 전략을 수립할 수 있습니다.

---

Netflix는 단일 Java 애플리케이션에서 700개 이상의 마이크로서비스로 전환하여 하루에 수천 번 배포하고 개별 컴포넌트를 독립적으로 스케일링할 수 있게 되었습니다. 그러나 기초도 모른 채 마이크로서비스에 무작정 뛰어든 팀들은 결국 처음보다 더 운영하기 어려운 "분산 모놀리스(distributed monolith)"를 만들어내곤 합니다. 이 레슨에서는 시스템을 현명하게 분해하고 가장 흔한 함정을 피하기 위한 개념적 토대를 쌓습니다.
