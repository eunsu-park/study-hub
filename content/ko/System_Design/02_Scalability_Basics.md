...
1. 확장성(Scalability)을 정의하고 현대 시스템에서 왜 중요한지 설명할 수 있다
2. 수직 확장(vertical scaling)과 수평 확장(horizontal scaling)을 비교하고 각 접근법의 트레이드오프(trade-off)를 파악할 수 있다
3. 무상태(stateless) 아키텍처와 상태 유지(stateful) 아키텍처를 구별하고 확장에 미치는 영향을 설명할 수 있다
4. 일반적인 세션 관리(session management) 전략(스티키 세션(sticky sessions), 중앙 집중 저장소(centralized store), JWT)을 설명할 수 있다
5. CAP 정리(CAP theorem)를 설명하고 실세계 시스템을 CP 또는 AP로 분류할 수 있다
6. PACELC 확장 정리와 정상 운영 상태에서 지연 시간-일관성 트레이드오프를 어떻게 포착하는지 설명할 수 있다
7. 주어진 워크로드(workload)를 스케일 업(scale up)과 스케일 아웃(scale out) 중 어느 것으로 처리하는 것이 더 나은지 추정할 수 있다
...
단일 서버는 물리적 한계에 도달하기 전까지만 성장할 수 있다 — 무한한 코어를 가진 CPU도, 무한한 용량의 하드 드라이브도 존재하지 않습니다. 확장성(Scalability)은 수요가 증가함에 따라 시스템이 우아하게 성장하도록 설계하는 기술로, 10명의 사용자를 처리하든 1천만 명을 처리하든 마찬가지입니다. 확장 전략과 분산 시스템 보장 간의 근본적인 트레이드오프를 이해하는 것은 앞으로 내리게 될 모든 아키텍처 결정을 형성하게 됩니다.
