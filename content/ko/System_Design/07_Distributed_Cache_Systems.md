...
1. 단일 노드 캐시가 대규모 시스템에 불충분한 이유와 분산 캐시(Distributed Cache)가 이 문제를 해결하는 방식을 설명할 수 있다
2. Redis의 핵심 데이터 구조(문자열, 해시, 리스트, 집합, 정렬된 집합)를 설명하고 각각에 적합한 사용 사례를 식별할 수 있다
3. 고가용성(High Availability)과 수평 확장(Horizontal Scaling)을 위한 Redis 클러스터(Redis Cluster)와 Redis 센티넬(Redis Sentinel) 아키텍처를 비교할 수 있다
4. Redis와 Memcached의 트레이드오프(Trade-off)를 평가하고 주어진 워크로드에 적합한 도구를 선택할 수 있다
5. 가상 노드(Virtual Nodes)를 활용한 일관성 해싱(Consistent Hashing)을 구현하여 캐시 노드 간에 키를 균등하게 분산할 수 있다
6. 최소한의 중단으로 노드 장애와 리밸런싱(Rebalancing)을 처리하는 분산 캐싱 계층(Distributed Caching Tier)을 설계할 수 있다
...
### 3.3 Redis 영속성 모드 (Persistence Modes)
...
│  ┌────────────────────────────────────────────────────────────┐ │
│  │                                                            │ │
│  │  • 설정된 간격으로 특정 시점 스냅샷 생성                   │ │
│  │  • save 900 1   → 900초 내 1개 이상 키 변경 시 스냅샷     │ │
│  │  • save 300 10  → 300초 내 10개 이상 키 변경 시 스냅샷    │ │
│  │  • 컴팩트한 바이너리 파일, 빠른 재시작                     │ │
│  │  • 스냅샷 사이 데이터 손실 가능                             │ │
│  │                                                            │ │
│  └────────────────────────────────────────────────────────────┘ │
...
│  │                                                            │ │
│  │  • 모든 쓰기 연산을 로깅                                   │ │
│  │  • appendfsync always    → 가장 안전, 가장 느림            │ │
│  │  • appendfsync everysec  → 권장 (최대 1초 데이터 손실)     │ │
│  │  • appendfsync no        → OS가 결정, 가장 빠름            │ │
│  │  • 파일이 크고 재시작이 느림                                │ │
│  │  • AOF 재작성(rewrite)으로 주기적 압축                     │ │
│  │                                                            │ │
│  └────────────────────────────────────────────────────────────┘ │
│                                                                  │
│  비교:                                                           │
...
│  │ 파일 크기  │ 컴팩트            │ 더 큼            │           │
│  │ 재시작     │ 빠름              │ 느림             │           │
│  │ I/O 영향   │ 주기적 fork       │ 지속적 쓰기      │           │
│  └────────────┴──────────────────┴──────────────────┘           │
│                                                                  │
│  모범 사례: 운영 환경에서는 RDB + AOF 모두 활성화.               │
│  RDB는 백업용, AOF는 내구성용으로 사용.                          │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

> **참고**: Redis Cluster는 크로스 슬롯(cross-slot) 멀티 키 연산(예: `MGET`, `SUNION`, 서로 다른 슬롯의 키를 사용하는 트랜잭션)을 지원하지 않습니다. 멀티 키 명령의 모든 키는 동일한 슬롯에 해싱되어야 합니다. 멀티 키 연산이 필요한 경우 해시 태그(예: `{user:123}`)를 사용하여 관련 키를 동일 슬롯에 배치하세요.
