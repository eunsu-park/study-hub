...
1. 클라이언트 사이드(Client-Side)와 서버 사이드(Server-Side) 서비스 디스커버리(Service Discovery) 메커니즘을 설명하고, 레지스트리 기반 방식(Consul, Eureka)과 DNS 기반 방식을 비교할 수 있다.
2. 서킷 브레이커(Circuit Breaker) 패턴을 구현하여 의존 서비스 간 연쇄 장애(cascading failure)를 방지할 수 있다.
3. 격벽(Bulkhead) 패턴을 적용하여 장애를 격리하고 단일 컴포넌트로 인해 핵심 자원이 고갈되는 것을 보호할 수 있다.
4. 서비스 메시(Service Mesh)의 아키텍처(데이터 플레인(Data Plane) vs 컨트롤 플레인(Control Plane))와 사이드카 프록시(Sidecar Proxy)의 역할을 설명할 수 있다.
5. 트레이스 컨텍스트 전파(Trace Context Propagation), 스팬(Span) 계층 구조, 샘플링 정책을 활용한 분산 추적(Distributed Tracing) 전략을 설계할 수 있다.
6. 시스템 성숙도, 운영 복잡도, 장애 유형에 따라 각 패턴의 적용 여부를 평가할 수 있다.
