...
CRDT는 여러 노드에 복제되어 조율(coordination) 없이 독립적이고 동시적으로 업데이트될 수 있으며, 항상 일관된 상태로 수렴하는 데이터 구조입니다.
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  핵심 속성: 복제본이 일시적으로 발산할 수 있지만, 충돌 해결 로직 없이   │
│  항상 수렴합니다.                                                       │
│                                                                         │
│  예시: G-Counter (증가 전용 카운터)                                      │
│                                                                         │
│  각 노드는 자체 카운터를 유지합니다. 전체 합이 총값입니다.               │
│                                                                         │
│  Node A: {A: 3, B: 0, C: 0}  →  로컬 카운트 = 3                        │
│  Node B: {A: 0, B: 5, C: 0}  →  로컬 카운트 = 5                        │
│  Node C: {A: 0, B: 0, C: 2}  →  로컬 카운트 = 2                        │
│                                                                         │
│  병합 후 (요소별 최댓값):                                                │
│  All:    {A: 3, B: 5, C: 2}  →  총합 = 10                               │
│                                                                         │
│  병합은 교환 법칙(commutative), 결합 법칙(associative),                  │
│  멱등성(idempotent)을 만족하므로, 동기화 순서와 중복은 무관합니다.       │
│                                                                         │
│  주요 CRDT 유형:                                                         │
│  ┌─────────────────┬───────────────────────────────────────────────┐   │
│  │ G-Counter        │ 증가 전용 카운터                               │   │
│  │ PN-Counter       │ 증가 및 감소 카운터                            │   │
│  │ G-Set            │ 추가 전용 집합                                 │   │
│  │ OR-Set           │ 관찰 제거 집합 (추가 + 제거)                   │   │
│  │ LWW-Register     │ 최종 작성자 우선 레지스터                      │   │
│  └─────────────────┴───────────────────────────────────────────────┘   │
│                                                                         │
│  활용 사례: Redis CRDT, Riak, Cassandra 카운터, 협업 편집                │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
