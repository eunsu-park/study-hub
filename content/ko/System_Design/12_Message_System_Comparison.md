# 메시지 시스템 비교 (Message System Comparison)

**이전**: [메시지 큐 기초](./11_Message_Queue_Basics.md) | **다음**: [마이크로서비스 기초](./13_Microservices_Basics.md)
...
1. Apache Kafka의 아키텍처와 핵심 추상화(토픽, 파티션, 컨슈머 그룹, 로그 기반 저장소)를 설명한다
2. RabbitMQ의 AMQP 모델(익스체인지, 바인딩, 큐, 확인(acknowledgment) 메커니즘)을 설명한다
3. 클라우드 네이티브 메시징 시나리오에서 AWS SQS(표준 vs FIFO)와 SNS를 비교한다
4. 처리량, 지연시간, 순서 보장, 내구성, 운영 복잡도 등의 선택 기준에 따라 메시지 시스템을 평가한다
5. 서로 다른 워크로드 특성에 맞게 여러 시스템을 조합한 하이브리드 메시징 아키텍처(hybrid messaging architecture)를 설계한다
6. 주어진 요구사항과 제약 조건에 적합한 메시지 시스템을 선택한다

---

메시지 시스템 선택은 팀이 내리는 가장 중요한 인프라 결정 중 하나로, 처리량 상한선·운영 부담·데이터 파이프라인 모델링 방식에 오랫동안 영향을 미칩니다. Kafka, RabbitMQ, 그리고 AWS SQS/SNS와 같은 클라우드 관리형 서비스는 근본적으로 서로 다른 설계 철학을 구현하고 있으며, 잘못된 선택은 나중에 비용이 큰 재설계로 이어질 수 있습니다. 이 레슨은 확신을 갖고 선택할 수 있는 사고 체계를 갖추도록 도와줍니다.
