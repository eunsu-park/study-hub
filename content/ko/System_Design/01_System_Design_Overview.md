...
1. 시스템 설계(System Design)가 무엇이며 대규모 소프트웨어 시스템 구축에 왜 중요한지 설명할 수 있다
2. 시스템 설계 면접에서 사용하는 평가 기준(evaluation criteria)을 설명할 수 있다
3. 시스템 설계 문제에 접근하기 위한 구조화된 4단계 프레임워크(framework)를 적용할 수 있다
4. 용량(capacity), 저장소(storage), 대역폭(bandwidth) 추정을 위한 규모 추정(Back-of-the-envelope) 계산을 수행할 수 있다
5. 빠른 추정을 위한 주요 성능 수치(지연 시간(latency), 처리량(throughput), 저장소)를 떠올릴 수 있다
6. 주어진 요구사항으로부터 간단한 시스템의 고수준(high-level) 아키텍처 다이어그램을 스케치할 수 있다
