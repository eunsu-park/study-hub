...
**다음**: [확장성 기초](./02_Scalability_Basics.md)
...
1. 시스템 설계(System Design)가 무엇이며 대규모 소프트웨어 시스템 구축에 왜 중요한지 설명할 수 있습니다
2. 시스템 설계 면접에서 사용하는 평가 기준(evaluation criteria)을 설명할 수 있습니다
3. 시스템 설계 문제에 접근하기 위한 구조화된 4단계 프레임워크(framework)를 적용할 수 있습니다
4. 용량(capacity), 저장소(storage), 대역폭(bandwidth) 추정을 위한 규모 추정(Back-of-the-envelope) 계산을 수행할 수 있습니다
5. 빠른 추정을 위한 주요 성능 수치(지연 시간(latency), 처리량(throughput), 저장소)를 떠올릴 수 있습니다
6. 주어진 요구사항으로부터 간단한 시스템의 고수준(high-level) 아키텍처 다이어그램을 스케치할 수 있습니다
...
매일 사용하는 모든 애플리케이션 — 소셜 미디어 피드부터 차량 호출 서비스까지 — 은 초당 수백만 건의 요청을 거뜬히 처리하는 정교하게 설계된 시스템 위에서 동작합니다. 시스템 설계는 이를 가능하게 하는 학문이며, 시니어 엔지니어링 면접에서 가장 비중 있게 평가되는 역량 중 하나이기도 합니다. 이 레슨은 이후 과정 전반에 걸쳐 활용하게 될 기본 용어, 추정 도구 모음, 그리고 구조화된 사고 프레임워크를 제공합니다.
