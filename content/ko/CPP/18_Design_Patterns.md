...
**이전**: [C++20 심화](./17_CPP20_Advanced.md) | **다음**: [프로젝트: 학생 관리 시스템](./19_Project_Student_Management.md)
...
1. GoF 디자인 패턴의 목적과 분류(생성 패턴, 구조 패턴, 행동 패턴)를 설명할 수 있다
2. 모던 C++ 관용구를 사용하여 생성 패턴(싱글턴(Singleton), 팩토리 메서드(Factory Method), 빌더(Builder))을 구현할 수 있다
3. 구조 패턴(어댑터(Adapter), 데코레이터(Decorator), 퍼사드(Facade))을 적용하여 유연한 객체 계층 구조를 구성할 수 있다
4. 행동 패턴(옵저버(Observer), 전략(Strategy), 커맨드(Command), 템플릿 메서드(Template Method))을 사용하여 가변적인 행동을 캡슐화할 수 있다
5. 정적 다형성을 위한 CRTP와 컴파일 방화벽을 위한 PIMPL을 포함한 C++ 특화 관용구를 활용할 수 있다
6. 모던 C++에서 패턴 기반 접근법과 람다/함수 기반 접근법 사이의 트레이드오프를 평가할 수 있다

---

디자인 패턴은 학문적 연습이 아니라, 비자명한 모든 C++ 프로젝트에서 반드시 마주치는 문제들에 대한 실전 검증된 해결책입니다. 팩토리(Factory)가 객체 생성을 단순화할 때, 옵저버(Observer)가 모듈을 분리할 때, 또는 CRTP가 가상 함수 호출 오버헤드를 제거할 때를 파악하면, 과도한 설계 없이도 유연한 아키텍처를 작성할 수 있습니다. 스마트 포인터, 람다, 템플릿 같은 모던 C++ 기능과 결합하면, 이 패턴들은 교과서 형태보다 더 가볍고 표현력 있게 됩니다.
...
정적 다형성(Static Polymorphism)을 구현하는 기법입니다.
...
- [15_C++17_Features](15_C++17_Features.md) - 모던 C++ 기능 복습
