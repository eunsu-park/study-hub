...
**이전**: [C++ 학습 가이드](./00_Overview.md) | **다음**: [변수와 자료형](./02_Variables_and_Types.md)
...
1. C++가 무엇인지 설명하고 주요 특징과 버전 역사를 파악한다
2. Windows, macOS, 또는 Linux에 C++ 개발 환경을 설치한다
3. g++를 사용하여 "Hello, World!" 프로그램을 구현하고 컴파일한다
4. `-std=c++17`, `-Wall`, `-Wextra` 등 자주 쓰는 컴파일러 옵션을 적용한다
5. 기본 입출력 연산에서 `std::cout`과 `std::cin`을 구분하여 사용한다
6. 입출력, 메모리 관리, 문자열 처리에서 C와 C++의 접근 방식을 비교한다
7. C++ 개발을 위해 VS Code 또는 Visual Studio를 설정한다

---

모든 프로그래밍 여정은 작동하는 툴체인에서 시작됩니다. 클래스, 템플릿, 표준 라이브러리를 탐험하기 전에, 소스 코드를 실행 파일로 변환하는 컴파일러와 코드를 편안하게 작성할 수 있는 편집기가 필요합니다. 이 레슨은 그 필수적인 첫걸음을 안내하고, 어떤 언어를 배우든 가장 뿌듯한 순간인 첫 번째 프로그램을 직접 실행하는 경험을 선사합니다.
...
[변수와 자료형](./02_Variables_and_Types.md)에서 C++의 변수와 자료형을 배워봅시다!
