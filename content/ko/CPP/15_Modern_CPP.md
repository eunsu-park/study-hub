...
**이전**: [스마트 포인터와 메모리 관리](./14_Smart_Pointers_Memory.md) | **다음**: [멀티스레딩과 동시성](./16_Multithreading_Concurrency.md)
...
1. C++11 핵심 기능인 `auto`, 범위 기반 for 루프, `nullptr`, 초기화 리스트(initializer list), 람다(lambda)를 사용할 수 있다
2. 이동 생성자와 이동 대입 연산자를 구현하여 이동 시맨틱(move semantics)을 적용할 수 있다
3. `constexpr`을 사용하여 표현식을 컴파일 타임에 평가할 수 있다
4. C++14에서 도입된 제네릭 람다(generic lambda)를 작성하고 반환 타입 추론을 활용할 수 있다
5. C++17 구조적 바인딩(structured bindings)으로 반환값을 분해하고, `if constexpr`로 컴파일 타임 분기를 사용할 수 있다
6. C++17 어휘 타입(vocabulary types)인 `std::optional`, `std::variant`, `std::string_view`를 활용할 수 있다
7. 컨셉(concepts), 레인지(ranges) 파이프라인, 우주선 연산자(spaceship operator), `std::format`을 포함한 C++20 기능을 사용할 수 있다

---

C++는 지난 10년간 그 이전 20년을 합친 것보다 더 많이 발전했습니다. C++11, 14, 17, 20 각 표준은 런타임 코드를 한 줄도 바꾸지 않고도 언어를 더 안전하고, 표현력 있게, 그리고 종종 더 빠르게 만드는 기능들을 도입했습니다. 이러한 기능을 이해하는 것은 선택이 아닙니다. 현대적인 라이브러리와 코드베이스는 이 기능들을 전제로 하며, 무시하면 더 많은 코드를 작성해야 하고 읽기 어렵고 버그가 발생하기 쉬운 코드가 됩니다.
