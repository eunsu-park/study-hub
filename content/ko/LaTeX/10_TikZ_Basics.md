...
TikZ(TikZ ist kein Zeichenprogramm - "TikZ는 그리기 프로그램이 아닙니다")는 벡터 그래픽을 프로그래밍 방식으로 생성하기 위한 강력한 LaTeX 패키지입니다. 외부 이미지를 가져오는 것과 달리 TikZ 그래픽은 코드를 사용하여 생성되므로 확장 가능하고 문서 타이포그래피와 일관성이 있으며 버전 제어가 쉽습니다. TikZ는 하위 레벨 그래픽 시스템인 PGF(Portable Graphics Format) 위에 구축되었으며 다이어그램, 차트, 플로차트, 기술 일러스트레이션 및 수학적 시각화에 널리 사용됩니다.
...
> **출력:** 밝은 회색 기준 격자가 있는 4×3 cm 캔버스. 왼쪽 아래에 파란색 윤곽선의 정사각형, 가운데에 빨간색 윤곽선의 원, 오른쪽에 녹색 윤곽선의 타원, 왼쪽 위에 보라색 윤곽선의 삼각형이 배치되어 있습니다. 각 도형 아래에는 이름이 레이블로 표시됩니다.
...
> **출력:** 상단에 "Start" 직사각형, 아래에 주황색 다이아몬드 "Check" 결정 노드, 좌우로 "Yes Path"와 "No Path" 직사각형이 분기되고, 하단에 "End" 직사각형이 있는 수직 플로차트. 두 분기 박스에서 나온 화살표가 L자 경로를 통해 "End"로 다시 연결되며, 결정 분기에는 "yes"와 "no" 레이블이 표시됩니다.
...
> **출력:** 밝은 회색 격자, 굵은 x·y 축(화살표 끝과 축 레이블 $x$, $y$), 각 축의 정수 눈금, 원점 $O$ 레이블이 있는 데카르트 평면. 함수 $0.5x^2 - x + 1$을 나타내는 부드러운 파란색 포물선이 $x=0$에서 $x=3.5$까지 그려져 있습니다.
...
> **출력:** "Input", "Process", "Output"이라고 표시된 세 개의 파란색 직사각형이 가로로 나란히 배치되어 왼쪽에서 오른쪽으로 실선 화살표로 연결되어 있습니다. 빨간색 점선 화살표가 "Process"에서 아래로 내려가 다시 왼쪽 "Input"으로 돌아가는 피드백 루프를 형성하여 제어 흐름 순환을 나타냅니다.
