...
`microtype` 패키지는 문서 전체에 걸쳐 미묘하지만 누적적인 개선을 가져옵니다:

- **문자 돌출(character protrusion)** (hanging punctuation): 쉼표나 마침표 같은 구두점이 오른쪽 여백을 약간 넘어 배치되어 시각적으로 더 반듯한 텍스트 가장자리를 만들어 줍니다.
- **글꼴 확장(font expansion)**: 문자 너비를 최대 ±2% 범위에서 조정하여 TeX이 더 나은 줄 바꿈 지점을 찾을 수 있도록 돕고, 결과적으로 overfull 또는 underfull 경고가 줄어듭니다.
- **하이픈 감소**: 글꼴 확장 덕분에 줄 바꿈 알고리즘에 더 많은 유연성이 생기므로 단어를 하이픈으로 나눌 필요가 줄어듭니다.
- **정렬 향상**: 돌출과 확장의 조합으로 단어 사이 간격이 더 균일해지고, 단락을 따라 흐르는 공백의 "강(rivers)" 현상이 사라집니다.

이러한 효과는 실제 PDF 렌더링에 의존하기 때문에 나란히 놓인 코드 예시로는 확인하기 어렵지만, 긴 단락의 양쪽 정렬 텍스트에서는 차이가 분명히 보입니다. `\usepackage{microtype}`를 사용하여 컴파일한 문서는 레이아웃 경고가 더 적고 더 전문적으로 조판된 것처럼 보입니다.
...
**이전**: [상호 참조 및 인용(Cross-References & Citations)](08_Cross_References.md)
