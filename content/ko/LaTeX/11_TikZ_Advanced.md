...
> **출력:** "Comparison of Activation Functions"이라는 제목의 12×8 cm 플롯으로, 중앙 축, 주요 격자선, 오른쪽 하단에 범례가 있습니다. 파란색 시그모이드(Sigmoid) S자 곡선, 빨간색 tanh 곡선(비슷한 모양이지만 더 가파름), 녹색 조각 선형 ReLU(음수 $x$에서는 0, 양수 $x$에서는 선형)가 정의역 $[-5, 5]$에서 y축 범위 $[-1.5, 1.5]$로 그려져 있습니다.
...
> **출력:** 세 열의 파란색으로 채워진 원으로 구성된 순방향 신경망 다이어그램. 왼쪽 열에는 입력 뉴런($x_1$, $x_2$) 2개, 가운데 열에는 은닉 뉴런($h_1$, $h_2$, $h_3$) 3개, 오른쪽 열에는 출력 뉴런($y$) 1개가 있습니다. 모든 입력 뉴런은 모든 은닉 뉴런과 연결되고, 모든 은닉 뉴런은 출력 뉴런과 연결되며, 화살표가 순방향을 나타냅니다. 각 열 위에는 레이어 이름이 표시됩니다.
...
> **출력:** $q_0$, $q_1$, $q_2$를 나타내는 세 개의 원이 가로로 나란히 배치되어 있습니다. $q_0$에는 왼쪽에서 들어오는 "initial" 화살표가 있습니다. $q_2$는 수락 상태를 나타내는 이중 원 테두리를 가집니다. "a" 레이블의 굽은 화살표가 $q_0$에서 $q_1$로, "b" 레이블의 굽은 화살표가 $q_1$에서 $q_0$으로 돌아오고, 직선 화살표 "c"가 $q_1$에서 $q_2$로 연결되며, "a,b,c" 레이블의 자기 루프가 $q_2$에 있습니다.
...
> **출력:** "Training Progress"라는 제목의 14×8 cm 플롯으로, x축에 에포크(1–10), y축에 손실/정확도(0–1), 주요 격자선, 플롯 영역 오른쪽 외부에 범례가 있습니다. 파란색 실선(원 마커)은 훈련 손실의 감소를, 빨간색 실선(정사각형 마커)은 약간 높은 검증 손실의 감소를, 녹색 실선(삼각형 마커)은 10 에포크 동안 0.6에서 0.96으로 상승하는 훈련 정확도를 보여줍니다.
