...
> **참고**: `\frontmatter`, `\mainmatter`, `\backmatter`는 `book` 클래스에서만 사용 가능합니다. 이 명령들이 필요하다면 `\documentclass{book}`(또는 `memoir`)을 사용하세요. `report` 클래스는 이 명령들을 제공하지 않습니다.
...
**참고**: `\frontmatter`, `\mainmatter`, `\backmatter`는 `book` 클래스(및 `memoir` 같은 파생 클래스)에서만 사용 가능합니다. `report` 클래스는 이 명령들을 제공하지 **않습니다**.
