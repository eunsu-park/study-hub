...
### 왜 풍상(Upwind) 스킴인가? 물리를 따라가기

이류 방정식(advection equation)은 유동 속도 c로 운반되는 양을 기술합니다. **정보는 유동 방향으로 전파됩니다** — 오른쪽으로 이동하는 파동은 상류(왼쪽)에 있는 것에만 의존합니다. 풍상 스킴은 상류 방향을 바라보는 편측 차분을 사용하여 이를 존중합니다:

- c > 0 (오른쪽 유동): 후방 차분(backward difference) (u_i - u_{i-1})/Δx 사용
- c < 0 (왼쪽 유동): 전방 차분(forward difference) (u_{i+1} - u_i)/Δx 사용

이것이 중심 차분(FTCS)이 순수 이류에서 실패하는 이유입니다: 양쪽을 동일하게 바라보며 유동의 방향성을 존중하지 않아 무조건적 불안정성을 초래합니다. 이류에 대한 FTCS의 증폭 인자는 |G|² = 1 + C²·sin²(kΔx) > 1로, 모든 파수(wavenumber)에서 — 스킴이 *항상* 모든 모드를 증폭합니다.
...
## 탐색
- 이전: [11. 라플라스/포아송 방정식](11_Laplace_Poisson.md)
- 다음: [13. CFD 기초](13_CFD_Basics.md)
