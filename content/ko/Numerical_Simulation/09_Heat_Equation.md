...
### 왜 r ≤ 0.5인가? 폰 노이만 안정성 분석(Von Neumann Stability Analysis)

열 방정식의 FTCS 스킴에서 안정성 파라미터는 r = α·Δt/Δx²입니다. 안정성 한계를 결정하기 위해 푸리에 모드(Fourier mode) u_j^n = G^n · e^{ikjΔx}를 이산 방정식에 대입하고 증폭 인자(amplification factor)를 구합니다:

```
G = 1 - 4r·sin²(kΔx/2)
```

안정성을 위해 |G| ≤ 1이 필요하며, 이는 0 ≤ 4r·sin²(kΔx/2) ≤ 2를 요구합니다. 최악의 경우는 sin² = 1 (최고 주파수 모드)이므로 **r ≤ 0.5**가 됩니다. 물리적으로 이는 시간 단계가 열이 격자 셀을 "건너뛰지" 않을 만큼 충분히 작아야 함을 의미합니다 — 수치 확산 속도가 물리적 확산 속도와 일치하거나 초과해야 합니다.
...
## 탐색
- 이전: [8. 유한차분법 기초](08_Finite_Difference_Basics.md)
- 다음: [10. 파동방정식](10_Wave_Equation.md)
