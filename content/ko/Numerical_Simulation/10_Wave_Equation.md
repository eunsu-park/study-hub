...
**파동방정식은 왜 열방정식과 다른 접근이 필요한가?** 열방정식은 **산일적(Dissipative)** -- 시간이 지남에 따라 교란을 평탄화하고 에너지가 단조 감소합니다. 파동방정식은 **분산적(Dispersive)** -- 교란을 감쇠 없이 전파하고 에너지가 보존됩니다. 이 근본적인 차이는 열에 잘 작동하는 수치 기법(고주파 모드를 감쇠시키는 암시적 방법)이 파동에는 불필요하며, 파동 진폭을 보존하고 수치 분산(Numerical Dispersion)을 최소화하는 기법이 필수적임을 의미합니다.
...
## 탐색
- 이전: [9. 열방정식](09_Heat_Equation.md)
- 다음: [11. 라플라스/포아송 방정식](11_Laplace_Poisson.md)
