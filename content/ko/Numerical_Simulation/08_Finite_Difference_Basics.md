...
쿠란트-프리드리히스-레비(Courant-Friedrichs-Lewy, CFL) 조건은 양시적 유한 차분법에서 가장 중요한 안정성 제약 조건입니다:

```
c · Δt / Δx ≤ 1     (쿠란트 수 C ≤ 1)
```

물리적 의미는 우아합니다: **수치적 의존 영역이 물리적 의존 영역을 포함해야 합니다**. PDE에서 정보는 속도 c로 전파됩니다. 한 시간 단계 Δt 동안 물리적 정보는 거리 c·Δt를 이동합니다. 수치 스텐실은 한 단계에서 Δx에 도달합니다. c·Δt > Δx이면 물리적 파동이 수치 격자를 앞지르고 스킴이 중요한 정보를 놓치게 되어 불안정성이 발생합니다.
...
## 탐색
- 이전: [7. PDE 개요](07_PDE_Overview.md)
- 다음: [9. 열방정식](09_Heat_Equation.md)
