...
1. 연립 ODE(Coupled ODE System)를 정식화하고 고차 ODE를 수치 적분을 위한 1계 시스템으로 변환할 수 있습니다.
2. Lotka-Volterra 포식자-피식자(Predator-Prey) 모델을 구현하고 분석하여 생태계 역학을 해의 관점에서 해석할 수 있습니다.
3. 비선형 진자(Nonlinear Pendulum) 운동을 시뮬레이션하고 선형화된 소각(Small-Angle) 근사와 비교할 수 있습니다.
4. 로렌츠 시스템(Lorenz System)을 구현하고 초기 조건에 대한 민감한 의존성이 혼돈(Chaotic) 거동으로 이어지는 방식을 설명할 수 있습니다.
5. 위상 평면(Phase Plane) 분석과 궤적 시각화를 적용하여 ODE 시스템의 정성적 거동을 해석할 수 있습니다.
...
**이 레슨이 중요한 이유:** 단일 변수 ODE는 고립된 과정을 모델링할 수 있지만, 자연계는 상호작용하는 양들로 가득합니다: 포식자와 피식자 개체수, 진자의 위치와 속도, 반응기의 온도와 농도. 연립 ODE 시스템은 이러한 상호작용을 포착하며, 그 해는 진동, 혼돈(Chaos), 패턴 형성 등 단일 방정식으로는 생성할 수 없는 창발 현상을 보여줍니다.

---

## 1. Lotka-Volterra (포식자-피식자)

Lotka-Volterra 모델은 순수한 상호작용 역학으로부터 지속적인 진동을 생성하는 가장 간단한 시스템 중 하나입니다. 본질적 진동이 없는 두 종이 결합만으로 주기적인 폭등-폭락 순환을 만들어냅니다. 이것은 비선형 피드백이 어떻게 복잡한 역학을 생성하는지 이해하기 위한 핵심 예제입니다.

### 1.1 모델 설명

Lotka-Volterra 방정식은 두 개의 연립 ODE로 포식자-피식자 상호작용을 모델링합니다:

$$\frac{dx}{dt} = \alpha x - \beta x y \qquad \text{(피식자 성장 - 포식)}$$
$$\frac{dy}{dt} = \delta x y - \gamma y \qquad \text{(포식자 성장 - 자연 사망)}$$

여기서 $x$는 피식자 개체수, $y$는 포식자 개체수, $\alpha$는 피식자 출생률, $\beta$는 포식률, $\gamma$는 포식자 사망률, $\delta$는 포식자 전환 효율입니다. 평형점 $(\gamma/\delta, \alpha/\beta)$은 중심(Center)이며, 이는 가까운 궤도가 닫힌 곡선이라는 것을 의미합니다 -- 시스템은 감쇠나 증폭 없이 영구적으로 진동합니다.
...
## 탐색
- 이전: [5. ODE 고급 방법](05_ODE_Advanced.md)
- 다음: [7. PDE 개요](07_PDE_Overview.md)
