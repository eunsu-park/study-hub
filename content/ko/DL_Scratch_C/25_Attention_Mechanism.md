# 25. 어텐션 메커니즘 (Attention Mechanism)

**이전**: [레이어 정규화](./24_Layer_Normalization.md) | **다음**: [KV 캐시](./26_KV_Cache.md)

---

## 학습 목표

이 레슨을 완료하면 다음을 할 수 있습니다:

1. C로 멀티헤드 셀프 어텐션(MHA) 순전파 구현
2. 자기회귀(autoregressive) 언어 모델링을 위한 인과적 마스킹(causal masking) 적용
3. 수치 안정성을 고려한 스케일된 내적 어텐션 구현
4. 저장된 Q, K, V, 어텐션 가중치를 사용하여 어텐션 역전파 구현
5. 샘플 시퀀스에 대해 MHA 출력을 PyTorch와 수치적으로 검증

---

## 1. 멀티헤드 어텐션 개요

```
입력:  X [N, T, d_model]

1단계: Q, K, V로 프로젝션
  Q = X × W_Q  [N, T, d_model]
  K = X × W_K  [N, T, d_model]
  V = X × W_V  [N, T, d_model]

2단계: h개 헤드로 분할, 각 헤드 크기 d_head = d_model / h
  Q_i = Q[:, :, i*d_head:(i+1)*d_head]   [N, T, d_head]  (i = 0..h-1)

3단계: 헤드별 스케일된 내적 어텐션
  A_i = softmax(Q_i × K_i^T / √d_head + mask) × V_i
  mask[t, s] = -inf if s > t (인과적: 미래 토큰 참조 불가)

4단계: 헤드 연결(concatenate)
  concat(A_0, ..., A_{h-1}) → [N, T, d_model]

5단계: 출력 프로젝션
  output = concat × W_O + b_O
```

---

## 2. QKV 프로젝션

```c
// qkv_forward: 단일 융합 행렬 곱으로 X를 Q, K, V로 프로젝션
// W_qkv: [3*d_model, d_model]  (융합 가중치 행렬)
// b_qkv: [3*d_model]
// qkv_out: [N*T, 3*d_model]
void qkv_forward(
    const float *X,      // [N*T, d_model]  (M = N*T)
    const float *W_qkv,  // [3*d_model, d_model]
    const float *b_qkv,  // [3*d_model]
    float       *qkv,    // [N*T, 3*d_model]
    int M, int d_model) {

    // qkv = X × W_qkv^T + b_qkv
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                M, 3 * d_model, d_model,
                1.0f, X,     d_model,
                       W_qkv, d_model,
                0.0f, qkv, 3 * d_model);
    // 바이어스 추가
    for (int m = 0; m < M; m++)
    for (int j = 0; j < 3 * d_model; j++)
        qkv[m * 3 * d_model + j] += b_qkv[j];
}
```

---

## 3. 스케일된 내적 어텐션

```c
// attention_forward: 단일 헤드 어텐션
// Q, K, V: [N, T, d_head]
// attn_w:  [N, T, T] — 역전파를 위해 저장되는 softmax 가중치
// output:  [N, T, d_head]
void attention_forward(
    const float *Q,      // [N, T, d_head]
    const float *K,      // [N, T, d_head]
    const float *V,      // [N, T, d_head]
    float       *attn_w, // [N, T, T] — 역전파용으로 저장
    float       *output, // [N, T, d_head]
    int N, int T, int d_head,
    int causal) {        // 1 = 인과적 마스크

    float scale = 1.0f / sqrtf((float)d_head);

    for (int n = 0; n < N; n++) {
        const float *Qn = Q + (long)n * T * d_head;
        const float *Kn = K + (long)n * T * d_head;
        const float *Vn = V + (long)n * T * d_head;
        float       *An = attn_w + (long)n * T * T;
        float       *On = output  + (long)n * T * d_head;

        // scores[T, T] = Q[T, d_head] × K^T[d_head, T] × scale
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                    T, T, d_head,
                    scale, Qn, d_head,
                           Kn, d_head,
                    0.0f, An, T);

        // 인과적 마스크: 상삼각(upper triangle)을 -inf로 설정
        if (causal) {
            for (int t = 0; t < T; t++)
            for (int s = t + 1; s < T; s++)
                An[t * T + s] = -1e9f;
        }

        // 마지막 차원에 대한 Softmax (쿼리 위치 t별)
        for (int t = 0; t < T; t++) {
            float *row = An + t * T;
            float max_v = row[0];
            for (int s = 1; s <= t || (!causal && s < T); s++)
                if (row[s] > max_v) max_v = row[s];
            float sum = 0.0f;
            int lim = causal ? t + 1 : T;
            for (int s = 0; s < lim; s++) {
                row[s] = expf(row[s] - max_v);
                sum += row[s];
            }
            for (int s = 0; s < lim; s++) row[s] /= sum;
            if (causal)
                for (int s = lim; s < T; s++) row[s] = 0.0f;
        }

        // output[T, d_head] = attn_w[T, T] × V[T, d_head]
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
                    T, d_head, T,
                    1.0f, An,  T,
                          Vn, d_head,
                    0.0f, On, d_head);
    }
}
```

---

## 4. 전체 멀티헤드 어텐션 순전파

```c
// mha_forward: 전체 멀티헤드 셀프 어텐션
// QKV 프로젝션 + 헤드별 어텐션 + 출력 프로젝션을 융합
void mha_forward(
    const float *X,      // [N, T, d_model]
    const float *W_qkv,  // [3*d_model, d_model]
    const float *b_qkv,  // [3*d_model]
    const float *W_o,    // [d_model, d_model]
    const float *b_o,    // [d_model]
    float       *output, // [N, T, d_model]
    // 역전파용으로 저장:
    float       *qkv_buf,   // [N*T, 3*d_model]
    float       *attn_w,    // [N, n_heads, T, T]
    float       *head_out,  // [N, n_heads, T, d_head]
    int N, int T, int d_model, int n_heads, int causal) {

    int M = N * T;
    int d_head = d_model / n_heads;

    // 1. 융합 QKV 프로젝션
    qkv_forward(X, W_qkv, b_qkv, qkv_buf, M, d_model);

    // 2. 헤드별 어텐션
    for (int h = 0; h < n_heads; h++) {
        // 헤드 h의 Q, K, V 추출 (qkv_buf에 인터리브되어 있음)
        // qkv_buf: [M, 3*d_model] = [M, n_heads*d_head * 3]
        // Q_h는 각 행의 첫 번째 d_model 원소 내 h*d_head 오프셋에 위치

        // 단순화를 위해 이 헤드의 Q/K/V를 연속된 버퍼로 복사
        float *Qh = malloc(M * d_head * sizeof(float));
        float *Kh = malloc(M * d_head * sizeof(float));
        float *Vh = malloc(M * d_head * sizeof(float));

        for (int m = 0; m < M; m++) {
            const float *row = qkv_buf + m * 3 * d_model;
            memcpy(Qh + m * d_head, row + h * d_head,           d_head * sizeof(float));
            memcpy(Kh + m * d_head, row + d_model + h * d_head, d_head * sizeof(float));
            memcpy(Vh + m * d_head, row + 2*d_model + h * d_head, d_head * sizeof(float));
        }

        // [N, T, d_head]로 재형성
        float *attn_wh = attn_w   + (long)h * N * T * T;
        float *head_oh = head_out + (long)h * N * T * d_head;

        attention_forward(Qh, Kh, Vh, attn_wh, head_oh,
                          N, T, d_head, causal);
        free(Qh); free(Kh); free(Vh);
    }

    // 3. 헤드 연결: [N, n_heads, T, d_head] → [N, T, d_model]
    float *concat = malloc(M * d_model * sizeof(float));
    for (int m = 0; m < M; m++)
    for (int h = 0; h < n_heads; h++)
    for (int j = 0; j < d_head; j++) {
        int n = m / T, t = m % T;
        concat[m * d_model + h * d_head + j]
            = head_out[((long)h * N + n) * T * d_head + t * d_head + j];
    }

    // 4. 출력 프로젝션: [M, d_model] × W_o^T + b_o
    cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                M, d_model, d_model,
                1.0f, concat, d_model,
                       W_o,   d_model,
                0.0f, output, d_model);
    for (int m = 0; m < M; m++)
    for (int j = 0; j < d_model; j++)
        output[m * d_model + j] += b_o[j];

    free(concat);
}
```

---

## 5. 인과적 마스크 시각화

```
T=4 시퀀스, 인과적 마스크:

점수 행렬 (마스크 전):            인과적 마스크 적용 후:
  pos  0    1    2    3              0    1    2    3
  0  [s00  s01  s02  s03]    0  [s00  -∞   -∞   -∞ ]
  1  [s10  s11  s12  s13]    1  [s10  s11  -∞   -∞ ]
  2  [s20  s21  s22  s23]    2  [s20  s21  s22  -∞ ]
  3  [s30  s31  s32  s33]    3  [s30  s31  s32  s33]

행 0 (토큰 0): 자기 자신에게만 어텐션
행 1 (토큰 1): 토큰 0과 1에 어텐션
행 3 (토큰 3): 이전 모든 토큰에 어텐션
```

---

## 6. 어텐션 역전파 (상위 수준)

어텐션 역전파는 softmax와 두 번의 행렬 곱을 통해 역전파해야 합니다:

```c
// attention_backward: 스케일된 내적 어텐션을 통한 역전파
// 주어진: dO [N, T, d_head], 저장된: Q, K, V, attn_w
// 출력: dQ, dK, dV [N, T, d_head]
void attention_backward(
    const float *dO,     // [N, T, d_head]
    const float *Q, const float *K, const float *V,
    const float *attn_w, // [N, T, T] — 저장된 softmax 출력
    float *dQ, float *dK, float *dV,
    int N, int T, int d_head, int causal) {

    float scale = 1.0f / sqrtf((float)d_head);

    for (int n = 0; n < N; n++) {
        const float *An  = attn_w + (long)n * T * T;
        const float *Qn  = Q  + (long)n * T * d_head;
        const float *Kn  = K  + (long)n * T * d_head;
        const float *Vn  = V  + (long)n * T * d_head;
        const float *dOn = dO + (long)n * T * d_head;
        float       *dQn = dQ + (long)n * T * d_head;
        float       *dKn = dK + (long)n * T * d_head;
        float       *dVn = dV + (long)n * T * d_head;

        // dV = A^T × dO   [T, d_head]
        cblas_sgemm(CblasRowMajor, CblasTrans, CblasNoTrans,
                    T, d_head, T,
                    1.0f, An,  T,
                          dOn, d_head,
                    1.0f, dVn, d_head);  // 누적

        // dA = dO × V^T   [T, T]
        float *dA = calloc(T * T, sizeof(float));
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                    T, T, d_head,
                    1.0f, dOn, d_head,
                          Vn,  d_head,
                    0.0f, dA, T);

        // Softmax 역전파: dS = A ⊙ (dA - Σ_j A_j dA_j)
        float *dS = malloc(T * T * sizeof(float));
        for (int t = 0; t < T; t++) {
            float dot = 0.0f;
            int lim = causal ? t + 1 : T;
            for (int s = 0; s < lim; s++) dot += An[t*T+s] * dA[t*T+s];
            for (int s = 0; s < lim; s++)
                dS[t*T+s] = An[t*T+s] * (dA[t*T+s] - dot);
            for (int s = lim; s < T; s++) dS[t*T+s] = 0.0f;
        }

        // dQ = dS × K × scale   [T, d_head]
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
                    T, d_head, T,
                    scale, dS, T, Kn, d_head,
                    1.0f, dQn, d_head);

        // dK = dS^T × Q × scale  [T, d_head]
        cblas_sgemm(CblasRowMajor, CblasTrans, CblasNoTrans,
                    T, d_head, T,
                    scale, dS, T, Qn, d_head,
                    1.0f, dKn, d_head);

        free(dA); free(dS);
    }
}
```

---

## 핵심 정리

- **QKV 프로젝션**: 단일 융합 행렬 곱 `X × W_qkv^T`로 Q, K, V를 동시에 생성
- **스케일된 내적**: d_head가 클 때 softmax에서 그래디언트 소실을 방지하기 위해 √d_head로 나눔
- **인과적 마스크**: softmax 전에 상삼각 항목을 -∞로 설정 → 자기회귀 생성
- 어텐션 역전파는 `Q, K, V, attn_w` 저장이 필요 — 메모리 비용이 O(N × T²)으로 증가
- FlashAttention (Lesson 41)은 어텐션 연산을 융합하여 이 메모리 비용을 O(N × T)로 줄임

---

**다음**: [26. KV 캐시](./26_KV_Cache.md) — 효율적인 자기회귀 생성을 위한 KV 캐시 사전 할당; 토큰당 메모리 사용량 분석; 캐시 추가 로직 구현.
