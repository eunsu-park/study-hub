...
| [영구 볼륨(Persistent Volumes)](./13_Persistent_Volumes.md) | ⭐⭐⭐ | 볼륨, 바인드 마운트, tmpfs, 명명/익명 볼륨 |
| [멀티 스테이지 빌드 패턴(Multi-Stage Build Patterns)](./14_Multi_Stage_Build_Patterns.md) | ⭐⭐⭐ | 멀티 스테이지 빌드, 빌더 패턴, 경량 이미지 |
| [Podman과 OCI(Podman and OCI)](./15_Podman_and_OCI.md) | ⭐⭐⭐ | OCI 표준, Podman 데몬리스/루트리스 아키텍처 |
| [컨테이너 디버깅(Container Debugging)](./16_Container_Debugging.md) | ⭐⭐⭐ | docker exec, docker logs, 로그 드라이버, 디버깅 |
...
### 5단계: 심화 학습
5. 영구 볼륨 → 멀티 스테이지 빌드 패턴 → Podman과 OCI → 컨테이너 디버깅
