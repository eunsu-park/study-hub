...
### 이론: 단계 격리 — 빌드, 테스트, 배포가 분리되는 이유

컨테이너 CI/CD 파이프라인은 "매 커밋마다 도는 스크립트와 같지 않습니다". **신뢰 경계(trust boundaries)**의 시퀀스입니다 — 각 단계가 다음 단계가 소비할 artifact를 만들고, 각 핸드오프마다 침해 기회가 있습니다.

`build && test && deploy`를 한 셸 프로세스에서 하는 파이프라인은 쓰기 빠르고 운영하기 위험합니다. 단계가 분리되는 이유 세 가지 —

- **다른 자격 증명.** 빌드 단계는 레지스트리 푸시 토큰 필요. 테스트 단계는 아무것도 필요 없음. 배포 단계는 클러스터 자격 증명 필요. 결합하면 모든 단계가 모든 자격 증명을 가짐 — 침해된 테스트 단계(아마 악성 의존성을 통해)가 클러스터 접근을 얻음.
- **다른 실패 모드.** 빌드 실패는 보통 결정적(컴파일 오류). 테스트 실패는 종종 flaky. 배포 실패는 인프라 관련. 분리하면 처음부터 다시 빌드하지 않고 맞는 것을 재시도할 수 있음.
- **다른 롤백 의미.** 빌드가 레지스트리에 푸시되면 불변. 배포가 성공하면 이전 불변 이미지로 롤백 가능. 이미지가 버전 단위, 파이프라인은 그것을 단계 사이에 옮기는 오케스트레이션만.

단계 사이의 artifact 핸드오프는 **이미지 다이제스트**입니다. 빌드가 `myapp@sha256:abcd...`를 푸시. 테스트가 정확히 그 다이제스트를 풀. 배포가 정확히 그 다이제스트를 참조하는 Kubernetes 매니페스트를 적용. "latest" 태그 없음, 파이프라인 중간에 태그가 재지정되는 경합 조건 없음.
...
### 이론: 파이프라인 트리거 — push, PR, tag, schedule, manual

모든 CI 시스템이 트리거를 구별하는 이유 — *올바른 응답*이 트리거에 따라 다르기 때문. GitHub Actions는 이를 `on:`으로 모델링.

| 트리거 | 일반 목적 |
|--------|-----------|
| `push` to main | 빌드, 이미지 푸시, 스테이징 배포 |
| `pull_request` | 빌드, 테스트 실행, 상태 체크 게시 — 이미지는 푸시 *안 함* |
| `push` of a tag (`v*`) | 릴리스 이미지 빌드, 프로덕션 레지스트리 푸시, 프로덕션 배포 |
| `schedule` (cron) | 야간 보안 스캔, 의존성 업데이트 PR |
| `workflow_dispatch` (manual) | 일회성 작업 — 롤백, 프로덕션 핫픽스, 데이터 마이그레이션 |

PR 빌드는 시크릿에 주의해야 합니다 — 포크에 쓰기 권한 있는 누구나 임의 코드의 PR을 제출할 수 있습니다. GitHub Actions의 기본 `pull_request` 이벤트는 포크의 PR에 시크릿을 노출하지 *않습니다*. `pull_request_target`은 노출하며, 사용자가 보안 트레이드오프를 깨닫지 못한 채 사용해서 많은 공급망 사고의 원인이 됩니다.
...
### 이론: 컨테이너 레지스트리 인증 — 토큰에서 OIDC로

모든 컨테이너 레지스트리 교환은 작은 OAuth2 토큰 춤을 돕니다.

1. 클라이언트(CI 러너)가 `ghcr.io/myorg/myapp:1.0`에 푸시 시도.
2. 레지스트리가 `WWW-Authenticate: Bearer realm=...,service=...,scope=repository:myorg/myapp:push,pull`로 `401 Unauthorized` 응답.
3. 클라이언트가 자기 자격 증명(사용자명/비밀번호, 정적 토큰, 또는 OIDC ID 토큰)을 auth realm으로 가져감.
4. Auth realm이 검증하고 정확한 `(repository, action)` 쌍에 스코프된 단명 베어러 토큰 반환.
5. 클라이언트가 `Authorization: Bearer <token>`으로 푸시 재시도.
6. 레지스트리가 토큰 검증하고 푸시 수락.

자격 증명 모델이 보안 자세를 결정합니다.

- **사용자명/비밀번호** — 장기, 종종 파이프라인 사이에 공유. 누설 쉬움. 침해 쉬움.
- **개인/서비스 액세스 토큰** — 장기, 특정 레포에 스코프. 더 나음, 하지만 여전히 CI 시크릿으로 저장해야 하는 정적 자격 증명.
- **OIDC 페더레이션(베스트 프랙티스)** — CI 공급자(GitHub Actions, GitLab CI, CircleCI)가 작업 런타임에 단명 OIDC ID 토큰을 발급, "이 실행은 이 레포, 이 브랜치, 이 워크플로에서 왔다"를 증명. 레지스트리(또는 AWS, GCP, Azure)가 특정 subject 패턴에 대해 CI 공급자의 OIDC 발행자를 신뢰하도록 구성. CI 구성에 장기 시크릿이 결코 존재하지 않음.

OIDC 패턴이 GitHub Actions의 `permissions: id-token: write`를 동작하게 합니다. 작업이 실행 정체성을 주장하는 GitHub 서명 JWT를 받습니다. AWS / GCP / Azure / Vault / 자체 레지스트리가 그 JWT를 워크플로가 선언한 정확한 범위의 클라우드 자격 증명으로 교환. CI 시크릿 스토어를 침해해도 그 안에 아무것도 없으니 아무것도 못 얻음.

### 이론: CI의 빌드 캐싱 — 레이어 캐시 대역폭 문제

CI 러너는 보통 신선합니다 — 모든 실행이 Docker 레이어 캐시 없이 시작. 순진한 `docker build`는 매 실행마다 모든 레이어 재실행 — 모든 의존성 재다운로드, 모든 것 재컴파일, 매 커밋마다. 그래서 CI 빌드가 악명 높게 느림.

복잡성 순서로 세 개선책 —

1. 비-Docker artifact용 **`actions/cache`** — `~/.npm`, `~/.cache/pip`, `~/.cargo`를 실행 사이에 캐시. 의존성 fetch 시간 절약.
2. **BuildKit의 `--cache-from` / `--cache-to`** + 레지스트리 백엔드. 빌드가 이미지와 함께 레이어 캐시를 레지스트리에 푸시, 다음 실행이 빌드 전에 그 캐시를 풀. `type=registry,ref=ghcr.io/myorg/myapp:buildcache`가 표준 주문.
3. GitHub Actions 전용 **`docker buildx` + `cache-to: type=gha`**. Actions 캐시 백엔드를 네이티브로 사용, 추가 레지스트리 오버헤드 없음.

캐싱이 구성되면 변경 없는 의존성 트리의 CI 빌드는 수 초 — 없으면 매 빌드가 처음부터 다운로드와 재컴파일. 차이는 팀 크기에 따라 복리로 — 개발자 10명이 매일 5번 빌드, 빌드당 10분 절약 = 매일 8+시간의 컴퓨트 회수.
...
전통 CD는 CI에서 푸시 — 파이프라인이 클러스터에 `kubectl apply`. **GitOps**는 이를 뒤집습니다. 클러스터 *내부*에서 도는 컨트롤러가 매니페스트의 Git 레포를 watch — 레포가 바뀌면 컨트롤러가 클러스터를 일치하도록 조정.

흐름 —

1. CI가 이미지 빌드, 레지스트리 푸시.
2. CI가 (보통 별도 Git 레포의) 배포 매니페스트를 새 이미지 다이제스트로 갱신.
3. ArgoCD나 Flux(클러스터 내 컨트롤러)가 매니페스트 커밋을 보고 풀, 적용.

푸시 기반 대비 장점 —

- **클러스터에 CI 자격 증명 없음.** 클러스터가 풀, 어떤 것도 안으로 푸시하지 않음. CI에 클러스터 admin 접근 필요 없음.
- **감사 로그 = git log.** 프로덕션 변경마다 커밋, 작성자/타임스탬프/메시지/diff. 롤백 = `git revert`.
- **드리프트 감지.** 컨트롤러가 클러스터 상태와 레포 상태를 끊임없이 비교. 외부 변경(kubectl edit, 수동 삭제)이 감지되어 되돌리거나 알림.
- **멀티 클러스터가 자연스럽게 스케일.** 같은 레포를 가리키는 자체 GitOps 컨트롤러를 가진 다섯 클러스터 — 모두 같은 상태로 수렴.

ArgoCD와 Flux가 두 지배적 구현이며 둘 다 CRD 기반 컨트롤러(레슨 08에서 다룸)입니다.
...
이미지의 모든 의존성이 잠재적 공격 벡터 — 베이스 이미지, OS 패키지, 언어 패키지(npm, pip, gem, ...), 애플리케이션 코드. **공급망 공격** 모델은 가장 약한 어디서나 사슬을 공격합니다.

- 인기 오픈소스 라이브러리에 악성 코드 주입, 하류 빌드가 풀하기를 기다림.
- 빌드 인프라를 침해해 그 외 깨끗한 이미지에 페이로드 조용히 삽입.
- 서명 키를 훔쳐 신뢰받는 이름으로 악성 이미지 게시.
- 레지스트리 계층에서 다른 이미지로 대체.

**SLSA(Supply-chain Levels for Software Artifacts)**는 방어를 형식화하는 프레임워크. 네 레벨 —

- **L1** — provenance 존재. 빌드 프로세스 문서화, 누가 무엇을 빌드했는지 앎.
- **L2** — provenance가 빌드 플랫폼에 의해 서명. artifact가 특정 빌드에서 왔음을 검증할 수 있음.
- **L3** — provenance가 변조 방지하는 강화된 빌드 플랫폼에서 옴. 빌드가 격리 환경에서 실행, 위조 불가능한 provenance.
- **L4** — 모든 변경의 양자 리뷰, hermetic 빌드(빌드 중 네트워크 접근 없음), 재현 가능한 빌드(같은 소스가 비트 동일한 출력).

실제로 클라우드 CI와 Sigstore(이미지 서명용 `cosign sign`, 서명된 attestation용 `cosign attest`)로 L2나 L3에 도달할 수 있습니다. **이미지 서명** + 클러스터 경계에서 미서명 이미지를 거부하는 **어드미션 정책**(`Connaisseur`, `Kyverno`, `OPA Gatekeeper`)이 배포 측 루프를 닫습니다.

`SBOM(Software Bill of Materials)` — 빌드 시 생성(`syft`, `trivy sbom`, `docker sbom`), 서명되어 이미지에 첨부 — 이미지를 열지 않고 "이 이미지에 뭐가 들었나?"에 답하게 해 줍니다. 다음 Log4Shell급 CVE가 떨어지면 레지스트리 전체의 SBOM을 질의해 영향받는 모든 이미지를 며칠이 아닌 몇 분 안에 찾을 수 있습니다.
