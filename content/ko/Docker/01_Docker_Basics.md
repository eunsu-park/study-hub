...
CLI 실습 전에 [**이론과 원리**](#이론과-원리) 섹션을 먼저 읽으세요. 컨테이너를 컨테이너답게 만드는 리눅스 커널 메커니즘(네임스페이스, cgroups, 유니온 파일시스템)과, `docker run` 한 줄이 실제 프로세스로 이어지는 dockerd / containerd / runc 구조를 다룹니다.
...
컨테이너는 새로운 종류의 가상 머신이 아닙니다. 호스트 커널 위에서 동작하는 평범한 프로세스(또는 작은 프로세스 그룹)에 다음 세 가지 리눅스 기능을 얹은 것일 뿐입니다.

- **(A) 네임스페이스(Namespaces)** — 프로세스가 *볼 수 있는* 자원(PID, 마운트, 네트워크 인터페이스, 호스트네임, IPC 객체, 사용자 등)을 격리합니다.
- **(B) cgroups(control groups)** — 프로세스가 *소비할 수 있는* 자원(CPU 시간, 메모리, 블록 I/O, PID 수)에 한도를 둡니다.
- **(C) 유니온 파일시스템(Union filesystem)** — 읽기 전용 이미지 레이어와 쓰기 가능한 최상단 레이어를 결합해, 수 GB짜리 파일을 복사하지 않고도 컨테이너마다 독립된 루트 파일시스템을 제공합니다.

이 세 가지를 체화하고 나면 Docker의 나머지는 이미지(패키지 포맷), 데몬(dockerd / containerd), 그리고 시스템 콜을 친근한 동사로 감싼 CLI에 불과합니다.

### A. 컨테이너 vs 가상 머신: 두 가지 격리 모델
...
### B. 리눅스 네임스페이스: 프로세스가 볼 수 있는 것을 격리
...
### E. Docker 엔진 스택: dockerd, containerd, runc
...
다음 섹션들은 위 메커니즘에 친근한 동사를 붙인 것에 불과합니다.
...
남은 본문은 실용 영역입니다. 아래 모든 명령은 결국 여기서 설명한 네임스페이스, cgroup, 유니온 파일시스템 연산으로 변환된다는 그림을 머릿속에 가지고 읽으세요.
