...
CLI 투어 전에 [**이론과 원리**](#이론과-원리) 섹션을 읽으세요. 이미지가 콘텐츠 주소화된 레이어 블롭(매니페스트 + config + 레이어 tarball)으로 구성되는 방식과, 레이어 캐시가 `docker pull`/`docker build`의 실제 작업량을 어떻게 결정하는지를 다룹니다.
...
### C. 매니페스트, Config, 그리고 멀티 아키텍처 인덱스
...
OCI image-spec이 이 모든 것을 표준화합니다 — JSON 스키마, 미디어 타입, 다이제스트 알고리즘. Docker 자체 포맷(Docker Image Manifest V2, Schema 2)은 OCI 명세와 거의 동일하며, 레지스트리는 둘을 호환되게 서빙합니다.

### D. 레이어 캐시: `docker pull`과 `docker build`이 자주 싸게 끝나는 이유
...
BuildKit(현대 빌드 엔진)은 레이어 시스템 *바깥에* 빌드 사이에 살아남는 마운트 기반 캐시(`RUN --mount=type=cache,target=/root/.cache/pip`)와, 독립 스테이지의 병렬 실행으로 이를 확장합니다.

### E. 컨테이너 라이프사이클: CLI가 숨기는 다섯 가지 상태
...
`docker run --rm`은 `start`에 종료 시 자동 `rm`을 체이닝합니다. `docker run -d`는 stdio를 붙이지 않습니다(컨테이너가 백그라운드로 돌아가지만 그 외 경로는 동일). `run = create + start`임을 알면, 이미지 빌드, 실패한 시작 디버깅(`create`와 `start`를 분리), `--rm`이 정확히 무엇을 지우는지에 대한 추론이 모두 명확해집니다.

### 이론에서 아래의 명령으로

- `docker pull <image>` — 매니페스트를 가져온 뒤, 로컬 스토어에 없는 레이어 블롭만 SHA256 기준 중복 제거되어 받아옵니다.
- `docker images` — 로컬 이미지 *매니페스트*와 각 태그가 가리키는 다이제스트를 나열합니다.
- `docker image inspect <image>` — **config JSON**을 덤프합니다 — env, cmd, entrypoint, 레이어 히스토리, 노출 포트.
- `docker history <image>` — config의 `history` 배열을 읽어, 어떤 Dockerfile 명령이 어떤 레이어를 만들었는지 재구성합니다.
- `docker run <image>` — `create`(쓰기 레이어 + 네임스페이스 + cgroup 할당)와 `start`(`clone() + execve()`)의 연결.
- `docker ps` — `running` 상태의 컨테이너만. `docker ps -a`는 `exited`, `paused` 등도 포함.
- `docker rmi <image>` — 매니페스트 참조를 제거합니다. 어떤 매니페스트도 참조하지 *않는* 레이어만 디스크에서 사라집니다.
- `docker system prune` — 참조 없는 레이어를 가비지 컬렉트합니다. CAS 모델은 그렇지 않으면 자체적으로 회수하지 않는 스토리지를 회수합니다.

이어지는 본문은 이 모델을 CLI로 다듬은 것입니다. 어떤 명령이 의외로 동작할 때마다 자문하세요 — 어떤 매니페스트, 어떤 config, 어떤 레이어, 어떤 쓰기 레이어를 건드리고 있는가?
