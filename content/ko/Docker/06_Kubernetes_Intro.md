...
- `kube-scheduler` — 스케줄되지 않은 각 Pod을 어느 노드에 돌릴지 결정(아래 스케줄러 이론 참고).
- `kube-controller-manager` — 내장 컨트롤러들을 돌림(Deployment 컨트롤러, ReplicaSet 컨트롤러, Node 컨트롤러, ...). 각각이 조정 루프.
...
#### 이론: 컨테이너가 아니라 Pod이 원자(atom)인 이유
...
결과 — **Pod은 휘발성이고 IP도 휘발성입니다.** 애완동물(pet)이 아닙니다. 재시작 시 IP가 바뀝니다. 안정적 네트워크 정체성이 필요한 것은 Service를 거칩니다.
