[← 이전: 6. Prefect 모던 오케스트레이션](06_Prefect_Modern_Orchestration.md) | [다음: 8. PySpark DataFrame →](08_PySpark_DataFrames.md)
...
Spark는 API 우선으로 가장 쉽게 학습됩니다 — `spark.read.parquet(...).filter(...).groupBy(...).count()`는 익숙한 pandas 스타일 체이닝처럼 보입니다. 그러나 API는 잡(job)이 30초 만에 실행되는지 3시간 만에 실행되는지를 결정하는 네 가지 아이디어를 숨깁니다: 계보 그래프(lineage graph)로서의 RDD, narrow vs wide 변환, DAG 스케줄러를 가진 지연 평가(lazy evaluation), 그리고 Spark를 MapReduce와 구별 짓는 인메모리 모델.

- **(A) 계보 그래프로서의 RDD** — 원래의 Spark 추상, 내결함성이 복제(replication)가 아닌 그래프에서 오는 이유
- **(B) Narrow vs wide 변환** — Spark 성능을 지배하는 셔플(shuffle) 경계
- **(C) 지연 평가와 DAG 스케줄러** — 변환은 계획을 만들고; 액션이 그것을 실행
- **(D) Spark vs MapReduce** — Spark가 중간 결과를 메모리에 유지했을 때 무엇이 바뀌었는가

### A. 계보 그래프로서의 RDD

RDD(Resilient Distributed Dataset)는 Spark가 구축된 핵심 추상입니다(이제 DataFrame이 권장 API이지만 RDD로 컴파일됨). 세 가지 속성:

- **분산(Distributed):** 데이터가 N개의 익스큐터에 파티셔닝됨. 200개 파티션의 1 TB RDD는 200개 익스큐터 각각에 대략 5 GB.
- **회복성(Resilient):** 파티션이 손실되면(익스큐터 죽음) Spark는 복제 없이 계보로부터 재계산.
- **불변(Immutable):** 변환은 새 RDD를 생성, 기존을 변형하지 않음.

#### A.1 계보 트릭

전통적 분산 시스템(HDFS, Cassandra)은 *복제(replication)* 로 내결함성을 달성: 모든 블록이 3대의 머신에 저장됨. 저장 비용: 3배.

Spark는 다르게 합니다. 각 RDD는 *어떻게 파생되었는지* — 부모 RDD에 적용된 함수 — 를 기억합니다. `rdd_3 = rdd_2.map(f)`의 파티션이 손실되면, Spark는 그것을 재구축하기 위해 `rdd_2`의 해당 파티션에 `f`를 다시 실행. `rdd_2`도 손실되면 재귀: `rdd_1`에서 `rdd_2` 재구축, 그 다음 `f` 재적용. 계보 그래프는 복구 계획입니다.

저장 비용: 1배(복제 없음, 소스 데이터만). 대가: 긴 계보의 경우 재계산이 비쌀 수 있으며, 그래서 `cache()` / `persist()`가 존재(§B에서 다룸).

#### A.2 5튜플

모든 RDD는:
1. 파티션 목록.
2. 부모로부터 각 파티션을 계산하는 함수.
3. 부모 RDD에 대한 의존성.
4. (선택) 파티셔너 — 키/값 RDD가 키별로 어떻게 파티셔닝되는지.
5. (선택) 각 파티션에 대한 선호 위치(데이터 지역성 힌트).

이 5개 필드는 어떤 RDD든 평가, 재계산, 스케줄하기에 충분합니다.

### B. Narrow vs Wide 변환

Spark에서 가장 성능 관련성 있는 구분.

#### B.1 Narrow 변환

각 출력 파티션이 **하나의** 입력 파티션에 의존. 예: `map`, `filter`, `mapPartitions`. 익스큐터 간에 데이터가 이동할 필요 없음 — 각 익스큐터가 자체 파티션을 독립적으로 변환.

```
입력 파티션      →  narrow op  →  출력 파티션
[A][B][C][D]                       [A'][B'][C'][D']
```

Narrow 변환은 *파이프라인됨* — `filter().map().filter()`이 인메모리에서 익스큐터 간 통신 없이 각 파티션에 대한 단일 패스로 실행됨.

#### B.2 Wide 변환 (셔플)

각 출력 파티션이 **모든** 입력 파티션에 의존. 예: `groupByKey`, `reduceByKey`, `join`, `distinct`, `repartition`. 데이터가 익스큐터 간 이동 — *셔플(shuffle)*.
...
셔플:
1. 각 입력 파티션이 셔플 키로 키된 출력을 작성.
2. 키별로 데이터가 정렬/해시됨.
3. 각 출력 파티션이 모든 입력 파티션에서 자신의 키 범위에 대한 모든 행을 pull.

셔플은 Spark에서 **그** 병목입니다. 디스크 쓰기, 네트워크 전송, 정렬/병합을 포함. 100 GB를 셔플하는 잡은 같은 데이터에 대한 동등한 map-only 작업보다 10배 더 걸릴 수 있습니다.

#### B.3 스테이지(Stage)와 셔플 경계

DAG 스케줄러는 모든 wide 변환에서 계보 그래프를 *스테이지* 로 분할합니다. 각 스테이지는 함께 파이프라인되는 narrow 변환의 시퀀스입니다. 스테이지 사이에 셔플이 있습니다.

Spark UI를 읽는 것은 대부분 스테이지를 세고 최소화하는 것입니다.

#### B.4 셔플 비용 줄이기

- **가능하면 `groupByKey`보다 `reduceByKey` 사용** — 셔플 전에 로컬에서 줄임.
- **조인을 위해 작은 테이블 브로드캐스트** — 작은 쪽을 모든 익스큐터에 보내, 큰 쪽의 셔플 회피.
- **사전 파티셔닝** — 같은 키로 두 RDD를 반복적으로 조인할 거라면 한 번 repartition하고 persist.
- **DataFrame 사용** — Catalyst 옵티마이저가 셔플을 자동으로 최소화하도록 쿼리를 다시 작성.

### C. 지연 평가와 DAG 스케줄러

Spark 변환은 즉시 실행되지 않습니다. *계획*(계보 그래프)을 만듭니다. 실행은 *액션* — `count`, `collect`, `save`, `show` — 이 호출될 때만 발생.

#### C.1 왜 지연(laziness)이 중요한가

```python
df = spark.read.parquet("...")              # 계획: scan
df2 = df.filter(col("amount") > 100)         # 계획: scan + filter
df3 = df2.select("user_id", "amount")        # 계획: scan + filter + project
df3.count()                                  # ACTION → 계획 실행
```

Spark는 실행 전에 전체 계획을 보기 때문에:

- **술어 푸시다운(Predicate pushdown).** `filter(amount > 100)`이 *Parquet 읽기에 푸시* 되어, 매치되지 않는 row group은 전혀 읽히지 않음.
- **조기 프로젝션.** `user_id`와 `amount` 컬럼만 디스크에서 읽힘; 다른 50개 컬럼은 스킵.
- **재정렬.** 조인 전에 필터, 작은 쪽 브로드캐스트 등.

지연이 없으면 각 오퍼레이터가 다음이 시작되기 전에 자신의 전체 출력을 구체화해야 합니다. 지연이 최적화를 가능하게 만드는 것입니다.

#### C.2 Catalyst 옵티마이저

DataFrame 연산은 Catalyst를 거칩니다 — 4단계 규칙 기반 + 비용 기반 옵티마이저:

1. **분석(Analysis):** 컬럼명을 스키마에 대해 해결.
2. **논리적 최적화:** 규칙 적용(predicate pushdown, 상수 폴딩, projection pruning).
3. **물리적 계획:** 조인 알고리즘 선택(broadcast vs shuffle hash vs sort merge).
4. **코드 생성:** Tungsten이 물리적 계획을 타이트한 루프를 위한 JVM 바이트코드로 컴파일.

결과: 관용적 DataFrame 코드는 종종 손으로 작성한 JVM 코드의 2배 이내이며, RDD 경로가 Catalyst를 우회하므로 동등한 RDD 연산보다 훨씬 빠릅니다.

### D. Spark vs MapReduce

전임자 Hadoop MapReduce는 잔인하게 단순한 실행 모델을 가졌습니다: 모든 잡은 1개 Map 스테이지 + 1개 Reduce 스테이지이며, 그 사이의 중간 결과는 *HDFS에 작성됨*. 다단계 워크플로우는 N개의 MapReduce 잡 시퀀스, 각각 스테이지 사이에 디스크에 작성.

Spark의 혁신:

- **한 잡 안의 다단계 DAG.** N개 잡을 체이닝할 필요 없음; Spark가 하나로 계획.
- **인메모리 중간 결과.** 스테이지 출력이 메모리에 살거나(또는 메모리 압박 시에만 디스크로 spill).
- **복제 대신 계보 기반 복구.** 저장 3배가 아닌 1배.

성능 이득은 주로 HDFS 왕복 회피에서 옵니다. (각 스테이지 후) 4번 HDFS에 쓴 5단계 파이프라인이 끝에 한 번만 쓰는 5단계 파이프라인이 됩니다 — 자릿수 단위 더 빠름.

신화: "Spark는 인메모리이기 때문에 빠르다." 현실: Spark는 중간 결과를 영구 저장소에 쓰지 않기 때문에 빠릅니다. 익스큐터의 작업 집합이 메모리를 초과할 때 Spark는 로컬 디스크로 spill하고도 여전히 MapReduce보다 훨씬 빠른데, 로컬 디스크가 HDFS보다 빠르기 때문입니다.

### From Theory to the Code Below

이어지는 각 절은 위 프레임워크의 한 조각을 운영합니다:

- §1 (Spark 개요)는 §D MapReduce 비교와 생태계.
- §2 (Spark 아키텍처)는 §A — 파티셔닝된 RDD를 실행하는 익스큐터 모델.
- §3 (RDD vs DataFrame vs Dataset)은 §A와 Catalyst(§C.2) 이야기.
- §4 (DataFrame 변환)는 구체 API에서의 §B — narrow op는 파이프라인, wide op는 스테이지를 야기.
- §5 (지연 평가)는 §C 명시적.
- §6 (구성 및 데이터 소스)는 Catalyst가 술어를 푸시하는 실용적 I/O 계층.
