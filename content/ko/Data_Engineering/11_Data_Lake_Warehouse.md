...
분석 저장의 이야기는 두 극단 사이의 왕복입니다: *웨어하우스*(엄격한 스키마, 비싼 컴퓨트, ACID, SQL만)와 *레이크*(유연한 파일, 저렴한 저장, 트랜잭션 없음, 개방형 컴퓨트). 산업 진화의 각 단계는 이전의 실제 한계에 응답했습니다. 차이를 외우는 것보다 각 아키텍처가 *왜* 이겼고 그다음 대체되었는지 이해하는 것이 더 유용합니다.

- **(A) 웨어하우스-레이크-레이크하우스 진화** — 각각이 무엇을 다루었고, 무엇을 미해결로 남겼는가
- **(B) Schema-on-write vs schema-on-read** — 다른 시점에 비용이 지불되는 중심 트레이드오프
- **(C) 테이블 포맷: Delta Lake, Iceberg, Hudi** — 객체 스토리지 위에 ACID 트랜잭션이 어떻게 존재하는가
- **(D) 메달리온(bronze/silver/gold) 원칙** — 정제 수준과 멱등 재구축 의미론
- **(E) 저장 경제학** — 클라우드 가격 모델이 레이크하우스 아키텍처를 선호하는 이유

### A. 웨어하우스-레이크-레이크하우스 진화
...
### C. 테이블 포맷: Delta Lake, Iceberg, Hudi
...
- §1 (데이터 레이크)는 §A.2 — 레이크의 약속과 함정.
- §2 (데이터 웨어하우스)는 §A.1 — 웨어하우스의 강점.
- §3 (레이크하우스)는 §A.3 + §C — 종합과 그것을 가능하게 하는 테이블 포맷.
- §4 (저장 포맷 비교)는 실용적 Parquet/Delta/Iceberg/Hudi 선택.
- §5 (클라우드 솔루션)은 §A 파도를 특정 제품(Snowflake, BigQuery, Databricks 등)에 매핑.
- §6 (아키텍처 패턴)은 §D — 실무에서의 메달리온.
