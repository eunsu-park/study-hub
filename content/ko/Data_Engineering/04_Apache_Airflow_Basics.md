[← 이전: 3. ETL vs ELT](03_ETL_vs_ELT.md) | [다음: 5. Airflow 심화 →](05_Airflow_Advanced.md)
...
스케줄러와 같은 프로세스에서 한 번에 하나의 태스크를 실행. 테스트에만 유용 — 병렬성 없음.

#### C.2 LocalExecutor

스케줄러와 같은 머신에서 서브프로세스를 fork. 병렬성 = 코어 수. 소형 배포에 좋음; 머신 간 분산 없음.

#### C.3 CeleryExecutor

전통적인 프로덕션 설정. Celery 브로커(Redis 또는 RabbitMQ)가 태스크 인스턴스를 큐잉; Celery 워커 fleet가 큐에서 가져와 실행. 워커는 long-lived 프로세스; 워커 추가로 확장.

- **장점:** 성숙, 잘 이해됨, 태스크당 오버헤드 낮음.
- **단점:** 워커는 stateful(Python 의존성이 사전 설치되어야 함); 태스크 간 자원 격리 제한적(나쁜 태스크 하나가 전체 워커를 OOM 시킬 수 있음); 확장에 용량 계획 필요.

#### C.4 KubernetesExecutor

각 태스크 인스턴스가 Kubernetes pod가 되며, Kubernetes에 의해 스케줄되고, 태스크가 끝나면 죽음.

- **장점:** 완벽한 자원 격리(각 태스크가 자체 컨테이너, 메모리 제한, CPU 제한); 태스크별 이미지를 통한 태스크별 Python 의존성; Kubernetes 클러스터 오토스케일러를 통한 자동 확장.
- **단점:** 태스크당 pod 시작 지연(10-30초) — 작은 태스크가 많을 때 나쁨; Kubernetes 운영 복잡도.

하이브리드: CeleryKubernetesExecutor는 혼합 가능 — 작은 빠른 태스크는 Celery, 무거운 격리 태스크는 Kubernetes.

#### C.5 익스큐터 선택

| 워크로드 | 익스큐터 |
|----------|----------|
| 로컬 개발 | SequentialExecutor 또는 LocalExecutor |
| 소형 프로덕션 (1머신, 적은 태스크) | LocalExecutor |
