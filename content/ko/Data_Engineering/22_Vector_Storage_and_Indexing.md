...
벡터 검색은 — "내 쿼리에 가장 가까운 K개 벡터 찾기" — 100만 벡터를 넘어 확장하고 brute-force 비교가 불가능하게 느리다는 것을 발견할 때까지 기만적으로 단순해 보입니다. 모든 흥미로운 벡터 데이터베이스 결정은 한 가지 근본적 긴장의 다운스트림입니다: **정확한(exact)** 최근접 이웃 검색이 고차원에서 증명 가능하게 어렵기 때문에("차원의 저주(curse of dimensionality)"), 프로덕션 시스템은 약간의 recall을 자릿수 단위 속도와 맞바꾸는 **근사(approximate)** 최근접 이웃(ANN, Approximate Nearest Neighbor) 알고리즘을 사용합니다.

- **(A) 차원의 저주와 brute force가 무너지는 이유** — 고차원 검색을 어렵게 만드는 것
- **(B) 거리 메트릭: cosine, Euclidean, dot product** — 각각이 측정하는 것과 각각이 옳은 시점
- **(C) 주요 ANN 알고리즘 가족** — IVF(inverted file), HNSW(hierarchical small-world graph), PQ(product quantization)
- **(D) Recall-vs-속도-vs-메모리 삼각형** — 모든 인덱스가 이 표면 어딘가에 살고 있음
- **(E) 대규모 벡터 저장 아키텍처** — 파티셔닝, 샤딩, 데이터 엔지니어링 문제로서의 임베딩 파이프라인
...
- §1 (벡터 검색 기초)는 §A와 §B — 거리 메트릭과 차원성 문제.
- §2 (FAISS)는 §C — IVF, HNSW, PQ를 구현하는 기초 라이브러리.
- §3 (벡터 데이터베이스)는 §E — 이 알고리즘들을 중심으로 빌드된 프로덕션 시스템.
- §4 (인덱스 선택)는 §D — 실무에서의 recall-속도-메모리 삼각형.
- §5 (필터링과 하이브리드 쿼리)는 §E.3 — pre-filter, post-filter, 필터 인덱스.
- §6 (벤치마크 비교)는 도구 간 구체적 recall/QPS/메모리 숫자 제공.
