[← 이전: 13. 데이터 품질과 거버넌스](13_Data_Quality_Governance.md) | [다음: 15. Airflow TaskFlow API →](15_Airflow_TaskFlow_API.md)
...
실용적 종단간 파이프라인은 이전 레슨의 모든 개념이 추상이기를 그치는 곳입니다. 도전은 더 이상 "Airflow가 무엇을 하는가"가 아니라 — "이 조각들이 어떻게 맞춰져서 시스템이 신뢰받는 데이터를 일정에 맞춰 생산하고, 실패에서 우아하게 복구하고, 그것을 작성하지 않은 엔지니어가 새벽 3시에 디버깅 가능한가"입니다. 네 가지 설계 원칙이 도구 스택을 실제 프로덕션 파이프라인으로 바꿉니다.

- **(A) 데이터 제품(Data Product) 사고** — consumer, SLA, 계약을 가진 제품으로서의 파이프라인
- **(B) 메달리온 + 오케스트레이션 아키텍처** — 저장 계층(레슨 11), 변환(레슨 12), 오케스트레이션(레슨 4-6) 결합
- **(C) 관측성(Observability): 데이터 파이프라인을 위한 세 기둥** — 메트릭, 로그, 계보; 무엇에 알림하고, 무엇을 로깅할지
- **(D) SLO와 사건 대응** — "파이프라인이 건강한가"를 정량적으로 정의, 그렇지 않을 때 응답

### A. 데이터 제품 사고

"파이프라인을 빌드했음"에서 "데이터 제품을 ship했음"으로의 시프트.

#### A.1 모든 출력 테이블에는 고객이 있음

어떤 사람이나 시스템이 각 gold 테이블에 의존 — 일일 리포트를 실행하는 분석가, 50명의 viewer가 있는 대시보드, 시간당 100만 요청을 서빙하는 ML 모델. 그 의존성이 당신의 고객 관계입니다.

데이터 제품은 다음을 가짐:
- **명확한 consumer**(이름이 있는 팀이나 시스템).
- **문서화된 SLA**("날짜 D의 데이터는 D+1의 6am UTC까지 가용, 99.5% 정시 전달").
- **스키마 계약**(컬럼 이름, 타입, 의미 — 그리고 안정성 약속).
- **소유자**(깨졌을 때 고치는 팀).

이것 없이는 모든 이슈가 "이건 누구의 문제인가?"가 됨. 이것과 함께 소유권이 명확하고 결정에 제약이 있음.

#### A.2 Producer-consumer 계약

Producer가 consumer에게 약속:
- "나는 이 테이블을 시간 T까지 전달할 것이다."
- "나는 N일 통지 없이 이 컬럼의 이름이나 타입을 변경하지 않을 것이다."
- "업스트림 소스가 변경되면 나는 적응할 것이다 — 당신의 계약은 유지된다."

Consumer가 producer에게 약속:
- "나는 사전에 선언한 컬럼만 읽을 것이며, 문서화되지 않은 것은 읽지 않을 것이다."
- "나는 문서화된 것 너머의 행 순서를 가정하지 않을 것이다."
- "나는 내가 의존하는 것을 알려서 당신이 변경을 계획할 수 있게 할 것이다."

이는 정신적으로 데이터 계약(레슨 21이 형식적 버전을 다룸)입니다. 비공식 버전조차도 팀 간 사건율을 극적으로 줄입니다.

### B. 메달리온 + 오케스트레이션 아키텍처

레슨 1, 4, 7, 10, 11, 12, 13을 결합하는 참조 아키텍처.
...
소스 (DB, API, 파일)
    ↓
수집 (Airbyte / Kafka / Spark) → bronze (원시 lake)
    ↓
정제 (Spark / dbt) ──────────────→ silver (타입됨, 검증됨)
    ↓
모델링 (dbt) ─────────────────────→ gold (비정규화, 비즈니스 준비됨)
    ↓
서빙 (BI 도구, ML 피처 저장소, API)
```

각 전이는 오케스트레이션됨(Airflow, Dagster)이고 품질 게이트됨(Great Expectations, dbt 테스트).

#### B.2 오케스트레이션 척추

Airflow / Dagster / Prefect는 단지 스케줄하는 것이 아님 — 다음을 소유:
- **트리거 로직.** 스케줄, 파일을 기다리는 sensor, webhook을 통한 이벤트 구동.
- **의존성 관리.** Bronze가 성공하기 전에 silver를 실행하지 않음; silver가 성공하기 전에 gold를 실행하지 않음.
- **재시도 정책.** 일시적 오류는 재시도; 지속적 오류는 알림.
- **백필.** 3일 된 버그를 찾으면 그 3일을 결정적으로 재생.
- **SLA 모니터링.** "이 DAG는 6am까지 끝나야 함; 그렇지 않으면 알림."

오케스트레이터는 운영 두뇌입니다. 의도적으로 선택하세요.

#### B.3 계층별 도구 선택

| 계층 | 일반적 도구 | 이유 |
|------|-------------|------|
| 수집 (배치) | Airbyte, Fivetran, 커스텀 Python | 단순, 잘 이해됨, 벤더가 connector 유지보수 처리 |
| 수집 (스트리밍) | Kafka + Connect, Debezium (CDC) | 낮은 지연, 재생, 로그 기반 정확성 |
| 무거운 변환 | Spark | 분산, >100 GB를 효율적으로 처리 |
| SQL 변환 | dbt | 버전 관리, 테스트, 계보 추적된 SQL |
| 오케스트레이션 | Airflow, Dagster, Prefect | DAG 관리, 스케줄, 재시도, 관측성 |
| 품질 | Great Expectations, dbt 테스트 | 선언적 expectation |
| 저장 (원시) | S3/GCS + Parquet | 저렴, 다중 엔진 읽기 가능 |
| 저장 (큐레이션) | Delta / Iceberg / Snowflake / BigQuery | ACID, 성능, SQL 접근 |

이는 "유일한 방법"이 아닙니다 — 그러나 2024년 시대 대부분 회사를 위해 수렴된 스택입니다.

### C. 관측성: 데이터를 위한 세 기둥

#### C.1 메트릭: 숫자로서의 파이프라인 건강

측정할 것:
- **파이프라인 지연.** 실행당 종단간 시간의 P50, P95.
- **파이프라인 성공률.** 재시도 없이 완료된 실행의 %.
- **데이터 신선도.** 테이블당 `now() - max(updated_at)`.
- **행 수.** 일일·테이블별; 7일 평균에서 >50% 편차에 알림.
- **테스트 통과율.** 실행당 통과하는 GE/dbt 테스트의 %.

이것들은 Prometheus / DataDog / CloudWatch로 감. 파이프라인별로 대시보드. SLO를 깨뜨릴 때 알림.

#### C.2 로그: 무엇이 일어났는가, 자세하게

태스크당 / 스테이지당 구조화된 로그(JSON). 포함:
- 타임스탬프.
- 논리 날짜(이 실행이 처리하는 날짜).
- 스테이지당 행 수(입력 행, 출력 행).
- 스테이지당 지속 시간.
- 오류 컨텍스트(전체 stack trace, 샘플 실패 행).

로그는 중앙 로그 집계자(Elastic, CloudWatch Logs, Datadog Logs)로 감. run_id와 logical_date로 검색 가능.

#### C.3 계보: 무엇이 무엇에 의존하는가

계보가 데이터 엔지니어링 특화 세 번째 기둥입니다. 메트릭 이상이 발화하면 "어떤 테이블이 이 메트릭을 피드하는가?"를 묻고 편차가 시작된 곳을 찾기 위해 계보를 위로 걸어 올라감. 계보 없이 이는 시간 단위의 수동 SQL 고고학.

현대 카탈로그 도구(DataHub, OpenLineage)는 dbt와 Spark에서 자동으로 계보 캡처.

### D. SLO와 사건 대응

#### D.1 SLO 정의

파이프라인을 위한 Service Level Objective:

- **신선도:** "날짜 D의 데이터는 D+1의 6am UTC까지 가용, 30일 롤링 윈도우에서 99% 시간."
- **완전성:** "행 수가 7일 평균의 5% 이내, 99% 시간."
- **정확성:** "모든 dbt 테스트 통과, 99.5% 시간."

각 SLO는 오류 예산을 가짐(일일 파이프라인의 경우 1% = ~7시간/월). 그것보다 빠르게 예산을 태우면 기능 작업을 멈추고 신뢰성에 집중.

#### D.2 사건 플레이북

SLO가 깨질 때 on-call 엔지니어는 다음을 필요로 함:

1. **컨텍스트와 함께 알림.** "파이프라인 X가 스테이지 Y에서 실패; 논리 날짜 Z; 마지막 성공 실행은 T 전; consumer SLA가 영향받음."
2. **Runbook 링크.** "흔한 원인: A(수정: ...), B(수정: ...); 둘 다 적용되지 않으면 팀 Z로 escalate."
3. **계보 뷰.** "이 다운스트림 테이블/대시보드/모델이 차단되거나 오래되었음."
4. **백필 계획.** 버그가 수정되면 놓친 실행을 재생하는 단계.

플레이북 없이 모든 사건이 처음부터 재발명됨. 하나가 있으면 on-call이 더 차분하고 빠름.

#### D.3 Postmortem

모든 의미 있는 사건 후 postmortem 작성:
- 무엇이 일어났는가(타임라인).
- 근본 원인("X 실패"가 아닌 실제 기저 이슈).
- 감지(어떻게 알았나? 알림이 적절했나?).
- 완화(무엇을 했나? 얼마나 걸렸나?).
- 예방(코드, 프로세스, 또는 도구의 어떤 변경이 재발을 방지할 것인가?).

비난 없음. 목표는 다음 번에 같은 클래스의 실패를 더 잘 처리하는 시스템.

### From Theory to the Code Below

이어지는 각 절은 위 프레임워크의 한 조각을 운영합니다:

- §1 (프로젝트 개요)는 §A — 데이터 제품, 그 consumer, 그 SLA 정의.
- §2 (아키텍처 설계)는 §B.1 — 적용된 메달리온 + 오케스트레이션 형태.
- §3 (수집 계층 구현)은 §B.3 행 1 — 구체적 Airbyte / Kafka / Spark 패턴.
- §4 (변환 계층 빌드)는 §B.3 행 3-4 — Spark와 dbt의 협력.
- §5 (품질과 모니터링)은 §C와 §D — SLO를 가진 계기된 파이프라인.
- §6 (프로덕션 배포)는 통합: CI/CD, 환경 분리, 관측성 스택.
