[← 이전: 0. 개요](00_Overview.md) | [다음: 2. 데이터 모델링 기초 →](02_Data_Modeling_Basics.md)
...
역할과 도구 둘러보기에 들어가기 전에, 모든 데이터 시스템에 반복적으로 등장하는 몇 가지 아키텍처 개념을 먼저 정리해 둡시다. "파이프라인", "웨어하우스", "레이크", "람다(Lambda)", "카파(Kappa)" 같은 용어는 단순한 명명이 아니라 — 각각이 동일한 세 가지 질문에 대한 다른 답을 인코딩하고 있습니다: 데이터는 어디에 저장되는가, 변환은 언제 수행되는가, 소비자는 어떤 일관성(Consistency)을 보게 되는가.

- **(A) 단계의 방향성 그래프(DAG)로서의 파이프라인** — 추출, 변환, 적재(Extract, Transform, Load), 그리고 재시도를 안전하게 만드는 멱등성(Idempotency) 원칙
- **(B) 저장 계층: 웨어하우스, 레이크, 레이크하우스** — schema-on-write vs schema-on-read, 그리고 메달리온(Medallion) 패턴(bronze/silver/gold)
- **(C) Lambda vs Kappa** — 배치(Batch)와 스트리밍(Streaming)을 결합하는 두 가지 방식, 그리고 업계가 대체로 Kappa 스타일 스트리밍 + 배치 재처리(Reprocessing)로 수렴한 이유
- **(D) 배치 vs 스트리밍 트레이드오프** — 지연 시간(Latency), 처리량(Throughput), 비용으로 이루어진 단일 삼각형

### A. 단계의 방향성 그래프로서의 파이프라인

데이터 파이프라인은 단계들의 DAG(Directed Acyclic Graph, 방향성 비순환 그래프)이며, 각 단계는 입력을 소비하고 출력을 생산합니다. 정형화된 분해는 *추출(Extract, 소스에서 가져오기)*, *변환(Transform, 정제·조인·집계)*, *적재(Load, 싱크에 기록)* 이지만, 더 깊은 진실은 그래프의 모든 단계가 동일한 형태를 가진다는 것입니다: 입력을 읽고, 작업하고, 출력을 쓴다.

#### A.1 왜 선형 스크립트가 아니라 DAG인가

선형 스크립트(`extract.py && transform.py && load.py`)는 다음 순간에 무너집니다:

- 같은 변환을 먹이는 소스가 두 개 이상 있을 때
- 한 변환이 여러 싱크로 팬아웃(fan-out)될 때
- 5단계의 실패 때문에 1-4단계를 다시 하지 않고 5단계만 재실행해야 할 때

DAG는 의존성을 명시적으로 만들고, 스케줄러가 이미 완료된 단계를 건너뛸 수 있게 하며, 독립적인 분기의 병렬 실행을 가능하게 합니다. 모든 현대적 오케스트레이터(Airflow, Prefect, Dagster)가 파이프라인을 DAG로 모델링하는 이유입니다 — 레슨 4-6 참조.

#### A.2 멱등성: 파이프라인의 가장 중요한 원칙

파이프라인 단계가 *멱등(Idempotent)* 하다는 것은 같은 입력으로 두 번 실행해도 한 번 실행한 것과 동일한 출력이 나온다는 뜻입니다. 이는 실패가 필연적이기 때문에 중요합니다 — 워커가 죽고, 네트워크 끊김으로 연결이 망가지고, 오케스트레이터가 재시도합니다. 단계가 멱등하면 재시도가 안전하고, 그렇지 않으면 재시도가 데이터를 오염시킵니다.

세 가지 패턴이 단계를 멱등하게 만듭니다:

1. **날짜에 의한 결정적 파티셔닝.** "어제의 데이터를 `orders_2024_03_15` 파티션에 적재한다." 재실행하면 같은 파티션을 덮어쓰고, 다운스트림은 동일한 최종 상태를 봅니다.
2. **Insert 대신 Upsert(Merge).** PostgreSQL의 `INSERT ... ON CONFLICT DO UPDATE`나 데이터 웨어하우스의 `MERGE`를 사용합니다. 두 번째 실행은 행이 이미 있는 것을 발견하고 중복 대신 업데이트합니다.
3. **콘텐츠 주소 기반 출력(Content-addressed output).** 입력을 해시(Hash)하고, 그 해시의 파일이 이미 있으면 쓰기를 건너뜁니다. ML 피처(Feature) 파이프라인에서 흔합니다.

피해야 할 패턴: `INSERT INTO sales SELECT * FROM raw_sales WHERE date = today()`. 두 번 실행하면 모든 행이 두 번 들어옵니다.

#### A.3 단계 경계에서의 원자성(Atomicity)

단계는 완전히 완료되거나, 부분 상태를 남기지 않습니다. 구체적으로: 임시 위치에 쓴 다음, 쓰기가 성공했을 때만 최종 위치로 원자적으로(atomically) 이름을 바꿉니다. HDFS/S3에서는 `_SUCCESS` 마커이고, SQL 웨어하우스에서는 트랜잭션이며, Delta Lake / Iceberg에서는 테이블 커밋 로그입니다. 단계의 출력을 보는 리더는 *전체* 를 보거나 *아무것도* 보지 못하며, 절반만 쓰여진 파일을 보지 않습니다.

### B. 저장 계층: 웨어하우스, 레이크, 레이크하우스

세 가지 저장 아키텍처가 분석(Analytics) 워크로드를 지배합니다. 이들은 *언제* 스키마가 강제되는지와 *어떤* 컴퓨트가 이들에 대해 실행될 수 있는지에서 차이가 납니다.

#### B.1 데이터 웨어하우스 (schema-on-write)

분석에 튜닝된 관계형 데이터베이스: 컬럼 스토리지(Columnar Storage), MPP(Massively Parallel Processing) 실행, 술어 푸시다운(Predicate Pushdown)을 수행하는 옵티마이저. 예: Snowflake, BigQuery, Redshift.

- **스키마는 쓰기 시점에 강제됨.** 데이터를 적재하려면 정의된 테이블 스키마에 맞아야 하며, 잘못된 행은 거부되거나 격리(Quarantine)됩니다.
- **SQL이 유일한 인터페이스.** 분석가에게는 좋지만 비정형 데이터(이미지, 로그, JSON)에는 불편합니다.
- **저장과 컴퓨트의 결합(과거) vs 분리(현대).** Snowflake/BigQuery는 저장과 컴퓨트를 분리하므로 컴퓨트를 독립적으로 확장할 수 있습니다.

#### B.2 데이터 레이크 (schema-on-read)

원시 파일을 담는 블롭 스토어(S3, GCS, ADLS): Parquet, JSON, CSV, 이미지, 무엇이든. 예: AWS S3 + Glue 카탈로그, Databricks lake.

- **쓰기 시점에 스키마 없음.** 바이트를 던져 넣고, 소비자가 읽을 때 파싱합니다.
- **모든 컴퓨트가 읽을 수 있음.** Spark, Presto/Trino, Python 노트북, ML 학습 파이프라인.
- **저렴한 저장, 비싼 분석.** 인덱싱이나 테이블 포맷이 없으면 스캔이 풀파일(full-file)입니다.

레이크의 유연성은 비용을 동반합니다: 강제된 스키마가 없으면 쓰레기가 누적됩니다(소위 "데이터 스왐프(data swamp)"). ACID가 없으면 동시 라이터(concurrent writer)가 서로 손상시킵니다.

#### B.3 레이크하우스 (테이블 포맷이 있는 schema-on-read)

종합: 데이터를 저렴한 블롭 스토리지에 두되, ACID 트랜잭션, 스키마 진화(Schema Evolution), 시간 여행(Time Travel)을 제공하는 *테이블 포맷* 레이어(Delta Lake, Apache Iceberg, Apache Hudi)를 추가합니다. 예: Databricks Lakehouse, Snowflake Iceberg 테이블.

- **객체 스토리지 위의 ACID.** 트랜잭션 로그 파일(`_delta_log/`)이 각 커밋을 기록하며, 리더는 일관된 스냅샷(snapshot)을 봅니다.
- **쓰기 시점에 스키마 강제됨**(켜놓은 경우), 하지만 원시 파일을 던져 넣고 나중에 정제할 수도 있습니다.
- **SQL과 ML 컴퓨트 모두.** Spark/Trino가 읽는 동일한 테이블을 BI 도구가 쿼리합니다.

#### B.4 메달리온 아키텍처

레이크하우스 시대의 정제 수준 정리 관례:

- **Bronze:** 원시 적재 데이터, append-only, schema-on-read. 진실의 원천(source of truth); 절대 삭제하지 않음.
- **Silver:** 정제·중복 제거·조인되어 스키마에 맞춰진 데이터. 분석가가 바로 쓸 수 있는 상태.
- **Gold:** 비즈니스 집계·비정규화되어 특정 소비(대시보드, ML 피처)에 최적화된 데이터.

각 계층은 이전 계층에서 읽고 bronze로부터 독립적으로 재구축 가능합니다. 이것이 레이크하우스 파이프라인의 현대적 운영 패턴입니다 — 레슨 11과 19에서 자세히 다룹니다.

### C. Lambda vs Kappa 아키텍처

배치와 스트리밍을 결합하는 두 가지 아키텍처 패턴. 선택은 팀 구조부터 모니터링까지 모든 것을 결정합니다.

#### C.1 Lambda 아키텍처

```
              ┌──── 배치 레이어 (Spark) ──── batch view ─────┐
source ──────┤                                              ├── serving layer (query)
              └──── 스피드 레이어 (Storm) ──── realtime view ┘
```

- **배치 레이어(Batch layer)** 는 전체 데이터셋(또는 큰 윈도우)을 처리하여 높은 정확도를 제공합니다.
- **스피드 레이어(Speed layer)** 는 최근 이벤트를 실시간으로 처리하여 낮은 지연 시간을 제공합니다.
- **서빙 레이어(Serving layer)** 는 쿼리 시점에 두 뷰를 병합합니다.

장점: 배치의 정확성과 스트리밍의 신선함을 결합. 단점: 동일 로직을 구현하는 두 코드베이스(배치 + 스트림) + 병합 로직. 운영 부담이 큽니다.

#### C.2 Kappa 아키텍처

```
source ──── 스트림 프로세서 (Flink/Spark Structured Streaming) ──── materialized view
                              ↑
                        replay from log
                        (Kafka retention)
```

단일 스트림 프로세서가 실시간과 과거 재처리(historical reprocessing)를 모두 처리합니다. 뷰를 "재구축"하려면 처음부터 스트림을 재생합니다(Kafka는 영구 보존 가능; 또는 레이크하우스에서 재생). 하나의 코드베이스, 하나의 운영 스토리.

장점: 단순함, 단일 진실의 원천. 단점: 스트림 프로세서가 고처리량 배치 스타일 재생을 처리할 수 있어야 하며, 일부 워크로드(수년치 데이터에 걸친 무거운 조인)는 여전히 야간 배치가 더 낫습니다.

#### C.3 어느 쪽이 이겼는가

실제로는 현대 스택은 Kappa-with-batch-supplement 쪽으로 기울어 있습니다: 저지연 뷰와 지속적 ETL은 스트리밍, 복잡한 집계와 과거 재구축은 스케줄된 배치(Airflow + Spark + dbt). Lambda 패턴은 레거시 시스템에 살아 있지만, 두 구현을 유지하는 비용이 대부분의 분석 사용 사례에서 지연 시간 이득을 초과하기 때문에 새 시스템에서는 거의 선택되지 않습니다.

### D. 배치 vs 스트리밍 트레이드오프

선택은 "스트리밍은 현대적, 배치는 오래된 것"이 아니라 — 의도적인 트레이드오프 삼각형입니다.

| 속성 | 배치 | 스트리밍 |
|------|------|----------|
| 지연 시간 | 분 ~ 시간 | 밀리초 ~ 초 |
| 비용당 처리량 | 매우 높음 (컴퓨트가 분산상각됨) | 낮음 (항상 켜진 워커) |
| 운영 복잡도 | 낮음 (cron + DAG) | 높음 (상태, 워터마크, exactly-once) |
| 재처리 | 단순함 (잡 재실행) | 복잡함 (로그에서 재생, 상태 관리) |
| 적합한 용도 | 분석, ML 학습, 리포트 | 알림, 대시보드, 실시간 피처 |

실용적 규칙: 기본은 배치를 쓰고, 지연 시간 요구가 진정으로 그것을 요구할 때만 스트리밍을 도입합니다. 스트리밍은 워터마크(이벤트 시간 vs 처리 시간), 상태 관리, 늦게 도착하는 데이터처럼 배치에는 없는 운영 부담을 누적시킵니다. 스트리밍 특화 이론(이벤트 시간, exactly-once, 윈도잉)은 레슨 10, 16, 17 참조.

### From Theory to the Tools Below

이어지는 각 절은 위 프레임워크의 한 조각을 운영합니다:

- §1 (데이터 엔지니어의 역할)은 §A의 DAG를 운영하는 인간 측면입니다.
- §2 (데이터 파이프라인 구성 요소)는 §A.1의 단계와 §A.2-3 멱등성/원자성 원칙을 따릅니다.
- §3 (배치 vs 스트리밍)은 §D를 구체적인 코드로 보여줍니다.
- §4 (데이터 아키텍처 패턴)은 §B(웨어하우스/레이크/레이크하우스)와 §C(Lambda/Kappa)를 배포 수준에서 다룹니다.
- §5 (도구 생태계)는 각 계층을 특정 도구에 매핑합니다(오케스트레이션은 Airflow, 컴퓨트는 Spark, 스트리밍은 Kafka, 변환은 dbt).
- §6 (모범 사례)는 §A.2 멱등성 + §A.3 원자성 + 재시도/관측성(Observability) 원칙을 구체화합니다.
