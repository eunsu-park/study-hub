...
스트림 처리는 "이벤트 읽고, 무언가 하기"의 단순한 프레이밍이 분산 시스템의 잔인한 현실을 만나는 곳입니다: 이벤트는 순서 없이, 늦게, 때로는 결코 도착하지 않음; 상태는 실패 시 잃지 않고 유지되어야 함; 같은 논리적 계산이 한 번 실행되든 백필을 위해 재생되든 동일한 결과를 생산해야 함. Kafka Streams는 가장 아키텍처적으로 흥미로운 답 중 하나인데, 스트림 프로세서를 Kafka *위에* 빌드 — Kafka를 내구성 상태, 전송, 내결함성 backbone으로 사용하기 때문입니다.

- **(A) Stream-table 이중성** — 모든 스트림이 테이블을 정의하고, 모든 테이블이 변경 스트림을 정의; 이 이중성이 중심 개념적 움직임
- **(B) 이벤트 시간 vs 처리 시간, 워터마크, 늦은 이벤트** — 배치 프로세서가 걱정하지 않는 시간 의미론
- **(C) 윈도잉(Windowing)** — tumbling, hopping, sliding, session — 그리고 각각이 무엇을 계산하는가
- **(D) 상태 저장소(State Store)와 changelog 토픽** — 상태 기반 연산이 충돌 후 어떻게 복구하는가
- **(E) ksqlDB: 스트림 위의 SQL** — Kafka Streams primitive 위의 더 높은 수준 추상
...
- §1 (스트림 처리 개념)은 §A와 §B — 이중성과 시간 의미론.
- §2 (Kafka Streams 토폴로지)는 §A.3 + §D — stateful 토폴로지 빌드.
- §3 (Faust: Python Kafka Streams)는 §2의 Python 동등물.
- §4 (윈도우 집계)는 §C — 구체적 코드의 tumbling / hopping / session.
- §5 (Stream-Stream과 Stream-Table 조인)은 §D.3 — co-partitioning과 조인 의미론.
- §6 (ksqlDB)는 §E — SQL 표면과 pull/push 쿼리 모드.
