...
>
> - **L - Local (지역)**: 현재 함수 내부
> - **E - Enclosing (감싸는 함수)**: 바깥 함수의 지역 변수
> - **G - Global (전역)**: 모듈 레벨에서 정의된 변수
> - **B - Built-in (내장)**: print, len, range, ...
...
> **비유:** 중첩 함수가 자신을 감싸는 스코프를 벗어날 때, 캡처한 변수들을 배낭(클로저)에 넣어 가지고 나갑니다. 배낭은 함수가 어디로 가든 함께 이동하므로, 외부 함수가 이미 반환된 후에도 그 변수들에 접근할 수 있습니다.
