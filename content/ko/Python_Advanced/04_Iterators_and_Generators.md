...
> **Iterable**은 **Iterator**를 포함
>
> - **Iterator**: `__iter__()` returns self, `__next__()` returns 다음 값 or raises StopIteration
...
> **제너레이터 실행 흐름**: `count_up_to(3)`
>
> 1. count = 1, `yield 1` --> 반환, 일시 정지
> 2. (next 호출) count = 2, `yield 2` --> 반환, 일시 정지
> 3. (next 호출) count = 3, `yield 3` --> 반환, 일시 정지
> 4. (next 호출) while 종료 --> StopIteration
