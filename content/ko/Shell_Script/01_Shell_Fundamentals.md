...
1. 기능, POSIX 준수성, 스크립팅 적합성 측면에서 bash, sh, zsh, fish 셸을 비교할 수 있습니다
2. POSIX 호환 구문을 식별하고 이식성을 해치는 bashism을 피할 수 있습니다
3. 로그인, 대화형(interactive), 비대화형(non-interactive), 제한(restricted) 셸 모드를 구별할 수 있습니다
4. 셸 호출 방식에 따른 시작 파일 로딩 순서를 추적할 수 있습니다
5. 종료 코드(exit code)를 설명하고 이를 활용해 견고한 조건부 로직을 구성할 수 있습니다
6. `set`과 `shopt` 옵션을 사용해 셸 동작을 설정할 수 있습니다
7. `/usr/bin/env`를 사용해 이식 가능한 shebang 줄을 작성할 수 있습니다

---

모든 셸 스크립트는 실행 환경(execution environment) 안에서 동작하며, 이 환경은 어떤 기능을 사용할 수 있는지, 어떤 설정 파일이 로드되는지, 오류를 어떻게 처리하는지를 결정합니다. 이러한 기초를 제대로 이해하지 못하면 로컬에서는 잘 동작하는 스크립트가 cron 작업, Docker 컨테이너, 또는 다른 시스템에서 실패하는 상황이 발생합니다. 이 레슨은 다양한 환경에서 예측 가능하게 동작하는 스크립트를 작성하기 위한 기초 지식을 제공합니다.

## 1. 셸의 종류

다양한 셸은 서로 다른 기능과 호환성 수준을 제공합니다. 이러한 차이점을 이해하는 것은 이식 가능한 스크립트를 작성하는 데 중요합니다.
...
| 시작 성능 | 중간 | 빠름 | 느림 | 중간 |
| 스크립팅 초점 | 예 | 예 | 예 | 아니오 |
| Debian/Ubuntu 기본값 | bash | dash (/bin/sh) | bash | 아니오 |
...
**fish**: 대화형 셸, 사용자 친화적, 이식 가능한 스크립트에는 적합하지 않음.
...
POSIX (Portable Operating System Interface)는 셸 동작에 대한 표준을 정의합니다. POSIX 호환 스크립트는 모든 POSIX 셸(sh, bash, dash, ksh 등)에서 실행됩니다.
...
셸은 호출 방식에 따라 다른 모드로 작동합니다. 이는 어떤 시작 파일을 읽을지에 영향을 줍니다.

### Login vs Non-Login 셸

**Login 셸**: 로그인할 때 시작됨(SSH, 콘솔 로그인, `bash --login`).

**Non-login 셸**: 기존 세션에서 시작됨(GUI에서 터미널 열기, bash에서 bash 실행).
...
### Interactive vs Non-Interactive 셸
...
bash가 설정 파일을 읽는 순서는 셸 모드에 따라 다릅니다.
...
**~/.bash_profile** (login 셸 진입점):

```bash
# ~/.bash_profile - login 셸이 로딩

# 사용자 바이너리를 위한 PATH 설정
export PATH="$HOME/bin:$HOME/.local/bin:$PATH"

# .bashrc가 존재하면 로딩 (대화형 login 셸용)
if [ -f ~/.bashrc ]; then
    . ~/.bashrc
fi
...
```bash
# ~/.profile - POSIX 호환 login 설정
# bash가 sh로 호출될 때, 또는 다른 POSIX 셸에서 사용
...
Cron 작업은 대화형 셸과 크게 다른 최소 환경에서 실행됩니다. 터미널에서 잘 동작하는 스크립트가 cron에서는 조용히 실패하는 경우가 많습니다.

**주요 차이점**:
- **최소 PATH**: Cron의 기본 PATH는 보통 `/usr/bin:/bin`이므로 `docker`, `node`, `/usr/local/bin`에 있는 명령을 찾을 수 없습니다.
- **profile 미소싱**: Cron은 `~/.bash_profile`, `~/.bashrc`, `/etc/profile`을 소싱하지 않습니다. 거기에 설정한 환경 변수는 사용할 수 없습니다.
- **터미널 없음**: TTY가 없으므로 대화형 프롬프트와 터미널 의존 명령이 실패합니다.
...
# cron 실패 디버깅을 위한 로그 출력
exec >> /tmp/my_cron_job.log 2>&1
echo "--- $(date) ---"

# 여기에 스크립트 로직 작성
```

**팁**: 축소된 환경으로 스크립트를 실행하여 cron 동작을 로컬에서 테스트할 수 있습니다:

```bash
env -i HOME="$HOME" /bin/bash /path/to/your/script.sh
```
...
셸 옵션은 셸 동작을 제어합니다. 두 명령으로 옵션을 관리합니다:
...
> **왜 `IFS=$'\n\t'`인가?**
>
> 기본 `IFS`(Internal Field Separator)는 공백, 탭, 개행입니다. 이로 인해 `for` 루프가 공백이 포함된 파일명을 별개의 단어로 분리하여 미묘한 버그를 발생시킵니다.
>
> ```bash
> # 버그: 기본 IFS가 "my file.txt"를 "my"와 "file.txt"로 분리
> for f in $(ls); do
>     echo "Processing: $f"
> done
> # 출력:
...
> ```
>
> strict mode 헤더에서 `IFS=$'\n\t'`를 설정하면 공백에 의한 단어 분할을 방지하여, 따옴표 없는 확장에서도 공백이 포함된 파일명과 문자열이 올바르게 처리됩니다.
...
- 실행 중인 셸 (bash, zsh, sh 등)
- login 셸인지 non-login 셸인지
- 대화형인지 비대화형인지
- 셸 버전
...
- login vs non-login 셸에서 로딩되는 순서 표시
- 각 파일의 첫 5줄 표시
...
다음을 수행하는 스크립트를 작성하세요:
- 현재 환경 변수를 파일에 저장
- 현재 셸 옵션(set -o, shopt -p)을 파일에 저장
