...
모든 소프트웨어 프로젝트에는 빌드, 테스트, 린트, 배포, 정리와 같은 반복적인 명령이 쌓입니다. 개발자들은 보통 Make, npm 스크립트, 또는 언어별 도구를 사용하지만, 이러한 도구는 의존성과 복잡성을 추가합니다. bash 기반 작업 실행기는 리포지토리에 단일 파일로 존재하고, bash가 설치된 곳이면 어디서든 동작하며, 작업을 일반 셸 함수로 정의할 수 있습니다 -- 이전 레슨에서 배운 의존성 해결, 병렬 실행, CLI 설계를 결합하는 이상적인 종합 프로젝트입니다.
