...
> **보안 경고**: 위의 `eval "$job"` 패턴은 작업 문자열이 신뢰할 수 없는 입력에서 오는 경우 위험합니다. `eval`은 임의 코드를 실행하여 명령 주입(command injection)을 가능하게 합니다(예: 작업 문자열에 포함된 `"; rm -rf /"`). 프로덕션 사용 시 명령 문자열 대신 함수 이름을 전달하거나, 적절히 이스케이프된 인수와 함께 `bash -c`를 사용하세요.
...
> **참고**: 트랩은 서브셸이나 백그라운드 작업에 상속되지 않습니다. 부모 스크립트에서 설정한 트랩은 `(subshell)`, `cmd &`, 파이프라인 구성 요소로 실행되는 명령에 적용되지 않습니다. 각 서브셸은 트랩이 기본 동작으로 초기화된 상태에서 시작됩니다. 백그라운드 작업에 정리가 필요한 경우 서브셸이나 백그라운드 함수 내부에서 명시적으로 트랩을 설정해야 합니다.
