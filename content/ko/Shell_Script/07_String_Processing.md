...
1. 내장 매개변수 확장(parameter expansion)을 사용하여 부분 문자열 추출, 치환, 대소문자 변환을 수행할 수 있습니다
2. `printf`를 사용하여 패딩, 정렬, 타입 안전 포맷을 포함한 형식화된 출력을 생성할 수 있습니다
3. `tr`을 사용하여 문자 변환(transliteration)과 삭제를 수행할 수 있습니다
4. 구분자 모드와 문자 범위 모드로 `cut`을 사용하여 구조화된 텍스트에서 필드를 추출할 수 있습니다
5. `paste`와 관계형(relational) `join`을 사용하여 파일을 열(column) 단위로 병합할 수 있습니다
6. `column` 명령어로 표 형식 출력을 정렬할 수 있습니다
7. `jq` 필터, 조건문, 파이프라인을 사용하여 JSON 데이터를 조회하고 변환할 수 있습니다
8. 내장 도구와 외부 도구를 결합하여 다단계 텍스트 처리 파이프라인을 구축할 수 있습니다
