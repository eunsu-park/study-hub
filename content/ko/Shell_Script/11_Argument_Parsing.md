...
1. `while`/`case` 루프를 사용해 짧은 옵션, 긴 옵션, 결합 옵션을 지원하는 수동 인수 파싱(manual argument parsing)을 구현할 수 있습니다
2. POSIX `getopts`를 적용해 조용한 모드(silent mode)에서 적절한 에러 처리와 함께 짧은 옵션을 파싱할 수 있습니다
3. `getopts`(POSIX)와 GNU `getopt`를 비교하고, 주어진 이식성 요건에 맞는 도구를 선택할 수 있습니다
4. NAME/SYNOPSIS/DESCRIPTION/OPTIONS 관례를 따르는 자기 문서화(self-documenting) 도움말 메시지를 작성할 수 있습니다
5. 터미널 기능 자동 감지 및 `NO_COLOR` 지원과 함께 색상 출력(color output)을 구현할 수 있습니다
6. 스피너(spinner), 진행 막대(progress bar), 다중 작업 대시보드를 포함한 진행 표시기(progress indicator)를 제작할 수 있습니다
7. 유효성 검사, 메뉴, 비밀번호 마스킹이 포함된 대화형 입력 프롬프트(interactive input prompt)를 설계할 수 있습니다
8. 인수 파싱, 색상 출력, 진행 표시를 결합해 전문적인 CLI 도구를 만들 수 있습니다
