...
`local` 내장 명령은 항상 0을 반환하여, 할당된 명령 치환의 종료 코드를 가립니다. 이는 `set -e`와 함께 사용할 때 무음 실패(silent failure)의 흔한 원인입니다.

```bash
#!/usr/bin/env bash
set -e

# 버그: local이 $(false)의 종료 코드를 가림
my_func() {
    local result=$(false)  # local이 0을 반환, set -e가 트리거되지 않음
...
# 수정: 변수를 먼저 선언한 후 별도로 할당
my_func_safe() {
    local result
    result=$(false)  # 이제 set -e가 실패를 올바르게 잡음
    echo "This never runs"
}

my_func_safe  # 스크립트가 여기서 에러와 함께 종료
```

> **경험 법칙**: `local`과 명령 치환을 한 문장에서 결합하지 마세요. 먼저 선언한 후 할당하세요.
