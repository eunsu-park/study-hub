...
### macOS / BSD Coreutils 차이점

Linux에서 작성한 스크립트가 macOS에서 깨지는 경우가 많습니다. macOS는 GNU coreutils가 아닌 BSD 계열 coreutils를 탑재하기 때문입니다. 다음 표는 가장 흔한 비호환성을 정리합니다:

| 명령 | GNU/Linux | macOS/BSD | 이식 가능한 대안 |
|---------|-----------|-----------|---------------------|
| `sed -i` | `sed -i 's/a/b/' file` | `sed -i '' 's/a/b/' file` | `sed -i'' -e 's/a/b/' file` 또는 임시 파일에 쓰기 |
| `date` (epoch) | `date -d @1609459200` | `date -r 1609459200` | `python3 -c` 또는 `awk` 사용 |
| `date` (상대) | `date -d '+1 day'` | 지원 안 됨 | macOS에서 `date -v+1d` 사용 |
| `stat` (파일 크기) | `stat -c%s file` | `stat -f%z file` | `wc -c < file` 사용 |
| `grep -P` (PCRE) | 지원됨 | 사용 불가 | `grep -E` 사용 또는 `ggrep` 설치 |
| `readlink -f` | 심볼릭 링크 완전 해석 | 지원 안 됨 | `realpath` 또는 커스텀 함수 사용 |
| `mktemp` | `mktemp /tmp/foo.XXXX` | 동일 문법 동작 | 둘 다 `mktemp -t prefix` 지원 |
| `find -printf` | 지원됨 | 지원 안 됨 | `find -exec stat` 사용 |
