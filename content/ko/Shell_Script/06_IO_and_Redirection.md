...
> **이식성 참고**: 프로세스 치환(`<(...)`과 `>(...)`)은 bash(및 zsh) 확장 기능입니다. **POSIX sh**, dash 또는 기타 최소 셸에서는 **사용할 수 없습니다**. POSIX 호환이 필요한 경우 명명된 파이프(FIFO)나 임시 파일을 대신 사용하세요. `#!/bin/sh`를 사용하는 스크립트에서는 이 문법을 완전히 피해야 합니다.
