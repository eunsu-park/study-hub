# Claude Cowork: AI 디지털 동료

**이전**: [10. Claude 데스크톱 애플리케이션](./10_Claude_Desktop.md) | **다음**: [12. 모델 컨텍스트 프로토콜 (MCP)](./12_Model_Context_Protocol.md)

---

Claude Cowork(간단히 "Cowork"라고도 함)는 Claude를 자율적인 **디지털 동료(digital colleague)**로 배포하는 Anthropic의 GUI 기반 제품으로, 코딩을 넘어 더 넓은 작업을 수행합니다. 커맨드 라인 도구가 필요 없는 비기술 사용자를 위해 설계되었으며, 시각적 인터페이스를 통해 다단계 작업을 Claude에게 위임할 수 있습니다. Claude Code가 터미널이나 IDE에서 코딩에 집중하는 반면, Cowork는 포인트 앤 클릭 인터페이스를 통해 더 독립적으로 운영되어 문서 생성, 연구 종합, 프로젝트 관리 작업, 플러그인 생태계와 MCP 커넥터를 통한 서비스 간 통합과 같은 다단계 워크플로를 처리합니다. 이 레슨에서는 Cowork가 무엇인지, 어떻게 작동하는지, 언제 사용해야 하는지를 다룹니다.

> **상태 (2026년 1월 기준):** Claude Cowork는 **리서치 프리뷰(Research Preview)**로 제공됩니다. 제품이 성숙함에 따라 기능과 역량이 변경될 수 있습니다.
...
1. [Claude Cowork란?](#1-claude-cowork란)
...
Claude Cowork는 Claude를 **디지털 동료**로 자리매김합니다 — 챗봇보다 더 자율적으로 작동하지만 잘 정의된 경계 내에서 운영되는 AI입니다. 커맨드 라인이 필요 없는 비기술 사용자를 위해 설계된 GUI 기반 도구로, 시각적 인터페이스를 통해 다단계 작업을 Claude에게 위임할 수 있습니다. 한 번에 하나의 프롬프트에 응답하는 대신, Cowork는 고수준의 목표를 받아 단계별로 분해하고, 사용 가능한 도구와 통합을 사용하여 해당 단계를 실행하며, 포괄적인 결과를 생성할 수 있습니다.
...
Claude Cowork는 현재 **리서치 프리뷰(Research Preview)**(2026년 1월부터)입니다. 다음을 통해 사용할 수 있습니다:
1. Claude Desktop 애플리케이션 (통합 모드로)
2. Claude.ai 웹 인터페이스 (Team 및 Enterprise 플랜)

리서치 프리뷰 단계이므로 일부 기능이 제한되거나 변경될 수 있습니다. 최신 가용성 세부 정보는 [Anthropic 문서](https://docs.anthropic.com/)를 확인하세요.
