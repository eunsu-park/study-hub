...
**학습 목표**:
- 에이전트 팀과 독립적인 서브에이전트의 차이 이해
- 팀 아키텍처 설명: 팀 리더(오케스트레이터) + 특화된 에이전트
- 공유 작업 목록을 통해 에이전트가 작업을 조율하는 방법 설명
- 복잡한 작업을 위한 에이전트 팀 설정 및 구성
- 팀원 역할과 기능을 적절하게 정의
- 실제 시나리오에 에이전트 팀 적용 (마이그레이션, 리뷰, 문서화)
- 멀티에이전트 워크플로의 제한사항 및 비용 영향 파악
- 에이전트 팀이 가치를 더하는 경우와 더 간단한 접근 방식으로 충분한 경우 결정
