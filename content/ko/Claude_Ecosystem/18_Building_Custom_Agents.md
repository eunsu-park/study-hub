...
**학습 목표**:
- 명확한 범위, 적절한 도구, 안전 제약을 갖춘 에이전트 설계
- JSON Schema 정의, 실행기(executor), 오류 처리를 포함한 커스텀 도구 구축
- 에이전트 동작을 안정적으로 안내하는 효과적인 시스템 프롬프트 작성
- 실제 사용 사례를 위한 4가지 완전한 에이전트 예제 구현
- 단위, 통합, 평가 수준의 에이전트 테스트
- 속도 제한(rate limiting), 비용 모니터링, 로깅, 보안을 갖춘 에이전트 배포
- 오케스트레이션 패턴을 활용한 멀티 에이전트 시스템 설계
