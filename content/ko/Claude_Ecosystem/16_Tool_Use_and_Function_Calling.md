...
1. 이름, 설명, JSON Schema 입력 사양으로 도구 정의하기
2. 전체 도구 사용 대화 흐름 구현하기 (요청, tool_use, tool_result, 응답)
3. Claude가 동시에 여러 도구를 요청할 때 병렬 도구 실행 처리하기
4. `tool_choice` 매개변수를 사용하여 Claude가 도구를 사용하는 방법과 시기 제어하기
5. 도구에서 오류를 반환하고 엣지 케이스 처리하기
6. 고급 패턴 적용하기: 체이닝, 구조화된 출력, 이미지 반환
7. 모범 사례를 따르는 효과적인 도구 설계하기
