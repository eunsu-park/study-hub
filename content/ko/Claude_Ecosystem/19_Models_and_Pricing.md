...
1. 지능, 속도, 비용, 기능 면에서 Claude 모델 계층 비교
2. 다양한 모델에서 API 사용 비용 계산
3. 프롬프트 캐싱(prompt caching)으로 최대 90% 비용 절감 구현
4. 시간에 민감하지 않은 작업에 배치 API(Batch API)로 50% 절감
5. 다중 계층 아키텍처를 위한 모델 선택 전략 설계
6. 일반적인 개발 및 프로덕션 워크플로우의 비용 추정
7. 개인 및 팀 사용에 적합한 구독 계획 선택
...
**Claude Opus 4.6** -- 최고의 추론 능력이 필요한 작업:
- 복잡한 다단계 수학 증명
...
**Claude Sonnet 4.6** -- 대부분의 개발 작업에 대한 기본 선택:
- 코드 생성 및 리팩토링
- 코드 리뷰 및 버그 식별
- 문서 요약 및 번역
- API 통합 및 보일러플레이트 생성
...
**Claude Haiku 4.5** -- 대용량, 지연 시간에 민감한 작업:
- 지원 티켓 또는 사용자 의도 분류
- 텍스트에서 구조화된 데이터 추출
- 알려진 콘텐츠에 대한 간단한 Q&A
- 데이터 검증 및 형식 지정
...
```
┌──────────────────────┬──────────────┬──────────────┬──────────────┐
│ 기능                 │Claude Opus4.6│ClaudeSonnet46│Claude Haiku45│
├──────────────────────┼──────────────┼──────────────┼──────────────┤
│ 지능 수준            │ 최고         │ 높음         │ 양호         │
│ 추론 깊이            │ ★★★★★       │ ★★★★☆       │ ★★★☆☆       │
│ 코딩 능력            │ ★★★★★       │ ★★★★☆       │ ★★★☆☆       │
...
Claude Sonnet 4.6과 Haiku 4.5는 200K 토큰 컨텍스트 창(약 150,000 단어 또는 500페이지 분량의 텍스트)을 가지며, Claude Opus 4.6은 1M 토큰 컨텍스트 창(약 750,000 단어 또는 2,500페이지)을 가집니다. 이를 효과적으로 사용하는 방법을 이해하는 것은 비용 관리에 매우 중요합니다.
...
# Sonnet/Haiku (200K) 또는 Opus (1M)의 경우:
#   - 입력 토큰 + 출력 토큰 <= 컨텍스트 창 크기
...
# 200K 컨텍스트 창(Sonnet/Haiku; Opus는 1M)에 수용 가능한 내용:
examples = {
    "짧은 프롬프트": 50,
...
print("200K 컨텍스트 창의 토큰 예산 (Sonnet/Haiku; Opus는 1M):")
...
├──────────────────┼──────────────┼──────────────┼───────────────────┤
│ 입력 토큰        │    $5.00     │    $3.00     │     $1.00         │
│ 출력 토큰        │   $25.00     │   $15.00     │     $5.00         │
├──────────────────┼──────────────┼──────────────┼───────────────────┤
│ 프롬프트 캐싱:   │              │              │                   │
│  캐시 쓰기       │    $6.25     │    $3.75     │     $1.25         │
│  캐시 읽기       │    $0.50     │    $0.30     │     $0.10         │
├──────────────────┼──────────────┼──────────────┼───────────────────┤
│ 배치 API:        │              │              │                   │
│  입력 토큰       │    $2.50     │    $1.50     │     $0.50         │
│  출력 토큰       │   $12.50     │    $7.50     │     $2.50         │
└──────────────────┴──────────────┴──────────────┴───────────────────┘
...
def cascade_query(
    ...
    """
    models = [
        ("claude-haiku-4-5", 1024),    # 가장 저렴한 것부터 시도
        ("claude-sonnet-4-6", 4096),      # 중간 계층으로 에스컬레이션
        ("claude-opus-4-6", 8192),        # 최종 에스컬레이션
    ]
