...
표준 오류 메시지만으로 충분하지 않을 때, SDK 수준의 디버그 로깅을 활성화하면 코드와 Anthropic API 사이의 전체 HTTP 요청/응답 트래픽을 확인할 수 있습니다.

**Python 및 TypeScript SDK** 모두 `ANTHROPIC_LOG` 환경 변수를 지원합니다:

```bash
# 전체 요청/응답 페이로드 — 심층 디버깅에 사용
export ANTHROPIC_LOG=debug

# 요청 요약만 표시 (메서드, URL, 상태, 지연 시간)
export ANTHROPIC_LOG=info

# 재시도 이벤트만 표시 (재시도 발생 시점 및 이유)
export ANTHROPIC_LOG=warn
```

`ANTHROPIC_LOG=debug`를 설정하면 SDK가 모든 HTTP 교환 내용을 stderr에 출력합니다. 여기에는 요청 헤더, API로 전송되는 전체 JSON 본문, 완전한 응답이 포함됩니다. 이를 통해 메시지 배열, 도구 정의, 시스템 프롬프트가 예상대로 직렬화되고 있는지 쉽게 확인할 수 있습니다.

```bash
# 예시: 셸 프로필을 변경하지 않고 단일 스크립트 실행 디버깅
ANTHROPIC_LOG=debug python my_script.py 2>debug.log
cat debug.log  # 캡처된 트래픽 검사
```

**주의**: 디버그 출력에는 전체 메시지 내용이 포함됩니다. 로그 파일을 커밋하거나 디버그 출력을 다른 사용자가 접근할 수 있는 위치로 리다이렉트하지 마세요. 민감한 프롬프트 데이터나 사용자 정보가 포함될 수 있습니다.

SDK가 아닌 Claude Code 자체에 대한 상세 로깅은 다음과 같이 활성화합니다:

```bash
claude --verbose   # 도구 호출, 훅 이벤트, MCP 메시지 표시
```
...
이 레슨으로 Claude Ecosystem 토픽의 핵심 문제 해결 역량이 완성됩니다. 이제 권한 오류와 훅 실패에서 컨텍스트 한계, MCP 문제, API 오류까지 가장 일반적인 문제를 진단하고 해결하기 위한 체계적인 참조서를 갖추었습니다. 가장 효과적인 Claude Code 사용자들은 좋은 프롬프팅 습관 (레슨 21)과 여기서 다룬 디버깅 기법을 결합하여 생산적이고 효율적인 개발 세션을 유지합니다.

세 가지 심화 레슨이 토픽을 더욱 확장합니다:

- **레슨 23 — 비전 에이전트**: 이미지 분석, 컴퓨터 사용, 스크린샷 워크플로우, 비전 기반 에이전트 패턴.
- **레슨 24 — 프롬프트 캐싱과 배치 API**: 캐시 중단점, 캐시 워밍 전략, 메시지 배치 API, 비용 최적화 기법.
- **레슨 25 — RAG 패턴**: 검색 증강 생성, 임베딩, 벡터 스토어, 하이브리드 검색 방식.

계속 학습하기 위해 새로운 시나리오를 만날 때마다 이전 레슨을 다시 방문하고, 위의 심화 레슨을 학습하며, Claude 생태계가 계속 발전함에 따라 Anthropic 문서를 최신 상태로 유지하세요.
