...
**학습 목표**:
- 네 가지 훅 타입과 각각이 실행되는 시점 이해
- 매처(matcher)와 명령어가 포함된 JSON 훅 설정 작성
- 훅 컨텍스트 데이터에 접근하는 환경 변수 사용
- 포맷팅, 린팅, 테스트, 알림을 위한 실용적인 훅 구축
- 훅이 실패하거나 예상치 못한 결과를 낼 때 디버깅
- 훅(결정론적)과 CLAUDE.md 지침(제안적)의 차이 구분
