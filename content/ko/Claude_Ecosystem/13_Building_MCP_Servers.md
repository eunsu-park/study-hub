...
1. TypeScript와 Python에서 MCP 서버 프로젝트를 스캐폴딩하고 구성할 수 있다
2. URI 템플릿과 동적 콘텐츠 핸들러로 리소스를 정의할 수 있다
3. JSON Schema 입력 유효성 검사와 핸들러 함수로 도구를 정의할 수 있다
4. 타입이 지정된 인수로 프롬프트 템플릿을 정의할 수 있다
5. 로컬(stdio)과 원격(HTTP/SSE) 배포를 위한 전송을 설정할 수 있다
6. MCP 인스펙터와 단위 테스트를 사용하여 MCP 서버를 테스트할 수 있다
7. MCP 서버를 배포, 보안 설정 및 게시할 수 있다
