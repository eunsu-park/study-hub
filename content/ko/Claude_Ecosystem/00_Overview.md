...
| 23 | [비전 에이전트](23_Vision_Agents.md) | ⭐⭐⭐⭐ | 이미지 분석, 컴퓨터 사용, 스크린샷 워크플로우, 비전 기반 에이전트 |
| 24 | [프롬프트 캐싱과 배치 API](24_Prompt_Caching_and_Batch_API.md) | ⭐⭐⭐ | 캐시 중단점, 캐시 워밍, 메시지 배치 API, 비용 최적화 |
| 25 | [RAG 패턴](25_RAG_Patterns.md) | ⭐⭐⭐⭐ | 검색 증강 생성, 임베딩, 벡터 스토어, 하이브리드 검색 |
...
**4단계 — API 및 심화 (레슨 15–25)**
프로덕션급 AI 애플리케이션 구축: Claude API와 에이전트 SDK 활용, 커스텀 도구 및 에이전트 생성, 비용 최적화, 고급 개발 워크플로우 마스터, 팀 모범 사례 및 문제 해결 학습. 심화 레슨에서는 비전 기반 에이전트(레슨 23), 비용 최적화를 위한 프롬프트 캐싱과 배치 API(레슨 24), 검색 증강 생성 패턴(레슨 25)을 다룹니다.
