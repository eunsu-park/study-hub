...
### 6.4 O'Neil의 비선형 Landau 감쇠

파동 진폭이 무한소가 아닐 때, 선형 Landau 감쇠만으로는 전체 이야기를 설명할 수 없습니다. **O'Neil (1965)**은 장기 거동이 선형 감쇠 시간 $\tau_L = 1/|\gamma_L|$과 파동 포텐셜에 포획된 공명 입자의 **bounce 주기** 사이의 경쟁에 의해 지배됨을 보였습니다.

**포획 (bounce) 주파수.** 파동 포텐셜 우물의 바닥 근처에 포획된 공명 입자는 포획 주파수로 진동합니다:

$$
\omega_{\text{tr}} = \sqrt{\frac{eEk}{m}} = \sqrt{\frac{eE_0 k}{m}}
$$

여기서 $E_0$는 파동 전기장 진폭, $k$는 파수, $e$와 $m$은 입자 전하와 질량입니다. 대응하는 bounce 주기는 $\tau_b = 2\pi/\omega_{\text{tr}}$입니다.

**두 가지 regime.**

- **선형 regime** ($|\gamma_L|\,\tau_b \gg 1$, 즉 $|\gamma_L| \gg \omega_{\text{tr}}$): 포획된 입자가 한 번의 bounce도 완성하기 전에 파동이 감쇠합니다. 속도 공간에서 분포 함수가 매끄러워지고 표준 선형 결과 $E(t)\propto e^{\gamma_L t}$가 적용됩니다.

- **비선형 포획 regime** ($|\gamma_L|\,\tau_b \lesssim 1$, 즉 $|\gamma_L| \lesssim \omega_{\text{tr}}$): 포획된 입자가 파동이 감쇠하기 전에 많은 bounce 진동을 완성합니다. 파동 진폭은 지수적으로 영으로 감소하지 **않습니다**. 대신, 에너지가 파동과 포획 입자 집단 사이에서 왔다 갔다 하며, 대략 $\omega_{\text{tr}}$에서 **전기장 진폭의 진동**을 만들어냅니다:
...
**물리적 그림.** 일단 포획되면, 공명 입자는 위상 공간에서 닫힌 궤도를 실행합니다 (6.2절의 고양이 눈 구조). 위상 공간 분포는 $\omega_{\text{tr}}$에서 회전하며, 번갈아 파동에 에너지를 돌려주고 흡수합니다. 많은 bounce 주기에 걸쳐 분포가 위상 혼합되고, 개별 bounce 주파수의 분산($\delta\omega_{\text{tr}} \propto$ 분리선으로부터의 거리)으로 인해 진동이 감쇠되며, 파동은 유한한 점근 진폭 — 영보다 훨씬 큰 값 — 에 정착합니다. 이것이 **비선형, 비소산 Landau 감쇠**의 특징입니다.

**실험적 특징.** 포획 진동은 빔-플라즈마 실험과 입자 내 셀(PIC) 시뮬레이션 모두에서 직접 관측되었습니다 (7절의 Python 코드 참조). 이는 또한 위에서 논의된 위상 공간 홀 (BGK 모드) 형성의 시드 메커니즘이기도 합니다.
