...
완전한 Boltzmann 충돌 적분은 Coulomb 상호작용에는 비실용적입니다. Coulomb 힘이 $1/r^2$로만 감소하기 때문에, 산란 단면적이 작은 산란 각도에서 대수적으로 발산하여 적분 $\int \sigma(\Omega)\,d\Omega$이 형식적으로 무한대가 됩니다. 더 중요하게는, 플라즈마에서 무수한 **먼 거리의 작은 각도 편향**의 누적 효과가 중성 기체 역학을 지배하는 드문 근접 충돌을 훨씬 능가합니다. Coulomb 대수 $\ln\Lambda \sim 10\text{–}20$은 약한 장거리 산란이 90° 충돌보다 얼마나 많은 자릿수로 많은지를 정량화합니다. 이러한 이유로, 완전한 Boltzmann 연산자는 **Fokker-Planck 근사**로 대체됩니다. 이 근사는 충돌당 속도 변화의 처음 두 moment — 체계적 마찰(항력)과 속도 공간에서의 확산 — 만을 유지합니다. 이는 $1/\ln\Lambda$의 주요 차수까지 정확하며, 모든 플라즈마 수송 이론의 표준 출발점입니다.
...
Neoclassical 수송은 **충돌성 매개변수**에 의존합니다:

$$\nu_* \equiv \frac{\nu}{\omega_b \epsilon^{3/2}}$$

여기서 $\nu$는 충돌 주파수이고, $\omega_b \sim v_{\text{th}} \epsilon^{1/2} / (qR)$은 포획 입자의 bounce 주파수이며, $q$는 안전 인자(safety factor), $R$은 주 반경입니다. 세 가지 regime과 그 수송 계수 스케일링은 다음과 같습니다:

**바나나 regime** ($\nu_* \ll 1$): 입자가 충돌에 의해 포획 집단에서 벗어나기 전에 많은 bounce를 완성합니다. 유효 반경 방향 스텝 크기는 바나나 폭 $\Delta_b \sim q\rho_L/\epsilon^{1/2}$으로, 고전적인 Larmor 스텝 $\rho_L$보다 훨씬 큽니다. 포획된 입자의 비율 $f_{\text{trap}} \sim \epsilon^{1/2}$만이 기여하여:

$$D_{\text{banana}} \sim f_{\text{trap}}\,\frac{\Delta_b^2}{\tau_{\text{coll}}} \sim \frac{q^2}{\epsilon^{3/2}}\,D_{\text{classical}}$$

고전적 수송 대비 향상: 인자 $q^2/\epsilon^{3/2}$ (전형적으로 $10^2$–$10^3$).

**고원 regime** ($\nu_* \sim 1$): 충돌 주파수가 bounce 주파수와 비교 가능합니다. 포획 및 통과 집단이 빠르게 교환됩니다; 주요 차수에서 수송 계수는 $\nu$와 $\epsilon$에 무관해집니다:

$$D_{\text{plateau}} \sim \frac{q^2}{\epsilon}\,D_{\text{classical}} \sim \frac{\pi}{2}\frac{q\rho_L^2 v_{\text{th}}}{R}$$

**Pfirsch-Schlüter regime** ($\nu_* \gg 1$): bounce당 많은 충돌이 발생하며, 포획 입자는 지속적으로 통과 상태로 산란됩니다. 잔류 neoclassical 향상은 토로이달 기하구조에서 $\nabla\cdot\mathbf{j}=0$을 유지하는 데 필요한 귀환 전류에서 발생하며 다음과 같이 스케일링됩니다:

$$D_{\text{PS}} \sim (1 + 2q^2)\,D_{\text{classical}}$$

큰 $q$의 경우 이는 잘 알려진 Pfirsch-Schlüter 확산 계수의 $q^2$ 인자를 제공합니다.

요약 표:

| Regime | $\nu_*$ | $D_{\text{nc}}/D_{\text{cl}}$ | 주된 운반체 |
|---|---|---|---|
| 바나나 | $\ll 1$ | $q^2/\epsilon^{3/2}$ | 포획 입자 |
| 고원 | $\sim 1$ | $\sim q^2/\epsilon$ | 둘 다 |
| Pfirsch-Schlüter | $\gg 1$ | $1 + 2q^2$ | 통과 입자 |

전형적인 토카막 매개변수는 전자를 plateau/banana regime에, 이온을 banana regime에 배치합니다.

### 5.4 Bootstrap 전류

주목할 만한 neoclassical 효과: 외부 전류 구동 없이 순전히 압력 구배에 의해 구동되는 **자가 생성 토로이달 전류**가 흐릅니다.

**물리적 기원.** 압력 구배가 있는 플라즈마에서 통과 입자와 포획 입자 집단은 서로 다른 전류를 운반합니다. 토러스의 고자기장(내측) 면에서 포획 입자는 방향이 바뀌어 통과 입자로 전환됩니다; 저자기장(외측) 면에서는 반대 과정이 일어납니다. 이 비대칭성은 통과 전자와 포획 전자 사이에 순 마찰력을 생성하고, 이것이 평행 전류 — bootstrap 전류 — 를 구동합니다. 이 메커니즘은 유한한 포획 비율($f_{\text{trap}} \sim \epsilon^{1/2}$)과 압력 구배 모두를 필요로 합니다.

**Bootstrap 전류 밀도.** 바나나 regime에서 주요 차수 표현식은:
...
여기서 $B_p$는 poloidal 자기장이고, $\alpha \approx 0.5$–$1.5$는 충돌성에 의존합니다. 전체 압력 $p = nT$ 및 압력 구배 길이 $L_p = -p/(dp/dr)$로 표현하면:

$$j_{\text{bs}} \approx -\frac{\epsilon^{1/2}}{B_p}\,\frac{p}{L_p}\,\hat{f}(\nu_*)$$

여기서 $\hat{f}(\nu_*)$는 바나나와 Pfirsch-Schlüter 극한을 매끄럽게 연결하는 크기 1 정도의 수치 인자입니다. regime에 걸쳐 유효한 널리 사용되는 근사 공식은:

$$j_{\text{bs}} \approx -\frac{\epsilon^{1/2}}{1+\epsilon^2}\,\frac{nT}{eB_p}\,\frac{d\ln p}{dr}$$

**Bootstrap 비율.** $f_{\text{bs}} = I_{\text{bs}}/I_p$를 정의합니다 (여기서 $I_p$는 전체 플라즈마 전류). ITER (Q=10 시나리오)의 경우:
- $f_{\text{bs}} \approx 20$–$30\%$ — 외부 전류 구동 전력 요구량을 상당히 감소시킵니다.

"하이브리드"와 "정상 상태" 고급 시나리오는 $f_{\text{bs}} \sim 50$–$100\%$를 목표로 하여, 변압기 작용 없이 완전한 비유도(DC) 운전을 가능하게 합니다. 높은 $f_{\text{bs}}$는 피크가 있는 압력 프로파일(큰 $\nabla p$)과 높은 $\beta_p$ (플라즈마 압력 대 poloidal 자기장 압력 비율)를 필요로 합니다.
