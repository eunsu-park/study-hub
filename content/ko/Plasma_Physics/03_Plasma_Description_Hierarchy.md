...
- N-체 모델에서 kinetic 모델, fluid 모델까지의 플라즈마 기술 계층 구조 이해
...
플라즈마는 여러 수준의 세밀도(detail)로 기술될 수 있으며, 정확도와 계산 가능성(computational tractability) 사이의 trade-off를 만드는 **모델 계층 구조**를 형성합니다:
...
이것은 **6N-차원** 동역학 시스템입니다. $N \sim 10^{20}$ 입자를 가진 일반적인 플라즈마의 경우, 이것은 완전히 다룰 수 없습니다(intractable).
...
이것이 무충돌 플라즈마 역학을 기술하는 **Vlasov-Maxwell 시스템**입니다.
...
자화된 플라즈마의 경우, kinetic theory에서 유도된 평행 및 수직 열 흐름과 점성 텐서를 사용합니다.
...
간단한 1D 플라즈마 진동에 대해 N-body, Vlasov, fluid 기술을 비교하겠습니다.
...
플라즈마 기술의 계층 구조는 다양한 세밀도 수준에서 플라즈마를 모델링하기 위한 체계적인 프레임워크를 제공합니다:

1. **N-body**: 모든 입자의 정확한 고전 역학, 실제 플라즈마($N \sim 10^{20}$)에는 다룰 수 없음.
...
각 수준이 언제 적절한지, 그리고 그것들 사이를 어떻게 탐색할지 이해하는 것은 효율적이고 정확한 플라즈마 모델링에 필수적입니다.
...
(c) 물리적으로, $\langle \delta f \delta \mathbf{E} \rangle$ 항은 무엇을 나타냅니까? 왜 약하게 결합된 플라즈마에서 무시될 수 있습니까?

### 문제 2: Moment Closure

전자 분포 $f_e(x, v, t)$를 가진 1D 정전기 플라즈마에 대해:
