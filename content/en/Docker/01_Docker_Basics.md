...
### Theory: Two Different Isolation Models
...
A container is, in concrete terms, a process — or a small group of processes — running on the host kernel with three Linux features layered on top: **namespaces** (isolating what the process can *see*), **cgroups** (limiting what the process can *consume*), and a **union filesystem** (giving each container its own root filesystem assembled from read-only image layers plus a writable top layer). The diagram below shows the architectural difference; the next section unpacks each of these mechanisms.
...
#### Theory: Union Filesystems — Layered Storage Without Duplication
...
#### Theory: Linux Namespaces — Isolating What a Process Sees
...
#### Theory: cgroups — Limiting What a Process Can Consume
...
### Theory: The Docker Engine Stack — dockerd, containerd, runc
...
Concretely, every command in this lesson is one of these mechanisms with a friendly verb in front:
