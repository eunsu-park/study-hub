...
### Theory: The Container Boundary — One Process or One Service?
...
The cost of an extra container is small (a few MB of memory, one cgroup, one namespace set). The cost of a shared lifecycle that should not be shared is large (you cannot scale, restart, or upgrade independently). This first example demonstrates the cleanest such split — Node API in one container, Postgres in another.

### Theory: Dependency Readiness vs Service Existence
...
### Theory: Image Choice — Base Image as a Trade-Off
...
The pattern most production teams converge on: multi-stage builds where the *builder* stage uses a heavy base (full Node, full Go, full JDK) and the *runtime* stage uses distroless or alpine. You get fast iterative builds and a small, hard-to-attack runtime image. This example demonstrates exactly that — `node:18-alpine` builds, `nginx:alpine` serves, only the latter ships.
...
### Theory: Configuration Surface — Environment Variables, Files, Secrets
...
### Theory: Networking Topology — Service-to-Service vs Public-Facing
...
### Theory: Where State Lives — and Where It Does Not
...
The classic mistake: putting Postgres data in a bind mount on macOS or Windows for "easy access," then watching the database get corrupted by host-OS file locking quirks. Use named volumes for databases unless you are explicitly running on Linux and need host-side access. This example uses two named volumes — one for MySQL data, one for WordPress uploaded files — exactly because both need to survive `docker compose down -v`-less restarts.
