...
### Theory: Control Plane vs Data Plane
...
- `kube-scheduler` — decides which node each unscheduled Pod runs on (see scheduler theory below).
- `kube-controller-manager` — runs the built-in controllers (Deployment controller, ReplicaSet controller, Node controller, ...). Each is a reconciliation loop.
...
### Theory: The Scheduler — A Two-Phase Filter and Score
...
### Theory: The Resource Model — Everything Is YAML in etcd
...
#### Theory: Why the Pod, Not the Container, Is the Atom
...
A consequence: **Pods are ephemeral and have ephemeral IPs.** They are not pets. The IP changes on restart. Anything that needs a stable network identity goes through a Service.
...
#### Theory: The Reconciliation Loop — Declarative State Made Real
...
#### Theory: Stable Names for Ephemeral Pods
