...
- **Different credentials.** The build stage needs a registry push token. The test stage needs nothing. The deploy stage needs cluster credentials. Combining them means every stage has every credential — a compromised test step (perhaps via a malicious dependency) gets cluster access.
- **Different failure modes.** Build failures are usually deterministic (compilation error). Test failures are often flaky. Deploy failures are infrastructure-related. Separating them lets you retry the right thing without rebuilding from scratch.
