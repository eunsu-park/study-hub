...
### Node-to-Segment-Tree Index Mapping

The table below illustrates HLD on a small tree where node 1 is the root, 2 is the heavy child of 1, 4 is the heavy child of 2, and 3 and 5 are light children:

| Tree Node | DFS Order (pos) | Chain Head | Chain Index | Segment Tree Index |
|-----------|----------------|------------|-------------|-------------------|
| 1 (root) | 0 | 1 | 0 | 0 |
| 2 (heavy child of 1) | 1 | 1 | 1 | 1 |
| 4 (heavy child of 2) | 2 | 1 | 2 | 2 |
| 3 (light child of 1) | 3 | 3 | 0 | 3 |
| 5 (light child of 2) | 4 | 5 | 0 | 4 |

Nodes 1, 2, and 4 form one heavy chain (head = 1, contiguous positions 0–2). Nodes 3 and 5 each start their own chain. A path query from node 4 to node 3 jumps across one light edge: it queries the segment [pos[3], pos[3]] = [3, 3] for node 3's chain, then the segment [pos[1], pos[4]] = [0, 2] for the chain containing node 4, accumulating results in O(log²N).
