...
## 8. Aho-Corasick: Multi-Pattern Matching

### Motivation

KMP, Rabin-Karp, and Z-algorithm each search for **one** pattern at a time. When you need to find all occurrences of **k** patterns simultaneously in a text of length n, running KMP k times costs O(n × k + total pattern length). **Aho-Corasick** reduces this to O(n + total pattern length + number of matches) — linear in the text length regardless of how many patterns there are.

### Core Idea

Aho-Corasick builds a **trie** from all patterns, then adds **failure links** (analogous to the KMP failure function) so that when a partial match fails, the automaton transitions to the longest proper suffix of the current state that is also a prefix of some pattern. This allows the text to be processed in a single left-to-right scan.

```
Patterns: {"he", "she", "his", "hers"}

Trie with failure links (simplified):
  root → h → e → r → s   (pattern: "hers")
             ↑ (pattern: "he")
  root → s → h → e       (pattern: "she")
             ↑ failure link: s→h→e reuses h→e from first branch
  root → h → i → s       (pattern: "his")
```

The failure links form the key optimization: instead of restarting from the root on every mismatch, the automaton follows the failure link to a shorter matching prefix and continues.

### When to Use Aho-Corasick

| Scenario | Best Algorithm |
|---------|---------------|
| Single pattern in text | KMP or Z-algorithm |
| Multiple short patterns, few queries | Rabin-Karp (simpler to implement) |
| Many patterns, large text, need all matches | Aho-Corasick |
| Patterns change frequently | Rabin-Karp or repeated KMP |

Typical competitive programming problems that require Aho-Corasick: virus/keyword filtering, DNA sequence screening, dictionary matching, and any problem that asks "which of these k words appear in this text?"

### Complexity

| Phase | Time | Space |
|-------|------|-------|
| Build trie + failure links | O(total pattern length × alphabet size) | O(total pattern length × alphabet size) |
| Search text | O(n + number of matches) | O(1) extra |

For an alphabet of size Σ (26 for lowercase English), the trie nodes each store Σ child pointers. For large alphabets, a hash map per node reduces space at a small constant time cost.
