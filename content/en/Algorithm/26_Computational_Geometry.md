...
#### Deeper Dive: Epsilon Choice and Collinear Points in Convex Hull

Choosing the right epsilon is non-trivial. Too small (e.g., 1e-15) and accumulated floating-point errors cause collinear points to be misclassified as non-collinear. Too large (e.g., 1e-3) and genuinely distinct points are wrongly treated as collinear. A common heuristic is `EPS = 1e-9` for coordinates in [−10⁶, 10⁶], but when coordinates can be much larger or computations involve many multiplications, scale EPS accordingly or switch to **integer arithmetic** (multiplying all coordinates by a power of 10 to eliminate decimals).

**Collinear points in convex hull** require a deliberate choice. The two main policies are:

1. **Exclude collinear points** (strict hull): use `ccw(...) <= 0` to pop, so no three consecutive hull vertices are collinear. This gives the minimal vertex set.
2. **Include collinear points** (all boundary points): use `ccw(...) < 0` to pop (only remove genuine right turns). This keeps intermediate points on hull edges.

```python
# Strict hull — excludes collinear boundary points (most common in competitive programming)
while len(hull) >= 2 and ccw(points[hull[-2]], points[hull[-1]], points[i]) <= 0:
    hull.pop()

# Include-collinear hull — keeps all points on the boundary
while len(hull) >= 2 and ccw(points[hull[-2]], points[hull[-1]], points[i]) < 0:
    hull.pop()
```

When the problem asks for the number of boundary lattice points or the perimeter, use the include-collinear variant. When it asks for the minimum set of vertices that defines the hull, use the strict variant.
