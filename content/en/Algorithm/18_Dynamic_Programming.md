...
#### LIS O(n log n): Patience Sorting Intuition

The O(n log n) algorithm is equivalent to a card game called **patience sorting**. Imagine placing each number onto a pile: you must place a card on the leftmost pile whose top card is ≥ the current number, or start a new pile if none exists. The number of piles at the end equals the LIS length.

The `tails` array in the code above stores the minimum possible tail value for each pile length seen so far. `tails[i]` is the smallest number that can end an increasing subsequence of length `i+1`. This array is always sorted, so `bisect_left` finds the correct pile in O(log n). Replacing `tails[pos]` with a smaller number never changes the LIS length but keeps the tails as small as possible, allowing more future numbers to extend.

**Important**: the `tails` array is **not** the actual LIS — it is a compressed encoding. To reconstruct the actual subsequence, you need to record each element's predecessor during the binary search.

#### Rolling Array Memory Optimization

The standard O(n²) DP uses an `n`-element `dp` array, which is already O(n). However, for 2D DP variants (e.g., LIS with weighted elements, or 2D knapsack), space can dominate. The **rolling array** technique reduces a 2D `dp[i][j]` table (O(n²) space) to two 1D rows (O(n) space) whenever `dp[i][j]` depends only on the previous row `dp[i-1][...]`:

```python
# Full 2D table: O(n*W) space
dp = [[0] * (W + 1) for _ in range(n + 1)]

# Rolling array: O(W) space — overwrite previous row in-place
dp = [0] * (W + 1)
for i in range(n):
    for w in range(W, weights[i] - 1, -1):  # reverse to avoid using item twice
        dp[w] = max(dp[w], dp[w - weights[i]] + values[i])
```

The reverse iteration direction is critical for 0/1 knapsack: forward iteration would allow the same item to be selected multiple times (unbounded knapsack). This pattern applies to any DP where each row depends only on the immediately preceding row.
