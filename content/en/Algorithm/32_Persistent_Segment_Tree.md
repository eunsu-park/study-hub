...
### Version Subtraction Diagram

Subtracting two prefix trees node-by-node yields the frequency distribution for the range [l, r]:

```
K-th Smallest Query: count[version_r] - count[version_l-1]

Version l-1:          Version r:           Difference:
    [5]                  [8]                  [3]
   /   \                /   \                /   \
  [3]  [2]            [5]  [3]            [2]  [1]
  / \  / \            / \  / \            / \  / \
[2][1][1][1]        [3][2][2][1]        [1][1][1][0]
```

Each node in the "Difference" tree holds the count of elements in `arr[l..r]` whose rank falls in that node's range. To find the K-th smallest, walk down the difference tree: if the left child's count ≥ K, go left; otherwise subtract the left count from K and go right. This O(log M) walk replaces an O(M) linear scan.
