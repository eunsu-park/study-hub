...
### Step-by-Step Augmentation Table

The table below traces Ford-Fulkerson on a small network with nodes s, a, b, c, d, t and edge capacities s→a=6, s→c=4, a→b=4, a→d=3, b→t=5, c→d=3, d→t=5:

| Step | Augmenting Path | Bottleneck | Total Flow |
|------|----------------|------------|------------|
| 1 | s → a → b → t | 4 | 4 |
| 2 | s → c → d → t | 3 | 7 |
| 3 | s → a → d → t | 2 | 9 |
...
At each step, the bottleneck (minimum residual capacity on the path) limits how much flow can be pushed. After step 3, no s→t path with positive residual capacity exists, so the algorithm terminates.
...
### Ford-Fulkerson Pitfalls and Beyond

**Real-valued capacities and non-termination**: Ford-Fulkerson with DFS is only guaranteed to terminate when all capacities are integers (or rationals). With irrational capacities, the algorithm can cycle indefinitely, making arbitrarily small improvements that sum to less than the true maximum flow. The classic counterexample uses capacities involving 1/φ (the golden ratio), where DFS keeps selecting augmenting paths that converge to the wrong value. **Fix**: always use Edmonds-Karp (BFS) or Dinic's algorithm, which terminate in polynomial time regardless of capacity type.

**Pathological DFS path selection**: Even with integer capacities, DFS-based Ford-Fulkerson can take O(E × max_flow) augmenting steps — for example, alternating between two paths when there is a central bottleneck edge of capacity 1. Edmonds-Karp avoids this by always choosing the shortest augmenting path.

**Min-Cost Max-Flow (MCMF)**: When each edge carries both a capacity and a cost-per-unit-flow, the goal becomes maximizing flow while minimizing total cost (or finding the minimum-cost way to push a given amount of flow). MCMF replaces BFS with SPFA (Bellman-Ford queue) or Dijkstra with potentials to find minimum-cost augmenting paths. Key applications include optimal job-worker assignment with costs, cheapest routing under capacity constraints, and transportation problems.
