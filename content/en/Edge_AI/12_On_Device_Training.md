...
**Previous**: [Edge Hardware](./11_Edge_Hardware.md) | **Next**: [Real-Time Inference](./13_Real_Time_Inference.md)

## Learning Objectives

After completing this lesson, you will be able to:

1. Explain the Federated Averaging (FedAvg) algorithm and its communication efficiency trade-offs
2. Implement on-device fine-tuning with frozen feature extractors and trainable heads
3. Apply transfer learning on edge devices with limited data and compute
4. Design privacy-preserving ML pipelines using federated learning and secure aggregation
5. Add differential privacy guarantees to model updates with calibrated noise
6. Build personalization systems that adapt models to individual users on-device

---

Most edge AI focuses on inference -- running a pre-trained model on device. But many real-world applications demand that the model improves after deployment: a keyboard learns your typing patterns, a voice assistant adapts to your accent, a manufacturing inspector adjusts to new product variants. On-device training keeps user data local, reduces cloud dependency, and enables real-time adaptation. This lesson covers the techniques that make learning on resource-constrained devices practical, from federated learning at fleet scale to fine-tuning a single model on a single device.
...
### 2.2 PyTorch On-Device Fine-Tuning
...
### 4.1 Privacy Techniques Overview
...
### 5.2 DP-SGD for On-Device Training
...
### 6.2 Adapter-Based Personalization
...
### Exercise 1: Federated Learning Simulation
1. Implement FedAvg with 10 clients, each holding a different subset of MNIST digits (non-IID)
2. Train for 50 rounds and plot global model accuracy over rounds
3. Compare convergence with IID vs non-IID data splits

### Exercise 2: On-Device Fine-Tuning
1. Load a pre-trained MobileNetV2 and freeze all layers except the classifier
2. Fine-tune on 20 images from 3 new classes (simulate a custom use case)
3. Measure training time and memory usage on CPU

### Exercise 3: Differential Privacy
1. Train a simple MLP on a small dataset with and without DP-SGD
2. Compare accuracy at epsilon values of 1, 5, and 10
3. Plot the privacy-utility trade-off curve
