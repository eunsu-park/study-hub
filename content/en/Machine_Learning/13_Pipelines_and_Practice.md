...
- **Estimator**: implements `.fit(X, y)` and `.predict(X)` (or `.predict_proba(X)`). Examples: `LogisticRegression`, `RandomForestClassifier`.

A `Pipeline([(name_1, T_1), ..., (name_n, T_n), (name_final, E)])` composes them into a single object that behaves like an estimator:
...
Pipeline.predict(X):
    X_1 = T_1.transform(X)              ← note: .transform, not .fit_transform
