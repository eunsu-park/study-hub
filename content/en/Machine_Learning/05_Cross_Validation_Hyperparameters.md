...
K-fold cross-validation runs the same fit-and-evaluate procedure `K` times on `K` non-overlapping holdout slices, then averages. This averaging reduces variance by a factor close to `1/K` for the *estimator* (not the model itself), while the bias stays roughly the same. The trade-off:
