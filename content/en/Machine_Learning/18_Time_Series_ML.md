...
```
P(y_{t_1}, y_{t_2}, ..., y_{t_n})  =  P(y_{t_1 + h}, y_{t_2 + h}, ..., y_{t_n + h})  for all h
```

Strict stationarity is too strong for practice. **Weak (covariance) stationarity** is the working definition:

1. `E[y_t] = μ` (constant mean)
2. `Var[y_t] = σ²` (constant variance)
3. `Cov(y_t, y_{t+h}) = γ(h)` (autocovariance depends only on lag `h`, not absolute time)

Most classical ML assumes the data is stationary or can be made so. Two common transforms:

- **Differencing** `Δy_t = y_t - y_{t-1}` removes trends. The "I" in ARIMA(p, d, q) is `d` orders of differencing.
