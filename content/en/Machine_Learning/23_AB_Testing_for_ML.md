...
```
H_0 (null):       no difference between A and B (μ_A = μ_B)
H_1 (alternative):  there is a difference (μ_A ≠ μ_B)

Decision rule:    if p < α, reject H_0  ⟹  declare a "winner"
```

`α` is the **significance level** — the probability of falsely declaring a winner when none exists (Type I error). Standard `α = 0.05` says "I am willing to be wrong 5% of the time when the null is actually true."
