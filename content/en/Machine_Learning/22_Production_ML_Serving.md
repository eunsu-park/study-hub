...
- **Latency**: time from a single request arriving to its response leaving. Reported as p50, p95, p99 — the median, 95th percentile, 99th percentile of the distribution. Driven by per-request compute and serialization cost.
- **Throughput**: total number of requests served per second. Driven by parallelism — batching, multiple replicas, GPU utilization.

The distinction matters because the levers are different:

- To improve **latency**: smaller model (quantization, distillation), faster preprocessing (avoid Python in the hot path), avoid network hops, warm caches.
