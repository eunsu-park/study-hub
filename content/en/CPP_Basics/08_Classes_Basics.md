...
**Previous**: [Namespaces and IO Streams](./07_Namespaces_and_IO_Streams.md) | **Next**: [Classes Advanced](./09_Classes_Advanced.md)
...
`const` member functions cannot modify any data member -- unless the member is declared `mutable`. Use `mutable` for state that is an implementation detail (caching, lazy computation, access counting) rather than part of the object's logical value.
...
private:
    std::string raw;
    mutable std::string cached_result;  // mutable: modifiable inside const functions
    mutable bool cache_valid = false;

public:
    ExpensiveData(const std::string& s) : raw(s) {}

    // const function -- but cache_ members may be updated
    std::string getProcessed() const {
        if (!cache_valid) {
            cached_result = raw + "_processed";  // OK: mutable member
            cache_valid = true;
        }
...
Use `mutable` sparingly -- only when the mutation is truly invisible to callers and does not change the object's observable state.
...
Let's learn about operator overloading and copy/move in [Classes Advanced](./09_Classes_Advanced.md)!
