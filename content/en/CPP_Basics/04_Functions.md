...
### Exercise 1: Overloaded Area Calculator

Write three overloaded functions named `area`: one that computes the area of a circle (takes `double radius`), one for a rectangle (takes `double width, double height`), and one for a triangle (takes `double base, double height`). Test all three from `main`.

### Exercise 2: Power Function with Default Parameter

Write a function `double power(double base, int exponent = 2)` that raises `base` to `exponent`. Call it with and without the second argument to verify the default works.

### Exercise 3: Recursive Sum of Digits

Write a recursive function `int digitSum(int n)` that returns the sum of all digits in `n`. For example, `digitSum(1234)` returns 10.

### Exercise 4: Structured Bindings

Write a function that takes a `std::vector<int>` and returns a `std::tuple<double, int, int>` containing the average, minimum, and maximum. Use C++17 structured bindings in `main` to capture the result.

### Exercise 5: Callback Sorting

Write a function `void sortArray(int arr[], int size, bool (*compare)(int, int))` that sorts an array using the provided comparison function. Create `ascending` and `descending` comparison functions and demonstrate both orderings.
