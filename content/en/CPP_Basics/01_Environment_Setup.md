...
**Previous**: [C++ Basics](./00_Overview.md) | **Next**: [Variables and Types](./02_Variables_and_Types.md)
...
### Exercise 1: Hello Customization

Write a program that prints your name, age, and favorite programming language on separate lines using `std::cout`.

### Exercise 2: Compiler Flags

Compile a program with `-Wall -Wextra` and intentionally introduce a warning (e.g., an unused variable). Observe the compiler output and fix the warning.

### Exercise 3: Input Echo

Write a program that asks the user for their first name and birth year, then prints a greeting that includes their approximate age.

### Exercise 4: C vs C++ I/O

Rewrite the simple calculator from Section 9 using C-style `printf`/`scanf` instead of `cout`/`cin`. Compare both versions.

### Exercise 5: Multi-line Output

Write a program that prints a box made of `*` characters (5 wide, 3 tall) using only `std::cout` statements and the `\n` escape sequence (no `std::endl`).
