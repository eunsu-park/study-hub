...
**Previous**: [Class Basics](./08_Classes_Basics.md) | **Next**: [Inheritance and Polymorphism](./10_Inheritance_and_Polymorphism.md)
...
5. Identify the Rule of Five and explain when a class must define all five special member functions
6. Implement `static` member variables and functions for class-wide shared state
7. Apply `explicit` to prevent unintended implicit conversions
...
## 4. Move Semantics (C++11) -- Introduction

Move semantics allow "moving" resources from temporary objects to avoid unnecessary copies. Instead of allocating new memory and copying data, a move constructor simply transfers ownership of the existing data.
...
Classes that manage resources should define all 5 special member functions:
...
The Rule of Zero is the preferred modern C++ approach: if your class does not directly manage a raw resource, **define none** of the five special member functions and let the compiler generate them.

```cpp
// Rule of Zero: compiler generates all five special members correctly
class Document {
    std::string title;          // std::string manages its own memory
    std::vector<int> page_ids;  // std::vector manages its own memory
    std::unique_ptr<Config> cfg; // unique_ptr manages heap lifetime

    // No destructor, no copy/move constructor or assignment needed.
    // The compiler-generated versions do exactly the right thing.
};
```

The key insight: replace raw pointers and raw arrays with `std::string`, `std::vector`, or smart pointers. When every member already obeys RAII, the class itself naturally follows RAII at zero extra effort.

### noexcept on Move Operations

Always mark move constructors and move assignment operators `noexcept` when they cannot throw:
...
class Buffer {
    ...
    std::size_t size;
public:
    Buffer(Buffer&& other) noexcept   // noexcept is critical
...
Without `noexcept`, `std::vector` cannot use the move constructor during reallocation (it must fall back to the potentially-throwing copy constructor to preserve strong exception safety). Marking moves `noexcept` enables `vector::push_back` and `vector::resize` to move elements instead of copying them -- a significant performance difference for large types.

For a deep dive into move semantics, rvalue references, perfect forwarding, and advanced move patterns, see [Move Semantics Deep Dive](../CPP_Advanced/01_Move_Semantics_Deep_Dive.md) in C++ Advanced.
...
Let's learn about inheritance and polymorphism in [Inheritance and Polymorphism](./10_Inheritance_and_Polymorphism.md)!
