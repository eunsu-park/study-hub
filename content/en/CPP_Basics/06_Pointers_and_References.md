...
**Previous**: [Arrays and Strings](./05_Arrays_and_Strings.md) | **Next**: [Namespaces and IO Streams](./07_Namespaces_and_IO_Streams.md)
...
Even experienced C++ developers introduce pointer bugs. AddressSanitizer (ASan) is the industry-standard tool for catching them at runtime.
...
- **Memory leaks**: heap memory never freed (with `-fsanitize=leak`)
...
> ```
>
> When you write `int* ptr = new int(42);`, the variable `ptr` itself resides on the **stack**, but the integer it points to is allocated on the **heap**. Forgetting to `delete` leaves that heap block unreachable -- a memory leak.
...
### Exercise 1: Pointer Tracing

Given the following code, trace through each line and predict the output without running it. Then verify by compiling.
...
Write a program that asks the user how many names they want to enter, dynamically allocates a `std::string` array with `new[]`, reads the names, prints them in reverse order, and frees the memory with `delete[]`.

### Exercise 3: Reference Swap

Write a generic-feeling swap function using references. Test it with `int`, `double`, and `std::string` pairs (use three separate overloaded functions -- templates come later in CPP Advanced).

### Exercise 4: const Correctness

Write a function `void printArray(const int* arr, int size)` that prints all elements. Verify that the `const` prevents modification inside the function. Then write a second version that takes `int* const arr` and explain the difference.

### Exercise 5: Smart Pointer Linked List Node

Create a `struct Node { int data; std::unique_ptr<Node> next; };` and build a three-node linked list. Print all values by traversing with raw pointer observations (`node->next.get()`). Verify that no manual `delete` is needed.

---

## Next Steps

Let's learn about namespaces and IO streams in [07_Namespaces_and_IO_Streams.md](./07_Namespaces_and_IO_Streams.md)!
