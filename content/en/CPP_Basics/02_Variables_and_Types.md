...
Declare variables of every basic type (`short`, `int`, `long long`, `float`, `double`, `char`, `bool`) and print their values and sizes using `sizeof`.

### Exercise 2: Temperature Conversion

Write a program that takes Celsius temperature as input and converts it to Fahrenheit. (F = C x 9/5 + 32). Use `double` for precision.

### Exercise 3: Narrowing Detection

Write three variable declarations -- one using copy initialization, one using brace initialization that compiles, and one using brace initialization that triggers a narrowing error. Comment on which is safest.

### Exercise 4: constexpr Circle

Define a `constexpr double PI = 3.14159265358979` and a `constexpr` function `circleArea(double r)` that returns the area. Print the area for radius 5 at compile time.

### Exercise 5: Fixed-Width Types

Write a program that declares `int8_t`, `int16_t`, `int32_t`, and `int64_t` variables, assigns each to its maximum value (use `<climits>` or `<cstdint>` limits), and prints them. Observe what happens when you add 1 to the maximum value of `int8_t`.
