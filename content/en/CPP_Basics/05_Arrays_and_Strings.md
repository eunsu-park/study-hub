...
Write a program that declares an `std::array<double, 10>`, fills it with user input, and prints the minimum, maximum, and average values.

### Exercise 2: Caesar Cipher

Write a function `std::string encrypt(const std::string& text, int shift)` that shifts every lowercase letter by `shift` positions (wrapping around from 'z' to 'a'). Write a corresponding `decrypt` function and verify a round trip.

### Exercise 3: CSV Parser

Write a program that reads a single CSV line from the user (e.g., `"Alice,90,85,92"`) and splits it by comma. Print the name and the average of the numeric fields.

### Exercise 4: string_view Performance

Write two functions -- one taking `const std::string&` and one taking `std::string_view` -- that count vowels in their argument. Call each with a string literal and explain which avoids a temporary `std::string` construction.

### Exercise 5: Matrix Transpose

Declare a 3x4 `int` matrix, initialize it with values 1-12, and write a function that transposes it into a 4x3 matrix. Print both the original and transposed matrices.
