...
For $m > n$ (over-determined — more task dimensions than joints):
...
When multiple IK solutions exist (e.g., up to 16 for a 6-DOF robot with spherical wrist), the controller must choose the best one. A practical selection algorithm:

1. **Compute all feasible solutions** using analytical IK (or sample multiple initial guesses for numerical IK)
2. **Filter by joint limits**: discard any solution where $q_i \notin [q_{i,min}, q_{i,max}]$
3. **Filter by self-collision**: run a fast geometric collision check between links
4. **Score remaining candidates** by weighted criteria:
   - **Distance from current configuration**: $w_1 \|\mathbf{q}_{candidate} - \mathbf{q}_{current}\|$ — prefer small joint motions for smooth transitions
   - **Joint limit proximity**: $w_2 \sum_i \frac{1}{(q_i - q_{i,min})(q_{i,max} - q_i)}$ — penalize configurations near joint limits
   - **Manipulability**: $w_3 / \sqrt{\det(J J^T)}$ — penalize configurations near singularities
5. **Select the candidate with the lowest total score**

For real-time applications, the distance-from-current criterion typically dominates (weight $w_1$ is largest), ensuring the robot takes the shortest path between configurations and avoiding sudden jumps between IK branches.
