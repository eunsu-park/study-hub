...
### 2.4 Data Association with Mahalanobis Gating

In practice, the robot does not know which landmark it is observing — it must infer this from the measurement itself. **Nearest-neighbor data association** with a **Mahalanobis distance validation gate** is the standard approach:

For each observation $\mathbf{z}$, compute the innovation and its covariance for every known landmark $j$:

$$\boldsymbol{\nu}_j = \mathbf{z} - \hat{\mathbf{z}}_j, \qquad S_j = H_j P H_j^T + R$$

The **Mahalanobis distance** is:

$$d_{M,j} = \sqrt{\boldsymbol{\nu}_j^T S_j^{-1} \boldsymbol{\nu}_j}$$

The **validation gate** rejects implausible matches:

$$d_{M,j}^2 \leq \chi^2_{m, \alpha}$$

where $\chi^2_{m, \alpha}$ is the chi-squared threshold with $m$ measurement dimensions at confidence level $\alpha$ (typically $\alpha = 0.95$, giving $\chi^2_{2, 0.95} = 5.99$ for range-bearing measurements).

**Algorithm**:
1. For each observation $\mathbf{z}$, compute $d_{M,j}$ for all known landmarks
2. Reject all landmarks with $d_{M,j}^2 > \chi^2_{m, \alpha}$ (outside the gate)
3. Among gated landmarks, select the one with the smallest $d_{M,j}$ (nearest neighbor)
4. If no landmark passes the gate, initialize a new landmark

This approach is more robust than Euclidean nearest-neighbor because it accounts for the uncertainty structure: a large innovation in a well-known direction is more suspicious than the same innovation in an uncertain direction.

### 2.5 EKF-SLAM Limitations
