...
The standard manipulator dynamics equation $M(\mathbf{q})\ddot{\mathbf{q}} + C(\mathbf{q}, \dot{\mathbf{q}})\dot{\mathbf{q}} + \mathbf{g}(\mathbf{q}) = \boldsymbol{\tau}$ assumes frictionless joints. In practice, joint friction is significant (often 5-15% of rated torque) and must be modeled for accurate control.

The most common friction model combines **Coulomb friction** (constant magnitude, opposes motion) and **viscous friction** (proportional to velocity):

$$\tau_{friction,i} = b_i \dot{q}_i + \mu_{c,i} \, \text{sgn}(\dot{q}_i)$$

where:
- $b_i$ is the **viscous friction coefficient** [N$\cdot$m$\cdot$s/rad] — dominates at high speeds
- $\mu_{c,i}$ is the **Coulomb friction coefficient** [N$\cdot$m] — constant torque opposing motion
- $\text{sgn}(\dot{q}_i)$ is the sign function

The full dynamics with friction become:

$$M(\mathbf{q})\ddot{\mathbf{q}} + C(\mathbf{q}, \dot{\mathbf{q}})\dot{\mathbf{q}} + \mathbf{g}(\mathbf{q}) + \boldsymbol{\tau}_{friction} = \boldsymbol{\tau}$$

**Effect on control**: Unmodeled friction degrades tracking performance, especially at low velocities where Coulomb friction causes **stick-slip behavior** (the joint alternates between sticking and slipping). In computed torque control, adding a friction feedforward term $\hat{\boldsymbol{\tau}}_{friction}$ to the control law significantly improves low-speed tracking accuracy. For high-precision applications, more advanced models such as the **LuGre model** capture stiction dynamics and presliding displacement.
