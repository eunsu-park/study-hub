...
### 6.4 Filter Health Monitoring

A running filter can silently diverge — producing confident but wrong estimates. Monitoring filter health is essential for safety-critical applications.

**Normalized Innovation Squared (NIS)**: At each measurement update, the innovation (measurement residual) $\boldsymbol{\nu}_k = \mathbf{z}_k - h(\hat{\mathbf{x}}_k^-)$ should be consistent with its predicted covariance $S_k = H P_k^- H^T + R$. The NIS statistic:

$$\epsilon_k = \boldsymbol{\nu}_k^T S_k^{-1} \boldsymbol{\nu}_k$$

follows a $\chi^2$ distribution with $m$ degrees of freedom (where $m$ is the measurement dimension) when the filter is consistent. If $\epsilon_k$ exceeds the $\chi^2$ threshold (e.g., 95th percentile) persistently, the filter is likely diverging — either the process noise $Q$ is too small, the measurement model is wrong, or there is an unmodeled bias.

**Practical divergence detection**: Compute a windowed average of NIS over the last $N$ steps. If $\bar{\epsilon} > \chi^2_{m, 0.95}$ for $N$ consecutive steps, flag a divergence. Common recovery actions include resetting $P$ to a larger value, increasing $Q$ temporarily, or triggering a re-initialization.

**Initial covariance $P_0$ guidance**: Setting $P_0$ too small makes the filter overconfident in a possibly wrong initial state, rejecting valid measurements. Setting $P_0$ too large wastes initial convergence time. A practical rule: set $P_0$ diagonal entries to the square of the maximum expected initial error in each state component. For example, if initial position uncertainty is $\pm 1$ m, set $P_{0,xx} = 1.0$ m$^2$.
