...
### 1.3 Anti-Windup Strategies

When actuators saturate (commanded torque exceeds motor capacity), the integral term continues accumulating error — a phenomenon called **integrator windup**. After saturation ends, the inflated integral causes large overshoot and oscillation. Two common anti-windup strategies:

**Back-calculation**: When the actuator saturates, compute the difference between the commanded and actual (saturated) torque, and feed it back to reduce the integral accumulator:

$$\dot{e}_{I,i} = e_i + \frac{1}{T_{t,i}}(\tau_{sat,i} - \tau_{cmd,i})$$

where $T_{t,i}$ is the tracking time constant (typically $T_{t,i} \approx \sqrt{T_i / T_d}$, with $T_i = K_p/K_i$ and $T_d = K_d/K_p$).

**Clamping**: Simply stop accumulating the integral when the output is saturated and the error has the same sign as the integral:

$$\text{If } |\tau_{cmd}| > \tau_{max} \text{ and } \text{sgn}(e_i) = \text{sgn}(e_{I,i}): \quad \dot{e}_{I,i} = 0$$

In practice, clamping is simpler to implement and sufficient for most robotic applications. Back-calculation provides smoother transients and is preferred for high-performance systems.

> **Note**: For more advanced trajectory tracking under constraints, **Model Predictive Control (MPC)** optimizes a finite-horizon cost function subject to actuator limits, joint constraints, and obstacle avoidance — handling saturation intrinsically rather than as an afterthought. MPC is increasingly practical for robotic manipulators as computational power grows.

### 1.4 Limitations of Independent Joint PID
...
The following block diagram shows the computed torque feedback loop:
...
```
   ...
   ---->(+)---------->  | PD        |------>(+)------------>| Robot   |------+---->
          ...
          +--------------------------------------------------+              |
                                  q (measured)                              |
                                                                            |
   Key: u = qdd_d + Kp*e + Kd*ed  (auxiliary input)
        tau = M(q)*u + C(q,qd)*qd + g(q)  (inverse dynamics)
        d = external disturbances (friction, payload, etc.)
```
...
## Putting It All Together: Simulation Example
