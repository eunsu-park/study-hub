...
The following diagram illustrates DH frame assignment on a 3-joint (RRR) arm. Each frame $\{i\}$ is placed so that $z_i$ aligns with the axis of joint $i+1$, $x_i$ lies along the common normal from $z_{i-1}$ to $z_i$, and the origin sits at their intersection.
...
> **Convention note: Classical DH vs Modified DH.** This lesson uses the **classical (standard) DH convention** (Spong, Siciliano, Paul), where the transformation order is $Rot_z(\theta_i) \cdot Trans_z(d_i) \cdot Trans_x(a_i) \cdot Rot_x(\alpha_i)$ and frame $\{i\}$ is attached to the distal end of link $i$. An alternative **Modified DH convention** (Craig) reverses the parameter assignment: $a_{i-1}$, $\alpha_{i-1}$, $d_i$, $\theta_i$, with frame $\{i\}$ placed at the proximal end of link $i$. The transformation order becomes $Rot_x(\alpha_{i-1}) \cdot Trans_x(a_{i-1}) \cdot Rot_z(\theta_i) \cdot Trans_z(d_i)$. Both conventions yield identical end-effector FK when applied consistently, but DH parameter tables from one convention cannot be used with the other's transformation formula. Always check which convention a reference uses before reusing its DH table.
