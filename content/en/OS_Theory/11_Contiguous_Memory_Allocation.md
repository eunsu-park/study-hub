...
### 1.2 Single-Programming vs Multi-Programming
...
│  └──────────────┘                    └──────────────┘              │
│                                                                     │
│  - Only one process at a time        - Multiple processes loaded   │
│  - Heavy memory waste                - Improved CPU utilization    │
│                                                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
### 2.2 Variable-Size Fixed Partitions

```
┌─────────────────────────────────────────────────────────────┐
│                 Fixed Partitioning (Variable Sizes)          │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  ┌──────────────────────────────────────────┐               │
│  │                  OS                       │ 64KB         │
...
│  └──────────────────────────────────────────┘               │
│                                                              │
│  Advantage: Can accommodate processes of varying sizes      │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### 2.3 Partition Table Management

```c
// Fixed partition management structure
typedef struct {
    int partition_id;
    int base_address;
    int size;
    int is_allocated;      // Whether allocated
    int process_id;        // Assigned process (-1 if empty)
} Partition;

// Example partition table
Partition partition_table[] = {
    {0, 0x10000, 32 * 1024,  1, 101},  // 32KB, process 101
    {1, 0x18000, 64 * 1024,  1, 102},  // 64KB, process 102
    {2, 0x28000, 128 * 1024, 0, -1},   // 128KB, empty
    {3, 0x48000, 256 * 1024, 1, 103}   // 256KB, process 103
};

// Find a suitable partition for a process
...
When processes terminate, holes (gaps) appear between remaining processes, leading to external fragmentation:

```
┌─────────────────────────────────────────────────────────────┐
│               Memory State After P2 Terminates               │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  ┌──────────────────────────────────────────┐               │
│  │                  OS                       │               │
...
│  └──────────────────────────────────────────┘               │
│                                                              │
│  Hole: Free space between processes                         │
...
The OS maintains a linked list of free blocks, sorted by address, to track available memory:
...
// Free space list example (sorted by address)
/*
...
// Allocate memory
int allocate_memory(FreeBlock** free_list, int size) {
    FreeBlock* prev = NULL;
...
// Free memory (with adjacent hole merging)
...
### 5.3 The 50-Percent Rule

Statistically, given N allocated blocks, approximately 0.5N holes are generated. This means roughly one-third of memory can be lost to fragmentation:

```
Memory = Allocated blocks + Holes
N allocated blocks → approximately 0.5N holes

Loss rate = Holes / (Blocks + Holes) = 0.5N / (N + 0.5N) = 1/3 ≈ 33%
```

This result motivates the need for compaction or, better yet, non-contiguous allocation schemes like paging.
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  1. Requires execution-time binding                         │
│     - Process addresses change after relocation             │
│     - Not possible with compile-time or load-time binding   │
│                                                              │
│  2. High cost                                                │
│     - Processes must be copied in memory                    │
│     - System must be temporarily halted                     │
│                                                              │
│  3. Optimal compaction strategy is hard to determine        │
│                                                              │
│     Strategy 1: Move all processes to the bottom            │
│                 → Maximum data movement                     │
│                                                              │
│     Strategy 2: Move only some to merge holes               │
│                 → NP-complete problem (hard to optimize)    │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### 6.3 Compaction Algorithm Example

```c
// Simple compaction algorithm (move all processes down)
void compact_memory(Process* processes[], int n, int os_end) {
    int next_address = os_end;  // Start right after OS
    ...
    for (int i = 0; i < n; i++) {
        if (processes[i]->is_active) {
            if (processes[i]->base != next_address) {
                ...
                // Copy memory (in practice, memcpy)
                ...
                // Update MMU relocation register
    ...
    // Remaining space is one large hole
    printf("Free space: from %d to end\n", next_address);
}
```
...
Next topic: [Paging](12_Paging.md)
