...
**Worked example — $\frac{d}{d\mathbf{x}}(A\mathbf{x})$ in both layouts**

Let $A \in \mathbb{R}^{m \times n}$ and $\mathbf{y} = A\mathbf{x}$, so $y_i = \sum_k A_{ik} x_k$.

| Layout | Result | Shape |
|--------|--------|-------|
| **Numerator** | $\frac{\partial \mathbf{y}}{\partial \mathbf{x}} = A$ | $m \times n$ |
| **Denominator** | $\frac{\partial \mathbf{y}}{\partial \mathbf{x}} = A^T$ | $n \times m$ |

The two layouts produce transposes of each other. **PyTorch and NumPy follow numerator layout** for Jacobians: `torch.autograd.functional.jacobian` returns a tensor whose $(i,j)$ entry is $\partial y_i / \partial x_j$, matching the numerator convention. When reading papers, always check which layout the authors use before applying gradient formulas.
...
By Schwarz's theorem, if $f$ is of class $C^2$, then $H$ is symmetric.
...
**Next lesson**: [04. Norms and Distance Metrics](04_Norms_and_Distances.md)
