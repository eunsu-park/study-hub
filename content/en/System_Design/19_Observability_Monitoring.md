...
### 2.4 Cardinality Explosion Warning

High-cardinality labels are one of the most common pitfalls in Prometheus deployments. Each unique label combination creates a new time series, and unchecked growth can exhaust memory and crash the server.

```
┌─────────────────────────────────────────────────────────────────┐
│              Cardinality Explosion                               │
│                                                                 │
│  BAD: Labels with unbounded values                              │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │                                                            │ │
│  │  http_requests_total{                                      │ │
│  │    method="GET",                                           │ │
│  │    path="/api/users/12345",    ← unique per user ID!       │ │
│  │    user_id="u-67890",          ← unbounded                 │ │
│  │    session_id="sess-abc123"    ← unbounded                 │ │
│  │  }                                                         │ │
│  │                                                            │ │
│  │  With 1M users × 3 methods × 100 paths = 300M series!     │ │
...
│  │  http_requests_total{                                      │ │
│  │    method="GET",                                           │ │
│  │    path="/api/users/:id",      ← parameterized template   │ │
│  │    status="200"                ← small finite set          │ │
│  │  }                                                         │ │
│  │                                                            │ │
│  │  3 methods × 20 paths × 5 statuses = 300 series            │ │
│  │  → Perfectly manageable                                    │ │
│  │                                                            │ │
│  └────────────────────────────────────────────────────────────┘ │
│                                                                 │
│  Rules of thumb:                                                │
│  • Never use user IDs, session IDs, or trace IDs as labels     │
│  • Parameterize URL paths (/users/:id, not /users/12345)       │
│  • Keep total series per metric under 10,000                    │
