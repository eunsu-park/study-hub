...
│  │            │ RDB               │ AOF              │           │
│  ├────────────┼──────────────────┼──────────────────┤           │
│  │ Durability │ Possible loss     │ ~1 second loss   │           │
│  │ File size  │ Compact           │ Larger           │           │
│  │ Restart    │ Fast              │ Slower           │           │
│  │ I/O impact │ Periodic fork     │ Continuous write │           │
│  └────────────┴──────────────────┴──────────────────┘           │
│                                                                  │
│  Best practice: Enable both RDB + AOF for production.            │
│  Use RDB for backups, AOF for durability.                        │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

> **Note**: Redis Cluster does not support cross-slot multi-key operations (e.g., `MGET`, `SUNION`, transactions with keys on different slots). All keys in a multi-key command must hash to the same slot. Use hash tags (e.g., `{user:123}`) to colocate related keys when multi-key operations are required.

### 3.4 Sentinel vs Cluster
