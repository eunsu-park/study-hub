...
When standard error messages are not enough, enable SDK-level debug logging to see the full HTTP request and response traffic between your code and the Anthropic API.

**Python and TypeScript SDKs** both respect the `ANTHROPIC_LOG` environment variable:

```bash
# Full request/response payloads — use for deep debugging
export ANTHROPIC_LOG=debug

# Request summaries only (method, URL, status, latency)
export ANTHROPIC_LOG=info

# Retry events only (shows when and why retries occur)
export ANTHROPIC_LOG=warn
```

With `ANTHROPIC_LOG=debug` set, the SDK prints every HTTP exchange to stderr, including request headers, the full JSON body sent to the API, and the complete response. This makes it straightforward to verify that your messages array, tool definitions, and system prompt are being serialised exactly as you expect.

```bash
# Example: debug a single script run without changing your shell profile
ANTHROPIC_LOG=debug python my_script.py 2>debug.log
cat debug.log  # Inspect the captured traffic
```

**Caution**: Debug output includes your full message content. Do not commit log files or redirect debug output to locations accessible by other users, as they may contain sensitive prompt data or user information.

For Claude Code itself (rather than the SDK), verbose logging is enabled with:

```bash
claude --verbose   # Show tool calls, hook events, and MCP messages
```
...
This lesson rounds out the core troubleshooting skills for the Claude Ecosystem topic. You now have a systematic reference for diagnosing and resolving the most common problems — from permission errors and hook failures to context limits, MCP issues, and API errors. The most effective Claude Code users combine good prompting habits (Lesson 21) with the debugging techniques covered here to maintain productive, efficient development sessions.

Three advanced lessons extend the topic further:

- **Lesson 23 — Vision Agents**: Image analysis, computer use, screenshot workflows, and vision-powered agent patterns.
- **Lesson 24 — Prompt Caching and Batch API**: Cache breakpoints, cache warming strategies, the Message Batches API, and cost-optimization techniques.
- **Lesson 25 — RAG Patterns**: Retrieval-augmented generation, embeddings, vector stores, and hybrid search approaches.

For continued learning, revisit earlier lessons as you encounter new scenarios, work through the advanced lessons above, and stay current with the Anthropic documentation as the Claude ecosystem continues to evolve.
