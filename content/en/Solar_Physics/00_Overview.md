...
| File | Description |
|------|-------------|
| `01_solar_structure.py` | Standard solar model: radial profiles of temperature, density, and pressure via polytropic integration |
| `02_pp_chain.py` | Gamow peak calculation, pp chain energy generation rate, neutrino flux |
| `03_helioseismology.py` | Spherical harmonic decomposition, l-nu diagram, acoustic ray tracing |
| `04_limb_darkening.py` | Eddington-Barbier solution, limb darkening curves, spectral line profiles |
| `05_chromosphere_dem.py` | Differential Emission Measure (DEM) analysis of the solar atmosphere from multi-wavelength EUV |
| `06_pfss_model.py` | Potential Field Source Surface (PFSS) extrapolation, coronal magnetic topology |
| `07_sunspot_cycle.py` | Solar cycle analysis: sunspot number time series, FFT periodicity, Waldmeier effect |
| `08_flare_energetics.py` | Solar flare energy partition model: magnetic energy estimation and multi-wavelength emission |
| `09_cme_propagation.py` | Drag-based CME propagation model with ensemble forecasting, arrival time prediction |
| `10_parker_spiral.py` | Parker spiral: interplanetary magnetic field geometry, frozen-in flux, wind speed profile |
| `11_solar_spectrum.py` | Synthetic solar spectrum: Planck function, absorption lines, Zeeman splitting |
| `12_sep_transport.py` | Focused transport equation: 1D SEP propagation along the Parker spiral |
