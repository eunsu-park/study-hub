...
**Previous**: [Project: Mini Shell](./10_Project_Mini_Shell.md) | **Next**: [Network Programming](./12_Network_Programming.md)
...
Concurrency is one of the most challenging aspects of systems programming, but it is also one of the most rewarding. Whether you are building a web server that handles thousands of connections or a data pipeline that crunches numbers in parallel, understanding threads, mutexes, and condition variables gives you the tools to harness modern multi-core hardware. This lesson takes you from your first thread all the way to a reusable thread pool.
...
## Stage 8: C11 Atomics and Memory Ordering

C11 introduced native atomic operations via `<stdatomic.h>`, providing a portable alternative to platform-specific intrinsics or mutex-protected single variables.
...
_Atomic int counter = 0;  // C11 atomic integer
...
Each atomic operation accepts an optional memory-order argument that controls how surrounding memory accesses are reordered by the compiler and CPU:

| Order | Description |
|-------|-------------|
| `memory_order_relaxed` | No ordering guarantees; only atomicity. Fastest. |
| `memory_order_acquire` | All subsequent reads/writes happen after this load. |
| `memory_order_release` | All preceding reads/writes happen before this store. |
| `memory_order_seq_cst` | Full sequential consistency (default). Slowest. |

```c
// Producer: write data, then publish flag
...
`acquire`/`release` pairs are sufficient for most producer-consumer flags and are cheaper than `seq_cst` on weakly-ordered architectures (ARM, POWER).

### Spinlock vs Mutex Tradeoff

A spinlock is a mutex built entirely from atomics:
...
**When to use a spinlock**: the critical section is extremely short (a few instructions) and contention is low. Spinlocks avoid the syscall overhead of `pthread_mutex_lock` but burn CPU cycles while waiting. On a single-core machine or under high contention, always prefer a mutex.

---

## Stage 9: Practical Example -- Parallel Sorting
...
Compute N x N matrix multiplication using multiple threads.

### Exercise 5: Concurrent Hash Map
Implement a hash map with per-bucket locking that supports concurrent insert, lookup, and delete operations from multiple threads.
...
With threads and synchronization mastered, proceed to:
- [Network Programming](./12_Network_Programming.md) -- Building TCP/UDP servers and clients with the Berkeley socket API
