...
**Previous**: [Multithreading](./11_Multithreading.md) | **Next**: [Inter-Process Communication and Signals](./13_IPC_and_Signals.md)
...
> |---|---|---|
> | Application Process | Network | Application Process |
...
| Use Cases | HTTP, SSH, file transfer | DNS, VoIP, gaming, streaming |
...
|---------|--------|------|-------|
| Max FDs | FD_SETSIZE (usually 1024) | Unlimited | Unlimited |
| Complexity | O(n) | O(n) | O(1) amortized |
| Portability | POSIX | POSIX | Linux only |
| Overhead | Copy fd_set each call | Copy array each call | Kernel-managed |
...
### 5.4 Protocol-Independent Name Resolution with getaddrinfo()

`inet_pton` only handles numeric IP strings. `getaddrinfo()` resolves hostnames and works transparently for both IPv4 and IPv6, making code future-proof without protocol-specific branches.
...
### 5.5 Scalable I/O with epoll (Linux)

`select` and `poll` scan all monitored file descriptors on every call -- O(n) per wakeup. `epoll` maintains a kernel-side interest list and returns only the ready events, giving O(1) amortized performance for high-connection servers.
...
Use `EPOLLET` (edge-triggered mode) with non-blocking sockets for even higher throughput; level-triggered (the default) is simpler and safe for most applications.

### 5.6 Essential Socket Options for Production Servers

```c
int opt = 1;

// SO_REUSEADDR: allow binding while TIME_WAIT sockets exist
// (prevents "Address already in use" after server restart)
setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));

// SO_REUSEPORT: allow multiple sockets to bind the same port
// Enables multi-process/thread accept() without a shared lock
setsockopt(server_fd, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt));
```

`SO_REUSEADDR` is practically mandatory for any server. `SO_REUSEPORT` (Linux 3.9+) lets multiple worker processes each call `accept()` independently, distributing connection load across cores without a bottleneck.
