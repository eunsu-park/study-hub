# 03. MCMC Fundamentals

[Previous: Probabilistic Graphical Models](./02_Probabilistic_Graphical_Models.md) | [Next: PyMC Introduction](./04_PyMC_Introduction.md)

---

> **Framework Note**: This lesson implements MCMC from scratch using NumPy.
> Production-quality MCMC is covered in Lessons 04 (PyMC) and 07 (Stan).
>
> Installation: `pip install numpy scipy matplotlib arviz`

## Learning Objectives

- Understand the motivation for Markov Chain Monte Carlo methods
- Implement the Metropolis-Hastings algorithm from scratch
- Implement Gibbs sampling for multi-parameter models
- Diagnose convergence using trace plots, R-hat, and effective sample size
- Understand the principles behind Hamiltonian Monte Carlo (HMC)

---

## 1. The Monte Carlo Principle

When we cannot compute integrals analytically, we approximate them with samples.

### 1.1 Why We Need Sampling

In Bayesian inference, we frequently need to compute expectations under the posterior:

$$\mathbb{E}[\theta | D] = \int \theta \, P(\theta | D) \, d\theta$$

For most real models, this integral has no closed-form solution. Monte Carlo integration provides the solution: if we can draw samples $\theta^{(1)}, \ldots, \theta^{(S)}$ from $P(\theta | D)$, then:

$$\mathbb{E}[\theta | D] \approx \frac{1}{S} \sum_{s=1}^{S} \theta^{(s)}$$

### 1.2 Direct Sampling vs MCMC
...
A Markov chain is a sequence of random variables where the next state depends only on the current state:
...
### 2.2 Ergodicity and Convergence

For MCMC to work, the Markov chain must be:
...
These conditions guarantee that the chain converges to a unique **stationary distribution**.

---

## 3. Metropolis-Hastings Algorithm

The most fundamental MCMC algorithm. It constructs a Markov chain whose stationary distribution is the target posterior.
...
### 3.3 Multi-dimensional Metropolis-Hastings
...
### 3.4 Tuning the Proposal Distribution

The acceptance rate should be around 23% for high-dimensional targets and 44% for 1D targets (Roberts, Gelman, & Gilks, 1997).
...
Gibbs sampling updates one variable at a time, sampling from the **full conditional** distribution of each variable given all others.
...
### 4.2 Gibbs Sampler for Normal Distribution
...
### 4.3 Gibbs Sampler for Gaussian Mixture Model
...
Running MCMC is not enough — we must verify that the chain has converged to the target distribution.
...
### 5.2 R-hat (Gelman-Rubin Diagnostic)

R-hat compares within-chain and between-chain variance. Values close to 1.0 indicate convergence.
...
Due to autocorrelation, MCMC samples are not independent. ESS estimates the number of "effectively independent" samples.
...
### 5.4 Using ArviZ for Diagnostics
...
## 6. Hamiltonian Monte Carlo (Preview)
...
- **Position** q = parameter values θ
- **Momentum** p = auxiliary variables (drawn from Normal)
- **Hamiltonian** H(q, p) = U(q) + K(p) where U(q) = -log P(θ|D) (potential energy) and K(p) = p²/2 (kinetic energy)
...
### 6.2 Why HMC is Better Than Random-Walk MH
...
### 6.3 NUTS: The No-U-Turn Sampler

NUTS automatically tunes the number of leapfrog steps by detecting when the trajectory begins to turn back. This eliminates the need to manually set `n_leapfrog`.

NUTS is the default sampler in PyMC and Stan (covered in Lessons 04 and 07).

---

## 7. Practical MCMC Tips

### 7.1 Parameterization Matters
...
### 8.2 Divergent Transitions

Divergent transitions in HMC indicate the leapfrog integrator is failing due to sharp curvature in the posterior geometry. This usually means the model needs reparameterization.
...
|-----------|------|------|----------|
| Metropolis-Hastings | Simple, general | Random walk, slow in high dimensions | Low-dimensional, educational |
| Gibbs Sampling | No tuning, uses conjugacy | Slow for correlated parameters | Conjugate models, GMMs |
| HMC | Efficient in high dimensions | Requires gradients, tuning | Continuous parameters |
| NUTS | Auto-tuned HMC | Requires gradients | Default choice for most models |

| Diagnostic | Target | Interpretation |
|-----------|--------|----------------|
| R-hat | < 1.01 | Chains have converged to same distribution |
| ESS | > 400 | Enough independent information per chain |
| Trace plot | Fuzzy caterpillar | Good mixing, no trends or stuck periods |
| Divergences | 0 | No geometry issues |
