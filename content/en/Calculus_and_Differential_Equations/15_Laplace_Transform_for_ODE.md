...
- Second-order linear ODE and initial value problems (Lesson 13 (Second-Order ODEs))
...
These two theorems let you extract key information about $f(t)$ directly from $F(s)$ **without computing the inverse Laplace transform** -- a major practical shortcut in engineering analysis.

### Initial Value Theorem

$$\lim_{t \to 0^+} f(t) = \lim_{s \to \infty} s F(s)$$

This holds whenever both limits exist and $f(t)$ has no impulse at $t = 0$.

**Interpretation**: The behavior of $f(t)$ near $t = 0$ corresponds to the behavior of $sF(s)$ as $s \to \infty$. Large $s$ values "probe" the short-time behavior.

### Final Value Theorem

$$\lim_{t \to \infty} f(t) = \lim_{s \to 0^+} s F(s)$$

**Critical condition**: This theorem is valid **only if** all poles of $sF(s)$ have strictly negative real parts (i.e., the system is stable and $f(t)$ actually has a finite steady-state value). If any pole of $sF(s)$ lies on the imaginary axis or in the right half-plane, the theorem does not apply.

**Example**: For the IVP solved above, $Y(s) = \dfrac{1}{s+1} + \dfrac{1}{s+2} - \dfrac{1}{s+3}$.

- **Initial value**: $\lim_{s \to \infty} sY(s) = \lim_{s \to \infty} \left(\dfrac{s}{s+1} + \dfrac{s}{s+2} - \dfrac{s}{s+3}\right) = 1 + 1 - 1 = 1$. This matches $y(0) = 1$.
- **Final value**: $\lim_{s \to 0^+} sY(s) = 0 + 0 - 0 = 0$. The solution decays to zero as $t \to \infty$ (all poles are negative: $-1, -2, -3$), confirming the system is stable.

**Why these matter**: In control systems, the Final Value Theorem gives the steady-state output of a closed-loop system without solving the full inverse transform. If you design a controller and need to verify that the output settles to a desired set-point, a single limit computation suffices.
