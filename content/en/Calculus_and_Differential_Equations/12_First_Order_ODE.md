...
### 5.1 Form and Substitution

A **Bernoulli equation** has the form:

$$y' + P(x)\,y = Q(x)\,y^n, \quad n \neq 0, 1$$

When $n = 0$ this is just the linear first-order ODE from Section 4. When $n = 1$ it is separable. For all other values of $n$, the equation is nonlinear, but a single substitution linearizes it.

**Substitution:** Let $v = y^{1-n}$. Then $v' = (1-n)y^{-n}y'$, so $y' = \dfrac{y^n}{1-n}v'$.

Dividing the original equation by $y^n$ and substituting:
...
Solve $y' - y = xy^2$ (here $n = 2$).

**Step 1:** Let $v = y^{1-2} = y^{-1}$, so $v' = -y^{-2}y'$.

**Step 2:** Divide through by $y^2$: $y^{-2}y' - y^{-1} = x$. Substitute $v$ and $v'$:

$$-v' - v = x \quad \Longrightarrow \quad v' + v = -x$$

**Step 3:** Solve by integrating factor $\mu = e^x$:

$$\frac{d}{dx}[e^x v] = -xe^x \quad \Longrightarrow \quad e^x v = -xe^x + e^x + C$$
...
- **Mathematical Methods Lesson 09** provides more advanced first-order ODE techniques (substitution methods, Riccati equations) and a deeper treatment of existence theorems.
