...
The second derivative test finds local extrema of $f(x, y)$ when the variables are **free**. Many real problems require optimizing $f$ subject to a **constraint** $g(x, y) = 0$ -- for example, maximizing area with a fixed perimeter, or minimizing cost subject to a production target.

### The Method

At a constrained extremum, the gradient of $f$ must be parallel to the gradient of $g$:

$$\nabla f = \lambda \, \nabla g$$

for some scalar $\lambda$ called the **Lagrange multiplier**. Combined with the constraint, this gives the system:

$$f_x = \lambda \, g_x, \qquad f_y = \lambda \, g_y, \qquad g(x, y) = 0$$

**Geometric intuition**: The level curves of $f$ and the constraint curve $g = 0$ must be tangent at the optimum -- if they crossed, you could move along the constraint to increase $f$ further.

### Worked Example

**Maximize** $f(x, y) = xy$ **subject to** $x + y = 10$.

Write the constraint as $g(x, y) = x + y - 10 = 0$.

Setting up the system:

$$f_x = y = \lambda \cdot 1, \qquad f_y = x = \lambda \cdot 1, \qquad x + y = 10$$

From the first two equations: $y = \lambda$ and $x = \lambda$, so $x = y$. Substituting into the constraint: $2x = 10$, giving $x = y = 5$.

**Maximum value**: $f(5, 5) = 25$.

**Verification**: The AM-GM inequality states $xy \leq \left(\frac{x+y}{2}\right)^2 = 25$, confirming that equality (and hence the maximum) occurs when $x = y$.

---

## 8. Cross-References

- **Mathematical Methods Lesson 04** covers **Lagrange multipliers** for constrained optimization in greater depth, including inequality constraints and multiple constraints.
