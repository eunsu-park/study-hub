...
**Previous**: [Move Semantics Deep Dive](./01_Move_Semantics_Deep_Dive.md) | **Next**: [Template Metaprogramming](./03_Template_Metaprogramming.md)
...
7. Organize template code correctly using the header-only compilation model
...
int main() {
    ...
    const char* s1 = "Hello";
    const char* s2 = "Hello";
    std::cout << isEqual(s1, s2) << std::endl;           // true (content comparison)
...
## 6. SFINAE and if constexpr

SFINAE (Substitution Failure Is Not An Error): Template argument substitution failure is not an error--the compiler simply discards that overload.
...
## 8. Template Compilation Model

### Why Define in Headers

| | Regular Function | Template |
|---|---|---|
| **header.h** | Declaration | Declaration + Definition |
| **source.cpp** | Definition | (Instantiated at use site) |
...
### Exercise 1: Min/Max Variadic Function

Write a function template that returns the minimum and maximum values from an arbitrary number of arguments using fold expressions.

### Exercise 2: Generic Queue

Referencing the Stack example, write a Queue class template with `enqueue`, `dequeue`, `front`, `empty`, and `size` operations.

### Exercise 3: Type-Specific Serialization

Write a `serialize` function template that converts various types to strings (basic types, containers, etc.) using `if constexpr`.

### Exercise 4: Compile-Time Matrix

Implement a `Matrix<T, Rows, Cols>` class template using non-type template parameters. Support addition and multiplication with compile-time dimension checking.

### Exercise 5: Tuple Implementation

Implement a simplified `Tuple` class template using variadic templates and recursive inheritance.

---

## Next Steps

Let's dive deeper into template metaprogramming techniques including SFINAE, type traits, and compile-time computation in [03_Template_Metaprogramming.md](./03_Template_Metaprogramming.md).
