...
**Previous**: [C++20 Modules and Utilities](./11_Modules_and_CPP20_Utilities.md) | **Next**: [Advanced Concurrency](./13_Concurrency_Advanced.md)
...
2. [Mutex and Locks](#2-mutex-and-locks)
3. [Condition Variables](#3-condition-variables)
...
A `std::future` returned by `std::async(std::launch::async, ...)` has a special rule: **its destructor blocks until the associated task finishes**. This means discarding the return value — or binding it to a temporary — causes the call to block immediately, defeating the entire purpose of async:

```cpp
// WRONG: temporary future is destroyed at the semicolon → blocks here
std::async(std::launch::async, heavy_task);  // synchronous!

// CORRECT: store the future to keep the task running concurrently
auto fut = std::async(std::launch::async, heavy_task);
do_other_work();
fut.get();  // block only when you actually need the result
```

This pitfall does not apply to futures obtained from `std::promise` or `std::packaged_task` — only to those returned directly by `std::async`.

### Detach and Dangling References

`thread.detach()` severs the thread from its `std::thread` object, letting it run independently. If the detached thread captures local variables by reference, those references become dangling the moment the enclosing scope exits — the thread may be reading or writing deallocated stack memory:
...
void risky() {
    ...
    std::thread t([&local]() {         // reference to stack variable
        ...
        std::cout << local << "\n";    // UNDEFINED: local is gone
    });
    t.detach();
    // risky() returns here; local is destroyed; thread still running
}
```

**Rule**: If you must detach a thread, capture only by value, or use shared ownership (`std::shared_ptr`) to guarantee the data outlives the thread.
...
// Each thread detects contention and backs off, but they keep retrying
// at the same time. Solution: random backoff, priority assignment.
```
...
// because other threads monopolize the lock.
// Solution: fair locks, priority queues, or reader-writer locks with
// write-preference.
```
...
// Or use C++11 static local variable (thread-safe guaranteed)
class Singleton2 {
    Singleton2() = default;
...
Calculate the sum of a vector of 10 million integers in parallel using N threads (where N is `hardware_concurrency()`). Each thread sums its own chunk, and the main thread combines the partial sums. Compare the wall-clock time against a single-threaded sum.

### Exercise 2: Thread-Safe Queue

Implement a `ThreadSafeQueue<T>` with `push()`, `pop()` (blocking), and `try_pop()` (non-blocking). Test with multiple producers and multiple consumers.

### Exercise 3: Async Pipeline

Use `std::async` to build a three-stage pipeline: stage 1 reads data, stage 2 processes it, stage 3 writes results. Each stage runs concurrently. Pass results between stages using `std::future`.

### Exercise 4: Dining Philosophers

Implement the dining philosophers problem with 5 philosophers and 5 forks using `std::scoped_lock` to prevent deadlock. Print the state transitions (thinking, hungry, eating) for each philosopher.

### Exercise 5: Reader-Writer Cache

Build a thread-safe cache (`std::unordered_map`) using `std::shared_mutex`. Multiple threads read concurrently, but writes acquire exclusive access. Benchmark the throughput with a 90% read / 10% write workload.

---

## Next Steps

This lesson covered the standard threading primitives. The next lesson dives into advanced concurrency: C++20 synchronization primitives, memory ordering, lock-free programming, and parallel algorithms.

- [Advanced Concurrency](./13_Concurrency_Advanced.md)
