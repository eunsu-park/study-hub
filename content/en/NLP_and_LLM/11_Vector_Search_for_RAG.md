...
Specifically: in `d` dimensions, the ratio of distances to the nearest and farthest neighbor approaches 1 as `d → ∞`. All points become approximately equidistant from a random query — a phenomenon called **distance concentration**. Any branching index loses its discriminating power.
...
```
argmax_i  q · vᵢ    (not the same as nearest L2 unless ||vᵢ|| are equal)
```

Reduction trick: append a coordinate so that `||v'||` becomes constant. For each `vᵢ`, define `v'ᵢ = (vᵢ, sqrt(M² − ||vᵢ||²))` for some `M ≥ max ||vᵢ||`. Then for `q' = (q, 0)`:
...
Explore advanced RAG techniques in [12_Advanced_RAG.md](./12_Advanced_RAG.md).
