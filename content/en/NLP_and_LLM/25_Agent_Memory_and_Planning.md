# 25. Agent Memory and Planning

Previous: [Production LLM Patterns](./24_Production_LLM_Patterns.md) | Next: [Agent Evaluation and Benchmarks](./26_Agent_Evaluation_and_Benchmarks.md)

## Learning Objectives

- Classify memory architectures for agents (episodic, semantic, procedural)
- Implement short-term and long-term memory systems with different storage backends
- Build practical memory patterns: conversation buffer, summary, entity, and vector store
- Design planning frameworks using task decomposition and hierarchical planning
- Apply iterative refinement and self-reflection to improve agent plan quality
...
4. [Summary Memory](#4-summary-memory)
5. [Entity Memory](#5-entity-memory)
6. [Vector Store Memory](#6-vector-store-memory)
7. [Planning Frameworks](#7-planning-frameworks)
8. [Plan-and-Execute Pattern](#8-plan-and-execute-pattern)
...
> **The Memory Problem**
>
> LLMs are stateless by default — each API call starts from scratch.
> Agents need memory to:
> - Maintain context across multi-turn conversations
> - Remember facts learned during task execution
> - Avoid repeating work already completed
> - Build a persistent knowledge base over time

### Three Types of Agent Memory

| Memory Type | Description | Human Analogy | Agent Example |
|-------------|-------------|---------------|---------------|
| **Episodic** | Records of specific events and experiences | "I remember going to Paris in 2019" | Past conversation logs, task execution traces |
| **Semantic** | General facts and knowledge | "Paris is the capital of France" | Extracted entities, learned facts, user preferences |
| **Procedural** | How to do things | "I know how to ride a bicycle" | Tool usage patterns, successful strategies, workflows |
...
### Theory: Short-Term vs Long-Term
...
Short-term memory lives inside the LLM's context window. It is fast but limited in capacity.
...
Long-term memory persists beyond the context window and requires explicit retrieval.
...
|--------|-----------|-----------|
| Storage | Context window (RAM) | Database / file / vector store |
| Capacity | 4K–200K tokens | Unlimited |
| Speed | Instant (in-context) | Requires retrieval step |
| Persistence | Lost after session | Survives across sessions |
| Cost | Tokens per request | Storage + retrieval cost |
| Accuracy | Perfect (verbatim) | Depends on retrieval quality |
...
The simplest memory pattern: keep the last N messages verbatim.
...
Instead of keeping all messages, periodically summarize older context.
...
Entity memory extracts and maintains structured information about people, places, and concepts mentioned in conversation.
...
Vector store memory uses embeddings to store and retrieve memories by semantic similarity, making it the most powerful retrieval mechanism.
...
## 7. Planning Frameworks

### Theory: Planning Frameworks
...
Complex tasks require decomposition into smaller, manageable sub-tasks.
...
## 9. Self-Reflection for Planning

### Theory: Self-Reflection
...
Self-reflection allows agents to evaluate their own plans and outputs, then improve them iteratively.
...
## 10. Memory-Augmented Generation

### Theory: Memory + Planning Together
...
Memory-augmented generation combines retrieval from multiple memory stores with the generation step.
...
Design a memory system with three tiers: **hot** (last 5 messages, always in context), **warm** (last 50 messages, keyword-searchable), and **cold** (all history, stored on disk). Implement the `TieredMemory` class with `add()`, `search()`, and `get_context()` methods. The `search()` method should first check hot, then warm, then cold, returning as soon as it finds a match.
...
**Key design points:**
- Hot tier is always available in the context window — zero retrieval cost
- Warm tier requires keyword search but is still in-memory — fast
- Cold tier is on disk and only accessed as a last resort — handles unbounded history
- Overflow cascades automatically: hot -> warm -> cold
</details>

---

### Exercise 2: Planning with Dependency Resolution

Implement a `DependencyPlanner` that takes a list of tasks with dependencies and returns a valid execution order. It should detect circular dependencies and support parallel execution where possible.
...
**Key points:**
- Topological sort via Kahn's algorithm groups tasks into parallelizable waves
- Wave 1 tasks have no dependencies and can all run simultaneously
- Circular dependency detection catches impossible plans before execution begins
- Critical path analysis identifies the bottleneck chain
</details>

---

### Exercise 3: Summary Memory with Importance Scoring

Build a `SmartSummaryMemory` that uses importance scoring to decide what to keep verbatim, what to summarize, and what to discard. Messages with importance >= 0.8 are always kept verbatim. Messages with importance 0.4-0.8 are summarized. Messages below 0.4 are discarded.
...
- Not all messages deserve the same treatment — triage early
- High-importance messages (decisions, errors, deadlines) are never compressed
- Medium-importance messages are summarized rather than kept verbatim
- Low-importance chit-chat is discarded entirely, saving token budget
- The importance scorer can be replaced with an LLM-based scorer in production
</details>

---

### Exercise 4: Iterative Plan Refinement

Implement a `RefinablePlan` class that supports creating a plan, executing steps, detecting failures, and regenerating the remaining plan. The agent should be able to substitute alternative approaches when a step fails.
...
**Key patterns:**
- Each step can have alternative actions (fallback strategies)
- Retry logic is per-action, not just per-step
- Failed steps can be skipped so the pipeline continues
- Execution log provides full observability for debugging
- Context flows from one step to the next (chaining)
</details>
...
Implement a vector memory store that uses the Ebbinghaus forgetting curve to decay memory importance over time. Memories that are accessed frequently decay slower. Include a `consolidate()` method that removes decayed memories below a threshold.
...
**Key principles:**
- The Ebbinghaus forgetting curve (`R = e^(-t/S)`) models natural memory decay
- Stability increases with each memory access (spaced repetition effect)
- Effective importance = base importance x retention, so even important memories fade if never accessed
- Consolidation permanently removes decayed memories, keeping the store efficient
- Searching for a memory strengthens it (just like in human memory)
</details>
...
In [Agent Evaluation and Benchmarks](./26_Agent_Evaluation_and_Benchmarks.md), we explore how to systematically measure agent performance using established benchmarks and build custom evaluation frameworks.
