...
**A.1 Query–document mismatch.** The user asks "what's the deal with the new chip?" but the corpus phrases it as "Foundry-Series-7nm announcement". A single embedding of the user's question lands far from the relevant chunk.

**A.2 Multi-hop questions.** "Who founded the company that makes the iPhone?" requires retrieving Apple → Steve Jobs across two facts. Single retrieval can find one or the other, rarely the joint reasoning chain.

**A.3 Context-window saturation.** A long document might be split into 50 chunks. Retrieval finds the right chunk, but the answer requires understanding context from chunks 5 chunks earlier.

**A.4 Retrieval failure.** The relevant fact is not in the corpus, or is buried below the cutoff. Single-shot RAG silently produces a confident wrong answer (hallucination from in-context distractors).
