...
```
ADD <network> <container-id> <netns>     # set up networking for a new container
DEL <network> <container-id> <netns>     # tear it down
CHECK <network> <container-id> <netns>   # verify the setup
```

When the kubelet creates a Pod, the runtime (containerd/CRI-O) calls the configured CNI plugin's `ADD`. The plugin's job is exactly:

1. Allocate an IP from the cluster pod CIDR (or whatever IPAM scheme is configured).
2. Create a network interface inside the Pod's network namespace.
3. Wire it up so traffic can reach the rest of the cluster (route tables, encapsulation tunnels, BGP advertisements, eBPF programs — implementation choice).
4. Return the IP and any routes back to the runtime as JSON.
