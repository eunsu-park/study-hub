# 05. Direct Preference Optimization

**Previous**: [Constitutional AI](./04_Constitutional_AI.md) | **Next**: [Scalable Oversight](./06_Scalable_Oversight.md)

## Learning Objectives
- Derive the DPO objective from the RLHF framework and explain its theoretical basis
- Implement DPO training with PyTorch and compare it to PPO-based RLHF
- Understand and implement KTO, IPO, ORPO, and SimPO as alternative preference methods
- Evaluate the trade-offs between online and offline preference optimization
- Choose the appropriate preference optimization method for a given alignment scenario

---

Direct Preference Optimization (DPO) is a landmark contribution to alignment
methodology. It achieves the same objective as RLHF -- optimizing a language model
to match human preferences -- but eliminates the need for training a separate reward
model and running reinforcement learning. By reparameterizing the RLHF objective,
DPO converts the alignment problem into a simple classification-like loss on
preference pairs. This lesson covers DPO and its successors, providing both
theoretical grounding and practical implementation guidance.

## Table of Contents
1. [DPO Derivation and Intuition](#1-dpo-derivation-and-intuition)
2. [DPO vs RLHF Comparison](#2-dpo-vs-rlhf-comparison)
3. [KTO: Kahneman-Tversky Optimization](#3-kto-kahneman-tversky-optimization)
4. [IPO: Identity Preference Optimization](#4-ipo-identity-preference-optimization)
5. [ORPO: Odds Ratio Preference Optimization](#5-orpo-odds-ratio-preference-optimization)
6. [SimPO: Simple Preference Optimization](#6-simpo-simple-preference-optimization)
7. [Choosing Between Methods](#7-choosing-between-methods)
8. [Implementation with Transformers and TRL](#8-implementation-with-transformers-and-trl)
9. [Offline vs Online Preference Optimization](#9-offline-vs-online-preference-optimization)
10. [Practical Considerations](#10-practical-considerations)
11. [Exercises](#exercises)

---

## 1. DPO Derivation and Intuition

### 1.1 From RLHF to DPO

The key insight of DPO is that the optimal policy under the RLHF objective
(maximize reward while staying close to the reference policy) has a closed-form
solution. This allows us to bypass the RL step entirely.
...
### 2.2 When to Choose Each Method
...
KTO (Kahneman-Tversky Optimization) addresses a major practical limitation of DPO:
it does not require paired preferences. Instead, it uses binary feedback (good/bad)
...
IPO (Identity Preference Optimization) addresses a theoretical concern with DPO:
the DPO loss can overfit to the preference data because it tries to drive the
likelihood ratio to infinity for strongly preferred examples.
...
ORPO (Odds Ratio Preference Optimization) takes a different approach: it
eliminates the need for a reference model entirely by incorporating the
preference objective directly into the SFT loss.
...
SimPO (Simple Preference Optimization) simplifies DPO further by using
length-normalized log-probabilities and removing the reference model dependency.
...
### 7.3 Methods at a Glance

The table below summarizes the key properties of each method for quick reference:

| Method | Reference Model Required | Paired Data Required | Computational Cost | Key Advantage | Key Limitation |
|--------|------------------------|---------------------|-------------------|---------------|----------------|
| DPO | Yes | Yes | Low | Simple, stable | Offline only, distribution shift |
| KTO | Yes | No (binary signals) | Low | Works with thumbs up/down | Less precise than paired |
| IPO | Yes | Yes | Low | Theoretically grounded | Similar to DPO in practice |
| ORPO | No | Yes | Very Low | No reference model needed | Newer, less validated |
| SimPO | No | Yes | Very Low | Length-normalized, simple | Very new, limited studies |
| Online DPO | Yes | Yes (iterative) | Medium | Addresses distribution shift | More complex pipeline |

---

## 8. Implementation with Transformers and TRL
...
## 9. Offline vs Online Preference Optimization

### 9.1 The Online-Offline Spectrum
...
### 9.2 Distribution Shift Analysis
...
### 10.1 Common Pitfalls and Solutions
...
### 10.3 Distribution Collapse and Covariate Shift

Two failure modes that deserve special attention in offline preference optimization are
distribution collapse and covariate shift. Understanding them helps practitioners
diagnose degraded models and choose the right mitigation strategy.

**Distribution collapse** occurs when DPO training causes the policy to assign
near-zero probability to all but a narrow subset of responses. The model effectively
"collapses" onto a small region of the output space. Observable signs include:

- Reduced output diversity: similar or identical responses across varied prompts
- Repetitive generations with limited vocabulary or phrasing variation
- Loss of capability on tasks not represented in the preference dataset

Distribution collapse is most likely when beta is low (weak regularization), the
preference dataset is narrow in coverage, or training runs for too many steps.

**Covariate shift** is a subtler problem. Offline DPO is trained on a fixed dataset
generated by some earlier policy (typically the SFT model). As training progresses and
the policy diverges from that reference point, the training distribution no longer
matches the distribution the current policy would generate. The model is being
optimized against data that is increasingly off-distribution for itself. This causes
performance to degrade after an initial improvement, especially over long training runs.

**Offline vs. online DPO — when to switch:**

- Use **offline DPO** when you have a high-quality, well-curated preference dataset,
  compute is limited, and the task distribution is relatively narrow.
- Switch to **online or iterative DPO** when: you observe distribution collapse or
  covariate shift symptoms, the task distribution is broad or open-ended, or you need
  continuous improvement as the model improves.

**Small dataset guidance:** With fewer than approximately 5,000 preference pairs, DPO
is prone to overfitting to the training examples rather than learning a generalizable
preference signal. Practical mitigations include:

- Increase beta (e.g., from 0.1 to 0.3–0.5) to apply stronger regularization toward
  the reference policy and slow down policy movement.
- Augment data through paraphrasing: generate semantically equivalent variants of
  existing chosen/rejected pairs to expand coverage without new annotation.
- Switch to KTO, which works with unpaired binary feedback and can leverage a larger
  pool of single-response ratings instead of requiring matched pairs.

---

## Exercises

### Exercise 1: DPO Implementation from Scratch

Implement a complete DPO training pipeline with model training and evaluation.
...
### Exercise 2: Multi-Method Comparison

Implement DPO, IPO, and KTO losses and compare their behavior on the same data.
...
Analyze how the DPO beta parameter affects training dynamics and final model quality.
...
### Exercise 4: Preference Data Quality Auditor

Build a tool that audits preference data quality and suggests improvements.
...
### Exercise 5: Method Ablation Study

Run an ablation study comparing DPO, IPO, KTO, and SimPO on the same dataset
