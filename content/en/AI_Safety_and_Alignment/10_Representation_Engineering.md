# Lesson 10: Representation Engineering

[Previous: Robustness and Adversarial](./09_Robustness_and_Adversarial.md) | [Next: Guardrails and Filters](./11_Guardrails_and_Filters.md)

---

## Learning Objectives

- Understand representation engineering as a framework for reading and controlling model behavior through internal representations
- Train linear probes to detect safety-relevant features (honesty, harmfulness, refusal) in model activations
- Implement activation steering to add or remove concepts from model behavior at inference time
- Apply the contrast pairs methodology to identify meaningful directions in representation space
- Compare RepE with fine-tuning approaches and evaluate practical applications and limitations

---

> **Prerequisite note**: This lesson assumes familiarity with neural network internals (activations, hidden states, layers) and basic linear algebra. It builds on the alignment methods from Lessons 3-6 and the evaluation methods from Lessons 7-8. Representation engineering takes a fundamentally different approach: instead of training the model differently, we directly read and manipulate its internal representations.

---

## Table of Contents

1. [Representation Engineering Overview](#1-representation-engineering-overview)
2. [Linear Probes for Safety-Relevant Features](#2-linear-probes-for-safety-relevant-features)
3. [Activation Steering](#3-activation-steering)
4. [Contrast Pairs Methodology](#4-contrast-pairs-methodology)
5. [Reading Model Representations](#5-reading-model-representations)
6. [Controlling Model Representations](#6-controlling-model-representations)
7. [Safety-Relevant Directions](#7-safety-relevant-directions)
8. [RepE vs Fine-Tuning](#8-repe-vs-fine-tuning)
...
## 1. Representation Engineering Overview
...
The two-phase workflow for representation engineering — probe training and activation steering — is summarized below:
...
Probe training (Step 1) is an offline process: it identifies *where* and *how* a concept is encoded. Activation steering (Step 2) is applied at inference time and requires no weight updates.

---

## 2. Linear Probes for Safety-Relevant Features
...
### RepE Reproducibility and Capability Degradation

Two practical concerns arise when deploying representation engineering in real systems:

**Reproducibility.** Probe accuracy is sensitive to layer choice, random seed, and dataset composition. A probe trained on one set of contrast pairs may identify a direction vector that differs noticeably from one trained on a different but equally valid dataset. Different runs — especially across model versions or fine-tunes — can converge on distinct "safety directions" that may not generalize. Best practices include:

- Report results across multiple layers (e.g., layers 8, 12, 16, 20 of a 24-layer model) to show where the concept is most robustly encoded.
- Run each probe experiment with at least three different random seeds and report mean ± standard deviation of accuracy and AUC-ROC.
- Use held-out contrast pairs that were not seen during probe training to validate that the direction generalizes beyond the training distribution.

**Capability degradation monitoring.** Activation steering modifies the model's forward pass globally at a chosen layer — it does not affect only the target concept. Adding a steering vector with a large coefficient α can suppress unrelated capabilities (coherence, factual recall, instruction following) in ways that are hard to predict before deployment. Recommended monitoring protocol:

- Measure perplexity on a held-out general-domain corpus before and after steering. A meaningful perplexity increase (e.g., >5%) indicates broad capability impact.
- Track benchmark scores (MMLU, HellaSwag, or task-specific evals) before and after applying the steering vector. A reasonable degradation threshold is a maximum 2% drop on general benchmarks.
- Test edge cases: very short inputs, multilingual inputs, and inputs far from the training distribution of the probe. These are the most likely points of steering instability.
...
## 5. Reading Model Representations
...
## 6. Controlling Model Representations
...
- **Representation engineering (RepE)** provides a framework for understanding and controlling model behavior through internal representations, based on the Linear Representation Hypothesis: high-level concepts are encoded as linear directions in representation space.
- **Linear probes** are simple linear classifiers trained on model activations that detect safety-relevant features. High probe accuracy proves a concept is linearly encoded at that layer. Training probes across layers reveals where concepts are best represented.
- **Activation steering** modifies model behavior at inference time by adding concept direction vectors to hidden states during the forward pass. The steering strength alpha controls the effect, and the entire modification is reversible.
- **Contrast pairs** are the standard methodology for finding concept directions: create pairs of texts differing only in the target concept, extract activations, and take the mean difference as the direction. Methods include mean difference, PCA, and logistic regression.
- **Representation reading** uses extracted directions to "read" what the model is thinking — detecting honesty, harmfulness, refusal, and other concepts from activations. This enables real-time safety monitoring during generation.
- **Representation control** steers model behavior by adjusting safety profiles (honesty boost, harm reduction, refusal adjustment) through activation modification. This is cheaper and more reversible than fine-tuning.
- **Safety-relevant directions** include honesty, harmfulness, refusal, sycophancy, confidence, and power-seeking. These form a catalog of tools for monitoring and controlling model safety properties.
- **RepE vs fine-tuning**: RepE is cheaper, reversible, and more precise but limited to linearly represented concepts. Fine-tuning is more powerful but expensive, irreversible, and risks capability loss. They are complementary approaches.
- **Practical applications** include real-time safety monitoring, adaptive safety levels, deception detection, and interpretability. RepE provides a unique window into model internals that output-level methods cannot offer.
- **Key limitations** include the linearity assumption, layer selection sensitivity, concept entanglement, scale uncertainty, and adversarial vulnerability. Orthogonalization of directions helps mitigate entanglement.

---

## Exercises

### Exercise 1: Training a Honesty Probe

Create 20 contrast pairs for honesty (10 honest, 10 deceptive statements). Extract activations from a language model at 3 different layers. Train linear probes at each layer and compare accuracy. Identify which layer best encodes honesty. Plot the probe accuracy across layers.
...
### Exercise 2: Activation Steering for Safety

Implement activation steering on a GPT-2 model. Extract a "harmfulness" direction using 10 contrast pairs. Generate text from 5 prompts with steering strengths alpha = {-2, -1, 0, 1, 2}. Evaluate whether positive alpha (reducing harmfulness) makes outputs safer and whether the outputs remain coherent. Use an LLM judge for evaluation.
...
### Exercise 3: Concept Entanglement Analysis

Extract directions for at least 4 safety concepts (honesty, harmfulness, refusal, sycophancy). Compute pairwise cosine similarities to measure entanglement. Apply Gram-Schmidt orthogonalization and show that entanglement is reduced. Verify that orthogonalized directions still classify correctly on held-out contrast pairs.
...
### Exercise 4: RepE vs Fine-Tuning Comparison

Compare representation engineering against fine-tuning on a simple safety task. Train a classifier, then: (a) apply RepE steering, (b) fine-tune on safety data. Compare both methods on clean accuracy, safety improvement, compute time, and reversibility. Create a comparison table and identify when each approach is preferable.
...
### Exercise 5: Safety Monitoring Dashboard

Build a real-time safety monitoring dashboard that tracks multiple concept scores during text generation. For each generated token, compute projections onto honesty, harmfulness, and refusal directions. Visualize the concept trajectories as line plots over token positions. Implement an alert system that flags when any concept exceeds a safety threshold.
...
[Previous: Robustness and Adversarial](./09_Robustness_and_Adversarial.md) | [Overview](./00_Overview.md) | [Next: Guardrails and Filters](./11_Guardrails_and_Filters.md)
