...
- **Container** (file format): defines how bytes are laid out on disk and how tracks are interleaved. MP4, AVI, MKV, WebM are containers.
- **Codec** (encoder/decoder): defines how pixel data is compressed. H.264, H.265/HEVC, VP9, AV1 are codecs. OpenCV uses FFmpeg or GStreamer to decode these.

The distinction matters because a file ending in `.mp4` tells you nothing about whether your system can decode it — the codec is what matters. OpenCV's codec support depends on the FFmpeg build it was compiled against, which is why the same `VideoCapture` code may fail on one machine and work on another.

Key properties you can query (`cap.get(cv2.CAP_PROP_*)`):

- `CAP_PROP_FRAME_WIDTH`, `CAP_PROP_FRAME_HEIGHT` — per-frame image dimensions.
- `CAP_PROP_FPS` — nominal frame rate.
