...
### Batch Partial Failure (HTTP 207 Multi-Status)

When processing multiple items in a single request (bulk create, bulk update), some operations may succeed while others fail. Returning a single top-level status code loses information. Use HTTP 207 Multi-Status with per-item results instead:
...
async def bulk_create_users(users: list[BulkUserCreate]):
    """
    Bulk create users. Returns 207 Multi-Status with per-item outcome.

    Example response:
    {
        "results": [
            {"index": 0, "status": 201, "id": 101, "email": "alice@example.com"},
            {"index": 1, "status": 409, "error": {"code": "DUPLICATE_EMAIL",
                                                   "message": "Email already exists"}},
            {"index": 2, "status": 201, "id": 103, "email": "carol@example.com"}
        ],
    ...
    for i, user in enumerate(users):
        ...
        is_duplicate = user.email == "bob@example.com"
    ...
    return JSONResponse(
        status_code=207,  # 207 Multi-Status: some may have failed
        content={
            "results": results,
            "summary": {"total": len(users), "succeeded": succeeded, "failed": failed},
        },
    )
```

Use 207 any time the overall request "succeeded" as an operation but individual items have mixed outcomes. Return 400 or 422 (not 207) if the entire request is structurally invalid before any processing begins.
