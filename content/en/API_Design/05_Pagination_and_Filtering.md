...
Opaque cursors should be tamper-resistant. A client that modifies a cursor could expose internal data or trigger server errors:
...
CURSOR_SECRET = "your-secret-key-change-in-production"

def encode_cursor_signed(data: dict) -> str:
    """Encode a cursor and sign it with HMAC-SHA256 to prevent tampering."""
    payload = base64.urlsafe_b64encode(json.dumps(data).encode()).decode()
    signature = hmac.new(
        CURSOR_SECRET.encode(), payload.encode(), hashlib.sha256
    ).hexdigest()
    return f"{payload}.{signature}"

def decode_cursor_signed(cursor: str) -> dict:
    """Verify the HMAC signature and decode the cursor. Raises ValueError on tampering."""
    try:
...
### Large Dataset Total Count

`SELECT COUNT(*)` becomes expensive at millions of rows because the database must scan all matching rows. Alternatives:

- **Approximate count from catalog** (PostgreSQL): `SELECT reltuples FROM pg_class WHERE relname = 'users'` — returns a statistics estimate, updated by autovacuum. Fast but imprecise (may be off by thousands).
- **Cached count with periodic refresh**: store the count in Redis, update it on inserts/deletes, and refresh via a scheduled job every few minutes. Balances freshness and cost.
- **Omit the total entirely**: some APIs (including the GitHub API) return only `has_next_page` and a cursor, leaving total count as an opt-in or not providing it at all. This is the most scalable approach for very large datasets.
...
| Strategy | Best For | Consistency | Performance | Supports Jump | Cursor Tampering Risk |
|----------|---------|-------------|-------------|--------------|----------------------|
| Offset | Small datasets, admin UIs | Low (page drift) | Degrades with depth | Yes | N/A |
| Cursor | Feeds, timelines | High | Constant | No | Yes (needs signing) |
| Keyset | Large sorted datasets | High | Constant | No | Low |
