...
### SSRF Prevention in Webhook Registration

Validating only the URL string is not enough. Attackers can use DNS rebinding to register a public hostname that later resolves to an internal IP. Always resolve DNS and check the resolved address at registration time:
...
def validate_webhook_url_strict(url: str) -> None:
    """
    Validate a webhook URL against SSRF vectors.

    Blocks:
    - Non-HTTPS schemes
    - Localhost and loopback (127.x, ::1)
    - Private RFC 1918 ranges (10.x, 172.16-31.x, 192.168.x)
    - Link-local addresses (169.254.x — AWS instance metadata endpoint lives here)
    - Reserved and documentation ranges
    - IPv6 loopback (::1)

    DNS is resolved at validation time to prevent DNS rebinding attacks
    where a hostname initially resolves to a public IP but later switches
    ...
    for _, _, _, _, sockaddr in addr_infos:
        ...
        if (
            ...
            or ip.is_private      # 10.x, 172.16-31.x, 192.168.x
            or ip.is_link_local   # 169.254.x.x (AWS metadata: 169.254.169.254)
            or ip.is_reserved     # other reserved ranges
            or ip.is_unspecified  # 0.0.0.0 / ::
        ):
            raise ValueError(
                f"Webhook URL resolves to a blocked IP address: {ip} ({hostname})"
            )
```

Apply this check at registration time, and optionally re-validate at delivery time if the registration-to-delivery interval is long.
