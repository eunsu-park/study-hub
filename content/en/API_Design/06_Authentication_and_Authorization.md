...
PKCE (RFC 7636) is required for SPAs and mobile apps that cannot keep a `client_secret` confidential. Without PKCE, an attacker who intercepts the authorization code can exchange it for tokens.
...
# Step 1 (client-side): Generate a code_verifier and code_challenge before redirecting.
...
# Step 2: Include code_challenge + code_challenge_method=S256 in the auth request URL.
# The auth server stores the challenge and associates it with the auth code.

# Step 3 (token exchange): Send code_verifier with the authorization code.
# The auth server hashes the verifier and compares it to the stored challenge.
# If they match, the token is issued. An intercepted code alone is useless.
...
token_exchange_params = {
    ...
    "redirect_uri": "https://yourapp.com/callback",
    "client_id": "your_client_id",
    "code_verifier": code_verifier,  # proves the requester generated the challenge
    # No client_secret needed — the verifier is the proof of identity
}
```

### When to Use Asymmetric JWT (RS256 / ES256)

Symmetric JWTs (`HS256`) use the same secret to sign and verify. Any service that needs to verify tokens must share the secret — a security risk.

Asymmetric JWTs solve this:
- The **auth server** signs tokens with a **private key** (kept secret).
- Any **downstream service** verifies tokens with the corresponding **public key** (safe to distribute).

```python
# Auth server: sign with RSA private key
import jwt
...
token = jwt.encode({"sub": "user_42", "scope": "read"}, private_key, algorithm="RS256")

# Resource server: verify with RSA public key (no private key needed)
with open("public_key.pem", "rb") as f:
    public_key = serialization.load_pem_public_key(f.read())

payload = jwt.decode(token, public_key, algorithms=["RS256"])
# Use ES256 (ECDSA) for smaller tokens at equivalent security to RS256.
```

Use `RS256`/`ES256` when multiple independent services need to verify tokens without sharing a secret — typical in microservices architectures and multi-tenant platforms.
...
│              │                  │                            │
│ {"alg":"RS256│ {"sub":"user123" │ RSASHA256(                 │
│  "typ":"JWT"}│  "exp":17119...  │   base64(header) + "." +   │
...
└──────────────┴──────────────────┴────────────────────────────┘
       ▲               ▲                    ▲
       │               │                    │
   Algorithm &     Claims &           Verifiable with
   token type     custom data         public key (RS256)
```

| Algorithm | Type | Key | Best For | Security Level |
|-----------|------|-----|----------|---------------|
| HS256 | Symmetric | Shared secret | Single service, internal APIs | Medium |
| RS256 | Asymmetric | RSA key pair | Microservices, public APIs | High |
| ES256 | Asymmetric | ECDSA key pair | Mobile, performance-critical | High |
| EdDSA | Asymmetric | Ed25519 key pair | Modern systems | Very High |
