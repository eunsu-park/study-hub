...
**Previous**: [Error Handling](./08_Error_Handling.md) | **Next**: [API Documentation](./10_API_Documentation.md)
...
When Redis is unavailable, the rate limiter cannot check counters. You must decide between two failure modes:
...
async def check_rate_limit_fail_open(key: str, redis_client: redis.Redis) -> bool:
    """
    Fail-open: if Redis is down, allow all requests.

    Suitable for: public APIs, read-heavy endpoints, scenarios where
    availability matters more than strict enforcement.
    Risk: a Redis outage removes all rate limiting protection temporarily.
    """
    try:
        ...
        return True  # Redis is down — allow the request


async def check_rate_limit_fail_closed(key: str, redis_client: redis.Redis) -> bool:
    """
    Fail-closed: if Redis is down, reject all requests.

    Suitable for: payment APIs, authentication endpoints, any flow
    where security matters more than availability.
    Risk: a Redis outage causes a full API outage for rate-limited paths.
    """
    try:
        count = await redis_client.incr(key)
        if count == 1:
        ...
        return False  # Redis is down — deny to be safe


@app.middleware("http")
async def rate_limit_middleware(request: Request, call_next):
    client_key = f"rl:{request.client.host}"
    redis_client = request.app.state.redis

    # Choose fail-open or fail-closed based on the route
    if request.url.path.startswith("/api/payments"):
        allowed = await check_rate_limit_fail_closed(client_key, redis_client)
    else:
        allowed = await check_rate_limit_fail_open(client_key, redis_client)
...
The choice between fail-open and fail-closed is a business decision, not a technical one: weigh the cost of downtime (fail-open risk) against the cost of abuse (fail-closed risk).
...
**Previous**: [Error Handling](./08_Error_Handling.md) | [Overview](./00_Overview.md) | **Next**: [API Documentation](./10_API_Documentation.md)
