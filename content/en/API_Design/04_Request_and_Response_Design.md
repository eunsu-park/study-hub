...
Two strategies exist for implementing DELETE:

```python
# Soft delete: mark the record as deleted without removing it.
# Advantages: preserves audit trail, enables undo, satisfies referential integrity.
# Disadvantage: data still occupies storage; GDPR "right to erasure" requires extra steps.
@app.delete("/api/books/{book_id}", status_code=204)
async def soft_delete_book(book_id: int):
    # UPDATE books SET is_deleted=True, deleted_at=NOW() WHERE id=:id
    return Response(status_code=204)

# Hard delete: remove the row from the database.
# Advantages: frees storage, naturally satisfies GDPR deletion requests.
# Disadvantage: no recovery; breaks foreign-key references if not handled.
@app.delete("/api/books/{book_id}", status_code=204)
async def hard_delete_book(book_id: int):
    # DELETE FROM books WHERE id=:id
    return Response(status_code=204)
```

**Choosing the right status code after deletion:**
- `404 Not Found` — the resource no longer exists from the client's perspective. Use this as the default; it is consistent with "the thing you asked for is not here."
- `410 Gone` — explicitly communicates that the resource existed but was intentionally removed. Prefer 410 when clients may have cached the URL and you want them to stop retrying (e.g., a published article that was permanently deleted).
