...
Two common API inventory problems identified in OWASP API9 (Improper Inventory Management):

**Shadow APIs** are undocumented endpoints that exist in a running service but are absent from the OpenAPI specification. They often bypass security controls (authentication, rate limiting, input validation) that were applied to documented routes.

```python
# Detection: compare routes registered in the framework against the OpenAPI spec.
# Run this check in CI to alert on mismatches.
...
def detect_shadow_routes(app: FastAPI) -> list[str]:
    """Return route paths that are registered in the app but absent from the OpenAPI spec."""
    # Collect all routes defined in the OpenAPI schema
    schema = get_openapi(
        title=app.title,
        version=app.version,
        routes=app.routes,
    ...
    # Collect all HTTP routes registered with the framework
    registered_paths = set()
    for route in app.routes:
        if hasattr(route, "path") and hasattr(route, "methods"):
            registered_paths.add(route.path)

    shadow = registered_paths - documented_paths
    return sorted(shadow)


# In CI (e.g., pytest):
def test_no_shadow_routes():
    shadows = detect_shadow_routes(app)
    assert not shadows, (
        f"Shadow API routes detected (undocumented but registered): {shadows}. "
        "Add them to the OpenAPI schema or remove them from the application."
    )
```

**Zombie APIs** are deprecated API versions that still receive traffic long after a migration deadline has passed. They accumulate security debt because patches are applied to the current version but not the old one.

Mitigation strategy:
1. **Sunset headers**: include `Sunset: Sat, 01 Jan 2026 00:00:00 GMT` and `Deprecation: true` in responses from deprecated endpoints so clients are warned programmatically.
2. **Traffic monitoring**: track request counts per API version in your observability stack; alert when a deprecated version still has significant traffic after the migration deadline.
3. **Forced migration deadlines**: announce a hard cutoff date, block deprecated versions at the API gateway (return 410 Gone), and provide a migration guide.

```python
from fastapi import FastAPI
from starlette.middleware.base import BaseHTTPMiddleware

DEPRECATED_PREFIXES = ["/api/v1/", "/api/v2/"]
SUNSET_DATE = "Sat, 01 Jan 2026 00:00:00 GMT"


class DeprecationHeaderMiddleware(BaseHTTPMiddleware):
    """Inject Sunset and Deprecation headers on deprecated API versions."""
