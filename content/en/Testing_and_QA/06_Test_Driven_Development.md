...
There are two major schools of TDD practice:

**Detroit School (Classicist / Inside-Out)**: Originated with Kent Beck. Start with the innermost domain objects and build outward. Use real collaborators whenever practical; avoid mocks except for external boundaries (database, network). Tests verify *state* — what the system produces.

**London School (Mockist / Outside-In)**: Popularized by Steve Freeman and Nat Pryce (*Growing Object-Oriented Software, Guided by Tests*). Start with the outermost acceptance test and work inward, using mocks for undeveloped collaborators. Tests verify *interactions* — how objects communicate.

| Aspect | Detroit (Classicist) | London (Mockist) |
|--------|---------------------|------------------|
| Direction | Inside-out | Outside-in |
| Test doubles | Minimal (real objects) | Heavy (mock collaborators) |
| Verifies | State / output | Interactions / messages |
| Refactoring cost | Low (tests survive internal changes) | Higher (tests coupled to structure) |
| Design guidance | Emergent from domain | Driven by roles and protocols |
| Best for | Domain logic, algorithms | Layered architectures, APIs |

Neither school is universally superior. Many practitioners blend both: London-style for high-level integration design and Detroit-style for domain logic.
