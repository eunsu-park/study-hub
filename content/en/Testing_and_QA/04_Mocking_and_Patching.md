...
```
Module Import Graph — Where to Patch
=====================================

  myapp/client.py          myapp/service.py
  ┌──────────────┐         ┌──────────────────┐
  │  defines     │  import │  uses            │
  │  ApiClient ──┼────────►│  ApiClient       │
  └──────────────┘         └──────────────────┘
        ▲                         ▲
        │                         │
  WRONG patch target        CORRECT patch target
  "myapp.client.ApiClient"  "myapp.service.ApiClient"
...
`spec` validates attribute *names* but does **not** validate method call *signatures*. `autospec=True` (or `create_autospec`) does both — it inspects the real method signatures and raises `TypeError` on wrong arguments.
...
# spec: catches wrong attribute names, but NOT wrong arguments
mock_svc = Mock(spec=UserService)
mock_svc.create_user("alice")          # No error — wrong arg count goes undetected
mock_svc.create_user(1, 2, 3, 4, 5)   # No error — too many args, still silent

# autospec: catches wrong attribute names AND wrong arguments
auto_svc = create_autospec(UserService)
auto_svc.create_user("alice")
# TypeError: missing a required argument: 'email'

auto_svc.create_user("alice", "a@b.com")  # OK — matches real signature
```

Prefer `autospec=True` (or `create_autospec`) over plain `spec` for production code mocks. It catches a class of bugs — such as calling a method with the wrong number of arguments — that `spec` alone misses.
