...
> **Caveat — CI runner instability**: Shared CI runners (GitHub Actions, GitLab shared runners, etc.) have inconsistent CPU performance due to noisy neighbors and thermal throttling. Using `--benchmark-compare-fail` on shared infrastructure can produce spurious failures — a benchmark may appear 20% slower simply because the runner was under contention. Mitigations:
> - Use **dedicated/self-hosted** runners for benchmark jobs
> - Set a generous threshold (e.g., `mean:25%` instead of `mean:10%`)
> - Compare against a baseline recorded on the **same runner class**
> - Use `--benchmark-warmup=on` and increase `--benchmark-min-rounds`
> - Treat CI benchmark regressions as **warnings**, not hard failures, unless you control the hardware
