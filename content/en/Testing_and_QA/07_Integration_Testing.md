...
> **Caveat**: SQLite does **not** enforce foreign key constraints by default. Without explicit activation, `INSERT` statements that violate foreign key relationships will succeed silently. You must enable foreign keys on every connection:
>
> ```python
...
> ```
>
> Place this listener right after `create_engine()`. Without it, referential integrity tests against SQLite are meaningless.
