...
**Critical caveat**: SQLite does **not** enforce foreign key constraints by default. Without explicit activation, `INSERT` statements that violate foreign key relationships will succeed silently, making referential integrity tests meaningless. Enable foreign keys on every connection using a SQLAlchemy event listener:
...
def enable_sqlite_foreign_keys(dbapi_conn, connection_record):
    """Activate FK enforcement for every SQLite connection."""
    cursor = dbapi_conn.cursor()
    cursor.execute("PRAGMA foreign_keys = ON")
    cursor.close()
```

Place this listener immediately after creating the engine so it applies to all connections, including those created by the session fixture.
...
```
Connection / Transaction / Savepoint Stack
═══════════════════════════════════════════

  engine.connect()
  │
  └─► connection ─── begin() ──► TRANSACTION (outer)
       │                          │
       │  Session(bind=connection) │
       │  │                        │
       │  └─► session.begin_nested()  ──► SAVEPOINT sp1
       │       │                            │
       │       │  ── app code: INSERT ──    │  (visible inside session)
       │       │  ── app code: commit() ──  │  → releases sp1
       │       │                            │
       │       │  (listener restarts)       │
       │       └─► session.begin_nested()  ──► SAVEPOINT sp2
       │            │                           │
       │            │  ── more app code ──      │
       │            │                           │
       │  ◄─────────┘                          │
       ...
       └─── transaction.rollback() ◄───────────┘
             ↑
             Everything inside is discarded — DB unchanged
```
