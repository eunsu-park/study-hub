...
The following timeline shows when each scope creates and destroys its fixture across a session with two test modules, each containing two test functions:

```
Session   ┌──────────────────────────────────────────────────────────┐
scope     │  fixture created                          fixture torn  │
          │  ↓                                        down ↓        │
          │  ╔════════════════════════════════════════════════╗      │
          │  ║              fixture alive                     ║      │
          │  ╚════════════════════════════════════════════════╝      │
          └──────────────────────────────────────────────────────────┘
Module    ┌──────────── module_a.py ──────────┐┌── module_b.py ─────┐
scope     │  ╔══════════════════════════╗      ││ ╔════════════════╗ │
          │  ║   fixture alive          ║      ││ ║ fixture alive  ║ │
          │  ╚══════════════════════════╝      ││ ╚════════════════╝ │
          └───────────────────────────────────┘└─────────────────────┘
Class     │  ╔══════════════╗ ╔════════╗      ││ ╔══════╗ ╔══════╗ │
scope     │  ║ ClassA tests ║ ║ClassB  ║      ││ ║ClsC  ║ ║ClsD  ║ │
          │  ╚══════════════╝ ╚════════╝      ││ ╚══════╝ ╚══════╝ │
Function  │  ╔════╗ ╔════╗   ╔════╗          ││ ╔════╗   ╔════╗   │
...
### Fixture Finalization Errors

If teardown code (after `yield`) raises an exception, pytest reports it as an **ERROR**, not a FAILURE. This can mask the original test result — a passing test may appear as ERROR, and a failing test's assertion message is buried. Always guard teardown with `try`/`finally`:
...
def db_connection():
    conn = create_connection("test.db")
    yield conn
    # BAD: if close() raises, the test result is masked
    conn.close()


@pytest.fixture
def db_connection_safe():
    conn = create_connection("test.db")
    try:
        yield conn
    finally:
        # GOOD: cleanup runs even if the test fails,
        # and errors here are reported separately
        try:
            conn.close()
        except Exception:
            pass  # Log but do not mask the original failure
```
