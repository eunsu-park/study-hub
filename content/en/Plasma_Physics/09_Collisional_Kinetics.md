...
The full Boltzmann collision integral is impractical for Coulomb interactions. Because the Coulomb force falls off only as $1/r^2$, the cross-section diverges logarithmically at small scattering angles, making the integral $\int \sigma(\Omega)\,d\Omega$ formally infinite. More importantly, in a plasma the cumulative effect of countless **distant, small-angle deflections** far outweighs the rare close encounters that dominate neutral-gas kinetics. The Coulomb logarithm $\ln\Lambda \sim 10\text{–}20$ quantifies how many orders of magnitude the weak, long-range scatterings outnumber 90° collisions. For these reasons, the full Boltzmann operator is replaced by the **Fokker-Planck approximation**, which retains only the first two moments of the velocity change per collision — a systematic friction (drag) and a diffusion in velocity space. This is exact to leading order in $1/\ln\Lambda$ and is the standard starting point for all plasma transport theory.
...
Neoclassical transport depends on the **collisionality parameter**:

$$\nu_* \equiv \frac{\nu}{\omega_b \epsilon^{3/2}}$$

where $\nu$ is the collision frequency, $\omega_b \sim v_{\text{th}} \epsilon^{1/2} / (qR)$ is the bounce frequency of trapped particles, $q$ is the safety factor, and $R$ is the major radius. The three regimes and their transport coefficient scalings are:

**Banana regime** ($\nu_* \ll 1$): particles complete many bounces before a collision deflects them out of the trapped population. The effective radial step size is the banana width $\Delta_b \sim q\rho_L/\epsilon^{1/2}$, much larger than the classical Larmor step $\rho_L$. Only the fraction $f_{\text{trap}} \sim \epsilon^{1/2}$ of trapped particles contributes, giving:

$$D_{\text{banana}} \sim f_{\text{trap}}\,\frac{\Delta_b^2}{\tau_{\text{coll}}} \sim \frac{q^2}{\epsilon^{3/2}}\,D_{\text{classical}}$$

Enhancement over classical transport: factor $q^2/\epsilon^{3/2}$ (typically $10^2$–$10^3$).

**Plateau regime** ($\nu_* \sim 1$): collision frequency is comparable to the bounce frequency. Trapped and passing populations exchange rapidly; the transport coefficient becomes independent of $\nu$ and $\epsilon$ to leading order:

$$D_{\text{plateau}} \sim \frac{q^2}{\epsilon}\,D_{\text{classical}} \sim \frac{\pi}{2}\frac{q\rho_L^2 v_{\text{th}}}{R}$$

**Pfirsch-Schlüter regime** ($\nu_* \gg 1$): many collisions occur per bounce; trapped particles are constantly scattered back to passing. The residual neoclassical enhancement arises from the return currents needed to maintain $\nabla\cdot\mathbf{j}=0$ in toroidal geometry, and scales as:

$$D_{\text{PS}} \sim (1 + 2q^2)\,D_{\text{classical}}$$

For large $q$ this gives the well-known $q^2$ factor of the Pfirsch-Schlüter diffusion coefficient.

A summary table:

| Regime | $\nu_*$ | $D_{\text{nc}}/D_{\text{cl}}$ | Dominant carriers |
|---|---|---|---|
| Banana | $\ll 1$ | $q^2/\epsilon^{3/2}$ | Trapped particles |
| Plateau | $\sim 1$ | $\sim q^2/\epsilon$ | Both |
| Pfirsch-Schlüter | $\gg 1$ | $1 + 2q^2$ | Passing particles |

Typical tokamak parameters place electrons in the plateau/banana regime and ions in the banana regime.

### 5.4 Bootstrap Current

A remarkable neoclassical effect: a **self-generated toroidal current** flows without any external current drive, driven entirely by pressure gradients.

**Physical origin.** In a pressure-gradient plasma, the passing-particle and trapped-particle populations carry different currents. On the high-field (inner) side of the torus, trapped particles are turned around and converted to passing particles; on the low-field (outer) side the reverse happens. This asymmetry produces a net friction force between passing and trapped electrons, which drives a parallel current — the bootstrap current. The mechanism requires both a finite trapped fraction ($f_{\text{trap}} \sim \epsilon^{1/2}$) and a pressure gradient.

**Bootstrap current density.** The leading-order expression in the banana regime is:

$$\boxed{j_{\text{bs}} \approx -\frac{\epsilon^{1/2}}{B_p}\left[p_e\frac{d\ln n}{dr} + p_e\frac{d\ln T_e}{dr} + \alpha\,p_i\frac{d\ln T_i}{dr}\right]}$$

where $B_p$ is the poloidal field and $\alpha \approx 0.5$–$1.5$ depends on the collisionality. In terms of the total pressure $p = nT$ and pressure gradient length $L_p = -p/(dp/dr)$:

$$j_{\text{bs}} \approx -\frac{\epsilon^{1/2}}{B_p}\,\frac{p}{L_p}\,\hat{f}(\nu_*)$$

where $\hat{f}(\nu_*)$ is a numerical factor of order unity that smoothly connects the banana and Pfirsch-Schlüter limits. A widely-used approximate formula valid across regimes is:

$$j_{\text{bs}} \approx -\frac{\epsilon^{1/2}}{1+\epsilon^2}\,\frac{nT}{eB_p}\,\frac{d\ln p}{dr}$$

**Bootstrap fraction.** Define $f_{\text{bs}} = I_{\text{bs}}/I_p$ where $I_p$ is the total plasma current. For ITER (Q=10 scenario):
- $f_{\text{bs}} \approx 20$–$30\%$ — reduces the external current-drive power requirement significantly.

Advanced "hybrid" and "steady-state" scenarios aim for $f_{\text{bs}} \sim 50$–$100\%$, enabling fully non-inductive (DC) operation without transformer action. High $f_{\text{bs}}$ requires a peaked pressure profile (large $\nabla p$) and a high $\beta_p$ (ratio of plasma pressure to poloidal-field pressure).
