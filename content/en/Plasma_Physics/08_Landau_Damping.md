...
### 6.4 O'Neil's Nonlinear Landau Damping

When the wave amplitude is not infinitesimally small, linear Landau damping ceases to be the whole story. **O'Neil (1965)** showed that the long-time behavior is governed by a competition between the linear damping timescale $\tau_L = 1/|\gamma_L|$ and the **bounce period** of resonant particles trapped in the wave potential.

**Trapping (bounce) frequency.** A resonant particle trapped near the bottom of the wave potential well oscillates at the trapping frequency:

$$
\omega_{\text{tr}} = \sqrt{\frac{eEk}{m}} = \sqrt{\frac{eE_0 k}{m}}
$$

where $E_0$ is the wave electric-field amplitude, $k$ is the wavenumber, $e$ and $m$ are the particle charge and mass. The corresponding bounce period is $\tau_b = 2\pi/\omega_{\text{tr}}$.

**Two regimes.**

- **Linear regime** ($|\gamma_L|\,\tau_b \gg 1$, i.e. $|\gamma_L| \gg \omega_{\text{tr}}$): the wave damps away before trapped particles complete even one bounce. The distribution function in velocity space is smoothed and the standard linear result $E(t)\propto e^{\gamma_L t}$ applies.

- **Nonlinear trapping regime** ($|\gamma_L|\,\tau_b \lesssim 1$, i.e. $|\gamma_L| \lesssim \omega_{\text{tr}}$): trapped particles complete many bounce oscillations before the wave damps. The wave amplitude does **not** decrease exponentially to zero. Instead, energy sloshes back and forth between the wave and the trapped-particle population, producing **oscillations of the electric-field amplitude** at roughly $\omega_{\text{tr}}$:

$$
|E(t)| \sim E_0\left[1 - A\,e^{\gamma_L t}\cos(\omega_{\text{tr}} t + \phi_0)\right]
$$

The criterion for the onset of nonlinear behavior is:

$$
\boxed{|\gamma_L|\,\tau_b < 1 \quad \Longleftrightarrow \quad |\gamma_L| < \omega_{\text{tr}} = \sqrt{\frac{eE_0 k}{m}}}
$$

**Physical picture.** Once trapped, resonant particles execute closed orbits in phase space (the cat's-eye structure of Section 6.2). The phase-space distribution rotates at $\omega_{\text{tr}}$, alternately handing energy back to the wave and absorbing it. Over many bounce periods the distribution becomes phase-mixed, the oscillations damp out due to the spread in individual bounce frequencies ($\delta\omega_{\text{tr}} \propto$ distance from separatrix), and the wave settles to a finite asymptotic amplitude — much larger than zero. This is the hallmark of **nonlinear, non-dissipative Landau damping**.

**Experimental signatures.** The trapping oscillations have been observed directly in both beam-plasma experiments and particle-in-cell simulations (see the Python code in Section 7). They are also the seed mechanism for the formation of phase-space holes (BGK modes) discussed above.

### 6.5 Recurrence and Echoes
