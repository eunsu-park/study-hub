...
### 3.4 Bloch Sphere Visualization
...
### 3.6 Geometric Interpretation of Measurement
...
### 3.7 Mixed States and the Bloch Ball

The Bloch sphere represents *pure* states -- those that can be written as $|\psi\rangle = \cos(\theta/2)|0\rangle + e^{i\phi}\sin(\theta/2)|1\rangle$. However, a qubit can also be in a *mixed* state, described not by a state vector but by a **density matrix** $\rho$. A mixed state arises when we have classical uncertainty about which quantum state the system is in (e.g., after tracing out part of an entangled system).

Any single-qubit density matrix can be written as $\rho = \frac{1}{2}(I + \vec{r} \cdot \vec{\sigma})$, where $\vec{r} = (r_x, r_y, r_z)$ is the **Bloch vector** and $\vec{\sigma} = (X, Y, Z)$ are the Pauli matrices. For pure states, $|\vec{r}| = 1$ (on the surface of the Bloch sphere). For mixed states, $|\vec{r}| < 1$ (inside the sphere). The maximally mixed state $\rho = I/2$ sits at the origin with $\vec{r} = \vec{0}$. The full set of valid states fills the **Bloch ball** (the solid sphere), not just its surface. We will formalize density matrices in [Lesson 6](06_Quantum_Measurement.md) and explore how noise channels transform the Bloch ball in [Lesson 20](20_Noise_and_Quantum_Channels.md).
