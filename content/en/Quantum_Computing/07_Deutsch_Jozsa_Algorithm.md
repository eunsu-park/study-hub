...
$$U_f|x\rangle|-\rangle = U_f|x\rangle \frac{|0\rangle - |1\rangle}{\sqrt{2}} = |x\rangle \frac{|f(x) \oplus 0\rangle - |f(x) \oplus 1\rangle}{\sqrt{2}}$$

**Case $f(x) = 0$**: The output qubit becomes $\frac{|0\rangle - |1\rangle}{\sqrt{2}} = |-\rangle$, so the state is $+1 \cdot |x\rangle|-\rangle$.

**Case $f(x) = 1$**: The output qubit becomes $\frac{|1\rangle - |0\rangle}{\sqrt{2}} = -|-\rangle$, so the state is $-1 \cdot |x\rangle|-\rangle$.
