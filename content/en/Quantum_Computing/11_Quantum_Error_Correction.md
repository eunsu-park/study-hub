...
### 9.4 Concrete Overhead Estimates

The surface code threshold of $\sim 1\%$ is the most practically relevant. Below are representative overhead estimates for a physical error rate $p$:

| Physical error rate $p$ | Code distance $d$ | Physical qubits per logical qubit ($\sim 2d^2$) | Logical error rate (approx.) |
|---|---|---|---|
| $10^{-2}$ (at threshold) | Threshold | — | Cannot improve |
| $10^{-3}$ | $d = 17$ | $\sim 578$ | $\sim 10^{-10}$ |
| $10^{-3}$ | $d = 27$ | $\sim 1{,}458$ | $\sim 10^{-17}$ |
| $10^{-4}$ | $d = 7$ | $\sim 98$ | $\sim 10^{-10}$ |

For running Shor's algorithm on RSA-2048, approximately 4,000 logical qubits are needed. At $p = 10^{-3}$ with $d = 17$, this requires roughly $4{,}000 \times 578 \approx 2.3$ million physical qubits. Reducing the physical error rate to $10^{-4}$ would cut this to $\sim 400{,}000$ physical qubits — illustrating why improving gate fidelity has enormous practical impact.
