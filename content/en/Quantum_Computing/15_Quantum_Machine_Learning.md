...
### 7.4 Dequantization Results

A major development tempering quantum ML enthusiasm was the **dequantization** program initiated by Ewin Tang (2018). Tang showed that the quantum recommendation system algorithm of Kerenidis and Prakash — previously believed to offer an exponential quantum speedup — could be matched by a classical algorithm with only polynomial overhead, provided the classical algorithm has "sample and query" access to the data (analogous to quantum amplitude access).

Subsequent work by Chia, Gilyen, Li, Lin, Tang, and Wang extended dequantization to other quantum linear algebra algorithms:

| Quantum algorithm | Original claimed speedup | Dequantized? |
|---|---|---|
| Quantum recommendation (Kerenidis-Prakash) | Exponential | **Yes** (Tang 2018) |
| Quantum PCA | Exponential | **Yes** (Tang 2018) |
| Quantum supervised clustering | Exponential | **Yes** (Chia et al. 2020) |
| Quantum SVM (least-squares) | Exponential | **Yes** (Chia et al. 2020) |
| HHL linear systems solver | Exponential (with caveats) | **Partially** (depends on output requirements) |

The key lesson: many quantum ML speedups relied on the assumption that loading classical data into quantum states is "free." When the cost of data loading is properly accounted for, the quantum advantage often shrinks or disappears. Genuine quantum advantages are most likely for *quantum data* (data generated by quantum processes) or problems where the quantum structure of the feature map is essential and classically inimitable.

### 7.5 Where Quantum ML Might Help
