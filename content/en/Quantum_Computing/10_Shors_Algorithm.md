...
Each control qubit j triggers U^(2^j) where U|y⟩ = |a·y mod N⟩
```
...
### 4.4 Why the QFT Reveals the Period
...
### 4.5 Continued Fraction Algorithm
...
### 6.5 Detailed Continued Fraction Extraction for N = 15

Let us trace the continued fraction algorithm for the measurement outcome $m = 384$ with $2^t = 512$.

We compute the continued fraction expansion of $384/512 = 3/4$:

$$\frac{384}{512}: \quad 384 = 0 \times 512 + 384, \quad 512 = 1 \times 384 + 128, \quad 384 = 3 \times 128 + 0$$

Continued fraction coefficients: $[0; 1, 3]$.

Convergents:
- $a_0 = 0$: $h_0/k_0 = 0/1$
- $a_1 = 1$: $h_1/k_1 = 1/1$
- $a_2 = 3$: $h_2/k_2 = (3 \times 1 + 0)/(3 \times 1 + 1) = 3/4$

The convergent $3/4$ has denominator $k = 4 \leq N = 15$, so the candidate period is $r = 4$. We verify: $7^4 = 2401 \equiv 1 \pmod{15}$. Confirmed.

For $m = 128$: $128/512 = 1/4$. The continued fraction is $[0; 4]$, giving convergent $1/4$, so $r = 4$ directly.

For $m = 256$: $256/512 = 1/2$. The continued fraction gives $r = 2$. We check: $7^2 = 49 \equiv 4 \not\equiv 1 \pmod{15}$, so $r = 2$ is incorrect. We can try doubling: $2r = 4$, and verify $7^4 \equiv 1$. This illustrates that sometimes the continued fraction gives a *divisor* of the true period, which can be handled by trying small multiples.
...
In August 2024, NIST finalized the first three post-quantum cryptographic standards (FIPS 203, 204, 205), marking a major milestone in preparing for the quantum threat:

- **ML-KEM (FIPS 203)** — formerly CRYSTALS-Kyber: Lattice-based key encapsulation for key exchange
- **ML-DSA (FIPS 204)** — formerly CRYSTALS-Dilithium: Lattice-based digital signatures
- **SLH-DSA (FIPS 205)** — formerly SPHINCS+: Hash-based digital signatures (stateless)

A fourth standard, **FN-DSA** (formerly Falcon, NTRU-lattice-based signatures), is expected to follow. These algorithms are designed to resist both classical and quantum attacks, and organizations are urged to begin migration immediately due to the "harvest now, decrypt later" threat.
