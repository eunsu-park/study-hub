...
The state vector formalism describes **pure states** — states with complete quantum information. However, partial measurement and tracing out subsystems naturally produce **mixed states**, which require the **density matrix** formalism.

**Pure state density matrix**: For a state $|\psi\rangle$, the density matrix is:

$$\rho = |\psi\rangle\langle\psi|$$

This is a rank-1, positive semidefinite matrix with $\text{Tr}[\rho] = 1$ and $\text{Tr}[\rho^2] = 1$.

**Mixed state density matrix**: When a system is in state $|\psi_i\rangle$ with classical probability $p_i$, the density matrix is:

$$\rho = \sum_i p_i |\psi_i\rangle\langle\psi_i|$$

For a mixed state, $\text{Tr}[\rho^2] < 1$. The **purity** $\text{Tr}[\rho^2]$ ranges from $1/d$ (maximally mixed, $d$ = dimension) to $1$ (pure).

**Partial trace**: When a bipartite system $AB$ is in state $\rho_{AB}$, the reduced state of subsystem $A$ is obtained by tracing out $B$:

$$\rho_A = \text{Tr}_B[\rho_{AB}] = \sum_j (I_A \otimes \langle j|_B) \, \rho_{AB} \, (I_A \otimes |j\rangle_B)$$

For an entangled pure state $|\psi\rangle_{AB}$, the reduced density matrix $\rho_A$ is a mixed state. The von Neumann entropy $S(\rho_A) = -\text{Tr}[\rho_A \log_2 \rho_A]$ quantifies the entanglement between $A$ and $B$.

**Measurement rule for density matrices**: The probability of outcome $m$ and the post-measurement state are:

$$P(m) = \text{Tr}[P_m \rho], \quad \rho_m' = \frac{P_m \rho P_m}{\text{Tr}[P_m \rho]}$$

This generalizes the Born rule to mixed states and is essential for analyzing noise, decoherence, and open quantum systems (see [Lesson 20](20_Noise_and_Quantum_Channels.md)).
