...
```
    |w⟩ (target)
     ^
     |         ● |s_2⟩  (after 2 iterations)
     |       /
     |     ● |s_1⟩  (after 1 iteration)
     |   ↗  2θ
     | /  2θ
     ● |s⟩ = |s_0⟩  (initial, angle θ from |w⊥⟩)
    /|
   / |  θ ≈ 1/√N
  /  |
 /   |
...
Each Grover iteration G = D · O_f:
  1. O_f reflects about |w⊥⟩ axis (flips |w⟩ component)
  2. D   reflects about |s⟩  axis
  Net effect: rotation by 2θ toward |w⟩
```

### 5.3 Grover Iteration as Rotation
...
2. **Quantum counting**: Use quantum phase estimation on the Grover operator $G$ to estimate the number of solutions $M$.

### 6.4 Quantum Counting Algorithm

The **quantum counting algorithm** (Brassard, Hoyer, Tapp 1998) combines Grover's operator with quantum phase estimation (see [Lesson 9](09_Quantum_Fourier_Transform.md)) to estimate the number of solutions $M$ without knowing it in advance.

The eigenvalues of the Grover operator $G$ in the 2D subspace are $e^{\pm 2i\theta}$ where $\sin^2\theta = M/N$. By applying quantum phase estimation to $G$, we obtain an estimate of $\theta$ and hence $M$:

$$M = N \sin^2(\theta)$$

The algorithm uses $O(\sqrt{N/M})$ oracle calls to estimate $M$ with relative accuracy $\epsilon$, matching the query complexity of Grover search itself. This is useful in applications where the number of solutions is unknown and directly affects the optimal iteration count.
