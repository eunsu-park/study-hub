...
[← Previous: Quantum Machine Learning](15_Quantum_Machine_Learning.md) | [Next: Quantum Simulation →](18_Quantum_Simulation.md)
...
Understanding where quantum computing fits in the landscape of computational complexity is essential for assessing quantum advantage. The following table summarizes the key complexity classes:

| Class | Definition | Example problems | Relationship to BQP |
|-------|-----------|-----------------|---------------------|
| **P** | Deterministic polynomial time | Sorting, shortest path, linear programming | $\text{P} \subseteq \text{BQP}$ |
| **BPP** | Bounded-error probabilistic polynomial time | Primality testing (Miller-Rabin), polynomial identity testing | $\text{BPP} \subseteq \text{BQP}$ (widely believed) |
| **BQP** | Bounded-error quantum polynomial time | Factoring (Shor), discrete log, Jones polynomial | — |
| **NP** | Nondeterministic polynomial time (verifiable in P) | SAT, graph coloring, traveling salesman | $\text{BQP} \not\subseteq \text{NP}$ and $\text{NP} \not\subseteq \text{BQP}$ (believed) |
| **QMA** | Quantum Merlin-Arthur (quantum analog of NP) | Local Hamiltonian problem, quantum SAT | $\text{BQP} \subseteq \text{QMA} \subseteq \text{PSPACE}$ |
| **PSPACE** | Polynomial space (possibly exponential time) | QBF, generalized chess, quantum circuit simulation | $\text{BQP} \subseteq \text{PSPACE}$ |

**Key relationships** (known and conjectured):

$$\text{P} \subseteq \text{BPP} \subseteq \text{BQP} \subseteq \text{PSPACE}$$

It is widely believed (but unproven) that $\text{BPP} \subsetneq \text{BQP}$ — that quantum computers can solve strictly more problems efficiently than classical randomized computers. Shor's algorithm provides strong evidence, since integer factoring is in BQP but is not known to be in BPP. However, proving $\text{BQP} \neq \text{BPP}$ would imply $\text{P} \neq \text{PSPACE}$, which is a major open problem.

### 8.2 Fundamental Questions
...
### 8.4 Software and Algorithm Research
