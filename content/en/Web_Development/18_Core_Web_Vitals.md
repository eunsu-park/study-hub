...
Before the reference, read [**Theory & Principles**](#theory--principles) — the three Core Web Vitals (LCP, INP, CLS) measure three different aspects of perceived performance (speed, responsiveness, stability) on the *real* user's device, and each metric maps to a specific bottleneck in the rendering pipeline.

---

## Theory & Principles

"Make the page fast" is too vague to act on. Core Web Vitals translate the vague goal into three quantitative metrics, each measuring a different *moment* in the user's experience: how long until they see the main content (**LCP**), how long the page takes to respond to their input (**INP**), and how much the layout jumps around while they read (**CLS**). Each metric corresponds to a specific stage of the rendering pipeline, which means each one has a finite, predictable list of fixes. The numbers are not arbitrary — Google publishes "good / needs improvement / poor" thresholds based on percentile data from real users.

### A. The Three Metrics and Their Thresholds
...
### B. LCP: Speed of First Meaningful Render
...
### C. INP: Responsiveness Under Real Use
...
- **Understanding Core Web Vitals** (section 1) is §A — the three metrics, the thresholds, what the 75th percentile means.
- **Measuring Core Web Vitals** (section 2) covers field data (Chrome User Experience Report, web-vitals library) vs. lab data (Lighthouse, WebPageTest).
- **LCP** (section 3) is §B — the four bottlenecks and their fixes.
- **INP** (section 4) is §C — long tasks, breaking up work, off-main-thread.
- **CLS** (section 5) is §D — the four causes and their mechanical fixes.
- **Performance Observer API** is the §A measurement primitive that the web-vitals library uses.
- **Resource hints** (`preload`, `prefetch`, `preconnect`, `dns-prefetch`) and **image optimization** are the cross-cutting techniques that improve all three metrics.

Read the rest of the lesson knowing that every optimization is targeted at one specific stage of the pipeline — and the metrics tell you which stage to target.
