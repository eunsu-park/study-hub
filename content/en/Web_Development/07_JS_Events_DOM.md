...
1. **Reading layout values flushes pending writes.** Properties like `offsetTop`, `offsetWidth`, `getBoundingClientRect()`, and `getComputedStyle()` can force the browser to run **synchronous layout** so the answer is correct. Interleaving reads and writes in a tight loop produces "layout thrashing" — `for (each item) { read; write }` is hundreds of forced layouts; `for (read all); for (write all);` is one. The fix is to *batch* reads, then writes.
