...
Before the framework reference, read [**Theory & Principles**](#theory--principles) — the observer pattern that underlies Django signals, the cache hierarchy (LocMem/Memcached/Redis/database) and its consistency tradeoffs, and the middleware chain composition revisited.
...
The "advanced" Django features cluster around three orthogonal patterns, not eight unrelated tools. Understanding each pattern in isolation lets you predict where any new feature in this space will fit.

- **(A) Observer pattern: Django signals as the canonical example** — decoupled "publish/subscribe" within a single process, plus its dangers.
- **(B) The cache hierarchy** — multiple layers with different latencies, capacities, and consistency models, and the cache-aside pattern that wires them in.
- **(C) The middleware chain — what advanced workloads add to it** — async middleware, state injection, response post-processing in production.

### A. The Observer Pattern: Django Signals
...
### C. The Middleware Chain in Production
...
- §1 (Channels for WebSockets) is Django over ASGI (Lesson 02 §A) — async middleware (§C.1) and consumer classes that handle long-lived connections.
- §2 (Celery) is the right home for "slow signal handlers" from §A.3 — explicit out-of-band tasks instead of synchronous receivers.
- §3 (Redis as cache and broker) is the §B.1 backend choice for both caching (§B.2) and the Celery broker that backs §2.
- §4 (Django signals) is the §A observer pattern in concrete code, with the §A.3 dangers spelled out.
- §5 (Management commands) is the CLI counterpart to web requests — same Django app, no HTTP layer.
- §6 (Caching strategies) walks the §B.4 levels of integration with examples.
- §7 (Django middleware) is the §C.1–§C.3 middleware patterns expanded with concrete production examples.
