...
> **Note**: `\frontmatter`, `\mainmatter`, and `\backmatter` are only available in the `book` class. If you need these commands, use `\documentclass{book}` (or `memoir`). The `report` class does not provide them.
...
**Note**: `\frontmatter`, `\mainmatter`, `\backmatter` are only available in the `book` class (and derivatives such as `memoir`). The `report` class does **not** provide these commands.
