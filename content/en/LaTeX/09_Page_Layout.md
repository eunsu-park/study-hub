...
### What Microtype Improves

The `microtype` package makes improvements that are subtle but cumulative across a full document:

- **Character protrusion** (hanging punctuation): Punctuation marks such as commas and periods are allowed to extend slightly beyond the right margin, producing a visually straighter text edge.
- **Font expansion**: Character widths are adjusted by up to ±2% to help TeX find better line-breaking points, resulting in fewer overfull or underfull lines.
- **Reduced hyphenation**: Because font expansion gives the line-breaking algorithm more flexibility, fewer words need to be hyphenated.
- **Tighter justification**: The combination of protrusion and expansion yields more even inter-word spacing and eliminates many "rivers" (chains of white space flowing down a paragraph).

These effects cannot be shown in a side-by-side code listing because they depend on actual PDF rendering, but the difference is clearly visible in long paragraphs of justified text: documents compiled with `\usepackage{microtype}` consistently receive fewer layout warnings and look more professionally typeset.
...
**Previous**: [Cross-References & Citations](08_Cross_References.md)
