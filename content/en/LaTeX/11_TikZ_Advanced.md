...
> **Output:** A 12×8 cm plot titled "Comparison of Activation Functions" with centered axes, major grid lines, and a legend in the lower-right corner. A blue sigmoid S-curve, a red tanh curve (similar shape but steeper), and a green piecewise-linear ReLU (zero for negative $x$, linear for positive $x$) are plotted over the domain $[-5, 5]$ with the y-axis limited to $[-1.5, 1.5]$.
...
> **Output:** A feedforward neural network diagram with three columns of blue filled circles. The leftmost column contains two input neurons ($x_1$, $x_2$), the middle column contains three hidden neurons ($h_1$, $h_2$, $h_3$), and the rightmost column contains one output neuron ($y$). Every input neuron is connected to every hidden neuron, and every hidden neuron connects to the output neuron, with arrows indicating forward direction. Column labels appear above each layer.
...
> **Output:** Three circles arranged horizontally representing states $q_0$, $q_1$, and $q_2$. $q_0$ has an "initial" arrow entering from the left. $q_2$ has a double-circle border indicating an accepting state. A curved arrow labelled "a" goes from $q_0$ to $q_1$, a curved arrow "b" returns from $q_1$ to $q_0$, a straight arrow "c" connects $q_1$ to $q_2$, and a self-loop labelled "a,b,c" loops on $q_2$.
...
> **Output:** A 14×8 cm plot titled "Training Progress" with epoch (1–10) on the x-axis and loss/accuracy (0–1) on the y-axis, major grid lines, and a legend placed outside the plot area to the right. A blue line with circle markers shows decreasing training loss, a red line with square markers shows slightly higher validation loss also decreasing, and a green line with triangle markers shows training accuracy rising from 0.6 to 0.96 over 10 epochs.
