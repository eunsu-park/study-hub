...
> **Output:** A 4×3 cm canvas with a light-gray reference grid. A blue outlined square sits in the lower-left, a red outlined circle in the center, a green outlined ellipse on the right, and a purple outlined triangle in the upper-left. Each shape is labelled below it.
...
> **Output:** A vertical flowchart with a "Start" rectangle at the top, an orange diamond "Check" decision node below it, "Yes Path" and "No Path" rectangles branching left and right, and an "End" rectangle at the bottom. Arrows from both branch boxes reconnect to "End" using L-shaped paths, with "yes" and "no" labels on the decision branches.
...
> **Output:** A Cartesian plane with a light-gray grid, bold x- and y-axes with arrow tips and axis labels ($x$ and $y$), integer tick marks along each axis, and the origin labelled $O$. A smooth blue parabola (the function $0.5x^2 - x + 1$) is plotted from $x=0$ to $x=3.5$.
...
> **Output:** Three blue rectangles labelled "Input", "Process", and "Output" arranged in a horizontal row connected by solid arrows from left to right. A red dashed arrow forms a feedback loop from "Process" down and back left to "Input", illustrating a control-flow cycle.
