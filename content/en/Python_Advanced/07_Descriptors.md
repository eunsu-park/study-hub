...
> **Attribute Lookup Order** (when accessing `obj.attr`)
>
> 1. Check **data descriptor** (`type(obj).__dict__`) -- Has both `__get__` and `__set__`
> 2. Check **instance `__dict__`** (`obj.__dict__`)
> 3. Check **non-data descriptor** -- Has only `__get__`
> 4. Call `__getattr__`
