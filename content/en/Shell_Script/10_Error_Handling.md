...
### The `local var=$(cmd)` Gotcha

The `local` builtin always returns 0, masking the exit code of the command substitution assigned to it. This is a common source of silent failures with `set -e`.

```bash
#!/usr/bin/env bash
set -e

# BUG: local masks the exit code of $(false)
my_func() {
    local result=$(false)  # local returns 0, set -e does NOT trigger
    echo "This still runs! result='$result'"
}

my_func
echo "Script continues — the failure was silently ignored"

# FIX: declare the variable first, then assign separately
my_func_safe() {
    local result
    result=$(false)  # Now set -e correctly catches the failure
    echo "This never runs"
}

my_func_safe  # Script exits here with an error
```

> **Rule of thumb**: Never combine `local` with command substitution in one statement. Declare first, then assign.
