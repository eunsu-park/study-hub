...
> **Portability Note**: Process substitution (`<(...)` and `>(...)`) is a bash (and zsh) extension. It is **not available in POSIX sh**, dash, or other minimal shells. If you need POSIX compatibility, use named pipes (FIFOs) or temporary files instead. Scripts using `#!/bin/sh` must avoid this syntax entirely.
