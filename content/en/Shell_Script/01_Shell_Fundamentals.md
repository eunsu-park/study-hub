# Lesson 01: Shell Fundamentals and Execution Environment
...
| Default on Debian/Ubuntu | bash | dash (/bin/sh) | bash | No |
...
Cron jobs run in a minimal environment that differs significantly from your interactive shell. Scripts that work fine in your terminal often fail silently in cron.

**Key differences**:
- **Minimal PATH**: Cron's default PATH is typically `/usr/bin:/bin`, so commands like `docker`, `node`, or anything in `/usr/local/bin` won't be found.
- **No profile sourcing**: Cron does not source `~/.bash_profile`, `~/.bashrc`, or `/etc/profile`. Environment variables you set there are not available.
- **No terminal**: There is no TTY, so interactive prompts and terminal-dependent commands will fail.
...
# Source environment file if needed
[[ -f "$HOME/.env" ]] && source "$HOME/.env"

# Use absolute paths for commands when possible
LOGGER="/usr/bin/logger"

# Log output for debugging cron failures
...
**Tip**: Test cron behavior locally by running your script with a stripped environment:
...
> **Why `IFS=$'\n\t'`?**
>
> The default `IFS` (Internal Field Separator) is space, tab, and newline. This causes `for` loops to split filenames containing spaces into separate words, leading to subtle bugs.
>
> ```bash
> # BUG: default IFS splits "my file.txt" into "my" and "file.txt"
> for f in $(ls); do
>     echo "Processing: $f"
> done
> # Output:
...
> #   Processing: file.txt
>
> # FIX: set IFS to newline and tab only
> IFS=$'\n\t'
> for f in $(ls); do
>     echo "Processing: $f"
> done
...
> ```
>
> Setting `IFS=$'\n\t'` in your strict mode header prevents word splitting on spaces, so filenames and strings with spaces are handled correctly in unquoted expansions.
