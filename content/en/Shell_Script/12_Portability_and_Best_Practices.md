...
### macOS / BSD Coreutils Differences

Scripts written on Linux often break on macOS because macOS ships BSD-derived coreutils, not GNU coreutils. The following table summarizes the most common incompatibilities:

| Command | GNU/Linux | macOS/BSD | Portable Alternative |
|---------|-----------|-----------|---------------------|
| `sed -i` | `sed -i 's/a/b/' file` | `sed -i '' 's/a/b/' file` | Use `sed -i'' -e 's/a/b/' file` or write to temp file |
| `date` (epoch) | `date -d @1609459200` | `date -r 1609459200` | Use `python3 -c` or `awk` |
| `date` (relative) | `date -d '+1 day'` | Not supported | Use `date -v+1d` on macOS |
| `stat` (file size) | `stat -c%s file` | `stat -f%z file` | Use `wc -c < file` |
| `grep -P` (PCRE) | Supported | Not available | Use `grep -E` or install `ggrep` |
| `readlink -f` | Resolves symlinks fully | Not supported | Use `realpath` or custom function |
| `mktemp` | `mktemp /tmp/foo.XXXX` | Same syntax works | Both support `mktemp -t prefix` |
| `find -printf` | Supported | Not supported | Use `find -exec stat` instead |

**Portable workaround pattern**:

```bash
#!/usr/bin/env bash

# Detect OS and select correct flags
...
"${SED_INPLACE[@]}" 's/old/new/' config.txt
file_size=$("${STAT_SIZE[@]}" config.txt)
```
