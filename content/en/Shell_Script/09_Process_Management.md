...
> **Security Warning**: The `eval "$job"` pattern above is dangerous if job strings come from untrusted input. `eval` executes arbitrary code, enabling command injection (e.g., `"; rm -rf /"` embedded in a job string). For production use, prefer passing function names instead of command strings, or use `bash -c` with properly escaped arguments.
...
> **Note**: Traps are NOT inherited by subshells or background jobs. A trap set in the parent script does not apply to commands run with `(subshell)`, `cmd &`, or pipeline components. Each subshell starts with traps reset to their default behavior. If background jobs need cleanup, you must set traps explicitly inside the subshell or background function.
