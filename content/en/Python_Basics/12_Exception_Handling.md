...
**Previous**: [File I/O](./11_File_IO.md) | **Next**: [Standard Library Essentials](./13_Standard_Library_Essentials.md)

> **Topic**: Python Basics
> **Lesson**: 12 of 14
> **Prerequisites**: Functions, File I/O, Classes and Objects

## Learning Objectives

After completing this lesson, you will be able to:

1. Write `try`/`except`/`else`/`finally` blocks to handle errors gracefully
2. Catch specific exceptions and handle multiple exception types differently
3. Navigate the Python exception hierarchy from `BaseException` down to specific exceptions
4. Raise exceptions with `raise` and chain exceptions using `raise ... from ...`
5. Define custom exception classes tailored to your application's error scenarios
6. Distinguish between LBYL and EAFP programming styles and apply each appropriately
7. Identify and handle the most common built-in exceptions (`ValueError`, `TypeError`, `KeyError`, etc.)
8. Apply exception handling best practices to write robust, maintainable code

---

## Introduction

Errors happen. Users enter invalid data, files go missing, network connections drop, and servers run out of memory. Exception handling is how Python programs respond to these situations gracefully instead of crashing.

An **exception** is an event that disrupts the normal flow of a program. When Python encounters an error, it creates an exception object and "raises" it. If the exception is not caught (handled), the program terminates with a traceback. Exception handling lets you intercept these errors, respond appropriately, and keep your program running.
...
Use `raise` to signal that an error condition has occurred:
...
Sometimes you want to catch an exception, do something (like logging), and then re-raise it:
...
Using `raise` without an argument re-raises the current exception:
...
When an exception occurs inside an `except` block, Python automatically chains them:
...
Raised when a function receives an argument with the right type but invalid value:
...
Try the operation and handle exceptions:
...
| Factor | LBYL | EAFP |
|--------|------|------|
| Pythonic? | Less | More (Python style) |
| Race conditions | Vulnerable | Safe |
| Performance (success) | Slower (check + operate) | Faster (just operate) |
| Performance (failure) | Faster (no exception) | Slower (exception overhead) |
| Readability | Clear preconditions | Clear intent |

**Python convention:** Prefer EAFP. It is the standard Python style, avoids race conditions (e.g., a file could be deleted between your check and your read), and reads naturally.
...
For situations that are not errors but deserve attention:
...
### 1. Be Specific — Do Not Catch Too Broadly
...
### 6. Use Custom Exceptions for Application Logic
...
|---------|---------|--------|
| `try/except` | Catch and handle exceptions | `try: ... except ExcType as e: ...` |
| `else` | Code that runs only on success | `try: ... except: ... else: ...` |
| `finally` | Code that always runs | `try: ... finally: ...` |
| `raise` | Throw an exception | `raise ValueError("msg")` |
| `raise from` | Chain exceptions | `raise NewError() from original` |
| Custom exceptions | Application-specific errors | `class MyError(Exception): ...` |
| EAFP | Try first, handle errors | Python's preferred style |
| LBYL | Check first, then act | Use when checks are cheap |

Key takeaways:
- **Catch specific exceptions** — never use bare `except:` or catch `BaseException`
- **Use `else` and `finally`** to structure your error handling clearly
- **Prefer EAFP** over LBYL for Pythonic code
- **Create custom exceptions** for your application's domain-specific errors
- **Always clean up resources** using `with` statements or `finally`
- **Log exceptions** with enough context to diagnose problems
...
- [Python Errors and Exceptions (Official Tutorial)](https://docs.python.org/3/tutorial/errors.html)
...
- [Effective Python: Know the Differences Between Exceptions](https://effectivepython.com/)
