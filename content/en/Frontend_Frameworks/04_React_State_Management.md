...
```
View → dispatch(action) → reducer(state, action) → newState → View re-renders
```

Three rules make this work:

1. **State lives in a single store** (one source of truth).
2. **State is updated only by reducers** — pure functions `(state, action) => newState`. No in-place mutation; reducers always return a new object.
3. **Components dispatch actions** (`{ type: 'CART_ADD', payload: item }`) — they describe *what happened*, not *how to update*.
