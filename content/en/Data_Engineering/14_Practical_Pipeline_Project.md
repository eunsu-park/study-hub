[← Previous: 13. Data Quality and Governance](13_Data_Quality_Governance.md) | [Next: 15. Airflow TaskFlow API →](15_Airflow_TaskFlow_API.md)
...
#### A.1 Every output table has a customer

Some person or system depends on each gold table — an analyst running daily reports, a dashboard with 50 viewers, an ML model serving 1M requests/hour. That dependency is your customer relationship.

A data product has:
- **A clear consumer** (named team or system).
- **A documented SLA** ("data for date D is available by 6am UTC of D+1, with 99.5% on-time delivery").
- **A schema contract** (column names, types, semantics — and stability commitments).
- **An owner** (the team that fixes it when it breaks).

Without these, every issue becomes "whose problem is this?" With them, ownership is clear and decisions have constraints.

#### A.2 The producer-consumer contract

Producer commitments to consumer:
- "I will deliver this table by time T."
- "I will not change this column's name or type without N days notice."
- "If the upstream source changes, I will adapt — your contract holds."

Consumer commitments to producer:
- "I will read columns I declared in advance, not undocumented ones."
- "I will not assume row order beyond what's documented."
