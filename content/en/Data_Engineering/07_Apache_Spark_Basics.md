[← Previous: 6. Prefect Modern Orchestration](06_Prefect_Modern_Orchestration.md) | [Next: 8. PySpark DataFrames →](08_PySpark_DataFrames.md)
...
- **Multi-stage DAGs in one job.** No need to chain N jobs; Spark plans them as one.
- **In-memory intermediate results.** Stage outputs live in memory (or spill to disk only if memory pressure).
- **Lineage-based recovery instead of replication.** Storage 1x not 3x.
