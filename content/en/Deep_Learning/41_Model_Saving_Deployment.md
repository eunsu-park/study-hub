...
```python
# State dict (preferred): just the parameters
torch.save(model.state_dict(), "model.pt")
# Loading: instantiate model architecture, then load parameters
model = MyModel(); model.load_state_dict(torch.load("model.pt"))

# Full pickle (discouraged): saves the actual Python class
torch.save(model, "model.pt")
# Loading: needs the original class definition available
model = torch.load("model.pt")
```

State dict wins because it is *future-proof*: you can change the Python class implementation, add helper methods, etc., as long as the parameter names and shapes match. Pickled full models break the moment you reorganize your code.
