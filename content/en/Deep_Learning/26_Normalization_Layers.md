...
Later analysis (Santurkar et al. 2018) argued the dominant benefit is **loss-landscape smoothing**: normalized networks have less curvature variation, so gradient steps work consistently across the surface. Larger learning rates become safe; convergence speeds up. This view is consistent with the empirical observation that BN works even when the input distributions are *not* shifting much.
