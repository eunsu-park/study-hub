...
The Variational Autoencoder (Kingma & Welling 2014) is the cleanest example of probabilistic deep learning: a latent variable model trained via variational inference, with a continuous latent space that is both *meaningful* (interpolation works) and *sampleable*. Every concept in the lesson — encoder, decoder, ELBO, KL divergence, reparameterization — comes from a single line of math: the variational lower bound on the marginal likelihood.

This section covers:

- **A.** The latent variable model and intractable marginal likelihood
- **B.** Variational inference and the ELBO derivation
- **C.** The reparameterization trick: how to backprop through sampling
- **D.** Beta-VAE and the disentanglement trade-off
...
```
z ~ p(z) = N(0, I)               (prior over latent)
x ~ p_\theta(x | z)              (decoder, parameterized by neural net)
```
...
This integral is **intractable** for high-dimensional `z`. Maximum likelihood `max_\theta sum_i log p_\theta(x_i)` cannot be computed directly. We need an approximation.

### B. Variational Inference and the ELBO

Variational inference introduces a tractable approximation `q_\phi(z | x)` to the true posterior `p(z | x)` and bounds the log-likelihood:
...
The two terms have intuitive meaning:

- **Reconstruction term** `E_{q}[log p_\theta(x | z)]`: how well does the decoder reconstruct `x` from latent codes sampled from the encoder?
- **KL term** `KL(q_\phi(z | x) || p(z))`: how much does the encoder's posterior deviate from the prior `N(0, I)`?

Maximizing ELBO is a balance: reconstruct well *and* keep latent codes close to a standard normal. The second condition makes the latent space *usable for generation* — sampling `z ~ N(0, I)` produces a plausible code that the decoder can turn into a sample.

For Gaussian `q_\phi(z | x) = N(\mu_\phi(x), \sigma_\phi^2(x))` and Gaussian prior `p(z) = N(0, I)`, the KL has a closed form:

```
KL = 0.5 * sum_j [\mu_j^2 + \sigma_j^2 - log \sigma_j^2 - 1]
```

This is the entire VAE loss (negated): reconstruction MSE + KL.

### C. The Reparameterization Trick

To compute `E_{q_\phi}[...]`, we sample `z ~ q_\phi(z | x)`. But sampling is non-differentiable — the gradient of "draw a sample" with respect to `\phi` does not exist.

The reparameterization trick rewrites the sampling as a deterministic transformation of an exogenous noise variable:

```
z = \mu_\phi(x) + \sigma_\phi(x) \odot \epsilon,   \epsilon ~ N(0, I)
```

Now `z` is a *function* of `\phi` and `\epsilon`. Gradients flow through `\mu_\phi` and `\sigma_\phi` exactly. The randomness lives in `\epsilon`, which has no parameters to differentiate.

This trick is the single ingredient that makes VAE training feasible. Without it, one would need score-function estimators (REINFORCE), which have orders-of-magnitude higher variance and rarely work for high-dimensional latent spaces.

### D. Beta-VAE and Disentanglement

Higgins et al. (2017) added a hyperparameter `\beta` weighting the KL term:

```
ELBO_\beta = E_{q}[log p(x | z)] - \beta * KL(q || p)
```

`\beta = 1` is standard VAE. `\beta > 1` overweights the KL, forcing the encoder to use latent codes that are *more like* an isotropic Gaussian. The empirical effect: each latent dimension tends to capture *one independent factor of variation* (face angle, lighting, expression for face datasets). This is **disentanglement**.

The trade-off is reconstruction quality: heavy KL pressure makes the latent codes less informative about specifics, blurring outputs. Beta-VAE is preferred when interpretability matters (analyzing what the latents represent); plain VAE when sample quality matters more.

### From Theory to the Code Below

| Theory concept | Code construct in this lesson |
|----------------|-------------------------------|
| Encoder produces `\mu, \log \sigma^2` | `mu, log_var = self.encoder(x).chunk(2, dim=-1)` |
| Reparameterization | `z = mu + (0.5 * log_var).exp() * torch.randn_like(mu)` |
| Decoder | `recon = self.decoder(z)` |
| ELBO loss | `recon_loss + kl_loss` (with KL closed form) |
| Beta weighting | `recon_loss + beta * kl_loss` |
