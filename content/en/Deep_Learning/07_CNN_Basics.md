...
The minus sign in `g(i - m, j - n)` flips the kernel before sliding. Deep learning instead uses **cross-correlation** (no flip), but calls it "convolution":

```
y(i, j) = sum_{m, n} x(i + m, j + n) * w(m, n) + b
```

For learned kernels this distinction is harmless — gradient descent simply learns the flipped version of whatever it would have learned with the math convention. With multiple input/output channels, the formula generalizes to:

```
y_{c_out}(i, j) = sum_{c_in} sum_{m, n} x_{c_in}(i + m, j + n) * w_{c_out, c_in}(m, n) + b_{c_out}
```

Output spatial size with kernel `K`, padding `P`, stride `S`:
