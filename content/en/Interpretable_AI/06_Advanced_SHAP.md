# Lesson 6: Advanced SHAP

[Previous: Probing and Representation Analysis](./05_Probing_and_Representation_Analysis.md) | [Next: Concept-Based Explanations](./07_Concept_Based_Explanations.md)

---

## Learning Objectives

- Understand DeepSHAP and GradientSHAP as efficient approximations of Shapley values for deep neural networks
- Compute and interpret SHAP interaction values to identify pairwise feature dependencies
- Apply Asymmetric Shapley values and Causal SHAP to incorporate causal structure into explanations
- Implement computational optimizations (background sampling, batching, GPU acceleration) for production-scale SHAP
- Critically evaluate TreeSHAP pathologies and the Sundararajan & Najmi critique of feature correlation bias

---

> **Prerequisite note**: For SHAP basics (TreeExplainer, KernelExplainer, force plots, summary plots, dependence plots), see [Machine Learning Lesson 16: Model Explainability](../Machine_Learning/16_Model_Explainability.md). This lesson assumes you are comfortable with Shapley value theory, the KernelSHAP algorithm, and basic SHAP visualization. We build on those foundations with deep-learning-specific methods, interaction effects, causal extensions, and computational optimization.

---

## 1. DeepSHAP: DeepLIFT-Based Shapley Values

### 1.1 From DeepLIFT to DeepSHAP
...
### 1.2 DeepSHAP with Multiple Backgrounds
...
### 1.3 Background Selection Strategies
...
## 2. GradientSHAP: Expected Gradients

### 2.1 The Expected Gradients Formulation
...
### 2.2 Deep Comparison: DeepSHAP vs GradientSHAP vs KernelSHAP
...
### 3.1 The Shapley Interaction Index
...
## 4. Asymmetric Shapley Values and Causal SHAP

### 4.1 The Problem with Standard SHAP
...
### 5.1 Background Sampling and Mini-batching
...
## 6. TreeSHAP Pathologies

### 6.1 The Correlated Feature Problem
...
## 7. Putting It All Together: Complete Pipeline
...
- **DeepSHAP** combines DeepLIFT's backward propagation with Shapley value averaging over multiple background samples. Background selection (random, k-means, stratified) critically affects results.
- **GradientSHAP** implements Expected Gradients — averaging Integrated Gradients over random baselines and interpolation points. It is more theoretically grounded than DeepSHAP but noisier.
- When comparing methods, **KernelSHAP is the most accurate** (model-agnostic), **DeepSHAP is the fastest** (single backward pass), and **GradientSHAP balances** between the two.
- **SHAP interaction values** decompose attributions into main effects (diagonal) and pairwise interactions (off-diagonal), directly measuring feature synergy.
- **Asymmetric Shapley values** restrict permutations to respect causal ordering, while **Causal SHAP** uses interventional conditionals instead of observational ones. Both address the fundamental limitation that standard SHAP ignores causal structure.
- **TreeSHAP pathologies** arise with correlated features: attribution can leak between correlated features regardless of which conditional (observational or interventional) is used. Always audit correlations before interpreting individual SHAP values.
- **Computational optimizations** — k-means background compression, GPU batching, and early-stopping convergence checks — are essential for production deployment.

---

## Exercises

### Exercise 1: Background Sensitivity Analysis (Beginner)

Train a gradient boosting model on the California Housing dataset. Compute DeepSHAP values using three different background strategies: (a) 50 random samples, (b) 200 random samples, (c) 50 k-means centroids. Compare the resulting SHAP values using Pearson correlation. Which strategy gives the most stable results? How does computation time scale?

### Exercise 2: Deep Method Comparison (Intermediate)

Train a 4-layer MLP on a tabular dataset of your choice. Compare DeepSHAP, GradientSHAP, and KernelSHAP on 50 test instances. For each pair of methods, compute the rank correlation of feature importances. Identify instances where the methods disagree the most and investigate why (hint: look at the non-linearities in those regions).

### Exercise 3: Interaction Discovery (Intermediate)

Using the Friedman #1 synthetic dataset (which has a known x0*x1 interaction), compute SHAP interaction values with a gradient boosted model. Verify that the x0-x1 interaction is the strongest. Then add a correlated feature (x10 = x0 + noise) and re-run. How does the spurious correlation affect the interaction values?

### Exercise 4: Causal SHAP for Loan Decisions (Advanced)

Build a loan approval model with features: age, education, income, savings, credit_score. Define a causal graph (education and age cause income, income causes savings, etc.). Compare standard SHAP, asymmetric Shapley, and causal SHAP values. Write a report explaining which explanation would be most appropriate for (a) a regulatory audit, (b) a customer-facing explanation, and (c) a model debugging session.

### Exercise 5: Production SHAP Pipeline (Advanced)

Implement a production-ready SHAP service that: (a) compresses backgrounds using k-means, (b) uses GPU-accelerated GradientSHAP, (c) implements early-stopping convergence, (d) caches results for repeated queries, and (e) includes a correlation audit warning system. Benchmark latency on a model with 50+ features and 10,000+ test instances. Target: <100ms per explanation.

---

[Previous: Probing and Representation Analysis](./05_Probing_and_Representation_Analysis.md) | [Overview](./00_Overview.md) | [Next: Concept-Based Explanations](./07_Concept_Based_Explanations.md)
