# 16. Agent Prompting Patterns

**Previous**: [15. Prompt Management in Production](./15_Prompt_Management_in_Production.md) | **Next**: [17. Capstone: Prompt Library](./17_Capstone_Prompt_Library.md)

## Learning Objectives

- Design system prompts for autonomous agent systems with clear identity and boundaries
- Implement tool-use prompting with well-structured function descriptions and error handling
- Build planning prompts that decompose complex tasks into executable steps
- Create reflection and self-critique loops that improve agent output quality
- Apply established agent patterns (ReAct, MRKL, Toolformer) to real-world applications

---

An LLM agent is more than a chatbot — it is a system that can reason about goals, make plans, take actions (via tools), observe results, and iterate. The quality of an agent depends critically on its prompts: the system prompt defines identity and boundaries, tool descriptions determine how effectively the model selects and uses tools, planning prompts shape the agent's reasoning strategy, and reflection prompts enable self-correction. This lesson covers the prompting patterns that make agents reliable, capable, and safe.

## Table of Contents
1. [Agent System Prompts](#1-agent-system-prompts)
2. [Tool-Use Prompting](#2-tool-use-prompting)
3. [Planning Prompts](#3-planning-prompts)
4. [Reflection and Self-Critique](#4-reflection-and-self-critique)
5. [Observation-Action Loops](#5-observation-action-loops)
6. [Multi-Agent Prompting](#6-multi-agent-prompting)
7. [Error Recovery in Agent Prompts](#7-error-recovery-in-agent-prompts)
8. [Memory Management Prompts](#8-memory-management-prompts)
9. [Guardrails for Agent Actions](#9-guardrails-for-agent-actions)
10. [ReAct, MRKL, and Toolformer Patterns](#10-react-mrkl-and-toolformer-patterns)

---

## 1. Agent System Prompts

Agent system prompts are fundamentally different from chatbot system prompts. They must define not just personality and constraints, but also the agent's decision-making framework, tool awareness, and autonomy boundaries.

### 1.1 Anatomy of an Agent System Prompt
...
### 1.2 Progressive Autonomy Levels
...
### 1.3 Identity Stability

Agents need strong identity anchoring to resist manipulation:
...
How you describe tools to the model directly impacts how well it selects and uses them.
...
Planning prompts help agents decompose complex tasks into manageable steps before executing them.
...
Reflection prompts enable agents to evaluate their own outputs, catch errors, and improve quality before delivering results.

### 4.1 Self-Critique Pattern
...
### 4.2 Structured Self-Evaluation
...
## 5. Observation-Action Loops

The core agent loop: observe the environment, decide on an action, execute it, observe the result, repeat.

### 5.1 Basic Agent Loop
...
Complex tasks benefit from multiple specialized agents, each with a focused role, coordinated by an orchestrator.

### 6.1 Coordinator-Specialist Pattern
...
## 7. Error Recovery in Agent Prompts

Agents must handle failures gracefully — tool errors, unexpected results, and dead ends.
...
## 8. Memory Management Prompts

Agents operating over long conversations or multiple sessions need strategies for managing what they remember.

### 8.1 Working Memory Pattern
...
### 8.2 Conversation Summarization for Context Management
...
## 9. Guardrails for Agent Actions

Agents with tool access need guardrails to prevent harmful or unintended actions.

### 9.1 Action Validation Framework
...
## 10. ReAct, MRKL, and Toolformer Patterns

These are established patterns for building tool-augmented agents.

### 10.1 ReAct Pattern (Reasoning + Acting)

The ReAct pattern interleaves reasoning (Thought) with acting (Action) and observing (Observation):
...
### 10.2 MRKL Pattern (Modular Reasoning, Knowledge, and Language)

MRKL routes sub-problems to specialized modules (tools, APIs, models):
...
### 10.3 Toolformer-Inspired Pattern

The Toolformer pattern teaches the model to decide when and how to insert tool calls inline:
...
Implement a ReAct agent that can solve multi-step problems by interleaving reasoning, tool use, and observation.

**Requirements:**
- Implement the full Thought → Action → Observation loop
- Support at least 3 tools (search, calculate, lookup)
- Parse the model's output to extract tool calls
- Execute tools and feed results back
- Maximum 10 steps before declaring failure
...
Build a multi-agent system where specialized agents debate a topic and a judge synthesizes the discussion into a balanced analysis.

**Requirements:**
- At least 2 debating agents with distinct perspectives
- A judge agent that synthesizes without bias
- 2-3 rounds of debate with responses to each other's arguments
- Structured final output with "strongest argument from each side"
...
Build an agent that has access to tools but operates within strict guardrails — action validation, rate limiting, confirmation for high-risk actions, and an audit log.

**Requirements:**
- Register at least 4 tools with different risk levels
- Implement action validation before tool execution
- Rate limiting (per-tool, per-minute limits)
- High-risk actions require simulated user confirmation
- Complete audit log of all actions (attempted and executed)
...
### Exercise 4: Adaptive Planning Agent

Build an agent that creates plans, executes them step by step, and adapts the plan when steps fail or produce unexpected results.

**Requirements:**
- Create an initial plan with 4-6 steps
- Execute each step and evaluate the result
- When a step fails, replan the remaining steps
- Track plan modifications in a history
- Report final results with a summary of adaptations
...
### Exercise 5: Memory-Augmented Agent

Build an agent with explicit working memory that persists across steps, including facts learned, goals tracked, and a scratchpad for intermediate reasoning.

**Requirements:**
- Working memory with facts, goals, scratchpad, and context sections
- Memory is injected into each prompt as structured context
- Agent can explicitly add/remove items from memory
- Memory summarization when it grows too large
- Demonstrate across a multi-step research task
