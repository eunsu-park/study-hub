...
VPC Peering works well for connecting two VPCs, but it does not scale for large organizations. With N VPCs, peering requires N×(N-1)/2 individual connections — 10 VPCs need 45 peering connections, each with its own route table entries.

**Transit Gateway** acts as a central hub (hub-and-spoke model) that connects multiple VPCs and on-premises networks through a single gateway.

```
VPC Peering (N² connections):          Transit Gateway (N connections):
VPC-A ──── VPC-B                        VPC-A ──┐
  │    ╲  ╱  │                          VPC-B ──┤
...
- **Transitive routing**: VPC-A can reach VPC-C through the Transit Gateway without a direct peering connection.
- **Centralized route management**: One route table instead of N² individual peering route entries.
- **On-premises integration**: Connect your corporate data center via Site-to-Site VPN or Direct Connect to the Transit Gateway, instantly giving all attached VPCs access to on-premises resources.
...
aws ec2 create-transit-gateway-vpc-attachment \
    ...
    --vpc-id vpc-A \
    --subnet-ids subnet-A-private

# 3. Add route in VPC-A's route table pointing to TGW for VPC-B's CIDR
aws ec2 create-route \
...
|----------|------------|-----------------|
| Number of VPCs | 2–5 | 5+ |
| Transitive routing needed | No | Yes |
| On-premises connectivity | Not directly | Yes (VPN/Direct Connect) |
| Cost | Free (only data transfer) | $0.05/hour + $0.02/GB |
...
### 8.2 GCP Private Service Connect
...
### 10.1 Connection Issue Checklist
