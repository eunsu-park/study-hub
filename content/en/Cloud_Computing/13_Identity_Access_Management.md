...
## 9. Enterprise IAM Patterns

### 9.1 IAM Identity Center (AWS SSO)

Managing IAM users and credentials separately for each AWS account is unscalable in organizations with dozens or hundreds of accounts. **IAM Identity Center** (formerly AWS Single Sign-On) provides centralized access management across all accounts in an AWS Organization.

```bash
# Enable IAM Identity Center (done via console or Organizations)
# Then assign permission sets to users/groups per account
...
Organizations with an existing identity provider (Okta, Azure AD, Google Workspace) can federate corporate identities into AWS using SAML 2.0. Users log in once with their corporate credentials and receive temporary AWS credentials — no separate IAM users required.

```bash
# Create an IAM SAML identity provider (IdP metadata from Okta/Azure AD)
aws iam create-saml-provider \
    --saml-metadata-document file://idp-metadata.xml \
    --name CorporateIdP

# Create a role that federated users can assume
# Trust policy references the SAML provider
...
EOF
aws iam create-role --role-name FederatedDeveloper --assume-role-policy-document file://saml-trust.json
```

### 9.3 Service Control Policies (SCPs)

SCPs are organization-wide guardrails set at the AWS Organizations level. They define the **maximum permissions** any account in an organizational unit (OU) can have, regardless of what IAM policies in that account allow. SCPs do not grant permissions themselves — they only restrict.

```json
// Example SCP: Prevent disabling CloudTrail in any account
{
    "Version": "2012-10-17",
    "Statement": [{
...
### 9.4 Permission Boundaries

A **permission boundary** sets the maximum permissions an IAM entity (user or role) can have. Even if a policy attached to the entity grants `s3:*`, the entity can only perform actions that are also allowed by the permission boundary.

Permission boundaries are commonly used to **delegate role creation safely**: allow developers to create IAM roles for their applications, but prevent them from creating roles that exceed their own permission level (privilege escalation).

```bash
# Create a permission boundary policy (limits to read-only S3 + CloudWatch)
aws iam create-policy \
    --policy-name DeveloperPermBoundary \
    --policy-document file://perm-boundary.json

# Create a role with the boundary applied
aws iam create-role \
    --role-name AppRole \
    --assume-role-policy-document file://trust.json \
    --permissions-boundary arn:aws:iam::123456789012:policy/DeveloperPermBoundary
```
