...
Every tuple on disk carries two transaction IDs in its header (lesson 03 §B.1):

- **`xmin`** — the transaction that *created* this row version. Set by `INSERT` or by the `INSERT half` of an `UPDATE`.
- **`xmax`** — the transaction that *deleted or superseded* this row version. Set by `DELETE` or by the `delete half` of an `UPDATE`. Zero if still live.

When a transaction commits, PostgreSQL writes a single bit to the **clog** (`pg_xact`) marking that `xid` as committed. Every other transaction that asks "is xid 12345 committed?" reads that one bit.

#### A.2 The snapshot

When a transaction starts (or its first SQL statement runs, depending on isolation level), PostgreSQL captures a **snapshot**: a triple `(xmin_horizon, xmax_horizon, in_progress[])`.

- `xmin_horizon`: the smallest `xid` of any *currently running* transaction. Anything older than this is definitively committed (or definitively aborted).
- `xmax_horizon`: the next `xid` to be assigned. Anything ≥ this is in the future and invisible.
- `in_progress[]`: explicit list of `xid`s between `xmin_horizon` and `xmax_horizon` that are running but not committed yet.

#### A.3 The visibility rule

A row version is visible to a snapshot if and only if **both** of the following hold:

1. **`xmin` is committed and visible** to this snapshot:
   - `xmin < xmax_horizon` AND
   - `xmin` is not in `in_progress[]` AND
   - `xmin` shows committed in the clog (or is the snapshot's own `xid`)
2. **`xmax` is either zero, or in_progress, or aborted, or in the future**:
   - `xmax = 0` OR
   - `xmax` shows aborted in clog OR
   - `xmax` is in `in_progress[]` (and not the snapshot's own `xid`) OR
   - `xmax >= xmax_horizon`
