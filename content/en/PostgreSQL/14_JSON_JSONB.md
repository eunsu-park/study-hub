...
```
JSONB header (4 bytes)
├── Type tag: object | array | scalar
├── Number of children (count)
└── Total size

For each child:
├── JEntry (4 bytes): type + offset/length
└── Key/value bytes (sorted by key for objects)
```

#### A.1 Why "binary" matters

Compared to JSON-as-text, JSONB:

- **Skips re-parsing** on every operator call. `doc->'a'` directly reads the JEntry for key `'a'`.
- **Sorts object keys** (lexicographically) at parse time, enabling binary-search lookup of keys in O(log K) instead of linear scan.
- **Deduplicates keys** — `{"a": 1, "a": 2}` becomes `{"a": 2}`. JSON text would keep both.
- **Loses whitespace and key order**. If you need to preserve those, use `json` (text) instead of `jsonb`.
