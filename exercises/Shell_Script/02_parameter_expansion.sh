...
exercise_5() {
    ...
    resolve_path() {
        ...
        # Expand environment variables using parameter substitution
        # NOTE: eval on user-supplied paths is a security risk — an attacker
        # could inject arbitrary commands (e.g., '$(rm -rf /)').  Instead we
        # expand only well-known variables via safe substitution.
        path="${path//\$HOME/$HOME}"
        path="${path//\$USER/$USER}"
        path="${path//\$PWD/$PWD}"
