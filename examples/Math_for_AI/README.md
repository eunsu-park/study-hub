...
Files are numbered to match lesson order. Some lesson numbers have dedicated examples; others share coverage with nearby files.
...
### 05_multivariate_calculus.py (~230 lines)
**Multivariate Calculus**

Demonstrates:
- Symbolic partial derivatives using SymPy (with chain rule verification)
- Gradient vectors, directional derivatives, and their geometric meaning
- Second-order Taylor expansion and approximation accuracy
- Gradient field and level curve visualization

**Key concepts:**
- Partial derivatives ∂f/∂xᵢ and the gradient ∇f
- Directional derivative D_u f = ∇f · u (unit vector u)
- Clairaut's theorem (symmetry of mixed partials)
- Taylor expansion: f(x) ≈ f(a) + ∇f(a)ᵀ(x-a) + ½(x-a)ᵀH(a)(x-a)

**Output:** `multivariate_calculus.png` - Gradient field with level curves and Taylor accuracy
...
### 11_probability_distributions_advanced.py (~240 lines)
**Advanced Probability Distributions**

Demonstrates:
- Exponential family of distributions: natural parameters, sufficient statistics, log-partition
- Multivariate Gaussian: PDF, sampling, marginals, conditional distributions, Mahalanobis distance
- Beta-Binomial conjugate prior: sequential Bayesian updates for coin flips
- Gaussian-Gaussian conjugate prior: known variance, unknown mean update

**Key concepts:**
- Exponential family: p(x|η) = h(x) exp(η·T(x) - A(η))
- Sufficient statistics for MLE in exponential family
- Conjugate priors enable closed-form posterior updates
- Multivariate Gaussian conditioning and marginalization

**Output:** `probability_distributions_advanced.png` - Multivariate Gaussian contours and conjugate update dynamics

---

### 14_convexity_and_duality.py (~230 lines)
**Convexity, Lagrangian Duality, and Proximal Operators**

Demonstrates:
- Convex function verification: Jensen's inequality and Hessian PSD condition
- Lagrangian duality for equality-constrained QP (strong duality, KKT conditions)
- KKT conditions for inequality-constrained problems
- Proximal operators: soft-thresholding (L1), L2 shrinkage, projection onto convex sets
- Proximal gradient descent for Lasso

**Key concepts:**
- Convexity: Jensen's inequality and PSD Hessian
- Lagrangian: L(x, λ) = f(x) + λᵀg(x); dual g(λ) = min_x L(x, λ)
- Strong duality: primal optimal = dual optimal (holds for convex problems under constraint qualifications)
- KKT complementary slackness: μᵢ gᵢ(x*) = 0
- prox_{λf}(v) = argmin_x [f(x) + ½λ‖x-v‖²]

**Output:** Console output only

---

### 16_manifold_and_representation_learning.py (~230 lines)
**Manifold Learning and Representation Learning**

Demonstrates:
- Swiss roll dataset and manifold hypothesis
- Intrinsic dimension estimation via correlation dimension
- Geodesic distances using Isomap (neighborhood graph + shortest paths)
- t-SNE dimensionality reduction on Digits dataset
- Comparison of PCA (linear), Isomap, and t-SNE

**Key concepts:**
- Manifold hypothesis: data lies near a low-dimensional manifold in high-dim space
- Euclidean vs geodesic distance on curved manifolds
- t-SNE: minimize KL(P || Q) with heavy-tailed Student-t kernel in low-dim
- Isomap: MDS on geodesic distance matrix

**Output:** `manifold_representation_learning.png` - Swiss roll unrolling and Digits 2D projections

---

### 18_math_of_generative_models.py (~240 lines)
**Mathematics of Generative Models**

Demonstrates:
- VAE: ELBO decomposition (reconstruction + KL), reparameterization trick, mini training loop
- GAN: minimax objective, optimal discriminator, 1D toy GAN training
- Diffusion model: linear noise schedule, signal/noise levels, score function derivation
...
- Diffusion closed form: q(x_t|x_0) = N(√ᾱ_t x_0, (1-ᾱ_t)I)
- Score matching: predict noise ε to learn score ∇ log p

**Output:** `math_of_generative_models.png` - VAE ELBO, GAN output distribution, diffusion schedule
...
# ... and so on
python 05_multivariate_calculus.py
python 11_probability_distributions_advanced.py
python 14_convexity_and_duality.py
python 16_manifold_and_representation_learning.py
python 18_math_of_generative_models.py
```
...
Additional dependencies for specific files:
- `05_multivariate_calculus.py`: sympy
- `16_manifold_and_representation_learning.py`: sklearn (includes TSNE, Isomap)

Install with:
```bash
pip install numpy matplotlib torch scikit-learn scipy sympy
```
...
5. `05_multivariate_calculus.py` - Multivariate calculus and Taylor expansion
6. `05_gradient_descent.py` - Optimization algorithms
7. `06_optimization_constrained.py` - Constrained optimization
8. `07_probability_distributions.py` - Probability and inference
9. `08_information_theory.py` - Information theory for ML
10. `09_mcmc_sampling.py` - Sampling techniques
11. `10_tensor_ops_einsum.py` - Tensor operations and einsum
12. `11_graph_spectral.py` - Graph theory and spectral methods
13. `11_probability_distributions_advanced.py` - Advanced probability and conjugate priors
14. `12_attention_math.py` - Attention mechanism mathematics
15. `14_convexity_and_duality.py` - Convexity, duality, and proximal methods
16. `16_manifold_and_representation_learning.py` - Manifold learning
17. `18_math_of_generative_models.py` - Mathematics of generative models (VAE, GAN, diffusion)
...
- `multivariate_calculus.png` - Gradient field and Taylor approximation
...
- `probability_distributions_advanced.png` - Multivariate Gaussian and conjugate priors
...
- `manifold_representation_learning.png` - Manifold unrolling and 2D embeddings
- `math_of_generative_models.png` - VAE ELBO, GAN samples, diffusion schedule
