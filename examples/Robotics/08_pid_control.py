...
def demo_pid_control():
    ...
    # RMS and max error comparison
    rms_errors = {}
    max_errors = {}
    for name, res, color in controllers:
        rms = np.sqrt(np.mean(res["error"]**2, axis=0))
        peak = np.max(np.abs(res["error"]), axis=0)
        rms_errors[name] = rms
        max_errors[name] = peak
        print(f"  {name:20s}: RMS error = [{np.degrees(rms[0]):.3f}, {np.degrees(rms[1]):.3f}] deg")
        print(f"  {'':20s}  Max error = [{np.degrees(peak[0]):.3f}, {np.degrees(peak[1]):.3f}] deg")
    ...
    # Max error comparison — use actual peak instantaneous errors
    max_j1 = [np.degrees(max_errors[n][0]) for n in names]
    max_j2 = [np.degrees(max_errors[n][1]) for n in names]
