#!/usr/bin/env python3
"""
WiFi Network Scanner and Socket Programming Examples

This script provides the following features:
1. WiFi network scanning (simulation mode supported)
2. TCP client/server examples
3. HTTP client for sensor data transmission
4. Network connection status monitoring
5. Network error handling and retry logic

Reference: content/ko/IoT_Embedded/04_WiFi_Networking.md
"""

import subprocess
import re
import socket
import json
import time
import random
import sys
from typing import Optional, List, Dict
from datetime import datetime


# ============================================================================
# WiFi Network Scanning
# ============================================================================

# Why: Detecting the platform early lets the script auto-switch to simulation
# mode on laptops/CI, so developers never see confusing GPIO errors.
def is_raspberry_pi() -> bool:
    """Check if running on Raspberry Pi"""
    try:
        with open('/proc/cpuinfo', 'r') as f:
            cpuinfo = f.read()
            return 'BCM' in cpuinfo or 'Raspberry' in cpuinfo
    except FileNotFoundError:
        return False


def get_wifi_info() -> dict:
    """
    Get current WiFi connection information

    Returns:
        dict: WiFi info (ssid, ip_addresses, signal_dbm, mac_address)
    """
    info = {}

    if not is_raspberry_pi():
        # Simulation mode
        print("[Simulation] Not a real Raspberry Pi. Using virtual data")
        info = {
            'ssid': 'SimulatedWiFi',
            'ip_addresses': ['192.168.1.100'],
            'signal_dbm': -45,
            'mac_address': 'AA:BB:CC:DD:EE:FF'
        }
        return info

    try:
        # Get SSID
        result = subprocess.run(
            ['iwgetid', '-r'],
            capture_output=True,
            text=True,
            timeout=5
        )
        info['ssid'] = result.stdout.strip()

        # Get IP address
        result = subprocess.run(
            ['hostname', '-I'],
            capture_output=True,
            text=True,
            timeout=5
        )
        ips = result.stdout.strip().split()
        info['ip_addresses'] = ips

        # Get signal strength
        result = subprocess.run(
            ['iwconfig', 'wlan0'],
            capture_output=True,
            text=True,
            timeout=5
        )
        match = re.search(r'Signal level=(-?\d+)', result.stdout)
        if match:
            info['signal_dbm'] = int(match.group(1))

        # MAC address
        result = subprocess.run(
            ['cat', '/sys/class/net/wlan0/address'],
            capture_output=True,
            text=True,
            timeout=5
        )
        info['mac_address'] = result.stdout.strip()

    except Exception as e:
        info['error'] = str(e)

    return info


def get_wifi_networks() -> List[dict]:
    """
    Scan nearby WiFi networks

    Returns:
        list: Network info list (ssid, quality, signal_dbm)
    """
    networks = []

    if not is_raspberry_pi():
        # Simulation mode
        print("[Simulation] Generating WiFi scan results...")
        networks = [
            {'ssid': 'HomeNetwork', 'quality': '70/70', 'signal_dbm': -35},
            {'ssid': 'OfficeWiFi', 'quality': '65/70', 'signal_dbm': -42},
            {'ssid': 'GuestNetwork', 'quality': '50/70', 'signal_dbm': -58},
            {'ssid': 'Neighbor_2.4G', 'quality': '40/70', 'signal_dbm': -65},
            {'ssid': 'CafeWiFi', 'quality': '30/70', 'signal_dbm': -75},
        ]
        return networks

    try:
        result = subprocess.run(
            ['sudo', 'iwlist', 'wlan0', 'scan'],
            capture_output=True,
            text=True,
            timeout=10
        )

        current_network = {}
        for line in result.stdout.split('\n'):
            if 'ESSID:' in line:
                ssid = re.search(r'ESSID:"(.+)"', line)
                if ssid and current_network:
                    networks.append(current_network)
                current_network = {'ssid': ssid.group(1) if ssid else ''}

            elif 'Quality=' in line:
                quality = re.search(r'Quality=(\d+)/(\d+)', line)
                if quality:
                    current_network['quality'] = f"{quality.group(1)}/{quality.group(2)}"

                signal = re.search(r'Signal level=(-?\d+)', line)
                if signal:
                    current_network['signal_dbm'] = int(signal.group(1))

        if current_network:
            networks.append(current_network)

    except Exception as e:
        print(f"Scan failed: {e}")
        print("Switching to simulation mode...")
        return get_wifi_networks()  # Recursive call for simulation mode

    return networks


def monitor_wifi_signal(threshold_dbm: int = -70, interval: int = 5, duration: int = 30):
    """
    WiFi Signal Strength Monitoring

    Args:
        threshold_dbm: Warning threshold (dBm)
        interval: Check interval (seconds)
        duration: Monitoring duration (seconds)
    """
    print(f"=== WiFi Signal Monitoring Started ===")
    print(f"Warning threshold: {threshold_dbm} dBm")
    print(f"Check interval: {interval}s, total {duration}s")
    print()

    start_time = time.time()

    while time.time() - start_time < duration:
        info = get_wifi_info()

        if 'error' in info:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Error: {info['error']}")
        else:
            signal = info.get('signal_dbm', 0)
            ssid = info.get('ssid', 'Unknown')

            status = "Good" if signal > threshold_dbm else "Warning"
            print(f"[{datetime.now().strftime('%H:%M:%S')}] {ssid}: {signal} dBm ({status})")

            if signal <= threshold_dbm:
                print(f"  Warning: Signal is weak! ({signal} dBm <= {threshold_dbm} dBm)")

        time.sleep(interval)

    print("\nMonitoring ended")


# ============================================================================
# TCP Socket Programming
# ============================================================================

def tcp_server(host: str = '0.0.0.0', port: int = 9999):
    """
    TCP Server - Receive sensor data

    Args:
        host: IP address to bind
        port: Port number
    """
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        # Why: SO_REUSEADDR lets you restart the server immediately after a crash.
        # Without it, the OS holds the port in TIME_WAIT for ~60 s, blocking rebind.
        server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

        server.bind((host, port))
        server.listen(5)

        print(f"TCP server started: {host}:{port}")
        print("Waiting for client connections... (Ctrl+C to stop)")

        try:
            while True:
                client, address = server.accept()
                print(f"\nClient connected: {address}")

                with client:
                    while True:
                        data = client.recv(1024)
                        if not data:
                            break

                        try:
                            # Parse JSON data
                            message = json.loads(data.decode('utf-8'))
                            timestamp = datetime.now().strftime("%H:%M:%S")
                            print(f"[{timestamp}] Received: {message}")

                            # Send response
                            response = {
                                "status": "ok",
                                "received": message.get("sensor_id"),
                                "timestamp": time.time()
                            }
                            client.sendall(json.dumps(response).encode('utf-8'))

                        except json.JSONDecodeError:
                            print(f"Invalid JSON: {data}")

                print(f"Client disconnected: {address}")

        except KeyboardInterrupt:
            print("\nServer stopped")


def tcp_client(server_host: str, server_port: int = 9999, count: int = 10):
    """
    TCP Client - Send sensor data

    Args:
        server_host: Server IP address
        server_port: Server port
        count: Number of transmissions
    """
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client:
            print(f"Connecting to server: {server_host}:{server_port}")
            client.connect((server_host, server_port))
            print(f"Connected to server!")

            sensor_id = "temp_sensor_01"

            for i in range(count):
                # Generate sensor data (simulation)
                data = {
                    "sensor_id": sensor_id,
                    "temperature": round(random.uniform(20, 30), 1),
                    "humidity": round(random.uniform(40, 70), 1),
                    "timestamp": time.time(),
                    "sequence": i + 1
                }

                # Send
                message = json.dumps(data).encode('utf-8')
                client.sendall(message)
                print(f"[{i+1}/{count}] Sent: temp={data['temperature']}C, humidity={data['humidity']}%")

                # Receive response
                try:
                    client.settimeout(5.0)
                    response = client.recv(1024)
                    if response:
                        resp_data = json.loads(response.decode('utf-8'))
                        print(f"         Response: {resp_data['status']}")
                except socket.timeout:
                    print(f"         Response timeout")

                if i < count - 1:
                    time.sleep(2)

            print("\nTransmission complete")

    except ConnectionRefusedError:
        print(f"Error: Cannot connect to server ({server_host}:{server_port})")
        print("Check that the server is running.")
    except Exception as e:
        print(f"Error: {e}")


# ============================================================================
# HTTP Client
# ============================================================================

def send_sensor_data_http(api_base: str, sensor_id: str, data: dict) -> bool:
    """
    Send sensor data via HTTP POST

    Args:
        api_base: API base URL
        sensor_id: Sensor ID
        data: Data to send

    Returns:
        bool: Success status
    """
    try:
        import requests
    except ImportError:
        print("requests library required: pip install requests")
        return False

    url = f"{api_base}/sensors/{sensor_id}/data"

    try:
        response = requests.post(
            url,
            json=data,
            headers={"Content-Type": "application/json"},
            timeout=5
        )

        if response.status_code in (200, 201):
            print(f"Data sent successfully: {data}")
            return True
        else:
            print(f"Send failed: {response.status_code} - {response.text}")
            return False

    except requests.exceptions.RequestException as e:
        print(f"Network error: {e}")
        return False


def http_client_with_retry(api_base: str, sensor_id: str, retries: int = 3):
    """
    HTTP Client with retry logic

    Args:
        api_base: API base URL
        sensor_id: Sensor ID
        retries: Number of retries
    """
    print(f"=== HTTP Client (retries: {retries}) ===")
    print(f"API: {api_base}")
    print(f"Sensor ID: {sensor_id}\n")

    for i in range(10):
        data = {
            "temperature": round(random.uniform(20, 30), 1),
            "humidity": round(random.uniform(40, 70), 1),
            "timestamp": int(time.time())
        }

        success = False
        for attempt in range(retries):
            if send_sensor_data_http(api_base, sensor_id, data):
                success = True
                break
            else:
                if attempt < retries - 1:
                    # Why: Exponential backoff (1 s, 2 s, 4 s...) avoids overwhelming
                    # a struggling server with rapid retries, giving it time to recover.
                    wait_time = 2 ** attempt  # Exponential backoff
                    print(f"  Retry wait: {wait_time} seconds...")
                    time.sleep(wait_time)

        if not success:
            print(f"  Failed after {retries} retries")

        time.sleep(5)


# ============================================================================
# Network Connection Status Check
# ============================================================================

# Why: Connecting to Google's DNS (8.8.8.8) on port 53 is the fastest portable
# internet check — it avoids DNS resolution itself, so a DNS outage won't
# produce a false negative for raw IP connectivity.
def check_internet_connection(host: str = "8.8.8.8", port: int = 53, timeout: int = 3) -> bool:
    """
    Check internet connection

    Args:
        host: Host to test (default: Google DNS)
        port: Port
        timeout: Timeout (seconds)

    Returns:
        bool: Whether connection is possible
    """
    try:
        socket.setdefaulttimeout(timeout)
        socket.socket(socket.AF_INET, socket.SOCK_STREAM).connect((host, port))
        return True
    except socket.error:
        return False


# Why: Opening a UDP socket to an external address (without sending data)
# makes the OS pick the correct outbound interface. This is more reliable
# than parsing ifconfig output, which varies across OSes and locales.
def get_local_ip() -> str:
    """
    Get local IP address

    Returns:
        str: IP address
    """
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        s.connect(('8.8.8.8', 80))
        ip = s.getsockname()[0]
        s.close()
        return ip
    except Exception:
        return "127.0.0.1"


def network_status():
    """Check network status"""
    print("=== Network Status ===\n")

    # Local IP
    local_ip = get_local_ip()
    print(f"Local IP: {local_ip}")

    # Internet connection
    internet = check_internet_connection()
    print(f"Internet connection: {'Connected' if internet else 'Not connected'}")

    # WiFi info
    wifi_info = get_wifi_info()
    if 'error' not in wifi_info:
        print(f"\nWiFi SSID: {wifi_info.get('ssid', 'N/A')}")
        print(f"Signal strength: {wifi_info.get('signal_dbm', 'N/A')} dBm")
        print(f"MAC address: {wifi_info.get('mac_address', 'N/A')}")


# ============================================================================
# Main Function
# ============================================================================

def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("WiFi Network Scanner and Socket Programming Examples")
        print("\nUsage:")
        print("  python wifi_scanner.py info         - WiFi info query")
        print("  python wifi_scanner.py scan         - WiFi network scan")
        print("  python wifi_scanner.py monitor      - WiFi signal monitoring")
        print("  python wifi_scanner.py status       - Network status check")
        print("  python wifi_scanner.py server       - Start TCP server")
        print("  python wifi_scanner.py client <IP>  - Start TCP client")
        print("\nExamples:")
        print("  python wifi_scanner.py info")
        print("  python wifi_scanner.py client 192.168.1.100")
        return

    command = sys.argv[1].lower()

    if command == 'info':
        print("=== WiFi Connection Info ===")
        info = get_wifi_info()
        for key, value in info.items():
            print(f"  {key}: {value}")

    elif command == 'scan':
        print("=== Nearby WiFi Networks ===")
        networks = get_wifi_networks()
        print(f"Networks found: {len(networks)}\n")
        for net in networks:
            ssid = net.get('ssid', 'Unknown')
            signal = net.get('signal_dbm', 'N/A')
            quality = net.get('quality', 'N/A')
            print(f"  {ssid:20} - Signal: {signal:4} dBm, Quality: {quality}")

    elif command == 'monitor':
        monitor_wifi_signal(threshold_dbm=-70, interval=5, duration=30)

    elif command == 'status':
        network_status()

    elif command == 'server':
        tcp_server()

    elif command == 'client':
        if len(sys.argv) < 3:
            print("Error: Please enter the server IP address")
            print("Example: python wifi_scanner.py client 192.168.1.100")
            return
        server_ip = sys.argv[2]
        tcp_client(server_ip, count=10)

    else:
        print(f"Unknown command: {command}")
        print("Run 'python wifi_scanner.py' for help")


if __name__ == "__main__":
    main()
