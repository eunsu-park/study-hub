...
main() {
    ...
    # Demonstrate ERR trap with a deliberate failure inside a subshell.
    # The subshell isolates the error so the main script continues,
    # while the stack trace is still printed by the trap handler.
    echo -e "${BLUE}=== ERR Trap Demo (subshell) ===${NC}"
    (function_that_fails 3) || true
