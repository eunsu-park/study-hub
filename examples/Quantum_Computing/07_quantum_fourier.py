...
def demo_qft_vs_fft():
    ...
    # Classical DFT (adjusted to match QFT convention)
    # Why: numpy.fft uses the convention F[k] = Σ x[n]·e^{-2πink/N}, without
    # the 1/√N normalization.  QFT uses F[k] = (1/√N) Σ x[n]·e^{+2πink/N}.
    # Since QFT uses +2πi and numpy.fft uses -2πi, they are complex conjugates
    # of each other (up to normalization).  We conjugate the DFT result and
    # normalize by 1/√N to match the QFT output exactly.
    dft_result = np.fft.fft(state).conj() / np.sqrt(N)
    ...
    # Why: QFT uses ω = e^{+2πi/N} while numpy.fft uses e^{-2πi/N}.  They are
    # complex conjugates of each other (up to normalization).  By conjugating
    # the DFT output, we recover exactly the QFT result.
    print(f"\n  Note: QFT uses ω = e^{{+2πi/N}}, numpy.fft uses e^{{-2πi/N}}")
    print(f"  QFT(x) = conj(DFT(x)) / √N — conjugating the DFT matches QFT exactly")
