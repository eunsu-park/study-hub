...
def quantum_order_finding(a: int, N: int, n_qubits: int = None,
    ...
    # Why: We directly construct the post-measurement state.  After computing
    # f(x) = a^x mod N into the second register and measuring it, the first
    # register collapses to a uniform superposition over all x with f(x) = f₀.
    # We pick a random f₀ and keep only the matching x values.
