...
class LRUCache:
    ...
    def put(self, key: str, value: Any) -> tuple[str, Any] | None:
        evicted = None
        if key in self.cache:
            self.cache.move_to_end(key)
        else:
...
class WriteBack:
    ...
    def write(self, key: str, value: Any) -> None:
        ...
        # Pass evicted value directly — the key is already removed from cache
        if evicted and evicted[0] in self.dirty:
            self._flush_key(evicted[0], evicted_value=evicted[1])

    def _flush_key(self, key: str, *, evicted_value: Any = None) -> None:
        if key in self.dirty:
            if evicted_value is not None:
                # Value provided directly (from eviction path)
                self.db.put(key, evicted_value)
            else:
                # Key still in cache (e.g., called from flush_all)
                hit, value = self.cache.get(key)
                if hit:
                    self.db.put(key, value)
